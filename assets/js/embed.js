...
<link rel="stylesheet" href="https://braze-inc.github.io/embed-like-gist/github.min.css">
<style>
...
/* braze docs overrides */
.emgithub-container table, .emgithub-container table td {
...
`);
  ...
  Promise.all(showLineNumbers ? [fetchFile, loadHLJS, loadHLJSNum] : [fetchFile, loadHLJS]).then((result) => {
    ...
    const doneObserver = new MutationObserver((mutations) => {
      setAdaTableRole();
      doneObserver.disconnect();
    });
    doneObserver.observe(targetDiv, {childList: true, subtree: true });
  }).catch((error) => {
...
function embedCodeToTarget(targetDiv, codeText, showBorder, showLineNumbers, showFileMeta, showCopy, isDarkStyle, fileURL, rawFileURL, lang, startLine, endLine, tabSize, serviceProvider) {
  ...
  code.style.padding = "2rem 1rem 0";
...
delivered <span class="hide-in-phone">with <i class="fa-solid fa-heart"></i> </span>by <a target="_blank" href="${serviceProvider}">EmGithub</a>`;
