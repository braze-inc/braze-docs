#!/usr/bin/env python3
"""Second pass: broaden 'you need to' -> 'you must' for remaining edge cases (followed by punctuation/brackets).
Skips protected spans same as pass1.
Appends to report _reports/you-need-to-replacements-pass2.csv.
"""
...
REPORT = Path('_reports/you-need-to-replacements-pass2.csv')
...
PATTERN = re.compile(r'\b([Yy]ou) need to\b')
...
def main():
    ...
    for f in files:
        ...
        if changes:
            ...
            for off, orig, repl in changes:
                line = txt[:off].count('\n')+1
                rows.append([str(f), line, orig, repl, 'you-need-to-pass2'])
    ...
    with REPORT.open('w', encoding='utf-8', newline='') as csvf:
        ...
        for r in rows:
            w.writerow(r)
    print(f"Pass2 replacements: {len(rows)} in {modified} files. Report: {REPORT}")
