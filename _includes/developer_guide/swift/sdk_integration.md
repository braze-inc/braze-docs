...
{% alert note %}
By default, SwiftUI projects are generated without an `AppDelegate`. The following setup assumes you have an existing `AppDelegate` in your project. If you do not wish to create one, ensure that the Braze SDK is initialized at the earliest point in your app run (i.e. at launch).
{% endalert %}
