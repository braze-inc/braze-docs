...
{% alert note %}
**Custom domains for Web SDK requests aren't supported**: The Web SDK `baseUrl` must be a Braze SDK endpoint (for example, `sdk.iad-05.braze.com`). Braze does not support routing Web SDK traffic through a customer-owned domain via CNAME records. If you need Web SDK requests to originate from your own domain, the current option is to implement a reverse proxy in your infrastructure.
{% endalert %}
