...
<div class="braze-tutorial-intro">
  <p>This tutorial walks you through customizing in-app message styling on the web using key-value pairs from the Braze dashboard. Each step covers one part of the flow: turning off automatic display, subscribing for messages, reading extras, and choosing between the default UI and your own layout. The last section shows the full example next to a preview of a styled slideup message.</p>
</div>

<div class="braze-learn-box">
  <h4>You will learn how to</h4>
  <ul>
    <li>Stop using automatic in-app message display when you need custom handling</li>
    <li>Enable SDK logging during development (optional)</li>
    <li>Subscribe to in-app messages with <code>subscribeToInAppMessage</code></li>
    <li>Read styling and metadata from <code>message.extras</code> (all string values)</li>
    <li>Call <code>showInAppMessage</code> for the default UI or render the message yourself</li>
  </ul>
</div>

### Step 1: Skip automatic in-app message display

If you call `braze.automaticallyShowInAppMessages()`, the SDK shows every eligible message with the built-in UI. That runs before your custom logic can run, so remove any existing calls when you handle messages yourself.

Start with an import. Do not add `automaticallyShowInAppMessages()` to your bundle:

```javascript
import * as braze from "@braze/web-sdk";
```

Keep the rest of your integration unchanged except for removing automatic display.

### Step 2: Enable logging during development (optional)
...
Verbose logging helps confirm that messages are received and that your subscriber runs in the order you expect.

### Step 3: Subscribe to the in-app message callback
...
Register this callback after `initialize()` and before you rely on any messages firing, so you don't miss deliveries during the session.

### Step 4: Read key-value pairs from `message.extras`

Use the `extras` object on the message for values you configure in the dashboard (for example, template type, colors, or IDs). Every value is a string; cast or parse if you need numbers or booleans.
...
Key names and semantics are up to your team; align them with what marketers set on each campaign or Canvas step.

### Step 5: Show the default UI or your own layout

When `extras` indicate a custom template (or any rule you define), update the DOM or mount your own component. Otherwise, hand the message back to Braze with `showInAppMessage()` so the standard UI appears.
...
If you omit `showInAppMessage()` for messages that should use Braze's UI, those messages won't appear unless you render them yourself.

### Putting it all together

This example wires initialization, logging, subscription, extras, and the branch between custom handling and `showInAppMessage()`. The preview shows a web app with a navigation bar and a bottom slideup styled like a message driven by key-value data (template label, color, and ID). Remove `enableLogging` before you ship to production.
...
<div class="braze-tutorial-sandbox">
  ...
  <div class="braze-tutorial-sandbox-body">
    <div class="braze-tutorial-editor" id="sandbox-web-iam-full">
...
braze.subscribeToInAppMessage(function (message) {
  ...
  if (customTemplateType) {
    ...
    <div class="braze-tutorial-preview">
      ...
      <iframe id="braze-preview-web-iam-full" class="braze-tutorial-iframe" title="In-app message slideup preview"></iframe>
    </div>
  </div>
</div>
...
(function() {
  function initBrazeTutorial() {
    ...
    var slideupBg = '#0d9488';
    var iamFullPreview = '<!DOCTYPE html><html><head><style>' + sharedStyles +
      '.app-wrap { position: relative; min-height: 260px; padding-bottom: 120px; }' +
      '.iam-slideup { position: fixed; left: 0; right: 0; bottom: 0; padding: 16px 20px 20px; border-radius: 12px 12px 0 0; box-shadow: 0 -4px 24px rgba(27,37,54,0.18); color: #fff; background: linear-gradient(135deg, ' + slideupBg + ' 0%, #0f766e 100%); }' +
      ...
      '<p class="text-muted">Key-value pairs such as <code>custom-template</code>, <code>custom-color</code>, and <code>message-id</code> can drive your own slideup layout instead of the default Braze UI.</p>' +
      '</div>' +
      '<div class="iam-slideup">' +
      '<div class="iam-slideup-head">In-app message · slideup</div>' +
      '<div class="iam-slideup-title">Flash sale ends tonight</div>' +
      '<p class="iam-slideup-meta">Message ID: promo-iam-42 · Accent from extras: ' + slideupBg + '</p>' +
      '<a href="javascript:void(0)" class="iam-slideup-cta">Shop the sale</a>' +
      '</div>' +
      '</body></html>';

    var previews = {
      'braze-preview-web-iam-full': iamFullPreview
    };
