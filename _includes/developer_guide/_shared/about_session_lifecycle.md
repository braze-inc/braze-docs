...
If your app goes to the background, `UIApplicationDidEnterBackgroundNotification` will be triggered. The app does not remain in an active session while in the background. When your app returns to the foreground, the SDK checks the time elapsed since the session started (unless you [change the default session timeout]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/?tab=swift#change-session-timeout)). If this elapsed time exceeds the timeout period, a new session will begin.
{% endtab %}
{% endtabs %}