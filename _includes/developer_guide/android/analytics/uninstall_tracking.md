## About uninstall tracking with FCM

Uninstall tracking uses a silent push from Firebase Cloud Messaging (FCM) to detect uninstalled devices. Braze will intelligently drop the uninstall tracking notification and not wake up any custom push callbacks in your app with the regular silent push intent. This article covers how to configure uninstall tracking for your Android or FireOS application.

## Tracking uninstalls

To detect if the push notification is uninstall-tracking yourself, use [`isUninstallTrackingPush()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.models.push/-braze-notification-payload/is-uninstall-tracking-push.html).
