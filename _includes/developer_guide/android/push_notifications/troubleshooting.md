...
If you implemented a custom receiver, check that you called the appropriate method (for example, `BrazeNotificationUtils.routeUserWithNotificationOpenedIntent(context, intent)`) when the push intent is broadcasted. If the incorrect method is called, the push click analytics event is not logged.
