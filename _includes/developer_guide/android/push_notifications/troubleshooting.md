...
If you implemented a custom `BroadcastReceiver`, check that you called the appropriate method (for example, `Braze.getInstance().logPushNotificationOpened()`) when the push intent is broadcasted. If the incorrect method is called, the push click analytics event is not logged.
