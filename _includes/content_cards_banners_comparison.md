...
| **Canvas support** | Yes | No |
| **Content updates** | Dynamic Liquid becomes **static**, meaning it is set when sent and does not change. | Dynamic content **refreshes** every time a Banner is requested. |
...
- Trigger messages from an API or based on your users' actions.
...
- Allows content and personalization to update automatically every time a Banner is requested.
- Manage message placement and priority within your app.

You can also use both Content Cards and Banners. For example, you might use Banners for homepage promotions that need to update dynamically, while using Content Cards for a notification center where users can review past offers and updates.
