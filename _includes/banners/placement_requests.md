...
Each call to `requestBannersRefresh` consumes one token. If you attempt a refresh when no tokens are available, the SDK doesn't make the request and logs an error until a token refills. This mechanism is important for mid-session and event-triggered updates. To implement dynamic updates (e.g., after a user completes an action on a same page), call the refresh method after the custom event is logged, but be mindful of the necessary delay for Braze to ingest and process the event before the user qualifies for a different Banner campaign.
