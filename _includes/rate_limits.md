...
Starting on October 28th, 2024, we apply a base speed limit of 3,000 requests per three seconds to this endpoint for all customers who have data points. The limit is 225 requests per three seconds for customers who don't have data points. Each `/users/track` request can contain up to 75 event objects, 75 attribute objects, and 75 purchase objects. Each object (event, attribute, and purchase arrays) can update one user each. In total, this means you can update up to 225 users in a single call. In addition, you can update a single user profile with multiple objects.
