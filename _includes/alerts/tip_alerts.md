...
{% alert tip %}
You don't need a Context step to reference properties from the triggering event in [Audience Paths]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/audience_paths) or [Decision Split]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/decision_split) steps. You can reference the properties directly in the filter groups without Liquid by using the **Context Variable** filter. Make sure to select the correct data type.
{% endalert %}
