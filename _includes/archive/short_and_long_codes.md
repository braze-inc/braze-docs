...
| Pros | Cons |
| ---- | ---- | 
| {::nomarkdown} <ul> <li> Must be registered before sending. </li> </ul> {:/} | {::nomarkdown} <ul> <li> Toll-free numbers are only supported in the US and Canada </li><li> MMS is not enabled by all service providers </li> </ul> {:/} |
{: .reset-td-br-1 .reset-td-br-2} 
