...
[As propriedades de entrada do Canvas]({{site.baseurl}}/api/objects_filters/context_object/) são as propriedades que você mapeia para Canvas que são baseadas em ações ou disparadas por API. Note que o objeto `canvas_entry_properties` tem um limite máximo de tamanho de 50 KB.
