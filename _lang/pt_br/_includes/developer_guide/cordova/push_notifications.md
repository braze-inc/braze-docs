...
Por padrão, o SDK Braze Cordova não lida automaticamente com links profundos de notificações por push. Para ativar o deep linking por push, siga os passos de configuração em [Deep linking]({{site.baseurl}}/developer_guide/cordova/deep_linking/).
Para mais detalhes sobre essas e outras opções de configuração de push, veja [Configurações opcionais]({{site.baseurl}}/developer_guide/sdk_integration?sdktab=cordova#optional).

## Desativando notificações por push básicas (apenas iOS)
