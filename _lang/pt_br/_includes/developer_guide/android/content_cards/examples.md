...
{% alert important %}
Está procurando o guia básico de integração do desenvolvedor do cartão de conteúdo? Encontre-o [aqui]({{site.baseurl}}/developer_guide/content_cards/?sdktab=android).<br><br>Mais informações sobre a personalização dos cartões de conteúdo podem ser encontradas no [Guia de personalização.]({{site.baseurl}}/developer_guide/content_cards/)
{% endalert %}
...
Ao criar cartões de conteúdo, você deve expor o SDK da Braze por meio de um único singleton do gerenciador. Esse padrão protege o código do seu aplicativo dos detalhes de implementação do Braze por meio de uma abstração compartilhada que faz sentido para o seu caso de uso. Isso também facilita o rastreamento, a depuração e a alteração do código. Para ver um exemplo de implementação de gerenciador, consulte [`BrazeManager.kt`](https://github.com/braze-inc/braze-growth-shares-android-demo-app/blob/main/app/src/main/java/com/braze/advancedsamples/BrazeManager.kt) em nosso app de demonstração no GitHub.

### Cartões de conteúdo como objetos personalizados

É possível estender os objetos personalizados já existentes em seu app para armazenar dados do cartão de conteúdo, de modo que você possa abstrair sua fonte de dados em um formato já compatível com seu código. Isso lhe dá flexibilidade para trabalhar com diferentes back-ends de dados de forma intercambiável e em conjunto.

No exemplo a seguir, definimos a classe base abstrata `ContentCardable` para representar nossos dados existentes (alimentados por um arquivo JSON local) e os novos dados (alimentados pelo Braze SDK). A classe base também expõe os dados brutos do cartão de conteúdo para os consumidores que precisam acessar a implementação original do `Card`.
...
Depois de explorar essas considerações de código, confira nossos [casos de uso](#sample-use-cases) para que você possa começar a implementar seus próprios objetos personalizados.

{% tabs local %}
{% tab No Card Dependencies %}
{% subtabs global %}
{% subtab Kotlin %}
**Não há dependências do site `Card` **<br>
