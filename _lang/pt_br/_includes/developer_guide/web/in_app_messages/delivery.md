...
Para mais informações sobre a semântica de início de sessão do SDK, veja [Ciclo de Vida da Sessão]({{site.baseurl}}/developer_guide/platform_integration_guides/analytics/tracking_sessions/).

### Limites de taxa

Por padrão, você pode enviar uma mensagem no app uma vez a cada 30 segundos.

Para substituir isso, adicione a seguinte propriedade à sua configuração do Braze—antes que a instância do Braze seja inicializada. Você pode definir como qualquer número inteiro positivo, que representa o intervalo mínimo de tempo em segundos. Por exemplo:
...
Remova a chamada para `braze.automaticallyShowInAppMessages()` dentro do seu snippet de carregamento, e então crie uma lógica personalizada para lidar com a exibição ou não exibição de mensagens no app.
...
{% alert important %}
Se você não remover `braze.automaticallyShowInAppMessages()` do seu site, então chamar `braze.showInAppMessage`, a mensagem pode ser exibida várias vezes.
{% endalert %}
...
## Acionando mensagens de intenção de saída
