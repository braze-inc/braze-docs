...
{% alert tip %}
Para puxar imagens para itens de disparo de catálogo, seu catálogo deve incluir um campo chamado `image_url`. Você pode então referenciá-lo usando {%raw%}``{{ items[0].image_url }}``{%endraw%}.
{% endalert %}
