...
Para obter etapas sobre como configurar eventos personalizados para uma plataforma específica, consulte o Guia de integração de plataformas no [Guia do desenvolvedor]({{site.baseurl}}/developer_guide/home/). Consulte o artigo relevante com base em sua plataforma:
...
Para atualizar somente os perfis de usuário existentes no Braze, você deve passar a chave `_update_existing_only` com o valor `true` no corpo da solicitação. Se esse valor for omitido, a Braze criará um novo perfil de usuário se o `external_id` ainda não existir.
...
| Datetimes | Deve ser formatado como cadeias de caracteres no formato [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) ou em qualquer um dos seguintes formatos: <br>- `yyyy-MM-ddTHH:mm:ss:SSSZ` <br>- `yyyy-MM-ddTHH:mm:ss` <br>- `yyyy-MM-dd HH:mm:ss` <br>- `yyyy-MM-dd` <br>- `MM/dd/yyyy` <br>- `ddd MM dd HH:mm:ss.TZD YYYY` <br><br>Não é compatível com matrizes. <br><br>Note que "T" é um designador de tempo, não um espaço reservado, e não deve ser alterado ou removido. <br><br>Os atributos de horário sem um fuso horário terão como padrão a meia-noite UTC (e serão formatados no painel como o equivalente à meia-noite UTC no fuso horário da empresa). <br><br> Os eventos com registros de data e hora no futuro terão como padrão a hora atual.  |
...
{% alert important %}
O uso de chaves reservadas como nomes de propriedades de eventos personalizados resultará em erros de API ao enviar solicitações para o ponto de extremidade `/users/track`.
{% endalert %}
