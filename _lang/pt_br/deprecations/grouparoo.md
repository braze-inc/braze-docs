...
nav_title: Grouparoo
page_order: 1
description: "Este artigo descreve a parceria entre o Braze e o Grouparoo, uma ferramenta de ETL reversa de código aberto usada para alimentar ferramentas de marketing, vendas e suporte com dados do seu data warehouse."
page_type: update
...
> [Grouparoo](https://www.grouparoo.com/) é uma ferramenta de ETL reversa de código aberto que sincroniza dados do seu data warehouse para ferramentas de marketing, vendas e suporte. Sua interface centrada em modelos permite que membros da equipe não técnicos configurem e agendem sincronizações de dados.

A integração entre Braze e Grouparoo sincroniza dados do data warehouse com o Braze. Agendas de sincronização automáticas mantêm as comunicações com os clientes atualizadas com informações recentes.
...
- **Sincronização**: Adicione, atualize e remova usuários da empresa conforme necessário. Essa opção procura novos registros, alterações em registros existentes e exclusões.
- **Aditivo**: Adicione e atualize usuários da empresa conforme necessário, mas não remova ninguém. Essa opção procura novos usuários para adicionar ao Braze e alterações nos usuários existentes da empresa, mas não controla as exclusões.
...
![Exemplo de campos de mapeamento de propriedades. O userID do Grouparoo é configurado para mapear para external_id. email, firstName e lastName são definidos como equivalentes aos campos "email", "first_name", e "last_name" do grouparoo.]({% image_buster /assets/img/grouparoo/mapping.png %}){: style="max-width:80%;"}
...
![Em "Braze Subscription Groups" (Grupos de inscrição Braze) na janela de configuração de destino do Grouparoo, o grupo Grouparoo "High value with recent automotive purchase" (Alto valor com compra automotiva recente) será adicionado ao grupo de inscrições Braze "High value with recent automotive purchase" (Alto valor com compra automotiva recente).]({% image_buster /assets/img/grouparoo/lists.png %}){: style="max-width:80%;"}
