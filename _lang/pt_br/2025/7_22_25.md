...
A Braze coleta informações de visitantes, como endereços de e-mail e números de telefone, através de mensagens no navegador. Essas informações são então enviadas para a Shopify. Esses dados ajudam os comerciantes a reconhecer visitantes em sua loja e criar uma experiência de compra mais personalizada.

### Okendo — eCommerce

A integração da Braze e [Okendo]({{site.baseurl}}/partners/okendo/) funciona em vários produtos na plataforma da Okendo, incluindo Avaliações, Fidelidade, Referências, Pesquisas e Questionários. Okendo envia eventos personalizados e atributos de usuário para a Braze, que podem ser usados para personalizar e acionar mensagens.

### Lemnisk - Plataforma de Dados do Cliente

A integração da Braze com [Lemnisk]({{site.baseurl}}/partners/lemnisk/) permite que marcas e empresas desbloqueiem todo o potencial da Braze, atuando como uma camada de inteligência liderada por CDP que unifica os dados do usuário em tempo real entre plataformas, enviando as informações e comportamentos do usuário coletados para a Braze em tempo real.
...
- [Web SDK 6.0.0](https://github.com/braze-inc/braze-web-sdk/blob/master/CHANGELOG.md)
    - Removido o método `Banner.html`, `logBannerClick` e `logBannerImpressions`. Em vez disso, use [`insertBanner`](https://js.appboycdn.com/web-sdk/latest/doc/modules/braze.html#insertbanner), que gerencia automaticamente o rastreamento de impressões e cliques.
    ...
    - Os campos created e categories que eram usados por cartões de Feed de Notícias legados foram removidos das subclasses [`Card`](https://js.appboycdn.com/web-sdk/latest/doc/classes/braze.card.html).
    - O campo linkText também foi removido da subclasse Card [`ImageOnly`](https://js.appboycdn.com/web-sdk/latest/doc/classes/braze.imageonly.html) e seu construtor.
    - Esclarecidas definições e tipos atualizados para notar que certos métodos do SDK retornam explicitamente undefined quando o SDK não está inicializado, alinhando os tipos com o comportamento real em tempo de execução. Isso pode introduzir novos erros de TypeScript em projetos que dependiam dos tipos anteriores (incompletos).
    - As imagens de mensagens in-app com `cropType` de `CENTER_CROP` (como `FullScreenMessage` por padrão) agora são renderizadas via uma tag `<img>` em vez de `<span>` para melhorar a acessibilidade. Isso pode quebrar personalizações CSS existentes para a classe `.ab-center-cropped-img` ou seus filhos.
- [Cordova SDK 13.0.0](https://github.com/braze-inc/braze-cordova-sdk/blob/master/CHANGELOG.md#1300)
    ...
    - Atualizamos a ponte nativa do Android [do SDK da Braze para Android 36.0.0 para 37.0.0](https://github.com/braze-inc/braze-android-sdk/compare/v36.0.0...v37.0.0#diff-06572a96a58dc510037d5efa622f9bec8519bc1beab13c9f251e97e657a9d4ed).
