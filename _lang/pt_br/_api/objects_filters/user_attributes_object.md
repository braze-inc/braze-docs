...
{% alert note %}
Ao importar tokens por push de outros sistemas, um `external_id` nem sempre está disponível. Para manter a comunicação com esses usuários durante a transição para a Braze, é possível importar os tokens legados para usuários anônimos sem fornecer `external_id` especificando `push_token_import` como `true`.
{% endalert %}
...
Após a importação, quando o usuário anônimo inicia a versão do seu app habilitada para Braze, a Braze automaticamente move seu token por push importado para seu perfil de usuário Braze e limpa o perfil temporário.
