...
Se você especificar `identified` no array, isso significaria que o usuário **must** ter um `external_id` para ser inserido no Canvas. Se você quiser que usuários com endereços de e-mail entrem na mensagem, independentemente de estarem identificados ou não, use apenas o parâmetro `most_recently_updated` ou `least_recently_updated`.
