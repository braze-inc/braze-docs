...
É altamente recomendável configurar suas próprias credenciais do S3 ou do Azure (em **Parceiros de tecnologia**) ao usar esse endpoint para aplicar suas próprias políticas de bucket na exportação. Se não tiver fornecido suas credenciais de armazenamento em nuvem, a resposta à solicitação fornecerá o URL onde um ZIP contendo todos os arquivos do usuário pode ser baixado. O URL só se tornará um local válido depois que a exportação estiver pronta.
