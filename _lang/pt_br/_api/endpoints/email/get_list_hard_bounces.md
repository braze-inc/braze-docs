...
| ----------|-----------| ----------|----- |
| `start_date` | Opcional* | String no formato YYYY-MM-DD| \*Um de `start_date` ou `email` é necessário. Esta é a data de início do intervalo para recuperar os hard bounces e deve ser anterior a `end_date`. Isso é tratado como meia-noite no horário UTC pela API. |
...
| `email` | Opcional* | String | \*Um de `start_date` ou `email` é necessário. Se fornecido, retornaremos se o usuário teve hard bounce ou não. Verifique se as strings de e-mail estão formatadas corretamente. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }

{% alert important %}
Você deve fornecer um `end_date`, e ou um `email` ou um `start_date`. Se você fornecer todos os três, `start_date`, `end_date`, e um `email`, priorizaremos os e-mails fornecidos e desconsideraremos o intervalo de datas.
{% endalert %}
