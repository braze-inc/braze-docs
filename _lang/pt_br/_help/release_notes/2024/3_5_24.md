...
É possível usar [o arquivamento de mensagens]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/message_archiving/) para salvar uma cópia das mensagens enviadas aos usuários para fins de arquivamento ou conformidade no bucket S3 da AWS, no contêiner do Azure Blob Storage ou no bucket do Google Cloud Storage.
...
Para sempre [atribuir caminhos aleatórios a]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/experiment_step) uma etapa da jornada experimental, selecione **Jornadas aleatórias em Jornadas experimentais** na etapa. Essa opção não está disponível ao usar as jornadas vencedoras ou personalizadas.
