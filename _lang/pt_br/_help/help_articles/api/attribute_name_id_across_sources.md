...
| ID da campanha | {% raw %}`{{campaign.${api_id}}}`{% endraw %} | N/A (usado como entrada para a própria chamada da API) | id_da_campanha |
...
| ID do canva | {% raw %}`{{canvas.${api_id}}}`{% endraw %} | N/A (usado como entrada para a própria chamada da API) | canva_id |
