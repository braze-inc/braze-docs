...
Agora você pode [adicionar atributos de HTML]({{site.baseurl}}/user_guide/message_building_by_channel/email/drag_and_drop/#creative-details/) a qualquer URL nos blocos de editor `Image`, `Button` ou `Text` no editor de arrastar e soltar para e-mail. Com os atributos personalizados, você pode facilmente anexar informações adicionais às tags HTML nos e-mails. Isso pode ser especialmente útil para personalização, segmentação e estilização de mensagens. 
