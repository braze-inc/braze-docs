...
Disponível para todos os clusters, um novo processo de exportação de público do Facebook Braze simplifica o fluxo de trabalho com etapas de integração claras. Você não precisa mais colocar os URIs do OAuth Redirect na lista de permissões para enviar públicos personalizados ou ajustar as configurações do aplicativo do Facebook para fazer a integração.
...
![Endereço personalizado de origem]({% image_buster /assets/img/personalized-from-name.png %}){: style="max-width:80%"}
