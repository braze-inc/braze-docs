...
Se `content-available` não estiver definido, a prioridade padrão do serviço de notificação por push da Apple (APNs) é 10. Se `content-available` estiver definido, essa prioridade é 5. Consulte [Apple push object]({{site.baseurl}}/api/objects_filters/messaging/apple_object) para mais detalhes. 
...
| `notification` | Opcional | Objeto | Inclua um [`apple_push`]({{site.baseurl}}/api/objects_filters/messaging/apple_object/) objeto para definir uma notificação por push. O comportamento dessa notificação por push depende se o usuário está ativo ou se o usuário está usando um dispositivo proxy. {::nomarkdown}<ul><li>Se uma <code>notificação</code> estiver incluída e o usuário estiver ativo em seu iPhone quando a atualização for entregue, a interface de Atividade ao Vivo atualizada deslizará para baixo e será exibida como uma notificação por push.</li><li>Se uma <code>notificação</code> estiver incluída e o usuário não estiver ativo em seu iPhone, sua tela acenderá para exibir a interface de Atividade ao Vivo atualizada em sua tela de bloqueio.</li><li>O <code>alerta de notificação</code> não será exibido como uma notificação por push padrão. Além disso, se um usuário tiver um dispositivo proxy, como um Apple Watch, o <code>alerta</code> será exibido lá.</li></ul>{:/} |
