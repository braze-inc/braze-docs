...
{% alert tip %}
Se você estiver interessado em enviar mensagens imediatamente para usuários designados, use o [`/messages/send` endpoint]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages) em vez disso.
{% endalert %}
...
| --------- | ---------| --------- | ----------- |
|`broadcast`| Opcional | Booleano | Você deve definir `broadcast` como verdadeiro ao enviar uma mensagem para um segmento inteiro que uma campanha ou canva segmenta. Este parâmetro tem como padrão `false`. <br><br> Se `broadcast` estiver definido como `true`, uma lista de destinatários não pode ser incluída. No entanto, tenha cuidado ao definir `broadcast: true`, pois definir essa flag inadvertidamente pode fazer com que você envie sua mensagem para um público maior do que o esperado. |
...
| `override_messaging_limits` | Opcional | Booleano | Ignorar limitação de frequência para campanhas, padrão é falso. |
