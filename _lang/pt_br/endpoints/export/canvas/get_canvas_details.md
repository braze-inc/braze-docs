...
| `post_launch_draft_version` | Opcional | Booleano | Para Canvases que têm um rascunho pós-lançamento, definir isso como `true` mostrará todas as alterações de rascunho disponíveis. O padrão é `false` |
...
{% alert note %}
Todas as etapas do canva têm um campo `next_paths`, que é uma matriz de dados `{name, next_step_id}`. Para as etapas de Mensagem, o campo `next_step_ids` estará presente, mas não conterá dados para outras etapas do Canvas.
{% endalert %}
