...
Se você adicionou suas credenciais [do S3]({{site.baseurl}}/partners/data_and_infrastructure_agility/cloud_storage/amazon_s3) ou [do Azure]({{site.baseurl}}/partners/data_and_infrastructure_agility/cloud_storage/microsoft_azure_blob_storage_for_currents/) ao Braze por meio da respectiva página de **parceiros de tecnologia**, cada arquivo será carregado em seu bucket como um arquivo ZIP com o formato de chave semelhante a `segment-export/SEGMENT_ID/YYYY-MM-dd/RANDOM_UUID-TIMESTAMP_WHEN_EXPORT_STARTED/filename.zip`. Se estiver usando o Azure, certifique-se de que a caixa **Tornar este o destino padrão de exportação de dados** esteja marcada na página de visão geral do parceiro do Azure no Braze. 
...
Se não tiver fornecido suas credenciais de armazenamento em nuvem, a resposta à solicitação fornecerá o URL onde um ZIP contendo todos os arquivos do usuário pode ser baixado. O URL só se tornará um local válido depois que a exportação estiver pronta.
