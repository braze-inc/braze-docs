...
- Resumos do app
- Last_X_at (o Braze atualizará os campos se os campos do perfil órfão forem mais recentes)
...
{% alert important %}
É necessário um dos seguintes dados: `aliases_to_identify`, `emails_to_identify`, ou `phone_numbers_to_identify` por solicitação. Por exemplo, você pode usar esse endpoint para identificar usuários por e-mail usando `emails_to_identify` na sua solicitação.
{% endalert %}
...
Se você especificar `identified` na matriz, isso significaria que o usuário **deve** ter um `external_id` para ser inserido no Canvas. Se quiser que os usuários com endereços de e-mail insiram a mensagem, independentemente de estarem identificados ou não, use apenas o parâmetro `most_recently_updated` ou `least_recently_updated`.
