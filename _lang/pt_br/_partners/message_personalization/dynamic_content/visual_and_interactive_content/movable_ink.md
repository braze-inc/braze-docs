...
|| Vídeo | ✔* | ✔* | \* Apenas GIFs animados. <br>Para Android, a Braze requer [suporte a GIF]({{site.baseurl}}/developer_guide/platform_integration_guides/android/in-app_messaging/customization/gifs/) na implementação |
...
Antes de finalizar sua campanha, certifique-se de visualizar as imagens dinâmicas e testar os parâmetros de consulta para ver como as imagens aparecerão ao serem visualizadas. Quando completo, uma URL dinâmica será gerada que pode ser inserida na Braze!
...
{% tabs local %}
{% tab Email %}
Na plataforma Braze, cole sua tag criativa no corpo do seu e-mail.![]({% image_buster /assets/img/movable_ink/web2.png %}){: style="max-width:90%"}<br><br>
...
1. Na plataforma Braze, cole a URL no campo **Rich Notification Media**.![]({% image_buster /assets/img/movable_ink/image.png %}){: style="max-width:60%"}<br><br>
...
```
{% endraw %}
Esse modelo pega o horário atual (em segundos), anexa-o ao fim da guia da imagem da Movable Ink (como parâmetro de consulta) e depois calcula o resultado final. Você pode visualizá-lo com a aba **Teste**—isso avaliará o código e mostrará uma prévia.

**3\.** Por fim, reavalie a inscrição no segmento. Para fazer isso, ative a opção `Re-evaluate audience membership and liquid at send-time` localizada na etapa **Públicos-alvo** de uma campanha. Se esta opção não estiver disponível, entre em contato com seu gerente de sucesso do cliente ou suporte da Braze. Esta opção instruirá os SDKs do Braze a solicitar novamente a campanha, fornecendo uma URL única cada vez que uma mensagem no app for acionada.
...
- **Mensagens no app e cartões de conteúdo**: confirme se o URL da imagem é exclusivo para cada impressão. Isso pode ser feito anexando o Liquid apropriado para que cada URL seja diferente. Veja [instruções de mensagens no app e cartões de conteúdo]({{site.baseurl}}/partners/message_personalization/dynamic_content/visual_and_interactive_content/movable_ink/#step-4-braze-experience). 
...
- O Android requer suporte a GIFs na implementação. Siga o artigo de [personalização de mensagens no app Android]({{site.baseurl}}/developer_guide/platform_integration_guides/android/in-app_messaging/customization/gifs/) se você não tiver essa configuração.
