...
> Você pode sincronizar seus produtos da sua loja da Shopify em um [catálogo]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs) da Braze, automatizando a importação dos dados do produto para uma personalização mais profunda de suas mensagens. 
...
![Etapa 3 do processo de configuração com "Shopify Variant ID" como o "Catalog product identifier" (Identificador de produto do catálogo).][1]{: style="max-width:70%;"}
...
1. Acessar a página de integração do Shopify > **Editar configuração**.<br><br>![Página de integração do Shopify.]({% image_buster /assets/img/Shopify/edit_config.png %})

{: start="2"}
2\. Na etapa **Sync catalog**, desative o catálogo e, em seguida, atualize as configurações.
3\. Ative o catálogo e atualize as configurações.<br><br>![Shopify "Sincronizar catálogo" etapa com alternância de catálogo.]({% image_buster /assets/img/Shopify/catalog_toggle.png %})
...
Para configurar notificações de queda de preço, siga as etapas [aqui]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/price_drop_notifications).
...
Para alterar o identificador de produto de seu catálogo da Shopify, você precisará desativar a sincronização. Primeiro confirme que você interrompeu todos os envios que usam os dados do catálogo da Shopify. Execute novamente a sincronização inicial do catálogo da Shopify e selecione o identificador de produto desejado seguindo as etapas [de sincronização do produto](#setting-up).

## Desativar a sincronização do produto {#deactivate}

A desativação do recurso de sincronização de produtos da Shopify excluirá o catálogo completo e os produtos. Isso também pode afetar quaisquer envios que possam estar usando ativamente os dados do produto desse catálogo. Confirme se você atualizou ou pausou esses envios antes da desativação, pois isso pode resultar no envio de mensagens com detalhes de produtos ausentes. Não exclua o catálogo da Shopify diretamente na página de catálogos.
