...
> A [Olo][1] é uma plataforma SaaS aberta líder para restaurantes que permite hospitalidade em cada ponto de contato.
...
Use a ferramenta de webhooks [self-service][2] no dashboard da Olo para configurar webhooks para enviar para sua Transformação de Dados.
...
{:start="3"}
3\. Verifique se o webhook está configurado corretamente enviando um [Evento de Teste][3] para sua Transformação de Dados. Somente os usuários do dashboard Olo com a [permissão de Ferramentas de Desenvolvedor][4] podem enviar Eventos de Teste.
...
Depois de ativar sua Transformação de Dados no Braze, use a [ferramenta de webhooks de autoatendimento][2] no dashboard do Olo para publicar seu webhook. Quando o webhook for publicado, a transformação de dados começará a receber mensagens de eventos do webhook da Olo.
