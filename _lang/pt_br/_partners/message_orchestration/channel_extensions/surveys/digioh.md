...
!["Crie centrais de preferências de e-mail e comunicações flexíveis com a Digioh"][5]{: style="border:0"}
...
| Endpoint da Braze API `/users/track/`  | Seu URL de endpoint REST com os detalhes de `/users/track/` anexados a ele. Seu endpoint dependerá do [URL da Braze para sua instância][6].<br><br>Por exemplo, se seu endpoint da API REST for `https://rest.iad-01.braze.com`, seu endpoint `/users/track/` será `https://rest.iad-01.braze.com/users/track/`. |
...
!["Selecione a integração correta no menu suspenso"][2]{: style="max-width:50%;"}
...
!["Adicionar a integração a uma caixa de luz"][3]{: style="max-width:90%"}
