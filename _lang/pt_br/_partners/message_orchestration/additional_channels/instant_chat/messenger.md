...
Quando tiver certeza de que está recebendo PSIDs, coordene e compartilhe isso com seus desenvolvedores para enviar os PSIDs ao Braze como um [atributo personalizado]({{site.baseurl}}/user_guide/Data_and_Analytics/Custom_Data/Custom_Attributes/#custom-attributes). PSIDs são strings que podem ser acessadas por meio de uma [chamada de API](https://developers.facebook.com/docs/messenger-platform/reference/send-api).
...
![Guia Teste no modelo de webhook do Facebook Messenger, mostrando que é possível fazer uma prévia da mensagem enviando-a para um usuário existente.][60]

Se a mensagem for recebida com êxito, você poderá definir as configurações de entrega.

## Usando esta integração

Depois de configurada, use essa integração para direcionar os usuários do Facebook Messenger. Se não estiver enviando mensagens usando os números de telefone dos usuários e planeja enviar mensagens do Messenger repetidamente, você deve [criar um segmento][62] para todos os usuários para os quais o Messenger ID existe como um atributo personalizado e ativar [análise de dados de rastreamento][61] para rastrear suas taxas de inscrição no Messenger ao longo do tempo. 

![Filtro de segmento "messenger_id" definido como "não está em branco".][63]
...
[61]: {{site.baseurl}}/user_guide/data_e_analytics/reporting/viewing_and_understanding_segment_data/
