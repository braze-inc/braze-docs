...
> A [Inkit][1] e a Braze ajudam organizações a gerar e distribuir documentos de forma segura, tanto em meio digital quanto por mala direta.

_Esta integração é mantida pelo Inkit._
...
![Uma seleção de modelos de webhook predefinidos disponíveis na guia "Modelos de webhook" da seção "Modelos e mídia".][7]
...
![O código do corpo da solicitação e o URL do webhook são exibidos na guia de composição do criador de webhooks do Braze.][5]
...
Seu texto bruto será automaticamente destacado se for uma tag Braze aplicável. As tags `street`, `unit`, `state` e `zip` devem ser configuradas como [atributos personalizados][3] para enviar esse Webhook.
