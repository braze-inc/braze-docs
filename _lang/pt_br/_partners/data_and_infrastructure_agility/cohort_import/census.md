...
> Este artigo descreve como fazer a importação de coortes de usuários do [Census][1] para o Braze. Para saber mais sobre a integração da Census, consulte o [artigo principal sobre a Census]({{site.baseurl}}/partners/data_and_infrastructure_agility/workflow_automation/census/).
...
3. Configure o local para onde o modelo será sincronizado. Selecione **Braze** como os destinos e **User & Coorte** como o objeto a ser sincronizado.<br>![No prompt "Select a Destination" (Selecione um destino), "Braze" é selecionada como a conexão, e vários objetos são listados.][10]{: style="max-width:80%;"}<br><br>
...
7. Por fim, mapeie os campos de dados do Census para os campos equivalentes do Braze.<br>![Mapeamento do Census][11]{: style="max-width:80%;"}<br><br>
