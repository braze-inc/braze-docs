...
![Um gráfico de visão geral da Tealium que mostra como os diferentes produtos da Tealium e a plataforma da Braze se encaixam para ativar campanhas entre canais em tempo real.][22]{: style="border:0;"}
...
- Sincronize o público do Tealium com o [AudienceStream]({{site.baseurl}}/partners/data_and_infrastructure_agility/customer_data_platform/tealium/tealium_audience_stream/) para o Braze para uso na personalização de campanhas e telas do Braze ou na criação de segmentos.
...
{% alert important %}
A Tealium oferece ações de conector em lote e sem lote. O conector não em lote deve ser usado quando solicitações em tempo real são importantes para o caso de uso e não há preocupações sobre atingir as especificações de limite de frequência da API Braze. Entre em contato com suporte da Braze ou com seu gerente de sucesso do cliente se tiver alguma dúvida.<br><br>
...
| ----------- | ------- |
| [Lado a lado](#side-by-side-sdk-integration) | Usa o SDK da Tealium para traduzir eventos nas chamadas nativas do Braze, permitindo acesso a recursos mais profundos e um uso mais abrangente do Braze do que a integração servidor a servidor.<br><br>Caso pretenda usar comandos remotos da Braze, observe que a Tealium não é compatível com todos os métodos da Braze (por exemplo, cartões de conteúdo). Para usar um método da Braze que não esteja mapeado por um comando remoto correspondente, será necessário invocar o método adicionando o código nativo da Braze à sua base de código.|
| [De servidor para servidor](#server-to-server-integration) | Encaminha dados da Tealium para os endpoints da API REST do Braze.<br><br>Não oferece suporte aos recursos da interface do usuário do Braze, como envio de mensagens no app, cartões de conteúdo ou notificações por push. Também existem dados capturados automaticamente, como campos em nível de dispositivo, que não estão disponíveis por meio desse método.<br><br>Considere uma integração lado a lado se quiser usar esses recursos.|
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
2. Localize a plataforma desejada nas categorias disponíveis e dê um nome à sua fonte; esse campo é obrigatório.<br>![][6]{: style="max-width:80%;margin-left:15px;margin-bottom:15px;"}
3. Nas opções de **Event Specifications (Especificações do evento** ), escolha as [especificações do evento](https://docs.tealium.com/server-side/event-specifications/about/) que você gostaria de incluir. As especificações de eventos o ajudam a identificar os nomes dos eventos e as atribuições necessárias para rastreamento em sua instalação. Essas especificações serão aplicadas aos eventos recebidos.<br>![][7]{: style="max-width:80%;margin-left:15px;margin-bottom:15px;"}<br>Considere quais dados são mais valiosos para você e quais especificações parecem mais apropriadas para o seu caso de uso. [Especificações de eventos personalizados][19] também estão disponíveis. <br>
...
5. Agora você poderá visualizar a fonte salva, bem como adicionar ou remover especificações de eventos. <br>![][18]{: style="max-width:80%;margin-left:15px;margin-bottom:15px;"}<br>Na visualização detalhada da fonte de dados, você pode executar as seguintes ações:
...
Seu conector agora é exibido na lista de conectores na página inicial da Tealium. <br>![][13]{: style="max-width:80%;"}
...
Consulte a documentação da Tealium [documentação do Trace][21] para obter instruções mais detalhadas sobre a implementação da ferramenta Trace da Tealium.
