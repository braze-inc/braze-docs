...
- [Importar dados entre as duas plataformas](#integration-options). 
...
| ----------- | ------- |
| [Lado a lado<br>(modo dispositivo)](#side-by-side-sdk-integration) |<br><br>Observe que a Segment não é compatível com todos os métodos da Braze (por exemplo, cartões de conteúdo). Para usar um método da Braze que não esteja mapeado por um mapeamento correspondente, será necessário invocar o método adicionando o código nativo da Braze à sua base de código. |
...
#### Integração de servidor para servidor
 ...
 Também existem dados [capturados automaticamente]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/#user-data-collection) (como usuários anônimos e campos no nível do dispositivo) que não estão disponíveis no modo de nuvem.
...
| Gerenciar o trabalhado de serviços externamente | Se você tiver seu próprio trabalho de serviço para registrar e controlar o ciclo de vida, defina essa opção como `TRUE`, e o Braze SDK não registrará nem cancelará o registro de um trabalho de serviço.  Quando essa opção é `TRUE`, a opção `serviceWorkerLocation` é irrelevante e é ignorada. |
