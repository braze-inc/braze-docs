...
| Fonte configurada | Uma [fonte][3] é essencialmente a origem de qualquer dado enviado à RudderStack, como sites, apps móveis ou servidores back-end. É necessário configurar a fonte antes de configurar o Braze como um destino no RudderStack. |
...
| Centro de dados | Seu data center está alinhado com seu dashboard da Braze [instância][15].  |
...
Para começar a enviar dados para a Braze, primeiro você precisa confirmar se há uma fonte configurada no seu app da RudderStack. Visite [RudderStack][22] para saber como configurar sua fonte de dados.
...
Configure os mapeamentos do SDK do RudderStack para sua plataforma no repositório da Braze no GitHub, conforme descrito em [métodos suportados](#supported-methods):
...
**Pedido concluído**<br>
Ao usar a [API de e-commerce do RudderStack][20] para chamar o método de rastreamento de um evento com o nome `Order Completed`, o RudderStack envia os produtos listados nesse evento para a Braze como [`purchases`][21].
...
{% alert note %}
Para as operações de atualização e remoção, `identifier` é uma chave obrigatória. Se as operações de adição, atualização ou remoção não estiverem presentes na matriz aninhada, o RudderStack usará a operação de criação para criar as propriedades por padrão. Consulte [Vetor de objetos]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/array_of_objects/) para saber mais sobre o envio de atributos personalizados aninhados.
{% endalert %}
...
[21]: {{site.baseurl}}/user_guide/data_e_analytics/export_braze_data/exporting_revenue_data/#revenue-data
...
[reagir]: https://github.com/rudderlabs/rudder-sdk-react-native/tree/develop/libs/rudder-integration-braze-react-native
