...
| Ponto de extremidade REST do Braze  | Sua URL de endpoint REST. Seu endpoint dependerá do [URL do Braze para sua instância][1]. |
...
No [dashboard do Fivetran](https://fivetran.com/dashboard), clique em **\+ Connector** e selecione o conector **Braze** para iniciar o formulário de configuração. Em seguida, selecione **Amazon S3**. Note o ID externo fornecido aqui; você precisará dele para permitir que o Fivetran acesse seu bucket S3. 
...
Em seguida, clique na guia **JSON** e cole a seguinte política. Substitua `{your-bucket-name}` pelo nome de seu bucket S3.
...
Por fim, clique em **Revisar política** e dê à política um nome e uma descrição exclusivos. Clique em **Create Policy (Criar política** ) para criar sua política. 
...
Em seguida, clique em **Next: Permissions (Permissões)** para selecionar a política que você acabou de criar.

![]({% image_buster /assets/img/fivetran_as3_select_policy.png %})

Clique em **Next: revisar** (Avançar: revisar), dê um nome à sua nova função (como "Fivetran") e clique em **Create Role** (Criar função). Depois que a função for criada, clique nela e note o ARN da função mostrado.
...
No Fivetran, clique em **\+ Connector** e selecione o conector **Braze** para iniciar o formulário de configuração. No formulário, preencha os campos fornecidos com os valores apropriados:
...
- `Bucket`: encontrado na sua conta Braze, em **Integração > Currents > [Nome do Curent] > Nome do bucket**.
...
Por fim, clique em **Save & Test (Salvar e testar)**, e o Fivetran fará o resto, sincronizando com os dados de sua conta Braze!
...
No [dashboard do Fivetran](https://fivetran.com/dashboard), clique em **\+ Connector** e selecione o conector **Braze** para iniciar o formulário de configuração. Em seguida, selecione **Google Cloud Storage**. Anote o endereço de e-mail que aparece.
...
Navegue até o [Google Storage Console](https://console.cloud.google.com/storage/browser), selecione o bucket com o qual você configurou o Braze Currents e clique em **Edit bucket permissions (Editar permissões do bucket**).

![Os compartimentos disponíveis no Console de armazenamento do Google. Localize um bucket e clique no símbolo vertical de três pontos para abrir o menu suspenso que permite editar as permissões do bucket.]({% image_buster /assets/img/fivetran_edit_bucket_permissions_gcs.png %})
...
- `API URL`: Seu endpoint da API REST da Braze.
- `API Key`: Sua chave da API REST do Braze. 
- `Bucket Name`: encontrado na sua conta Braze, em **Integração > Currents > [Nome do Curent] > Nome do bucket**.
- `Folder`: encontrado na sua conta Braze, em **Integração > Currents > [Nome do Curent] > Prefixo**.
