...
| Requisito | Descrição |
| --- | --- |
| Conta do Census | É necessário ter uma [conta Census][1] para aproveitar essa parceria. |
...
5. Em seguida, para fins de correspondência de registros, escolha uma chave de sincronização para [mapear](#supported-objects) o objeto do Braze para um campo do modelo.<br>![No prompt "Select a Sync Key" (Selecionar uma chave de sincronização), "External User ID" (ID de usuário externo) da Braze corresponde a "user_id" na origem.][9]{: style="max-width:80%;"}<br><br>
6. Por fim, mapeie os campos de dados do Census para os campos equivalentes do Braze.<br>![Mapeamento do Census][11]{: style="max-width:80%;"}<br><br>
