...
- [Importação de coorte]({{site.baseurl}}/partners/data_and_infrastructure_agility/cohort_import/amplitude/): Uma integração que lhe permite encaminhar coortes do Amplitude para o Braze.
...
A característica do usuário e as sincronizações de computação criarão novos usuários para IDs de usuário que ainda não existem no Braze. Os cálculos e as características do usuário só podem ser sincronizados usando a ID do usuário.
...
Para verificar alguns dos endpoints comuns da API do Amplitude que podem ser usados com o conteúdo conectado, consulte nossa [documentação específica da API da Amplitude]({{site.baseurl}}/partners/data_and_infrastructure_agility/analytics/amplitude/amplitude_user_profile_api/).
