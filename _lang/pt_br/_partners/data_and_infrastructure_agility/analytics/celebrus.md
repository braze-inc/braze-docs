...
Essa integração usa a ingestão de dados do Braze Cloud. Siga as instruções em [Integrações de data warehouse]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/integrations/) para definir as [configurações da ingestão de dados na nuvem]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/overview/) de acordo com o tipo de data warehouse usado. 
...
O Celebrus usará os identificadores definidos para enviar os atributos do cliente que estão definidos no construtor de perfil do Celebrus, mas somente quando os valores dos atributos forem alterados. Note que os nomes das atribuições serão definidos no construtor de perfil do Celebrus, de modo que os atributos no Braze espelharão os do perfil do Celebrus. Talvez seja necessário ajustá-los para que estejam em conformidade com as convenções de nomenclatura do Braze. Por exemplo, em conformidade com as [convenções de nomenclatura de atribuições padrão]({{site.baseurl}}/api/objects_filters/user_attributes_object/) do Braze.  
