...
O Braze calcula o momento ideal para o envio com base em uma análise estatística das interações anteriores dos usuários com seu app e suas interações com cada canal de envio de mensagens. São usados os seguintes dados de interação: 
...
Se um usuário não tiver dados de engajamento relevantes para que o Braze calcule o horário ideal de envio, você pode especificar um horário de fallback.
...
{% tabs local %}
{% tab Campaign %}
### Etapa 1: Adicione Intelligent Timing
...
4. Defina a frequência de entrada. Para envios únicos, selecione **Uma vez** e escolha uma data de envio. Para envios recorrentes, selecione **Diariamente**, **Semanalmente** ou **Mensalmente** e configure as opções de recorrência. Consulte [as limitações](#limitations) para obter mais orientações.
5. Opcionalmente, configure [o horário de silêncio](#quiet-hours).
6. Especifique um [prazo de fallback](#campaign-fallback). É quando a mensagem é enviada se o perfil do usuário não tiver nenhum evento relevante para calcular o horário ideal.
...
Use o recurso Horário de silêncio para impedir que mensagens sejam enviadas durante horários específicos. Isso é útil quando você deseja evitar o envio de mensagens durante as primeiras horas da manhã ou durante a noite, mas ainda assim permitir que o Intelligent Timing determine a melhor janela de entrega.

{% alert note %}
A opção Horário de silêncio substituiu a configuração **Enviar apenas em horários específicos**. Em vez de escolher quando as mensagens podem ser enviadas, agora você escolhe quando elas não devem ser enviadas. Por exemplo, para enviar mensagens entre 16h e 18h, defina o horário de silêncio das 18h às 16h do dia seguinte.
{% endalert %}

1. Selecione **Ativar Horário de Silêncio**.
2. Selecione a hora de início e término em que **não** deseja realizar o envio de mensagens.

![A opção Horário de silêncio está ativada, com a hora de início e término definidas para bloquear o envio de mensagens durante a noite.]({% image_buster /assets/img/intelligent_timing/quiet_hours.png %})

Quando o recurso Horário de silêncio está ativado, o Braze não enviará mensagens durante o período de silêncio, mesmo que esse horário corresponda ao horário ideal de envio do usuário. Se o horário ideal do usuário estiver dentro da janela silenciosa, a mensagem será enviada no limite mais próximo da janela.

Por exemplo, se o horário de silêncio for definido das 22h às 6h e o horário ideal do usuário for 5h30, o Braze reterá a mensagem e a entregará às 6h — o horário mais próximo fora do horário de silêncio.
...
#### Inicie a campanha com 48 horas de antecedência

Lance sua campanha pelo menos 48 horas antes da data de envio programada. Isso é por causa das variações nos fusos horários. Braze calcula o horário ideal à meia-noite no horário de Samoa (UTC+13), um dos primeiros fusos horários do mundo. Um único dia dura cerca de 48 horas em todo o mundo, o que significa que, se você lançar uma campanha dentro desse intervalo de 48 horas, é possível que o horário ideal para o usuário já tenha passado em seu fuso horário e a mensagem não seja enviada.
...
Se você estiver utilizando [testes A/B com uma otimização]({{site.baseurl}}/user_guide/engagement_tools/testing/multivariant_testing/optimizations/), como o envio automático da **variante vencedora** ou o uso de uma **variante personalizada**, o Intelligent Timing poderá afetar a duração e o tempo de sua campanha.
...
### Etapa 3: Escolha um horário de entrega (opcional)

Opcionalmente, você pode optar por limitar o prazo de entrega. Isso pode ser útil se sua campanha estiver relacionada a um evento, venda ou promoção específica, mas geralmente não é recomendado ao usar o Intelligent Timing. Para saber mais, consulte as [limitações](#limitations).

Quando especificado, o Braze usa apenas os dados de engajamento dentro dessa janela para determinar o tempo de entrega ideal para o usuário. Se não houver nenhum evento relevante dentro desse intervalo, a mensagem será enviada no horário de fallback definido por você.

Para definir um intervalo de entrega:
...
![Caixa de seleção para "Enviar mensagens apenas dentro de horários específicos" selecionada, onde a janela de tempo é definida entre 8h e 12h no fuso local do usuário.]({% image_buster /assets/img/intelligent_timing_hours.png %})
...
![Exemplo de prévia dos tempos de entrega para Android Push.]({% image_buster /assets/img/intel-timing-preview.png %})
...
O gráfico mostra os usuários que tiveram eventos relevantes para calcular um horário ideal em azul e os usuários que usarão o horário de fallback em vermelho. Use os filtros de cálculo para ajustar a {prévia} para uma visão mais granular de cada grupo de usuários.
{% endtab %}
...
No seu Canvas, adicione uma [etapa Mensagem]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step/), acesse as **Configurações de entrega** e selecione **Usando Intelligent Timing**.

As mensagens serão enviadas aos usuários que entraram na etapa naquele dia, no fuso local ideal. No entanto, se o horário ideal já tiver passado nesse dia, a entrega será feita no horário correspondente do dia seguinte. Etapas de mensagens que têm como alvo vários canais podem enviar ou tentar enviar mensagens em horários diferentes para canais diferentes. Quando a primeira mensagem em uma etapa de Mensagem tenta enviar, todos os usuários são avançados automaticamente.

### Etapa 2: Escolha um horário de fallback

Escolha um horário alternativo para enviar a mensagem aos usuários do seu público que não possuem dados de engajamento relevantes para que o Braze calcule o horário ideal para o envio. {% multi_lang_include brazeai/intelligent_suite/fallback_time.md %}
...
Em vez disso, adicione uma [etapa]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/) [de postergação]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/) de pelo menos dois dias corridos entre o momento em que o usuário entra no Canvas e o momento em que recebe a etapa Intelligent Timing.
...
Ao utilizar o Intelligent Timing, recomendamos usar dias corridos para postergações, em vez de dias de 24 horas. Isso ocorre porque, com os dias corridos, a mensagem será enviada no último dia da postergação, no momento ideal. Com um dia de 24 horas, há uma chance de que o horário ideal para o usuário seja antes de ele entrar na etapa, o que significa que um dia extra será adicionado à sua postergação.
...
![Gráfico que ilustra a diferença entre dias e dias corridos, em que, se o horário ideal do usuário for 14h, mas ele entrar na etapa de postergação às 14h01, e a postergação estiver definida para 2 dias. Os dias entregam a mensagem três dias depois porque o usuário entrou na etapa após o tempo ideal, enquanto os dias corridos entregam a mensagem dois dias depois, no último dia da postergação.]({% image_buster /assets/img/intelligent_timing_daysvcalendardays.png %}){: style="border:none;"}
{% endtab %}
{% endtabs %}

## Limitações

- As mensagens no app e os webhooks são entregues imediatamente e não recebem horários ideais.
...
Se não houver eventos relevantes para um usuário (por exemplo, novos usuários com pouco ou nenhum engajamento), a Braze usa a configuração de fallback definida — seja o seu tempo de fallback personalizado ou o horário mais popular para usar o app entre todos os usuários.

### Impacto do fuso horário na entrega do Intelligent Timing

Intelligent Timing depende do fuso local especificado por cada usuário, portanto, a data e a hora de entrega programadas podem variar entre os usuários.

Se os usuários não receberem as mensagens como esperado, verifique se o campo de fuso horário em seus perfis está preenchido corretamente. Se o campo do fuso horário estiver vazio, o usuário poderá receber mensagens que correspondem ao fuso horário da empresa, em vez do seu fuso local.
...
Se não houver eventos de engajamento relevantes para um usuário (por exemplo, novos usuários com poucos ou nenhum clique ou abertura), o Intelligent Timing usa a configuração de fallback definida — seja o seu tempo de fallback personalizado ou o horário mais popular para usar o app entre todos os usuários.
...
O limite de frequência pode ser usado em uma campanha que utiliza Intelligent Timing. No entanto, a natureza da limitação de frequência significa que alguns usuários podem receber suas mensagens em um momento menos que ideal, especialmente se um grande número de usuários em relação ao tamanho do limite de frequência estiver programado para o momento de fallback porque não têm eventos relevantes. 
