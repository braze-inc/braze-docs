...
| **Métrica de sucesso** | A métrica de negócios específica que você deseja otimizar, como receita, conversões ou receita média por usuário (ARPU). Esta é a métrica que o agente decisório procurará maximizar por meio de suas ações. |
...
> Quem será o público do engajamento do agente decisório?
...
Restrições são as regras que o agente deve seguir. Isso pode impedir que uma oferta específica seja selecionada para clientes em uma região geográfica não elegível ou definir um orçamento máximo para o agente de decisão gastar.

## Melhores práticas e exemplos

Para maximizar o impacto do seu agente de decisão, você deve:

- Escolher uma métrica de sucesso que esteja alinhada com suas metas comerciais e objetivos, como receita, conversões ou ARPU.
- Concentrar-se nas dimensões ou "alavancas" a testar, como oferta, linha de assunto, criativo, canal ou hora de envio, que são mais propensas a ter um impacto significativo na métrica de sucesso.
- Selecionar as opções para cada dimensão, como e-mail versus SMS ou frequência diária versus semanal, que provavelmente terão um impacto significativo na métrica de sucesso.
...
- O BrazeAI<sup>TM</sup> aprende quais clientes são suscetíveis a ofertas de salto e quais exigem descontos ou outros incentivos para fazer upgrade
- Orquestra envios personalizados pela Braze para maximizar a ARPU
{% endtab %}
...
- O BrazeAI<sup>TM</sup> identifica clientes que são menos sensíveis ao preço e precisam de descontos menos significativos para renovar
- Orquestra envios personalizados pela Braze para maximizar renovações de contratos e VPL
{% endtab %}
{% tab Winback agent %}
...
- O BrazeAI<sup>TM</sup> descobre a melhor combinação para cada cliente individualmente
- Orquestra envios personalizados pela Braze para maximizar as taxas de reativação
{% endtab %}
{% tab Referral agent %}
Você poderia criar um agente de indicação para maximizar a abertura de novas contas por meio de indicações de cartões de crédito empresariais de clientes existentes:
...
- Orquestra envios personalizados pela Braze para maximizar as conversões de indicações
{% endtab %}
...
- Defina o público e a mensagem na Braze
- O Decisioning Studio executa automaticamente experimentos diários, testando diferentes segmentos de clientes, metodologias de lance, níveis de lance e criativos
- O BrazeAI<sup>TM</sup> utiliza dados primários robustos para otimizar a performance dos anúncios pagos à medida que as políticas de privacidade mudam
- Orquestra envios personalizados pela Braze para maximizar a receita e otimizar o custo por cliente
{% endtab %}
...
Pronto para criar seu próprio agente de tomada de decisão? Siga as próximas etapas para o seu nível do Decisioning Studio:
