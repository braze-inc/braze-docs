...
Selecione um tipo de recomendação. Cada tipo usa os últimos seis meses de dados de interação com itens, como dados de compra ou de evento personalizado. Para informações mais detalhadas e casos de uso para cada um, veja [Tipos e Casos de Uso]({{site.baseurl}}/user_guide/brazeai/recommendations/).
...
Você pode encontrar a recomendação na página **Previsões**, onde poderá editá-la ou arquivá-la conforme necessário. As recomendações serão automaticamente re-treinadas uma vez por semana (pago) ou uma vez por mês (grátis).