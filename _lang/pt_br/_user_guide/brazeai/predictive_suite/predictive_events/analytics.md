...
## Sobre as análises preditivas de eventos
...
Estes são os componentes que compõem as análises preditivas de eventos:
...
Para visualizar a pontuação de probabilidade de um único usuário, procure esse usuário no dashboard e acesse **Engajamento** > **Previsões** para ver sua pontuação. Para acessar pontuações e categorias para vários usuários de uma só vez, crie um [segmento]({{site.baseurl}}/user_guide/engagement_tools/segments/creating_a_segment/) usando os filtros [Pontuação de probabilidade de evento]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/#event-likelihood-score) ou [Categoria de probabilidade de evento]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/#event-likelihood-category) e, em seguida, exporte os usuários desse segmento. Ao exportar, você pode incluir as pontuações de probabilidade nos dados exportados.

{% alert note %}
Embora tanto os eventos de previsão quanto [a rotatividade de previsão]({{site.baseurl}}/user_guide/brazeai/predictive_churn/) atribuam pontuações aos usuários, existem diferenças importantes:<br><br>

- **Eventos preditivos** (previsões de compra): Considere todos os usuários no público-alvo da previsão, independentemente de eles já terem realizado o evento-alvo anteriormente. Por exemplo, uma previsão de compra pode identificar usuários propensos a fazer sua primeira compra.
- **Churn preditivo**: Considera apenas usuários que já realizaram o evento personalizado. As previsões de churn identificam usuários que já realizaram alguma ação anteriormente e que provavelmente deixarão de fazê-la. Um usuário que nunca fez login não pode ser considerado “desistente” se não fizer login.
...
![O público selecionado e a precisão estimada exibidos no dashboard do Braze.]({% image_buster /assets/img/purchasePrediction/purchaseEstimatedResults.png %})

### Espera-se que tenha uma boa performance

Você pode usar a precisão estimada para verificar quantos usuários selecionados devem realizar a performance do evento.
...
### Não se espera performance

Você pode usar a precisão estimada para verificar quantos usuários selecionados provavelmente não realizarão a performance do evento.
