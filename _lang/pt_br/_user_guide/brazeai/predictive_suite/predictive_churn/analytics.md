---
nav_title: Análise de Churn
article_title: Análise Preditiva de Churn
...
- [Score de Churn e Categoria](#churn_score)
...
Esta é uma estimativa de quantos churners reais serão corretamente alvo. Claro, não conhecemos o futuro perfeitamente, então não sabemos exatamente quais usuários do público de previsão irão sofrer churn no futuro. Mas a previsão é uma inferência confiável. Com base na performance passada, esta barra de progresso indica quantos do total de "churners" "atuais" ou "verdadeiros" esperados no público de previsão (com base nas taxas de churn anteriores) serão direcionados com a seleção de direcionamento atual. Esperaríamos esse número de usuários com churn se você não os direcionar com qualquer envio de mensagens extra ou incomum.

### Usuários esperados para não churnar 

Esta é uma estimativa de quantos usuários que não teriam churnado serão incorretamente alvo. Todos os modelos de machine learning cometem erros. Pode haver usuários em sua seleção que tenham uma alta _pontuação de risco de churn_, mas que não acabem com churn. Eles não sofrerão churn mesmo que você não faça nada. Eles serão segmentados de qualquer maneira, então isso é um erro ou "falso positivo." A largura total desta segunda barra de progresso representa o número esperado de usuários que não churnarão, e a parte preenchida representa aqueles que serão incorretamente alvo usando a posição atual do controle deslizante.
