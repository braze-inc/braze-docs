...
- Quantos usuários selecionados são esperados **não** churn
...
### Usuários esperados para não churn 

Esta é uma estimativa de quantos usuários que não teriam churn serão incorretamente segmentados. Todos os modelos de machine learning cometem erros. Pode haver usuários em sua seleção que tenham uma alta _pontuação de risco de churn_, mas que não acabem com churn. Eles não sofrerão churn mesmo que você não faça nada. Eles serão segmentados de qualquer maneira, então isso é um erro ou "falso positivo." A largura total desta segunda barra de progresso representa o número esperado de usuários que não churn, e a parte preenchida representa aqueles que serão incorretamente segmentados usando a posição atual do controle deslizante.
