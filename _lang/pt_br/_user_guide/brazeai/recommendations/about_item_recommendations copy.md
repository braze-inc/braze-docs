---
nav\_title: Sobre o item Recomendações article\_title: Sobre o item Descrição das recomendações: "Este artigo de referência descreve diferentes casos de uso para recomendar itens aos clientes que usam o Braze." page\_order: 10
---
...
Com as recomendações de itens da IA, você também pode filtrar ainda mais os itens disponíveis com [seleções]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/selections/). No entanto, as seleções com Liquid não podem ser usadas em recomendações de IA, portanto, lembre-se disso ao criar suas seleções de catálogo.

Para obter exemplos, consulte a seção [Casos de uso](#use-cases) deste artigo.

{% alert tip %} As recomendações personalizadas de IA funcionam melhor com centenas ou milhares de itens e, normalmente, com pelo menos 30.000 usuários com dados de compra ou interação. Esse é apenas um guia aproximado e pode variar. Os outros tipos de recomendação podem funcionar com menos dados. {% endalert %}
...
### Recomendações de itens de tendência {#trending}
