...
Para configurar um webhook do Braze Salesforce Sales Cloud que atualiza os leads no Salesforce, você precisa de um identificador comum entre o Salesforce Sales Cloud e o Braze. O exemplo abaixo usa o Salesforce `lead_id` como o Braze `external_id`, mas você também pode fazer isso usando um `user_alias`. 
