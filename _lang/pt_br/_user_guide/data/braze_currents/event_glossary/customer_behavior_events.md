...
{% api %}
## Atividade ao vivo Eventos de mudança de token por push para iniciar

{% apitags %}
Atividade ao vivo, token por push para iniciar
{% endapitags %}

Esse evento ocorre quando o Braze sincroniza o token de início de atividade ao vivo com o usuário
...
{% api %}
## Eventos de alteração de token de atualização de atividade ao vivo
...
{% api %}
## Eventos de mudança de estado do token por push

{% apitags %}
Push, mudança de estado do token
{% endapitags %}

Esse evento ocorre quando um token por push é inserido, atualizado ou removido. Use isso para rastrear os estados dos tokens por push.
