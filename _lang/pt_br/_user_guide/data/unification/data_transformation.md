...
Sim, na maioria dos casos. O Braze Data Transformation eventualmente cria uma `/users/track` chamada que grava os atributos, eventos e compras que você deseja. Eles registrarão os pontos de dados da mesma forma como se a chamada para `/users/track` tivesse sido feita de forma independente. Você tem controle sobre quantos pontos de dados são registrados com base em como você escreve a sua transformação.
