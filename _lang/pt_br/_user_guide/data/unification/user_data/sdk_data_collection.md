...
| País | País identificado pela geolocalização do endereço IP. Se a geolocalização do endereço IP não estiver disponível, isso é identificado pela [local do dispositivo](#optional-data-collected-by-default). O valor pode ser alternativamente o que os SDKs definem diretamente com `setCountry`, mas note que passar um valor de atributo através do SDK ou API registrará pontos de dados.| Este atributo é usado para direcionar mensagens com base na localização. |
...
| Localização mais recente do dispositivo           | Android, iOS      | O local padrão mais recente do dispositivo                                                   | Este atributo vem das configurações do dispositivo do usuário e é usado para traduzir mensagens para o idioma preferido do usuário. É independente do atributo `Most Recent Location`.                                                                                            |
...
| Idioma                | Android, iOS, Web | Idioma do dispositivo ou navegador, retirado do local do dispositivo                                                            | Este atributo é usado para traduzir mensagens para o idioma preferido do usuário. É baseado no local do dispositivo.                                                                                            |
...
{% alert note %}
O SDK Braze não armazena endereços IP localmente.
{% endalert %}
