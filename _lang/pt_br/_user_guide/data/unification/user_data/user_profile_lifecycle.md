...
Todos os IDs de usuário podem ser usados para encontrar e identificar usuários no dashboard para testes. Para encontrar seu usuário no dashboard do Braze, consulte [Adicionando Usuários de Teste]({{site.baseurl}}/user_guide/administrative/app_settings/developer_console/internal_groups_tab/#adding-test-users).

{% alert important %}
O Braze banirá ou bloqueará os usuários com mais de 5.000.000 de sessões ("usuários fictícios") e não ingerirá mais seus eventos SDK, pois esses usuários geralmente são o resultado de uma integração incorreta. Se você descobrir que isso aconteceu com um usuário legítimo, entre em contato com seu gerente de conta do Braze.
{% endalert %}