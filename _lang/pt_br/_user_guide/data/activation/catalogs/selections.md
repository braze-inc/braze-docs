...
Depois de criar um [catálogo]({{site.baseurl}}/user_guide/data/activation/catalogs/), você pode fazer referência adicional aos dados do catálogo incorporando seleções em suas campanhas ou recomendações do Braze.
...
![Um cartão de conteúdo com o cabeçalho "You will LOVE these highly rated meals!" (Você vai adorar essas refeições bem avaliadas!) com a seleção "recommendations_be_recent_category" na seção de composição de mensagens.]({% image_buster /assets/img_archive/catalog_selections3.png %}){: style="max-width:90%;"}
