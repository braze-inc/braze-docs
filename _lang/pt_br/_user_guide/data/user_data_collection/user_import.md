...
> A Braze oferece uma variedade de maneiras de importar dados de usuários para a plataforma: SDKs, APIs, ingestão de dados na nuvem, integrações com parceiros de tecnologia e arquivos CSV.
...
Selecione **Pesquisar arquivos** e seu arquivo. Braze fará upload do seu arquivo e verificará os cabeçalhos das colunas e os tipos de dados de cada coluna.
...
Especialmente para matrizes, as vírgulas em seu arquivo CSV serão interpretadas como um separador de coluna, portanto, qualquer vírgula nos valores causará erros na análise do arquivo.<br><br>Para fazer upload desses tipos de valores, use o [endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) ou a [ingestão de dados da nuvem]({{site.baseurl}}/user_guide/data/cloud_ingestion/).
{% endalert %}
...
## Criação de um filtro de redirecionamento a partir de uma importação de usuário

A importação de usuários pode ser usada para transformar o arquivo CSV em um filtro de redirecionamento, selecionando **Importe os usuários neste CSV e possibilite o redirecionamento de um lote específico de usuários como grupo.**. Para filtrar pelo arquivo em um segmento ou onde quer que a filtragem seja uma opção, selecione o filtro **Atualizado/Importado de CSV** e, em seguida, pesquise o nome exato do arquivo.

![Um grupo de filtros com o filtro "Atualizados/importados via CSV", incluindo um arquivo CSV intitulado "Halloween season fun".][5]

## Criação de segmentos a partir de uma importação de usuário

A importação de usuários também pode ser usada para criar segmentos selecionando **Importe os usuários neste CSV e possibilite o redirecionamento de um lote específico de usuários como grupo.** e marcando **Gere automaticamente um segmento a partir dos usuários importados deste CSV** antes de iniciar a importação.
