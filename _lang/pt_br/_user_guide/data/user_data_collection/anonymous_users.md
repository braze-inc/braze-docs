...
article_title: "Primeiros passos: Usuários anônimos"
page_order: 0
page_type: reference
...
Os usuários anônimos não têm `external_ids`, mas é possível atribuir aos perfis de usuários anônimos um identificador alternativo: aliases de usuário. Isso permite que você execute as mesmas ações em um perfil de usuário anônimo como se ele fosse identificado por `external_ids`. Por exemplo, você pode usar a Braze API para registrar eventos e atribuições associadas a usuários anônimos e direcionar esses usuários em seu envio de mensagens com o filtro de segmentação [External User ID is blank]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#external-user-id).
