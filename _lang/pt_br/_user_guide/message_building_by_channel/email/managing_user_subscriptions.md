...
## Estados de inscrição {#subscription-states}
 ...
 Por exemplo, os usuários considerados `unsubscribed` não receberão mensagens direcionadas ao estado de inscrição global de `subscribed` ou `opted-in`.
...
Se um usuário tiver marcado um ou mais de seus e-mails como spam, o Braze só enviará e-mails de transação para esse usuário. 
