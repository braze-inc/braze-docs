...
# Crie um e-mail com arrastar e soltar
...
![A seção "Email Variants" (Variantes de e-mail) com um exemplo de corpo de e-mail.]({% image_buster /assets/img/dnd/dnd_emailvariant.png %})

### Etapa 2: Crie seu e-mail

A experiência de edição de arrastar e soltar é dividida em três seções: **Configurações de Envio**, **Conteúdo**, e **Prévia & Teste**. A mágica de criar o corpo do e-mail acontece na seção **Conteúdo**. Antes de criar seu e-mail, é importante entender os principais componentes que orientam sua experiência de criação de e-mail. Se você precisar revisar, consulte [Sobre o editor](#about-the-editor).
...
- Quando você seleciona uma configuração de linha, as configurações de **Propriedades da Linha** aparecem para personalização adicional das cores de fundo da linha, imagens e tamanhos de colunas personalizados.
...
- **Selecione Usuário:** É possível selecionar um usuário específico com base em seu endereço de e-mail ou ID externo. O e-mail será pré-visualizado com base nas características e informações do evento desse usuário
...
Você também pode selecionar **Copiar link de prévia** para gerar e copiar um link de prévia compartilhável que mostra como o e-mail ficará para um usuário aleatório. O link durará sete dias antes de precisar ser regenerado. 

Observe que quaisquer edições feitas em um modelo de e-mail não refletirão em um link gerado anteriormente. Você precisará gerar uma nova prévia de link para ver quaisquer edições.

![Prévia do e-mail com um botão para "Copiar link de prévia" e copiar o link gerado.]({% image_buster /assets/img/dnd_email_link_preview.png %})

#### Use o Inbox Vision

O Inbox Vision permite que você visualize suas campanhas de e-mail a partir da perspectiva de clientes de e-mail e dispositivos móveis. Para testar sua mensagem de e-mail usando o Inbox Vision, selecione **Inbox Vision** na seção **Prévia & Teste** e selecione **Executar Inbox Vision**.
...
![Opção de largura automática na guia Content (Conteúdo) do editor de arrastar e soltar.]({% image_buster /assets/img/dnd/dnd1.png %})
...
Você pode definir o atributo de idioma acessando a aba **Configurações** e selecionando o idioma desejado. Você também pode direcionar o atributo do usuário {%raw%} `{{${language}}}` {%endraw%} se a mensagem for destinada a usuários com valores de idioma dinâmicos.

![Definindo o valor "Idioma" para um e-mail.]({% image_buster /assets/img/dnd/language_setting_dnd.png %}){: style="max-width:70%;"}
...
4. Copie seu trecho Liquid gerado e cole-o no corpo do seu e-mail.
...
![A seção "Atribuições" com o atributo "clicktracking" desativado para um link.]({% image_buster /assets/img/dnd_custom_attributes.png %}){: style="float:right;max-width:35%;margin-left:15px;"}
...
Use `<head>` tags para adicionar CSS e metadados na sua mensagem de e-mail. Por exemplo, você pode usar essas tags para adicionar uma folha de estilo ou favicon. Liquid é suportado em tags `<head>`.
...
| `base` | `target`| Alvo padrão para todos os hyperlinks e formulários. | ```<base target="_blank">``` |
| `link` | `href` | URL para o recurso externo. | ```<link href="style.css">``` |
| `link` | `rel` | Define relacionamentos entre a mensagem atual e a mensagem vinculada. | ```<link rel="stylesheet">``` |
