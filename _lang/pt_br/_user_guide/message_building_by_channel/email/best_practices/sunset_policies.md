...
Para e-mails, seu IP de envio tem uma pontuação de reputação que leva em consideração o engajamento, o relatório de spam, a lista de bloqueio e mais. Você pode usar ferramentas como [Sender Score](https://www.senderscore.org/ "Sender Score") ou [Smart Network Data Service do Outlook](https://postmaster.live.com/snds/ "Smart Network Data Service do Outlook") para monitorar a pontuação da sua reputação. Se sua pontuação de reputação estiver consistentemente baixa, provedores de acesso à internet e filtros de caixa de correio podem automaticamente classificar seus e-mails em uma pasta de spam ou de baixa prioridade para todos os destinatários, até mesmo os engajados. Criar uma política de sunset ajuda a entregar seus e-mails apenas para destinatários ativos. 
...
Para configurar esses segmentos, escolha os filtros `Has Marked You As Spam` e `Last Engaged With Message` localizados na seção **Atividade de Marketing** no menu suspenso de filtros. 
...
![Página de Detalhes do Segmento com o filtro "Última Interação com a Mensagem" selecionado.][20]

Embora o Braze pare automaticamente de enviar e-mails para usuários que marcaram você como spam, o filtro `Has Marked You As Spam` permite que você também envie mensagens push direcionadas e notificações no app para esses usuários. Este filtro é útil para campanhas de redirecionamento][21]. Por exemplo, você pode enviar mensagens para usuários desengajados que os lembrem dos recursos e ofertas que estão perdendo quando não abrem seus e-mails.
