...
| Pools de IP | Trata-se de uma configuração opcional usada para separar a reputação de diferentes tipos de e-mail (como "promocional" e "transacional") para evitar que a reputação de um afete o outro e para oferecer maior entregabilidade. | A equipe de integração configurará os pools para você. Em seguida, ao criar seu e-mail, selecione o pool de IPs do seu e-mail no menu suspenso **Pool de IPs** na página **Usuários de destino**.|
