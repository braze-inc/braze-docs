...
page_order: 3
description: "Este artigo de referência aborda como o Braze lida com envios de mensagens SMS bidirecionais e respostas automáticas. Isso inclui explicações sobre como funciona o disparo de palavras-chave, bem como categorias de palavras-chave personalizadas e suporte a vários idiomas."
page_type: reference
channel:
  - SMS
...
> Este artigo de referência aborda como o Braze lida com envios de mensagens SMS bidirecionais e respostas automáticas. Isso inclui explicações sobre como funciona o disparo de palavras-chave, bem como categorias de palavras-chave personalizadas e suporte a vários idiomas.
...
O SMS com o Braze lhe dá a opção de criar disparadores de palavras-chave, respostas personalizadas, definir conjuntos de palavras-chave para vários idiomas e estabelecer categorias de palavras-chave personalizadas. 
...
1. No dashboard da Braze, acesse **Público** > **Grupos de inscrições** e selecione seu grupo de inscrições de SMS.<br><br>
2. Em **SMS Global Keywords**, clique no ícone de lápis ao lado da categoria de palavra-chave à qual você deseja adicionar uma palavra-chave. ![]({% image_buster /assets/img/sms/sms_keywords.png %})<br><br>
...
{% alert tip %}
Interessado em ver como essas palavras-chave podem ser usadas em suas campanhas e Canvas para redirecionar e disparar mensagens? Visite nosso artigo sobre [redirecionamento de SMS]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/retargeting/) para saber mais.
{% endalert %}
{% endtab %}
...
1. No dashboard da Braze, acesse **Público** > **Grupos de inscrições** e selecione seu grupo de inscrições de SMS. <br><br>
2. Em **SMS Global Keywords**, selecione uma categoria de palavra-chave para editar uma resposta selecionando o ícone de lápis. ![]({% image_buster /assets/img/sms/sms_keywords.png %})<br><br> 
...
{% alert tip %}
Se um Canva baseado em ação for disparado por uma mensagem SMS recebida, você poderá fazer referência às propriedades do SMS na primeira [etapa de mensagens]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step/) do Canvas.
{% endalert %}
...
Clique em **Add a Language (Adicionar um idioma** ) e selecione seu idioma de direcionamento ou pesquise um idioma no menu suspenso.

{% alert important %}
Note que outros idiomas não vêm com palavras-chave e respostas predefinidas como o inglês, portanto, os remetentes precisarão trabalhar com seus profissionais de marketing e equipes jurídicas para adicionar quaisquer palavras-chave necessárias a esse conjunto. Caso contrário, a Braze não tratará as mensagens recebidas com localização para esses idiomas.
{% endalert %}

Se você precisar excluir um idioma, clique no botão **Excluir idioma** no canto inferior direito.

![Página Palavras-chave globais de SMS com a guia "Francês" selecionada. Existem guias adicionais para cada idioma adicionado.][5]
...
![Palavras-chave para uma categoria "Doubleoptin". Se um usuário enviar "Y", ele receberá a mensagem "Thank you for confirming your enrollment in Hair Cuttery SMS" (Obrigado por confirmar sua inscrição no Hair Cuttery SMS).][12]
...
2. Clique em **Adicionar palavra-chave personalizada**. ![][13]{: style="max-width:90%;"}
...
Depois que essa categoria de palavra-chave for criada, ela estará disponível para ser [filtrada e disparada]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/retargeting/) em suas campanhas e Canvas.
...
![Etapa do canva baseada em ação com o disparo Enviar SMS de entrada para o grupo de inscrições "Serviço de mensagens" dentro da categoria de palavra-chave "Outro", em que o corpo da mensagem corresponde à expressão regular "pular símbolo de acento circunflexo".][14]{: style="max-width:90%;"}

#### Campanha interrompida

![Campanha baseada em ação com o disparo Enviar SMS de entrada para o grupo de inscrições "Serviço de mensagens de marketing A" dentro da categoria de palavra-chave "Outro" em que o corpo da mensagem é "Palavra-chave1" ou é "Palavra-chave2" ou não é "Palavra-chave A".][15]{: style="max-width:90%;"}

### Lidando com palavras-chave desconhecidas

Embora não seja obrigatório, recomendamos fortemente a configuração de uma resposta automática para quando os usuários enviarem palavras-chave de SMS de entrada que não correspondam a uma palavra-chave existente. Essa mensagem notificará o usuário de que a palavra-chave não foi reconhecida e oferecerá algumas orientações. 

Isso pode ser feito criando uma campanha de mensagens SMS com uma mensagem como "Desculpe! Não reconhecemos essa palavra-chave, enviamos a mensagem STOP para parar ou HELP para ajudar." Em seguida, na etapa de entrega, selecione **Entrega baseada em ação** e use o disparo **Enviar SMS de entrada para o grupo de inscrições dentro da categoria de palavras-chave OUTROS**.

![]({% image_buster /assets/img/sms/sms_other.png %})

{% alert tip %}
Interessado em ver como essas palavras-chave e categorias de palavras-chave podem ser usadas em suas campanhas e Canvas para redirecionar e disparar mensagens? Visite nosso artigo sobre [redirecionamento de SMS]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/retargeting/) para saber mais.
{% endalert %}

[oblink]: {{site.baseurl}}/user_guide/message_building_by_channel/sms/sms_subscription_group/#setup-process
[1]: {% image_buster /assets/img/sms/keyword_edit2.png %}
[2]: {% image_buster /assets/img/sms/keyword_home.png %}
[desconhecido]: {{site.baseurl}}/user_guide/message_building_by_channel/sms/keywords/unknown_phone_numbers/
