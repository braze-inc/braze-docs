...
- **Entrega de mensagens flexível:** Facilite a entrega de mensagens flexível e confiável com um fallback de SMS contínuo que atinge todos os segmentos de público, independentemente das capacidades do dispositivo, enquanto preserva uma experiência de usuário consistente.
...
| --- | --- |
| Promoções de produtos interativas | Dê vida às promoções de produtos combinando imagens envolventes ou vídeos curtos com documentos detalhados do produto. Aproveite respostas sugeridas (como "Adicionar ao Carrinho" ou "Aprender") e ações openURL para impulsionar a exploração imediata do produto e conversão—tudo dentro de uma experiência rica em mensagem. |
...
| Fallback de SMS | Se uma mensagem não puder ser entregue com RCS (por exemplo, falta de suporte da operadora na região), a Braze ainda tentará entregar a mensagem por SMS quando um código SMS existir dentro do grupo de inscrições. |
| RCS básico | Mensagens apenas de texto com até 160 caracteres. Cobrado como uma única mensagem. <br><br> Esta categoria é usada apenas no modelo global. |
