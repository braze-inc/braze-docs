...
- **Interações de marca confiáveis:** Alcance interações de marca confiáveis através de um ID de remetente verificado que não apenas exibe os ativos da sua marca, mas também cumpre os principais padrões de privacidade do setor, aumentando a confiança e a fidelidade do cliente.
- **Entrega de mensagens flexível:** Facilite a entrega de mensagens flexível e confiável com um fallback de SMS contínuo que alcança todos os segmentos de público, independentemente das capacidades do dispositivo, enquanto preserva uma experiência de usuário consistente.
...
| Rico | Mensagens apenas de texto, com ou sem sugestões limitadas ou botões. Cobrado por segmento (160 bytes UTF-8). Por exemplo, uma mensagem com 161 caracteres de texto simples conta como dois segmentos. <br><br> Esta categoria é usada apenas no modelo dos Estados Unidos. |
