...
description: "Este artigo de referência cobre como os usuários podem redirecionar suas mensagens com base nas interações de SMS e RCS de um usuário."
page_type: reference
page_order: 4
...
> Além de alterar o estado da inscrição do usuário e enviar respostas automáticas com base nas palavras-chave recebidas, o Braze também registrará as interações com o perfil do usuário para filtrar e disparar mensagens.<br><br>Esses filtros e disparadores permitem que você filtre ações com base em usuários que receberam ou responderam a campanhas de SMS, MMS e RCS, ou se envolvam mais com usuários que clicaram em URLs encurtadas.
...
Os usuários podem ser filtrados com base em quando receberam pela última vez um SMS, MMS ou RCS ou se receberam um SMS, MMS ou RCS de uma campanha específica. Os filtros podem ser definidos na etapa **Públicos-alvo** do construtor de campanhas. 

**Filtrar por último SMS/MMS/RCS recebido**<br>
...
Filtra usuários que receberam uma mensagem de uma campanha específica. Com este filtro, você também tem a opção de filtrar aqueles que não receberam mensagens de uma campanha. <br>
...
Para disparar mensagens à medida que os usuários recebem mensagens de SMS, MMS ou RCS de uma campanha específica, selecione **Interagir com a Campanha** como a ação de disparo para uma campanha baseada em ações. Em seguida, selecione **Receber SMS** e a campanha de SMS, MMS ou RCS que você gostaria de usar.
...
**Redirecionar usuários que clicaram em uma campanha específica de SMS, MMS ou RCS**
1. Crie um segmento usando o filtro **Clicked/Opened Campaign (Campanha clicada/aberta** ).
2. Selecione **link de sms encurtado clicado**.
...
Além das três categorias de palavras-chave padrão (Aceitação, Desaceitação e Ajuda), também é possível criar até 25 categorias de palavras-chave próprias, o que permite identificar palavras-chave e respostas arbitrárias. Essas categorias podem ser usadas para filtragem e redirecionamento. Para ler mais sobre categorias de palavras-chave globais e como configurá-las, consulte [Processamento de palavras-chave]({{site.baseurl}}/user_guide/message_building_by_channel/sms_mms_rcs/keywords/). 

### Filtrar por recência

Filtrar pela recência de um usuário respondendo ao seu programa de SMS, MMS ou RCS. Este filtro avaliará a última data em que um usuário enviou uma mensagem de entrada que está dentro de uma das categorias de palavras-chave. 
...
Filtrar usuários que responderam a uma campanha específica de SMS, MMS ou RCS ou componente do Canvas, categoria de palavra-chave ou tag.

**Filtrar por resposta a uma campanha específica com categoria de palavra-chave**<br>
...
![Campanha com o filtro "Has replied to SMS" para campanha ou Canva com a tag "Curbside Messaging Service C".]({% image_buster /assets/img/sms/clicked_opened_campaign_canvas_tag.png %})

**Filtrar por resposta a uma etapa específica**
![Campanha com o filtro "Respondeu o SMS" para a etapa "Aceitação dupla de SMS" "Etapa – Ajuda".]({% image_buster /assets/img/sms/clicked_opened_step.png %})
...
{% alert tip %}
Se um Canvas baseado em ações for disparado por uma mensagem de SMS ou MMS recebida, você pode referenciar [propriedades Liquid de SMS suportadas]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/supported_personalization_tags/) em qualquer etapa do Canvas até o próximo caminho de ação.
{% endalert %}

**Disparar por categoria de palavra-chave de entrada**<br>
![Campanha de SMS baseada em ações com o filtro de segmentação Palavra-chave enviada "Aceitação" para o grupo de inscrições "Marketing por SMS".]({% image_buster /assets/img/sms/retargeting2.png %}){: style="margin-top:10px;"}
...
![Campanha de mensagens SMS baseada em ações com categoria de palavra-chave como "Outros", em que o corpo da mensagem é exatamente "Olá" ou "Ei".]({% image_buster /assets/img/sms/retargeting3.png %}){: style="margin-top:10px;"}
