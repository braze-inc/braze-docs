...
| Envio de mensagens transacionais | As mensagens SMS são uma maneira eficiente de enviar notificações transacionais, como confirmações de pedidos e informações de remessa, fornecendo a eles todas as informações de que precisam em um local conveniente. Observe que existem diretrizes legais que devem ser observadas no envio de mensagens transacionais. Se não tiver certeza sobre essas diretrizes, entre em contato com a equipe jurídica interna.|
