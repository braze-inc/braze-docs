...
Mensagens RCS são cobradas por mensagem. Entender os tipos de mensagens RCS cobradas é fundamental para entender seu faturamento.

### Tipos de mensagens RCS cobradas
...
O tipo de cobrança correspondente será exibido dentro do criador de mensagem RCS em um rótulo que tem um dos dois valores: **Texto apenas RCS** (RCS Básico) e **RCS** (RCS Único).

Os dados do seu tipo de cobrança RCS serão preenchidos no seu [dashboard de Uso de Mensagens]({{site.baseurl}}/message_usage_dashboard/), que exibe seu consumo de créditos de mensagem especificando sua proporção de crédito e número de créditos de mensagem usados. 