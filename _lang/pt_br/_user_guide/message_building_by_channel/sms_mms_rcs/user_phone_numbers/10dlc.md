...
> A2P 10DLC refere-se a um sistema nos Estados Unidos que permite que as empresas enviem mensagens do tipo Application-to-Person (A2P) por meio de um número de telefone padrão de código longo de 10 dígitos (10DLC). Esses códigos longos registrados têm maior rendimento, melhor capacidade de entrega e melhor conformidade do que o código longo padrão.
...
O serviço 10DLC foi criado para facilitar especificamente o envio de mensagens A2P usando códigos longos. Historicamente, os códigos longos eram destinados a mensagens pessoa a pessoa (P2P), mas, quando usados por motivos de marketing, causavam restrições às empresas devido à taxa de transferência limitada e ao aumento da filtragem. 
...
- **Melhor capacidade de entrega**: Os números 10DLC são designados para o tráfego A2P, portanto, as mensagens enviadas com esses números têm maior probabilidade de chegar ao destinatário e são menos propensas a serem filtradas ou rejeitadas pela operadora do que as mensagens enviadas por meio de códigos longos locais regulares. 
- **Conformidade aprimorada**: O uso de um código longo local para mensagens de texto comerciais é contra as diretrizes [da CTIA](https://api.ctia.org/wp-content/uploads/2019/07/190719-CTIA-Messaging-Principles-and-Best-Practices-FINAL.pdf). Os números 10DLC foram designados para mensagens em massa e permitem que as marcas cumpram as normas do setor sem depender de códigos curtos.
- **Econômico**: O 10DLC é uma ótima opção para empresas que desejam começar a enviar SMS ou enviar SMS em pequenos volumes. Para marcas que enviam volumes maiores de mensagens, acima de 100.000 mensagens por dia, recomendamos o uso de um código curto. 

Desde 2019, as operadoras começaram a adotar o 10DLC para mensagens comerciais, com a Verizon e o AT&T atualmente oferecendo suporte ao 10DLC, e esperamos que todas as principais operadoras o sigam em breve. Embora isso possa causar inconvenientes a curto prazo, a longo prazo, os clientes terão melhores taxas de entrega e, ao mesmo tempo, protegerão seus consumidores de mensagens indesejadas. 
...
| Tarifas da operadora | Taxas cobradas pelas operadoras por mensagens SMS e MMS enviadas aos usuários após o registro do 10DLC. A partir de 1º de outubro de 2021, as taxas da operadora serão mais altas para o tráfego não registrado (códigos longos padrão) do que para o tráfego registrado (10DLC). |
...
A taxa de transferência de mensagens para seu 10DLC depende de vários fatores, incluindo a pontuação de confiança da marca, os limites diários de mensagens e seus casos de uso de mensagens.

#### Índice de confiança na marca {#trust-score}

O The Campaign Registry (TCR) é uma agência terceirizada que usa um algoritmo de reputação para analisar critérios específicos relacionados à sua empresa e atribuir uma pontuação de confiança que determina o rendimento das mensagens para cada marca. Essa pontuação de confiança será atribuída quando um cliente se registrar para receber mensagens do US 10DLC. Quanto maior for a pontuação de confiança, melhor será o número de mensagens por segundo (MPS) que você receberá. 

|     | Índice de confiança | AT&T | T-Mobile | Verizon |
| --- | ----------- | ---- | -------- | ------- |
| Alta | 75-100 | 75 MPS | 75 MPS | 75 MPS |
| Médio | 50-74 | 40 MPS | 40 MPS | 40 MPS |
...
A taxa de transferência também é afetada pelo tipo de caso de uso de mensagens que você escolher. A maioria dos clientes se enquadra no caso de uso de marketing padrão ou marketing misto. Outros casos de uso menos comuns serão suscetíveis a valores de taxa de transferência diferentes.
...
| Misto | Campanha que abrange vários casos de uso, como o Atendimento ao Cliente. | 
...
| Alertas de fraude | Mensagens sobre atividades potencialmente fraudulentas. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

{% tabs %}
{% tab Declared Use Case %}
Um caso de uso declarado significa que você escolheu um caso de uso específico que não seja de marketing (por exemplo, 2FA ou Notificações de conta).
...
Os casos de uso de marketing misto podem ser registrados para clientes que desejam enviar mensagens para vários casos de uso a partir do mesmo conjunto de números ou para marketing.
