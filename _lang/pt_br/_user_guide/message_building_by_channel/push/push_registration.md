...
| Push em segundo plano | Não        | Uma notificação é entregue silenciosamente em segundo plano sem ser exibida. Frequentemente usado para funcionalidades como rastreamento de desinstalação. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

Quando um usuário aceita notificações por push para seu app, ele será considerado "registrado para push", significando que agora pode ser segmentado usando o filtro de segmentação `Foreground Push Enabled for App` no Braze.

{% alert note %}
Isso é diferente do filtro de segmentação `Foreground Push Enabled`, que é usado para identificar usuários que aceitaram pelo menos um dos seus apps—não um app específico. Para mais informações, veja [Filtros de segmentação]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/#foreground-push-enabled).
{% endalert %}

### Múltiplos usuários em um dispositivo

Tokens por push são únicos tanto para o dispositivo quanto para o app, significando que tokens por push não podem ser usados para segmentar usuários específicos se múltiplos usuários estiverem usando o mesmo dispositivo.
...
![O changelog do token por push na guia \*\*Engagement** do perfil de um usuário, que lista quando o token por push foi movido para outro usuário e qual era o token.]({% image_buster /assets/img/push_token_changelog.png %})
...
{% tab android %}
Quando seu app é instalado, um token por push é gerado automaticamente para seu app—no entanto, ele só pode ser usado para [notificações por push em segundo plano](#foreground-vs-background) até que o usuário opte explicitamente por participar. Além disso, o registro é tratado de maneira diferente em diferentes versões do Android:
...
{% tab ios %}
O iOS não gera automaticamente tokens por push para um app quando é instalado. Além disso, o registro é tratado de maneira diferente em diferentes versões do iOS: 
...
### Verificando o estado de inscrição por push do usuário
...
![Changelog do push com um exemplo de token por push.]({% image_buster /assets/img/push_changelog.png %}){: style="float:right;max-width:40%;margin-left:15px;margin-top:10px;"}
