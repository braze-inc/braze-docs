...
Se quiser categorizar suas mensagens e agrupá-las na bandeja de notificações do usuário, você pode utilizar o recurso Notification Groups do iOS por meio da Braze.

Crie sua campanha de push para iOS, depois acesse a guia **Configurações** e abra o dropdown **Grupo de Notificação**.

![A guia "Configurações" com um dropdown "Grupo de Notificação" que selecionou um valor de "Cupons".]({% image_buster /assets/img_archive/notification_group_dropdown.png %}){: style="max-width:50%;" }
...
![Janela para gerenciar grupos de notificações push para iOS.]({% image_buster /assets/img_archive/managenotgroups.png %}){: style="max-width:70%;" }
...
Além de agrupar as notificações por IDs de thread, a Apple permite que você edite os resumos que aparecem quando as notificações são agrupadas. Os usuários da Braze podem especificar a categoria de resumo, a contagem de resumo e o argumento de resumo ao criarem uma campanha push usando nossa ferramenta.
...
A Apple permite que as marcas enviem notificações por push silenciosas para as centrais de notificações dos usuários antes da aceitação oficial e explícita, o que dá a chance de demonstrar o valor das suas mensagens com antecedência. Tudo o que você precisa fazer é [configurar notificações por push provisórias](#set-up-provisional-push-notifications) em seu app e, em seguida, qualquer usuário que tenha um token por push provisório receberá suas mensagens.

Diferente de um token por push tradicional para iOS, um token por push provisório atua como um "passe de teste" que permite que marcas alcancem novos usuários antes que eles tenham visto e clicado no pedido de aceitação nativo de push da Apple. Com esse recurso, sua notificação por push será entregue diretamente na bandeja de notificações do novo usuário, com a opção de "Manter" ou "Desativar" futuras notificações. Em vez de experimentar uma jornada de "aceitação", os usuários experimentarão algo mais parecido com uma jornada de "exclusão".
...
Qualquer que seja a opção escolhida pelo usuário, o token ou [o status de inscrição]({{site.baseurl}}/user_guide/message_building_by_channel/push/users_and_subscriptions/) apropriado será adicionado às suas [Configurações de contato]({{site.baseurl}}/user_guide/engagement_tools/segments/using_user_search/#engagement-tab) na guia **Engajamento** do perfil de usuário.
...
![Painel de Informações do Segmento com o filtro de segmento de amostra "Autorizado Provisoriamente no iOS Stopwatch (iOS) é verdadeiro" para segmentar usuários.]({% image_buster /assets/img/segment-push-prov-auth.png %})

{% alert tip %}
Se os usuários optarem por "Desativar" o push provisório, eles não verão mais nenhuma mensagem provisória por push sua. Seja cuidadoso com o conteúdo e a cadência das mensagens enviadas usando essa funcionalidade!
{% endalert %}

{% alert important %}
Se você usar prompts de push adicionais ou [primers de push no app](https://www.braze.com/resources/glossary/priming-for-push/) (uma mensagem no app que incentiva os usuários a aceitarem notificações por push), entre em contato com seu representante da Braze para orientações adicionais.
{% endalert %}

#### Configurar notificações por push provisórias

A Braze permite que você se registre para a autenticação provisória atualizando seu código no snippet de registro de token na implementação do SDK da Braze para iOS usando os seguintes snippets como exemplo (envie-os aos seus desenvolvedores ou garanta que eles [implementem a autenticação provisória por push durante o processo de integração]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/integration/#using-usernotification-framework-ios-10)).
...
Com o novo Modo Foco do iOS 15, os usuários têm mais controle sobre quando as notificações do app podem "interrompê-los" com um som ou vibração.

![Página de Configurações de Notificações do iOS que mostra notificações habilitadas para entrega imediata e com notificações sensíveis ao tempo habilitadas.]({% image_buster /assets/img/ios/ios15-notification-settings.png %}){: style="max-width:40%"}

Os apps agora podem especificar o nível de interrupção que uma notificação deve incluir, com base em sua urgência.

Para alterar o nível de interrupção de uma notificação por push do iOS, selecione a guia **Configurações** e escolha o nível desejado no menu suspenso **Nível de interrupção**.
...
|Nível de interrupção|Descrição|Quando usar|Ignora o Modo Foco|
|--|--|--|--|
...
|[Ativo](https://developer.apple.com/documentation/usernotifications/unnotificationinterruptionlevel/active) (padrão)|Só emitirá um som, uma vibração e ligará a tela se o usuário não estiver no Modo Foco.|Notificações que exigem atenção imediata, a menos que o usuário tenha o Modo Foco ativado.|Não|
|[Sensível ao tempo](https://developer.apple.com/documentation/usernotifications/unnotificationinterruptionlevel/timesensitive)|Emite um som, vibra e liga a tela mesmo no Modo Foco. Requer que o **recurso de Notificações sensíveis ao tempo** seja adicionado ao seu app no Xcode.|Notificações oportunas que devem alertar os usuários independentemente do Modo Foco, como uma notificação de compartilhamento de carona ou entrega.|Sim|
...
![Um resumo de notificações para iOS intitulado "Seu Resumo da Noite" com três notificações.]({% image_buster /assets/img/ios/ios15-notification-summary.png %}){: style="float:right;max-width:25%;margin-left:15px;border:0"}
...
Os apps podem especificar quais notificações por push são mais relevantes, definindo uma **pontuação de relevância**. A Apple usará essa pontuação para determinar quais notificações devem ser exibidas no Resumo de Notificações programado, enquanto outras ficam disponíveis quando os usuários clicam no resumo. 

Todas as notificações ainda poderão ser acessadas na central de notificações do usuário.

Para definir a pontuação de relevância de uma notificação do iOS, insira um valor entre `0.0` e `1.0` na guia **Configurações**. Por exemplo, a mensagem mais importante deve ser enviada com `1.0`, enquanto uma mensagem de importância média pode ser enviada com `0.5`.
...
- [Especificações de imagem e texto para push]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/media_library/#push)
