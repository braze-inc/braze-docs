...
Quando um usuário sai de um dispositivo ou site, ele continuará acessível por push até que outro usuário faça login. Nesse ponto, o token por push é reatribuído ao novo usuário. Isso ocorre porque cada dispositivo pode ter apenas uma inscrição ativa por push por app ou site.
...
Esta mensagem indica que o APNs rejeitou a solicitação de push devido a uma carga útil inválida (por exemplo, uma carga útil vazia ou uma carga útil que é muito grande).

Para detalhes e próximos passos, veja [Mensagens de Erro Comuns de Push]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_error_codes/).
...
2. Dispare um novo envio por push. Isso moverá o token de volta para a conta se eles ainda tiverem o push ativado no nível do dispositivo.
