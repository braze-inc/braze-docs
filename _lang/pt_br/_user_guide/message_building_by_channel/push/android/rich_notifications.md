---
nav_title: Crie notificações ricas
...
# Crie notificações push ricas para Android
...
- As imagens de notificação estendidas do Android devem ter uma proporção de 2:1, mas não têm um limite de tamanho.
...
- O SDK do Braze para Android deve estar habilitado para que a imagem seja renderizada.
...
Adicione o **Texto Resumido** que você gostaria de exibir antes da imagem na notificação.

![Uma notificação push rica de um aplicativo de ração para pets chamado Dog indicando que é hora de pedir mais ração para o Spot com texto resumido.]({% image_buster /assets/img_archive/android_rich_summarytext.png %})

### Etapa 3: Adicionar mídia

Adicione sua imagem no campo **Imagem de Notificação do Android** no criador da mensagem. As imagens podem ser feitas upload diretamente pelo dashboard ou especificando um URL de conteúdo que esteja hospedado em outro lugar.
...
![A seção de imagem de notificação do Android onde você pode adicionar uma imagem ou inserir uma URL de imagem.]({% image_buster /assets/img_archive/android_rich_image.png %})
