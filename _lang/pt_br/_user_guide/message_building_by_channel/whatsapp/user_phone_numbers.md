...
page_order: 1.5
description: "Este artigo de referência aborda a formatação de números de telefone do WhatsApp, como importar números de telefone e como adicionar usuários a grupos de assinatura do WhatsApp."
page_type: reference
channel: 
  - WhatsApp
...
Você pode importar números de telefone fazendo [o upload de um CSV]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_import/#csv) ou [via API]({{site.baseurl}}/api/endpoints/user_data/#user-track-endpoint) para criar um usuário.
...
Se um número de telefone for coagido para o formato E.164, mas não passar na validação, o Braze não poderá enviar mensagens do WhatsApp para esse número. Todos os usuários com números de telefone que não são formatáveis sairão automaticamente de uma etapa do Canvas que inclui o WhatsApp
...
Você pode ver as diferenças entre a formatação de números locais e a formatação universal, `E.164`, na tabela a seguir:
...
### Adição de usuários a um grupo de assinatura do WhatsApp

Para que um cliente receba uma mensagem do WhatsApp, ele deve ter um número de telefone válido e ter optado por participar de um grupo de assinatura. Para obter mais informações, consulte [Grupos de assinatura do WhatsApp]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/user_subscription/).


### Vários usuários com o mesmo número de telefone

Se vários usuários tiverem o mesmo número de telefone em um segmento de uma única campanha ou etapa do Canvas, o Braze desduplicará o envio e enviará apenas uma mensagem para esse número de telefone. 
