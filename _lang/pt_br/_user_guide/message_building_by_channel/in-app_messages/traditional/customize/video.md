...
> Para reproduzir um vídeo em uma mensagem no app em HTML, inclua o seguinte elemento `<video>` no HTML e substitua os nomes dos vídeos pelo nome do arquivo (ou o URL do ativo remoto). Você pode encontrar outras opções possíveis em `<video>` nos [documentos da Web da MDN][9].
...
Para incorporar vídeo e outros conteúdos HTML5 em mensagens HTML no app no Android, é necessário ativar a aceleração de hardware na atividade em que a mensagem no app é exibida. Para saber mais, consulte o [guia do desenvolvedor do Android]({{site.baseurl}}/developer_guide/platform_integration_guides/android/in-app_messaging/customization/youtube_in_html/).
