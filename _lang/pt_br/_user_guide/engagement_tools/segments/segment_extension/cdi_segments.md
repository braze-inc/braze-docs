...
{% alert warning %}
As extensões de segmento CDI consultam diretamente o seu data warehouse, portanto, você arcará com todos os custos associados à execução dessas consultas no seu data warehouse. As extensões de segmento CDI não consomem [créditos de segmento SQL]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments/#monitoring-your-sql-segments-usage), não contam para o limite de extensão de segmento e não realizam registros de pontos de dados.
{% endalert %}

## Pré-requisitos

Para usar os dados do data warehouse para segmentação no espaço de trabalho do Braze, você precisará criar uma [fonte conectada]({{site.baseurl}}/user_guide/data/cloud_ingestion/connected_sources/) e, em seguida, criar um segmento CDI nas [extensões de segmento]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/). As extensões de segmento CDI permitem que você escreva SQL que realiza consultas diretas a seu próprio data warehouse usando dados disponibilizados por meio de suas conexões CDI e crie um grupo de usuários que podem ser direcionados no Braze.
...
Como parte da configuração do CDI, você pode selecionar entre diferentes conexões para usar nas extensões de segmento do CDI. Cada conexão tem um conjunto específico de tabelas de dados. Sua equipe de desenvolvimento pode configurar suas conexões e tabelas de dados durante a configuração do CDI.

Para visualizar as tabelas de dados disponíveis, incluindo seu esquema e quaisquer descrições disponíveis, selecione **Referência**. Quando estiver pronto, selecione uma conexão.
...
Lembre-se de que todas as extensões de segmento CDI devem usar`external_user_id`  como coluna selecionada, e seu`external_user_id`  deve corresponder ao definido no Braze para usuários.

{% alert important %}
`external_user_id` deve ser um valor **da string**. Se o seu ID de origem estiver armazenado como um número (por exemplo,`client_id`como um inteiro), [converta-o em uma string no seu SQL](https://www.w3schools.com/sql/func_sqlserver_cast.asp) para que corresponda ao`external_id`tipo no Braze.
{% endalert %}

Se os resultados da sua consulta incluírem usuários que não existem no Braze, esses usuários serão ignorados. O Braze não cria novos usuários com base na saída da sua extensão de segmento CDI.

{% alert tip %}
Para saber como obter uma prévia de suas extensões de segmento, gerenciá-las e executar atualizações automáticas de associação, consulte [Extensões de segmento SQL]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments/).
{% endalert %}
