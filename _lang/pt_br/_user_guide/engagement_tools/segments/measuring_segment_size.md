...
Cada segmento exibe o número total de usuários que são membros desse segmento. Ao filtrar por **Usuários de todos os apps**, também exibe alguns dos canais de envio de mensagens mais utilizados (como web push ou e-mail) e o número de usuários alcançáveis para esses canais específicos. 
...
![Uma tabela exibindo o total de usuários alcançáveis, divididos por usuários alcançáveis por e-mail, push iOS, push Android, web push e push Kindle.]({% image_buster /assets/img_archive/segmenter_reachable_users.png %})
...
A métrica _Total_ representa usuários únicos. Por exemplo, se um usuário tem tanto push do Android quanto push do iOS, ele será contado para ambas as linhas, mas contará como 1 usuário na linha _Total_.
...
Para atualizar as estatísticas de um cálculo que você executou anteriormente, selecione **Atualizar estatísticas exatas**. A data em que este cálculo foi executado por último será atualizada automaticamente.
...
As estatísticas em nível de filtro sempre serão estimadas, mesmo que você calcule estatísticas exatas. **Calcular estatísticas exatas** calcula apenas as estatísticas exatas no nível do segmento, não no nível do filtro ou grupo de filtros. Esse cálculo pode levar alguns minutos para ser concluído. Espaços de trabalho maiores, em particular, podem exigir períodos mais longos para concluir os cálculos. Você pode acompanhar seu progresso na barra de progresso no painel **Usuários alcançáveis**. Quando um cálculo deve levar mais de cinco minutos, a Braze enviará um e-mail com os resultados. 

A Braze prioriza um cálculo por vez por espaço de trabalho, então executar múltiplos cálculos ao mesmo tempo causará atrasos. Você pode selecionar **Ver fila de cálculos** para ver quais segmentos estão à frente do seu, seu progresso e seu iniciador, e ter uma ideia de quando seu cálculo pode ser priorizado.

![Uma fila de cálculos com um cálculo.]({% image_buster /assets/img_archive/calculation_queue.png %})

Você pode cancelar um cálculo de estatísticas exatas selecionando **Cancelar**. Isso pode ser benéfico se houver múltiplos cálculos na fila e você quiser priorizar outro cálculo primeiro. 
...
A contagem de membros pode mudar significativamente por uma série de razões, como as listadas nesta tabela.

| Motivo | Exemplo |
| --- | --- |
| Comportamento normal do usuário | Usuários se inscrevem após uma campanha particularmente bem-sucedida. |
| Usuários são importados por CSV | Um arquivo CSV de usuários foi importado, aumentando significativamente a contagem de membros do segmento. |
