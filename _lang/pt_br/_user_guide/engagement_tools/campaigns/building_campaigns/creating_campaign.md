---
nav_title: Criar uma campanha
article_title: Criar uma campanha
layout: dev_guide
page_order: 0
guide_top_header: "Criar uma campanha"
guide_top_text: "Agora que você entende o básico das campanhas na Braze, confira os diferentes <a href='/docs/user_guide/message_building_by_channel/'>canais de envio de mensagens</a> disponíveis para ajudar a decidir que tipo de mensagens você gostaria de enviar. Selecione um canal de envio de mensagens da lista a seguir para aprender como criar e compor uma campanha de mensagens de canal único desse tipo, ou combine tipos criando uma campanha multicanal.<br><br>Também recomendamos fortemente que você confira nosso curso do Braze Learning <a href='https://learning.braze.com/campaign-setup-delivery-targeting-conversions'>Configuração de Campanha</a>!"
page_type: landing
description: "Esta landing page inclui links para artigos sobre como criar campanhas para diferentes canais de envio de mensagens da Braze."

search_rank: 1
guide_featured_title: "Selecione um Tipo de Campanha"
guide_featured_list:
- name: Campanha de cartão de conteúdo
  link: /docs/user_guide/message_building_by_channel/content_cards/create/
  image: /assets/img/braze_icons/channel-content-cards.svg
- name: Campanha de e-mail
  link: /docs/user_guide/message_building_by_channel/email/html_editor/creating_an_email_campaign/
  image: /assets/img/braze_icons/channel-email.svg
- name: Campanha de mensagem no app
  link: /docs/user_guide/message_building_by_channel/in-app_messages/traditional/create/
  image: /assets/img/braze_icons/channel-in-app-messages.svg
- name: Campanha por push
  link: /docs/user_guide/message_building_by_channel/push/creating_a_push_message/
  image: /assets/img/braze_icons/channel-mobile-push.svg
- name: Campanha de SMS
  link: /docs/user_guide/message_building_by_channel/sms/campaign/create/
  image: /assets/img/braze_icons/channel-sms.svg
- name: Campanha de MMS
  link: /docs/user_guide/message_building_by_channel/sms/mms/create/
  image: /assets/img/braze_icons/channel-sms.svg
- name: Campanha de webhook
  link: /docs/user_guide/message_building_by_channel/webhooks/creating_a_webhook/
  image: /assets/img/braze_icons/channel-webhook.svg
- name: Campanha multicanal
  link: /docs/user_guide/engagement_tools/campaigns/faq/#how-do-you-create-a-multichannel-campaign
  image: /assets/img/braze_icons/plus.svg

---