...
Para usar com sucesso este [modelo]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events), você precisará de um [evento personalizado]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events) que faça referência a quando os usuários usaram o recurso.
...
Para acessar o modelo de volta ao estoque, ao criar um novo canva, selecione **Usar um modelo de canva** > **Modelos de Braze**. Em seguida, ao lado de **Adoção de Recursos**, selecione **Aplicar Modelo**. Agora, podemos passar pelo modelo para adaptá-lo às nossas necessidades.

### Etapa 1: Defina os detalhes
...
![O novo nome e descrição para o canva. A nova descrição afirma: Uma adoção de canva para rastrear a adoção e o sentimento do usuário para o Controle de Cruzeiro, uma funcionalidade para agendar entregas de alimentos recorrentes.
...
Após testar e revisar nosso canva para garantir que funcione como esperado, selecione **Iniciar Canva** para iniciar o canva. Agora, podemos direcionar os usuários com uma jornada personalizada para incentivá-los a adotar nosso novo recurso Controle de Cruzeiro.

{% alert tip %}
Confira nossa [Lista de verificação pré e pós-lançamento]({{site.baseurl}}/user_guide/engagement_tools/canvas/ideas_and_strategies/pre_post_launch_checklist/#things-to-consider-before-launch) para coisas a considerar antes e depois de lançar um canva.
{% endalert %}
