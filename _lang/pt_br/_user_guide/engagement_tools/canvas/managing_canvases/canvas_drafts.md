...
{% alert note %}
Se um bloco de conteúdo for referenciado em um rascunho de canva, o canva é listado na contagem de inclusão do bloco de conteúdo. No entanto, se o bloco de conteúdo for referenciado em um rascunho de um **ativo** canva, o canva não será listado na contagem de inclusão do bloco de conteúdo.
{% endalert %}

### Priorização de mensagens no app

Para rascunhos de um Canvas ativo, a prioridade da mensagem no app dentro do construtor do Canvas será atualizada imediatamente quando um usuário alterar a prioridade. Isso significa que a prioridade das mensagens no app no nível do Canvas é aplicada imediatamente ao Canvas ativo, mesmo quando existe um rascunho. 
