...
A seleção de **Duration** permite a postergação de usuários por um número definido de segundos, minutos, horas, dias ou semanas, e em um horário específico. Por exemplo, é possível fazer a postergação de usuários por quatro horas ou por um dia.
...
{% alert important %}
Se estiver participando do [acesso antecipado do Canva Context]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/context), poderá definir postergações de até 2 anos.
{% endalert %}
...
Se o Canva tiver duas etapas de postergação, mas a primeira etapa de postergação for maior que a segunda etapa de postergação, os usuários também sairão do Canva. 
...
A seleção de **Dia da semana** permite manter os usuários na etapa até um dia específico da semana, em um horário específico. Por exemplo, é possível fazer a postergação dos usuários até que a próxima quinta-feira chegue às 16 horas no fuso horário da empresa. 
...
Ao configurar postergações personalizadas com a opção de **horário específico**, o comportamento do fuso horário depende do tipo de dados de sua atribuição ou variável de contexto:
...
![O site "product_reminder_interval" e seu valor.]({% image_buster /assets/img/context_step1.png %})
