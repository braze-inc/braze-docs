...
Para melhores resultados, use o agente de Otimizador de Conteúdo em Canvases onde os usuários entram na etapa gradualmente ao longo do tempo. Se todos os usuários entrarem na etapa de uma vez, o agente não terá tempo para aprender com os resultados iniciais. 
...
A mensagem base é o ponto de partida para sua etapa. As variantes para cada componente de conteúdo são inseridas dinamicamente com base nas combinações definidas na aba **Configurações do Otimizador de Conteúdo**. 

{% alert note %}
Durante o período beta, o e-mail é o único canal suportado.
{% endalert %}

Na aba **Canais de Mensagens**, selecione **E-mail** e crie sua mensagem base de e-mail. Consulte nossa seção dedicada [E-mail]({{site.baseurl}}/user_guide/message_building_by_channel/email) para ajuda. 

O agente de Otimizador de Conteúdo usa as configurações de envio (como o domínio de e-mail e o endereço de resposta) especificadas nesta variante para enviar todas as mensagens. Você pode começar com um novo design ou selecionar um modelo existente para esta mensagem. Nesta etapa, considere quais componentes da mensagem você deseja otimizar. Você definirá isso em [etapa 4](#step-4).
...
Após definir pelo menos duas variantes para cada componente, copie a tag Liquid associada para cada uma e cole-a na localização correspondente na sua mensagem base.
...
Se você não adicionar uma tag Liquid para um componente de conteúdo selecionado, verá um aviso na aba **Configurações do Otimizador de Conteúdo** e um erro na aba **Canais de Mensagens**. O Canvas não pode ser iniciado até que todos os componentes selecionados sejam adicionados corretamente à sua mensagem base.
...
Para e-mail, você pode otimizar para um dos seguintes eventos. O agente usa aberturas e cliques que são registrados dentro de 7 dias após o envio de uma mensagem para direcionar a entrega para combinações de conteúdo de melhor desempenho.
...
| Cliques | Otimiza para combinações que impulsionam o engajamento com links. Não inclui cliques de bots ou cliques de cancelamento de inscrição reconhecidos pelo Braze. | Gerando tráfego, engajamento ou conversão a partir de links |
...
Para revisar o desempenho, abra o painel de análise de nível de etapa para ver métricas por variante de conteúdo e desempenho geral da combinação. A etapa do Otimizador de Conteúdo usa a [mesma análise que a etapa de Mensagem]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step/#analytics).

![Análise do Otimizador de Conteúdo para três botões e a porcentagem de alocação de envios, que tende a aumentar.]({% image_buster /assets/img/content_optimizer/content_optimizer_analytics.png %})

## Solução de problemas

| Problema | Descrição | Consertar |
| --- | --- | --- |
| Tags Liquid ausentes | Se você adicionar um componente de conteúdo (como Assunto ou CTA) mas não inserir a tag Liquid correspondente na sua mensagem base, você verá: <br>\- Um aviso na aba **Configurações do Otimizador de Conteúdo** <br>\- Um erro na aba **Canais de Mensagem** | Copie o trecho Liquid mostrado sob cada componente na aba **Configurações do Otimizador de Conteúdo** e cole-o na parte apropriada da sua mensagem. |
| Tags Liquid órfãs | Se você excluir um componente de conteúdo, mas deixar sua tag Liquid na mensagem base, a mensagem pode não ser renderizada como esperado ao ser enviada. | Remova quaisquer tags `message_component` não utilizadas da sua mensagem base antes de lançar. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
