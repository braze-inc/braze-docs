...
Escolha o tipo de evento de conversão e, em seguida, selecione as conversões a serem registradas.  
...
Os controles de entrada determinam se os usuários têm permissão para entrar novamente em um Canva.  
...
O `canvas_entry_properties` é configurado na etapa do Cronograma de entrada da criação de um canva e indica o disparo que insere um usuário em um canva. Essas propriedades também podem acessar as propriedades de cargas úteis de entrada em canvas disparados por API.  
...
O Intelligent Selection for Canvas otimiza os resultados do Canvas, fazendo ajustes graduais em tempo real na distribuição de usuários classificados em cada variante.  
