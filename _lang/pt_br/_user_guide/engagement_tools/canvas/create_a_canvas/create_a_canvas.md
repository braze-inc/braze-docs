...
O construtor do Canvas vai guiar você passo a passo na configuração do canva — desde a nomeação até a definição de eventos de conversão e a inclusão dos usuários certos na jornada do cliente. Selecione cada uma das guias a seguir para visualizar as configurações que podem ser ajustadas em cada etapa do construtor.

{% tabs local %}
  {% tab Basics %}
    Aqui, você configurará o básico do seu canva:
  ...
  {% tab Entry Schedule %}
    ...
    - Baseado em ação: Seu usuário entrará no canva depois de executar uma ação definida
    - Disparado pela API: Use uma solicitação de API para inserir usuários no seu canva
  ...
  {% tab Send Settings %}
    ...
    - Defina um limite de taxa de envio para as mensagens do canva
...
{% alert tip %}
Coloque tags nos seus canvas para que seja fácil encontrá-los e criar relatórios. Por exemplo, ao usar o [Report Builder]({{site.baseurl}}/user_guide/analytics/reporting/report_builder/), você pode filtrar por tags específicas.
{% endalert %}
...
Escolha o tipo de evento de conversão e selecione as conversões a serem registradas. Esses [eventos de conversão]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/conversion_events/) medirão a eficiência do seu canva. 

![Evento de conversão primária A com o tipo de evento de conversão Makes Purchase para registrar conversões de usuários que fazem qualquer compra dentro de um prazo de conversão de três dias.]({% image_buster /assets/img/add_canvas_conversions.png %})
...
#### Etapa 1.2: Determine o cronograma de entrada no canva
...
{% tabs local %}
  {% tab Scheduled Delivery %}
    Com a entrega programada, os usuários entrarão em uma programação de tempo, da mesma forma que você programaria uma campanha. É possível inscrever usuários em um canva assim que ele for lançado, inseri-los na jornada em algum momento no futuro ou de forma recorrente (diária, semanal ou mensal). 
    
   Se você selecionar uma programação recorrente mensal, observe que alguns meses podem não ter o dia selecionado. Por exemplo, digamos que você configurou um canva para enviar mensalmente no dia 31. Nesse cenário, a Braze envia no último dia daquele mês, como 30 de abril, porque 31 de abril não existe.
  ...
  {% tab Action-Based Delivery %}
    ...
    Você pode controlar outros aspectos do comportamento do canva na janela **Público de Entrada**, incluindo regras de reelegibilidade e configurações de limite de frequência. Observe que a entrega baseada em ação não está disponível para componentes do Canvas com mensagens no app.
  ...
  {% tab API-Triggered Delivery %}
    ...
    ![Um exemplo de entrega disparada pela API com um Canvas ID e um exemplo de solicitação cURL.]({% image_buster /assets/img_archive/Canvas_API_Triggered_Delivery.png %})

    Você pode usar os seguintes endpoints para entrega disparada pela API:
...
Depois de selecionar o método de entrega, ajuste as configurações para corresponder ao seu caso de uso e continue definindo o público-alvo.
...
Apenas os usuários que correspondem aos critérios definidos podem entrar na jornada na etapa **Público-Alvo**, o que significa que a Braze avalia a elegibilidade do público-alvo primeiro, **antes** que os usuários entrem na jornada do canva. Por exemplo, se quiser direcionar novos usuários, você pode selecionar um segmento de usuários que usaram seu app pela primeira vez há menos de uma semana.
...
Os controles de entrada determinam se os usuários têm permissão para entrar novamente em um canva. Você também pode limitar o número de pessoas que potencialmente entrariam neste canva por uma cadência selecionada, dependendo do tipo de programação de entrada:

- **Programado:** Duração do canva ou toda vez que o canva estiver programado
- **Baseado em ação:** Por hora, diário ou a duração do canva
- **Disparado pela API:** Por hora, diário ou a duração do canva

Por exemplo, se você tiver um canva baseado em ação e selecionar **Limitar volume de entrada** e definir o campo **Entradas máximas** para 5.000 usuários com **Diário** como a cadência limite, então o canva enviará apenas para 5.000 usuários por dia.

![A página "Controles de Entrada" exibindo caixas de seleção para "Permitir que usuários reentrem no Canvas" e "Limitar volume de entrada". Este último permite que você defina as entradas máximas e escolha uma cadência que depende do tipo de programação de entrada (por exemplo, duração do Canvas ou toda vez que o Canvas estiver programado para entrada programada, e por hora, diário ou duração do Canvas para entrada baseada em ação e disparada por API).]({% image_buster /assets/img_archive/entry_controls.png %})
...
- **O usuário A** está inscrito no e-mail e tem o push ativado. Esse usuário não recebe o e-mail, mas receberá o push.
- **O usuário B** tem aceitação de e-mail, mas não tem o push ativado. Esse usuário receberá o e-mail, mas não receberá o push.
- **O usuário C** tem aceitação de e-mail e está com o push ativado. Esse usuário receberá tanto o e-mail quanto o push.

Para fazer isso, defina as **Configurações de inscrição** para enviar esse canva como "apenas usuários que aceitaram". Essa opção garantirá que apenas os usuários com aceitação receberão seu e-mail, e a Braze enviará seu push apenas para os usuários que estiverem com o push ativado por padrão. 
...
{% alert tip %}
Economize tempo e agilize a criação do seu canva usando os [modelos do Braze Canvas]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_templates/#available-braze-templates)! Navegue pela nossa biblioteca de modelos pré-criados para encontrar um que se adapte ao seu caso de uso e personalize-o para atender às suas necessidades específicas.
{% endalert %}
...
2. Crie uma campanha de webhook no início do seu canva. Essa campanha será o meio pelo qual você criará seu número aleatório e o armazenará como um atributo personalizado. Consulte [Criar um webhook]({{site.baseurl}}/user_guide/message_building_by_channel/webhooks/creating_a_webhook/#step-1-set-up-a-webhook) para mais informações. Defina a URL para nosso endpoint `/users/track`.<br><br>
3. Crie o gerador de números aleatórios. Você pode fazer isso com o código [especificado aqui](https://community.shopify.com/c/technical-q-a/is-there-any-way-to-generate-random-number-with-liquid-shopify/m-p/1595486), que aproveita o horário único de entrada de cada usuário para criar um número aleatório. Defina o número resultante como uma variável Liquid na sua campanha de webhook.<br><br>
4. Formate a chamada `/users/track` na sua campanha de webhook de modo que ela defina o atributo personalizado criado na etapa 1 como o número aleatório gerado no perfil do usuário atual. Quando essa etapa for executada, você terá criado com sucesso um número aleatório que muda cada vez que um usuário entra na sua campanha.<br><br>
...
Você pode adicionar mais etapas ao fluxo de trabalho do canva arrastando e soltando componentes da barra lateral **Componentes**. Ou selecione o botão <i class="fas fa-plus-circle"></i> de mais para adicionar um componente com o menu popover.
...
Quer editar uma etapa da jornada do usuário? Veja como fazer isso de acordo com o fluxo de trabalho do seu canva!

Você pode editar qualquer etapa no fluxo de trabalho do canva selecionando qualquer um dos componentes. Por exemplo, digamos que você queira editar a primeira etapa, um componente de [postergação]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/), no seu fluxo de trabalho para um dia específico. Selecione a etapa para visualizar suas configurações e ajustar a postergação para 1º de março. Isso significa que, em 1º de março, seus usuários passarão para a próxima etapa do canva.
...
{% alert tip %}
Você sabia que pode incluir nomes de componentes do Canvas nas suas mensagens e modelos de links?<br>
...
Selecione **Concluir** depois de terminar de configurar o componente do canva.

{% tabs local %}
{% tab Canvas Entry Properties %}

O [objeto `context`]({{site.baseurl}}/api/objects_filters/context_object) é configurado na etapa **Programação de Entrada** de criação de um Canvas e indica o gatilho que insere um usuário em um canva. Essas propriedades também podem acessar as propriedades de cargas úteis de entrada em canvas disparados por API. Observe que o objeto `context` pode ter até 50 KB. 

Use o seguinte Liquid ao referenciar essas propriedades criadas ao entrar no canva: {% raw %} ``context.${property_name}`` {% endraw %}. Note que os eventos devem ser eventos personalizados ou eventos de compra para serem usados dessa forma.
...
### Etapa 3: Adicione um grupo de controle
...
Os recursos de Seleção Inteligente agora estão disponíveis em canvas multivariantes. Semelhante ao recurso [Seleção Inteligente]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/) para campanhas multivariantes, a Seleção Inteligente para Canvas analisa a performance de cada variante do canva e ajusta a porcentagem de usuários sendo direcionados por cada variante. Essa distribuição é baseada nas métricas de performance de cada variante para maximizar o total esperado de conversões.

Lembre-se de que os canvas multivariantes permitem que você teste mais do que o texto — também o tempo e os canais. Por meio da Seleção Inteligente, é possível testar canvas com mais eficiência e ter a certeza de que seus usuários serão direcionados para a melhor jornada de Canvas possível.
...
Quando terminar de criar o canva, selecione **Lançar canva** para salvar e iniciar o canva. Depois de lançar o canva, você poderá visualizar a análise de dados da sua jornada à medida que eles chegam na página **Detalhes do Canvas**. 
...
{% alert tip %}
Precisa fazer edições no seu canva após o lançamento? Você pode! Para saber mais, confira [Editando canvas após o lançamento]({{site.baseurl}}/post-launch_edits/).
{% endalert %}