...
> Quando um Canvas é disparado por um evento personalizado, uma compra ou uma chamada de API, você pode usar metadados da chamada de API, do evento personalizado ou do evento de compra para personalização em cada etapa do fluxo de trabalho do Canvas. Você pode usar essas propriedades para enviar mensagens mais curadas.

{% alert important %}
As propriedades de entrada persistentes são um artefato do editor Canvas original, então existem referências depreciadas a termos como propriedades de entrada Canvas que permanecem para referência histórica. Para o editor Canvas atualizado atual, consulte [Propriedades de contexto e evento]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties).
{% endalert %}
...
Nessa solicitação, o valor global de "food allergies" (alergias alimentares) é "none" (nenhuma). Para Customer_123, o valor é "laticínios". Mensagens neste Canvas contendo o trecho Liquid {%raw%}`{{canvas_entry_properties.${food_allergies}}}`{%endraw%} serão modeladas com "laticínios" para Customer_123 e "nenhum" para todos os outros. 
