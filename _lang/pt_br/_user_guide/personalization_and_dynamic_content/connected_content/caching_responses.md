...
O Connected Content armazenará em cache o valor que retorna dos endpoints GET por um mínimo de cinco minutos. Se um tempo de cache não for especificado, o tempo de cache padrão é de cinco minutos.
...
- O armazenamento em cache pode ajudar a reduzir chamadas duplicadas do Connected Content. No entanto, não é garantido que sempre resulte em uma única chamada do Connected Content por usuário.
- O armazenamento em cache do Connected Content é baseado na URL e no espaço de trabalho. Se a chamada do Connected Content for para a URL idêntica, ela pode ser armazenada em cache entre campanhas e canvases.
- O cache é baseado em uma URL única, não em um ID de usuário ou campanha. Isso significa que a versão em cache de uma chamada do Connected Content pode ser usada entre vários usuários e campanhas em um espaço de trabalho se a URL for a mesma.
