...
| :------------------- | :----------------------------------------------------------------------------------------------------------------- | :-------- |
|           | Une os elementos de uma matriz com o caractere passado como parâmetro. O resultado é uma única string.          | ✅ Sim   |
|          | Retorna o primeiro elemento de uma matriz. Em uma matriz de atributos personalizados, esse é o valor adicionado mais antigo.                | ✅ Sim   |
|           | Retorna o último elemento de uma matriz. Em uma matriz de atributos personalizados, esse é o valor adicionado mais recentemente.          | ✅ Sim   |
|        | Remove todos os itens `nil` de uma matriz.                                                                             | ✅ Sim   |
|         | Combina uma matriz com outra matriz.                                                                              | ✅ Sim   |
|          | Retorna o item no local de índice especificado em uma matriz. O primeiro item em uma matriz é referenciado com `[0]`. | ✅ Sim   |
|            | Aceita a atribuição de um elemento da matriz como parâmetro e cria uma matriz a partir do valor de cada elemento da matriz.        | ✅ Sim   |
|        | Inverte a ordem dos itens em uma matriz.                                                                       | ✅ Sim   |
|           | Retorna o tamanho de uma string (o número de caracteres) ou de uma matriz (o número de elementos).                      | ✅ Sim   |
|          | Classifica os elementos de uma matriz por uma determinada atribuição de um elemento da matriz.                                    | ✅ Sim   |
|  | Classifica os itens de uma matriz em ordem alfabética sem distinção entre maiúsculas e minúsculas.                                                | ✅ Sim   |
|          | Remove quaisquer instâncias duplicadas de elementos em uma matriz.                                                           | ✅ Sim   |
|         | Filtra uma matriz para incluir apenas itens com um valor de propriedade específico.                                             | ✅ Sim   |
...
| Filtrar  | Definição      | Com suporte |
| :------ |:----------------| :-------- |
|         | Retorna o valor absoluto de um número.     | ✅ Sim   |
|     | Limita um número a um valor máximo.   | ✅ Sim   |
|    | Limita um número a um valor mínimo.   | ✅ Sim   |
|        | Arredonda uma saída até o número inteiro mais próximo.  | ✅ Sim   |
|  | Divide uma saída por um número. A saída é arredondada para o número inteiro mais próximo. Confira a dica a seguir para evitar o arredondamento. | ✅ Sim   |
|       | Arredonda uma saída para o número inteiro mais próximo.        | ✅ Sim   |
|       | Subtrai um número de uma saída.          | ✅ Sim   |
|        | Adiciona um número a uma saída.     | ✅ Sim   |
|       | Arredonda a saída para o número inteiro mais próximo ou para o número especificado de casas decimais.  | ✅ Sim   |
|      | Multiplica uma saída por um número.       | ✅ Sim   |
|     | Divide uma saída por um número e retorna o restante.   | ✅ Sim   |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
| Filtrar         | Definição          | Com suporte |
| :--------------- | :--------------- | :-------- |
|       | Formata números para garantir que os decimais estejam no lugar correto e que não haja zeros na ponta de nenhum número.   | ✅ Sim   |
|     | Formata números com o símbolo de moeda.     | ⛔ Não    |
|      | Formata números sem o símbolo de moeda.      | ⛔ Não    |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
{% endtabs %}
 ...
 Como solução alternativa, podemos usar o filtro `divided_by` para manipular o número em um decimal, antes de aplicar o filtro de dinheiro:
...
| :--------------- | ------------- | --------- |
|      | Acrescenta caracteres a uma string.           | ✅ Sim   |
|      | Converte uma string em CamelCase.             | ⛔ Não    |
|      | Coloca a primeira palavra em maiúscula em uma string e reduz a minúscula dos caracteres restantes.         | ✅ Sim   |
|       | Converte uma string em letras minúsculas.         | ✅ Sim   |
|     | Escapa uma string.             | ✅ Sim   |
|         | Formata uma string em um identificador.        | ⛔ Não    |
|     | Converte uma string em um hash MD5.    | ✅ Sim   |
|     | Converte uma string em um hash SHA-1.   | ✅ Sim   |
| hmac_sha1_hex<br> | Converte uma string em um hash SHA-1 usando um código de autenticação de mensagem de hash (HMAC). Passe a chave secreta da mensagem como um parâmetro para o filtro.  | ✅ Sim   |
|     | Converte uma string em um hash SHA-256 usando um código de autenticação de mensagem de hash (HMAC). Passe a chave secreta da mensagem como um parâmetro para o filtro.       | ✅ Sim   |
...
|      | Insere uma tag HTML de quebra de linha `<br>` na frente de cada quebra de linha em uma string.        | ✅ Sim   |
|    | Emite a versão singular ou plural de uma string em inglês com base no valor de um número.      | ⛔ Não    |
|      | Anexa caracteres a uma string.      | ✅ Sim   |
|       | Remove todas as ocorrências de uma substring de uma string.       | ✅ Sim   |
|     | Remove apenas a primeira ocorrência de uma substring de uma string.      | ✅ Sim   |
|         | Substitui todas as ocorrências de uma string por uma substring.   | ✅ Sim   |
|         | Substitui a primeira ocorrência de uma string por uma substring.      | ✅ Sim   |
|        | O filtro de fatia retorna uma substring, começando no índice especificado.       | ✅ Sim   |
|   | O filtro de divisão recebe uma substring como parâmetro. A substring é usada como um delimitador para dividir uma string em uma matriz.            | ✅ Sim   |
|    | Remove guias, espaços e novas linhas (todos os espaços em branco) dos lados esquerdo e direito de uma string.                                                                                                    | ✅ Sim   |
|      | Retira guias, espaços e novas linhas (todos os espaços em branco) do lado esquerdo de uma string.    | ⛔ Não    |
|              | Retira guias, espaços e novas linhas (todos os espaços em branco) do lado direito de uma string.          | ⛔ Não    |
|          | Retira todas as tags HTML de uma string.        | ✅ Sim   |
|   | Remove todas as quebras de linha/novas linhas de uma string.        | ✅ Sim   |
|     | Trunca uma string até o número de caracteres passado como primeiro parâmetro. Uma elipse (...) é anexada à string truncada e é incluída na contagem de caracteres.    | ✅ Sim   |
|    | Trunca uma string até o número de palavras passadas como primeiro parâmetro. Uma elipse (...) é anexada à string truncada.    | ✅ Sim   |
|    | Converte uma string em maiúsculas.      | ✅ Sim   |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
| Filtrar                | Descrição                                                                                                                      | Com suporte |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------- | :-------- |
|            | Converte um registro de data e hora em outro formato de data. Consulte [Filtro de data](#date-filter) para obter mais informações.         | ✅ Sim   |
|         | Define um valor padrão para qualquer variável sem valor atribuído. Pode ser usado com strings, matrizes e hashes.      | ✅ Sim   |
|  | Formata um endereço para imprimir os elementos do endereço em ordem, de acordo com sua localização.        | ⛔ Não    |
|       | Envolve as palavras dentro dos resultados da pesquisa com uma tag HTML `<strong>` com o destaque da classe se corresponder aos termos de pesquisa enviados. | ⛔ Não    |
