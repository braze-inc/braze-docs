...
tool: Reports
page_type: reference
description: "Este artigo de referência aborda como medir a retenção de usuários para usuários que realizaram um evento de retenção selecionado em uma campanha ou canva específica."
---
...
## Executar um relatório de retenção
...
Para selecionar um intervalo de datas, você deve navegar até o canto superior direito da página da campanha ou canva **análise de dados**. Aqui você pode selecionar vários intervalos ou definir um intervalo personalizado para o seu relatório.
...
Em seguida, role para baixo até a seção **Retenção da campanha**. A retenção da campanha mostra a taxa na qual qualquer usuário que recebeu esta campanha específica realizou um evento de retenção (especificado por você no relatório de retenção) ao longo dos 30 dias a partir do momento em que recebeu a campanha.

{% endtab %}
{% tab Canvas %}

Em seguida, clique em **Analisar variantes** na parte inferior da página. A partir daqui, você pode analisar suas variantes, conferir seu relatório de funil e visualizar seu relatório de retenção. A retenção da canva mostra a taxa na qual qualquer usuário que recebeu esta canva específica realizou um evento de retenção (especificado por você no relatório de retenção) ao longo dos 30 dias a partir do momento em que recebeu a canva.
...
Depois de selecionar um evento de retenção, clique em **Gerar relatório** para iniciar a consulta.

![Executar Relatório][2]{: style="max-width:80%"}
...
A retenção contínua mede quantos usuários retornam e realizam o evento de retenção no ou após qualquer um dos dias listados no topo do relatório. Então, se um usuário iniciou uma sessão entre o dia 3 e o dia 7, o usuário será contado como retido nas colunas "3 dias", "1 dia" e "0 dias". Qualquer usuário que for contado como retido após o marco de 30 dias a partir do envio da campanha ou canva será contado na coluna "30 dias" nessa linha.

Um usuário que completa o evento várias vezes durante um período de 30+ dias será contado como parte de vários períodos de tempo. Por exemplo, um usuário que completa uma sessão após 1 dia será incrementado nas colunas para >0 e >1. Se eles completarem o evento após 3 dias, eles serão novamente incrementados nas colunas anteriores (>0 e >1), o que pode resultar em uma taxa de retenção superior a 100%.

#### Como ler relatórios de retenção contínua

A maneira de ler o gráfico do relatório de retenção para uma coluna do dia 3 seria Y% ou Y número de usuários (com base nas unidades escolhidas) que realizaram o evento 3 ou mais dias após receber a campanha no dia Z.

![Relatório Contínuo]({% image_buster /assets/img/campaign_retention3.png %})

Como outro exemplo, referindo-se à tabela na imagem anterior, no dia 25 de março, um total de 38 usuários realizaram o evento de retenção. A retenção do dia 0 foi de 68,42%, o que significa que 68,42% dos usuários realizaram o evento de retenção 0 ou mais dias (no dia 0 ou depois) após receber a campanha. A retenção do dia 7 foi de 57,89%, o que significa que 57,89% dos usuários realizaram o evento 7 ou mais dias (no dia 7 ou depois) após receberem a campanha.
...
As medidas de retenção de intervalo mostram quantos usuários retornam no intervalo de dias listados no topo do relatório. Então, se um usuário iniciou uma sessão entre os dias 3 e 7 e depois novamente no dia 13, ele seria contado como retido em ambos os intervalos "Dia 3-7" e "Dia 7-14".
