...
Em sua conta do dashboard, navegue até **Análise de dados** > **Relatórios de engajamento.** Clique em **\+ Criar novo relatório**.

{% alert note %}
Se estiver usando a [navegação mais antiga]({{site.baseurl}}/navigation), poderá encontrar **os relatórios de engajamento** em **Dados**.
{% endalert %}
...
Por padrão, os dados exibidos nos relatórios de engajamento são diários (1 dia). Se quiser visualizar esses dados em diferentes intervalos, você pode escolher um número explícito de dias ou semanas para agregar os dados ao relatório. Assim, em vez de ver métricas diárias, você pode analisar seu engajamento por semana, mês, trimestre etc. Se uma agregação centrada no tempo não for suficiente, você também pode optar por exportar dados no nível da campanha ou do Canva.
...
- **Enviar em um horário determinado:** Essa opção lhe dá a flexibilidade de escolher a frequência com que você gostaria de receber esse relatório. Você pode optar por enviar esse relatório a cada número definido de dias, semanas ou meses. Você também pode definir quando interromper o envio do relatório.

![engagement_reports_schedule_report][5]{: style="max-width:65%;" }

### Etapa 5: Revisão e lançamento

A etapa final da configuração de seu relatório mostra uma visão geral apenas das opções configuradas. Revise seu relatório e, quando estiver tudo pronto, clique em **Iniciar relatório**.

### Etapa 6: Verifique seu e-mail  

Você receberá um e-mail com links para seus relatórios no horário ou na programação que escolher. **Esses links expiram uma hora após o envio do relatório.** Ao clicar nos links fornecidos, você baixará automaticamente um arquivo ZIP contendo seus arquivos CSV - um para todas as campanhas.
