...
page_order: 4
page_type: reference
description: "Este artigo de referência aborda como executar um relatório usando o criador de relatórios, incluindo campanhas e canvas, criando relatórios de comparação e criando relatórios e gráficos."
tool: 
  - Reports
...
> O Report Builder permite comparar os resultados de várias campanhas ou Canvas em uma única exibição, para que você possa determinar facilmente quais estratégias de engajamento tiveram maior impacto nas suas principais métricas. Para ambas as campanhas e canvas, você pode exportar seus dados e salvar seu relatório para visualizar no futuro.<br><br>Para obter uma lista descritiva das métricas que você encontrará em seus relatórios, consulte o [Glossário de métricas de relatórios][16].
...
{% alert tip %}
Tente usar os mesmos eventos de conversão para a conversão A, B, etc. em todas as campanhas e canvas que deseja comparar, para que possa alinhar essas conversões nos relatórios do Report Builder.
{% endalert %}
...
{% alert note %}
Se você estiver usando a [navegação mais antiga]({{site.baseurl}}/navigation), poderá encontrar **o Report Builder** em **Data**.
{% endalert %}

Clique em **Criar novo relatório** e selecione um relatório de comparação de campanhas ou um relatório de comparação do Canva.
...
| **Relatório de edição** | Você pode clicar em Editar relatório para adicionar ou excluir campanhas do seu relatório | Você pode editar seu relatório ajustando os critérios de filtragem. |
...
Depois de criar o relatório, você encontrará uma tabela em branco contendo campanhas em cada linha. A tabela será preenchida depois que você selecionar **Edit Columns (Editar colunas** ) e escolher as métricas que deseja adicionar.
...
Quando estiver pronto, clique em **Salvar**. Os relatórios salvos podem ser visualizados posteriormente na página **do Report Builder**.
...
{% alert important %}
Se o usuário excluir uma variante de uma campanha multivariante, os dados dessa variante não estarão disponíveis para uso em um relatório futuro.
{% endalert %}
...
A seleção da **divisão por variante** permitirá que você veja as estatísticas de alto nível de suas Canvas gerais, bem como as estatísticas de cada variante, que podem ser expandidas clicando na seta ao lado do nome da Canvas.

![Variantes][12]{: style="max-width:90%;"}

### Etapas 

A seleção do **detalhamento por etapas** lhe permitirá visualizar métricas em nível de etapa, com cada linha do relatório contendo a linha de uma etapa.
...
Ao acessar um **Relatório manual** salvo, você poderá visualizar as mesmas campanhas adicionadas anteriormente, pois essas campanhas ainda se enquadram no filtro "Last Sent" (Último envio).
...
Em um **Relatório manual**, você pode editar um relatório clicando em **Editar**. A partir daí, você pode selecionar ou desmarcar campanhas para incluir em seu relatório.

Em um **relatório automático**, basta alternar seus filtros para restringir os resultados em seu relatório.

## Exportação de relatórios

Você também pode clicar em **Exportar** para baixar seu relatório em CSV.
...
Sua exportação de CSV refletirá a visualização de detalhamento em que você estava quando clicou em **Exportar**. Por exemplo, se você estava na exibição de detalhamento em nível de etapa, sua exportação conterá dados sobre as métricas da etapa. Para exportar dados de um detalhamento diferente, você precisará navegar primeiro para esse detalhamento e clicar em **Exportar** a partir daí.
