...
page_order: 0
page_type: reference
description: "Este artigo de referência fornece uma visão geral da Ingestão de Dados na Nuvem, melhores práticas e limitações do produto."
...
A cobrança de pontos de dados para a Ingestão de Dados na Nuvem é equivalente à cobrança por atualizações através do [`/users/track` endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track#user-track). Consulte [pontos de dados]({{site.baseurl}}/user_guide/data_and_analytics/data_points/) para saber mais. 
...
| Número de linhas         | Não há limite para o número de linhas que você pode sincronizar. Cada linha será sincronizada apenas uma vez, com base na coluna `UPDATED`.                                                            |
...
| Tamanho da carga útil           | Cada linha pode conter uma carga útil de até 1 MB. Cargas úteis maiores que 1 MB serão rejeitadas, e o erro "Carga útil foi maior que 1MB" será registrado no registro de sincronização junto com o ID externo associado e a carga útil truncada. |
