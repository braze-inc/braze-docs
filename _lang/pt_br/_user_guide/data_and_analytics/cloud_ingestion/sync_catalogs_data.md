...
page_order: 4
page_type: reference
description: "Este artigo de referência fornece uma visão geral de como sincronizar dados de catálogo."
...
Antes de criar uma nova integração de Ingestão de Dados em Nuvem (CDI) para [catálogos]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/), você precisará criar um novo catálogo ou identificar um catálogo existente que deseja usar para a integração. Existem algumas maneiras de criar um novo catálogo e qualquer uma delas funcionará para a integração CDI:
...
2. Configurar um usuário e conceder as permissões adequadas. Se você já possui credenciais de uma sincronização existente, pode reutilizá-las—apenas certifique-se de estender o acesso à tabela de origem do catálogo.
...
{:start="3"}
3\. Se você tiver políticas de rede em vigor, deve conceder acesso à rede da Braze à sua instância do BigQuery. Para uma lista de IPs, consulte a [Ingestão de Dados na Nuvem]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/integrations/#step-1-set-up-tables-or-views).
