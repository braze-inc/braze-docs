...
Os atributos personalizados aninhados permitem que você defina um conjunto de atributos como uma propriedade de outro atributo. Em outras palavras, quando você define um objeto de atributo personalizado, pode definir um conjunto de atributos adicionais para esse objeto.

Por exemplo, digamos que você queira definir um atributo personalizado no perfil do usuário chamado `favorite_book`. Esse atributo personalizado pode ser definido como um objeto que tem os atributos aninhados `title`, `author` e `publishing_date`, da seguinte forma:
...
- Os objetos têm um tamanho máximo de 50 KB.
...
- Nem todos os Braze Partners suportam atributos personalizados aninhados. Consulte a [documentação do parceiro]({{site.baseurl}}/partners/home) para confirmar se a integração suporta esse recurso.
...
Observe que, para atributos personalizados aninhados, se o ano for menor que 0 ou maior que 3000, a Braze não armazenará esses valores no usuário.
...
Depois de adicionar um caminho à sua propriedade, clique em **Validar** para verificar se o valor no campo de jornada é válido.
...
A partir daí, você pode usar o site `segmented_nested_objects` e personalizar sua mensagem. Em nosso caso de uso, queremos pegar uma meta da primeira conta tributável ativa e personalizá-la:
...
No dashboard do Braze, navegue até **Data Settings** > Custom Attributes ( **Configurações de dados** > Atributos personalizados).

{% alert note %}
Se estiver usando a [navegação mais antiga]({{site.baseurl}}/navigation), poderá encontrar **os atributos personalizados** em **Manage Settings (Gerenciar configurações)**.
{% endalert %}

Procure seu objeto ou vetor de objetos. Na coluna **Nome da atribuição**, clique em **Gerar esquema**.
...
Clique no botão de adição <i class="fas fa-plus"></i> no campo da jornada. Isso exibirá uma representação de seu objeto ou vetor de objetos. Você pode selecionar qualquer um dos itens listados e a Braze os inserirá no campo da jornada para você. Para nosso caso de uso, precisamos obter o equilíbrio. Selecione o saldo e a jornada (nesse caso, `[].balance`) é preenchida automaticamente no campo de jornada.

![][12]{: style="max-width:70%" }

Você pode clicar em **Validar** para verificar se o conteúdo do campo de jornada é válido e, em seguida, criar o restante do filtro conforme necessário. Aqui, especificamos que o saldo deve ser menor que 100.
...
3. Na coluna **Attribute Name (Nome do atributo** ) de seu atributo, clique em <i class="fas fa-plus"></i> para gerenciar o esquema.
4. Será exibido um modal. Clique em **Regenerate Schema (Regenerar esquema**).
...
{% alert important %}
Se quiser redefinir o esquema de um vetor de objetos com um objeto existente, será necessário criar um novo atributo personalizado. A regeneração do esquema não exclui os objetos existentes.
{% endalert %}
