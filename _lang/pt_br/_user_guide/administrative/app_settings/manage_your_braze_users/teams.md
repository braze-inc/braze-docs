...
Administradores do Braze e usuários limitados com a permissão de nível de empresa "Pode Gerenciar Configurações da Empresa" podem atribuir permissões de nível de Equipe a um usuário da empresa com acesso limitado. Quando atribuído a uma Equipe, os usuários da empresa são limitados a apenas ler ou escrever dados disponíveis para suas respectivas Equipes, como idioma do usuário, localização ou atributo personalizado, conforme definido quando a Equipe foi criada.
...
{: start="2"}
2\. Selecione **\+ Adicionar permissões em nível de equipe**, e então selecione o **Time** ao qual você gostaria de adicionar este usuário.
3\. Atribua permissões específicas da coluna de permissões do **Time**.
...
![Menu suspenso de tags da equipe de campanha que exibe apenas a tag da equipe "Desenvolvimento".]({% image_buster /assets/img_archive/team_permissions_scenario1.gif %})
...
Como Michelle tem a permissão em nível de espaço de trabalho de "Acessar Campanhas, Canvases, Cartões, Blocos de Conteúdo, Sinalizadores de Recursos, Segmentos, Biblioteca de Mídia e Centros de Preferência", ela pode visualizar e atribuir outros filtros de equipe à campanha que cria.
