...
{% alert important %}
A capacitação SSL envolverá o URL do pixel de rastreamento com HTTPS em vez de HTTP. Se o SSL estiver mal configurado, isso poderá afetar a eficácia do pixel de rastreamento.
{% endalert %}
...
Para usar o recurso de cancelamento de inscrição do Braze para processar cancelamentos de inscrição diretamente, selecione **Incluir um cabeçalho de e-mail de lista de cancelamento de inscrição com um clique (mailto e HTTP) para e-mails enviados a usuários inscritos ou com aceitação** e selecione **Braze default** como URl e mail-to padrão do Braze. 

![Opção para incluir automaticamente um cabeçalho list-unsubscribe em e-mails enviados a usuários inscritos ou com aceitação.]({% image_buster /assets/img/email_settings/email_unsubscribe_header.png %}){: style="max-width:80%;"}
...
O uso do cancelamento de inscrição com um clique para o cabeçalho list-unsubscribe[(RFC 8058](https://datatracker.ietf.org/doc/html/rfc8058)) se concentra em fornecer uma forma fácil para os destinatários optarem pela aceitação de e-mails.
...
{% alert note %}
A configuração de cancelamento de inscrição na lista com um clique no nível da mensagem só está disponível ao usar o editor de HTML atualizado. Se estiver usando o editor de HTML anterior, mude para o editor de HTML atualizado para usar esse recurso.
{% endalert %}
...
- **Cancelar inscrição em um grupo de inscrições específico**: Usa o grupo de inscrições especificado. Os usuários que clicarem no botão cancelar inscrição terão a inscrição cancelada no grupo de inscrições selecionado.
    - Ao selecionar um grupo de inscrições, adicione o filtro **Grupo de inscrições** em **Públicos-alvo** para direcionar apenas os usuários que estão inscritos nesse grupo específico. O grupo de inscrições selecionado para cancelar inscrição com um clique deve corresponder ao grupo de inscrições que está sendo direcionado. Se houver uma incompatibilidade no grupo de inscrições, você corre o risco de enviar para um usuário que está tentando cancelar a inscrição de um grupo de inscrições do qual ele já cancelou a inscrição.
...
Essa configuração não afeta mensagens ou modelos de e-mail existentes. É possível substituir esse padrão a qualquer momento durante a composição de mensagens ou modelos. Para saber mais, consulte [CSS inlining][10].
...
Se você não tiver nenhum dos casos de uso para a configuração de cancelamento de inscrição na lista de um clique no nível da mensagem, não há nenhuma ação necessária, desde que a configuração esteja ativada em **Preferências de e-mail**. O Braze adicionará automaticamente os cabeçalhos de cancelamento de inscrição com um clique a todas as mensagens de marketing e promocionais enviadas. No entanto, se for necessário configurar o comportamento ao clicar para cancelar a inscrição em nível de mensagem, será necessário atualizar as campanhas anteriores e as etapas do Canva com envio de e-mail de acordo.
{% enddetails %}
...
{% details Se eu adicionar os cabeçalhos de e-mail para o one-click manualmente e tiver o cabeçalho de cancelamento de inscrição ativado, qual é o comportamento esperado? %}
...
Para uma campanha com Testes A/B, o Braze enviará aleatoriamente a um usuário uma das variantes. Se você tiver dois grupos de inscrições diferentes definidos na mesma campanha (a Variante A está definida para o Grupo de inscrições A e a Variante B está definida para o Grupo de inscrições B), não poderemos garantir que os usuários que estiverem inscritos apenas no Grupo de inscrições B receberão a Variante B. Pode haver um cenário em que os usuários estejam cancelando inscrição de um grupo de inscrições do qual já tenham feito a aceitação.
{% enddetails %}
...
{% details A configuração de cancelamento de inscrição com um clique estará disponível para modelos de e-mail? %}
...
Daqui a [6]: https://learning.braze.com/email-open-tracking-pixel
