...
article_title: Configurar a autenticação do SDK da Braze
...
Agora é possível impedir que usuários registrados não autenticados usem a Chave da API SDK do seu app para realizar ações mal-intencionadas, como se passar por outros usuários.
...
{% alert warning %}
Lembre-se de manter suas chaves privadas _em sigilo_. Nunca exponha ou codifique sua chave privada diretamente em seu app ou site. Qualquer pessoa que conheça sua chave privada pode se passar por outros usuários ou criar usuários em nome do seu aplicativo.
{% endalert %}
...
Normalmente, essa lógica pode ser inserida em qualquer lugar em que o aplicativo normalmente solicite o perfil do usuário atual, como um endpoint de login ou em qualquer lugar em que o aplicativo atualize o perfil do usuário atual.
...
| ----- | -------- | ----------------------------------- |
| `alg` | Sim  | O algoritmo compatível é `RS256`. |
...
| Campo | Obrigatório | Descrição                                                                            |
| ----- | -------- | -------------------------------------------------------------------------------------- |
| `sub` | Sim  | O "subject" deve ser igual ao ID do usuário que você fornece ao SDK da Braze ao chamar `changeUser`  |
| `exp` | Sim | A "expiration", ou seja, quando você deseja que esse token expire.                                |

{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

{% alert tip %}
Para saber mais sobre JSON Web Tokens ou para navegar pelas muitas bibliotecas de código aberto que simplificam esse processo de assinatura, consulte [https://jwt.io](https://jwt.io).
{% endalert %}
...
{% endtab %}
{% tab React Native %}
A autenticação do SDK deve ser ativada durante a inicialização do SDK nativo. Adicione a seguinte configuração ao seu código nativo do iOS e do Android:
...
Depois de ativar a autenticação do SDK na camada nativa, você pode usar os métodos JavaScript do React Native mostrados nas etapas a seguir.
{% endtab %}
{% tab Java %}
Ao configurar a instância da Braze, chame `setIsSdkAuthenticationEnabled` com o valor `true`.
```java
...
{% endtab %}
{% tab Flutter %}
A autenticação do SDK deve ser ativada como parte da inicialização do SDK no código nativo do iOS e do Android. Quando ativada na camada nativa, você pode usar os métodos do Flutter SDK para passar a assinatura JWT.
...
Depois de ativar a autenticação do SDK na camada nativa, você pode usar os métodos do Flutter SDK mostrados nas etapas a seguir.
{% endtab %}
{% tab Unity %}
A autenticação do SDK deve ser ativada durante a inicialização do SDK nativo. Adicione a seguinte configuração ao seu código nativo do iOS e do Android:
...
Defina a propriedade `SDKAuthenticationEnabled` como `true` em seu arquivo de configuração:
...
Depois de ativar a autenticação do SDK na camada nativa, você pode usar os métodos do Unity C# mostrados nas etapas a seguir.
{% endtab %}
{% tab Cordova %}
A autenticação do SDK deve ser ativada durante a inicialização do SDK nativo. Adicione a seguinte configuração ao seu código nativo do iOS e do Android:
...
Depois de ativar a autenticação do SDK, você pode usar os métodos .NET MAUI mostrados nas etapas a seguir.
{% endtab %}
{% tab Expo %}
Ao usar o plug-in Braze Expo, defina a propriedade `enableSdkAuthentication` como `true` na configuração do app. Isso configura automaticamente a autenticação do SDK nas camadas nativas do iOS e do Android sem exigir alterações manuais no código nativo.
...
Depois de ativar a autenticação do SDK na configuração do app, você pode usar os métodos JavaScript do React Native mostrados na guia React Native para as etapas a seguir.

{% alert note %}
Para obter um exemplo completo de implementação, consulte o [app de amostra do plug-in Braze Expo](https://github.com/braze-inc/braze-expo-plugin/blob/main/example/components/Braze.tsx) no GitHub.
{% endalert %}
{% endtab %}
{% endtabs %}
...
{% alert note %}
Lembre-se de que `changeUser` só deve ser chamado quando o ID do usuário _realmente mudar_. Você não deve usar esse método para atualizar o token de autenticação (JWT) se o ID do usuário não tiver sido alterado.
{% endalert %}
...
Ou quando você atualizar o token do usuário no meio da sessão:
...
Ao usar o plug-in Braze Expo, use os mesmos métodos do React Native SDK. Forneça o JWT ao chamar `changeUser`:
...
Quando esse recurso estiver definido como [Obrigatório](#enforcement-options), os cenários a seguir farão com que as solicitações do SDK sejam rejeitadas pela Braze:
- O JWT estava expirado no momento em que foi recebido pela API da Braze
- O JWT estava vazio ou ausente
- A verificação do JWT falhou para as chaves públicas cujo upload você fez para o dashboard da Braze
...
As solicitações com falha serão repetidas periodicamente até que seu app forneça um novo JWT válido. Se o usuário ainda estiver registrado, você poderá usar esse retorno de chamada como uma oportunidade para solicitar um novo JWT do seu servidor e fornecê-lo ao SDK da Braze.

Ao receber um erro de autenticação, verifique se o `userId` no erro corresponde ao seu usuário atualmente registrado e, em seguida, obtenha uma nova assinatura do seu servidor e forneça-a ao SDK da Braze. Também é possível registrar esses erros no seu serviço de monitoramento ou de relatório de erros.

{% alert tip %}
Esses métodos de retorno de chamada são um ótimo lugar para adicionar seu próprio serviço de monitoramento ou registro de erros para acompanhar com que frequência as solicitações da Braze estão sendo rejeitadas.
{% endalert %}
...
Defina o delegado de autenticação do SDK em sua implementação nativa do iOS:
...
{% endtab %}
{% tab Expo %}
Ao usar o plug-in Braze Expo, use os mesmos métodos do React Native SDK:
...
### Etapa 3: Ativar a autenticação no dashboard {#braze-dashboard}
...
Na página **Manage Settings** do dashboard, cada app tem três estados de autenticação do SDK que controlam como a Braze verifica as solicitações.
...
Um JWT inválido será relatado nos estados **Opcional** e **Obrigatório**, mas somente o estado **Obrigatório** rejeitará as solicitações do SDK, fazendo com que os apps tentem novamente e solicitem um novo JWT.
...
![Um gráfico que mostra o número de instâncias de erros de autenticação. Também são mostrados o número total de erros, o tipo de erro e o intervalo de datas ajustável.]({% image_buster /assets/img/sdk-auth-analytics.png %}){: style="max-width:80%"}
...
| 21 | `SUBJECT_MISMATCH` | Os subjects esperado e real não são os mesmos.| O campo `sub` deve ser o mesmo ID do usuário passado para o método `changeUser` do SDK. |
| 22 | `EXPIRED` | O token fornecido expirou.| Estenda sua expiração ou atualize periodicamente os tokens antes que eles expirem. |
...
| 24 | `INCORRECT_ALGORITHM` | O algoritmo do token não é compatível.| Altere seu JWT para usar a criptografia `RS256`. Outros tipos não são compatíveis. |
...
| 26 | `MISSING_TOKEN` | Nenhum token foi fornecido na solicitação.| Certifique-se de que esteja passando um token ao chamar `changeUser(id, token)` e que o token não esteja em branco.|
| 27 | `NO_MATCHING_PUBLIC_KEYS` | Nenhuma chave pública corresponde ao token fornecido.| A chave privada usada no JWT não corresponde a nenhuma chave pública configurada para seu app. Confirme que você adicionou as chaves públicas ao app correto em seu espaço de trabalho que corresponde a essa chave de API.|
| 28 | `PAYLOAD_USER_ID_MISMATCH` | Nem todos os IDs de usuário na carga útil da solicitação correspondem ao esperado.| Isso é inesperado e pode resultar em uma carga útil malformada. Abra um tíquete de suporte para obter assistência. |
...
#### Que expiração devo usar ao gerar um JWT? {#faq-expiration}
...
Se o token de um usuário expirar no meio da sessão, o SDK tem uma [função de retorno de chamada](#sdk-callback) que será invocada para informar ao seu app que um novo JWT é necessário para continuar enviando dados à Braze.

#### O que acontecerá se minha integração no lado do servidor falhar e eu não puder mais criar um JWT? {#faq-server-downtime}

Se o seu servidor não puder fornecer um JWT ou se você notar algum problema de integração, sempre será possível desativar o recurso no dashboard da Braze.
...
Ao usar segredos compartilhados, qualquer pessoa com acesso a esse segredo compartilhado, como a página do dashboard da Braze, poderia gerar tokens e se passar por seus usuários finais.
...
Quando uma solicitação for rejeitada devido a um erro de autenticação, o SDK invocará o retorno de chamada usado para atualizar o JWT do usuário. 

As solicitações serão repetidas periodicamente usando uma abordagem de backoff exponencial. Após 50 tentativas consecutivas sem sucesso, as novas tentativas serão pausadas até o início da próxima sessão. Cada SDK também tem um método para solicitar manualmente um envio de dados.

#### É possível usar a autenticação do SDK para usuários anônimos? {#faq-anonymous-users}

Não. A autenticação do SDK não funcionará para usuários anônimos.