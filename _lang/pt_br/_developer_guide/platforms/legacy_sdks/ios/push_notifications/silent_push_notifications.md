...
O sinalizador `content-available` pode ser definido no dashboard do Braze, bem como em nosso [objeto Apple push]({{site.baseurl}}/api/objects_filters/messaging/apple_object/) na [API de envio de mensagens]({{site.baseurl}}/api/endpoints/messaging/).

![O dashboard do Braze mostra a caixa de seleção "content-available" (conteúdo disponível) encontrada na guia "settings" (configurações) do criador do push.]({% image_buster /assets/img_archive/remote_notification.png %} "content available")
...
A capacitação do modo em segundo plano para notificações remotas é necessária para o [rastreamento da desinstalação]({{site.baseurl}}/developer_guide/analytics/tracking_uninstalls/?sdktab=swift).
