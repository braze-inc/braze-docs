...
| ---------------------| --------------- | ------------- |
| Vérifie si l’événement personnalisé s’est produit **plus de X fois** | **PLUS DE** | **ENTIER** |
| Vérifie si l’événement personnalisé s’est produit **moins de X fois** | **MOINS DE** | **ENTIER** |
...
| Vérifie si l’événement personnalisé a eu lieu **il y a moins de X jours** | **MOINS DE** | **IL Y A X JOURS** (entier positif) |
...
Par exemple, si une application d’e-commerce souhaite envoyer un message à un utilisateur lorsqu’il abandonne son panier, elle pourrait en outre améliorer son audience cible et permettre une personnalisation accrue de la campagne en ajoutant une propriété de l’événement personnalisé « Valeur du panier » sur les paniers des utilisateurs.
...
Les attributs au format string sont utiles pour stocker les entrées utilisateur, comme une marque préférée, un numéro de téléphone ou la dernière recherche dans votre application. La longueur maximale des attributs de chaîne de caractères est de 255 caractères.

| Options de segmentation | Filtre déroulant | Options de saisie |
| ---------------------| --------------- | ------------- |
| Vérifie si l’attribut de chaîne de caractères est **exactement identique** à une chaîne de caractères saisie| **ÉGAL A** | **STRING** |
...
#### Dates
Les attributs de date sont utiles pour enregistrer la dernière fois qu’une action spécifique a été effectuée, car ils vous permettent d’envoyer contenus spécifiques de réengagement à vos utilisateurs.
...
3. Utilisez nos [fonctionnalités de personnalisation et de modélisation de messages][17] pour faire glisser l’attribut de Promotion Code unique dans la communication envoyée aux utilisateurs.
...
- Essayez, dans la mesure du possible, de ne pas utiliser les abréviations obscures ou du jargon technique
  - Exemple : `usr_ctry` peut être un nom de variable pour le pays d’un utilisateur dans une partie de code, mais l’attribut personnalisé doit être envoyé à Braze en tant que `user_country`, au moins pour apporter des précisions aux marketeurs qui utiliseront le tableau de bord par la suite.
