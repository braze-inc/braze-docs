...
Braze permet de définir une police personnalisée à l’aide du [guide de la famille de polices][79]. Pour l’utiliser, remplacez le style du texte du message, des en-têtes et du texte du bouton et utilisez l’attribut `fontFamily` pour indiquer à Braze d’utiliser votre famille de polices personnalisée.
...
Pour définir une orientation fixe pour un message in-app, [définissez en premier un gestionnaire d’écouteur de messages in-app personnalisé][19]. Appelez ensuite `setOrientation()` sur l’objet `IInAppMessage` dans la méthode de délégation `beforeInAppMessageDisplayed()` :
