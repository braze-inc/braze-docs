...
#### Choses à vérifier
- [**Activé/abonné et push activé**]({{site.baseurl}}/user_guide/message_building_by_channel/push/users_and_subscriptions/) : Pour que les utilisateurs reçoivent un message push de Braze, ils doivent avoir un statut d’abonnement « opted-in (abonné) » (iOS), ou abonné (Android) et `Push Enabled = True (Notifications push activées = True)`. Notez qu’Android 13 introduit une modification majeure dans la façon dont les utilisateurs gèrent les applications qui envoient des notifications push. Le [Guide de mise à niveau du SDK pour Android 13]({{site.baseurl}}/developer_guide/platform_integration_guides/android/android_13/) de Braze continuera d’être mis à jour quand de nouvelles versions bêta Android 13 sont publiées.
...
#### Choses à savoir
- **Limites de cartes de contenu admissibles** : Il n’y a aucune limite pour afficher les cartes de contenu à un utilisateur. Braze ne retirera pas les cartes plus anciennes des flux.
