...
| current_location | (objet) Avec le format {"longitude": -73,991443, "latitude": 40.753824} |
...
| email | (string) |
| email_subscribe | (string) Les valeurs disponibles sont « opted_in » (explicitement consenti à recevoir des e-mails), « unsubscribed » (désabonné : explicitement refusé de recevoir des e-mails), et « subscribed » (abonné : ni accepté, ni refusé).  |
| email_open_tracking_disabled |(boolean) vrai ou faux accepté.  Définissez sur True (Vrai) pour désactiver le pixel de suivi d’ouverture dans tous les futurs e-mails envoyés à cet utilisateur.|
| email_click_tracking_disabled |(boolean) vrai ou faux accepté.  Définissez sur True (Vrai) pour désactiver le suivi de clic pour tous les liens dans les futurs e-mails envoyés à cet utilisateur.|
...
| téléphone | (string) |
| push_subscribe | (string) Les valeurs disponibles sont « opted_in » (explicitement consenti à recevoir des notifications push), « unsubscribed » (désabonné : explicitement refusé de recevoir des notifications push), et « subscribed » (abonné : ni accepté, ni refusé).  |
...
| subscription_groups| Array d’objets avec `subscription_group_id` et le string `subscription_state`, p. ex. `[{"subscription_group_id" : "subscription_group_identifier", "subscription_state" : "subscribed"}]`. Les valeurs disponibles pour `subscription_state` sont « subscribed » (abonné) et unsubscribed » (désabonné).<br>La fonction permettant d’utiliser cet endpoint pour mettre à jour les groupes d’abonnement d’un utilisateur est actuellement en accès anticipé. Contactez votre gestionnaire du succès des clients Braze si vous souhaitez participer à l’accès anticipé.|
| time_zone | (string) Du nom de fuseau horaire de la [base de données de fuseaux horaires IANA][26] (p. ex. « Amérique/New_York » ou « Heure de l’Est [États-Unis et Canada] »). Seules les valeurs de fuseau horaire valides seront définies. |
