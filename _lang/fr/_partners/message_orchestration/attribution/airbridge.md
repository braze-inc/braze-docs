...
| iApplication iOS ou Android | Cette intégration prend en charge les applications iOS et Android. Selon votre plateforme, les extraits de code peuvent être requis dans votre application. |
...
public void onCreate() {
    super.onCreate();
    // Initialiser le SDK Airbridge
    AirbridgeConfig config = new AirbridgeConfig.Builder("APP_NAME", "APP_TOKEN")
        // Demander explicitement au SDK Airbridge de démarrer le suivi
        .setAutoStartTrackingEnabled(false)
        .build();
    Airbridge.init(this, config);
    
    // Définir l’alias de l’appareil dans le SDK Airbridge
    Airbridge.getCurrentUser().setAlias("braze_device_id", Braze.getInstance(this).getDeviceId());
    // Démarrer explicitement le suivi
...
{% alert note %}
Les données d’attribution pour les campagnes qui ont menées à des événements ouverts deeplink sont mises à jour au niveau de l’appareil. Par exemple, si deux utilisateurs utilisent un seul appareil et qu’un utilisateur exécute un événement de deeplink, les données d’attribution de cet événement sont également reflétées dans les données de l’autre utilisateur.
{% endalert %}

Pour des instructions plus détaillées, consultez [Documents d’Airbridge.](https://help.airbridge.io/hc/en-us/articles/900004368546-Braze)
...
Airbridge peut envoyer quatre types de données d’attribution à Braze répertoriés dans le tableau des champs de données suivants. Ces données peuvent être affichées dans le tableau de bord d’Airbridge et sont utilisées pour l’attribution d’installation et le filtrage.
...
| -------------------- | ---------------------| ---- |
| `Channel` | Source d’attribution d’installation | Le canal d’installation ou de deeplink est attribué à |
| `Campaign` | Campagne d’attribution d’installation | La campagne d’installation ou de deeplink est attribuée à |
| `Publicité de groupe` | Groupe d’annonces d’attribution d’installation | Le groupe d’annonces ou les fenêtres d’installation ou de deeplink sont attribués à |
| `Publicité créative` | Annonce d’attribution d’installation | L’annonce publicitaire de l’installation ou de deeplink est attribuée à |
