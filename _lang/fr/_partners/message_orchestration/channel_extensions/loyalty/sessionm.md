...
#### Option 1 : Exportation vers l'endpoint SessionM Tag (recommandé)
...
Dans l'onglet **Paramètres**, ajoutez les paires clé-valeur pour chaque champ de l'en-tête de la demande :
...
<br><br>
L'exemple de demande suivant utilise cURL. 
...
Le client doit effectuer l'action requise ou satisfaire au ciblage et reçoit l'offre dans SessionM.
...
L'intégration entre SessionM et Braze permet aux données du profil utilisateur, aux détails de l'offre et aux soldes de points d'être dynamiquement intégrés dans les messages et envoyés en temps réel au client au point d'action.
...
- **Données relatives au déclencheur d'événement :** Toute donnée dans l'événement déclencheur qui utilise le résultat du webhook de déclenchement/d'envoi.
