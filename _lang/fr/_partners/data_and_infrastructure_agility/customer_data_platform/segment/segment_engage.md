...
1. Dans Segment.io, accédez à l’onglet **Caractéristiques** ou **Audiences calculées** dans **Engage**, puis cliquez sur **Nouveau**.
...
- Les caractéristiques et les audiences envoyées à l’aide de l’appel d’`identification` apparaîtront dans Braze en tant qu’attributs personnalisés.
- Les caractéristiques et les audiences envoyées à l’aide de l’appel de `suivi` apparaîtront dans Braze en tant qu’événements personnalisés.
...
Vous pouvez envoyer des caractéristiques et des audiences calculées à Braze en tant qu’appels d’`identification` pour créer des attributs personnalisés dans Braze. 
...
Vous pouvez envoyer des caractéristiques et des audiences calculées à Braze en tant qu’appels de `suivi` pour créer des événements personnalisés dans Braze. 
...
Bien que le paramètre par défaut pour la connexion entre Braze et Segment.io Engage soit `Temps réel`, certains des filtres empêcheront la persona de se synchroniser en temps réel, y compris certains filtres temporels qui limitent la taille de votre audience au moment où le message a été envoyé.

## Test de débogage Segment.io

Le tableau de bord de Segment.io inclut une fonction « Débogage » qui permet aux clients de tester si les données d’une « source » sont transférées vers une « destination » comme prévu.
...
Cela ne fonctionnera pas pour une intégration side-by-side de Braze. Aucune donnée serveur ne sera transmise si vous n’avez pas saisi les bonnes informations de l’API REST de Braze.