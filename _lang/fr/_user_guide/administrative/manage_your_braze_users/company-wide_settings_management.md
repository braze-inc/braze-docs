...
page_order: 5
page_type: reference
description: "Cet article de référence couvre les paramètres à l’échelle de l’entreprise, comme changer le nom de votre entreprise, définir votre fuseau horaire et demander à supprimer votre entreprise."
...
La page **[Company Settings][1]** (Paramètres de l’entreprise) vous permet de modifier le nom de votre entreprise, de définir votre fuseau horaire et de demander à supprimer votre entreprise.
...
- Bien que les campagnes prévues pour des horaires spécifiques dans des endroits spécifiques (c.-à-d. 21 h, heure de l’Est) fonctionnent correctement sur le calendrier jusqu’à ce qu’elles soient modifiées, l’analyse de campagne et les futurs calendriers de campagne seront affectés par le changement.
...
| Notification | Description | Support Webhook |
|--------------|-------------|-----------------|
| Erreurs d’identification AWS | Informe le destinataire lorsque Braze reçoit une erreur lors d’un essai d’utilisation de vos informations d’identification Amazon Web Services pour une exportation de données. | Oui |
| Campagne planifiée envoyée/non envoyée | Informe les destinataires lorsque les campagnes planifiées commencent à être envoyées ou lorsque des tentatives d’envoi sont effectuées, mais qu’aucun utilisateur éligible n’a été trouvé. | Oui |
| Limite de campagne planifiée atteinte | Informe les destinataires lorsqu’une campagne récurrente planifiée n’est pas envoyée parce que la limite totale de la campagne a été atteinte. | Oui |
| Envoi terminé d’une campagne planifiée | Informe les destinataires lorsque l’envoi d’une campagne planifiée est terminé. | Oui |
| Délais Webhook | Informe les destinataires lorsqu’une URL Webhook expire plus de 300 fois en 5 minutes. Cette notification ne peut pas être envoyée plus d’une fois toutes les deux heures. | Oui |
| Erreurs d’identification Push | Avertit les destinataires lorsque les notifications push d’identification de l’application sont invalides et lorsqu’elles sont bientôt expirées. | Oui |
| Carte de fil d’actualités publiée/en direct | Informe les destinataires lorsque les cartes de fil d’actualités sont programmées ou publiées. | Oui |
| Rapport hebdomadaire d’analyse | Envoie un résumé de l’activité du groupe d’apps de la semaine précédente aux destinataires tous les lundis. Les destinataires reçoivent un résumé pour chaque groupe d’apps auquel ils appartiennent. | Non |
...
1. Sélectionnez le canal Slack vers lequel vous souhaitez que les notifications soient envoyées et cliquez sur **Add Incoming Webhooks Integration** (Ajouter une intégration de Webhooks entrants).<br>
<br>

    ![Ajouter l’intégration des Webhook entrants dans Slack][63]<br>
<br>

  Slack génère une URL que vous devrez saisir dans Braze pour les notifications que vous souhaitez recevoir.<br>
<br>

2. Copier l’**URL du Webhook**.<br>
<br>

    ![Copier l’URL du Webhook][64]<br>
<br>

3. Naviguez jusqu’à l’onglet **Notification Preferences** (Préférences de notification) dans **Company Settings** (Paramètres de l’entreprise).<br>
<br>

4. Sélectionnez la notification que vous souhaitez activer pour Slack. Ou, si vous avez plusieurs notifications à envoyer à ce canal Slack, utilisez **Bulk Add** (Ajout groupé) pour ajouter le Webhook à plusieurs notifications.<br>
<br>
...
![Section Outbound Email Settings (Paramètres de messagerie sortante) de l’onglet Email Settings (Paramètres de messagerie)][7]
...
Pour plus d’informations sur liste-désabonnement, consultez [Paramètres de messagerie][2]

## Paramètres de sécurité

La page de paramètres de sécurité permet de configurer les règles d’authentification, la whiteliste IP du tableau de bord et l’authentification à deux facteurs. Ces paramètres sont situés dans l’onglet [Security Settings][83] (Paramètres de sécurité) de la page **Company Settings** (Paramètres de l’entreprise).
...
#### Présentation de la configuration Authentification à deux facteurs Authy 

1. Téléchargez l’application Authy.
2. Naviguer vers **Two-Factor Authentication** (Authentification à deux facteurs) sous **Account Settings** (Paramètres du compte) et saisissez votre numéro de téléphone.
3. Une notification sera envoyée au dispositif demandant à Authy d’obtenir le code pour Braze.
4. Ouvrez l’application Authy sur l’appareil lié à l’obtention du code. 
