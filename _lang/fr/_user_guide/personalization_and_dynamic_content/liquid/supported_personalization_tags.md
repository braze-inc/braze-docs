...
| Propriétés de l’événement <br> (Elles sont personnalisées pour votre groupe d'apps.)| `{{event_properties.${your_custom_event_property}}}` |
| Propriétés d’entrées de Canvas| `{{canvas_entry_properties}}` |
| Attributs personnalisés <br> (Elles sont personnalisées pour votre groupe d'apps.) | `{{custom_attribute.${your_custom_attribute}}}` |
