...
glossaries:
  ...
  - name: Impressions totales
    description: Nombre de fois où le message ou la carte dans l’application a été visualisé (si un utilisateur voir deux fois un message, il sera comptabilisé deux fois). Ce nombre est une somme du nombre d’événements d’impression que Braze reçoit des SDK.
    calculation: Total
    tags:
      - Message in-app
      - Fil d’actualité
  ...
  - name: Désinscriptions
    ...
    tags:
      - E-mail
  - name: Revenue
    description: Le revenu total en dollars de destinataires de campagne dans la fenêtre de conversion principale définie.
    tags:
      - Cartes de contenu
      - E-mail
      - Message in-app
