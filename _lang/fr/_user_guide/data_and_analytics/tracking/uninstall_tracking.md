...
Vous pouvez activer le suivi de désinstallation dans l’onglet **Paramètres** onglet de la page **Manage Settings** pour chaque application que vous souhaitez suivre.
...
Braze suit les désinstallations en observant lorsque les messages de notification push envoyés aux appareils des utilisateurs renvoient un signal provenant soit de Firebase Cloud Messaging (FCM) soit d’Apple Push Notification Service (APN) indiquant que l’application n’est plus installée. Si le suivi global de désinstallation est activé pour une application particulière, Braze envoie un message de notification push silencieux quotidien aux utilisateurs pour détecter si ils l’ont désinstallé. Cette notification push « silencieux » est envoyé à tous les utilisateurs (sauf si l’utilisateur a désactivé les push silencieux dans les paramètres de son application), mais il n’est pas visible pour les utilisateurs. Si Braze détecte qu’un utilisateur a désinstallé, la plate-forme :

* Incrémente de 1 le nombre total de désinstallation de l’application.
* Incrémente de 1 le nombre de désinstallation pour chaque campagne que l’utilisateur a reçue avec succès au cours des dernières 24 heures.
