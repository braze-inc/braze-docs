...
Consultez nos [bonnes pratiques en matière d’obtention des données]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/best_practices/) pour voir une description complète des méthodes permettant de recueillir les données des utilisateurs nouveaux ou existants dans Braze.
...
Vous pouvez charger et mettre à jour les profils d’utilisateur via des fichiers CSV sur la page **User Import (Importation d’utilisateurs)**. Lors de l’importation des données client, vous devez spécifier l’identifiant unique de chaque client, également appelé `external_id`.
...
Pour créer une campagne multicanale, rendez-vous sur la page **Campaigns**, sélectionnez **Create Campaign (Créer une campagne)** puis **Multichannel Campaign (Campagne multicanale)**. Une fois dans une campagne multicanale, sélectionnez **Add Messaging Channel (Ajouter un canal de communication)** depuis l’onglet « Composer » pour ajouter les canaux souhaités. Cliquez sur les icônes de canal qui apparaissent pour basculer entre différents composeurs de messages lorsque vous créez le texte de votre campagne pour les différents canaux.
...
### Quelle est la différence entre les Engagement Reports (Rapports d’engagement) et le créateur de rapports ?

{% apitags %}
Analytique
{% endapitags %}

Les Engagement Reports vous fournissent des CSV des statistiques d’engagement pour des messages donnés envoyés depuis des campagnes ou des Canvas à l’aide d’e-mails déclenchés. Certaines données sont agrégées au niveau de la campagne ou du Canvas et non pas au niveau de la variante ou de l’étape spécifique. Les rapports ne sont pas enregistrés dans le tableau de bord. Relancer le rapport peut fournir des statistiques actualisées.
