...
- 予約データとメッセージ・インタラクション・データの両方を使用して、ユーザーをセグメンテーションし、ターゲットに追加メッセージを送ることができる。例えば、最初のリマインダーメッセージを開封しないユーザーをリターゲティングして、予定の前に追加のリマインダーを送信できます。
...
![開始日の条件が1日超2日以下として設定された階層化カスタム属性「trips」。]({% image_buster /assets/img/use_cases/custom_nested_attribute.png %})()
