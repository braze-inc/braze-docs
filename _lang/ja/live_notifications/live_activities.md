...
- `Push Notification` 資格を Xcode プロジェクトの [**署名と機能**] の下に追加します。
...
- ライブアクティビティ更新トークンの変更ライブ・アクティビティ・アップデート（LAU）トークンの追加、更新、削除をトラッキング追跡する。
- ライブアクティビティの送信BrazeによってLive Activityが開始、更新、終了されるたびにログを記録する。
- ライブアクティビティ、結果Brazeから送信されたライブアクティビティのAppleプッシュ通知サービス(APN)への最終配信ステータスを示す。
...
現在、ライブ・アクティビティはiOSとiPadOS特有の機能だ。デフォルトでは、iPhoneまたはiPadで起動したアクティビティは、ペアリングされたwatchOS 11+またはMacOS 26+デバイスにも表示される。
