...
ヒートマップを使えば、1つのメールキャンペーンで異なるリンクがどれだけ成功したかを見ることができる。[**メッセージ分析**] セクションから、[**メールパフォーマンス**] パネルに移動します。メールキャンペーンのプレビューとヒートマップを表示するには、プレビューとヒートマップを選択する。または、バリアント名のハイパーリンクを選択してヒートマップを表示することもできます。
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="no-split">これは、同じユーザに対して複数回インクリメントできます。</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">これには、Brazeが提供する配信停止リンクのクリックも含まれる。</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="no-split">各ユーザーは1 回のみカウントされます。</td>
        </tr>
        <tr>
...
別の例として、バナーキャンペーンで5つの_ユニークインプレッションを_見たとする。つまり、以下のステップをすべて実行したユーザーのデバイスはわずか5台しかなかったことになる：
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-clicks">ユニーククリック数</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} これは、7日間にわたるメールのトラッキング、追跡、および測定によるものである。 <a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a>.これには、Brazeが提供する配信停止リンクのクリックも含まれる。この数値は5から10% の範囲内でなければなりません。10%を超えるものは例外です。
            </td>
        </tr>
        <tr>
