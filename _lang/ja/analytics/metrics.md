...
<i>本文のクリックは</i>、従来のエディターで作成されたボタン（ボタン1、ボタン2）のないメッセージをユーザーがクリックしたときや、HTMLエディターやドラッグ＆ドロップエディターで作成されたメッセージが引数なしで<code>brazeBridge.logClick</code>（）を使用したときに発生する。
{% endif %}
...
<i>Lifetime Revenueは</i>、開始以来受け取った<code>PurchaseEventsの</code>価格総額（米ドル）である。
{% endif %}
...
<i>1日の購入</i>件数は、期間中のユニークな<code>PurchaseEventの</code>合計の平均である。
{% endif %}
