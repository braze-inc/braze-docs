...
{% alert note %}
プライバシーの観点から、Brazeのプッシュトークン・プロバイダーは不定期にトークンを失効させることがあり、アンインストール数が一定期間に急増することがある。<br><br>これらの変更を検証するには、ダイレクトプッシュの開封率などのユーザーアクション指標と一緒にアンインストール追跡を監視する。アンインストールが急増しても、ダイレクトプッシュトークンの開封が安定している場合、その急増は実際のユーザー行動ではなく、パートナーが古いトークンを取り消したことを反映している可能性が高い。
{% endalert %}
