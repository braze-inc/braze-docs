...
バナーによって、マーケティングチームやプロダクトチームは、アプリやWebサイトのコンテンツをダイナミックにパーソナライズすることができ、リアルタイムのユーザーの適格性や行動を反映することができる。メッセージをインラインで持続的に表示し、ユーザーセッションの開始時に自動的に更新される、文脈に応じた非侵入型のエクスペリエンスを提供する。
...
| 販売・プロモーション | 特集コンテンツ、トレンド商品、継続中のブランドキャンペーンを、ユーザーエクスペリエンスを妨げることなく、持続的かつ直接的にホームページで紹介する。 |
...
- ダイナミックなパーソナライゼーションBrazeに内蔵されたパーソナライゼーションツールとLiquidロジックを使用して、新しいユーザーセッションごとにバナーがダイナミックに更新され、コンテンツが常に最新でパーソナライズされた状態に保たれる。
- **ネイティブの優先順位付け：**複数のバナーが同じプレースメントをターゲットにしている場合の表示優先度を設定することで、適切なメッセージを適切なタイミングでユーザーに届けることができる。
- **カスタムHTMLのサポート：**カスタムHTMLブロックを組み込むことで、高度なカスタマイズや既存のWebスタイルとのシームレスな統合が可能。

## {#about-banners}バナーについて{#about-banners}

### 配置 ID {#placement-id}

バナープレイスメントとは、[Braze SDKで作成した]({{site.baseurl}}/developer_guide/banners/placements/)アプリやWebサイトの特定の場所にバナーを表示すること。

よくある場所としては、ホームページのトップ、商品詳細ページ、チェックアウトフローなどがある。プレースメントが作成されると、[バナーキャンペーンで]({{site.baseurl}}/user_guide/message_building_by_channel/banners/creating_campaigns/)バナーを[割り当てる]({{site.baseurl}}/user_guide/message_building_by_channel/banners/creating_campaigns/)ことができる。
...
バナーメッセージはHTMLコンテンツとしてアプリやWebサイトに配信され、通常はiframe内にレンダリングされる。これにより、バナーがデバイス間で一貫してレンダリングされるようになり、スタイルとスクリプトを他のコードから分離することができる。
