...
| タイプ | アスペクト比 | 画質 | メモ |
| --- | --- | --- | --- |
| モーダル(画像のみ) | 1:1 | 最大推奨解像度：60 x 60 px <br> 最低解像度：60 x 60 px | メッセージは、ほとんどのアスペクト比の画像に合うようにリサイズされる。推奨される最大解像度はアスペクト比が3:5であるため、最適な結果が得られない可能性がある。より大きな画像は使用可能だが、読み込む時間が長くなる可能性がある。<br> 画像写真の理想的な縦横比は1:1であり、この比率を満たしていないとアップロード時に警告が出ることがある。この警告は最良の結果を得るための提案であり、より大きな画像のアップロードを妨げるものではない。 |
