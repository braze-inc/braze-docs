...
1. [**オーディエンス**] > [**セグメントエクステンション**] > [**新規エクステンションの作成**] > [**テンプレートで開始**] の順に移動し、[**イベント用のカタログセグメント**] テンプレートを選択します。<br><br> ]({% image_buster /assets/img/b2b/select_a_template.png %})[テンプレートを選択] モーダルで、イベントまたは購入のカタログセグメントオプションを選択できる。()<br><br>SQLエディタには、ユーザーイベントデータとカタログデータを結合し、特定のカタログアイテムにエンゲージしたユーザーをセグメンテーションするテンプレートが自動的に入力される。<br><br>]({% image_buster /assets/img/b2b/enter_new_name.png %})[変数] タブが開いた状態の新しいエクステンションの SQL エディター。()<br><br>
