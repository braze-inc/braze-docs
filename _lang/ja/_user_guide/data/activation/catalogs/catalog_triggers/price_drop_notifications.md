...
> このページでは、値下げ通知の仕組みと設定方法、使い方について説明する。BrazeカタログとCanvasによる価格低下通知を組み合わせることで、商品の価格が下がったときに顧客に通知することができます。
...
![値下げ機能がオンになっていることを示すカタログ設定。値下げルールは、元の価格に対して3％の変更である。]({% image_buster /assets/img/price_drop_notifications.png %})
...
- Brazeは、カタログアイテムを毎分更新するために10件のリクエストを処理します。更新エンドポイントでは、1リクエストにつき50アイテムの更新が可能で、1分あたり最大500アイテムの更新をサポートし、在庫切れ通知をトリガーすることができる。