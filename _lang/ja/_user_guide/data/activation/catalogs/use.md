...
任意のメッセージ作成画面で、プラスアイコン<i class="fas fa-plus-circle"></i>を選択して**「パーソナライゼーションを追加」**モーダルを開き、**パーソナライゼーションの種類**として**「カタログアイテム」**を選択する。次に、カタログ名を選択する。先ほどの例を使って、「ゲーム」カタログを選択する。
...
メッセージングで一つのアイテムしか入れられないわけではない。**「パーソナライゼーションを追加」**モーダルを使って、一度に最大3つのカタログアイテムを追加できる。さらに追加するには、コンポーザーで再度**「パーソナライゼーションを追加」**を選択し、追加で表示するカタログアイテムや情報を選ぶ。
...
この例では、カタログアイテムフィールドの文字数によって表示される`venue_name`メッセージが異なる。が空白の場合`venue_name`、メッセージは中止される。
