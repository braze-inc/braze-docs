...
- プロパティとしてを`r_details`列挙するAPIリクエスト
- その順序のネストされたプロパティ
...
![カスタムイベントのプロパティフィルターで、ユーザーがネストされたプロパティを選択する。]({% image_buster /assets/img/nested_object2.png %})
...
![プロパティフィルターを追加するユーザーは、カスタムr_details.nameイベント用のサンドイッチ皇帝だ。]({% image_buster /assets/img/nested_object1.png %})

`r_details.name`:サンドイッチ皇帝<br>
...
## メッセージング内のネストされたオブジェクトのテスト

ダッシュボードの**&プレビュー**テストツールは、ネストされたオブジェクトや階層化カスタム属性に対して、モックデータの追加をサポートしていない。Liquidを通じてネストされたデータを参照するメッセージをテストするには、そのネストされた属性を持つ既存ユーザーとしてメッセージをプレビューするか、カスタムイベントプロパティを持つメッセージをプレビューするためにライブキャンペーンを起動してテストユーザーを割り当てることができる。

### 階層化カスタム属性

1. APIを通じて、ネストされた属性をテストユーザープロファイルにインポートする。
2. キャンペーンまたはキャンバスで、**プレビュー&テスト**に進む。
3. **プレビューをユーザーとして**選択し、テストユーザーを検索する。そのユーザーのプロファイル上の実際のネストされた属性を使って、Liquidは解決される。

### ネストされたイベントプロパティ

ネストされたイベントプロパティは、ライブイベントのトリガーを必要とするため、ダッシュボードではプレビューできない。テストのため：

1. テストユーザーのみを対象とし、ネストされたプロパティを持つカスタムイベントによってトリガーされる（または参照される）キャンペーンまたはキャンバスステップを作成する。
2. テストオーディエンスにキャンペーンを開始する。
3. カスタムイベントをネストされたオブジェクトペイロードと共に、テストユーザーのユーザープロファイルに記録する（APIまたはSDKを使用）。
4. ネストされたプロパティ値でメッセージが正しく表示されることを確認せよ。

## よくある質問

### ネストされたオブジェクトを使用すると、追加のデータポイントが記録されるのか？

この機能を追加した結果、データポイントの記録方法に変更はない。ネストされたオブジェクトに基づくセグメンテーションは、追加のデータポイントを使用しないセグメントエクステンションを利用する。
