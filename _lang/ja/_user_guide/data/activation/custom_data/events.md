...
Brazeにおける標準イベントとは、プラットフォーム全体でBrazeが認識する事前定義されたアクションである。[カスタムイベント](#custom-events)とは異なり、標準イベントは作成したり名前を付けたりする必要はない。最初から組み込まれているからだ。しかし、すべての標準イベントが同じ方法でトラッキングされるわけではない。

SDKを統合すると、以下のイベントが自動的にトラッキングされる：

- セッション開始
- セッション終了

追加設定後、以下のイベントがトラッキングされる：

- [購入イベント](#purchase-events): 開発チームはSDKの購入メソッドを使ってこれらを記録する。詳細については、購入イベントのセクションを参照せよ。
- メールエンゲージメントイベント（メール開封やリンククリックなど）：Brazeメールを設定し、メールトラッキングのイネーブルメントを有効にすると、Brazeによって追跡される。
- プッシュエンゲージメントイベント（プッシュ通知の開封やクリックなど）を送信する：Brazeでプッシュ通知を設定し、アプリ内でBraze SDKとプッシュ通知処理を統合した後にトラッキングされる。

マーケターとして、標準イベントを活用すればユーザーの行動やエンゲージメントを理解できる。例えば、セッションデータはユーザーがアプリやサイトをどのくらいの頻度で開封するかを示し、購入イベントは時間の経過に伴う収益をトラッキングするのに役立つ。

## 購入イベント

購入イベントを記録し、ユーザーによる購入をトラッキングする。Braze SDKを統合した後、開発チームはSDKの購入メソッドを使って購入を記録できる。購入イベントを使って購入のトラッキングを行うと、Brazeから直接、時間の経過とともに、また異なる収益源にわたって収益を監視できる。
...
この例では、"product_name"は購入された商品の名前である。「USD」は購入の通貨である。「1.99」は商品の価格である。「1」は購入数量である。
...
この例では、"completed_level"はユーザーがゲームでレベルをクリアしたときに記録されるカスタムイベントの名前だ。そのカスタムイベントは Braze のユーザープロファイルに記録され、キャンペーンのトリガーやメッセージングのパーソナライゼーションに使用できます。
...
この例では、"updated_profile"はユーザーがプロファイルを更新した際に記録されるカスタムイベントの名前である。
