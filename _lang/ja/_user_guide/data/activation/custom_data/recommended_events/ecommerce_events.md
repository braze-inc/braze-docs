...
| プロパティ名 | required | データタイプ | 説明 | 
|---|---|---|---|
| `cart_id` | はい | string | `cart_id`を提供するサードパーティプラットフォームを使用していない場合は、[Braze セッション ID]({{site.baseurl}}/developer_guide/analytics/tracking_sessions)を使用できます。 |
...
| `cart_id` | いいえ | string | `cart_id`を提供するサードパーティプラットフォームを使用していない場合は、[Braze セッション ID]({{site.baseurl}}/developer_guide/analytics/tracking_sessions)を使用できます。 | 
| `total_value` | はい | フロート | カートの合計金額。 |
...
!["Trans アクション s"タブ(ユーザー計算フィールドs)。]({% image_buster /assets/img/Shopify/transactions_tab.png %}){: style="max-width:70%;"}
