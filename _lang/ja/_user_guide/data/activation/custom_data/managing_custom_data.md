...
カスタム属性やカスタムイベント、購入イベントの中には、データポイントの記録が多すぎるものや、マーケティング戦略に役立たなくなったもの、エラーで記録されたものがある場合がある。 
...
カスタムデータをブロックリストに登録するには、ワークスペースの以下のドロップダウンで[ユーザー権限]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/)が必要だ。
...
* カスタムデータを削除しても[、データポイントは]({{site.baseurl}}/user_guide/data/data_points)記録されない。また、使用するための新しいデータポイントも生成されない。
