...
ブレーズが無効な暗号化された電子メールアドレスを検出しました。Braze にインポートする前に、すべてのメールアドレスが適切に暗号化されていることを確認します。

- **[Braze でメールアドレス]({{site.baseurl}}/user_guide/analytics/field_level_encryption/#step-3-import-and-update-users)**を更新またはインポートする場合は、メールが含まれている場所でハッシュされたメール値を使用します。これらのハッシュメール値は、内部チームによって提供されます。 
- **新しいユーザ** を作成するときは、`email_encrypted` をユーザの暗号化されたメール値で追加する必要があります。それ以外の場合、ユーザーは作成されません。同様に、メールを持たない既存のユーザにメールアドレスを追加する場合は、`email_encrypted` を追加する必要があります。それ以外の場合、ユーザーは更新されません。

### 行の欠落

インポートされたユーザー数が CSV ファイルの合計行数と一致しない理由はいくつかあります。

- **重複する外部ID、ユーザエイリアス、ブレーズID、電子メールアドレス、または電話番号:**external ID 列が重複している場合、行の形式が正しくても、インポートされた行の形式が変形したり、行がインポートされなかったりする可能性があります。場合によっては、特定のエラーが報告されないことがあります。CSV に重複した external ID がないか確認してください。ある場合は、重複を削除して再度アップロードを試してください。
- **アクセント付きの文字:**CSV 内の名前や属性に、アクセント付きの文字が含まれている可能性があります。問題を防ぐために、ファイルが UTF-8 でエンコードされていることを確認してください。
- **Braze ID は孤立したユーザに属します。**ユーザが孤立している場合、孤立がマージされた残りのユーザは、孤立しているユーザのBraze ID をプロファイルに関連付けることはできません。この場合、Braze は更新するユーザーを見つけないため、行はインポートされたものとしてカウントされません。
- **空行:**CSV ファイルに空白行があります。これは、テキストエディタプログラムでCSV を開いた場合に確認できます(Excel やSheets を使用しないでください)。空の行でCSV をアップロードすると、不正な形式のデータを持つ行があることを示すエラーメッセージが表示されます。
- **二重引用符を含む(`"`):**この文字は無効で、行が不正形式になります。代わりに、一重引用符(`'`) を使用します。
- **一貫性のない改行:**たとえば、CSV ファイルが最初の行に`\n` を使用し、後続の各行に`\r\n` を使用する場合、データの最初の行はヘッダの一部として処理され、そのデータは期待どおりにインポートされません。これは、16進エディタまたは空白文字を区別する特殊なテキストエディタで確認できます。
- **不正にエンコードされたファイル**CSV ファイルには、アクセントを持つ名前または属性を含めることができますが、ファイルを適切にインポートするには、UTF-8 にエンコードする必要があります。その他の文字エンコーディングは、場合によっては機能しますが、完全に互換性があるのはUTF-8 のみです。
...
また、0 で始まる数値は文字列とみなされるため、0 で始まる数値属性では問題が発生します。Braze がその文字列を変換するときは、8 進数値(0 から7 までの数字を使用) のように扱うことができます。つまり、それは対応する10 進数値に変換されます。たとえば、CSV ファイルの値が0130 の場合、ろう付けプロファイルには88 が表示されます。この問題を回避するには、文字列データ型で属性を使用します。ただし、このデータ型はセグメンテーション番号の比較では使用できません。

### デフォルトの属性タイプ

一部のデフォルト属性では、ユーザー更新に対して有効な特定の値のみを受け入れることができます。ガイダンスについては、[CSV の構築]({{site.baseurl}}/user_guide/data/user_data_collection/user_import/#constructing-your-csv)を参照してください。

末尾のスペースと大文字と小文字の違いによって、値が無効と解釈される場合があります。たとえば、次のCSV ファイルでは、最初の行(`brazetest1`) のユーザのみが、受け入れられた値が`unsubscribed`、`subscribed`、および`opted_in` であるため、電子メールとプッシュステータスが正常に更新されます。 
...
### "Select CSV File" ボタンが動作しない

**Select CSV File**ボタンが機能しない理由はいくつかあります。

- **ポップアップブロッカー:**これにより、ページが表示されない場合があります。ブラウザでBraze ダッシュボードWeb サイトのポップアップが許可されていることを確認します。 
- **古いブラウザ:**ブラウザが最新であることを確認します。最新でない場合は、最新バージョンに更新します。
- **バックグラウンドプロセス:**すべてのブラウザインスタンスを閉じ、コンピュータを再起動します。
