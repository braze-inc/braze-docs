...
匿名ユーザには`external_ids` はありませんが、別の識別子(ユーザエイリアス) を使用して匿名ユーザプロファイルを割り当てることができます。これにより、匿名ユーザープロファイルに対して、`external_ids` で識別されたユーザーと同じアクションを実行できるようになります。たとえば、Braze API を使用して、匿名ユーザに関連付けられたイベントおよび属性をログに記録し、セグメンテーションフィルタ[External User ID is blank]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#external-user-id) を使用して、メッセージング内のこれらのユーザをターゲットにすることができます。
...
![空白の外部ユーザーID と真の"is_lead_profile"カスタム属性のセグメントフィルタ。][1]
