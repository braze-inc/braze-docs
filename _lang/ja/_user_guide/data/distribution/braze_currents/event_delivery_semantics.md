...
Currentsの連携を設定する際は、ストレージバケットとの接続を確認するために**「テストイベントを送信」**をクリックする。これらのテストイベントは、あなたの統合がデータを正しく受信し処理できることを確認するものである。

{% alert important %}
**テストイベントデータ形式：**テストイベントには、各フィールドの正しいデータ型に合致するプレースホルダー値が含まれているが、現実的あるいは正確なデータは含まれていない。例えば、ある`timezone`フィールドには有効なタイムゾーン識別子（「America/Chicago」など）ではなくUUIDのような文字列が含まれている場合がある。また、`campaign_name`やといった他のフィールドも、実際のデータではなくプレースホルダー値を含んでいることがある`ip_pool`。<br>

これは想定通りの動作だ。テストイベントは主に接続と統合設定のテストを目的としており、データの正確性を検証するためのものではない。正確なデータで実際のイベントを確認するには、テスト用のCurrents統合を使用し、実際のイベントデータをパイプライン経由で送信する。
{% endalert %}
...
{% alert tip %}
ファイルの命名規則は変更される可能性がある。Brazeは、バケット内のプレフィックスが.<your-bucket-prefix>で始まる全てのキーを検索することを推奨する。
{% endalert %}
