...
Currentsインテグレーションを設定する場合は、**テストイベントの送信**をクリックして、ストレージバケットとのコネクションを確認します。これらのテストイベントは、統合がデータを正しく受信して処理できることを検証します。

{% alert important %}
**テストイベントデータフォーマット:**テストイベントには、フィールドごとに正しいデータタイプと一致するプレースホルダー値が含まれますが、リアルなデータやAC キュレートデータは含まれません。たとえば、`timezone` フィールドには、有効なタイムゾーン識別子("America/Chicago" など) の代わりにUUID に似た文字列を含めることができます。また、`campaign_name` や`ip_pool` などの他のフィールドには、実際のデータではなくプレースホルダ値を含めることもできます。<br>

これは予期される動作です。テストイベントは、主に接続と統合のセットアップをテストするためのものであり、データの正確性を検証するためのものではありません。ac キュレート データを使用して実際のイベントを表示するには、テストCurrentsインテグレーションを使用して、パイプライン経由で実際のイベントデータを送信します。
{% endalert %}
