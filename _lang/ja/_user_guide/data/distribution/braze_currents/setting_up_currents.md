...
| 要件 | 提供元 | アクセス | 説明
|---|---|---|---|
| パートナーのアカウント | そのパートナーと取引先を整理するか、Braze アカウントマネージャーに連絡して提案を求めてください。 | パートナーのサイトを確認するか、そのパートナーに連絡してサインアップします。 | Brazeは、あなたの会社のアカウントを通じてそのデータにアクセスできない場合、パートナーにデータを送信しない。
| パートナーAPIキーまたはトークン | 通常はパートナーのダッシュボードにあります。 | 指定したBraze フィールドにコピーアンドペーストします。 | Braze には、パートナーの「連携」ページに、このための指定フィールドがあります。私たちはあなたのデータをどこに送るかを地図化するためにこれが必要です。**パートナーキーまたはトークンを最新の状態に保ちます。不正な認証情報によってコネクターが無効になり、イベントがドロップされる場合があります。**
...
| バケツ、フォルダパス | 一部のパートナーは、バケツごとにデータを整理し、分類している。これはパートナーのダッシュボードにあります。 | これが必要な場合は、バケット名またはファイルパスをBraze内の指定された領域に正確にコピーします。 | これはパートナーによっては必要なことではあるが、必要なときに正しく行うことが重要です。 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }

{% alert important %}
パートナーのキーまたはパートナーのトークンと認証の詳細を最新の状態に保つことが重要です。コネクターの認証情報の有効期限が切れると、コネクターはイベントの送信を停止します。これが **5 日** 以上続く場合、コネクタのイベントはドロップされ、データは永続的に失われます。
{% endalert %}
...
開始するには、**Partner Integrations**> **Currents** に移動します。Currents の [連携管理] ページが表示されます。
...
![フィールドトランスフォーメーションの追加]({% image_buster /assets/img/current3.png %})
...
![Braze ダッシュボードの" Currents Test" ページ。]({% image_buster /assets/img/currents/current_test_events.png %}){: style="max-width:70%;"}

#### 電流コネクタのテスト

Currents のテストコネクターは、弊社の既存のコネクターの無料版であり、さまざまな送信先のテストと試行に使用できます。Currents のテストは次のとおりです。

- 1 ワークスペースあたり最大10 個の試験Currentsコネクター。
- 24 時間の固定時間ごとに1500 件の大イベントを集計し、UTC の深夜0 時に再度設定します。このイベントの合計はダッシュボードで 1 時間ごとに更新されます。

Test Currents コネクターが送信制限に達すると、コネクターは翌日(UTC の午前0 時) までイベントを送信しません。
