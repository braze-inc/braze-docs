...
| 要件 | 提供元 | アクセス | 説明
|---|---|---|---|
| パートナーのアカウント | そのパートナーとアカウントを設定するか、提案が必要な場合はBrazeのアカウントマネージャーに連絡する。 | そのパートナーのサイトを確認するか、そのパートナーに連絡して登録するんだ。 | Brazeは、あなたの会社のアカウントを通じてそのデータにアクセスできない場合、パートナーにデータを送信しない。
| パートナーAPIキーまたはトークン | 通常はパートナーのダッシュボードにあります。 | 指定されたBrazeのフィールドにコピーして貼り付けろ。 | Braze には、パートナーの「連携」ページに、このための指定フィールドがあります。データを送信する先を特定するために、これが必要だ。**パートナーキーやトークンは常に最新の状態に保て。無効な認証情報はコネクタを無効化し、イベントを消失させる可能性がある。**
...
| バケツ、フォルダパス | 一部のパートナーは、バケツごとにデータを整理し、分類している。これはパートナーのダッシュボードにあります。 | これが必要な場合は、バケット名またはファイルパスをBrazeの指定されたスペースに正確にコピーせよ。 | これはパートナーによっては必要なことではあるが、必要なときに正しく行うことが重要です。 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }

{% alert important %}
パートナーのキーまたはパートナーのトークンと認証の詳細を最新の状態に保つことが重要です。コネクターの認証情報の有効期限が切れると、コネクターはイベントの送信を停止します。これが**5日**以上続くと、コネクタのイベントは破棄され、データは永久に失われる。
{% endalert %}
...
始めるには、**[パートナー連携]** > **[Currents]** に移動する。Currents の [連携管理] ページが表示されます。
...
- ワークスペースあたり最大10個のテストCurrentsコネクタ。
- 固定の24時間期間ごとに、合計最大1,500件のイベントが発生する。これはUTCの深夜0時にリセットされる。このイベントの合計はダッシュボードで 1 時間ごとに更新されます。

テストCurrentsコネクタが送信上限に達すると、そのコネクタは翌日の午前0時（UTC）までイベントを送信しない。
