...
{% alert note %}
[古いナビゲーション]({{site.baseurl}}/navigation)を使用している場合は、**設定の管理**にこれらのページがあります。
{% endalert %}
...
多数のイベントと属性をブロックリストに含めることは可能ですが、推奨されません。イベントが実行されたり、属性が Braze に送信される (可能性がある) たびに、このイベントや属性を禁止リスト全体と照合しなければならないためです。リストに存在するものは送信されません。この処理には時間がかかり、リストが大きくなるとアプリの動作が遅くなる可能性があります。もし、イベントや属性が今後不要であれば、次回のリリース時にアプリのコードから削除すべきです。
