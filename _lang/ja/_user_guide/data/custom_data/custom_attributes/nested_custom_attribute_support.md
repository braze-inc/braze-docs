...
たとえば、`favorite_book` というユーザプロファイルにカスタム属性を定義するとします。このカスタム属性は、階層化属性 `title`、`author`、および `publishing_date` を持つオブジェクトとして定義できます。
...
カスタム属性オブジェクトは、次のような[データ型]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/#custom-attribute-data-types)を格納できます。
