...
]({% image_buster /assets/img/cloud_ingestion/s3_ARN.png %})キューにアクセスできる人を定義するJSONオブジェクトの例で「詳細」を選択する。()
...
1. AWS コンソールの [IAM] セクションに移動し、ナビゲーションバーの [**ポリシー**] を選択してから [**ポリシーを作成**] を選択します。<br><br>]({% image_buster /assets/img/create_policy_1_list.png %})AWSコンソールの「ポリシーの作成」ボタン()<br><br>
...
]({% image_buster /assets/img/create_policy_3_name.png %})「new-policy-name」という名前のポリシーの例。"()
...
<br><br>]({% image_buster /assets/img/create_role_1_list.png %})[ロールの作成] ボタン。()<br><br>
...
<br><br>]({% image_buster /assets/img/create_role_3_attach.png %})new-policy-name が選択されたロールARN。()<br><br>

ロールに名前と説明を指定し、[**Create Role**] を選択します。

<br><br>]({% image_buster /assets/img/create_role_4_name.png %})「new-role-name」という名前のロールの例。()<br><br>
...
]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_1.png %})新しいインポート同期を作成するために S3で表示されるセキュリティ認証情報の例。()

{: start="3"}
3\.統合に名前を付け、この統合のデータ型を選択する。 

<br><br>]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_2.png %})データタイプとしてユーザー属性を選択して「cdi-s3-as-source-integration」の同期の詳細を設定する。()<br><br>

{: start="4"}
4\.アクセスや権限の問題で同期が切れた場合に通知を受け取る連絡先メールアドレスを追加します。オプションで、ユーザーレベルのエラーと同期の成功の通知をオンにします。 

<br><br> ]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_3.png %})同期エラー通知の通知設定を設定する。()<br><br>
