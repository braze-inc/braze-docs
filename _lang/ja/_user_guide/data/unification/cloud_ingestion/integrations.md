...
- **ユーザー識別子カラム**\- テーブルには、1つ以上のユーザー識別子カラムを含めることができる。各行は、識別子 (`external_id` 単独か、`alias_name` と `alias_label`、`braze_id`、`email` または `phone` の組み合わせ) を 1 つのみ含まなければなりません。ソーステーブルには、1つ、2つ、3つ、4つ、または5つすべての識別子タイプの列が含まれる場合があります。
    ...
    - `EMAIL` - ユーザーのメールアドレス。同じメールアドレスを持つ複数のプロファイルが存在する場合、更新は最も最近更新されたプロファイルを優先する。メールと電話番号の両方を入力した場合、メールが主要な識別子として使用される。
    - `PHONE` - ユーザの電話番号。同じ電話番号を持つ複数のプロファイルが存在する場合、更新は最も最近更新されたプロファイルを優先する。
...
CDIはBigQueryのパーティションをサポートする。(例えば、データセットのサイズに応じて、日単位、週単位、時間単位といった粒度で)関数`UPDATED_AT`によるパーティション分割を行うと、BigQueryはスキャンする必要のあるデータを絞り込める。これにより、非常に大きなテーブルのパフォーマンスと効率が向上する。

他のフィールドで分割するな。様々な設定を試して、自分のデータに最適な設定を見つけろ。

すべてのCDIクエリはで`UPDATED_AT`フィルターするが、この動作は変更される可能性がある。テーブルスキーマを設計するときは、クエリにこの句を含める必要_がない_ようにせよ。

詳細については、[BigQueryのパーティショニングに関するドキュメント](https://docs.cloud.google.com/bigquery/docs/partitioned-tables)を参照せよ。
{% endalert %}
...
Brazeダッシュボードで、**データ設定**＞**クラウドデータ取り込み**に移動し、**新規データ同期の作成**を選択する。その後、**Snowflakeインポート**を選択する。
...
{% alert note %}
**Snowflake**アカウントロケーターフィールドには、Snowflake[アカウント識別子](https://docs.snowflake.com/en/user-guide/admin-account-identifier)を入力する。通常、この識別`xy12345.us-east-1.aws`子は次のような形式に従う。これはデータベース名や倉庫名とは異なる。
{% endalert %}
...
Brazeダッシュボードで、**データ設定**＞**クラウドデータ取り込み**に移動し、**新規データ同期の作成**を選択する。その後、**Amazon Redshiftインポート**を選択する。

#### ステップ 2.1: Redshift の接続情報とソーステーブルの追加

Redshift データウェアハウスとソーステーブルの情報を入力します。プライベート・ネットワーク・トンネルを使用している場合は、スライダーを切り替えてトンネル情報を入力する。次にステップを進める。 

{% alert note %}
Brazeのダッシュボードでは、**データベース名の**フィールドは英字（A–Z、a–z）、数字（0–9）、アンダースコアのみ(_),を受け付ける。Amazon Redshiftがデータベース識別子で追加の文字をサポートしているにもかかわらず、である。
{% endalert %}
...
Brazeダッシュボードで、**データ設定**＞**クラウドデータ取り込み**に移動し、**新規データ同期の作成**を選択する。その後、**Google BigQueryインポート**を選択する。
...
Brazeダッシュボードで、**データ設定**＞**クラウドデータ取り込み**に移動し、**新規データ同期の作成**を選択する。その後、**Databricksインポート**を選択する。
...
Microsoft Fabric 用の新しいデータ同期を作成します。Brazeダッシュボードで、**データ設定**＞**クラウドデータ取り込み**に移動し、**新規データ同期の作成**を選択する。その後、**Microsoft Fabricインポート**を選択する。
