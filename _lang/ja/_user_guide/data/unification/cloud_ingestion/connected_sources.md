...
2. サービス・アカウントが、接続されたソースに使用されているスキーマの`CREATE TABLE` 権限を持っていることを確認する。 
...
ソースが作成されたら、それを使用して1つ以上のCDIセグメントエクステンションを作成できる。このソースでセグメントを作成する方法の詳細については、[CDI セグメントエクステンションのドキュメントを]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/cdi_segments/)参照のこと。
