...
* **同期名：**同期設定の名前。  
* **IDを実行する：**シンクの特定の実行に対する一意の識別子。詳細を見るには、このIDを選択する。これは[CDI APIエンドポイントでも]({{site.baseurl}}/api/endpoints/cdi)使用でき、Braze Supportで実行された同期を参照することもできる。   
...
{% alert important %}
すべての行の同期ログのエクスポートは、現在早期アクセス中である。早期アクセスへの参加に興味がある方は、Brazeのアカウントマネージャーに連絡を。
{% endalert %}
