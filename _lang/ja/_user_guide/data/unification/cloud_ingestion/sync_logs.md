...
同期ログにアクセスするには、**Data Settings**>**Cloud Data Ingestionで** **Sync Log**タブを選択する。

## シンクログダッシュボードを理解する

メインの**Sync Log**ページでは、最近実行されたシンクの現在または最終ステータス別の概要など、すべてのシンク実行のハイレベルな概要が表示される。
...
* **同期名：**同期設定の名前。  
* **IDを実行する：**シンクの特定の実行を示す一意の識別子。詳細を見るには、このIDを選択する。これは[CDI APIエンドポイントでも]({{site.baseurl}}/api/endpoints/cdi)使用でき、Braze Supportで実行された同期を参照することもできる。   
...
このセクションでは、開始時間、終了時間、期間、ソースから読み込まれた行の合計数など、選択されたランの概要を示す。また、何行が成功し、何行がエラーになったかをカウントすることもできる。
...
**Export rowsを**選択して、シンク実行の行レベルのログをエクスポートする。次に、エクスポート方法を選択する：

* **エラーのある行：****エラー**ステータスの行のみを含むファイルをダウンロードする。
* **全列である：**実行中に処理されたすべての行を含むファイルをダウンロードする。
...
- **行が変更されなくても警告を発する：**同期が成功し、新規または更新された行がゼロの場合でも通知を受け取る。