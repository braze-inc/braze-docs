...
]({% image_buster /assets/img/csv_import/import_users_page.png %})Braze ダッシュボードの [ユーザのインポート] ページ。
...
<!-- old image -->
]({% image_buster /assets/img/csv_import/upload_csv.png %})CSV のアップロードが完了しましたが、ある列でデータ型が混在しているエラーが発生しました{: style="max-width:70%"}
...
]({% image_buster /assets/img/csv_import/add_filter_group.png %})「ハロウィンシーズンの楽しみ」というタイトルの CSV ファイルを含む「CSVから更新/インポート」フィルターを持つフィルターグループ。{: style="max-width:85%;"}
