...
]({% image_buster /assets/img_archive/createsegment2.png %})セグメントを作成するモーダル。{: style="max-width:70%;"}()
...
]({% image_buster /assets/img_archive/filter_recent_location.png %})円内で最新の場所を示すためのフィルター()
...
]({% image_buster /assets/img_archive/location_circle.png %})ニュージャージー州とニューヨーク州の間の都市の円形の輪郭。{: style="max-width:70%;"}()
...
]({% image_buster /assets/img_archive/create_polygon.png %})選択された多角形領域としてのニューヨーク州の概要。{: style="max-width:70%;"}()
