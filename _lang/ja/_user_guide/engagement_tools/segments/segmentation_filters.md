...
glossaries:
  ...
  - name: アプリあり
    ...
    tags:
      - App
  - name: 最新のアプリバージョン名
    description: "ユーザーのアプリの最新の名前によってセグメント化します。<br><br>\"未満\"または\"以下\"を使用する場合、主なアプリ版が存在しない場合、このフィルターは`true`を返します。これは、アプリ版よりもユーザーが古いためです。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。"
    tags:
      - App 
  - name: 最新のアプリバージョン番号
    description: "ユーザーのアプリの最新バージョン番号によってセグメント化します。<br><br>\"少ない\" か\"以下\" を使うとき、主アプリ版が存在しない場合、このフィルターは`true` を返す。なぜなら、このユーザーはアプリ版より古いからである。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。<br><br>現行のアプリ版の入力には、時間がかかる場合があります。アプリ版のユーザープロファイル 更新は、情報がSDKによってキャプチャされたときに、ユーザーがアプリをいつ開封するかに依存します。ユーザーがアプリを開封していない場合、最新版は更新 d になりません。これらのフィルターも遡及的にアプリしないだろう。\"より大きい\"または\"等しい\"を現在および将来のバージョンに使用すると良いですが、以前のバージョンフィルターを使用すると予期しない動作が発生する可能性があります。"
    tags:
      - App 
  - name: アンインストール
