...
glossary_top_text: "Braze SDK には、特定の機能と属性に基づいてユーザーをSegmentし、ターゲットにするための強力なフィルターが備わっています。フィルターカテゴリごとに、これらのフィルターs を検索または絞り込むことができます。<br><br>ユーザーをセグメントするために使用できる、さまざまなカスタム属性のデータタイプについては、<a href=\"/docs/user_guide/data_and_analytics/custom_data/custom_attributes/#custom-attribute-data-types\"> カスタム属性データタイプ</a> をご確認ください。"
...
glossaries:
  ...
  - name: "作成日時:"
    ...
    tags:
      - Other Filters
  - name: 階層化カスタム属性
    description: "カスタム属性のプロパティである属性。<br><br>階層化された時間のカスタム属性にフィルターを適用する場合、基準として [年間通算日] または [時刻] を選択できます。\" Day of Year\" 比較のために月と日のみをチェックします。\"Time\" 年を含む完全なタイムスタンプを比較します。"
    tags:
      - Custom attribute
  - name: 定期的なイベントの日
    description: "このフィルターでは、データタイプが\"date\"のカスタム属性の月と日を調べますが、年は調べません。このフィルターは毎年発生するイベントに役立ちます。<br><br>タイムゾーン:<br>このフィルターは、メッセージがローカルタイムスケジュールオプションを使用して送信する限り、ユーザーがどのタイムゾーンにあるかを調整します。それ以外の場合は、このフィルターでは会社のタイムゾーンを使用します。"
    tags:
      - Custom attribute
  - name: カスタムイベント
    description: "ユーザーが特別に記録された行動を実行したかどうかを決定します。<br><br> 例: <br>プロパティ activity_name で完了したアクティビティー。<br><br>タイムゾーン:<br>UTC - カレンダ日= 1 暦日は、24 ～48 時間のユーザー履歴を表示します"
    tags:
      - Custom events
  ...
  - name: 最後に実行したカスタムイベント 
    ...
    tags:
      - Custom events
  - name: X Y日間のカスタムイベント
    description: "ユーザーが、最後に指定された暦日数 (1 ～ 30 日) の間に、特別に記録されたイベントを 0 ～ 50 回実行したかどうかを決定します。(暦日= 1 暦日は、24 ～48 時間のユーザー履歴を表示します)<br> <a href=\"/docs/x-in-y-behavior/\"> ここでは、X-in-Y の動作について詳しく説明します。</a><br><br>例: <br>過去 1 暦日内にカートをちょうど 0 回放棄した<br><br>タイムゾーン:<br>UTC - すべてのタイムゾーンを考慮すると、1 日の暦日は、ユーザー履歴の24 ～48 時間を、Segmentが評価された時刻に応じて、2 日の暦日は、48 ～72 時間のユーザー履歴を参照します。"
    tags:
      - Custom events
  - name: X Y 日間のカスタムイベントプロパティ
    description: "ユーザーが、最後に指定された暦日数 (1 ～ 30 日) の間に、特別に記録されたイベントを特定のプロパティについて 0 ～ 50 回実行したかどうかを決定します。(暦日= 1 暦日は、24 ～48 時間のユーザー履歴を表示します)<br><a href=\"/docs/x-in-y-behavior/\">ここでは、X-in-Y の動作について詳しく説明します。</a><br><br>例: <br> 過去 1 暦日内にカートをちょうど 0 回お気に入りに追加した<br><br>タイムゾーン:<br>UTC - すべてのタイムゾーンを考慮すると、1 日の暦日は、ユーザー履歴の24 ～48 時間を、Segmentが評価された時刻に応じて、2 日の暦日は、48 ～72 時間のユーザー履歴を参照します。"
    tags:
      - Custom events
  - name: 電子メールアドレス 
  ...
  - name: 過去 Y 日間の X 回のセッション
    ...
    tags:
      - Sessions
  - name: 最初に使用したアプリ
    description: "アプリを開封した記録の最も早い時刻によってユーザーをセグメント化します。<em>これは、Braze SDKを統合したアプリを使用している最初のセッションをキャプチャします。</em>(24時間)<br><br>タイムゾーン:<br>会社のタイムゾーン"
    tags:
      - Sessions
  - name: 最初に使用した特定のアプリ
  ...
  - name: キャンペーンまたはキャンバスからタグ付きで受信したメッセージ
    description: "指定したキャンペーンを受信したかどうか、または指定したタグのキャンバスを受信したかどうかによって、ユーザーを分割します。\"Campaign&amp;quot からの受信メッセージ;および\"Canvas Step\"からの受信メッセージとは異なり、このフィルターは、重複メッセージを受信した同じメールまたは電話番号を持つすべてのユーザーをキャプチャします。<br><br> コンテンツカード、バナー（キャンペーンのみ）、アプリ内メッセージについては、カードやアプリ内メッセージが送信された時点ではなく、ユーザーがインプレッションを記録した時点となる。<br><br>プッシュおよびwebhookの場合、これはメッセージがユーザーに送信されるときです。<br><br> WhatsAppの場合、これは、メッセージがユーザーの機器に配信されるときではなく、最後のメッセージAPIリクエストがWhatsAppに送信されるときです。<br><br>メールの場合、メールリクエストが (実際に配信されたかどうかに関係なく) メールサービスプロバイダーに送信されるときです。複数のユーザーが同じメールアドレスを共有している場合:<br>- 最初の送信では、指定されたターゲットユーザーのプロファイルのみが更新されます。<br>- メールが配信されたとき、またはユーザーがメールまたはメール内のリンクを開封する場合、そのユーザーはすべて、メッセージを受信したことを示すアプリを共有します。<br><br>SMSの場合、これは最後のメッセージがSMSプロバイダーに配信されたときです。これは、メッセージがユーザーの機器に配信されたことを保証するものではありません。"
    tags:
      - Retargeting
  ...
  - name: キャンバスのコントロールグループ内
    description: ユーザーを、指定したキャンバスのコントロールグループにあるかどうかによって分割します。このフィルターは、キャンバスに入ったユーザーのみを評価します。<br><br>たとえば、キャンバスのコントロールグループにないユーザーをフィルターすると、キャンバスに入力したがコントロールグループにはないすべてのユーザーが表示されます。
    tags:
      - Retargeting
  ...
  - name: 利用可能なメール
    description: "ユーザーは、有効なメールアドレスを持っているかどうか、また、サブスクライブされているか、メールに選択されているかどうかによってセグメント化されます。このフィルターは、ユーザーがメールの配信停止をしているか、Brazeがハードバウンスを受け取っているか、メールがスパムとしてマークされているかの3つの条件をチェックする。これらの基準のいずれかが満たされている場合、またはユーザーにメールが存在しない場合、ユーザーは含まれません。<br><br>Eメールが利用可能なユーザー <code>false</code> キャンペーン オーディエンスから除外され、メールを受信しません。送信設定s がすべてのユーザーs(配信停止d ユーザーs を含む)に送信するように設定されている場合でも同様です。<br><br>メール s でオプトインステータスが重要な場合は、<a href=\"/docs/user_guide/engagement_tools/segments/segmentation_filters#email-address\">メールアドレス</a> の代わりにメール Available を使用します。追加の基準は、メールを受ける資格のあるユーザーを対象にするのに役立ちます。"
    tags:
      - Channel subscription behavior
  - name: メールオプトイン日
  ...
  - name: フォアグラウンドのプッシュ通知が有効になりました
    description: 暫定的なプッシュ認証があるか、フォアグラウンドプッシュが有効になっているユーザーをセグメント化します。具体的には、このカウントには以下が含まれます。<br>1. 暫定的にプッシュが承認されている iOS ユーザー。<br>2.フォアグラウンドプッシュが有効で、プッシュサブスクリプション ステータスが配信停止 d でないユーザ(アプリs のいずれか)。これらのユーザーの場合、この数にはフォアグラウンドプッシュのみが含まれます。<br><br>フォアグラウンドプッシュ有効は、配信停止 d を持つユーザーを含みません。<br><br>このSegmentでフィルターを実行すると、<em>Reachable Users</em> という名前のAndroid、iOS、ウェブサイトのユーザーの内訳が下部に表示されます。
    tags:
      - Channel subscription behavior
  - name: アプリでフォアグラウンドのプッシュ通知が有効になりました
    description: デバイス上のアプリでプッシュ通知を有効にしているかどうかによってユーザーをセグメント化します。アプリに対してフォアグラウンドプッシュが有効になっているユーザ。これはプッシュサブスクリプション ステータスを考慮に入れません。この数には、仮にフォアグラウンドおよびバックグラウンドのプッシュトークンを暫定的に承認されたユーザーが含まれます。
    tags:
      - Channel subscription behavior
  - name: バックグラウンドまたはフォアグラウンドのプッシュ通知が有効になりました
    description: ユーザーがプッシュトークンを持っており配信停止していないかどうかに基づいてセグメント化します。バックグラウンドまたはフォアグラウンドプッシュが任意のアプリで有効になっているユーザ。
    tags:
      - Channel subscription behavior
  - name: プッシュ通知のオプトイン日
  ...
  - name: 支出金額
    ...
    tags:
      - Purchase behavior
  - name: X 日間に支出した金額
    description: "最後に指定された暦日数 (1 ～ 30 日) の間に、アプリで費やした金額によってユーザーをセグメント化します。この金額には、直近の50 件の購入の合計のみが含まれます。<br> <a href=\"/docs/x-in-y-behavior/\">ここでは、X-in-Y の動作について詳しく説明します。</a>"
    tags:
      - Purchase behavior
  - name: 最終発注日 (過去 730 日間)
    description: "ユーザーを最後に注文した時点でセグメント化します。これは、注文のための<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerceの推奨イベント</a>に基づいています(ワークスペースはeCommerceイベントにこのフィルターのデータがありません)。ユーザはこのフィルターに対して1 日に1 回評価され、最大ルックバックウィンドウは過去2 年間です。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 注文の総数 (過去 730 日間)
    description: "発注の<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerce推奨イベント</a>に基づいて、過去2年間のユーザーの注文の総数でユーザーを分割します(ワークスペース s not \"トラッキング eCommerceイベントにはこのフィルターのデータがありません)。この数は、キャンセル注文を除外します。注文キャンセルを行うには、<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerce推奨イベント</a>を使用して追跡する必要があります。ユーザは、このフィルターに対して1 日に1 回評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 注文数の合計
    description: "発注の<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eコマース推奨イベント</a>に基づいて、ユーザーの注文の総数で、その存続期間にわたってユーザーを分割します(ワークスペース s not \"トラッキング eコマースイベントにはこのフィルターのデータがありません)。この数は、キャンセル注文を除外します。注文キャンセルを行うには、<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerce推奨イベント</a>を使用して追跡する必要があります。ユーザは、このフィルターに対してリアルタイムで評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  ...
  - name: 顧客の生涯価値 (過去 730 日間)
    description: "ユーザーの購買履歴上に、ブランドで生成されると予想される総収益で、ユーザーをセグメント化します。計算では、直近の730 日間を考慮し、AOV (Average Order Value) を使用して、それに発注の総数を乗算し、ユーザーの有効な購買期間(最初の発注から直近の発注までの期間) を考慮します。このフィルターでは、<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerceの推奨イベント</a>sで追跡されたデータを使用します(ワークスペース s not \"トラッキング eCommerceイベントにはこのフィルターのデータがありません)。ユーザは、このフィルターに対して1 日に1 回評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 返金の総額 (過去 730 日間)
    description: "ユーザーを、<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eCommerceの推奨イベント</a>に基づいて、過去2年間にユーザーに付与された返金の金額で分割します(ワークスペースはeCommerceイベントにこのフィルターのデータがありません)。ユーザは、このフィルターに対して1 日に1 回評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 返金額の合計
    description: "ユーザーは、<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eコマース推奨イベント</a>に基づいて、返金された注文に対して、その存続期間中にユーザーに付与された返金の総額でセグメント化されます(ワークスペースは、eコマースイベントにこのフィルターのデータがありません)。ユーザは、このフィルターに対してリアルタイムで評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 総収益 (過去 730 日間)
    description: "過去2年間のユーザーの注文から生成された総収益によって、ユーザーをセグメント化します。これは、発注のためのeコマースイベントに関連付けられた収益から返金された注文の<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eコマース推奨イベント</a>に関連付けられた収益を差し引いて計算されます(ワークスペースには、このフィルターのデータがありません)。ユーザは、このフィルターに対して1 日に1 回評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  - name: 総収益
    description: "ユーザーの生涯にわたるユーザーの注文から生成される総収益で、注文のためにeコマースイベントに関連付けられた収益から返金された注文のための<a href=\"/docs/user_guide/data/activation/custom_data/recommended_events/ecommerce_events\">eコマース推奨イベント</a>に関連付けられた収益を差し引いて計算されます(ワークスペースにはこのフィルターのデータがありません)。ユーザは、このフィルターに対してリアルタイムで評価されます。<br><br>このフィルターはベータ版である。このフィルターを使用する場合は、Braze アカウントマネージャーにお問い合わせください。"
    tags:
      - eCommerce
  ...
  - name: 年齢
    ...
    tags:
      - Demographic attributes
  - name: 生年月日
    description: ユーザーをアプリ内で示した誕生日によってセグメント化します。<br> 2月29日に誕生日を迎えた方は、3月1日を含むSegmentsに含まれています。<br><br>12 月または 1 月の誕生日をターゲットにするには、ターゲットとする年の 12 か月の期間内のフィルターロジックのみを挿入してください。言い換えれば、前の暦年の12月を振り返るロジックや、翌年の1月までのフォワードロジックを挿入しないでください。たとえば、12 月の誕生日をターゲットにするには、「12 月 31 日」、「12 月 31 日の前」、または「11 月 30 日の後」でフィルタリングすることができます。
    tags:
      - Demographic attributes
  - name: 性別
  ...
  - name: アプリあり
    ...
    tags:
      - App
  - name: 最新のアプリバージョン名
    description: "ユーザーのアプリの最新の名前によってセグメント化します。<br><br>\"less than\"または\"less or equal to\"を使用する場合、主アプリ版が存在しない場合、このフィルターはユーザーがアプリ版より古いため`true`を返します。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。"
    tags:
      - App 
  - name: 最新のアプリバージョン番号
    description: "ユーザーのアプリの最新バージョン番号によってセグメント化します。<br><br>\"less\" または\"less than or equal to\" を使うとき、主アプリ版が存在しない場合、このフィルターは`true` を返します。これは、アプリ版よりもユーザーが古いためです。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。<br><br>現行のアプリ版の入力には、時間がかかる場合があります。アプリ版のユーザープロファイル 更新は、情報がSDKによってキャプチャされたときに、ユーザーがアプリをいつ開封するかに依存します。ユーザーがアプリを開封していない場合、最新版は更新 d になりません。これらのフィルターも遡及的にアプリしないだろう。\"greater than\"または\"equal\"を現在および将来のバージョンに使用すると良いですが、以前のバージョンフィルターを使用すると予期しない動作が発生する可能性があります。"
    tags:
      - App 
  - name: アンインストール
