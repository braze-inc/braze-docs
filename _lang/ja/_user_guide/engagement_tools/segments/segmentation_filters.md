...
glossaries:
  ...
  - name: 利用可能なメール
    description: "有効なEメールアドレスを持っているかどうか、Eメールを購読しているかどうか、またはオプトインしているかどうかでユーザーをセグメンテーションする。このフィルターは、ユーザーがメールの配信停止をしているか、Brazeがハードバウンスを受け取っているか、メールがスパムとしてマークされているかの3つの条件をチェックする。これらの基準のいずれかが満たされている場合、またはメールがユーザーに存在しない場合、ユーザーは含まれません。<br><br>trans アクション al メッセージを送信する場合、ユーザー s の\"Email Available\" は<code>false</code> となりますが、オーディエンスの計算には含まれませんが、メッセージを受信できます。ただしオーディエンスの計算には、配信登録済みユーザーまたはオプトインユーザーのみが含まれます。<br><br>オプトインのステータスが重要なメールには、<a href=\"/docs/user_guide/engagement_tools/segments/segmentation_filters#email-address\">メールアドレス</a>フィルターではなく、「利用可能なメール」フィルターを使用することをお勧めします。追加の条件を使用することで、実際にメッセージを希望しているユーザーをターゲットにできます。"
    tags:
      - Channel subscription behavior
  - name: メールオプトイン日
  ...
  - name: アプリあり
    ...
    tags:
      - App
  - name: 最新のアプリバージョン名
    description: "ユーザーのアプリの最新の名前によってセグメント化します。<br><br>\"less than\"または\"less or equal to\"を使用する場合、主なアプリ版が存在しない場合、このフィルターは`true`を返します。これは、アプリ版よりもユーザーが古いためです。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。"
    tags:
      - App 
  - name: 最新のアプリバージョン番号
    description: "ユーザーのアプリの最新バージョン番号によってセグメント化します。<br><br>\"less\" か\"less or equal to\" を使うとき、主アプリ版が存在しない場合、このフィルターは`true` を返す。なぜなら、このユーザーはアプリ版より古いからである。つまり、ユーザーの最後のメインアプリが存在しない場合、自動的にフィルターにマッチします。<br><br>現行のアプリ版の入力には、時間がかかる場合があります。アプリ版のユーザープロファイル 更新は、情報がSDKによってキャプチャされたときに、ユーザーがアプリをいつ開封するかに依存します。ユーザーがアプリを開封していない場合、最新版は更新 d になりません。これらのフィルターも遡及的にアプリしないだろう。\"greater than\"または\"equal\"を現在および将来のバージョンに使用すると良いですが、以前のバージョンフィルターを使用すると予期しない動作が発生する可能性があります。"
    tags:
      - App 
  - name: アンインストール
