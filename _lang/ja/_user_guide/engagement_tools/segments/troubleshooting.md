...
このまれなエラーは、ターゲットオーディエンスに正規表現値が多すぎる、正規表現値が長すぎる、詳細すぎるフィルター (" は30,000 zip コード s" のいずれか)、またはフィルター が多すぎる場合に発生します。これには、フィルター が参照Segments 内にあるか、**Target オーディエンス** ステップでフィルター として追加されたかにかかわらず、キャンペーンまたはキャンバスオーディエンス内のすべてのフィルター が含まれます。

![複雑性しきい値に達した対象オーディエンスの異常。]({% image_buster /assets/img/segment/target_audience_too_complex_error.png %})

Segment フィルターをキャンペーンまたはキャンバスに追加すると、これらのフィルターはBrazeでクエリーに変換されます(これらのクエリーの文字数は、ダッシュボード ユーザーが表示する文字数の1:1ではありません)。Braze がキャンペーンまたはキャンバスを送信すると、ターゲットオーディエンス内のすべてのフィルターを結合するクエリーが実行されます。対象オーディエンスに対する結果の問合わせの文字数を制限するしきい値をアプリします。特定のキャンペーンまたはキャンバスについて、すべての追加フィルター を含む、参照されるすべてのSegmentの文字数を合計します。与えられたSegmentに対して、すべてのフィルターとフィルターの文字数を合計します。
...
![4 つのアクティブまたは停止したキャンバスがオーディエンスの複雑性しきい値を超えていることを示すエラーバナー。]({% image_buster /assets/img/segment/audience_complexity_threshold_banner.png %})

### フィルタが10,000バイトを超えているか、保存するには長すぎます

Brazeは、個々のSegment フィルター を最大10,000 バイトに制限します。これは、10,000 の英字または3333 の日本語文字に相当します。フィルターがSegment内にあるか、キャンペーンまたはキャンバスに直接追加されているかにかかわらず、個々のフィルターが10,000 バイトを超えると、警告アプリが表示されます。 

![10,000 文字を超える値を持つフィルターのエラーバナー。]({% image_buster /assets/img/segment/filter_error.png %})

![10,000 文字を超える属性を持つカスタム属性 フィルター`menu_item`, のエラーメッセージです。]({% image_buster /assets/img/segment/segment_filter_error.png %})


このエラーはめったに起こりませんが、発生した場合は、通常、ユーザー IDs またはメールの住所の一覧を対象とする正規表現フィルター を使用します。この場合、次のステップs に従ってフィルター をCSV に変換できます。

1. 影響を受けるSegmentまたは固有の正規表現フィルターからユーザー をエクスポートします。 
...
3. [CSVファイルを再度インポートします。これにより、ユーザー が自動的に単一の高効率なCSV ベースのフィルターにまとめられます。
...
Segmentを作成するときに、[使用された固有のアプリs]({{site.baseurl}}/user_guide/engagement_tools/segments/creating_a_segment/#step-3-choose-your-app-or-platform) を持つユーザー を対象にすることができます。ユーザーには、特定のセッションをアプリに割り当てるアプリが必要ですが、ログに記録されたセッションをアプリに残さずに、特定のアプリに割り当てることができる2 つのシナリオがあります。 
