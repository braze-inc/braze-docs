...
![名前の横に編集アイコンがあるグループをフィルタリングします。]({% image_buster /assets/img_archive/edit_filter_group_name.png %})
...
**ファネル統計の表示**を選択して、そのフィルターグループの統計を表示し、追加されたフィルターがセグメント統計にどのように影響するかを確認します。その時点までのすべてのフィルターによってターゲットされているユーザーの推定数とパーセンテージが表示されます。フィルターグループの統計が表示されると、フィルターを変更するたびに自動的に更新されます。これらの統計は推定され、生成に少し時間がかかる場合があります。

フィルター の間にAND を使用すると、ファンネル統計が減少し、フィルター の間にOR を使用するとファンネル統計が増加することに注意してください。

![Segmentファンネル統計を持つ2つのフィルター。]({% image_buster /assets/img_archive/segment_funnel_statistics.png %})
