...
ネストされたイベントプロパティの使用は、アクションベースの配信][19]にはセグメントエクステンションを必要としません。イベント処理はリアルタイムで行われます。階層化カスタム属性も同様に、セグメントエクステンションを使用する必要はありません。
...
![Segment 拡張機能「オンラインショッパー拡張 - 90日」が「拡張機能を毎日再生成する」チェックボックスが選択された状態で。][2]
