---
nav_title: セグメントの長さを測る
article_title: セグメントの長さを測る
page_order: 5
page_type: reference
tool: 
- Segments
...
![メール、iOSプッシュ、Androidプッシュ、Web プッシュ、Kindleプッシュごとに到達可能なユーザー数を合計した表だ。]({% image_buster /assets/img_archive/segmenter_reachable_users.png %})
...
![セッション数がちょうど1回だったユーザー向けのフィルターを持つフィルターグループだ。]({% image_buster /assets/img_archive/segment_filter_stats.png %})
...
![「到達可能なユーザー」サイドパネルには正確なユーザー数が「31」と表示されている。]({% image_buster /assets/img_archive/reachable_users_panel.png %})
...
#### チャネル固有の到達可能なユーザーに適用されたフィルター

到達可能なユーザーを決定する際、各チャネルに対して以下のフィルターが適用される。

| チャネル | フィルター |
| --- | --- |
| メール | **メールが利用可能**である。 |
| プッシュ | **フォアグラウンドプッシュのイネーブルメントが**有効である。 |
| SMS | **サブスクリプショングループとは、**あらゆるSMSサブスクリプショングループを指す。**無効な電話番号は**偽である。 |
...
{% alert note %}
[ランダムなバケット番号]({{site.baseurl}}/user_guide/engagement_tools/testing/random_bucket_numbers/)で均等に分割されたセグメントは、同じサイズにはならない。例えば、フィルター**「ランダムバケット番号が5000未満」**で1つのセグメントを作成し、フィルター**「ランダムバケット番号が5000以上」**で別のセグメントを作成した場合、セグメントサイズが数パーセント程度変動することはあり得るし、想定されることだ。これは、非アクティブユーザーが削除されたり、ユーザーに連絡が取れなくなったりする状況によるものだ。
{% endalert %}

![「リーチ可能なユーザー」パネルには内訳を表示するオプションがある。]({% image_buster /assets/img_archive/reachable_users_breakdown.png %})
...
![計算が一つある計算キュー。]({% image_buster /assets/img_archive/calculation_queue.png %})

**Cancel**を選択すると、正確な統計計算をキャンセルできます。これは、キューに複数の計算があり、最初に別の計算に優先順位を付けたい場合に役立ちます。 

![キャンセル可能なアクティブな計算]({% image_buster /assets/img_archive/cancel_calculation.png %}){: style="max-width:35%"}
...
例えば、ワークスペースに1億人のユーザーがいて、セグメントには約700人のユーザーがいるとしよう。ある日には、セグメントにユーザーが一人もおらず、過去のメンバーシップ推定に使われるランダムなバケット範囲にユーザーが割り当てられない場合がある。その結果、その日のメンバーシップカウントは0になる。
