...
![メール、iOSプッシュ、Androidプッシュ、Webプッシュ、Kindleプッシュで到達可能なユーザー別に、到達可能なユーザーの合計を表示した表。]({% image_buster /assets/img_archive/segmenter_reachable_users.png %})
...
![セッション数がちょうど1回だったユーザーのフィルターを持つフィルターグループ。]({% image_buster /assets/img_archive/segment_filter_stats.png %}){: style="max-width:80%;"}
...
![「到達可能なユーザー」パネルには、2.3M-2.4Mの推定ユーザーがいると書かれている。]({% image_buster /assets/img_archive/reachable_users_side_panel.png %})
...
![到達可能なユーザー」パネルに内訳を表示するオプションが追加された。]({% image_buster /assets/img_archive/reachable_users_breakdown.png %})
...
![計算キューに1つの計算がある。]({% image_buster /assets/img_archive/calculation_queue.png %})

**Cancel**を選択すると、正確な統計計算をキャンセルできます。これは、キューに複数の計算があり、最初に別の計算に優先順位を付けたい場合に役立ちます。 

![キャンセル可能なアクティブ計算]({% image_buster /assets/img_archive/cancel_calculation.png %}){: style="max-width:25%"}
...
![セグメンテーションを日付の範囲でフィルターする。]({% image_buster /assets/img_archive/historical_membership2.png %})
