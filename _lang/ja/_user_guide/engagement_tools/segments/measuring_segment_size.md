...
![セッション数がちょうど1つであったユーザーのフィルターをもつフィルター群。]({% image_buster /assets/img_archive/segment_filter_stats.png %}){: style="max-width:80%;"}
...
![" Reachable ユーザー s" 2.3M-2.4M の推定ユーザー があることを示すパネル。]({% image_buster /assets/img_archive/reachable_users_side_panel.png %})
...
#### チャネル固有の到達可能なユーザーのための適用フィルター

次のフィルター は、到達可能なユーザー を決定するときに、チャネルごとにアプリ存在します。

| チャネル | フィルター |
| --- | --- |
| メール | **Email Available** is true. |
| プッシュ | **Foreground Push Enabled** がtrue です。 |
| SMS | **サブスクリプショングループ**は任意のSMS サブスクリプショングループです。**無効な電話番号** はfalse です。 |
...
たとえば、ワークスペースに1億 ユーザー s が含まれ、Segmentに約700 ユーザー s があるとします。数日間、ユーザーがSegmentに含まれておらず、過去のメンバーシップの推定に使用されたランダムバケット範囲にユーザーが含まれていないため、1日のメンバーシップ数が0になる可能性があります。
