...
グローバルコントロールグループでは、すべてのユーザーの割合をコントロールグループとして設定できます。保存すると、グループ内のユーザー はキャンペーンs またはキャンバスを受信しません。 
...
グローバルコントロールグループが生成されると、ランダムバケット番号を持つすべてのユーザーがグループの一部になります。さらに、この時点以降に参加する新しいユーザー(グローバルコントロールグループの生成後に取得されたもの)で、これらの乱数を持つものもグローバルコントロールグループに追加されます。同様に、多くのユーザーが削除された場合、削除されたユーザーのパーセンテージe がこのグループに属しているため、グローバルコントロールグループのサイズが縮小することが予想されます。これにより、グループの大きさは、ユーザー群全体に対する一定のパーセンテージe のタグとして維持されます。

### ユーザーを無作為に治療群に割り当ててレポートする
...
グローバルコントロールグループから特定のユーザー を削除することはできませんが、[除外設定s]({{site.baseurl}}/user_guide/engagement_tools/testing/global_control_group/#step-3-assign-exclusion-settings) を追加して、指定されたタグs **don't** のキャンペーンとキャンバスがグローバルコントロールグループを使用するようにすることができます。また、グローバルコントロールグループを無効にして再有効化し、メンバーシップをシャッフルすることもできます。シャッフルユーザーのアイデア時間は、実行中のテストの種類によって異なりますが、月に1 回までシャッフルすることをお勧めします。
...
**Global Control Group Settings**タブからいつでもGlobal Control Groupを無効にすることができますが、これを行うと、このグループのユーザーがすぐにキャンペーンおよびキャンバスに適格になることに留意してください。
...
コントロールグループを無効にした後、新しいコントロールグループを保存できます。割合を入力して保存すると、Braze はランダムに選択された新しいユーザーグループを生成します。以前と同じパーセンテージeを入力すると、Brazeはコントロール群と治療群に対して新しいユーザー群を生成します。
...
グローバルコントロールグループはランダムバケット番号を使用して形成されます。したがって、ランダムバケット番号Segment フィルターを使用して他のテストを実行している場合は、作成するSegmentとグローバルコントロールグループユーザーとの間に重複が存在する可能性があることに注意してください。

#### メールアドレスが重複している

異なるユーザーIDを持つ2つの外部ユーザーが同じメールアドレスを持ち、これらのユーザーの1つがコントロールグループにあり、もう1つがそうでない場合、メールは、非コントロールグループ ユーザーがメールに適格であるときに、そのメールアドレスに送信されます。これが発生すると、両方のユーザープロファイルs がそのメールを含むキャンペーンまたはキャンバスを受信したことを示します。
