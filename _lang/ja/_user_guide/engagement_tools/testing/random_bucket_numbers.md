...
> ランダムバケット番号は、ランダムユーザーの均一に分散したセグメントを作成するために使用できるユーザー属性です。 

## 概要

Braze でユーザープロファイルを作成すると、そのユーザーには0から9999までのランダムなバケット番号が自動的に割り当てられます。これらのセグメンテーションを使用して、複数のキャンペーンやキャンバスの効果をユーザーグループに対して長期的にテストすることができる。

### グローバルコントロールグループの使用

ランダムバケット番号は、グローバルコントロールグループ（キャンペーンやキャンバスを受け取らないユーザーグループ）で使用される。Braze はランダムなバケット番号の複数の範囲をランダムに選択し、選択されたバケットのユーザーを含めます。ランダムなバケット番号は、重み付けや最近割り当てられた番号を考慮せずに割り当てられる。 

{% alert note %}
ユーザーが削除され再作成されると、そのユーザーは新規ユーザーと見なされるため、異なるランダムなバケット番号が割り当てられる。
{% endalert %}
...
![ランダムなバケット番号が「3000」以下であるセグメントフィルターだ。]({% image_buster /assets/img_archive/random_buckets_filterexample.png %})
...
{% alert tip %}
キャンバスでランダムなバケット番号セグメントを使用する場合、例えば[条件分岐]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/decision_split/)ステップのフィルターとして使用する場合、キャンバスの[退出条件]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/exit_criteria/)、オーディエンスフィルター、および上流ステップが、バケット範囲のいずれかと重複するセグメントを対象としないように注意せよ。もしそうなれば、その範囲のユーザーは分割点に到達する前に不釣り合いに削除される可能性があり、パス間の分布が不均一になる。
{% endalert %}
