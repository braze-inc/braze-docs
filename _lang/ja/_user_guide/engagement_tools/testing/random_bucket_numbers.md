...
ランダムバケット番号は、グローバルコントロールグループ（キャンペーンやキャンバスを受け取らないユーザーグループ）で使用される。Braze はランダムなバケット番号の複数の範囲をランダムに選択し、選択されたバケットのユーザーを含めます。ランダムなバケット番号は、重み付けや最近割り当てられた番号の考慮なしで割り当てられます。 

{% alert note %}
ユーザーが削除され、再作成されると、ユーザーには新しいユーザーとみなされるため、別の乱数バケット番号が割り当てられます。
{% endalert %}
...
!["3000"以下のランダムバケット番号のSegment フィルター。]({% image_buster /assets/img_archive/random_buckets_filterexample.png %})
