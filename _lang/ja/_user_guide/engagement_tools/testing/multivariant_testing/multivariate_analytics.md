...
![多重バリアントsを持つメール キャンペーンのキャンペーン分析のパフォーマンスセクション。受信者 s、バウンス、クリック、コンバージョン s など、バリアントごとのさまざまなパフォーマンスメトリクスが一覧表示されます。]({% image_buster /assets/img_archive/ab_analytics_no_optimization.png %})
...
![初回検査の結果。統計的有意性の95%信頼区間を満たすだけの十分な信頼度で、他の検査よりも優れたバリアントが得られなかった場合。]({% image_buster /assets/img_archive/ab_analytics_wv_insufficient_confidence.png %})
...
![Winning Variantグループに送られたWinning Variantの結果。]({% image_buster /assets/img_archive/ab_analytics_wv_1.png %})
...
![ユーザーごとに最良のパフォーマンスバリアントを決定するために送信された最初のテストの結果。テーブルには、対象チャネルのさまざまなメトリクスに基づく各バリアントのパフォーマンスが表示されます。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_1.png %})
...
![ユーザーの特徴テーブル。これは、どのユーザーs が、最新性、頻度、および保有期間のために、どのバケットに該当するかに基づいて、バリアントA およびバリアントB を優先すると予測されているかを示します。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_2.png %})
...
![一意の開封に最適化されたキャンペーン用のパーソナライズされたバリアントタブ。カードには、プロジェクトされたリフト、全般的な一意のオープン(パーソナライズされたバリアントあり)、およびプロジェクトされた一意のオープン(勝者バリアントあり)が表示されます。]({% image_buster /assets/img_archive/ab_analytics_pv_1.png %})
