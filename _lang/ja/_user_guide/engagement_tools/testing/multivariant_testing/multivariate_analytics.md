...
![複数のバリアントがあるメールキャンペーンのキャンペーンアナリティクスのパフォーマンスセクション。受信者 s、バウンス、クリック、コンバージョン s など、バリアントごとのさまざまなパフォーマンスメトリクスが一覧表示されます。]({% image_buster /assets/img_archive/ab_analytics_no_optimization.png %})
...
![統計的有意性の95%信頼区間を満たすだけの十分な信頼度で他のバリアントよりも優れた成績が得られなかった場合に、当選バリアントを判定するために最初に送られた検定の成績。]({% image_buster /assets/img_archive/ab_analytics_wv_insufficient_confidence.png %})

#### 勝者バリアントの選択方法

Braze は、[ピアソンのカイ二乗検定](https://en.wikipedia.org/wiki/Pearson%27s_chi-squared_test)を使用してすべてのバリアントを相互にテストします。これは、1つのバリアントが有意水準p<0.05で他のすべてのバリアントよりも統計的に優れているかどうか、または95%有意と呼ばれるものを測定します。その場合、勝者バリアントには「勝者」ラベルが表示されます。
...
![Winning Variantグループに送信されたWinning Variantの結果。]({% image_buster /assets/img_archive/ab_analytics_wv_1.png %})
...
![各ユーザーの最高パフォーマンスのバリアントを判別するために送られた最初のテストの結果。テーブルには、対象チャネルのさまざまなメトリクスに基づく各バリアントのパフォーマンスが表示されます。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_1.png %})
...
たとえば、ほとんどのユーザーはバリアントAを好むが、3～12日前にセッションを持ち、セッションsの間に1～12日間を持ち、過去67～577日に作成されたユーザーsはバリアントBを好む傾向があることが、この検定でわかるかもしれません。そのため、そのサブ集団のユーザーsは2回目の送信でバリアントBを受信し、残りはバリアントAを受信しました。

![ユーザー特徴テーブルは、どのユーザーs が、最新性、頻度、および保有期間のために、どのバケットに属するかに基づいて、バリアントA およびバリアントB を優先すると予測されるかを示します。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_2.png %})
...
![ユニーク開封数のために最適化されたキャンペーンの「パーソナライズされたバリアント」タブ。カードには、プロジェクトされたリフト、全般的な一意のオープン(パーソナライズされたバリアントあり)、およびプロジェクトされた一意のオープン(勝者バリアントあり)が表示されます。]({% image_buster /assets/img_archive/ab_analytics_pv_1.png %})
...
アプリ内メッセージ キャンペーンs では、ユーザーs の追跡方法とインプレッションs の記録方法によって、コントロールグループ とバリアント の間の予想される分割に不一致が生じる可能性があります。これは、ログに記録される実際のインプレッションがこの分割を反映していない可能性があり、最終的にBrazeはトリガーを実行するユーザの個々のユーザーの振る舞いをコントロールできないためです。

たとえば、キャンペーンの起動時の目標オーディエンスが200 ユーザー s で、コントロールグループが100 ユーザー s、バリアントが100 ユーザー s であるとします。

バリアント内の100 ユーザーは、アプリ内メッセージの支払い読み込むを受け取り、そのうちの50 人がトリガー アクションを実行し、アプリ内メッセージを確認します。コントロールグループ内の100 ユーザーは、キャンペーンのトリガー アクションを実行した場合にのみ追跡され、そのうちの75 個はトリガー アクションを実行してインプレッションを記録しますが、アプリ内メッセージは表示されません。

最初の50/50分割にもかかわらず、ログに記録される一意のインプレッションはバランスが取れません。バリアント基は50 インプレッションsを有し、一方、コントロールグループは75 インプレッションsを有する。

### アプリ中の伝文遅延 

遅延表示を含むトリガー ed アプリ内メッセージ キャンペーン s の場合、エンドユーザーが最初にアプリ内メッセージを受信したときにコントロールグループ インプレッションs が記録されます。たとえば、キャンペーンがディスプレイを1 時間遅らせるように設定されている場合、コントロールグループ インプレッションs は1 時間遅れが経過するまでログに記録されません。これは、メッセージ配信の意図されたタイミングに関連するインプレッションのACキュレート "トラッキングに役立ちます。
