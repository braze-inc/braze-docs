...
たとえば、ほとんどのユーザーはバリアントAを好むが、3～12日前にセッションを持ち、セッションsの間に1～12日間を持ち、過去67～577日に作成されたユーザーはバリアントBを好む傾向があることが、この検定でわかるかもしれません。そのため、そのサブ集団のユーザーは2回目の送信でバリアントBを受信し、残りはバリアントAを受信しました。

![ユーザー特徴テーブルは、どのユーザー が、最新性、頻度、および保有期間のために、どのバケットに属するかに基づいて、バリアントA およびバリアントB を優先すると予測されるかを示します。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_2.png %})
...
アプリ内メッセージ キャンペーンs では、ユーザー の追跡方法とインプレッションs の記録方法によって、コントロールグループ とバリアント の間の予想される分割に不一致が生じる可能性があります。これは、ログに記録される実際のインプレッションがこの分割を反映していない可能性があり、最終的にBrazeはトリガーを実行するユーザの個々のユーザーの振る舞いをコントロールできないためです。
