...
![複数のバリアントがあるメールキャンペーンのキャンペーンアナリティクスのパフォーマンスセクション。この表は各バリアントの受信者数、バウンス数、クリック数、コンバージョン数といった様々なパフォーマンス指標を一覧表示している。]({% image_buster /assets/img_archive/ab_analytics_no_optimization.png %})
...
![初期テストの結果を送信した。勝者バリアントを決定するためだ。どのバリアントも、統計的有意性の95％信頼水準を満たす十分な確信をもって他より優れているとは認められなかった。]({% image_buster /assets/img_archive/ab_analytics_wv_insufficient_confidence.png %})

#### 勝者バリアントの選択方法

Braze は、[ピアソンのカイ二乗検定](https://en.wikipedia.org/wiki/Pearson%27s_chi-squared_test)を使用してすべてのバリアントを相互にテストします。これは、あるバリアントが統計的に他の全てを上回るかどうかを、有意水準<p=0.05、つまり我々が95%有意水準と呼ぶ水準で測定するものである。その場合、勝者バリアントには「勝者」ラベルが表示されます。
...
![勝者バリアントの結果は勝者バリアントグループに送られた。]({% image_buster /assets/img_archive/ab_analytics_wv_1.png %})
...
![各ユーザーの最高パフォーマンスのバリアントを判別するために送られた最初のテストの結果。表は、対象チャネルにおける各種指標に基づく各バリアントのパフォーマンスを示している。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_1.png %})
...
テストがカスタムイベントとパス設定の間に有意な関係を見つけられない場合、テストはセッションベースの分析方法に切り替わる。この場合、カスタムイベントデータテーブルは表示されない。
...
例えば、テストの結果、大半のユーザーはバリアントAを好むが、セッションが約3～12日前にあり、セッション間隔が1～12日で、かつアカウント作成から67～577日経過したユーザーはバリアントBを好む傾向がある。したがって、このサブグループに属するユーザーには2回目の配信でバリアントBを、それ以外のユーザーにはバリアントAを配信した。

![ユーザー特性表は、ユーザーが直近利用頻度・利用頻度・利用期間の3つの区分に分類され、それぞれについてバリアントAとバリアントBのどちらを好むと予測されるかを示すものである。]({% image_buster /assets/img_archive/ab_analytics_pv_initial_test_2.png %})
...
![ユニーク開封数のために最適化されたキャンペーンの「パーソナライズされたバリアント」タブ。カードには、予測リフト、全体ユニーク開封（パーソナライズされたバリアント付き）、および予測ユニーク開封（勝者バリアント付き）が表示される。]({% image_buster /assets/img_archive/ab_analytics_pv_1.png %})
...
## コントロールグループとバリアント群の相違点

アプリ内メッセージキャンペーンでは、ユーザーのトラッキング方法やインプレッションの記録方法が、コントロールグループとバリアント群の予想される割合に差異を生じさせる可能性がある。実際のインプレッション記録はこの分割を反映していない可能性があり、Brazeは最終的に、どのユーザーがトリガーを実行するかという個々のユーザー行動をコントロールできないからだ。

例えば、あるキャンペーンの開始時のオーディエンスが200人だとしよう。そのうち100人はコントロールグループに、残り100人はバリアントに割り当てられる。

このバリアントの100人のユーザーはアプリ内メッセージのペイロードを受け取り、そのうち50人がトリガーアクションを実行してアプリ内メッセージを確認する。コントロールグループの100人のユーザーは、キャンペーンのトリガーアクションを実行した場合にのみトラッキングされる。そのうち75人がトリガーアクションを実行し、インプレッションを記録したが、アプリ内メッセージは表示されなかった。

当初は50/50の分割だったにもかかわらず、記録されたユニークインプレッションは均等ではない。バリアントグループは50インプレッションであるのに対し、コントロールグループは75インプレッションである。

### アプリ内メッセージの遅延 

遅延表示を含むトリガー型アプリ内メッセージキャンペーンでは、コントロールグループのインプレッションは、エンドユーザーが当初アプリ内メッセージを受信する予定だった時点で記録される。例えば、キャンペーンが表示を1時間遅らせる設定の場合、コントロールグループのインプレッションは1時間の遅延が経過するまで記録されない。これにより、メッセージ配信の意図したタイミングに関連するインプレッションの正確なトラッキングが可能になる。
