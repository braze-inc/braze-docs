...
しかし、場合によっては、二番目の出来事がトリガーとなって最初に引き起こされることがある。これは、まだ存在しないユーザーにメッセージを送信しようとしていることを意味します。その結果、ユーザーがそれを受け取ることありません。イベントや属性が、まだ作成されていないユーザープロファイルに記録されようとしている場合も同様に当てはまります。
...
{% alert important %}
我々は非同期処理を用いて、速度と柔軟性を最大限に高めている。これは、API呼び出しが個別に送信された場合、送信された順序通りに処理されることを保証できないことを意味する。
{% endalert %}
...
ユーザー情報が [`/users/track` エンドポイント]({{site.baseurl}}/api/endpoints/user_data/post_user_track)を使用して Braze に送信される場合、処理に数秒かかることがあります。これは、と`/users/track`メッセージングエンドポイント（例`/campaign/trigger/send`：）に同時にリクエストが送信された場合、メッセージ送信前にユーザー情報が更新される保証がないことを意味する。

{% alert note %}
ユーザー属性とイベントが同じリクエストで送信された場合（Brazeから`/users/track`でもSDKからでも）、Brazeはメッセージ送信を試みる前に、イベントよりも先に属性を処理する。
{% endalert %}
...
これらのオブジェクトがトリガーに含まれる場合、メッセージがトリガーされる前に属性が処理される。これにより潜在的な競合状態が排除される。トリガープロパティはユーザープロファイルを更新するのではなく、メッセージのコンテキストの中だけで使われることに注意してください。
...
このベストプラクティスは、トリガーイベントで冗長なフィルターを回避することと似ています。通常、トリガーイベントがユーザープロファイルに更新されると仮定するフィルターは失敗する。
...
#### ユーザーデータがどのように管理されているかを確認する

Canvasエントリの評価中に競合が発生した場合、ユーザーは本来入るべきでないキャンバスに入ってしまう可能性がある。例えば、ユーザープロファイルをオーディエンスに含めるように設定し、その後、キャンバスがユーザーをキューに追加した後に、オーディエンスの対象外となるよう更新することもできる。 

ユーザーが同じ秒内に複数回Canvasエントリーイベントをトリガーした場合、Brazeはその秒に対して1回のエントリのみを許可する（再エントリーがイネーブルメントされている場合でも同様である）。これにより重複したエントリが防止されるため、キャンバスのエントリ総数はトリガーイベントの総数より少なくなる可能性がある。

ユーザーデータの管理と更新方法を確認することを推奨する。特に、SDK、API、バッチAPI、その他の方法など、特定の属性がいつ、どのように更新されるかについて確認すべきである。これにより、ユーザーがキャンペーンやキャンバスに参加した理由と、ユーザープロファイルが更新されたタイミングを識別し、明確にすることができる。
