...
| Braze カタログ | 以下のフィールドを含むBrazeカタログを作成する必要がある。これらは製品ブロックの設定で使用するものである：{::nomarkdown}<code><ul><li>product_title</li><li>product_url</li><li>variant_image_url</li></ul></code>{:/} |
| カタログ選択 | 静的な商品ブロックの場合、商品ブロックに含める商品を指定[するためにカタログ]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/)選択を作成しなければならない。 |
...
| 静的 | Brazeカタログに保存されたデータを使って製品をパーソナライズする。どの製品を含めるかを指定するには、[カタログ選択]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/)を使用しなければならない。 | 新製品の発売やカテゴリ別のオファリングを紹介するのに最適です。| |
...
![ダイナミックな製品ブロックの製品フィールドは、カタログデータとイベントデータに分けられる。]({% image_buster /assets/img/product_blocks/dynamic_fields.png %}){: style="max-width:50%;"}
...
![静的な商品ブロック用の商品フィールドは、全てカタログデータとして分類される。]({% image_buster /assets/img/product_blocks/static_fields.png %}){: style="max-width:50%;"}
...
![商品配置のレイアウトオプション。商品の向き、整列、1行あたりの最大商品数、商品間隔を設定できる。]({% image_buster /assets/img/product_blocks/layout_options.png %}){: style="max-width:50%;"}
...
製品ブロック内のテキストとボタン要素には、段落とボタンの既存スタイルが自動的に適用される。これは、製品ブロックが段落やボタンに設定した書式を一貫して使用し、メール全体で統一された見た目を維持することを意味する。
...
![必要なフィールドに加えて他のフィールドも含まれたサンプルCSVファイルだ。]({% image_buster /assets/img/ecommerce/sample_product_catalog.png %})

#### カタログフィールドへのマッピング

カタログ**の設定**タブでは、**商品ブロックの**切り替えを選択して、カタログ内の特定のフィールドや情報にマッピングできる。これにより、商品タイトル、商品URL、画像URLとして使用するフィールドを選択できる。Shopifyのカタログフィールドはデフォルトでマッピングされており、変更できないことに注意せよ。

{% alert note %}
Shopifyを使用していない場合、アカウントマネージャーに連絡してフィールドマッピングを有効にできる。これにより、任意のカタログを商品ブロックに接続し、そのフィールドを`product_title`、`product_url`、およびにマッピング`variant_image_url`できる。
{% endalert %}
...
製品ブロックに使用するカタログを選択します。商品ブロックに表示する商品を指定するには、カタログの選択項目を選ばなければならない。

{% endtab %}
{% endtabs %}

![「コンテンツ」タブには、製品ブロックなどのエディターブロックが含まれている。]({% image_buster /assets/img/product_blocks/product_block.png %}){: style="max-width:40%;"}
...
![商品の価格の前にドル記号が付いた商品ブロック。]({% image_buster /assets/img/product_blocks/liquid.png %}){: style="max-width:45%;"}
...
1. **プレビューテスト**セクションでは**&**、カスタムユーザーとしてメッセージをプレビューする。
2. プレビューでレンダリングする項目の数を指定します。
3. 正しい項目数が表示され、レイアウトオプションが正しく適用されていることを確認します。表示される項目はランダムに選択されることに注意してください。

![「ユーザーとしてプレビュー」タブには、ドロップダウンセクション「ダイナミックな製品ブロック」があり、4つのアイテムを表示するように指定する。]({% image_buster /assets/img/product_blocks/preview_as_a_user.png %}){: style="max-width:40%;"}
...
![メールのドラッグ＆ドロップ式作成ツールで、生成された商品ブロックに異なるアイテムタイルが表示されている。]({% image_buster /assets/img/product_blocks/static_block_preview.png %})
