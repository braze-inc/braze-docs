...
- **ブレーズダッシュボードの外観:**メッセージがユーザのデバイスに表示される場合、その外観は、そのデバイスのオペレーティングシステムと言語設定によって大きく決まります。つまり、ダッシュボードに表示される内容は、常に100% 正確であるとは限りません。
...
さらに、句読点と絵文字をチェックして、メッセージが標準または右から左にレンダリングされているかどうかを確認します。

| 標準ウェスタンレンダリング | 右から左へのレンダリング |
|------------------|------------------------|
...
Braze で右から左にメッセージを作成するには:

1. 標準メッセージをブレーズエディタで作成します。
2. メッセージテキストをBraze からコピーし、ローカライズツールを使用して右から左に変換します。
3. 変換したメッセージをBraze にペーストして戻します。
4. テキストの書式設定と配置を確認します。ドラッグアンドドロップまたはHTML メールメッセージを作成する場合は、コンポーザー内でこれを行うことができます。それ以外の場合は、別のワードプロセッサを使用する必要があります。<br><br>![E メールドラッグアンドドロップエディタメニュー(ボタン付き)で、右から左、左から右へのテキストの配置を切り替えます。]({% image_buster /assets/img/rtl_button.png %}){: style="max-width:50%;"}
