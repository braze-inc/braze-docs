...
#### 確認事項
- [**API レート制限**](https://braze.com/resources/articles/whats-rate-limiting): ワークスペースでエラーが発生しないように、Braze API [レート制限]({{site.baseurl}}/api/api_limits/) を確認してください。レート制限の引き上げを検討している場合（かつ既にリクエストをバッチ処理している場合）、顧客サクセスマネージャーに連絡すること。このプロセスにはリードタイムが必要であることを念頭に置いて、計画を立ててください。
...
- **ベストプラクティス**: Braze[メール]({{site.baseurl}}/user_guide/message_building_by_channel/email/best_practices)の[ベストプラクティスを]({{site.baseurl}}/user_guide/message_building_by_channel/email/best_practices)確認し、配信率向上サービスについて詳しく学習したい場合は、Brazeアカウントチームに連絡する。
...
#### 知っておくべきこと
- **Liquid のパーソナライゼーション:**Liquidパーソナライゼーションは、リフレッシュリクエストのたびに更新される。
- **配置とバナーの比率:**各バナー配置は、ワークスペース内で最大25件のメッセージに使用できる。  
- **クリック数とインプレッション数:**バナーのクリック数とインプレッション数は SDK で自動的に追跡されます。
- **制限事項:** 現在サポートされていない機能はキャンバス統合、APIトリガー型およびアクションベースのキャンペーン、コネクテッドコンテンツ、プロモーションコード、ユーザー制御による非表示、および  [タグ`:rerender`]({{site.baseurl}}/user_guide/data/activation/catalogs/using_catalogs/#using-liquid)`catalog_items`の使用。
...
- **配置の要求:**1回の更新リクエストで最大10件の配置情報をSDKに返すことができる。各配置には、ユーザーに対して表示できる優先度が最も高いバナーが含まれます。
...
- **SDKセッションとカード作成**：SDKセッションを持たないユーザーに対しては、たとえそのユーザーがセグメント条件を満たしていても、コンテンツカードは作成されない。ただし、ユーザーが既にAndroidセッションを持っている場合、iOS専用のクリックアクションを持つコンテンツカードは依然として作成される。ユーザーがiOSでセッションを開始すれば、それらのコンテンツカードをiOS上で閲覧できる。カードが作成されるタイミングについての詳細は、[カード作成]({{site.baseurl}}/user_guide/message_building_by_channel/content_cards/create/card_creation/)を参照せよ。  
