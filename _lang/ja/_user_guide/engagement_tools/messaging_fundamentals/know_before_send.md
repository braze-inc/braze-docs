...
- **SDKセッションとカード作成**: SDKからのセッションがないユーザーの場合、セグメント基準を満たしていても、コンテンツカードは作成されません。ただし、ユーザーがすでに1つのプラットフォーム（例えば、Android）でセッションを持っている場合、プラットフォーム固有のクリックアクション（例えば、iOS固有のアクション）を持つコンテンツカードは引き続き作成され、ユーザーは別のプラットフォームでセッションを持つとそのコンテンツカードを表示できます。カードが作成されるタイミングの詳細については、[カード作成]({{site.baseurl}}/user_guide/message_building_by_channel/content_cards/create/card_creation/)を参照してください。  
