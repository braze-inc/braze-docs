...
![特定のアプリのユーザー をターゲットとするSegment。"Test_Android".]({% image_buster /assets/img_archive/app_test_android.png %}){: style="max-width:60%;"}
...
- オーディエンス Segmentにユーザーが含まれていることを確認してから、エントリ条件を実行します。
- 行動後にlyだけアプリするオーディエンス フィルターは使用しないでください。フィルターが、トリガー時にh アプリが終了するものに依存している場合("セッション count = 0"など)、そのユーザーは、タイムBrazeの検査によってはもはや適格ではない可能性があります。
- 時間ベースのロジックを慎重に使用します。たとえば、新しいユーザー を対象にする場合は、次のようにします。
