...
![Brazeダッシュボードの「全ステータス」ドロップダウン。]({% image_buster /assets/img/messaging_fundamentals/filter-by-status.png %}){: style="max-width:70%;"}

## ステータスの変更

キャンペーンまたはキャンバスのステータスを変更するには、<i class="fas fa-ellipsis-vertical"></i> メニューを選択し、ステータスを選択します。

![Brazeダッシュボード内のキャンバスのリスト。そのうち1つのキャンバスのメニューが開いている状態。]({% image_buster /assets/img/messaging_fundamentals/change-status.png %})
...
| 停止した | 停止したキャンペーンとキャンバスは一時停止されますが、編集することはできます。キャンバスを再開するには、キャンバスビルダーの**「概要**」ステップに移動し、**「キャンバスを再開」**を選択する。キャンペーンについては、メニュー<i class="fas fa-ellipsis-vertical"></i>を選択し、次に「**再開」**を選択する。詳細については、[停止したキャンバスの動作](#stopped-canvas-behavior)を参照してください。 |
...
- **下書き変更案：**キャンバスが停止されると、キャンバスへの下書きはすべて破棄される。

キャンバスを再開するには、キャンバスビルダーの**「概要**」ステップに移動し、**「キャンバスを再開」**を選択する。再び有効にすると、以前に停止されたすべてのメッセージは、スケジュールされた時刻が経過していない限り、スケジュールどおりに送信されます。
