...
ロケール情報は、[Braze SDK]({{site.baseurl}}/developer_guide/sdk_integration/) (自動的に)、または[REST API]({{ site.baseurl }}/api/endpoints/user_data/post_user_track) を使用して収集したデータに基づいて、ユーザーのプロファイルに保存されます。ロケールには、言語と地域識別子が含まれています。この情報は、Braze セグメンテーションツールの [**国**] と [**言語**] で確認できます。

{% alert tip %}
SDK s によるロケールの収集方法については、正式な[iOS](https://developer.apple.com/library/ios/documentation/MacOSX/Conceptual/BPInternational/LanguageandLocaleIDs/LanguageandLocaleIDs.html)、[Android](http://developer.android.com/reference/java/util/Locale.html)、および[Web](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/language) ドキュメントを参照してください。
{% endalert %}
...
ユーザーの言語、ロケール、またはカスタム属性sに基づいてパーソナライズされたを送信するには、次のいずれかの方法を使用します。

### 換算リキッドタグ(推奨) {#translation-liquid-tag}

Braze は、{% raw %}`{% translation salutation %}Hello!{% endtranslation %}`{% endraw %} 1 つのメッセージで、さまざまな言語のユーザーを対象とするリキッドタグをサポートします。 

フルウォークスルーについては、変換タグsの使用に関する[ガイドを参照してください。
...
一部のユーザーは、{% raw %}`{{${language}}}`{% endraw %}大きな条件付きブロックを構築するのではなく、単一の言語に対して自動的に結果を返すように、SheetDBの[検索メソッド](https://docs.sheetdb.io/#get-search-in-document)をGETリクエストコールで実装することにより、より少ないリキッドおよび接続されたブロック依存性でSheetDBを実装することを好むかもしれません。
