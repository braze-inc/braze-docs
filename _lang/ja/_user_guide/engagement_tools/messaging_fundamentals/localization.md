...
ロケール情報は、[Braze SDK]({{site.baseurl}}/developer_guide/sdk_integration/)（自動）または[REST API]({{ site.baseurl }}/api/endpoints/user_data/post_user_track)]を使用して収集したデータに基づき、ユーザープロファイルに保存される。ロケールには、言語と地域識別子が含まれています。この情報は、Braze セグメンテーションツールの [**国**] と [**言語**] で確認できます。

{% alert tip %}
ロケールが当社のSDKによって収集される方法に関する技術的な詳細は、公式の[iOS](https://developer.apple.com/library/ios/documentation/MacOSX/Conceptual/BPInternational/LanguageandLocaleIDs/LanguageandLocaleIDs.html)、[Android](http://developer.android.com/reference/java/util/Locale.html)、および[Web](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/language)のドキュメントを参照すること。
{% endalert %}
...
ユーザーの言語、地域設定、またはカスタム属性に基づいてパーソナライズされたメッセージを送信するには、以下のいずれかの方法を使用する。

### 翻訳 Liquidタグ（推奨） {#translation-liquid-tag}

BrazeはLiquid{% raw %}`{% translation salutation %}Hello!{% endtranslation %}`{% endraw %}タグをサポートしており、単一のメッセージで異なる言語のユーザーをターゲットにできる。 

完全な手順については、[翻訳タグの使用方法に関するガイド]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/localization/locales)を参照せよ。
...
一部のユーザーは、大規模な条件分岐ブロックを構築する代わりに、Liquid{% raw %}`{{${language}}}`{% endraw %}タグに基づいてJSONオブジェクトをフィルターするGETリクエスト呼び出しでSheetDBの[検索メソッド](https://docs.sheetdb.io/#get-search-in-document)を実装し、単一言語の結果を自動的に返すことで、LiquidやConnected Blockへの依存を少なくしたSheetDBの実装を好むかもしれない。
