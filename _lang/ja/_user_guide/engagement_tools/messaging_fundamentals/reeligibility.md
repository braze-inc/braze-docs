...
さらに、再配信間隔を0分に設定して即時送信を試みる場合、ユーザーが過去のキャンペーンやキャンバスをどのように受信したかにかかわらず、常に即時スケジュールを試みる。
...
例えば、API トリガーキャンペーンを使用して、ユーザーが最近閲覧したアイテムに関するキャンペーンをユーザーに送信するとします。この場合、閲覧したアイテムの数に関わらず、1日あたり最大1通のメッセージ送信にキャンペーンを制限できる。ただし、各アイテムに対してAPIトリガーは発動する。一方、API トリガーのキャンペーンがトランザクションベースの場合、遅延時間を 0 分に設定することで、ユーザーがトランザクションを実行するたびにキャンペーンを受け取るようにできます。
{% endtab %}

{% tab canvas %}

キャンバスの再適格性を有効にするには、[**エントリコントロール**] セクションで [**ユーザーがこのキャンバスに再エントリできるようにする**] をオンにします。キャンバスの最大時間経過後、または指定した時間枠後にユーザーが再入場できるかどうかを選択できる。

キャンバスのバリアントへの再有効化は、メッセージの受信ではなく、キャンバスのエントリに紐づけられます。キャンバスに入り、メッセージを一切受信しないユーザーは、再適格性が有効になっていない限り、キャンバスに再び入ることができません。

再参加資格がゼロ秒に設定されている場合、ユーザーはキャンバスを一度退出する必要なく再参加できる。つまり、同じキャンバスに再度入ることができる。別の例を挙げると、キャンバスの期間が7日間に設定され、再参加可能期間が3日間に設定されている場合、ユーザーは最初の旅程を完了する前にキャンバスに再参加できる。

ユーザーが同じステップやメッセージを複数回受け取らないように、追加のフィルターを設定できる。ただし、ユーザーがキャンバスに二度目に入る場合、最初にキャンバスに入った際に受け取ったステップはユーザーには表示されない。これは、ユーザーが同じメッセージを再び受け取る可能性があることを意味する。これを防ぐには、キャンバスを再参加不可に設定するか、キャンバスの最大有効期間を再参加可能期間として設定する。

[ユーザー更新ステップ]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/user_update/)を使用することもできる。このステップを受け取ったユーザーは、これをカスタム属性として記録する。この属性は、Canvasの旅程中にステップを受け取ったユーザーをフィルタリングするために使用できる。
...
キャンバスの再有効化がキャンバスの期間よりも短く設定されている場合、ユーザーがキャンバスに複数回エントリする可能性があります。特に遅延時間の長いアプリ内メッセージを使用するキャンバスでは誤解を招く動作につながる可能性があります。同一のセッション開始によって複数のCanvasアプリ内メッセージがトリガーされる可能性があるため、特定のコンポーネントが他のコンポーネントよりも高速にレンダリングされる場合、ユーザーは同じメッセージを繰り返し受信する体験をすることになりかねない。
{% endtab %}
{% endtabs %}

## 再有効化の遅延の計算

キャンペーンとキャンバスの両方の再適格性は、暦日ではなく秒単位で計算される。つまり、1 日は暦日の深夜にカウントされるのではなく、ユーザーがメッセージを受信した時点から 24 時間 (または 86,400 秒) としてカウントされます。同様に、1 カ月はちょうど 2,592,000 秒で、約 30 日に相当します。
...
* 2月15日から3月15日までの日数は30日未満だ。 

つまり、2 月 15 日にキャンペーンを受信したユーザーは、3 月 15 日に送信されるキャンペーンの対象になりません。キャンペーンが毎日午前8時に送信され、再適格期間が1日と設定されている場合、メッセージ送信に遅延が生じると、午前8時30分にキャンペーンを受信したユーザーは、翌日の午前8時にはまだ再適格になっていない。
