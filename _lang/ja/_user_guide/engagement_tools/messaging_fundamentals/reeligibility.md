...
キャンバスの再適格性を有効にするには、[**エントリコントロール**] セクションで [**ユーザーがこのキャンバスに再エントリできるようにする**] をオンにします。キャンバスの最長継続時間の後、または指定したウィンドウの後に、ユーザー の再入力を許可するかを選択できます。
...
フィルター を追加して、ユーザー が同じステップまたはメッセージを複数回受信しないようにすることができます。ただし、ユーザーが2回目にキャンバスに再び入ると、キャンバスで初めて受信したステップは、ユーザーには表示されません。これは、ユーザーが同じメッセージを再び受信できることを意味します。これを防ぐには、キャンバスの再エントリを防止するようにキャンバスを設定したり、キャンバスの最長継続時間に再適格性を設定したりできます。
