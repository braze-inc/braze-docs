...
メールアラート、Webhook アラート、またはその両方を設定できます。Webhook アラートは、Slack チャネルにアラートを送信できるため、非常に便利です。キャンペーンアラートをSlackと連携させる詳細については、Slackの「[インコミングWebhookを使用したメッセージ送信](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks/)」のドキュメントを参照せよ。
