---
nav_title: キャンペーンを開始した後に編集する
article_title: キャンペーン開始後に編集する
page_order: 1
tool: Campaigns
page_type: reference
...
キャンペーンでインテリジェントタイミングまたはローカルタイムゾーン配信を使用している場合、スケジュールされた送信時間の編集は、元の送信時刻から24時間以内に編集が行われた場合、反映されない。これは以下の理由による：

- **インテリジェントタイミング：**Brazeはサモア時間の深夜0時に最適な送信時間の計算を開始する。この時間が過ぎている場合、メッセージの処理は既に始まっている。詳細については、[インテリジェントタイミング]({{site.baseurl}}/user_guide/brazeai/intelligence_suite/intelligent_timing/)を参照のこと。
- **ローカルタイムゾーンでの配達：**24 時間以内にスケジュールされているローカルタイムゾーンのキャンペーンを編集しても、メッセージのスケジュールは変更されません。詳細については、「[ローカルタイムゾーンキャンペーンをスケジュールするにはどうすればよいか？]({{site.baseurl}}/user_guide/engagement_tools/campaigns/faq/#how-do-i-schedule-a-local-time-zone-campaign)」を参照のこと。
...
![アクティブなキャンペーンの下書きで、アクティブなキャンペーンを表示するオプションがある。]({% image_buster /assets/img/campaign_draft.png %})
