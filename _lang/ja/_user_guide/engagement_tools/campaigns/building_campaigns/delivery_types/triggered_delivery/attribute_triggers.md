...
!["Action-Based配信とクォート;トリガーを選択するドロップダウン付きのセクション。]({% image_buster /assets/img_archive/trigger_attribute.png %})
...
たとえば、目標ユーザーは、報酬点数が変更されたときに、現在の点数を知らせるようになります。この例題では、ユーザーに85 個の報酬点があり、報酬点属性が新しい値に変わったときにトリガーするキャンペーンを設定したとします。このユーザーの報酬点属性が新しい値(83、84、86など)に変更されると、キャンペーンがトリガーされます。

次に、階層更新通知のユースケースの例を見てみましょう。ユーザーの報酬層が変更された場合は、s に警告することをお勧めします。このユースケースを実行するには、`Change Custom Attribute Value` をトリガーするキャンペーンを設定し、カスタム属性 報酬の階層が新しい値に変更されたときにトリガー に設定します。

{% alert important %}
属性トリガーは現在、配列属性では使用できません。
{% endalert %}

![A "Change Custom Attribute Value" "AA_current_rewards_tier" のトリガーが任意の値に変更されます。]({% image_buster /assets/img_archive/any_value.png %})

また、リキッドを使用して、顧客の新しい報酬階層でメッセージ本文をパーソナライズし、変更に関する詳細を顧客に提供することもできます。
...
たとえば、ターゲットユーザーは、報酬のs 層が最良の層に変更されたときにs になります。この例では、最適な報酬s 層がスーパーVIP であるとします。キャンペーンを設定してトリガーすると、ユーザーの報酬階層カスタム属性が`Super VIP` に変更され、ユーザーがスーパーVIP になったことをお祝いすることができます。

![A "Change Custom Attribute Value" "AA_current_rewards_tier" のトリガーが"super vip" の固有値に変更されます。]({% image_buster /assets/img_archive/super_vip.png %})
