...
例えば、ユーザーの報酬ランクがベストランクに変更されたときをターゲットにする。この例では、最高の報酬ランクがスーパーVIPだとする。ユーザーの報酬レベルカスタム属性が`Super VIP` に変更されたときにトリガーするキャンペーンを設定することで、スーパーVIPになったユーザーを祝福することができる。
