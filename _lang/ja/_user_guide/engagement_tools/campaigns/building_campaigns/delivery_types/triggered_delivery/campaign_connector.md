...
{% alert important %}
Campaign Connectorトリガーは、アプリ内メッセージキャンペーンのトリガーには使用できない。アプリ内メッセージは、カスタムイベントやセッション開始などのSDKイベントによってのみトリガーされる。詳しくは、[アプリ内メッセージの作成を]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/create/)参照のこと。
{% endalert %}
