...
{% alert important %}
Campaign Connectorトリガーは、アプリ内メッセージキャンペーンをトリガーするために使用できません。アプリ内メッセージは、カスタムイベントやセッション開始などのSDKイベントによってのみトリガーできます。詳細については、[アプリ内メッセージを作成する]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/create/)を参照してください。
{% endalert %}
