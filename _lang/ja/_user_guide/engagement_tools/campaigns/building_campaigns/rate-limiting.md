...
!["Last Received Any Message" Segment フィルターがハイライトされたSegment詳細セクション。]({% image_buster /assets/img_archive/rate_limit_daily.png %})
...
![キャンペーンを受け取る人数を制限するには、選択したチェックボックスを含むオーディエンスサマリー。]({% image_buster /assets/img_archive/total_limit.png %})
...
![視聴者サマリー(選択したチェックボックス付き)キャンペーンが終了する速度を制限し、速度は1 分あたり500,000 になります。]({% image_buster /assets/img_archive/per_minute_rate_limit.png %})
...
![Frequency キャップ 項目。規則がアプリしないキャンペーンとキャンバスの一覧が表示されます。]({% image_buster /assets/img_archive/rate_limiting_overview_2.png %})
...
![Frequency Cアプリing をオンにした状態でのデリバリーコントロールセクション。]({% image_buster /assets/img_archive/frequencycappingupdate.png %})
...
![Frequency Cアプリing セクションでは、2 プッシュ通知 キャンペーン s/Canvas ステップをすべてのキャンペーン s/Canvas ステップ s から1 週間ごとに1 つのユーザーに送信するという規則があります。]({% image_buster /assets/img/standard_rules_fnfn.png %})
...
![周波数Cアプリ セクション。1 週間ごとにユーザーに送信できるプッシュ通知 キャンペーン数を制限する2 つの規則があります。]({% image_buster /assets/img/tag_rule_fnfn.png %} "rules")
...
![Frequency Cアプリing セクションで、1 週間ごとにユーザーに送信されるプッシュ通知 キャンペーン s/Canvas ステップの数を制限します。]({% image_buster /assets/img/global_rules.png %} "global rules")
