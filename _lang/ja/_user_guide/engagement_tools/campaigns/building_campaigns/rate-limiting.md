...
キャンペーン構成の**Target Users** ステップで、メッセージを受信するユーザーの総数を制限することもできます。これは、キャンペーンフィルタとは独立したチェックとして機能し、スパム超過を気にせずにユーザを自由にセグメント化できます。
...
たとえば、ユーザーがレベルをクリアするとトリガーされるアプリ内メッセージを含むゲームがあり、インプレッションの上限を 100 回に設定したとします。これまでに 99 件のインプレッションがありました。アリスとボブは両方ともゲームを開き、Braze はレベルをクリアするとメッセージを受け取る資格があることをデバイスに伝えます。アリスは最初にレベルをクリアし、メッセージを受け取ります。Bobは次にレベルを打ち負かすが、彼のデバイスはセッションが開始されてからBrazeサーバと通信していないので、彼のデバイスはメッセージがキャップを満たしていることを知らず、メッセージも受け取るだろう。ただし、インプレッションの上限に達すると、次回いずれかのデバイスが対象となるアプリ内メッセージのリストをリクエストしても、そのメッセージは送信されず、そのデバイスから削除されます。
...
キャンペーンの作成中にユーザをターゲットにする場合、**Target Audiences** (キャンペーンの場合) または**Send Settings** (キャンバスの場合) に移動して、レート制限(10 ～最大500,000 メッセージ/分までのさまざまな増分) を選択できます。 
...
![キャンペーンが終了するレートを制限し、レートを500,000/分にするために、選択したチェックボックスを含むオーディエンスサマリ。][3]

別の例として、レート制限が10,000 分のメッセージを75,000 件送信しようとすると、配信は8 分間で分散されます。キャンペーンでは、最初の7 分間のメッセージは10,000 件まで、最後の1 分間のメッセージは5000 件まで送信されます。 

レート制限されたメッセージは、1 分ごとに均等に送信されないことがあります。1分あたり10,000のレート制限の例を使用すると、Brazeは1分あたり10,000を超えるメッセージを送信しないことを意味します。これは、10,000 件のメッセージが送信される割合が、最後の半分よりも前半分の方が高いことを意味します。 

{% alert important %}
このようなレート制限を行うと、時間に敏感なメッセージを遅らせることになるので注意が必要だ。セグメントに 3000万 人のユーザーが含まれているが、レート制限を 10,000/分に設定すると、ユーザーベースの大部分は翌日までメッセージを受信しません。
{% endalert %}
