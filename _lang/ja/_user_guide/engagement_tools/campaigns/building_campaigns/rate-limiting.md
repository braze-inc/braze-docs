...
キャンバスステップで複数のチャンネルを使用する場合、レート制限は各チャンネルに適用されます。これにより、コネクテッドコンテンツのリクエストが予想よりも多くなる可能性があることに注意してください。回避策として、キャンバスレベルでレート制限を下げます。 

たとえば、マルチチャンネルのキャンバスステップでは、キャンバスでメールと SMS を使用し、1 分あたり 10,000 件のレート制限を設定した場合、Braze は 1 分あたり最大 20,000 件のメッセージ (メールが 10,000 件、SMS が 10,000 件) を送信します。 

メッセージステップ内のすべてのプッシュ通知の宛先は、レート制限の目的で単一のチャネルとして扱われます。マルチチャネルのキャンバスステップが、メール、iOS プッシュ通知、Android プッシュ通知をキャンバスで活用して 1 分あたり 10,000 件のレート制限を設定した場合、Braze は 1 分あたり最大 20,000 通のメッセージを送信します (1 万件のメールと合計で 10,000 件のプッシュ)。

### 配送速度のレート制限

大規模なキャンペーンによってユーザーアクティビティが急増し、サーバーに過負荷がかかることが予想される場合は、メッセージ送信の 1 分あたりのレート制限を指定できます。キャンペーンの作成時にユーザーをターゲットする場合、詳細オプションに移動すると、（1 分あたり最低 50 件のメッセージから最大 50 万件のメッセージまでさまざまな段階で) レート制限を選択できます。レート制限のないキャンペーンは、これらの配信制限を超える場合があることに注意してください。ただし、レート制限が低いためにメッセージが 72 時間以上遅延した場合、メッセージは中止されることに注意してください。キャンペーンを作成したユーザーは、レート制限が低すぎる場合、ダッシュボードとメールでアラートを受け取ります。

![][3]

たとえば、1 分あたり 10,000 のレート制限で 75,000 件のメッセージを送信しようとすると、配信は 8 分間にわたって分散されます。キャンペーンでは、最初の 7 分間でそれぞれ 10,000 件、直前の 7 分間で 5,000 件が配信されます。ただし、この形式のレート制限では、時間に敏感なメッセージの遅延には注意が必要です。セグメントに 3,000 万人のユーザーが含まれていても、レート制限を 1 分あたり 10,000 人に設定した場合、ユーザーベースの大部分は翌日までメッセージを受信しません。
...
[コネクテッドコンテンツの再試行] [19] 機能を有効にすると、Braze は再送信ごとに設定したレート制限を尊重しながら、失敗した呼び出しを再試行します。1 分あたり 10,000 件のレート制限がある 75,000 件のメッセージについてもう一度考えてみましょう。最初の 1 分間は、通話が失敗するか遅くなり、4,000 通のメッセージしか送信されません。
...
次の例では、次のフリークエンシーキャップルールを使用しています。
...
| アクション | 結果 |
|---|---|
| `promotional` タグは、ユーザーがメッセージを受信した後、**キャンペーン B** が送信する前に**キャンペーン A** から削除されます。| ユーザーは**キャンペーン B** を受信します。|
| ユーザーがメッセージを受信した後に、`promotional` タグが誤って**キャンペーン A** から削除されます。<br> タグは、**キャンペーン B **が送信される前の火曜日に**キャンペーン A** に追加されます。| ユーザーには**キャンペーン B** は送信されません。|
