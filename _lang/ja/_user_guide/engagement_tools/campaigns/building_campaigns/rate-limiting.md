...
レート制限は、Canvasを介して送信されるすべてのメッセージに適用されます。CanvasまたはCanvasステップで複数のチャンネルを使用する場合、レート制限はチャンネル間で共有されます。 

### 配送速度のレート制限

大規模なキャンペーンによってユーザーアクティビティが急増し、サーバーに過負荷がかかることが予想される場合は、メッセージ送信の1分あたりのレート制限を指定できます。つまり、Brazeは1分以内にレート制限設定を超えるメッセージを送信しません。キャンペーンの作成時にユーザーをターゲットにする場合、**詳細オプションに移動してレート制限を選択できます**（1分あたり最低50メッセージから最大500,000メッセージまでさまざまな段階で）。レート制限のないキャンペーンは、これらの配信制限を超える場合があることに注意してください。ただし、レート制限が低いためにメッセージが 72 時間以上遅延した場合、メッセージは中止されることに注意してください。キャンペーンを作成したユーザーは、レート制限が低すぎる場合、ダッシュボードとメールでアラートを受け取ります。

![][3]

たとえば、1 分あたり 10,000 件のレート制限で 75,000 件のメッセージを送信しようとすると、配信は 8 分間にわたって分散されます。キャンペーンで配信されるのは、最初の 7 分間でそれぞれ 10,000 件まで、直前の 1 分間で 5,000 件までです。レート制限を使用して送信されたメッセージは、レート制限設定 (1 分あたり 10,000 など) が 60 秒以上にわたって均等に送信されないことに注意してください。代わりに、Braze は 1 分あたり 10,000 件を超えるメッセージが送信されないようにしています（つまり、10,000 件のメッセージのうち、最後の 30 分よりも前半 30 分以内に送信されるメッセージの割合が高くなる可能性があります）。この形式のレート制限では、時間に敏感なメッセージを遅延させないように注意してください。セグメントに3,000万人のユーザーが含まれていても、レート制限を1分あたり10,000人に設定した場合、ユーザーベースの大部分は翌日までメッセージを受信しません。
...
\[接続コンテンツ再試行] ][19] 機能を有効にすると、Brazeは再送信ごとに設定したレート制限を尊重しながら、失敗した通話を再試行します。1 分あたり 10,000 件のレート制限がある 75,000 件のメッセージについてもう一度考えてみましょう。最初の 1 分間は、通話が失敗するか遅くなり、4,000 通のメッセージしか送信されません。
...
この後、このキャンペーンを引き続きフリークエンシーキャップにカウントするかどうかを尋ねられます。フリークエンシーキャップの対象となるメッセージは、インテリジェントチャネルフィルターの計算に含まれます。トランザクション型であることが多い \[API] ][15] キャンペーンを送信する場合、][16]に設定することで、キャンペーンが API リクエスト内の \[フリークエンシーキャップルール] を無視するように指定できます。`override_messaging_limits` `true`
...
このシナリオでは、次のフリークエンシーキャップルールを使用します。
...
| アクション (Action) | 結果: |
|---|---|
| `promotional`タグは、ユーザーがメッセージを受信した後、**キャンペーン** **B が送信する前にキャンペーン A から削除されます。** | **ユーザーはキャンペーン B** を受け取ります。|
| ユーザーがメッセージを受信した後に、`promotional`**タグが誤ってキャンペーン A** から削除されます。<br> タグは、**キャンペーン** **B が送信される前の火曜日にキャンペーン A** に追加されます。 | **ユーザーにはキャンペーン B** は届きません。 |
