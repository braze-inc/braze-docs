...
]({% image_buster /assets/img_archive/rate_limit_daily.png %})[最後に受信したメッセージ] セグメントフィルターがハイライトされている [セグメントの詳細] セクション。()
...
]({% image_buster /assets/img_archive/total_limit.png %})キャンペーンを受信する人数を制限するための選択されたチェックボックスを含むオーディエンスサマリ。()
...
]({% image_buster /assets/img_archive/per_minute_rate_limit.png %})キャンペーンの送信レートを制限するためのチェックボックスが選択されておりレートが500,000/分に設定されている「オーディエンスの概要」。()
...
]({% image_buster /assets/img_archive/rate_limiting_overview_2.png %})ルールが適用される、および適用されないキャンペーンとキャンバスのリストを含む頻度の上限セクション。()
...
]({% image_buster /assets/img_archive/frequencycappingupdate.png %})フリークエンシーキャップがオンになっている「配信コントロール」セクション。()
...
]({% image_buster /assets/img/standard_rules_fnfn.png %})1週間ごとに、すべてのキャンペーン/キャンバスステップから最大2件までのプッシュ通知キャンペーン/キャンバスステップをユーザーに送信するというルールが示されている「フリークエンシーキャップ」セクション。()
...
]({% image_buster /assets/img/tag_rule_fnfn.png %} "rules")1週間ごとにユーザーに送信できるプッシュ通知キャンペーン/キャンバスの数を制限する2つのルールが示されている「フリークエンシーキャップ」セクション。
...
]({% image_buster /assets/img/global_rules.png %} "global rules")1 週間ごとにユーザーに送信されるプッシュ通知キャンペーン/キャンバスステップの数を制限するために、競合するルールを持つFrequency Capping セクション。
