...
Braze は、メッセージ送信を1 分間に均等に分散しようとしますが、これを保証することはできません。たとえば、レート制限が 1 分あたり 5000 メッセージのキャンペーンがある場合、1 分あたり 5000 件のリクエストを均等に (1 秒あたり約 84 件のメッセージ) 配信しようとしますが、1 秒あたりのレートには多少のばらつきがあります。
...
キャンペーン構成の**Target Users** ステップで、メッセージを受信するユーザーの総数を制限することもできます。これはキャンペーンフィルターとは別の独立したチェック機能なので、過剰なスパムの心配をせずにユーザーを自由にセグメント化できます。

![キャンペーンを受信する人数を制限するために、選択したチェックボックスを含むオーディエンスサマリ。][2]
...
大規模なキャンペーンでユーザーのアクティビティが急増し、サーバーが過負荷になると予想される場合は、1 秒あたりのメッセージ送信のレート制限を指定できます。つまり、Braze は 1 分以内にレート制限の設定を超えて送信しません。
...
![キャンペーンの送信レートを制限するためのチェックボックスが選択されておりレートが500,000/分に設定されている「オーディエンスの概要」。][3]
...
{% alert important %}
このようなレート制限を行うと、時間に敏感なメッセージを遅らせることになるので注意が必要です。セグメントに3,000万人のユーザーが含まれているが、レート制限を10,000/分に設定すると、このユーザー群の大部分は翌日までメッセージを受信しません。
{% endalert %}
...
[Connected Content retry][19]をオンにすると、再送信ごとに設定したレート制限を尊重しながら、Braze はコールの失敗を再試行します。速度制限が10,000/分の75,000メッセージを送信するシナリオを考えてみましょう。最初の1分間で、コールが失敗するか、または遅く、4000メッセージのみを送信すると仮定します。

遅延を補い、残りの6000のメッセージを秒間に送信するか、すでに送信するように設定されている10,000に追加するかわりに、Brazeは、これらの6000のメッセージを「キューの後ろ」に移動し、必要に応じて、メッセージの送信にかかる時間の合計に分を追加します。
...
接続されたコンテンツリクエストは個別にレート制限されず、Webhook レート制限に従います。つまり、Webhook ごとに一意のエンドポイントへの接続コンテンツコールが1 つある場合、1 分あたり5000 個のWebhook と5000 個の接続コンテンツコールが予想されます。キャッシュがこれに影響し、接続コンテンツコールの数が減る可能性があることに注意してください。さらに、再試行により接続コンテンツコールが増加する場合があるため、ここでは、Connected Content エンドポイントがある程度の変動を処理できることを確認することをお勧めします。
...
![1 週間ごとにユーザーに送信されるプッシュ通知キャンペーン/キャンバスステップの数を制限するために、競合するルールを持つFrequency Capping セクション。][11]
