...
### コンバージョンsの数が、マルチチャネル キャンペーンsの一意のユーザーsの数を超えることができるのはなぜですか?

マルチチャネル キャンペーンsの場合、Brazeはユーザーごとではなく、チャネルごとにコンバージョンを数えます。ユーザーがコンバージョン アクションをコンバージョンウィンドウ内で実行すると、そのコンバージョンをBraze 属性して、ユーザーが受信したチャネルに送信します。つまり、ユーザーが複数のチャネルs (たとえば、メールとプッシュの両方) でメッセージを受信し、変換すると、チャネルごとに1 つずつ、Braze が複数のコンバージョンs をカウントします。そのため、総コンバージョン数は、変換する一意のユーザーの数を超えることがあります。

たとえば、マルチチャネル キャンペーンがメールとプッシュ通知の両方をユーザーに送信し、そのユーザーがメッセージを受信した後とコンバージョンウィンドウ内で1つのコンバージョン アクションを実行する場合、同じユーザーによる単一のアクションであっても、Brazeはこれを2つのコンバージョンs、dからメールへの1つの属性、dをプッシュするための1つの属性dとして数えます。
...
### 空白アプリの耳元は、入力フィールドsと表示されるテキストで異なるのはなぜですか? 

空白の処理は、入力フィールドs と表示されるテキストコンポーネント間でCSSなスタイル付けのために異なります。デフォルトが`white-space: normal` CSSのテキストコンポーネントでは、複数の連続する空白が表示されたときに1 つの空白に折りたたまれます。これは、レンダリングされたテキストの標準的なHTMLの振る舞いです。 

入力フィールドでは、入力したとおりに複数の空白が保持されます。これは、ACキュレートデータエントリの間隔を正確に確認して変更する必要があるためです。つまり、複数の空白を含むテキストは、入力フィールド(すべての空白がp 予約)で表示される場合と、ダッシュボードの他の部分(CSSで複数の空白が折りたたまれる可能性がある場合)で表示される場合で、耳のアプリが異なることがあります。 

たとえば、入力フィールドに複数の空白を持つキャンペーンの名前またはUTM パラメータを入力すると、すべての空白p 予約が表示されます。ただし、同じテキストアプリが検索結果、キャンペーン一覧、または他のテキストコンポーネントを耳にする場合、空白文字の処理がCSSため、複数のスペースが単一のスペースとして耳にアプリされることがあります。 
