...
| コンテンツサイズ | 最大50KB。 |
| 配置 | コンテンツブロックはメールのフッター内で使用できない。ただし、メールで使用するための[フッターを含むコンテンツブロックを作成](#email-footers)することは可能だ。 |
...
このプレビューには、コンテンツブロックに関する情報が含まれている。具体的には、作成者、タグ、作成日時、最終編集日時、説明、編集者タイプ、使用状況（そのコンテンツブロックを使用しているメッセージやコンテンツブロックのクリック可能なリスト）、そして実際のコンテンツブロックのプレビューである。

![自転車とダンスに関する"Workout_Promo"コンテンツブロックのプレビューだ。一つだけ含まれている。]({% image_buster /assets/img/preview_tab_content_block.png %}){: style="max-width:60%;"} 
...
### コンテンツブロックでのメールフッターの使用 {#email-footers}

コンテンツブロックはメールのフッター内では使用できない。ただし、メールで使用するためのフッターコンテンツを含むコンテンツブロックを作成することは可能だ。そのためには、以下の手順に従ってほしい：

1. **設定**＞**メール設定**＞**カスタムフッター**に移動し、フッターを作成する。
2. **コンテンツブロックライブラリー**内のコンテンツブロックにフッターを追加する。
3. そのコンテンツブロックをメールテンプレートやメッセージに追加する。

複数のメッセージで同じフッターを使えるようになったぞ！
...
コンテンツブロックの使用を終えたら、[テンプレートメディア]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/managing_templates/)ページ[&]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/managing_templates/)からアーカイブできる。アーカイブされたコンテンツブロックは読み取り専用なので、編集する前にコンテンツブロックをアーカイブ解除してください。コンテンツブロックは、いずれかのメッセージで使用されている場合はアーカイブできません。
...
- このリストの最初の提案で合理的に管理できる範囲を超えて、ブロックが複数のメッセージで使用される場合、ブロック内のすべてのコンテンツを削除することを推奨する。これにより、いかなるメッセージングにも古い情報が含まれるのを防ぐ。
- コンテンツブロックを誤ってアーカイブした場合は、アーカイブを解除できます。

![保存済みコンテンツブロックパネルでは、設定ドロップダウンメニューが"Test_32"展開され、以下の3つのオプションが表示される：アーカイブ解除、複製、ワークスペースにコピー]({% image_buster /assets/img/unarchive-content-block.png %})
