...
- **方法1:**重複したキャンバスで、キャンバスビルダーの**エントリオーディエンス**部分を編集して、テストユーザーのみがキャンバスの対象となるようにします。また、**メールアドレス**のテストフィルターを追加することで、自分のメールアドレスをテストユーザーとして入力することもできます。以下の例では、キャンバスを、アプリを初めて使ってから 3 日以内の 2 人のテストユーザーに限定しています。

![][2]

- **方法2:**[プレビュー your ユーザー paths]({{site.baseurl}}/preview_user_paths/) by selecting the **Test キャンバス** button in the footer of the キャンバス builder.

## ステップ 4:テストを開始する

テストキャンバスを開始し、ユーザーが入り始めるようにします。アプリケーションでユーザーの行動を完了し、それぞれのキャンバスの旅にユーザーを送ります。
