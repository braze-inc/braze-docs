...
元のキャンバスエクスペリエンスを使用して、キャンバスを作成または複製することはできなくなりました。Brazeは[、キャンバスを]({{site.baseurl}}/user_guide/engagement_tools/canvas/managing_canvases/cloning_canvases/)最新のエディターに[複製することを]({{site.baseurl}}/user_guide/engagement_tools/canvas/managing_canvases/cloning_canvases/)推奨している。
{% enddetails %}
...
{% tabs local %}
  ...
  {% endtab %}
  {% tab Entry Schedule %}
    ここで、ユーザーがキャンバスに入る方法とタイミングを決めることになる。
...
![キャンバス詳細ページには、キャンバス名、説明、場所、タグを入力するフィールドがある。]({% image_buster /assets/img/canvas_details.png %}){: style="max-width:70%;"}
...
{% tabs local %}
  ...
  {% tab API-Triggered Delivery %}
    API トリガー配信では、ユーザーがキャンバスに入り、API 経由で [`/canvas/trigger/send` エンドポイント]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)を使用してメッセージが追加されると、メッセージを受け取り始めます。ダッシュボードでは、これを実行するcURLリクエストの例と、[コンテキストオブジェクト]({{site.baseurl}}/api/objects_filters/context_object/)を使用した[`context`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)オプションの割り当てを確認できる。 
...
**ターゲット**オーディエンスステップでは、定義した条件に合致するユーザーのみがジャーニーに参加できる。つまりBrazeは、ユーザーがCanvasジャーニー**に入る前に**、まず対象ユーザーの適格性を評価する。例えば、新規ユーザーをターゲットにしたい場合、初めてアプリを使用したのが1週間以内のユーザーをセグメントとして選択することができる。

**エントリーコントロール**では、キャンバスがスケジュールされたたびに、ユーザー数を制限できる。APIトリガーベースおよびアクションベースのキャンバスでは、この制限はUTC時間ごとに発生する。 
...
![「ユーザー検索」フィールドは、外部ユーザー ID または Braze ID による検索を可能にする。]({% image_buster /assets/img_archive/user_lookup.png %}){: style="max-width:80%;"}

##### エントリコントロールの選択

エントリコントロールは、ユーザーがキャンバスに再エントリできるかどうかを決定します。また、エントリスケジュールタイプに応じて、選択したペースでこのキャンバスにアクセスする可能性のある人数を制限することもできる。

- **スケジュールされた：**キャンバスの存続期間中、あるいはキャンバスがスケジュールされたたびに
- **アクションベース:**時間単位、日単位、あるいはキャンバスの存続期間
- **API トリガー:**時間単位、日単位、あるいはキャンバスの存続期間

例えば、アクションベースのキャンバスで**「入場者数を制限する」**を選択し、「**最大入場者数**」フィールドを5,000ユーザーに設定し、制限間隔を「**毎日**」にした場合、キャンバスは1日あたり5,000ユーザーにしか送信しない。

![[エントリーコントロール] ページに [ユーザーがこのキャンバスに再エントリできるようにする] および [エントリ数の制限] のチェックボックスが表示されます。後者は、最大エントリ数を設定し、エントリスケジュールタイプに応じた頻度を選択できる。例えば、キャンバスの有効期間中、あるいはキャンバスがスケジュールされたエントリごとに設定する場合がある。また、アクションベースやAPIトリガーによるエントリでは、時間単位、日単位、あるいはキャンバスの有効期間中といった設定が可能だ。]({% image_buster /assets/img_archive/entry_controls.png %})

{% alert tip %}
Brazeは、IPウォームアップのスケジュール設定において**「キャンバスがスケジュールされるたびに」**を選択することを推奨しない。これは送信量の増加につながる可能性があるためである。
{% endalert %}
...
![「バリアントを追加」ボタンを選択すると、コンテキストメニューが表示され、「バリアントを追加」というオプションが表示される。]({% image_buster /assets/img_archive/canvas_add_variant.gif %}){: style="float:right;max-width:40%;margin-left:15px;"}
...
1. 乱数を保存するカスタム属性を作成します。名前は簡単に見つけられるものに付けろ。例えば"lottery_number"「」とか「」とか"random_assignment".だ。属性は[ダッシュボードで]({{site.baseurl}}/user_guide/data/custom_data/managing_custom_data/)作成するか、APIを通じて我々の[`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)「[」エンドポイント]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)に呼び出して作成できる。<br><br>
2. キャンバスの始めに Webhook キャンペーンを作成します。このキャンペーンは、そこに乱数を作成し、カスタム属性として保存する媒体となります。詳細は「[Webhookの作成」]({{site.baseurl}}/user_guide/message_building_by_channel/webhooks/creating_a_webhook/#step-1-set-up-a-webhook)を参照せよ。URL を `/users/track` エンドポイントに設定します。<br><br>
...
5. キャンバスの分岐を調整して、ランダムに選んだバリアントで分けるのではなく、オーディエンスルールに基づいて分けるように設定します。各分岐のオーディエンスルールで、カスタム属性に応じてオーディエンスフィルターを設定します。<br><br>例えば、あるBranchでは「3インチ未満」"lottery_numberをオーディエンスフィルターとして設定している一方、別のBranchでは「3インチ以上6インチ未満」"lottery_numberをオーディエンスフィルターとして設定している。
...
![コンポーネント検索ウィンドウがBraze キャンバスに遅延ステップを追加する。]({% image_buster /assets/img_archive/add_components_flow.png %}){: style="max-width:80%;"}

{% alert important %}
キャンバスには最大200ステップを追加できる。キャンバスが200ステップを超えると、読み込む際に問題が発生する可能性がある。
{% endalert %}
...
キャンバスワークフローの任意のステップは、コンポーネントを選択することで編集できる。例えば、ワークフローの最初のステップである[遅延]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/)コンポーネントを特定の日に変更したいとします。ステップを選択して設定を表示し、3月1日までの遅延を調整する。すると、ユーザーは 3 月 1 日にキャンバスの次のステップに移行します。

![遅延ステップの例で、遅延が「特定の日にちまで」に設定されている。]({% image_buster /assets/img_archive/edit_delay_flow.png %})
...
![キャンバスの第二ステップ「アクション設定」では、評価期間を1日に設定する。]({% image_buster /assets/img_archive/action_paths_flow.png %})
...
![「メッセージ設定」ステップでは、「メッセージングチャネル」を選択すると、利用可能なメッセージングチャネルの一覧が表示される。例えば、Androidプッシュ通知、コンテンツカード、メールなどだ。]({% image_buster /assets/img_archive/message_setup_settings_flow.png %})
...
[オブジェクト`context`]({{site.baseurl}}/api/objects_filters/context_object)は、キャンバス作成時の**「エントリスケジュール**」ステップで設定され、ユーザーをキャンバスに誘導するトリガーを示す。これらのプロパティは、API トリガーキャンバスのエントリペイロードのプロパティにもアクセスできます。`context` オブジェクトは最大50KB まで可能であることに注意してください。 

キャンバスに入力した際に作成されるこれらのプロパティを参照する際は、以下のLiquidを使用する。{% raw %}``context.${property_name}``{% endraw %}このように使用するには、イベントがカスタムイベントまたは購入イベントでなければならないことに注意してください。
...
キャンバスでは、カスタムイベントと購入イベントのプロパティは、アクションパスステップに続く任意のメッセージステップにおいてLiquidで使用できる。これらの `event_properties` を参照する場合は、{% raw %} ``{{event_properties.${property_name}}}``{% endraw %} という Liquid を使用します。メッセージコンポーネントでこのように使用するには、これらのイベントがカスタムイベントまたは購入イベントである必要があります。
...
![Braze Canvas のバリアントの例。70% は"Variant 1" に移動します。これは、最初のステップで1 日間遅延し、2 番目のステップでメッセージを送信します。残りの30％は、何のフォローアップステップもない「コントロール」に振り分けられる。]({% image_buster /assets/img_archive/Canvas_Multivariate_Flow.png %})
...
![[バリアント分布を編集] ページで [インテリジェントセレクション] オプションが有効になります。キャンバスを分析して最適化すると、ページ全体に横向きのバーが表示されます。このバーは複数のセクションに分割され、それぞれ色とサイズが異なります。これは単なる視覚的な表現であり、特定の分析結果とは関連していない。]({% image_buster /assets/img_archive/canvas_intelligent_selection.png %})
