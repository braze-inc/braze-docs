...
アクションによってエントリがトリガーされるキャンバスステップの場合、ユーザーがセッションの途中でキャンバスに入る可能性があります。アプリ内メッセージはセッションが開始されるまでライブに設定されないため、ユーザーがメッセージステップに到達したときにセッションの途中であった場合、別のセッションを開始し、関連するトリガーを実行するまでアプリ内メッセージを受信することはない。
