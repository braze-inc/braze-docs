...
アクションによってトリガーされるキャンバスの場合、ユーザーがセッションの途中でキャンバスに入る可能性があります。ただし、前述のように、アプリ内メッセージは次のセッションが開始されるまでトリガーされず、これらのユーザーはセッション開始前にキャンバスに入る資格がないために、最初のアプリ内メッセージを見逃してしまいます。
...
| オプション | 説明 | 例 |
|---|---|---|
| メッセージは指定した期間が経過すると期限切れになる | 最初のオプションでは、ユーザーがステップを利用できるようになったときを基準にしてアプリ内メッセージを期限切れにすることができます。 | 例えば、有効期限が 2 日のアプリ内メッセージは、ステップの遅延が経過してオーディエンスオプションがチェックされた後に利用可能になります。その後、2 日間 (48 時間) 使用可能になり、その間にユーザーがアプリを開くとアプリ内メッセージを表示することができます。 |
| メッセージは指定の日付までに期限切れになる | 2 つ目のオプションでは、アプリ内メッセージが表示されなくなる特定の日時を選択できます。| 例えば、特定の日時に終了したセールがある場合、このオプションを選択して、セールの終了時に関連するアプリ内メッセージがユーザーに表示されなくなるよう設定します。|
...
プロモーション、クーポン、セールには、多くの場合、ハードな有効期限があります。次のキャンバスは、購入を促すために、ユーザーに利用可能なプロモーションがあることを最も適切なタイミングで通知するよう設定されています。このプロモーションは、会社のタイムゾーンの 2019 年 2 月 28 日午前 11 時 15 分までに期限切れになります。
...
<tbody>
  <tr>
    ...
    <td>プロモーションについてユーザーに警告する最初のプッシュ通知。これは、プロモーションを利用するようにユーザーをアプリに誘導することが目的です。</td>
  </tr>
  <tr>
    <td>アプリ内: 50% 割引</td>
    <td>なし</td>
    <td>エントリ全員</td>
    <td>アプリ内メッセージ</td>
    <td><b>有効期限:</b> 2/28/2019 11:15 AM Company Time</td>
    <td>アプリ内メッセージの表示</td>
...
| オプション | 説明 |
|---|---|---|
| メッセージの送信時にユーザーを昇進させる | キャンバスの次のステップに進むには、ユーザーがメール、Webhook、またはプッシュ通知を送信されるか、アプリ内メッセージを表示する必要があります。 <br> <br>  アプリ内メッセージの有効期限が切れても、ユーザーにメール、Webhook、プッシュが送信されていない、またはアプリ内メッセージが表示されていない場合、ユーザーはキャンバスを退出し、次のステップに進めません。 |
| オーディエンスをすぐに昇進させる | 延期期間が経過すると、メッセージを表示したかどうかにかかわらず、そのステップのオーディエンス全員が次のステップに進みます。 <br> <br> ユーザーが次のステップに進むには、ステップのセグメントとフィルターの条件に一致する必要があります。|
{: .reset-td-br-1 .reset-td-br-2}

![][3]

{% alert important %}
  「オーディエンス全体」を選択すると、ユーザーが後続のステップに移動した後も、アプリ内メッセージは有効期限が切れるまで利用可能になります。キャンバスの次のステップが配信されたときにアプリ内メッセージを有効にしたくない場合は、有効期限を後続のステップの遅延よりも短く設定してください。
{% endalert %}
