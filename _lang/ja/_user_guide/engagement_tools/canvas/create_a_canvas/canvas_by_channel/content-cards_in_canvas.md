...
新しいコンテンツカードを作成する際、送信時間に基づいてユーザーのフィードで使用できなくなる期限を選択できます。カードはユーザーのフィードで最大 30 日間使用することができます。コンテンツカードの有効期限のカウントダウンは、カードが送信されたキャンバス内のメッセージステップにユーザーが到達するとすぐに開始されます。この時点から有効期限が切れるまで、カードはユーザーのフィードでアクティブになります。
...
ユーザーがキャンバスジャーニーの後続のステップに進んだ場合でも、コンテンツカードは有効期限になるまでユーザーのフィードで使用できます。キャンバスの次のステップが配信されたときにコンテンツカードをライブにしたくない場合は、有効期限を後続のステップでの遅延よりも短く設定してください。

コンテンツカードの有効期限が切れると、ユーザーがまだ表示していなくても、次回の更新時に自動的にユーザーのフィードから削除されます。
...
| オプション | 説明 |
|---|---|
| メッセージの送信時にユーザーを昇進させる | コンテンツカードが正常に送信されると、ユーザーがキャンバスの次のステップに進みます。このオプションは、カードが送信され、中止されない場合にのみユーザーを前進させたい場合に使用します。 |
| オーディエンスをすぐに昇進させる | コンテンツカードの送信が試行されると、ユーザーがキャンバスの次のステップに進みます。カードが中止され、送信されない場合でも、ユーザーは次のステップに進みます。このオプションは、コンテンツカードが正常に送信されたか中止されたかに関係なく、ユーザーを前進させたい場合に使用します。 |
...
| オプション | 説明 |
|---|---|
| メッセージの送信時にユーザーを昇進させる | このステップのメッセージタイプのうち少なくとも 1 つが正常に送信されたときに、ユーザーがキャンバスの次のステップに進みます。|
| オーディエンスをすぐに昇進させる | このオプションを選択すると、メッセージを表示したかどうかにかかわらず、遅延時間が経過した後、コンポーネントのオーディエンス全員が次のステップに進みます。 <br> <br> _ユーザーが次のステップに進むには、コンポーネントのセグメントとフィルターの条件に一致する必要があります。|_
{: .reset-td-br-1 .reset-td-br-2}
...
ユーザーが閲覧できるさまざまなトピック、トランザクションフィードやマーケティングフィードといったカテゴリに基づいて、コンテンツカードを複数のフィードに分けることができます。キーと値のペアを使用して複数のフィードを作成する方法の詳細については、[コンテンツカードフィードのカスタマイズ][7] ガイドを参照してください。
