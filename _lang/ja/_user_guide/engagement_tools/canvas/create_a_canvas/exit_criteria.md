...
]({% image_buster /assets/img/exit_criteria.png %}){: style="max-width:90%;"}() 商品を閲覧したが、まだカートに入れたり注文したりしていないユーザーを再度エンゲージするために設定された終了基準。
...
]({% image_buster /assets/img_archive/exit_criteria_example.png %})例外イベントとして「何か購入する」が設定された終了条件。ユーザーが何らかの買い物をするとこのキャンバスから退出します。{: style="max-width:80%;"}()
