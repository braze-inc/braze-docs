...
- コンバージョンイベントは編集できない。 
- [オーディエンスパス]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/audience_paths/)、[アクションパス]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/action_paths/)、[実験パス]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/experiment_step)は、これらのステップで追加または削除できません。また、ランキングを調整するために並び替えることもできません。回避策として、アクティブなキャンバスを編集し、ステップを複製します。これは、下書きを開始するまで編集可能です。
