...
キャンバスユーザーの移動にステップを追加または編集する場合は、次の情報がアプリされます。
...
キャンバスがどのワークフローで作成されたかに応じて、開始後のキャンバスは以下の編集が可能です。キャンバスが元のキャンバスワークフローを使用している場合、起動後の編集を実行するには、最初にキャンバスフローにクローンを作成する必要があります。
...
ただし、ステップのスケジュールタイプとコントロールパーセンテージは開始後に編集できません。アクションパスとオーディエンスパスのステップでは、ランキングと評価ウィンドウは起動後に編集できません。
...
ローカルの送信時刻で起動するキャンバススケジュールされたは、スケジュールされたの送信時刻の24 時間前まで編集できます。この期間は「セーフゾーン」と呼ばれます。 

{% alert tip %}
完全に新しいキャンバスを作成するなどの大規模な編集を行う場合は、最初のキャンバスを受け取ったユーザーを除外し、タイムゾーンの送信を考慮して、キャンバスのスケジュール時間を再調整してください。
{% endalert %}

エントリ スケジュールが起動直後にユーザーs に入るように設定されている場合、キャンバスは最も近い時刻に5 分単位で起動します。たとえば、キャンバスを更新して午前8:31 PST にすぐにユーザーs を入力すると、起動時刻はPST の午前8:30 に設定され、企業のタイムゾーンに設定されます。
...
キャンバスエントリプロパティは、送信時にステップにテンプレート化されません。つまり、キャンバスが起動した後にキャンバスエントリのプロパティーが編集されると、これらの変更はキャンバスに入る新しいユーザーにのみアプリされます。キャンバスでユーザーがキャンバスに再入場できる場合、再入場するユーザーは、更新されたキャンバスのエントリプロパティによって決定されます。
...
切断されたステップを作成・編集することで、キャンバスの残りの部分に直接接続することなく、独立したステップに変更を加えることができます。これは、キャンバスを再起動する前にステップをテストするのに役立ちます。 

### 実験パスステップ

キャンバスにアクティブまたは進行中の勝者パスまたはパーソナライズされた パスの実験があり、アクティブなキャンバスを更新すると(実験パス ステップを更新するかどうかに関係なく)、進行中の実験は終了し、実験パスs ステップはs を決定しません。実験を再開するには、既存の実験パスをを解除して新しい実験パスを開始するか、キャンバスを複製して新しいキャンバスを起動します。それ以外の場合、ユーザーs は最適化メソッドが選択されていないかのように、実験パスを通過します。

### 遅延

遅延のあるキャンバスの編集は、多少複雑です。そのため、キャンバスを編集する際は、以下の事項に注意してください。

遅延ステップの遅延を更新すると、そのステップのキューに入れられていない新しいユーザーのみが、更新d の遅延でメッセージを受信します。

遅延のあるステップ (遅延やアクションパスなど) を削除し、そのユーザーを別のキャンバスステップにリダイレクトすると、ユーザーはステップの遅延の完了後にリダイレクトされます。たとえば、1 日の遅延で遅延ステップを削除し、それらのユーザーをメッセージステップにリダイレクトするとします。このケースでは、ユーザーs は1 日間の遅延が完了した後にのみリダイレクトされます。
...
たとえば、キャンバスフローワークフローを使用して作成されたキャンバスを、2 つのステップs を持つ1 つのバリアントで午後2 時に起動するように設定したとします。1 時間の遅延を持つ遅延ステップはメッセージステップに入ります。 
...
{% alert important %}
以下の問題は回避可能です。起動後にキャンバスを編集する必要がある場合は、キャンバスにすでに入力されているすべてのユーザーがユーザーの移動を完了したことを最初に確認することをお勧めします。また、少なくとも1 つのユーザーによって処理済みのステップは削除しないことをお勧めします。
{% endalert %}
