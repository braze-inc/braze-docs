...
例外イベントは、ユーザが関連付けられているキャンバスコンポーネントの受信を待機している間にのみトリガされます。ユーザーが事前にアクションを実行した場合、例外イベントはトリガーされません。特定のイベントを事前に実行したユーザーを除外したい場合は、代わりに[フィルター]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/)を使用してください。
