...
- **繰り返し、アクショントリガー、APIトリガーのキャンバス：**最初の実験は、指定された期間に実験パスに入るすべてのユーザーに対して実行されます。実験の完全性を維持するために、ユーザーがウィンドウの終了前に多数のメッセージを受信した場合、毎回同じバリアントに割り当てられます。実験期間の後、各ユーザーはコンバージョンの確率が高いパスに送られます。
...
{% endtab %}
{% tab 反復またはアクショントリガーまたはAPIトリガーキャンバス %}
