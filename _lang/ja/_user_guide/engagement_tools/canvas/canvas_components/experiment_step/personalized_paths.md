...
- **単一送信キャンバス:**ユーザー の集合が遅延グループに保持されます。残りのユーザーは、設定した期間中に予測モデルをトレーニングするための最初のテストに合格します。最良の結果を得るには、少なくとも24 時間かかります。テストの後、ユーザーの行動が特定のパスにおけるコンバージョンの高可能性にどのように関連付けられているかを学習するモデルが作成されます。最後に、遅延グループの各々のユーザーは、それらが示す挙動と最初の試験の間に学習した予測モデルに基づいて、それらに対するコンバージョンをもたらす可能性が最も高いパスに送られる。
...
実験ウィンドウが通過し、実験が完了すると、Brazeは、予測モデルの推奨に基づいて、コンバージョンの可能性が最もパーソナライズされた遅延グループのユーザー をそれぞれのパスs に送信します。
...
![ユーザー特性テーブルは、どのユーザー が、最新性、頻度、および保有期間のために下がる3つのバケットに基づいて、パス1およびパス2を優先すると予測されているかを示します。]({% image_buster /assets/img/experiment_step/experiment_personalized_analytics_tab1_2.png %})
