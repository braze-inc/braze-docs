...
ユーザーのエントリが 1 回に限定されているキャンバスで勝者パスを使用する場合、遅延グループが自動的に含められます。実験期間中、残りのユーザーが実験パスに入る間、何割かのユーザーはディレイグループに留まる。
...
ローカルタイム配信を使用したい場合は、24 ～ 48 時間以上の実験期間を使用してください。これにより、初期のタイムゾーンのユーザーがキャンバスに入って実験の開始がトリガーされた後でも実験期間に十分な時間を残すことができます。遅い時間帯にいるユーザーは、キャンバスや勝者パスがある実験ステップに入るのに十分な時間があり、実験ウィンドウが切れる前にコンバージョンできる可能性がある。

### クリック数に基づくバリアント

クリックに基づいて勝者バリアントを設定している場合、Brazeによって配信停止クリックと識別されない限り、すべてのインタラクションはクリックとしてカウントされる。
