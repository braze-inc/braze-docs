...
![キャンバスユーザーの移動中のエージェントステップ。]({% image_buster /assets/img/ai_agent/agent_step.png %}){: style="float:right;max-width:30%;margin-left:15px;"}
...
エージェント出力は、文字列、数値、ブール値、またはオブジェクトとして保存できます。これにより、キャンバス内のテキストパーソナライゼーションと条件付きロジックの両方に柔軟性を持たせることができます。各タイプの一般的な使用方法を次に示します。
...
| オブジェクト | 予測可能なデータ構造で、1 つのLLM 呼び出しで上記のデータ型の1 つ以上を利用する |
...
オブジェクト出力変数の特定のプロパティを使用するには、ドット表記を使用して、リキッドを使用してそのプロパティにアクセスします。 {% raw %}`{{context.${response_variable_name}.field_name}}`{% endraw %}

![変数"agent_output".]({% image_buster /assets/img/ai_agent/test_agent_step.png %} のオブジェクトデータタイプ出力を持つBody HTML Writer のエージェントステップ{: style="max-width:80%;"}

上に示したリキッド構文パターンを使用して、将来のキャンバスステップs のエージェント出力から特定のフィールドs を参照します。

### ステップ 4: エージェントに提供するコンテキストを決定する  

エージェントが実行時に受信するデータを決定する必要があります。以下のオプションがあります。  

- **すべてのキャンバスコンテキストを含める:**使用可能なすべてのキャンバスコンテキスト変数([キャンバスエントリプロパティ]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_entry_properties_event_properties)など)をエージェントステップに渡します。エージェントステップs の上流でコンテキストステップを使用して、前方のコンテキストにデータを追加できます。
...
エージェントステップを設定した後、このステップのアウトプットを試験し、プレビューすることができます。
