...
]({% image_buster /assets/img_archive/canvas_user_update_step.png %})属性を「Is Premium Member」に更新する「Update loyalty」というユーザー更新ステップ。{: style="float:right;max-width:30%;margin-left:15px;"}()
...
]({% image_buster /assets/img_archive/canvas_user_update_update.png %})2つの属性「Loyalty Member」と「Loyalty Program」を「true」に更新するユーザー更新ステップ。{: style="max-width:90%;"}()

### カスタム属性を削除する

カスタム属性を削除するには、ドロップダウンを使用して属性名を選択します。[高度な JSON コンポーザー](#advanced-json-composer)に切り替えて、さらに編集することができます。 

]({% image_buster /assets/img_archive/canvas_user_update_remove.png %})属性「Loyalty Member」を削除するユーザー更新ステップ。{: style="max-width:90%;"}()
...
]({% image_buster /assets/img_archive/canvas_user_update_composer.png %})
...
]({% image_buster /assets/img_archive/canvas_user_update_test_preview.png %})
