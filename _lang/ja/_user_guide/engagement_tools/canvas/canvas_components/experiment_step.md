...
]({% image_buster /assets/img/experiment_step/experiment_step.png %})Path 1、Path 2、Controlに分岐する実験パスステップ。{: style="float:right;max-width:50%;margin-left:15px;"}()
...
]({% image_buster /assets/img/experiment_step/exp_settings.png %})[実験設定 (Experiment Settings)] では、パスを追加し、各パスにおけるユーザーの割合を分配することができる。()
...
]({% image_buster /assets/img/experiment_step/experiment_downstream_paths.gif %})実験パスコンポーネントから分岐する各パスにステップを追加。{: style="max-width:75%"}()
