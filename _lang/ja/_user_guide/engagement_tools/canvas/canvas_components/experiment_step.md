...
{% alert note %}
キャンバスの開始後は、実験パスを編集できなくなります。例えば、実験パスステップのあるすでにアクティブなキャンバスで、「パーソナライズされたパス」または「勝者パス」をオンにすることはできません。詳細については、「[開始後のキャンバスの編集]({{site.baseurl}}/user_guide/engagement_tools/canvas/managing_canvases/change_your_canvas_after_launch/)」を参照してください。
{% endalert %}
