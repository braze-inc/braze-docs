...
![オーディエンスパスには二つのグループがある：エンゲージメントの高いユーザーと、それ以外の全員だ。]({% image_buster /assets/img/audience_path/audience_path.png %}){: style="float:right;max-width:45%;margin-left:15px;margin-top:15px;"}
...
![メッセージステップの後に24時間の遅延を示すキャンバス、続いてオーディエンスパスが続く。]({% image_buster /assets/img/audience_path/audience_path5.png %}){: style="float:right;max-width:40%;margin-left:15px;"}

ユーザーは、キャンバスに入った時点ではなく、**オーディエンスパスステップに到達した時点で**、フィルターとセグメントメンバーシップに基づいて評価される。評価後、彼らは直ちに対応するパスへ進む。ユーザーがオーディエンスグループに配置されると、その後ユーザープロファイルが変更されても、そのグループに留まる。

{% alert important %}
オーディエンスパスは、評価時点におけるユーザーの現在の属性、フィルター、およびセグメント所属に基づいて評価する。彼らは、キャンバスへのエントリをトリガーとした特定の事象に基づいて評価しない。ユーザーが実行したアクション（カスタムイベントなど）に基づいてユーザーをルーティングするには、代わりに[アクションパス]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/action_paths/)を使用する。
{% endalert %}

### ユーザー評価の時間を確保する

評価が即時に行われるため、パスの条件が前のステップでのユーザー操作に依存する場合、オーディエンスパスの前に遅延を追加することが重要だ。
...
ユーザーが送信されたメッセージとやり取りする時間を確保するため、メッセージステップとオーディエンスパスの間に遅延を追加する。例えば、24時間の遅延を設定すると、ユーザーはメッセージAが送信されてから24時間後まで、評価を行う前にメッセージAとやり取りできる。
...
![拡張されたオーディエンスパスには、「アジア料理が好き」「ラテン料理が好き」「ヨーロッパ料理が好き」および「その他全員」のグループが含まれる。]({% image_buster /assets/img/audience_path/audience_path3.png %})

オーディエンスパスステップの完了後、各オーディエンスグループは個別の分岐を持ちます。オーディエンスパスを使用してオーディエンスをさらに絞り込むことも、標準のキャンバスステップを使用してキャンバスのジャーニーを続けることもできます。 

![エンゲージメントに基づいて異なるグループに分かれた、二つのオーディエンスパス。]({% image_buster /assets/img/audience_path/audience_path4.png %}){: style="max-width:50%"}
...
![オーディエンスパスには「有名ブランドの靴が好き」グループ、「有名ブランドが好き」グループ、「その他全員」グループが含まれる。]({% image_buster /assets/img/audience_path/audience_path2.png %}){: style="float:right;max-width:50%;margin-left:15px;margin-bottom:15px;"}
