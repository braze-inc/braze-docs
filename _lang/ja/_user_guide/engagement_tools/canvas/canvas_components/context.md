...
無効なコンテキスト変数を作成する一般的なシナリオを必ず守ってください。ユーザー パスをプレビューするときに、コンテキスト変数を使用してパーソナライズされた遅延ステップs の結果、およびユーザーs とコンテキスト変数を照合するオーディエンスまたはデシジョンステップの比較を表示できます。
...
バッチ内のすべてのユーザーは、ユーザーが進むまでに処理されます。バッチ処理が完了すると、失敗したユーザーが個別に再試行される間、成功したユーザーsは次回のステップに移動します。成功したユーザーは、再試行が成功するのを待たずに進みます。
