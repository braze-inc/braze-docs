...
> キャンバス条件分岐コンポーネントは、ユーザーにパーソナライズされたリアルタイム体験を提供することを可能にする。

![プッシュ通知をイネーブルメントしていないユーザーとイネーブルメントしているユーザー向けの「プッシュ通知をイネーブルメントするか？」という条件分岐ステップ。]({% image_buster /assets/img/decision-split-1.png %}){: style="float:right;max-width:40%;margin-left:15px;margin-top:15px;margin-bottom:15px;"}
...
![「フォアグラウンドプッシュイネーブルメントが真」というフィルターを選択した条件分岐ステップ。]({% image_buster /assets/img/define-split-2.png %})
...
![プッシュが有効になっていないユーザーと、プッシュが有効になっているユーザーを分ける「Push enabled?」という名前の条件分岐ステップ。プッシュが有効になっていないユーザーの場合、3 日間の遅延が発生し、その後メールメッセージが送信される。プッシュ通知を有効にしているユーザーは、1日遅れて通知を受け取る。その後、さらに2日遅れてプッシュ通知が届き、最終的にプッシュ通知を無効にしているユーザーと同じメールメッセージを受け取る。]({% image_buster /assets/img/use-split-onboarding-3.png %}){: style="max-width:60%"}
