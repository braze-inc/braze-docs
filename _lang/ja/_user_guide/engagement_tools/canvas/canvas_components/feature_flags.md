...
]({% image_buster /assets/img/feature_flags/feature_flag_canvas_step.png %})ライブチャットボタン機能のフィーチャーフラグステップの例。{: style="float:right;max-width:40%;margin-left:15px;"}()
...
]({% image_buster /assets/img/feature_flags/feature_flags_canvas_details.png %})プロパティが「文字列」、プロパティキーが「url」のフィーチャーフラグ [ユーザー設定センター]。{: style="max-width:90%"}()
