...
| クールダウン | ユーザーは、個々のジオフェンスで出入りを移行した後、ジオフェンスからトリガーされる通知を受け取ります。移行が発生した後、そのユーザーが個々のジオフェンスで同じ移行を繰り返さない期間があらかじめ定義されています。この時間は「クールダウン」と呼ばれ、Brazeによってあらかじめ設定されている。デフォルトでは、クールダウンは6 時間です。クールダウンの主な目的は、不要なネットワーク要求を防止することです。 |
...
ジオフェンスは、`FeatureCollection` タイプの GeoJSON オブジェクトとして一括アップロードできます。各ジオフェンスはフィーチャ コレクション内の `Point` タイプのジオメトリです。それぞれの機能のプロパティーには、`radius` キーと、ジオフェンスごとにオプションの`name` キーが必要です。 
...
- アップロード ed の可能性のある最高ジオフェンス半径は10,000 メートル(約10 キロメートルまたは6.2 マイル) です。
...
典型的な精度は20～50mの範囲であり、最良の精度は5～10mの範囲である。地方では、精度が大幅に低下し、数キロメートルに達する可能性があります。Braze は、地方では半径の大きいジオフェンスを作成することを推奨しています。
...
ベストプラクティスとして、重複するジオフェンスs の設定は避けてください。これにより、通知s のトリガーに問題が発生する可能性があります。
