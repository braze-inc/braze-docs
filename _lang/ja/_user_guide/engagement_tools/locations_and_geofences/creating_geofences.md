...
]({% image_buster /assets/img/geofence_example.png %})「EastCoastGreaterNY」と「WesternRegion」の 2 つのジオフェンスが地図上に 2 つの円で描画されたジオフェンスセット。()

## ジオフェンスの一括アップロード {#creating-geofence-sets-via-bulk-upload}

ジオフェンスは、`FeatureCollection` タイプの GeoJSON オブジェクトとして一括アップロードできます。各ジオフェンスはフィーチャ コレクション内の `Point` タイプのジオメトリです。各機能のプロパティには、 キーと、各ジオフェンスのオプションの  キーが必要です。 
...
]({% image_buster /assets/img_archive/action_based_geofence_trigger.png %})ジオフェンスによるアクションベースのキャンペーンで、ユーザーがドイツの空港に入国したときにトリガーされる。()
