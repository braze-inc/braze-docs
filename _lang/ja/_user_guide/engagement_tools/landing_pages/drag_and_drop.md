...
{% alert important %}
ランディングページは現在早期アクセス中である。ランディングページは1社につき5ページまで。
{% endalert %}
...
様々なフォームブロックコンポーネントを使用して、カスタムおよび標準プロファイル属性やカスタムイベントを記録する。入力フィールドフォームブロックは、ユーザーの標準属性とカスタム属性の両方を記録することができ、電話キャプチャとメールキャプチャフォームブロックは、ユーザーのフォーム送信の電話フィールドとメールフィールドをキャプチャすることができる。ボタンのアクションは、カスタム属性、カスタムイベント、またはその両方として、フォーム送信時に記録することができる。 
...
ユーザーの統合は、将来的にLiquidタグによって自動的に処理されるようになるだろう。 
...
### ランディングページのロードマップにはどのような機能があるのか？ {#roadmap}

追加機能は開発中である。これらには以下が含まれる：
