---
nav_title: ランディングページを作成する
article_title: ランディングページを作成する
...
![Brazeダッシュボードのランディングページセクション。]({% image_buster /assets/img/landing_pages/landing-pages-homepage.png %})
...
![ドラッグ＆ドロップエディタで作成中のランディングページの例だ。]({% image_buster /assets/img/landing_pages/template.png %})

編集ツールはランディングページの構成に、基本ブロックとフォームブロックの2種類のコンポーネントを使用する。すべてのブロックは行に配置する必要があります。

![「ビルド」セクションには「行」と「フォームブロック」が含まれる。]({% image_buster /assets/img/landing_pages/dnd.png %}){: style="max-width:35%;"}
...
![「ページコンテナ」セクションには、バックグラウンド画像、色、枠線の詳細、コンテンツのスタイル設定をカスタマイズするオプションがある。]({% image_buster /assets/img/landing_pages/page_container.png %}){: style="max-width:40%;"}

#### ユーザーデバイスに対応するレスポンシブデザイン

ランディングページは、小さい画面では列を縦に積み重ねることで、ユーザーの端末サイズに応じてレスポンシブに対応できる。これをイネーブルメントするには、レスポンシブにしたい行に列を追加し、**列のカスタマイズ**セクションで「**小さい画面では縦に積み重ねる**」をオンにする。

イネーブルメントすると、小さな画面で複数列のコンテンツの縦方向の順序をコントロールするために、列の積み重ねを逆転させることもできる。これにより、カスタムコードなしでモバイルでのページの見た目と使い心地が向上する。

![「列のカスタマイズ」セクションにある「小さい画面では縦に積み重ねる」トグル。]({% image_buster /assets/img/landing_pages/device_responsive_toggle.png %}){: style="max-width:50%;"}
...
![「必須入力項目」のトグルが選択されたチェックボックスのフォームフィールド。]({% image_buster /assets/img/landing_pages/lp-optional-required.png %}){: style="max-width:50%;"}
...
{% alert note %}
確認ページが新しいタブで開かれる場合、元のランディングページに戻って更新情報を再送信したユーザーは、以前の送信内容を上書きしてしまう。その結果、データの整合性が損なわれる。
{% endalert %}

{% alert important %}
ボタンに「**ボタンクリック時にフォームを送信**」を設定し、**クリック時の動作**でWebURLを新しいタブで開封するようにした場合、iOS Safariの組み込みポップアップブロッカーが動作を妨げる可能性がある。これは、フォーム送信後に新しいタブを開く操作がポップアップとして扱われるためだ。<br><br>この問題を避けるには、フォーム送信ボタンを設定して、結果のURLを新しいタブではなく同じタブで開封するようにする。フォーム送信を伴わないボタンは、問題なく新しいタブでURLを開封することができる。
{% endalert %}
...
![メニューが開封された状態のランディングページで、「URLをコピー」オプションが表示されている。]({% image_buster /assets/img/landing_pages/copy-url.png %})
...
![ランディングページを保存、変更、削除するオプション付きのドロップダウンメニュー。]({% image_buster /assets/img/landing_pages/manage-lp-template.png %}){: style="max-width:60%;"}
