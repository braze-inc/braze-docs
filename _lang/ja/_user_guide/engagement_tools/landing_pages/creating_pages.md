...
{% alert important %}
ボタンをクリックしたときに**送信フォームを設定し、**On-click Behavior**を使用して新しいタブにWeb URLを開封すると、iOS Safariの内蔵ポップアップブロックがナビゲーションの動作を妨げることがあります。これは、フォーム送信の後に新しいタブを開封すると、ポップアップとして扱われるためです。<br><br>この問題を回避するには、フォーム送信を使用してボタンを設定し、結果のURL を同じタブに開封します(新しいタブではありません)。フォーム送信のないボタンは、問題なく新しいタブにURLを開封できます。
{% endalert %}
