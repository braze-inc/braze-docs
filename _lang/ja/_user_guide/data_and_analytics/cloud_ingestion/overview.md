...
Cloud Data Ingestion のデータポイント課金は、[`/users/track`エンドポイント]({{site.baseurl}}/api/endpoints/user_data/post_user_track#user-track) を介した更新の課金と同等です。詳細については、「[データポイント]({{site.baseurl}}/user_guide/data_and_analytics/data_points/)」を参照してください。 
...
| `purchases` | [購入オブジェクト]({{site.baseurl}}/api/objects_filters/purchase_object/)を参照 |
...
クエリは最適なパフォーマンスを得るために1時間以内に完了し、潜在的なエラーを回避することをお勧めします。クエリがこの時間枠を超える場合は、データウェアハウスの構成を見直すことを検討してください。倉庫に割り当てられたリソースを最適化することで、クエリ実行速度を向上させることができます。 

## 製品の制限事項

| 制限事項            | 説明                                                                                                                                                                        |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 統合の数 | 統合の設定数に制限はありません。ただし、テーブルまたはビューごとに1つの統合しか設定できません。                                             |
...
| ペイロードサイズ           | 各行には最大1 MBのペイロードを含めることができます。ペイロードが1MBを超えると拒否され、エラー「ペイロードが1MBを超えました」が同期ログに関連するexternal IDおよび切り捨てられたペイロードと共に記録されます。 |
| データタイプ              | Cloud Data Ingestionを通じて、ユーザー属性、イベント、および購入を同期できます。                                                                                                  |
| Braze地域           | この製品はすべてのBrazeリージョンで利用可能です。任意のBrazeリージョンは、任意のソースデータリージョンに接続できます。                                                                              |
| ソース地域       | Brazeは、任意のリージョンまたはクラウドプロバイダーのデータウェアハウスまたはクラウド環境に接続します。                                                                                        |
