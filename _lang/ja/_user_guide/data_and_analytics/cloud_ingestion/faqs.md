...
page_order: 100
page_type: FAQ
description: "この記事には、クラウドデータ取り込みに関してよくある質問と、その回答を記載しています。"
toc_headers: h2
---

# よくある質問（FAQ）

> 以下に、クラウドデータ取り込みに関してよくある質問と、その回答を記載しています。
...
CDI の認証情報が正しくないか、データウェアハウスの設定が正しくない可能性があります。詳細については、[データウェアハウスの連携]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/integrations/)を参照してください。
...
統合で設定したカタログがBrazeカタログに存在しない。カタログは、連携の設定後に削除できます。この問題を解決するには、別のカタログを使用するように統合を更新するか、統合のカタログ名と一致する新しいカタログを作成する。
...
IP許可リストにBrazeの公式IPを追加してみる。詳細については、[データウェアハウスの連携]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/integrations/)を参照してください。
