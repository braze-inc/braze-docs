...
カスタムイベントプロパティは Braze プロファイルに保存されないため、データポイントを消費しません(例外については[データポイント](#data-points)を参照)。
...
![過去30日間 (720〜744時間) の間にプロパティー「アイテム数」が「2つ」以上の放棄カートが1回以上発生したというユーザーを対象にするフィルターグループ。][3]
