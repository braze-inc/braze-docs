...
{% alert note %}
Brazeのダッシュボードでトラッキングをアンインストールするには、オプトインする必要がある。この機能は現在、iOS、Android、Fire OSのアプリで利用できる。
{% endalert %}
...
アンインストールのトラッキングは、**App Settings**ページの**Settingsの**下にある、トラッキングしたいアプリごとにオンにすることができる。

{% alert note %}
古いナビゲーションを使用している場合、**App Settingsは** **Settings**であり、**Manage Settingsの**下にある。
{% endalert %}
...
iOSアプリケーションのアンインストールトラッキングを設定するには、\[utility method]\[iOS docs].Androidアプリケーションの場合は、\[`BrazeNotificationUtils.isUninstallTrackingPush()`][8].Brazeがアンインストールを検出した場合、アンインストール追跡、または通常のプッシュキャンペーン配信のいずれであっても、ユーザーにおけるアンインストールの最良推定時間を記録する。この時間は、標準属性としてユーザープロファイルに保存され、ウィンバックキャンペーンのためのユーザーセグメントを定義するために使用することができる。

## アンインストールによるセグメントのフィルタリング

**セグメント**ページの**アンインストールフィルターは**、時間範囲内にアプリをアンインストールしたユーザーを選択する。アンインストールの正確な時刻を特定することは難しいため、アンインストールフィルタは、アンインストールするすべての人がある時点でセグメントに該当するように、より広い時間範囲を持つことを推奨する。

![セグメントをアンインストールする][5]

### アプリレベルの分析

アンインストールに関する日々の統計は**ホームページに**掲載されている。このグラフは、Brazeが提供する他の統計と同様に、アプリやセグメントごとに分類することができる。**パフォーマンス概要**セクションで、日付範囲と、必要であればアプリを選択する。そして、「**パフォーマンス・オーバー・タイム」**グラフまでスクロールダウンし、以下のようにする：

1. **Statistics For]**ドロップダウンで、\[**Uninstalls]**を選択する。
2. **Breakdown "**ドロップダウンで、"**By segment**"を選択する。
...
![キャンペーンレベルをアンインストールする][6]

### その仕組み

Braze は、ユーザーのデバイスに送信されたプッシュメッセージが Firebase Cloud Messaging (FCM) または Apple プッシュ通知サービス (APN) から、アプリがもうインストールされていないというシグナルを返すタイミングを観察することで、アンインストールを追跡します。特定のアプリでグローバルアンインストールトラッキングがオンになっている場合、ユーザーがアンインストールしたかどうかを検出するために、毎日サイレントプッシュメッセージを送信する。この "サイレント "プッシュは、（ユーザーがアプリの設定でサイレントプッシュを無効にしていない限り）すべてのユーザーに送信されるが、プッシュはユーザーには表示されない。ユーザーがアンインストールしたことを検出した場合、我々はアンインストールする：

* アプリの総アンインストール数を1増やす。
* 過去24時間以内にユーザーが受信に成功したすべてのキャンペーンのアンインストールカウントを1増やす。
* あるユーザーが24時間以内に3つのキャンペーンを受け、その後アンインストールした場合、3つのキャンペーンすべてについて「アンインストール」のカウントをインクリメントする。

アンインストール追跡には、FCM と APN によるこの情報に関する制限が適用されます。Brazeは、FCMまたはAPNからユーザーがアンインストールしたことが通知された場合にのみアンインストールカウントを増加させるが、これらのサードパーティシステムは、任意の時点でアンインストールを通知する権利を有する。その結果、アンインストールの追跡は、正確な統計とは対照的に、方向性の傾向を検出するために使用されるべきである。
