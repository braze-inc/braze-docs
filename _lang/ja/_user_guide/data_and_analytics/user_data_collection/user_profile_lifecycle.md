...
1) **匿名ユーザーが新たに特定ユーザーとなる：**<br>`external_id` が Braze にまだ存在しない場合、匿名ユーザーは識別された新規ユーザーになり、その匿名ユーザーと同じ属性と履歴をすべて保持します。 

2) **匿名ユーザーは、すでに存在するユーザーとして識別される：**<br>`external_id` が Braze に既に存在する場合、このユーザーは以前、タブレットなどの別のデバイスやインポートされたユーザーデータなどを介して、何らかの方法でシステム内のユーザーとして識別されています。 
...
`external_id` とは異なり、エイリアスは、[User Data endpoints][32] ]を介して、またはSDKを介して新しい名前を渡すことによって、設定された後、与えられたラベルの新しい名前で更新することができる。これにより、そのユーザーデータをエクスポートするときに、ユーザーエイリアスが表示されます。

![ユーザーエイリアスのラベルは同じだが、エイリアス名が異なる別々のユーザーに対して、2つの異なるユーザープロファイルを作成する。][29]

ユーザーエイリアスを使用すると、匿名ユーザーに識別子のタグを付けることもできます。例えば、ユーザーがあなたのeコマース・サイトにメールアドレスを提供したが、まだサインアップしていない場合、そのメールアドレスを匿名ユーザーのエイリアスとして使用することができる。このようなユーザーは、エイリアスを使用してエクスポートしたり、API で参照したりできます。
...
![Brazeにおけるユーザープロファイルのライフサイクルのフローチャート。匿名ユーザーについて changeUser () が呼び出されると、そのユーザーは識別されたユーザーになり、そのデータは識別されたユーザープロファイルに移行されます。識別されたユーザーは Braze ID と external ID を持ちます。この時点で、2 人目の匿名ユーザーに changeUser() を呼び出すと、その識別されたユーザーにまだ存在しないユーザーデータのフィールドがマージされます。識別されたユーザーの既存のユーザープロファイルにエイリアスが追加されている場合、データは影響を受けませんが、エイリアスを持つ識別ユーザーになります。Identifiedユーザーと同じエイリアスラベルを持ち、異なるエイリアス名を持つ第3の匿名ユー ザーがchangeUser()を呼び出した場合、Identifiedユーザーに存在しないフィールドはすべて統 合され、Identifiedユーザープロファイルのエイリアスラベルは維持される。][26]
...
あなたは、\[User Data endpoints][27]]を使用して、私たちのSDKと私たちのAPIを介して、識別された既存のユーザープロファイルに新しいユーザーエイリアスを設定することができる。ただし、既存の未知のユーザープロファイルには、API を使用してユーザーエイリアスを設定することはできません。
...
テストとして、ダッシュボードですべてのユーザー ID を使用して、ユーザーの検索および識別ができます。Brazeダッシュボードでユーザーを見つけるには、\[テストユーザーの追加][28] を参照のこと。
