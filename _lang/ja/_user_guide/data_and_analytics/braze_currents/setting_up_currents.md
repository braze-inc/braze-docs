...
| パートナーAPIキーまたはトークン | 通常はパートナーのダッシュボードにあります。 | それをコピーして指定のBrazeフィールドに貼り付けるだけだ。 | Braze には、パートナーの「連携」ページに、このための指定フィールドがあります。これは、データの送信先をマッピングするために必要です。**パートナーのキーまたはトークンを最新の状態に維持することが重要です。認証情報が無効の場合、コネクターが無効になり、イベントがドロップする可能性があります。**
