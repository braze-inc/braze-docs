...
5. **次へ」を**選択する。<br>![][9]
6. DNSにTXTレコードが追加されたら、Google Postmaster Toolsのダッシュボードに戻り、**Verifyを**選択する。このステップでは、ドメインの所有者であることが確認されるため、Postmaster アカウントで Gmail の配信到達性指標にアクセスできるようになります。<br> ![][10]
...
配信可能性センターを設定する前に、ドメインが\[Gmail Postmaster Tools][4] に追加されていることを確認する。
...
| ミディアム/フェア | ポジティブなエンゲージメントを生み出すことで知られているが、時折スパムからの苦情も受ける。このドメインからのメールは、スパムの苦情が増えた場合を除き、ほとんどが受信トレイに送られる。 |
| 低 | 定期的にスパムの苦情が多いことで知られている。この送信者からのメールは、スパムフォルダに振り分けられる可能性が高い。 |
| 悪い | スパムの苦情が多い。このドメインからのメールは、ほとんどの場合、接続時に拒否されるか、スパムフォルダに振り分けられる。 |
...
| 評判評価 | 定義 |
| ----- | ---------- |
| 高 | スパムの苦情が非常に少ないという実績がある。Gmail の送信者ガイドラインに準拠しています。フィルターによりメールがスパムフォルダーに入れられることはほとんどありません。スパム率が非常に低いという優れた追跡レコードがあります。[Gmailの送信者ガイドラインに][2]準拠する。 |
| ミディアム/フェア | 肯定的なエンゲージメントを生み出すことで知られているが、時折、低レベルのスパム苦情を受けている。このドメインからのメールのほとんどは受信トレイに届く（スパムレベルが著しく上昇した場合を除く）。 |
...
| グラフの種類 | 定義 |
| ----- | ---------- |
| 使用済み燃料プール | SPFを試みたドメインからの全メールに対して、SPFを通過したメールの割合を示す。これは、なりすましメールを除く。 |
...
配信性を向上させるためのアイデアについては、\[配信性の落とし穴とスパムの罠][6] を参照のこと。メールキャンペーンを送る前にチェックすべき点については、【メールのベストプラクティス][7] 】を必ず参照しよう。
...
| 結果: | 定義 |
| ----- | ---------- |
| グリーン | マイクロソフトのスパムフィルターにより、指定された時間枠の10％までスパムと判定される。 |
| イエロー | マイクロソフトのスパムフィルターによって、指定された時間枠の10％から90％の間でスパムと判定された。 |
| レッド | マイクロソフトのスパムフィルターにより、指定された時間枠の90％以上がスパムと判定された。| 
{: .reset-td-br-1 .reset-td-br-2}
...
| ----- | ---------- |
| 0.3%未満 | 理想的なクレーム率 |
| 0.3%以上 | 登録プロセスを見直し、登録解除リンクが機能していることを確認する。また、メールを読者に合わせてもっとパーソナライズできないか考えてみよう。 |
...
{% alert tip %}
Brazeで検証済みのドメインに関連するレコードを探している場合、Deliverability CenterにはGoogle PostmasterまたはMicrosoft SNDSのデータがリストアップされている。あるいは、一貫したメール配信を維持することも、高い評価につながるのでお勧めする。
{% endalert %}
