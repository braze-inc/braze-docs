...
最近編集または作成したファイルに直接アクセスして、Braze ダッシュボードのどこに残したかを選択できます。このセクションは、Braze ダッシュボードの [**ホーム**] ページの上部に表示されます。
...
![キャンバスドラフト、アクティブなセグメント、および&quot でのキャンペーンドラフト;オフ" セクションを終了した場所を選択します。][6]
...
*ユーザー数*とは、そのワークスペースで作成されたユーザーの総数です。これには、任意の時点でアプリまたはウェブサイトを使用して記録したすべてのユーザーと、特定のアプリまたはウェブサイトに関連付けられていないユーザーが含まれます。この数は、*Monthly Active Users* (MAU) として表示されるライフタイムユーザーの数の割合です。これは、長期間にわたるユーザーの保持を確認するのに役立ちます。

MAUとユーザーの比率が低いことは、メッセージングチャンネルを多様化したり、失効しているユーザーに手を差し伸べる努力を増やしたりする必要があることを示している可能性があります。詳細については、[離脱ユーザーの収集]][3] に関するクイックウィンを参照してください。一般的に、MAU対寿命比は、ユーザのチャーンのために、経時的に必然的に減少するが、Brazeツールは、ユーザをより長く関与させ続けることにより、この効果を最小化するのに役立つ。

### 生涯セッション数

*Lifetime sessions*は、統合以降にBrazeが記録したセッションの合計数です。簡潔に言うと、セッションとはユーザーによるアプリの使用や、Web サイトの表示が行われている個々の期間です。プラットフォームでセッションがどのように定義されているかをより正確に定義するには、対応するセッションを表示します
...
{% alert note %}
匿名ユーザもMAU にカウントされます。モバイルデバイスの場合、匿名ユーザーはデバイスによって決まります。Web ユーザーの場合、匿名ユーザーはブラウザーのキャッシュによって決まります。
{% endalert %}

### 1 日あたりのアクティブユーザー数

*1 日あたりのアクティブユーザー数* (DAU) には、特定の日にアプリやサイトで少なくとも 1 回のセッションが記録されたユニークユーザーの数が表示されます。DAU は、アプリやサイトの日々の利用状況の変動を調べたり、メッセージングキャンペーンを可能な限り効果的に調整したりするときに有用な統計値です。たとえば、週末にアプリの使用量が大幅に増加することがあります。これにより、最近のアプリ内メッセージでは、平日ではなく、より多くのユーザに到達できることが通知されます。
...
{% alert note %}
Braze を最初に統合すると、Braze は今までセッションを録画したことがないため、すべてのユーザーが新しいユーザーのように見えます。
{% endalert %}
...
例えば、日付範囲を 30 日間に設定した場合、50% の割合は平均して 1 人のアクティブユーザーが 30 日のうち 15 日アプリや Web サイトを利用している、またはアクティブユーザーの約半数が毎日戻ってきていることを示します。スティッキネスは、大半のユーザーが積極的に嫌いなのでアプリをやめるのではなく、日常のルーチンの一部になっていないので、成功のための重要な測定基準である。したがって、ユーザーへのエンゲージメントのレベルを示す別の指標として、スティッキネスを使用できます。
...
*MAU毎の毎日のセッション*は、所定の日の*毎日のセッション*とMAUの比率です。この統計値は、MAU の 1 人について記録されたと見なすことができる 1 日あたりのセッション数を示します。集計および平均化すると、ユーザーがアプリまたはサイトを使用する相対的な頻度を把握できます。つまり、*MAU ごとの 1 日あたりのセッション数*の平均が 0.5 だとすると、MAU のそれぞれについて約 2 日ごとにセッションが記録されると見なすことができます。  
