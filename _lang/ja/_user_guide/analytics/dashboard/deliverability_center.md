...
配信センターにアクセスするには、「キャンペーン、キャンバス、カード、セグメンテーション、メディアライブラリーへのアクセス」と「ユーザーデータの表示」の[ユーザー]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/)権限が必要である。

## Google Postmaster アカウントの設定

配信センターに接続するには、Google Postmaster Tools のアカウントを設定する必要があります。グーグルポストマスターの設定には、仕事用または個人用のGmailアカウントを使用できる。 
...
3. ルートドメインまたはサブドメインを入力してメールを認証する。ルートドメインを追加して検証している場合、これでサブドメインにも検証を適用できる。たとえば、`braze.com` を検証することで、`demo.braze.com` や他のサブドメインを個別に検証することなく、後から追加することができる。
...
{: start="4"}
4. Google広告は、あなたのドメインのDNSに直接追加できるTXTレコードを生成する。これは通常、DNS の管理者が所有します。特定の DNS の更新方法に関する情報とガイダンスについては、「[ドメインの所有権を証明する （ドメインホスト別の手順）](https://support.google.com/a/topic/1409901)」を参照してください。
5. [**次へ**] を選択します。<br>![メールを認証するドメイン "demo.braze.com" の例。]({% image_buster /assets/img_archive/domain_authentication.png %})
6. TXT レコードが DNS に追加されたら、Google Postmaster Tools ダッシュボードに戻り、[**検証**] を選択します。このステップでは、あなたがドメインを所有していることを確認し、ポストマスターアカウントでGmailの配信可能メトリックスにアクセスできるようにする。<br> ![ドメイン"demo.braze.com"の所有権を確認するプロンプトが表示される。]({% image_buster /assets/img_archive/domain_verification.png %})
...
認証されたドメインがDeliverability Centerに表示される。 
...
| 低 | 定期的にスパムの苦情が多いことで知られている。この送信者からのメールはスパムフォルダにフィルターされる可能性が高い。 |
| 悪い | スパムに関する苦情を高率で受信した履歴があります。このドメインからのメールは、ほとんどの場合、接続時に拒否されるか、スパムフォルダにフィルターされる。 |
...
| 評判評価 | 定義 |
| ----- | ---------- |
| 高 | スパムに関する苦情が非常に少ないという優れた実績があります。Gmail の送信者ガイドラインに準拠しています。メールがスパムフォルダにフィルターされることはほとんどない。スパム率が非常に低いという優れた追跡レコードがあります。[Gmail の送信者ガイドライン](https://developers.google.com/gmail/markup/registering-with-google)に準拠しています。 |
| 中/フェア | 積極的なエンゲージメントを生み出すことで知られているが、時折、スパムによる苦情が寄せられることもある。このドメインからのメールのほとんどは受信トレイに届く（スパムレベルが著しく上昇した場合を除く）。 |
| 低 | 定期的にスパムの苦情を受けることで知られている。この送信者からのメールはスパムフォルダにフィルターされる可能性が高い。 |
| 悪い | スパムに関する苦情を高率で受信した履歴があります。このドメインからのメールは、ほとんどの場合、接続時に拒否されるか、スパムフォルダにフィルターされる。 |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
| ----- | ---------- |
| 0.3%未満 | 理想的な苦情率 |
| 0.3%以上 | サインアッププロセスを確認し、配信停止リンクが機能していることを確認してください。また、オーディエンスにパーソナライズされたメールにできないかも検討する。 |
...
{% alert tip %}
検証済みのドメインに関連するレコードをBrazeで探している場合、Deliverability CenterにはGoogle PostmasterまたはMicrosoft SNDSのデータがリストアップされている。あるいは、一貫したメール配信を維持することも、高い評価につながるのでお勧めする。
{% endalert %}
