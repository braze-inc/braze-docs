...
6. TXT レコードが DNS に追加されたら、Google Postmaster Tools ダッシュボードに戻り、[**検証**] を選択します。このステップでは、ドメインの所有者であることが確認されるため、Postmaster アカウントで Gmail の配信到達性指標にアクセスできるようになります。<br> ![ドメイン「demo.braze.com」の所有権を検証するプロンプト。]({% image_buster /assets/img_archive/domain_verification.png %})
...
2. [**配信センター**] タブを選択します。<br>![Google Postmaster が接続されていない配信センター]({% image_buster /assets/img_archive/deliverability_center1.png %})
...
![Google Postmaster の検証済みのドメインが 2 つあり、レピュテーションは「中」と「低」である。]({% image_buster /assets/img_archive/deliverability_center2.png %})
...
![Microsoft SNDSの結果の例。サンプル IP、受信者、RCPT コマンド、データコマンド、フィルターの結果、苦情率、トラップメッセージ期間の開始と終了、スパムトラップのヒット数が含まれている。]({% image_buster /assets/img_archive/deliverability_center_msnds.png %})
