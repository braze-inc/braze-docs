...
]({% image_buster /assets/img/date_select_retention.png %})レポート日付{: style="float:right;max-width:30%;margin-left:15px;"}()
...
]({% image_buster /assets/img/retention_1.png %})リテンションイベントの選択{: style="max-width:80%"}()

### ステップ 3: レポートの生成

リテンションイベントを選択したら、[**レポートを実行**] を選択してクエリを開始します。

]({% image_buster /assets/img/retention_2.png %})レポートを実行{: style="max-width:80%"}()
...
]({% image_buster /assets/img/campaign_retention3.png %})フルレポート()

{% endtab %}
{% tab Canvas %}

]({% image_buster /assets/img/canvas_retention_report.png %})フルレポート{: style="max-width:70%"}()
...
]({% image_buster /assets/img/campaign_retention3.png %})ローリングレポート()
...
]({% image_buster /assets/img/range_retention.png %})リテンションレポート()
...
]({% image_buster /assets/img/variant_view.png %})Variant による表示()

{% endtab %}
{% tab Canvas %}

]({% image_buster /assets/img/variant_view_canvas.png %})Variant による表示()
...
- **信頼度**: Braze は、Z 検定と呼ばれる統計手順を使用して、各バリアントのコンバージョン率とコントロールのコンバージョン率を比較し、信頼度を計算します。
- **単位**: チャートの右上隅で、ユーザーの割合とユーザー数の単位を切り替えることができます。キャンペーンやキャンバスの影響を判断するときに、特定の単位がより重要である場合があります。
