...
削除されたメッセージバリアントの統計は、レポートをキャンペーンまたはキャンバス別に分類しても表示されません。ただし、チャネルレベルの合計には、バリアントが削除されたかどうかに関係なく、すべての統計が含まれます。たとえば、メールの_Sends_ にはすべてのメール送信が含まれますが、これらの統計をキャンペーン別に分類すると、削除されたメッセージバリアントの送信がフィルター除外されるため、番号が小さくなることがあります。
...
- **メールを送信またはスケジュールする:**1 時間後に期限切れになるダウン読み込むリンクを含むメールをすぐに、または指定した時刻に送信します。**メール 受信者 s** ドロップダウンにリストされている企業ユーザーs から受信者s を選択するか、他のメール住所を入力できます。
