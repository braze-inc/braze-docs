...
![セグメント内訳ページには、SQLエディタ、変数、使用可能なデータテーブル、クエリ履歴、AIクエリビルダのタブを含むサイドパネル、および結果セクションがあります。]({% image_buster /assets/img_archive/segment_breakdown.png %})
...
![レポートには、受領時の一意の購入、受領時の収益、クリック時の一意の購入、クリック時の収益、一意のメールの受信者、一意のクリックを含むメールメトリクスが表示されます。]({% image_buster /assets/img_archive/segment_breakdown_results.png %})
