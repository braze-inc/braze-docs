...
Brazeのメッセージングマトリックスは、**コンテンツの複雑**さと**配信の緊急度を**対応させることで、チャネル選択を合理化する。この2つの要素のバランスを取ることで、あなたのメッセージを邪魔するのではなく、響かせることができる。
...
このマトリックスはコアチャネルを強調するものだが、適応可能である：例えば、SMSやWhatsAppは緊急性の高いツールであり、マルチメディア形式を利用することでリッチコンテンツへとスケールアップする。このマトリックスを活用する方法については、[クロスチャネルメッセージングに関する](https://learning.braze.com/cross-channel-messaging)Brazeラーニングコースをご覧ください。
