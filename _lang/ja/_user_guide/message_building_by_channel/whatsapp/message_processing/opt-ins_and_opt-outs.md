...
![アクションベースのキャンバスエントリステップで、WhatsAppの受信メッセージを送信したユーザーを入力する。]({% image_buster /assets/img/whatsapp/whatsapp116.png %}){: style="max-width:85%;"}
...
![WhatsAppの受信メッセージを送信するステップだ。メッセージ本文が「STOP」または「NO MESSAGE」の場合に送信する。]({% image_buster /assets/img/whatsapp/whatsapp117.png %}){: style="max-width:85%;"}
...
![Webhookのステップ。]({% image_buster /assets/img/whatsapp/webhook_step.png %}){: style="max-width:85%;"}
...
| `Matches regex` | `STOP` | その大文字小文字の組み合わせの「STOP」の使用をすべて捕捉する。例えば、これは「STOP」と「PLEASE STOP」を捕捉するが、「stop」は捕捉しない。 |
