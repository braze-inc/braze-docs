---
nav_title: WhatsAppメールを作成する
article_title: WhatsAppメール作成
page_order: 0
...
次に、Segment s またはフィルター s を選択してオーディエンスを絞り込むことによって、[target ユーザー s]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/) を選択する必要があります。すでにサブスクリプショングループを選択しているはずです。これにより、必要な通信の程度やカテゴリによってユーザーが狭くなります。このステップでは、Segments からより大きなオーディエンスを選択し、そのSegmentをフィルターs でさらに絞り込みます。そのアプリの近接Segment集団のスナップショットが自動的に受信されます。完全なSegmentメンバーシップは、メッセージが送信される前に必ず計算されることを忘れないでください。
