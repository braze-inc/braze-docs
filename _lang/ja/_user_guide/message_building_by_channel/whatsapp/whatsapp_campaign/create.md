...
{% alert tip %}
アクションベースのキャンバスがインバウンドWhatsApp メッセージによってトリガーされる場合、次のアクションパスまで、任意のキャンバスステップでWhatsApp プロパティを参照できます。
{% endalert %}
...
クーポンコードメッセージテンプレート | メッセージテンプレートのみで利用可能。<br>これらは、他のメッセージテンプレートと同様に開いたり編集したりすることができ、液体およびろう付けのプロモーションコードと互換性があります。 
