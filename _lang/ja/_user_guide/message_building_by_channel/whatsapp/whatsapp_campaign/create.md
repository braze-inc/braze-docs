...
Meta Business Manager で WhatsApp テンプレートを作成する際に変数を追加した場合、それらはメッセージ作成画面に空白として表示されます。これらの空白をリキッドまたはプレーンテキストに置き換える。プレーン・テキストを使うには、二重中括弧で囲まれた "text here "という書式を使う。テンプレートのビルド時にイメージを含めることを選択した場合は、メディアライブラリからイメージをアップロードまたは追加するか、イメージのURL を参照することができます。可能な場合、一貫性と信頼性を確保するために、画像をメディアライブラリーに直接アップロードすることをお勧めします。
...
![属性"first_name"とデフォルト値「あなた」を持つ「パーソナライゼーションを追加」ツール。]({% image_buster /assets/img/whatsapp/whatsapp7.png %}){: style="max-width:80%;"}

### ダイナミック・リンク 

コールトゥアクション URL は、Meta では `{% raw %}https://example.com/{{variable}}{% endraw %}` のように URL の末尾になければなりませんが、URL に変数を含めることができます。この変数は Braze で Liquid に置き換えることができます。リンクはテンプレートの一部として本文に含めることもできる。これらのリンクは両方とも短縮でき、[クリックトラッキング]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/whatsapp_campaign/click_tracking/)でトラッキングできる。
...
![カスタムユーザー「Max」向けのプレビューメッセージだ。]({% image_buster /assets/img/whatsapp/whatsapp8.png %}){: style="max-width:80%;"}
...
次に、セグメントやフィルターを選択してオーディエンスを絞り込み、[ユーザーをターゲットに]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)設定しなければならない。既にサブスクリプショングループを選択しているはずだ。これは、ユーザーがあなたとどのようなレベルや種類のコミュニケーションを取りたいかによって、ユーザーを絞り込むものだ。このステップでは、セグメントの中からより大きなオーディエンスを選択し、当社のフィルターでそのセグメントをさらに絞り込む。そのおおよそのセグメント人口がどのようなものか、自動的にスナップショットを受け取る。正確なセグメントの所属は、メッセージが送信される前に常に計算されることを覚えておけ。
...
{% alert important %}
Metaには既知の問題があり、特定のエンコードやコンテナ設定が原因で、一部のMP4動画がAndroid端末で再生できないことがある。恒久的な修正が利用可能になるまでは、MP4ファイルを再フォーマットすることで、ほとんどの送信者にとって問題は解決する。すべての動画をAndroid端末でテストし、正常に配信されることを確認せよ。<br><br>MP4ファイルは、[CloudConvert](https://cloudconvert.com/mp4-converter)のようなWebツールを使って再フォーマットできる。MP4ファイルをツールにアップロードし、再度MP4に変換してから、変換されたファイルをダウンロードする。
{% endalert %}
...
| マーケティング・オプトアウト・ボタン | デフォルトでは、サブスクリプションのステータスは自動更新されない。完全な手順については、[オプトイン&とオプトアウトを]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/message_processing/opt-ins_and_opt-outs/#marketing-opt-out-selection)参照せよ。 |
