...
![WhatsAppメッセージ作成画面には、本文とボタンがある。]({% image_buster /assets/img/whatsapp/click_tracking/message_composer.png %})
...
4. CTA ボタンリンクの場合は、**Dynamic** を選択し、ベースURL (`brz.ai` またはカスタムドメイン) を指定します。<br><br>![アクションを促す文を作成するセクション。]({% image_buster /assets/img/whatsapp/click_tracking/create_cta.png %})<br><br>
5. 本文テキストのリンクの場合、WhatsApp Manager でテンプレートを記述するときに、追跡する本文に含まれているリンクの挿入済みスペースをすべて削除します。<br><br>![アクションを促す文面を入力するテキストボックス。]({% image_buster /assets/img/whatsapp/click_tracking/cta_textbox.png %})

#### ステップ 2:Braze でテンプレートを完成させる

作成時に、Braze は、本文とCTA ボタンの両方でサポート可能なURL ドメインを持つテンプレートを自動的に検出します。ステータスはテンプレートの下部に表示されます。 

![「リンク状態」セクションは、クリックトラッキングのステータスが有効な状態を示している。]({% image_buster /assets/img/whatsapp/click_tracking/link_status.png %}){: style="max-width:70%;"}
...
![「ボタン」セクションには、ボタン名、Web サイトURL、トラッキング URL のフィールドがある。]({% image_buster /assets/img/whatsapp/click_tracking/buttons.png %}){: style="max-width:70%;"}

{% alert important %}
**API経由でテンプレートメッセージを送信する**：WhatsAppのクリックトラッキング（カスタムトラッキングドメインとメッセージ作成画面の**クリックトラッキングURL**フィールドを使用）は`brz.ai`、WhatsAppテンプレートメッセージをWhatsApp[`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/)[APIエンドポイント]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/)経由で送信する場合、サポートされていない。

API経由でテンプレートメッセージを送信する場合、CTA URL変数（\`\`を使用`button_variables`）を設定できるが、BrazeはAPIリクエストフロー内でクリック追跡URLやリダイレクトリンクを生成しない。クリックトラッキングを使用するには、Brazeダッシュボードから、またはBrazeキャンペーントリガー経由でテンプレートを送信する。
{% endalert %}
...
![WhatsAppメッセージのパフォーマンス表。]({% image_buster /assets/img/whatsapp/click_tracking/message_performance.png %})

## ユーザーのリターゲティング 

`Clicked/Opened Step` フィルタと`clicked tracked WhatsApp link` インタラクションを使用して、リンクとのインタラクションに基づいてユーザーをセグメント化できます。

![「クリックされたトラッキング済みWhatsAppリンク」のフィルターでグループをフィルタリングする。]({% image_buster /assets/img/whatsapp/click_tracking/filter_group.png %})
