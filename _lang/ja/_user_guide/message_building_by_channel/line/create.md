...
セグメントから大きなオーディエンスを選択し、必要に応じて[フィルター]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/)でさらにセグメントを絞り込みます。そのおおよそのセグメント人口がどのようなものか、自動的にスナップショットを受け取る。正確なセグメントの所属は、メッセージが送信される前に常に計算されることを覚えておけ。
...
まだ済んでいないなら、キャンバスの残りのセクションを完了させろ。キャンバスの残りの構築方法、多変量テストやインテリジェントセレクションなどの詳細については、「[キャンバスを作成する]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/)」を参照せよ。
