...
セグメントから大きなオーディエンスを選択し、必要に応じて[フィルター]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/)でさらにセグメントを絞り込みます。セグメンテーションのおおよその母集団がどのようなものであるかのスナップショットを自動的に受け取ることができる。正確なセグメンテーションは常にメッセージ送信前に計算されることに留意してほしい。
