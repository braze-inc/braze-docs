---
nav_title: "ユーザーのオプトインを収集する"
article_title: 収集ユーザーSMSオプトインのベストプラクティス
page_order: 7
...
ユーザーに「START」、「UNSTOP」、「YES」、またはカスタムオプトインキーワードをあなたの番号にテキストメッセージで送信するように依頼して、サブスクリプショングループに自動的に追加します。Web サイト、モバイルアプリ、または広告で、ユーザーにオプトインを依頼し、役立つ場合はインセンティブを提供することができます。

## オプション 2: ユーザーはアプリ内メッセージでオプトインします
...
Brazeは、[SMSダブルオプトイン]({{site.baseurl}}/user_guide/message_building_by_channel/sms/keywords/sms_double_opt_in/)機能も使用することをお勧めします。この機能は、アプリ内メッセージの電話番号キャプチャフォームと自動的に連携し、ユーザーがフォームを介して電話番号を送信した後に意図を確認するよう促します。

## オプション 3: サインアップフロー

新しいユーザーがWebサイトやアプリにサインアップまたは登録するときに、電話番号とメールを尋ねてください。プロモーションメールとSMSを受信するためのチェックボックスを含めてください。 
