...
{% alert note %}
BrazeAIの[インテリジェントチャネルフィルター]({{site.baseurl}}/user_guide/sage_ai/intelligence/intelligent_channel/)を使用する予定があり、SMSチャネルを選択可能にしたい場合は、高度なトラッキングおよび[クリックトラッキング]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/link_shortening/#click-tracking)を使用してSMSリンクの短縮をオンにしてください。
{% endalert %}
