...
- **高度なトラッキング** は、キャンペーンレベルおよびユーザーレベルのクリックトラッキングを有効にし、クリックに依存するセグメンテーションおよびターゲット変更機能を使用できるようにします。クリックは、Currentsを通じて送信される[SMSクリックイベント]({{site.baseurl}}/user_guide/data/braze_currents/event_glossary/message_engagement_events/)も生成します。高度なトラッキング機能付き静的URLは27～28文字の長さになる。これにより、URLをクリックしたユーザーのセグメントを作成できる。パーソナライズされたURLの長さは32～33文字となる。

リンクは共有の短縮ドメイン（`brz.ai`）を使って短縮される。例として、URL は次のようになります: `https://brz.ai/8jshX` (基本的、静的) または `https://brz.ai/p/8jshX/2dj8d` (高度な、パーソナライゼーション)。詳細については[テスト](#testing)を参照してください。

または `http://`で始まる`https://`静的URLは短縮される。静的な短縮URLは、作成日から1年間有効だ。Liquidパーソナライゼーションを含む短縮URLは、2か月間有効だ。
...
Brazeは、または`http://`で始まるURLのみ`https://`を認識する。URL が認識されると、**プレビュー** セクションがプレースホルダURL で更新されます。Brazeは短縮後のURLの長さを推定するが、より正確な推定のためにはテストユーザーを選択し、メッセージを下書きとして保存するよう警告が表示される。
...
LiquidによってレンダリングされるURLは、APIトリガーのプロパティに含まれているものも短縮されます。例えば、が有効なURLを表す{% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %}場合、メッセージを送信する前にそのURLを短縮しトラッキングする。 

### エンド`/messages/send`ポイントでURLを短縮する
...
このプレビューは、関連するパーソナライゼーションと短縮されたURLで更新される。文字数と[課金対象セグメント]({{site.baseurl}}/user_guide/message_building_by_channel/sms_mms_rcs/segments/)の数も、レンダリングされたパーソナライゼーションと短縮されたURLを反映して更新される。

テストメッセージを送信する前に、必ずキャンペーンまたはキャンバスを保存すること。そうすれば、メッセージ内で送信される短縮URLの表示を確認できる。テスト送信前にキャンペーンやキャンバスが保存されていない場合、テスト送信にはプレースホルダーURLが含まれる。

キャンバスが「クリックされた短縮SMSリンク」フィルターに表示されるためには、短縮リンクを含むキャンバスステップも高度なトラッキングをイネーブルメントする必要がある。これによりユーザー単位のクリック追跡が可能となる。ショートリンクが基本トラッキングで設定されている場合、SMSショートリンクのクリックイベントをフィルターするオプションは利用できない。
...
リンク短縮が有効になっている場合、**SMS/MMS/RCSパフォーマンス**表には「**総クリック数**」という列が含まれる。この列には各バリアントごとのクリックイベント数と、それに対応するクリック率が表示される。メトリックの詳細については、[メッセージパフォーマンス]({{site.baseurl}}/sms_mms_rcs_reporting/)を参照してください。

![SMSとMMSのパフォーマンス・メトリクス表。]({% image_buster /assets/img/link_shortening/shortening4.png %})

**履歴パフォーマンス**と**SMS/MMS/RCSパフォーマンス**の表には**、総クリック**数のオプションも含まれており、クリックイベントの日次時系列データを表示する。クリック数はリダイレクト時（例えば、ユーザーがリンクを訪問したとき）に増加し、ユーザーごとに複数回増加することがあります。
...
リンク短縮はディープリンクでは機能しない。あるいは、BranchやAppsFlyerといったサードパーティ提供のユニバーサルリンクを短縮することもできる。ただし、ユーザーは短いリダイレクトや「ちらつき」効果を経験する可能性がある。これは、短縮リンクがまずWebを経由してから、アプリ起動をサポートするユニバーサルリンクに変換されるためだ。さらに、Brazeはユニバーサルリンクを短縮する際に発生する可能性のある問題、例えばアトリビューションの破損や予期せぬリダイレクトの発生などについては、トラブルシューティングを行えない。

{% alert note %}
ユニバーサルリンクによるリンク短縮を実装する前に、ユーザー体験をテストし、期待通りに動作することを確認せよ。
{% endalert %}
