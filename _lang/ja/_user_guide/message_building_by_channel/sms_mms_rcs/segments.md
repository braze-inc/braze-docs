...
RCSメッセージは、その内容とメッセージが配信される国に基づいて課金される。コストを正確に見積もるには、メッセージングの種類と課金方法を理解することが不可欠だ。

### RCS課金タイプ

当社のプラットフォームは、主に二つの課金モデルをサポートしている。グローバルモデルと米国モデルである。

#### グローバルモデル（米国以外の市場向け）

メッセージは1通ごとに課金され、基本メッセージか単発メッセージのいずれかに分類される。

{% tabs local %}
{% tab Basic %}

基本のRCSメッセージはテキストのみのメッセージで、最大160文字までである。また、1通のメッセージとして課金される。

{% alert note %}
ボタンやその他のリッチ要素を追加すると、メッセージタイプがシングルRCSメッセージに変わる。
{% endalert %}

{% endtab %}
{% tab Single %}

単一のRCSメッセージとは、160文字を超えるメッセージ、またはボタンやメディアなどのリッチ要素を含むメッセージを指す。これらはメッセージの長さに関わらず、単一のメッセージとして課金される。

{% alert note %}
テキストメッセージと別個のメディアファイルを送信しても、依然として2つの別々のメッセージとして課金される。
{% endalert %}
...
メッセージはリッチかリッチメディアのいずれかに分類される。

{% tabs local %}
{% tab Rich messages %}

リッチメッセージとは、ボタン付きまたはボタンのないテキストのみのメッセージである。セグメントごとに課金される。各セグメントは160 UTF-8バイトに制限されており、**つまりセグメントごとの文字数は固定されていない**。160文字の平文メッセージは1つのセグメントだが、より長いテキストや絵文字を含むメッセージは複数のセグメントになる可能性がある。

{% endtab %}
{% tab Rich media messages %}

リッチメディアメッセージには、メディアファイル（画像, 写真）またはリッチカードが含まれ、単一のメッセージとして課金される。
...
メッセージを作成する際、メッセージ作成画面では料金タイプがリアルタイムでラベル（基本RCS、シングルRCS、リッチ、リッチメディア）を通じて表示される。これにより送信前にコストのトラッキングができる。

[メッセージ使用状況ダッシュボード]({{site.baseurl}}/message_usage_dashboard/)にはこれらの課金タイプが反映され、米国向けメッセージで使用されたセグメント数が表示される。これにより、メッセージクレジットの消費状況を明確に把握できる。
