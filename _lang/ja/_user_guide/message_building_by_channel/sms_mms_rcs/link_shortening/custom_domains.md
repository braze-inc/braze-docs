...
Brazeのリンク短縮サービスやクリックトラッキングサービスとの適切なルーティングやインフラの互換性を確保するため、カスタムドメインをBrazeに委任していただく必要がある。ドメインを Braze に委任すると、サービスが失効しないように、証明書の更新が自動的に処理されます。 
...
## サブスクリプショングループにカスタムドメインを割り当てる

カスタムドメインは設定後、1つまたは複数のSMS、RCS、WhatsAppサブスクリプショングループに割り当てることができる。

1. **オーディエンス**>**サブスクリプショングループマネジメントに**進む。
2. リストからサブスクリプショングループを探し、選択する。
3. **サブスクリプショングループの詳細**で、**リンク短縮ドメインとして**カスタムドメインを選択する。
