...
このステップでは、Segments からより大きなオーディエンスを選択し、フィルターs を選択した場合はそのSegmentをさらに絞り込みます。そのアプリの近接Segment集団がどのように見えるかのプレビューを自動的に受け取ります。メッセージが送信されるまでは、必ずSegmentメンバーシップが計算されることに注意してください。
