...
BrazeのSMS作成画面に「Hifirst_name, we appreciate your support！このSMSを見せれば、特別割引が適用されます。メッセージの受信を中止するには STOP と返信してください。」(]({% image_buster /assets/img/sms_campaign_compose.png %}))
...
]({% image_buster /assets/img/ai_copywriter/ai_copywriter_sms.png %})SMS コンポーザーのメッセージフィールドにある [AI コピーライター] ボタンをクリックします。{: style="max-width:60%"}()
