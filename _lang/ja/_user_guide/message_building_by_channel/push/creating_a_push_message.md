...
![「キャンペーンタイプを選択するためのフローチャート。まず、複数のデバイスやプラットフォームをターゲットにしているかどうかを決定します。「いいえ」の場合は、「プッシュ通知を選択」につながります。はい」の場合、「どのようなプッシュメッセージのタイプですか」と尋ねられ、選択肢は「標準的なプッシュ」となり、「デバイス固有の設定を使用する必要がありますか」という判断ポイントにつながる。いいえ」の場合は、「プッシュ通知を選択し、クイックプッシュを使用する」につながる。イエスなら、『マルチチャンネルの選択』に進む。「プッシュメッセージの種類は?」に戻ります。答えが「プッシュストーリーまたはインライン"画像」の場合、「マルチチャネルを選択&クォート」になります。]({% image_buster /assets/img_archive/flowchart_quickpush.png %})
...
![プッシュ通知を作成するコンポーズタブです。]({% image_buster /assets/img_archive/push_compose.png %})
...
![プッシュコンポーザーのボディフィールドにあるAIコピーライターボタンを起動します。]({% image_buster /assets/img/ai_copywriter/ai_copywriter_push.png %}){: style="max-width:60%"}
{% endalert %}
...
次に、Segment s またはフィルター s を選択してオーディエンスを絞り込むことによって、[target ユーザー s]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/) を選択する必要があります。そのアプリの近接Segment集団がどのように見えるかのプレビューを自動的に受け取ります。キャンペーンがターゲットとするチャンネルの詳細なオーディエンス統計は、フッターに表示されます。ユーザー群の何パーセントがターゲットになっているか、およびそのセグメントの生涯価値を確認するには、[**統計をさらに表示する**] を選択します。
...
![到達可能ユーザの詳細なオーディエンス統計のテーブル。]({% image_buster /assets/img_archive/multi_channel_footer.png %})

メッセージが送信されるまでは、必ずSegmentメンバーシップが計算されることに注意してください。
