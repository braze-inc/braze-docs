...
{% alert tip %}
キャンペーンとキャンバスのどちらを使用すべきでしょうか。キャンペーンは単一のターゲットを絞ったメッセージングキャンペーンに適している。一方、キャンバスは複数ステップのユーザーージャーニーに適している。
{% endalert %}
...
![「キャンペーンタイプを選択するためのフローチャート。まず、複数のデバイスやプラットフォームをターゲットにしているかどうかを決定します。「いいえ」の場合は、「プッシュ通知を選択」につながります。はい」の場合、「どのようなプッシュメッセージのタイプですか」と尋ねられ、選択肢は「標準的なプッシュ」となり、「デバイス固有の設定を使用する必要がありますか」という判断ポイントにつながる。いいえ」の場合は、「プッシュ通知を選択し、クイックプッシュを使用する」につながる。イエスなら、『マルチチャンネルの選択』に進む。「プッシュメッセージの種類は？」に戻ると、答えが「プッシュストーリーまたはインライン画像」の場合、「マルチチャネルを選択」へ進む。]({% image_buster /assets/img_archive/flowchart_quickpush.png %})
...
![プッシュコンポーザーの本文フィールドにあるAIコピーライターボタンを起動する。]({% image_buster /assets/img/ai_copywriter/ai_copywriter_push.png %}){: style="max-width:60%"}
{% endalert %}
...
次に、セグメントやフィルターを選択してオーディエンスを絞り込み、[ユーザーをターゲットに]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)設定しなければならない。そのおおよそのセグメント人口がどのようになるか、自動的にプレビューが表示される。キャンペーンがターゲットとするチャンネルの詳細なオーディエンス統計は、フッターに表示されます。ユーザー群の何パーセントがターゲットになっているか、およびそのセグメントの生涯価値を確認するには、[**統計をさらに表示する**] を選択します。
...
正確なセグメントの所属は、メッセージが送信される前に常に計算されることを覚えておけ。
...
キャンペーンまたはキャンバスの最後の部分の作成が完了したら、その詳細を確認してください。キャンペーンの場合、最終ページでは設計したキャンペーンの概要が表示される。関連する詳細をすべて確認し、メッセージをテストしたことをチェックしてから送信し、データが送られてくるのを確認します。
