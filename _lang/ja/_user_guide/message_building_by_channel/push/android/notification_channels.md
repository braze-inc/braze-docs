...
アプリケーションに追加されたチャネルは、必要に応じて削除できます。しかし、消費者は常に、あなたが[削除]したチャネルの数を見ることができる。[3] Brazeダッシュボードには、プログラムでチャネルを作成するサポートは含まれていない。シームレスに体験できるようにするには、アプリケーションのコードでチャネルを作成し、定義する必要がある。
