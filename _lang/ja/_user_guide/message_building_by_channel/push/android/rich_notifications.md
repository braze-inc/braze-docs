...
画像, 写真の前に表示したい**サマリーテキストを**追加する。

![Dogというペットフードアプリからのリッチプッシュ通知で、スポット用のフードを追加注文する時間であることを要約テキストとともに示している。]({% image_buster /assets/img_archive/android_rich_summarytext.png %})

### ステップ 3:メディアを追加する

メッセージ作成画面の**Android Notification Image**フィールドに画像を追加する。画像は、ダッシュボードから直接アップロードすることも、他の場所でホストされているコンテンツ URL を指定してアップロードすることもできます。

サポートしている画像の詳細については、「[画像の仕様]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/media_library/#push)」を参照してください。

![Androidの通知画像セクションでは、画像を追加したり、画像のURLを入力したりできる。]({% image_buster /assets/img_archive/android_rich_image.png %})
