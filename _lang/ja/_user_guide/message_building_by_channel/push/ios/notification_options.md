...
iOSプッシュキャンペーンを作成したら、**設定**タブに移動し、**通知グループ**ドロップダウンを開く。

![「設定」タブにある「通知グループ」ドロップダウンで「クーポン」を選択した。]({% image_buster /assets/img_archive/notification_group_dropdown.png %}){: style="max-width:50%;" }
...
従来のiOSプッシュトークンとは異なり、暫定プッシュトークンは「お試しパス」として機能する。これにより、ブランドは新規ユーザーがAppleのネイティブプッシュオプトインプロンプトを閲覧・クリックする前に、彼らにリーチできる。この機能により、お客様のプッシュ通知は新規ユーザーの通知トレイに直接配信され、今後の通知を「保持」または「オフ」にするオプションが付きます。ユーザーは「オプトイン」ジャーニーを体験する代わりに、「オプトアウト」ジャーニーのような体験をします。
...
![プッシュ配信サブスクライバーステータスの連絡先設定。]({% image_buster /assets/img/profile-push-prov-auth.png %}){: width="50%"}
...
![ユーザーをターゲットにするため、サンプルセグメントフィルター「iOSストップウォッチで仮承認済み（iOS）が真」を設定したセグメント詳細パネル。]({% image_buster /assets/img/segment-push-prov-auth.png %})
...
{% alert important %}
追加のプッシュプロンプトや[アプリ内プッシュプロンプター](https://www.braze.com/resources/glossary/priming-for-push/)（プッシュ通知の受信をユーザーに促すアプリ内メッセージ）を使用する場合は、追加のガイダンスについてBraze担当者にお問い合わせください。
{% endalert %}
...
![iOSの通知設定ページには、即時配信がイネーブルドな通知と、時間制限のある通知がイネーブルドな通知が表示される。]({% image_buster /assets/img/ios/ios15-notification-settings.png %}){: style="max-width:40%"}
...
![関連性スコアは「0.5」だ。]({% image_buster /assets/img/ios/relevance-score.png %}){: style="max-width:80%;"}
