...
{% alert tip %}
**Contact Settings**>**Push Changelogで**エラーメッセージが表示された場合は、[よくあるプッシュエラーメッセージの]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_error_codes/)説明と次のステップを参照すること。
{% endalert %}
...
| **iOS 11またはそれ以前** | いいえ                          | すべてのユーザーは、プッシュ通知の受信を明示的にオプトインしなければなりません。プッシュトークンは、権限が付与された後にのみ生成される。                                     |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
{% endtab %}
{% endtabs %}
...
![前述のBraze、顧客、Appleプッシュ通知サービスまたはFirebase Cloud Messaging間のプッシュプロセスをマッピングしたフローチャート。]({% image_buster /assets/img/push_process.png %})
