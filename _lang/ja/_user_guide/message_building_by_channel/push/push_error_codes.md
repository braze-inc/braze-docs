...
### エラーが発生したため、プッシュ送信に失敗した。

このメッセージは、Appleプッシュ通知サービス（APN）が無効なペイロードを理由にプッシュリクエストを拒否した場合に、ユーザープロファイルの**「エンゲージメント」**タブ（**連絡先設定**＞**プッシュ変更履歴**）に表示されることがある。

Brazeでは、このダッシュボードメッセージは次のAPNエラー理由のいずれかに該当する可能性がある：

- `PayloadEmpty`:送信されたプッシュの種類に必要な内容がペイロードに欠けていた。
- `PayloadTooLarge`:ペイロードがAPNの最大ペイロードサイズを超えた。

一般的な原因には以下が含まれる：

- カスタムキー（とその値）がペイロードを過大にする（これには予想外に大きなLiquidレンダリング値が含まれる場合がある）。
- 必要な箇所で空または欠落したアラートや本文（あるいはその他の不正な`aps`ペイロード）。

次のステップに進む：

- カスタムキーを削除し、大きなダイナミックな値を短縮することでペイロードサイズを削減する。
- API経由で送信する場合、送信前に最終的なJSONペイロード（サイズを含む）を検証せよ。
