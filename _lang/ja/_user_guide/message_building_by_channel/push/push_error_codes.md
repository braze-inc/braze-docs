...
### ペイロードが無効なため、プッシュ送信エラーとなった。

このメッセージは、Appleプッシュ通知サービス（APNs）が無効なペイロードのためにプッシュ要求を拒否したときに、ユーザープロファイルの[**連絡先設定**]>[**プッシュチャンログ]**の[**エンゲージメント]**タブに表示されることがある。

Brazeでは、このダッシュボードメッセージは以下のAPNエラー理由のいずれかに対応付けられる：

- `PayloadEmpty`:ペイロードに、送信されるプッシュのタイプに必要なコンテンツが欠けていた。
- `PayloadTooLarge`:ペイロードがAPNの最大ペイロードサイズを超えた。

一般的な原因には以下のようなものがある：

- 顧客キー(とその値)がペイロードを大きくしすぎている(これには予期せぬ大きさのLiquidレンダリング値が含まれることがある)。
- アラートまたはボディが空である、または欠落している（あるいは`aps` のペイロードが不正である）。

次のステップに進む：

- ペイロードサイズを減らすには、カスタムキーをトリミングし、大きなダイナミックな値を短くする。
- APIを通じて送信する場合は、送信前に最終的なJSONペイロード（サイズを含む）を検証すること。
