...
3. (オプション） ナビゲーションメニューのボタンを選択して、コンテンツブロックの幅を調整します。デフォルトの幅は、メールのグローバルスタイル設定sで指定されていない場合は100%です。指定されていない場合は、グローバル設定sが適用されます。<br><br>![幅を編集するオプションを持つ両面矢印。]({% image_buster /assets/img_archive/content_block_width_updated.png %}){: style="max-width:30%;" }<br><br>

コンテンツブロックをメールエディタに追加した後、**Templates & Media** で作成した元のコンテンツブロックに影響しないコンテンツブロックを編集できます。ドラッグ＆ドロップで追加されたコンテンツブロックは、元のコンテンツブロックにリンクされていないからだ。元のコンテンツブロックに加えた変更を表示するには、メールエディターにもう一度ドラッグします。 
...
{% alert important %}
コンテンツブロックがメール エディタにLiquid で追加されると、このコンテンツブロックは**Templates & Media** で作成された元のコンテンツブロックにリンクされます。つまり、このコンテンツブロックは、元のコンテンツブロックテンプレートへの変更を反映して更新されます。
{% endalert %}
