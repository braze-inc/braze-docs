...
メールメッセージをキャンペーンとキャンバスのどちらで送るべきか迷っている？キャンペーンは単一のターゲットを絞ったメッセージングキャンペーンに適している。一方、キャンバスは複数ステップのユーザーージャーニーに適している。
...
ドラッグ＆ドロップによる編集は3つのセクションに分かれている：**設定**、**コンテンツ**、**プレビュー**の**送信****&テスト**。メール本文の作成は、**コンテンツ**セクションで行われる。Eメールを作成する前に、Eメール作成の指針となる重要な要素を理解することが重要である。レビューが必要な場合は、「[編集者について](#about-the-editor)」を参照してください。
...
![コピーライターボタンは、ドラッグ＆ドロップエディタのコンテンツパネル内にあるスタイル設定の隣に位置している。]({% image_buster /assets/img/ai_copywriter/ai_copywriter_dnd.png %})
{% endalert %}
...
![メールのプレビュー画面に「プレビューリンクをコピー」ボタンを表示し、生成されたリンクをコピーする。]({% image_buster /assets/img/dnd_email_link_preview.png %})

#### 受信トレイビジョンの使用

受信トレイビジョンを使用すると、さまざまなメールクライアントやモバイルデバイスでの表示でメールキャンペーンを確認できます。Inbox Visionでメールメッセージをテストするには、**[プレビュー&とテスト**]セクションで**Inbox Vision**を選択し、[**Inbox Visionを実行]**を選択する。
...
![メールの「言語」設定を行う。]({% image_buster /assets/img/dnd/language_setting_dnd.png %}){: style="max-width:70%;"}
...
メールメッセージングでダイナミックな画像を含めるには、画像ソース属性に[「コネクテッドコンテンツ]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content/about_connected_content/#about-connected-content)」または「[Liquid」]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/)を含めることで選択できる。例えば、静止画像の代わりに、{% raw %} `https://example.com/images/?imageBanner={{first_name}}` {% endraw %} を画像 URL として挿入することで、ユーザーの名を画像に含めることができます。これは、各ユーザーに応じたメールのパーソナライゼーションに役立ちます。

{% alert important %}
画像のURLは必ずで始まる必要がある`https://`。を使用すると、`http://`アプリがクラッシュする。
{% endalert %}
...
タグ`<head>`の外側に追加されたものは、メール内のタグ`<body>`の後に追加される。これは追加された内容がメールに表示されることを意味する。

##### タグごとに許可されたタグと属性

| タグ名 | 説明 | 例 |
| --- | --- | --- |
| `base` | メッセージング内のすべての相対URLのベースURLを指定する。 | `<base href="https://example.com" target="_blank">` |
| `link`| メッセージングと外部リソース間の関係を定義する。 | `<link href="styles.css" rel="stylesheet" type="text/css">` |
| `meta` | ページの説明やキーワードなどのメタデータを提供する。 | `<meta name="description" content="Free Web tutorials">` |
| `style` | 内部のCSSスタイルを埋め込む。 | `<style type="text/css" media="screen">body { font-size: 16px; }</style>` |
| `title` | ブラウザのタブに表示されるドキュメントのタイトルを設定する。 | `<title>StyleRyde</title>` |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

| タグ | 属性 | 説明 | 例 |
| --- | --- | --- | --- |
| `base` | `href` | 相対URLで使用するベースURL。 | ```<base href="https://braze.com">``` |
| `base` | `target`| すべてのハイパーリンクとフォームのデフォルトのターゲット。 | ```<base target="_blank">``` |
| `link` | `href` | 外部リソースへのURL。 | ```<link href="style.css">``` |
| `link` | `rel` | 現在のメッセージとリンクされたメッセージ間の関係を定義する。 | ```<link rel="stylesheet">``` |
| `link` | `type` | リンクされたリソースの種類。 | ```<link type="text/css">``` |
| `link` | `sizes` | アイコンのサイズを指定する。 | ```<link rel="icon" sizes="32x32" href="favicon-32.png">``` |
| `link` | `media` | スタイルが適用されるメディアまたはデバイスを指定する。 | ```<link rel="stylesheet" media="screen" href="style.css">``` |
| `meta` | `name` | ブラウザのタブに表示されるドキュメントのタイトルを設定する。 | ```<meta name="viewport" content="width=device-width, initial-scale=1">``` |
| `meta` | `content` | ブラウザのタブに表示されるドキュメントのタイトルを設定する。 | ```<meta name="description" content="Page about our newest products">``` |
| `meta` | `charset` | 文字エンコーディングを宣言する。 | ```<meta charset="UTF-8">``` |
| `meta` | `property` | ブラウザのタブに表示されるドキュメントのタイトルを設定する。 | ```<meta property="og:title" content="Website title">``` |
| `style` | `type` | スタイルコンテンツのMIMEタイプ。 | {% raw %}```<style type="text/css">p { color: red; }</style>```{% endraw %} |
| `style` | `media` | スタイルが適用されるメディアまたはデバイスを指定する。 | ```<style media="print">body { font-size: 12pt; }</style>``` |
| `title` | 属性なし | タグ`title`は属性を受け付けない。 | ```<title>Kitchenerie</title>``` |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation" }

{% alert note %}
リンク名は最大63バイトまでで、制限を超えると自動的に切り詰められる。
{% endalert %}
