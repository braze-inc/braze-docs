...
Brazeは、IPアドレスとドメインを設定する際にSPFレコードを設定する。当社が提供するDNSレコードを追加する以外に、追加のアクションは必要ない。
...
Brazeは、IPアドレスとドメインを設定する際にDKIMレコードを設定する。当社が提供するDNSレコードを追加する以外に、追加のアクションは必要ない。

### ドメインベースのメッセージ認証、報告、適合性（DMARC）

[ドメインベースメッセージ認証、レポート&、準拠（DMARC）](https://dmarc.org/)は、メール送信者が自身のメールの正当性を証明するためのメール認証プロトコルである。これにより受信側の信頼性が確保され、メールの受け入れが促進される。DMARC により、送信者ポリシーフレームワーク (SPF) またはドメインキー識別メール (DKIM) を使用して認証されなかったメールの処理方法をメール送信者が指定できます。これは、SPFとDKIMの両方のチェックがパスしていることを確認することで達成される。 

送信者は、署名や認証チェックに失敗したメールをどう処理するかをメールボックスプロバイダーに指示する。失敗はなりすましの可能性を示す。プロバイダーに、失敗したメールを拒否または隔離し、オートメーションによるレポートを送信するよう指示できる。これにより、プロバイダーはスパマーを識別し、悪意のあるメールをブロックし、誤検知を最小限に抑え、認証レポートの透明性を向上させることができる。
...
ルートドメインにDMARCポリシーを設定し、すべてのサブドメインに適用させる。これにより、現在および将来のサブドメインにおける追加の設定が不要になる。以下のポリシーのいずれかを設定できる：
...
![DMARC値が「PASS」であるメール。]({% image_buster /assets/img_archive/dmarc_example.png %})
