...
BrazeがDNSレコードを提供したら、DNSまたはITチームが可能になり次第、それらを追加する。ドメインの検証には時間がかかるため、レコードの追加が遅すぎると、DNSレコードが後に正しく解決されたとしても、検証に失敗することがある。DNSレコードが正しいように見えるが検証に失敗する場合は、オンボーディングまたはサポートチームに連絡して、検証を再開する。
...
検証に失敗し、DNSレコードが正しいと思われる場合は、Brazeサポートに問い合わせること。
