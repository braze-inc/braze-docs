...
Brazeは、メールアドレスのローカル部分とホスト部分の両方の構文を検証する。ローカル部分はアスペランド（@）より前、ホスト部分はアスペランドより後である。ローカ ル部分は、ピリオド(.)以外であれば、どのような文字で始まり、どのような文字で終わってもよい。この処理では、ドメインに有効なMXサーバーがあるかどうかや、そのドメインにユーザーが存在するかどうかは考慮されない。
...
ホスト部分はIPv4またはIPv6アドレスにはできない。トップレベルドメイン（.com、.org、.netなど）は、完全に数字にすることはできない。
...
ドメインの最終ラベルは、有効なトップレベルドメイン（TLD）でなければならない。このTLDは[ICANNのTLD](https://data.iana.org/TLD/tlds-alpha-by-domain.txt)リストに掲載されるべきである。Brazeバリデーターは構文しかチェックしない。タイプミスや存在しない住所をキャッチすることはできない。
