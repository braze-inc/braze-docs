...
単一のシンプルなメッセージングにはキャンペーン s を使用します。複数ステップ ユーザーのジャーニーにキャンバスを使用します。
...
Braze は、プレーンテキストへのエディットを検出するまで、HTML版のプレーンテキストバージョンを自動的に更新します。Braze は、編集を検出すると、意図的な変更を行ったと見なされるため、プレーンテキストの更新を中止します。自動同期を復元するには、**平文**に移動し、**HTML**から再生成を選択します(平文が同期していない場合にのみ表示されます)。
...
{% alert important %}
Braze は、属性s として参照されるHTMLイベントハンドラを自動的に削除します。これによりHTMLが変更されるため、完了後にメールを再確認してください。[HTML ハンドラー](https://www.w3schools.com/tags/ref_eventattributes.asp)の詳細を参照してください。
{% endalert %}
...
メールの設計と構築が完了したら、**Sending Settings**に送信情報を追加します。
...
**Sending Settings**> **Advanced**で、インラインCSSを有効にし、メール ヘッダー s とメール エクストラのパーソナライゼーションを追加して、追加データを他のメール サービスプロバイダに送り返します。

##### 電子メールのヘッダー

メールヘッダーを追加するには、**「新しいヘッダーを追加**」を選択する。電子メールのヘッダーには、送信される電子メールに関する情報が含まれている。これらの[キーと値のペア]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/key_value_pairs/)には、通常、送信者、受信者、認証プロトコル、およびルーティングインフォメーションが含まれます。Braze は、メール s が受信トレイプロバイダに到達するために、RFC が必要とするヘッダーを自動的に追加します。
...
メールの作成が終わったら、テストしてから送信してください。概要画面の下部から ［**プレビュー後にテスト**] を選択します。 
...
最終確認の準備ができたら、**Test Send**を選択し、自分またはテスタグループにテストメッセージを送信して、機器やクライアントs 間でメールが正しく表示されることを確認します。
...
{% tabs %}
{% tab Campaign %}
次に、キャンペーンの残りの部分を作成します。Braze ツールを使用してメール キャンペーンを構築する方法については、次の項を参照してください。

#### 配信スケジュールまたはトリガーを選択する

スケジュールされた時刻、アクション、またはAPI トリガーに基づいてメールを配信します。詳細については、[キャンペーンのスケジューリング]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/)を参照してください。
...
ユーザーが指定されたアクションをとった場合にBrazeがコンバージョンを数える最大30日間のウィンドウを許可できます。Brazeが開封を追跡し、自動的にクリックする間、開封にコンバージョンイベントを設定したり、クリックして[インテリジェントセレクション]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/)を使用することができます。
{% endtab %}
...
最後の節では、設計したキャンペーンを要約します。関連するすべての詳細を確認し、**Launch Campaign**を選択します。 
