...
]({% image_buster /assets/img_archive/choose_email_creation.png %})ドラッグ＆ドロップエディター、HTMLエディター、テンプレートのいずれかを選択し、メール編集を体験できる。{: style="max-width:75%" }()
...
]({% image_buster /assets/img_archive/regenerate_from_html.png %})「HTML から再生成」ボタン。{: style="max-width:30%;float:right;margin-left:15px;border:none;" }()
...
]({% image_buster /assets/img/email.png %})メールを作成するための [メールのバリアント] パネル。{: style="max-width:75%" }()
...
]({% image_buster /assets/img/ai_copywriter/ai_copywriter_email.png %})メール作成画面の [本文] タブにある [AI コピーライターを起動] ボタン。{: style="max-width:80%"}()
{% endalert %}
...
]({% image_buster /assets/img_archive/newEmailTest.png %})メール作成時の [テスト送信] オプションとプレビューの例。()
