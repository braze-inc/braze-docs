...
APIまたはSDKを通じてユーザーを作成する前に、エンドポイントを呼び出す。 [`/users/export/ids`]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier/)エンドポイントを呼び出し、ユーザーの`email_address` を指定する。ユーザープロファイルを返した場合、そのBrazeユーザーはすでにそのメールアドレスに関連付けられている。

新しいユーザーを作成する際には、ユニークなメールアドレスを探し、同じメールアドレ スを持つユーザーを渡したり、インポートしたりしないことを強く推奨する。そうしないと、メッセージ送信、ターゲティング、レポート、その他の機能に影響を与える、意図しない結果を招く可能性がある。
...
トラッキング, 追跡ドメインに設定ミスがある場合、開封やクリックがゼロになる可能性がある。これは、以下のいずれかの理由によるものである：
- トラッキング, 追跡URLが`https` ではなく`http` になるSSLの問題がある。
- 開封イベント、クリックイベント、またはその両方のユーザーエージェント文字列が入力されないというCDNの問題がある。
