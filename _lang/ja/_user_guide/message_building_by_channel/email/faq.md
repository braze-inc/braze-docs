...
- **シナリオ2:受信者オブジェクト内の異なる `user_ids` の重複メール:**同じメールが \`recipients\`\` オブジェクトによって参照される複数の`External_user_IDs` 内にある場合、メールは2回送信される。
- **シナリオ3:受信者オブジェクト内でuser_ids が重複し、メールが重複する：**同じユーザープロファイルを 2 回追加しようとすると、プロファイルのうち 1 つだけがメールを受け取ります。
