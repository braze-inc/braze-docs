...
]({% image_buster /assets/img/push_example.png %})John Doe のユーザープロファイルで、メール購読の状態が「購読中」に設定されている。{: style="float:right;max-width:35%;margin-left:15px;"}()
...
]({% image_buster /assets/img/sub_group_create.png %})サブスクリプショングループを作成するフィールド。{: style="max-width:75%"}()

### 購読グループによるセグメンテーション

セグメントを作成するときに、購読グループ名をフィルターとして設定します。これにより、グループにオプトインしたユーザーがメールを受信することが確認されます。これは、月刊ニュースレター、クーポン、メンバーシップ層などに最適です。

離脱ユーザー」セグメントのユーザーを「週刊メール」サブスクリプショングループのフィルターでターゲティングする例。]({% image_buster /assets/img/segment_sub_group.png %}){: style="max-width:90%"}
...
]({% image_buster /assets/img/campaign_analytics_sub_groups.png %})顧客が送信した状態変更の合計数を表示する「メールメッセージパフォーマンス」ページ。()
...
]({% image_buster /assets/img_archive/not_optin.png %})セグメントフィルタとして使用されるメールサブスクリプションステータス。()
