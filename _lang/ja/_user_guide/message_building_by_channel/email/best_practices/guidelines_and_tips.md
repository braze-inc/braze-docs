...
- 画像, 写真は5MB未満でなければならない。互換性を最大限に高めるため、PNG、JPEG、またはGIFの使用を推奨する。SVGとWebPは避けるべきだ。多くの主要なメールクライアントがまだそれらをサポートしていないからだ。
- 画像や写真の高さや幅を設定するな。そうすると、表示が崩れたメールに不要な余白が生じるからだ。
...
メールの検証は、ユーザーのメールアドレスが更新された時、またはAPI、CSVアップロード、SDKによってBrazeにインポートされる時、あるいはダッシュボードで変更された時に発生する。メールには空白を含めることはできない。APIを使用して送信する場合、空白が含まれるとエラー`400`が発生する可能性がある。
...
許可されていない文字やメール検証ルールについては、[メール検証を]({{site.baseurl}}/user_guide/message_building_by_channel/email/email_setup/email_validation/#how-it-works)参照せよ。
