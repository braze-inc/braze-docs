...
{% alert tip %}
すべてのメッセージングチャネルにおけるリンクタイプの比較と、AASAファイルが必要な場合のガイダンスについては、[iOSディープリンクガイドを]({{site.baseurl}}/developer_guide/push_notifications/ios_deep_linking_guide)参照せよ。
{% endalert %}
...
- `paths`: 関連付けから含まれるまたは除外されるパスを指定する文字列の配列。パスの前に`NOT`を使用してパスを無効にすることができます。この例では、このパス上のすべてのリンクはアプリを開くのではなく、Webに移動します。ディレクトリ内の全パスをイネーブルメントするにはワイルドカード`*`として  を使用できる。また`?`  は単一文字に一致させる（例：/archives/201?/ は2010年から2019年までの全数字に一致する）。
...
`AndroidManifest.xml` ファイルで、アプリライケーション要素内にメタデータ要素を追加します。メタデータ要素には、\``android:name`attribute` 属性"asset_statements"と、``android:resource`attribute\` 属性が含まれる。この属性は、文字列配列を持つリソースファイルを指すもので、その配列にはWeb サイトのURLが含まれている。
...
{% alert note %}
クリックトラッキングリンクは通常、メールのオンボーディングの一部として設定されます。顧客オンボーディング時に完了していなかった場合は、担当のアカウントマネージャーに連絡して支援を求めよ。
{% endalert %}
...
![テキストリンク用のカスタム属性。]({% image_buster /assets/img/text_click_tracking_off.png %}){: style="max-width:60%;"}
