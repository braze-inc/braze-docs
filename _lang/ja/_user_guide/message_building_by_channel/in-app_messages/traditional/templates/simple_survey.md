...
- **単一選択:**カスタム属性のデータタイプはストリングでなければなりません。文字列型でないカスタム属性は、ドロップダウンに耳がアプリされず、レスポンスは記録されません。
...
これを設定するには、「非常に不満足な😡」から「非常に満足した😍」までの5つの選択肢がある、単一の選択肢の調査を使用します。各選択肢は、カスタム属性 `customer_satisfaction` にマッピングされ、1 ～5 の数値が割り当てられています。1 は満足度が最も低い値であり、5 は満足度が最も高い値です。これらの数値は文字列として保存されることに注意してください。単一選択では文字列カスタム属性s が必要です。
...
ユーザがアンケートを送信すると、選択した値がカスタム属性としてログに記録されます。その後、オーディエンスフィルターを使用してフォローアップキャンペーンを作成できます。たとえば、`customer_satisfaction` 属性が"1" または"2" のユーザー s に対象のウィンバックメッセージを送信します。
