...
![通知の好み、食事の好み、および顧客満足度調査の 3 つのシンプルな調査のメッセージ。アンケートで選択されたオプションは、そのユーザーに対して記録されるカスタム属性に対応する。]({% image_buster /assets/img/iam/iam-survey.png %})
...
![簡易調査エディタの「作成」タブには、ヘッダー、任意の本文、任意のヘルプテキスト用のフィールドがある。]({% image_buster /assets/img/iam/iam-survey2.png %}){: style="max-width:90%"}
...
![「送信時に属性を記録する」が選択された選択肢ドロップダウン。]({% image_buster /assets/img/iam/collect-attributes.png %}){: style="max-width:60%"}
...
- **単一選択:**カスタム属性のデータ型は文字列でなければならない。文字列型でないカスタム属性はドロップダウンに表示されず、回答も記録されない。
...
![クリック時の動作は「回答を送信し、確認ページを表示する」に設定されている。]({% image_buster /assets/img/iam/confirmation-option.png %}){: style="max-width:60%"}

確認ページを追加する場合は、メッセージをカスタマイズするために**確認ページ**タブに切り替える:

![シンプルな調査エディターの [確認ページ] タブ。利用可能なフィールドは、ヘッダー、オプションの本文、ボタンテキスト、およびボタンのクリック時の動作である。]({% image_buster /assets/img/iam/confirmation-page.png %}){: style="max-width:90%"}
...
![ユーザーがカラーパレットをクリックした後、カラーテーマピッカーが展開された状態の簡易調査エディタの「作成」タブ。]({% image_buster /assets/img/iam/color-theme-picker.png %}){: style="max-width:80%"}
...
これを設定するには、「非常に不満足な😡」から「非常に満足した😍」までの5つの選択肢がある、単一の選択肢の調査を使用します。各選択肢は、カスタム属性 `customer_satisfaction` にマッピングされ、1 ～5 の数値が割り当てられています。1 は満足度が最も低い値であり、5 は満足度が最も高い値です。これらの数値は文字列として保存されることに注意せよ。単一選択には文字列のカスタム属性が必要だからだ。
...
ユーザがアンケートを送信すると、選択した値がカスタム属性としてログに記録されます。その後、オーディエンスフィルターを使用してフォローアップキャンペーンを作成できます。例えば、属性値`customer_satisfaction`が「1」または「2」のユーザーに対して、顧客奪還メッセージを送信する。
...
| アンケート&　フィードバック | `wants_surveys`        | `true` |
| コツ&チュートリアル   | `wants_tips`           | `true` |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
