...
次に、セグメントやフィルターを選択して[ユーザーを絞り込み、ターゲットを絞る]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)必要がある。そのセグメンテーションのおおよその母集団がどのようなものか、自動的にスナップショットが送られてくる。正確なセグメンテーションは、常にメッセージ送信前に計算されることを覚えておいてほしい。
