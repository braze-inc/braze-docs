...
次に、セグメントやフィルターを選んで[ユーザーを絞り込み、ターゲットを絞る]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)必要がある。そのセグメンテーションのおおよその母集団がどのようなものであるかのスナップショットを自動的に受け取ることができる。正確なセグメンテーションは、常にメッセージ送信前に計算されることを覚えておいてほしい。
