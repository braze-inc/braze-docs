...
メッセージタイプに応じて、[**画像をアップロード**]、[**バッジを選ぶ**]、または [**Font Awesome**] を使用できます。画像をアップロードするには、「**画像を追加**」を選択するか、画像のURLを入力する。**Add Image**を選択すると、**Media Library**が開き、以前にアップロードしたイメージを選択したり、新しいイメージを追加したりできます。メッセージの種類やプラットフォームによって、推奨される比率や要件が異なる場合がある。
...
次に、セグメントやフィルターを選択して[ユーザーを絞り込み、ターゲットを絞る]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)必要がある。セグメンテーションのおおよその母集団がどのようなものであるかのスナップショットを自動的に受け取ることができる。正確なセグメンテーションは常にメッセージ送信前に計算されることに留意してほしい。
...
アプリ内メッセージキャンペーンがユーザーのローカルタイムゾーンを使用してスケジュールされた場合、キャンペーンの開始時間と終了時間の評価はデバイス自体で処理される。

アプリ内メッセージキャンペーンは通常、アプリセッションの開始時または更新時にユーザーのデバイスにプッシュされる。その瞬間だ：

1. SDKは、ユーザーがトリガーベースのアプリ内メッセージの対象となるかどうかを評価する。
2. デバイスは、ユーザーのトリガーイベントがキャンペーンの開始時間と終了時間（ユーザーのローカルタイムゾーンによって定義される）内に発生したかどうかをチェックする。
3. 両方の条件を満たせば、アプリ内メッセージは表示対象となる。

#### 考慮事項

- アプリ内メッセージが配信された直後にユーザーがイベント（ボタンタップなど）をトリガーした場合、次のセッション更新までメッセージが表示されないことがある。
- アプリ内メッセージキャンペーンは、他のチャネルと同様、24～48時間前に開始するのが理想的である。このバッファは、ユーザーが資格を満たし、メッセージが評価され表示されるセッションを開始するのに十分な時間を与える。