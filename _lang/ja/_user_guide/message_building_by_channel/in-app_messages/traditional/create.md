...
| ウェブブラウザ                    | ウェブSDK                 |
| モバイルアプリ&とWebブラウザの両方 | iOS、Android&、Web SDK |
...
メッセージタイプに応じて、[**画像をアップロード**]、[**バッジを選ぶ**]、または [**Font Awesome**] を使用できます。画像をアップロードするには、**「画像を追加」**を選択するか、画像のURLを入力する。**Add Image**を選択すると、**Media Library**が開き、以前にアップロードしたイメージを選択したり、新しいイメージを追加したりできます。各メッセージの種類やプラットフォームには、それぞれ推奨される比率や要件がある。画像を一から作成したり依頼したりする前に、必ずそれらを確認すること。
...
![アプリ内メッセージにおけるプライマリボタンとセカンダリボタン]({% image_buster /assets/img/primary-secondary-buttons.png %})
...
トリガーメッセージの優先度設定には高、中、低の選択肢がある。これらはバケットであり、複数のメッセージが同じ優先度を選択される可能性がある。複数のメッセージが同じ優先度を持つ場合、最も最近作成または割り当てられたメッセージが優先され、最初に表示される。

- **デフォルトの優先度バケット：**2つのキャンペーンが同じトリガーを共有し、デフォルト（中）の優先度を使用する場合、最後に作成されたキャンペーンがトリガーを受け取る。
- **特定の優先度バケット：**複数のキャンペーンが同じトリガーを共有し、特定の優先度バケットに割り当てられている場合、そのバケットに最も最近割り当てられたキャンペーンがトリガーを受け取る。

これらのバケット内で優先順位を設定するには、**「正確な優先順位を設定**」をクリックし、キャンペーンをドラッグ＆ドロップして正しい優先順位で並べ替えられる。

![アプリ内メッセージキャンペーンとキャンバスにおける優先度の設定方法の例。]({% image_buster /assets/img_archive/bucket_prioritization.png %}){: style="max-width:70%"}

#### ターゲットとするユーザーを選択する

次に、セグメントやフィルターを選択してオーディエンスを絞り込み、[ユーザーをターゲットに]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)設定しなければならない。そのおおよそのセグメント人口がどのようなものか、自動的にスナップショットを受け取る。正確なセグメントの所属は、メッセージが送信される前に常に計算されることを覚えておけ。
...
![「表示前にキャンペーン適格性を再評価する」のチェックボックスが選択されている。]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %}){:style="max-width:60%"}
...
{% alert note %}
このオプションのイネーブルメントにより、ユーザーがアプリ内メッセージをトリガーしてからメッセージが表示されるまでに、わずかな遅延（100(<ミリ秒）が生じる。これは追加の適格性チェックとテンプレートリクエストによるものである。
<br><br>
...
Braze は信頼性とスピードを重視しています。必要なデータのみをBrazeに送信し、ブランドに価値をもたらさなくなったキャンペーンはすべて停止することをお勧めする。
...
アプリ内メッセージキャンペーンがユーザーのローカルタイムゾーンでスケジュールされた場合、キャンペーンの開始時刻と終了時刻の評価は端末側で処理される。

アプリ内メッセージキャンペーンは、通常、アプリのセッションが開始または更新された際にユーザーの端末にプッシュされる。その時：

1. SDKは、ユーザーがトリガーベースのアプリ内メッセージの対象となるかどうかを評価する。
2. このデバイスは、ユーザーのトリガーイベントがキャンペーンの開始時刻と終了時刻（ユーザーのローカルタイムゾーンで定義される）の間に発生したかどうかを確認する。
3. 両方の条件が満たされた場合、アプリ内メッセージは表示可能となる。

#### 考慮事項

- ユーザーがアプリ内メッセージの配信直後にイベント（ボタンタップなど）をトリガーした場合、すべての条件を満たしている場合でも、次のセッション更新までメッセージが表示されないことがある。
- 他のチャネルタイプと同様に、アプリ内メッセージキャンペーンは、理想的には24～48時間前に開始すべきだ。このバッファは、ユーザーが資格要件を満たし、メッセージの評価と表示のためのセッションを開始するのに十分な時間を与える。