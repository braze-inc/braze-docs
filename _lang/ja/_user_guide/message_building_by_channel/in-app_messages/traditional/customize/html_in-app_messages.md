...
{% alert note %}
Web SDKを介してHTML アプリ内メッセージ s を有効にするには、`allowUserSuppliedJavascript` 初期化オプションをBraze に指定する必要があります(`braze.initialize('YOUR-API_KEY', {allowUserSuppliedJavascript: true})` など)。これはセキュリティ上の理由によるもので、HTML のアプリ内メッセージは JavaScript を実行できるため、サイト管理者が有効にする必要があります。
{% endalert %}
...
1. メディアライブラリーを介してキャンペーンに追加されたアセットを使用すると、ユーザーがオフラインになっていたり、インターネットに接続されていない場合でも、メッセージを表示できます。
...
アセットのファイル名がローカルHTMLアセットの名前と一致する場合、自動的に置き換えられます(たとえば、`cat.png` はアップロード ed で、`<img src="cat.png" />` は存在します)。 
...
3. ボタンIDに`?abButtonId=0` を使用していた自動クリックトラッキングと、閉じるボタンの「ボディクリック」トラッキングは削除された。以下のコード例は、新しいクリック・トラッキングJavaScriptメソッドを使用するためにHTMLを変更する方法を示している：
   ...
   | 前 | その後 |
   |:-------- |:------------|
   |<code>&lt;a href="<mem_e994b2ea-d5fa-4258-be0e-c09f2a09162b/>"&gt;Close Button&lt;/a&gt;</code>|<code>&lt;a href="#" onclick="brazeBridge.logClick();brazeBridge.closeMessage()"&gt;Close Button&lt;/a&gt;</code>|
   |<code>&lt;a href="<mem_38537289-a3bb-4828-b2dc-a595791ae585/>"&gt;Close Button&lt;/a&gt;</code>|<code>&lt;a href="#" onclick="brazeBridge.logClick('0');brazeBridge.closeMessage()"&gt;Close Button&lt;/a&gt;</code>|
   |<code>&lt;a href="<mem_dbbc7d9c-41fc-4bca-838e-f48c718f8e68/>">Track button 1&lt;/a&gt;</code>|<code>&lt;a href="<mem_2039ea8d-ca36-4f9e-aed7-57953f322bb4/>" onclick="brazeBridge.logClick('0')"&gt;Track button 1&lt;/a&gt;</code>|
   |<code>&lt;script&gt;<br>location.href = "<mem_ee24f951-1ac4-4171-a495-510f96f1aa9a/>"<br>&lt;/script&gt;</code>|<code>&lt;script&gt;<br>window.addEventListener("ab.BridgeReady", function(){<br>&nbsp;&nbsp;brazeBridge.logClick("1");<br>&nbsp;&nbsp;brazeBridge.closeMessage();<br>});<br>&lt;/script&gt;</code>|
