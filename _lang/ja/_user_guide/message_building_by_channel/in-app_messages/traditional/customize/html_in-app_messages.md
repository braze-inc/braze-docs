...
]({% image_buster /assets/img/iam-beta-javascript-preview.gif %})ページをスワイプしてHTMLプレビューとインタラクトする。()
...
3. ボタンIDに`?abButtonId=0` を使用していた自動クリックトラッキングと、閉じるボタンの「ボディクリック」トラッキングは削除された。以下のコード例は、新しいクリック・トラッキングJavaScriptメソッドを使用するためにHTMLを変更する方法を示している：
   ...
   | 前 | その後 |
   |:-------- |:------------|
   |<code>hra;lt"braze://close">閉じるボタン</a></code>|[閉じる] ボタン|
   |<code>hra;lt"braze://close?abButtonId=0">閉じるボタン</a></code>|[閉じる] ボタン|
