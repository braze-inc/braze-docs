...
![「作成」タブと、Webhookのテンプレート例。]({% image_buster /assets/img_archive/webhook_compose.png %})
...
![リクエスト本文はJSON形式のキーと値のペアで設定されている。]({% image_buster /assets/img/webhook_json_1.png %})
...
![Liquidを使用した生のテキストのリクエストボディの例だ。]({% image_buster /assets/img_archive/webhook_rawtext.png %})
...
次に、[ユーザーをターゲットにする]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)には、セグメントやフィルターを選択してオーディエンスを絞り込む必要がある。このステップでは、セグメントの中からより大きなオーディエンスを選択し、必要に応じて当社のフィルターでそのセグメントをさらに絞り込む。そのおおよそのセグメント人口がどのようになるか、自動的にプレビューが表示される。正確なセグメントの所属は、メッセージが送信される前に常に計算されることを覚えておけ。
...
![Webhookエラーが発生した。メッセージは「現在のユーザーに関する情報を照会するには、有効なアクセストークンを使用する必要がある」だ。]({% image_buster /assets/img_archive/webhook-error.png %})
