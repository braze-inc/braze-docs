...
Brazeがコンテンツカードを送信すると、" 受信トレイ" ユーザーに配信する準備が整います(メール s と同様)。Brazeがコンテンツを表示時にコンテンツカードに取り込んだ後、コンテンツはカードの存続期間中に変更されません。これには、エンドポイント情報が変更された場合に、コネクテッドコンテンツを通じたAPI呼び出しが含まれます。Brazeはその情報を更新しない。新しいカードs の送信を中止して、フィードs から既存のカードs を削除することしかできません。キャンペーンを変更すると、将来のカードのみが更新を反映します。
...
Braze は、指定されたイベントを処理した後、後続のリフレッシュでカードを削除します。
...
コンテンツカードの送信後は編集できません。送信カードsを変更する必要がある場合は、[キャンペーン再適格性]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/reeligibility/)を次の選択肢とともに使用することを検討してください。
...
* **レポート:**カード版ごとに分析があります。
* **既存の受信者:**新規および既存の受信者s は、次回のフィード更新時に更新d カードを表示します(適格な場合)。
...
* **レポート:**1つのキャンペーンには、起動されたすべてのカード版のすべての分析が含まれます。Brazeは、バージョンを区別しません。
* **既存の受信者:**カードをすでに受信したユーザは、再利用可能になるまで更新d s を受信しません。再適格性が失われた場合、新しいカードを受け取ることはありません。
