...
## メッセージ 個別チャネルの開封尤度フィルター {#individual-channels}

Brazeにユーザーにとって最適なチャネルを1つだけ選ばせるのではなく、[「メッセージ開封可能性」セグメンテーションフィルターを]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#message-open-likelihood)使用して、選択した特定のチャネルでメッセージを開封する可能性があるかどうかに基づいて、ユーザーをフィルタリングすることができる。このフィルターは、インタラクションのパーセンテージを、チャネルごとに送信された直近の100メッセージの総受信メッセージ数で割って算出される。

ユーザーは、特定のチャネルで少なくとも3つのメッセー ジを受信していなければ、そのチャネルの尤度スコアは得 られない。チャネルの可能性を測定するのに十分なデータがないユーザーは、「空白である」を使用して選択できます。
