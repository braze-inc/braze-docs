...
]({% image_buster /assets/img/intelligent_channel_filter.png %})選択できる各種チャネルのドロップダウンが表示されているインテリジェントチャネルフィルター。{: style="float:right;max-width:40%;margin-left:10px;margin-top:10px;border:0"}()
...
]({% image_buster /assets/img/intelligent_example.png %})[モバイルプッシュ] または [データ不足] のインテリジェントチャネルフィルター。{:style="border:none"}()
