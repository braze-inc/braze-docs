...
- 再適格期間が 24 時間以上に設定されている。これよりも短い期間は、コントロールバリアントの整合性に影響する可能性があるため、サポートされていません。詳細については、[こちらのFAQを]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/#why-is-re-eligibility-in-less-than-24-hours-not-available-when-combined-with-intelligent-selection)参照されたい。
{% endtab %}
...
![キャンバスでインテリジェントセレクションオプションをオンにする]({% image_buster /assets/img_archive/canvas_intelligent_selection.png %})
...
## インテリジェントセレクション バリアント分配

インテリジェントセレクションは、キャンペーンのコンバージョンのステータスに基づき、バリアント配信を行う。トレーニング期間後の最終的な分布を決めるだけである。 

つまり、キャンペーンの初期段階では、99%と1%のインテリジェントセレクションの両方がほぼ同じ送信を受ける可能性があるが、バリアント割り当ての最終的なパーセンテージは99%-1%に設定される可能性がある。

キャンペーンの初期段階でインテリジェントセレクションを半々で送信したくない場合は、固定バリアントを使った従来のA/Bテストを使うことをお勧めする。
