...
AI によってパーソナライズされたおすすめは、購入やカスタムイベントなど、過去 6 か月分のアイテムインタラクションデータを使用してレコメンデーションモデルを構築します。ユーザーのパーソナライズされた一覧に十分なデータがない場合、最も人気のあるアイテムがフォールバックとして機能するため、ユーザーはまだ関連する提案を取得しています。
...
- AI によるアイテムのおすすめ
- 関連アイテムのカタログ
- "トラッキング買付けの手法で、買い付け対象かカスタムイベントのいずれか
{% enddetails %}
...
- AI によるアイテムのおすすめ
- 関連アイテムのカタログ
- "トラッキング購買の手法(購買対象かカスタムイベントのいずれか)
{% enddetails %}
...
ルールベースの推奨事項は、手動で設定する必要がある固定ロジックに基づいています。つまり、ロジックを更新しない限り、お勧めはユーザーの個々の購買履歴や嗜好に合わせて調整されません。したがって、このメソッドは、頻繁な更新を必要としないおすすめに最適です。
...
推奨は独立系的に動作し、他のモデルが推奨している内容については一切把握していないことに注意してください。つまり、各セクションには、同じメール内の他のAI レコメンドセクションにすでに表示されている重複項目を含めることができます。
...
### 複数の日が一度に期限切れになるすべての推奨事項を作成するにはどうすればよいですか?

特定の日にすべての複数日の推奨事項を期限切れにする(そのため、すべての有効な推奨事項には新しい予測が一度に表示される)場合は、Brazeサポートまたは顧客サクセスマネージャーにお問い合わせください。Braze人工知能のエキスパートがこれを手動で実行し、最大限の機種パフォーマンスを確保します。
