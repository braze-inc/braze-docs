...
- 再適格期間が 24 時間以上に設定されている。これよりも短い期間は、コントロールバリアントの整合性に影響する可能性があるため、サポートされていません。学習については、[こちらのFAQ]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/#why-is-re-eligibility-in-less-than-24-hours-not-available-when-combined-with-intelligent-selection)を参照せよ。
{% endtab %}
...
![インテリジェントセレクションオプションがキャンバスで有効になっている]({% image_buster /assets/img_archive/canvas_intelligent_selection.png %})
...
## インテリジェントセレクションのバリアント配布

インテリジェントセレクションは、キャンペーンのコンバージョンステータスに基づいてバリアントの配信を決定する。それはトレーニング期間終了後の最終的な分布を決定するだけだ。 

これは、キャンペーンの初期段階では、99％と1％のインテリジェントセレクションの両方がほぼ同数の配信を受ける可能性があるが、最終的なバリアント配分の割合は99％対1％に設定されるかもしれないことを意味する。

キャンペーンの初期段階でインテリジェントセレクションが50/50で送信されるのを望まない場合、固定されたバリアントを用いた従来のABテストの使用を推奨する。
