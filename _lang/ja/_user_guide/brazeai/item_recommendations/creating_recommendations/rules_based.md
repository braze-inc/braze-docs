---
nav_title: ルールに基づく推奨
article_title: ルールに基づくアイテム推薦を作成する
description: "この記事では、カタログに掲載されている項目について、AI項目のレコメンデーションを作成する方法を説明します。"
page_order: 2
---
...
2. 各商品について、推奨商品のリストを文字列として追加する。区切り文字（例：パイプ`|`）で区切り、列名は “product_recommendations”.
...
![Brazeの例のカタログ。]({% image_buster /assets/img/recs/catalog_items.png %})
...
![コンテンツカードキャンペーンからのメッセージ作成画面。]({% image_buster /assets/img/recs/content_card_preview.png %})
...
![コンポーザーにおけるクリック時の動作ブロックの例だ。]({% image_buster /assets/img/recs/on_click_behavior.png %}){: style="max-width:60%;"}<br><br>
...
![例示の最終呼び出しによって生成されたレストランリストのレンダリング。]({% image_buster /assets/img/recs/sample_response.png %}){: style="max-width:30%;"}
{% endtab %}
{% endtabs %}
