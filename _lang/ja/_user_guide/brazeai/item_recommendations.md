...
[AIによる商品推薦]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/)機能の一環として、AIパーソナライズド推薦はディープラーニングを活用し、ユーザーが過去に示した興味に基づいて、次に最も関心を持ちそうな商品を予測する。この方法は、ユーザーの行動に適応する動的でカスタマイズされた推奨システムを提供します。

AI によってパーソナライズされたおすすめは、購入やカスタムイベントなど、過去 6 か月分のアイテムインタラクションデータを使用してレコメンデーションモデルを構築します。十分なデータがないユーザーには、パーソナライズされたリストの代わりに人気アイテムがフォールバックとして表示される。これにより、ユーザーは依然として関連性の高い提案を受け取れる。
...
- AI によるアイテムのおすすめ
- 関連アイテムのカタログ
- 購入のトラッキング, 追跡。購入オブジェクトかカスタムイベントのいずれかである。
{% enddetails %}

{% details Setting it up %}
1. [AI によるアイテムのおすすめ]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/)を作成します。
...
「トレンド」推薦モデルは、最近のユーザーとのやり取りにおいて最も好ましい勢いを見せているアイテムを特徴としている。この計算には、約10週間のイベント履歴の加重分析を用いる。最も重い重みは、直近のおよそ2週間の期間に適用される。小さな変動が推薦品質に影響を与えないようにするため、活動閾値と統計的平滑化技術を適用する。
...
- AI によるアイテムのおすすめ
- 関連アイテムのカタログ
- 購入（購入オブジェクトまたはカスタムイベントのいずれか）のトラッキング, 追跡の方法
{% enddetails %}
...
{% alert note %}
選択項目を使用する場合、ソートフィールドと制限条件はAIアイテム推薦では使用されない。つまり、特定のソートフィールドで選択を作成し、返されるアイテム数を制限した場合、それらの制約はAIアイテム推薦の処理時には適用されない。
{% endalert %}
...
ルールベースの推奨事項は、手動で設定する必要がある固定ロジックに基づいています。これは、ロジックを更新しない限り、ユーザーの個々の購入履歴や好みに応じて推薦が調整されないことを意味する。したがって、頻繁な更新を必要としない推薦にはこの方法が最適である。
...
推奨は独立系で動作し、他のモデルが何をお勧めしているかについての知識を持たないことに注意せよ。これは、各セクションには、同じメール内の他のAI推薦セクションに既に表示されている重複アイテムが含まれる可能性があることを意味する。

### アイテムレコメンドプロにアップグレードした後、既存のレコメンドは週1回トレーニングされますか?

はい。ただしスケジュールされている次回の更新後のみです。既存の推奨事項は、アイテムの推奨事項Proにアップグレードした直後は、週次トレーニングおよび日次予測に切り替えられません。ただし、次回の再トレーニングサイクルで自動的に新しいスケジュールが適用されます。たとえば、推奨が2 月1 日に最後にトレーニングされ、30 日ごとに再トレーニングするように設定されている場合、3 月2 日の次回の更新後に新しい週単位のスケジュールが適用されます。

### 複数日間にわたるすべての推奨事項を、一度に期限切れにするにはどうすればよいのか？

特定の日にちで全ての複数日推奨を期限切れにしたい場合（つまり、そのような有効な推奨が全て同時に新しい予測を受け取るようにしたい場合）、Brazeサポートまたは顧客サクセスマネージャーに連絡して支援を依頼すること。Braze AIの専門家は、モデルのパフォーマンスを最大限に発揮させるため、これを手動で実行する。
