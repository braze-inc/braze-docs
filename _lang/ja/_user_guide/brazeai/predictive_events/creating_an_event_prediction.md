...
ユースケースによっては、フィルターを使用して、モデルに対して評価したいユーザーを指定したい場合がある。これを行うには、**自分の予測オーディエンスを定義**を選択し、オーディエンスフィルタを選択します。たとえば、「アプリの初回使用」フィルターを 30 日に設定して、アプリを少なくとも 30 日間使用していたユーザーに集中することができます。このオーディエンスを設定することで、Brazeに、（モデル実行時に）少なくとも30日間アプリを使用したユーザーから特別に学習させたいことを伝える。

{% alert important %}
アクティブユーザー、新規ユーザー、価値の高いユーザー、特定の国のユーザーなど、ユースケースに関連するユーザー特性にフィルターを絞る。ユーザーが予測対象のイベントをすでに実行しているかどうかで予測対象ユーザーをフィルターするのは避けよう。予測対象ユーザーは、イベントの結果そのものではなく、モデルに学習させたいユーザーを定義する。モデルが正確に学習し、将来のイベント完了の可能性を予測するためには、イベントを完了したユーザーと完了していないユーザーの両方を観察する必要がある。
{% endalert %}

予測対象ユーザーは、機械学習モデルが過去から学ぶために見るユーザーグループを定義する。Brazeは予測対象ユーザーのサイズを表示する。希望するオーディエンスを指定しても、モデルの実行に必要な最低条件を満たさない場合は、より広範なフィルターを指定するか、**All Users**オプションを使用する。多くのユースケースでは、特定の予測対象ユーザーを選択する必要はないことを覚えておこう。例えば、あなたのユースケースが、解約する可能性が最も高いEU地域のユーザーをターゲットにする場合、すべてのユーザーに対してモデルを実行し、キャンペーンのセグメンテーションにEU地域のフィルターを含めることができる。
...
機械学習モデルはユーザーのイベント尤度スコアを生成し、そのスコアはここで選択したスケジュールに基づいて更新される。イベントの可能性スコアに基づいてユーザーをターゲティングできるようになる。 

役立つと思われる**更新の最大頻度**を選択します。例えば、購入を予測し、毎週プロモーションを送る予定なら、更新頻度を「**毎週**、好きな曜日と時間に」に設定する。
...
**Build Prediction**をクリックすると、モデルを生成するプロセスが開始されます。データの量によっては、30 分から数時間かかる場合があります。この予測では、モデル構築プロセスの間、トレーニングが進行中であることを説明するページが表示されます。Brazeモデルは、カスタムイベント、購入イベント、キャンペーンインタラクションイベント、セッションデータを考慮に入れている。
