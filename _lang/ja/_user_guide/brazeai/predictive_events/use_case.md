...
トレーニング終了後、Brazeは対象となる各ユーザーに[イベント可能性スコア]({{site.baseurl}}/user_guide/brazeai/predictive_events/analytics/#purchase_score)（0～100）を割り当てる。ジョーダンはこのスコアを使って、アクション可能なセグメンテーションを作成する。1つは割引を必要としないハイインテントユーザー、もう1つはサポートなしではコンバージョンしない可能性の高いユーザーである。
