...
]({% image_buster /assets/img/recs/catalog_items.png %})Braze のカタログの例。()
...
]({% image_buster /assets/img/recs/content_card_preview.png %})コンテンツカードキャンペーンのメッセージコンポーザーの例。()
...
]({% image_buster /assets/img/recs/on_click_behavior.png %})作成画面でのクリック時動作ブロックの例。{: style="max-width:60%;"}()<br><br>
...
]({% image_buster /assets/img/recs/sample_response.png %})例の最終呼び出しで生成されたレストランリストのレンダリング。{: style="max-width:30%;"}()
{% endtab %}
{% endtabs %}
