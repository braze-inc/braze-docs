...
- フィールドは商品の配列の中にあってもいいし、IDの配列で終わってもいい。いずれの場合も、各製品IDは、同じタイムスタンプを持つ個別の連続したイベントとして扱われる。
...
イベント・プロパティが複数の製品を配列に含む場合、各製品IDは個別の連続したイベントとして扱われる。このイベントは、プロパティ`products.sku` 、サンプルカタログの1番目と3番目のアイテムに一致させることができる。
...
商品IDがオブジェクトではなく配列の値である場合も、同じ記法を使うことができ、各商品IDは個別の連続したイベントとして扱われる。これは、サンプル・カタログの1番目と3番目の項目に一致するようにプロパティを`purchase.product_skus` として設定することで、次のイベントでネストされたオブジェクトと柔軟に組み合わせることができる。
...
| モデルの再トレーニング頻度  | 毎月                               | 毎月                                   |
| 最大推奨モデル | 各タイプ<sup>2につき</sup>1モデル | 各タイプ100モデル<sup>2</sup> |
