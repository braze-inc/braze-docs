...
しかし、エージェントは試行錯誤を通じて学習することはなく、測定し最大化しようとする最終的なマーケティングゴールのアイデアもない。一般的にコンバージョンにつながるコピーを書くように指示したとしても、エージェントが書いたコピーがコンバージョンに与える影響を "モニター "し、そのデータを今後のエージェント呼び出しに反映させる仕組みはない。これは報酬ベースのAIデシジョニングではなく、"バイブ "デシジョニングと考えることができる。
...
| 用語 | 定義 |
| --- | --- |
| [モデル]({{site.baseurl}}/user_guide/brazeai/agents/reference/#models) | エージェントの「頭脳」、この場合は大規模言語モデル（LLM）だ。入力を解釈し、レスポンスを生成し、推論を行う。より強力なモデル（より関連性の高いデータでトレーニング）は、エージェントをより有能で万能なものにする。 |
| [指示]({{site.baseurl}}/user_guide/brazeai/agents/reference/#writing-instructions) | あなたがエージェントに与えるルールやガイドライン（システムプロンプト）。これらは、エージェントが実行されるたびにどのように振る舞うべきかを定義する。明確な指示は、エージェントの信頼性と予測性を高める。 |
...
| [出力フォーマット]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents/#step-3-select-the-output) | エージェントのレスポンスの定義済みデータ構造。 |
| [温度]({{site.baseurl}}/user_guide/brazeai/agents/reference/#temperature) | エージェントのアウトプットの偏差レベル。これは、エージェントがどれだけ正確で創造的であるかを定義する。 |
...
### 私のデータはどのように利用され、Brazeが提供するLLMに送られるのか？
...
Brazeが提供するBraze Agent用のLLM（識別子「Auto」）は、Google Geminiのモデルを使用している。Googleは、Brazeを通じて送信されたインプットとアウトプットを55日間保持し、その後データは削除される。
