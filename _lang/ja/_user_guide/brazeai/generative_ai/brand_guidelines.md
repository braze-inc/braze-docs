...
![「ブランドガイドラインを適用」のドロップダウンが展開され、「ブランドガイドラインを作成」ボタンがフォーカスされている状態。]({% image_buster /assets/img/ai_copywriter/create_brand_guideline_button.png %}){:style="max-width:75%"}
...
![メール件名用の春のセールプロモーションで、ブランドガイドラインをテストする。]({% image_buster /assets/img/ai_copywriter/test_brand_guidelines.png %})
