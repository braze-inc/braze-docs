...
]({% image_buster /assets/img/ai_liquid/ai_assistant_icon.png %})AI アシスタントが示されているメッセージ作成画面。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img/ai_liquid/use_app_activity.png %})アプリのアクティビティを使用」プロンプトの出力例{: style="max-width:45%;"}()
{% endtab %}

{% tab add countdown %}
このプロンプトは、イベントが開始されるまでの時間を含むメッセージを送信する Liquid コードを生成します。イベント日時の詳細を入力するように要求されます。

]({% image_buster /assets/img/ai_liquid/add_countdown.png %})「カウントダウンの追加」プロンプトの出力例。{: style="max-width:45%;"}()
{% endtab %}

{% tab inspire me %}
このプロンプトは、メッセージボックスに内容がある場合に表示されます。Liquid でメッセージをパーソナライズするために選択できるオプションのリストが生成されます。 

]({% image_buster /assets/img/ai_liquid/inspire_me.png %})「インスピレーションの指定」プロンプトの出力例。{: style="max-width:45%;"}()
{% endtab %}

{% tab improve my liquid %}
このプロンプトは、メッセージ作成画面に内容がある場合に表示されます。アシスタントでコードをより効率的に読みやすくする場合に選択します。

]({% image_buster /assets/img/ai_liquid/improve_my_liquid.png %})「Liquid を改善」プロンプトの出力例。{: style="max-width:45%;"}()
{% endtab %}
{% endtabs %}

Liquid コードを生成するには、[**作成ツールを更新**] を選択します。

]({% image_buster /assets/img/ai_liquid/ai_assistant_window.png %})プロンプトが表示された AI アシスタントウィンドウ。{: style="max-width:50%;"}()
