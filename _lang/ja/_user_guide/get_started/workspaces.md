...
- [チーム](#teams)
- [会社のユーザー権限](#company-user-permissions)（ただし会社のユーザーではない）
...
<table>
    ...
    <tr>
        ...
        <td>人為的なミスが発生する可能性が高くなる。誤って複数のアプリインスタンスにまたがるユーザーをターゲットにする可能性があります。<br><br>アプリ内メッセージを送信するには、あるキャンペーンが誤って別のアプリに表示されないように、アプリ固有のカスタムイベントが必要です。例えば、<code>app_1_action</code> 対 <code>app_2_action</code>.</td>
    </tr>
    <tr>
        <td>カスタムイベントと属性</td>
        ...
        <td>カスタム属性とイベントはすべてのアプリインスタンスに適用されるため、ユーザープロファイルのどのデータとどのアプリインスタンスが関連しているのかを判断しにくくなる可能性がある。例えば、それはアプリ1かアプリ2のどちら"date_of_parking"に関連するのか？これに対処するには、適切に構造化された命名規則を使用してください。</td>
    </tr>
    <tr>
        <td>フリークエンシーキャップ</td>
    ...
    <tr>
        ...
        <td>以下のサポート対象チャネルについて、<a href='{{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/copying_across_workspaces/'>ワークスペース間でキャンペーンを複製</a>できる：SMS、アプリ内メッセージ、メール、メールテンプレート、およびコンテンツブロック。<br><br>セグメント、キャンペーン、キャンバスを複製して、アプリインスタンス間でコンテンツを再利用できる。</td>
        <td>該当なし</td>
    </tr>
    <tr>
        <td>分析</td>
...
ワークスペースプランを決定したら、ワークスペースを作成し、アプリインスタンスを追加します。ステップについては、[「ワークスペースの作成と管理」]({{site.baseurl}}/user_guide/administrative/app_settings/workspaces/)を参照せよ。
