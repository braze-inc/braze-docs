...
- **コネクテッドコンテンツのレスポンスボディ：**Brazeが永久に保管するわけではない。一時的にメモリに保持することができ、キャッシングがイネーブルメントの場合、TTL（time-to-live）でキャッシュに保存される。
...
1. バックグラウンドワーカーは、メッセージが配送される準備ができたときに、メッセージの Liquid テンプレートをレンダリングする。
...
- **インプロセス・メモリ・キャッシュ：**ワーカープロセス内の一時キャッシュ。データは、ジョブの間（ワーカータイムアウトに基づき最大11分）だけ生きることができる。
