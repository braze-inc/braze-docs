...
# [![Brazeラーニングコース]](https://learning.braze.com/connected-content)([{% image_buster /assets/img/bl_icon3.png %})](https://learning.braze.com/connected-content){: style="float:right;width:120px;border:0;" class="noimgborder"}コネクテッドコンテンツAPIコールの作成
...
Braze はメッセージを非常に高速で配信するため、サーバーが数千の同時接続を処理できるようにして、コンテンツをプルダウンするときにサーバーが過負荷状態にならないようにしてください。パブリック API を使用する場合は、API プロバイダーが使用するレート制限に違反していないことを確認してください。Brazeは、パフォーマンス上の理由から、サーバーのレスポンスタイムが2秒未満であることを要求している。サーバーのレスポンスに2秒以上かかる場合、コンテンツは挿入されない。
...
新しい認証情報を追加するには、[**Add Credential**]を選択する。認証情報に名前を付け、ユーザーの名前とパスワードを入力します。
...
トークンが保存された後、そのトークンは、リクエスト を認可するために、その後のコネクテッドコンテンツ呼び出しにダイナミックなテンプレー トとして組み込むことができる：
...
Brazeは、メッセージペイロードをレンダリングするためにコネクテッドコンテンツAPIコールを行う必要があるため、受信者ごとに同じコネクテッドコンテンツAPIコールを複数回行うことがある。メッセージペイロードは、検証、再試行ロジック、その他の内部目的のために、受信者ごとに複数回レンダリングすることができる。

コネクテッド・コンテンツAPIの呼び出しは、再試行ロジックが使用されていない場合でも、受信者ごとに複数回行われることが予想される。コネクテッドコンテンツを含むメッセージのレート制限を設定するか、予想される量を処理できるようにサーバーを設定することをお勧めする。

### コネクテッドコンテンツでレート制限はどのように機能するのか？

コネクテッドコンテンツにはレート制限がない。その代わり、レート制限はメッセージ送信レートに基づいている。コネクテッドコンテンツの通話がメッセージの送信よりも多い場合は、メッセージングレートの制限をコネクテッドコンテンツのレート制限よりも低く設定することを推奨する。  

### キャッシュ動作とは何か？

デフォルトでは、POSTリクエストはキャッシュしない。しかし、`:cache_max_age` パラメータを追加することで、POSTコールを強制的にキャッシュさせることができる。
