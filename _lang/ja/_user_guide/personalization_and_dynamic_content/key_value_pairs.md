...
Braze では、`extras` として知られるカスタム定義の文字列キーと値のペアを送信できます。APIトリガー型およびスケジュールされたAPIトリガー型キャンペーンでエクストラにアクセスするには、ダッシュボードでキーを  、値を  "example_key",に設定する{% raw %}`"$json:{"foo": 1, "bar": 1}"`{% endraw %}。これにより、開発者コンソールに `"extras": { "test": { "foo": 1, "bar": 1 }` が出力されます。
...
[従来のエディタ]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/)でアプリ内メッセージにキーと値のペアを追加するには、**設定**タブを選択し、**新規ペアの追加**を選択し、キーと値のペアを指定する。

{% alert note %}
アプリ内メッセージでは、キーと値のペアをドラッグ＆ドロップエディタで設定することはできない。
{% endalert %}
