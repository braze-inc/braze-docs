...
[従来のエディターで]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/)アプリ内メッセージにキーと値のペアを追加するには、**設定**タブを選択し、**新しいペアを追加を**選択し、キーと値のペアを指定する。

{% alert note %}
キーと値のペアは、アプリ内メッセージのドラッグ＆ドロップエディターでは設定できない。
{% endalert %}
