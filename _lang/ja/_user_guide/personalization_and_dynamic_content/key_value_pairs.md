...
]({% image_buster /assets/img_archive/keyvalue_email.png %})Brazeのメールメッセージ作成画面の [送信情報] タブ。()

## コンテンツカードによって促進された

コンテンツカードにキーと値のペアを追加するには、Braze のメッセージ作成画面の [**設定**] タブに移動し、[**新しいペアを追加**] を選択します。

]({% image_buster /assets/img_archive/kvp_content_cards.png %})コンテンツカードにキーと値のペアを追加{: style="max-width:70%;"}()
