...
> プロモーションコード-プロモコードとも呼ばれる-は、購買に重点を置いたインタラクションを促進することで、ユーザーを引き留める素晴らしい方法である。

Braze Liquidの機能により、プロモーションコードを簡単に使用できるようになり、メッセージは自動的に直感的に提供されたプロモーションリストから取得できるようになった。プロモーションコード機能は、最長6ヶ月の有効期限と、1リストあたり最大20MMの個別コードをサポートする。
...
{% alert note %}
[古いナビゲーションを]({{site.baseurl}}/navigation)使用している場合は、「**統合」の**下に**プロモーション・コードが**ある。
{% endalert %}
...
次に、プロモーション・コードのコード・スニペットを作成する。このコードスニペットは、特定のプロモーションコードを表示するためにLiquidで参照するものになる。他のリストですでに使われていないコード・スニペットであることを確認する。
...
また、オプションでカスタマイズした閾値アラートを設定するオプションもある。これらのアラートを設定すると、リスト内の利用可能なプロモーションコードが少なくなったとき、またはプロモーションコードリストの有効期限が近づいたときに、指定された受信者にEメールが送信される。受信者には1日1回通知される。

![][5]

### ステップ 4:プロモーションコードのアップロード

Brazeはコードの作成や償還を管理しない。そのため、プロモーションコードをCSVファイルに生成し、Brazeにアップロードする必要がある。[Voucherifyとの]({{site.baseurl}}/partners/channel_extensions/loyalty/voucherify/)ビルトイン統合または [Talon.One]({{site.baseurl}}/partners/channel_extensions/loyalty/talonone/)を使用してプロモーションコードを作成し、エクスポートすることができる。各行にコードが1つしかないことを確認する。

{% alert note %}
最大ファイルサイズは100MB、最大リストサイズは20MMの未使用コードである。間違ったファイルがアップロードされた場合は、新しいファイルをアップロードすれば、前のファイルは置き換えられる。
{% endalert %}
...
保存を選択すると、**インポート履歴に**新しい行が表示される。インポートが完了したかどうかを確認するために表を更新するには、表の上部にある<span style="font-size: 14px;margin-bottom: .5rem;height: 16px;width: 16px;" class="fas fa-sync" ></span> **Syncを**選択する。

![][8]

{% alert note %}
大きなファイルはインポートに数分かかる。待っている間、インポートが進行している間、ページを離れて何か作業をすることができる。インポートが完了すると、表のステータスが**Completeに**変わるのがわかる。
{% endalert %}
...
リスト内のすべてのコードは、輸入日に関係なく、同じ有効期限を持つ。
...
Liquid][11] を使って、アップロードされたCSVファイルからユニークなプロモーションコードをメッセージに挿入することができる。そのコードはBrazeのバックエンドで送信済みとマークされ、他のメッセージが同じコードを送信しないようにする。 

マルチチャネルキャンペーンやキャンバスステップでコードスニペットが使用されると、各ユーザーは常にユニークなコードを受け取る。キャンバスのさまざまなステップで、各ユーザーは複数のプロモーションコードを受け取る。 

特定のユーザーが複数のチャネルを通じてコードを受け取る資格がある場合、そのユーザーは各チャネルを通じて同じコードを受け取る。例えば、あるユーザーが2つのチャンネルを通じて2つのメッセージを受け取った場合、そのユーザーは1つのコードしか受け取らない。1つのコードが送信され、ユーザーは2つのチャンネルを通じてこのコードを受信する。例えば、マルチチャンネルのキャンバス・ステップの場合、ユーザーが使用するコードは1つだけである。
...
デフォルトでは、テスト送信とシードグループのメール送信では、テスト送信ごとにユーザーごとのプロモーションコードが使用される。しかし、Brazeのアカウントマネージャーに連絡すれば、テスト中にプロモーションコードを使用しないようにこの動作を更新することができる。
...
同じプロモーションコードが、メッセージ内のリキッドスニペットのすべてのインスタンスにテンプレート化される。
