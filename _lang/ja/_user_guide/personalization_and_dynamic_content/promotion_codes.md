...
Brazeはコードの作成や償還を管理しない。そのため、プロモーションコードをCSVファイルに生成し、Brazeにアップロードする必要がある。[Voucherify]({{site.baseurl}}/partners/channel_extensions/loyalty/voucherify/) との組み込み統合または [Talon.One]({{site.baseurl}}/partners/channel_extensions/loyalty/talonone/) を使用して、プロモーションコードを作成しエクスポートすることができます。各行にコードが1つしかないことを確認する。
