...
3. グローバル・カタログ設定が構成されていない場合、通知のトリガーに使用されるカスタム・イベントとプロパティをセットアップするよう促される：
    ...
    ```
{% alert note %}
再入荷トリガーと値下げトリガーでは、ユーザーの通知のサブスクリプション登録に同じイベントを使用します。`type` を`price_drop` に設定して、価格低下通知を作成します。価格の下落と在庫通知の戻しの両方を設定することはできません。
{% endalert %}
...
5. 通知ルールを設定する。2 つのオプションがあります。
    ...
    \- [**一定の間隔 (分単位) で一定数のユーザーに通知**] は、設定した通知間隔ごとに、指定した数の顧客に通知します。Braze は、通知する顧客がいなくなるまで、または商品の価格が再び上がるまで、指定された数の顧客に段階的に通知します。通知速度は 1 分あたり 10,000 ユーザーを超えることはできません。
