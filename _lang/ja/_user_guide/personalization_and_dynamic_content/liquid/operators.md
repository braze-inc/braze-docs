...
この表は、サポートされている演算子の一覧である。かっこはLiquidでは無効な文字であり、タグが動作しないことに注意。
...
今、ユーザーの "Total Spend "カスタム属性が`0` より大きい場合、メッセージが表示される：
...
{: start="3"}
3\.`elsif` タグと "does not equal" (`!=`) および "and" (`and`) 演算子を使用して、ユーザーが最近のゲームを持っているかどうか（値が空白でないことを意味する）、そのゲームが*「Awkward Dinner Party*」や「*Proxy War 3」でないかどうかをチェックする：War of Thirst』*のフィードバックを測定する。次に、これらのユーザーに送信するメッセージを作成します。
