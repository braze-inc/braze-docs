...
{% raw %}
- `dispatch_id` Brazeはキャンバスステップを「スケジュールされた」場合でもトリガーイベントとして扱うため、動作が異なる（スケジュール可能なエントリステップは例外）。詳細については、[ディスパッチ ID の動作][50]を参照してください。
...
このユースケースでは、空白またはNULLの名字を持つユーザーは、"Thanks for downloading "というメッセージを受け取る。顧客が間違ってもLiquidを見ないように、名には[デフォルト値][47] ]を入れるべきである。
...
そして、`converse_viewer` がtrueのときにセール・メッセージを送る。それ以外の場合は、メッセージを中止します。
