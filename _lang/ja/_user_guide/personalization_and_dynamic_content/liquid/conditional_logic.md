...
`{% endif %}` タグは、条件ロジックの終了を知らせる。条件付きロジックを含むメッセージには、`{% endif %}` タグを含めなければならない。条件ロジックに`{% endif %}` タグを含めないと、Brazeがメッセージを解析できないのでエラーになる。
...
NULLまたはnil値は、カスタム属性の値が設定されていない場合に発生する。例えば、まだ名字を設定していないユーザーは、Brazeに名字が記録されない。
