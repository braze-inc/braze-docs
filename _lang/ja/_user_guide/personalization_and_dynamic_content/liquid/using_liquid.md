...
]({% image_buster /assets/img_archive/insert_var_shot.gif %})パーソナライゼーション追加モーダルの GIF。この GIF は、ユーザーがデフォルト値として「fellow traveler」を挿入すると、モーダルが作成画面で強調表示されているテキスト「name」が Liquid スニペットに置き換えられ様子を示す。()
