...
![メッセージの例：「この春、私たちの特別オファーで素敵なものを自分にご褒美を」、コード・スニペットが続く。]({% image_buster /assets/img/promocodes/promocode10.png %}){: style="max-width:70%"}
...
{% alert note %}
プロモーションコードがない場合、コードに依存するテストメッセージやライブメッセージは送信されない。
{% endalert %}
