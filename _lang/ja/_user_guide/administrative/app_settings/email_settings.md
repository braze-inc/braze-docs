...
{% alert tip %}
サブスクリプションページまたはフッターの**プレビュー**セクションで、**プレビューリンクをコピー**するを選択すると、ランダムなユーザーのメールフッター、配信停止ページ、またはオプトインページがどのように見えるかを示す共有可能なプレビューリンクが生成され、コピーされる。リンクは7日間持続し、その後再生が必要となる。
{% endalert %}
