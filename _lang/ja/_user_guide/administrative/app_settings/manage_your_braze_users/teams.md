...
> Braze 管理者は、ダッシュボードユーザーを、さまざまなユーザーロールと権限を持つチームにグループ化できます。
...
[**設定**] > [**内部チーム**] に移動して、[<i class="fas fa-plus"></i> **チームを追加**] をクリックします。
...
[**チーム名**] を入力します。必要に応じて、[**チームを定義**] フィールドを使用してカスタム属性、場所、または言語を選択し、チームがアクセスできるユーザーデータをさらに定義します。例えば、ユースケースの 1 つとして、カスタム属性で識別されるテストユーザーのみにアクセスできる開発チームを作成して、[チームによるテスト](#testing-with-teams)を実行できます。
...
{% alert note %}
[古いナビゲーション]({{site.baseurl}}/navigation)を使用している場合は、アカウントアイコンを選択し、[**ユーザーの管理**] をクリックすると、このページが表示されます。
{% endalert %}

次に、次の手順を実行します。

1. <i class="fa fa-edit"></i> [**編集**] をクリックします。
...
![キャンペーンへのチームタグの追加][3]{: style="max-width:70%;"}
...
![Braze 内のオブジェクトに関連付けられたチームのアーカイブ][86]{: style="max-width:70%;"}

Braze 管理者は、アーカイブ済みのチームを選択し、[**アーカイブ解除**] をクリックすることで、チームをアーカイブ解除できます。
