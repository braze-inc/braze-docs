...
]({% image_buster /assets/img_archive/internal_group.png %})「Email test group」という名前の内部グループ。()
...
]({% image_buster /assets/img_archive/internal_group_add_user.png %})新しい内部グループを作成するときの内部グループ設定()
...
]({% image_buster /assets/img_archive/content_test_preview.png %})コンテンツテストグループを選択するためのアプリ内メッセージエディターの [テスト] セクション。{: style="max-width:60%" }()
...
]({% image_buster /assets/img_archive/seed_group_campaign.png %})バリアント1のメールキャンペーンの送信先として「Email seed test」というシードグループが選択されている。()
