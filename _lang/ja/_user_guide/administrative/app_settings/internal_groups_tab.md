...
{% alert tip %}
また、[Quality Assurance and Debugging Tools](https://learning.braze.com/quality-assurance-and-debugging-tools-in-the-dashboard/) Braze Learning コースをチェックすることをお勧めします。このコースでは、内部グループを使用して独自のトラブルシューティングとデバッグを実行する方法について説明します。
{% endalert %}

## 前提条件

内部グループを作成および管理するには、ワークスペースの[Access Dev Console permission]({{site.baseurl}}/user_guide/administrative/manage_your_braze_users/user_permissions/#limited-and-team-role-permissions) が必要です。
...
!["Email test group".]({% image_buster /assets/img_archive/internal_group.png %})という名前の内部グループ
...
| **ユーザーを一括追加**      | メールアドレスまたは外部ID のリストをコピーして貼り付けます。ダッシュボードでは既知のユーザーのみを追加できます。詳細については、[ユーザーインポート]({{site.baseurl}}/user_guide/data/user_data_collection/user_import/)を参照してください。          |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

![新しい内部グループを作成するときの内部グループ設定]({% image_buster /assets/img_archive/internal_group_add_user.png %})
...
![アプリ内メッセージエディタのTest セクションで、Content Test Group.]({% image_buster /assets/img_archive/content_test_preview.png %})を選択します。{: style="max-width:60%" }
...
シードメールには、メールの件名行の先頭に`[SEED]` が追加されます。シードメール**do not**:
...
キャンペーンが多変量で、バリアントの送信割合が0% の場合、シードグループには送信されません。また、バリアントが送信済みで、****Target**ステップのSeed Groups**を編集するように更新されていない場合、デフォルトでは再送信されません。

{% alert note %}
定期的なキャンペーンがあり、いずれかのバリアントが更新された場合は、更新されたバリアントのみまたはすべてのバリアントに再度送信するか、更新時にSeed Group の送信をオフにすることができます。
{% endalert %}

!["Email seed test" Seed Group Variant 1 メールキャンペーンを送信するように選択されたSeed Group。]({% image_buster /assets/img_archive/seed_group_campaign.png %})
