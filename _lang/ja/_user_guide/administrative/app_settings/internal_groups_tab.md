...
シード送信は内部QAとレビューのために設計されているため、シードされたダッシュボードユーザーのサブスクリプションチェックを意図的にバイパスする。つまり、有効なメールアドレスを持つユーザーで、Seed Groupに属しているユーザーは、たとえサブスクライバーでなくてもメッセージを受け取ることになる。ただし、メッセージはそのグループにシードコピーを送るように設定されていなければならない。
