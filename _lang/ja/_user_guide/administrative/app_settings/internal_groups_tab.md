...
| グループのタイプ     | ユースケース     |
| :------------- | :------------- |
| ユーザーイベントグループ |テストデバイスからのイベントまたはログの検証に使用します。 |
| コンテンツテストグループ |テストリストと同様の概念です。プッシュメッセージ、メールメッセージ、およびアプリ内メッセージで使用して、メッセージのレンダリングされたコピーを送信できます。 |
...
![新規の内部グループを作成するときの内部グループ設定][8]

| 追加方法 | 説明 |
| :------------- | :------------- |
| 識別されたユーザー | 外部ユーザー ID またはメールアドレスでユーザーを検索します。|
| 匿名ユーザー | IP アドレスで検索します。次に、追加する各テストユーザーの名前を指定します。これは、[[イベントユーザーログ]({{site.baseurl}}/user_guide/administrative/app_settings/developer_console/event_user_log_tab/)] ページですべてのイベントログが関連付けられる名前です。|
| ユーザーを一括追加 | メールアドレスまたは external ID のリストをコピーして、表示されたセクションに貼り付けます。ダッシュボードでは既知のユーザーのみを追加できます.詳細については、「[ユーザーインポート]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_import/)」を参照してください。 |
{: .reset-td-br-1 .reset-td-br-2}
...
![コンテンツテストグループへのテスト送信][9]{: style="max-width:50%" }
...
内部グループを作成し、シードグループとして使用するタグを付けたら、キャンペーンコンポーザーの「**ターゲットユーザー**」ステップまたはキャンバスの「**送信設定**」ステップで選択できます。シードメールには、メールの件名行の先頭に識別子 `[SEED]` が追加されます。シードメールを送信しても、ダッシュボード分析で送信数が増加せず、ユーザープロファイルの [**受信したキャンペーン**] リストが更新されないことに注意してください。
