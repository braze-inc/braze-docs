---
nav_title: S3 でエクスポートされるセキュリティイベント
...
> セキュリティイベントは、クラウドストレージプロバイダであるAmazon S3 に自動的にエクスポートできます。日次ジョブはUTC の午前0 時に実行されます。設定アップ後は、ダッシュボードからセキュリティイベントを手動でエクスポートする必要はありません。ジョブは、過去 24 時間のセキュリティイベントを CSV 形式で設定した S3 ストレージにエクスポートします。CSVファイルは、手動でエクスポートしたレポートと同じ構成になっています。

{% alert note %}
10,000 行の制限アプリは、ダッシュボードからの手動CSV レポートダウン読み込むのみにあります。S3 へのセキュリティイベントエクスポートは、この行制限の対象にはなりません。
{% endalert %}
...
### ステップ1:アイデンティティとアクセスマネジメント(IAM)ユーザーの作成

シークレットアクセスキーとアクセスキー ID を取得するには、[ AWS アカウント ](https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started-account-iam.html#create-an-admin) の手順に従って IAM ユーザーを作成する必要があります。
...
1. **IAM** (Identity and Access Management) > **Policies** > **Create Policy** に移動して、ユーザーの権限を追加します。 
...
!["Security Event Down 読み込む"ページには、入力されたBrazeアカウントとBrazeの外部ID が表示されます。]({% image_buster /assets/img/security_export/security_event_download1.png %})

{: start="4"}
4\.[**変更内容を保存**] を選択します。 

!["Save changes"ボタン。]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:50%;"}
...
2. AWS で、**IAM** (Identity and Access Management) セクション> **Policies** に移動し、**Create Policy** を選択します。

![ポリシーとボタンのリストが"Create policy" のページ。]({% image_buster /assets/img/security_export/policies.png %})
...
!["Security Event Down 読み込む"ページには、入力されたBrazeアカウントとBrazeの外部ID が表示されます。]({% image_buster /assets/img/security_export/security_event_download2.png %})

4. AWS コンソールで、**IAM** (アイデンティティとアクセスマネジメント) セクション> **Roles**> **Create Role** に移動します。 
5. 信頼できるエンティティセレクターのタイプとして [**別の AWS アカウント**] を選択します。 
6. BrazeのアカウントID を入力し、[**外部 ID が必要**] をオンにして、Braze external ID を入力します。 
7. 完了したら [**次へ**] を選択します。

![信頼できるエンティティタイプを選択し、AWSアカウントに関する情報を提供するためのオプションを含むページ。]({% image_buster /assets/img/security_export/select_trusted_entity.png %})
...
![名前、説明、信頼ポリシー、権限、タグなど、ロールの詳細を提供する項目。]({% image_buster /assets/img/security_export/name_review_create.png %})
...
!["Security Event Down 読み込む" &quot のトグルがオンのセクション;AWS S3&quot にエクスポート;]({% image_buster /assets/img/security_export/security_event_download3.png %})
...
!["Save changes"ボタン。]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:40%;"}
