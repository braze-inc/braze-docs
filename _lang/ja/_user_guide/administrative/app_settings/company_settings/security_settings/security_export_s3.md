---
nav_title: S3でセキュリティ・イベントをエクスポートする
...
> セキュリティ・イベントをクラウド・ストレージ・プロバイダーのAmazon S3に自動的にエクスポートすることができる。設定後は、ダッシュボードからセキュリティイベントを手動でエクスポートする必要はない。このジョブは、設定したS3ストレージに過去24時間のセキュリティイベントをCSV形式でエクスポートする。CSVファイルは、手動でエクスポートされたレポートと同じ構造を持つ。
...
BrazeアカウントとBraze外部IDを入力した "Security Event Download "ページ。]({% image_buster /assets/img/security_export/security_event_download1.png %})

{: start="4"}
4\.[**変更内容を保存**] を選択します。 

変更を保存」ボタンをクリックする。]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:50%;"}
...
ポリシーのリストと "Create policy "のボタンがある。]({% image_buster /assets/img/security_export/policies.png %})
...
方針を検討し、作成するためのページ。]({% image_buster /assets/img/security_export/review_and_create.png %})
...
BrazeアカウントとBraze外部IDを入力した "Security Event Download "ページ。]({% image_buster /assets/img/security_export/security_event_download2.png %})
...
信頼できるエンティティタイプを選択し、AWSアカウントに関する情報を提供するためのオプションがあるページ。]({% image_buster /assets/img/security_export/select_trusted_entity.png %})
...
種類と説明の列を持つポリシーのリストである。]({% image_buster /assets/img/security_export/add_permissions.png %})
...
変更を保存」ボタンをクリックする。]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:40%;"}
