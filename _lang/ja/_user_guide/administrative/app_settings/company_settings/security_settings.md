...
社内の全ユーザーに2 要素認証が義務付けられている。これにより、アカウントログに2 段階目のID 認証が追加され、ユーザーの名前とパスワードよりも安全になります。ダッシュボードが2 要素認証に対応できない場合は、顧客のサクセスマネージャーにお問い合わせください。 
...
まず、「**会社設定」**>「**セキュリティ設定**」>「**2 要素認証**」と進み、ダッシュボードで 2FA がイネーブルメントになっているかどうかを確認する。トグルがグレーの場合、あなたの会社では2FAがオンになっておらず、会社の全ユーザーに義務付けられていない。
...
{% alert important %}
2FAは、シングルサインオン（SSO）がイネーブルメントされていない場合にのみ、すべての企業ユーザーに要求される。SSOが使用されていれば、2FAを会社レベルで実施する必要はない。
{% endalert %}
...
**PIIの閲覧**権限は、一部の選ばれた会社のユーザーしかアクセスできない。デフォルトで、すべての管理者はユーザー権限で [**PII を表示**] 権限が有効になっています。つまり、ダッシュボード全体で、あなたの会社がPIIとして定義したすべての標準およびカスタム属性を見ることができる。ユーザーに対してこの権限をオフにすると、そのユーザーはこれらの属性を見ることができなくなる。
...
以下の属性をPIIとして指定し、**PIIの表示**権限を持たない企業ユーザーから非表示にすることができる。
...
以下では、すべてのフィールドがPIIとして設定され、言及されたユーザーはBrazeプラットフォームを使用する企業ユーザーであると仮定している。また、"先行する "属性とは、[潜在的PII属性](#potential-pii-attributes)表にあるものを指す。ユーザーからPII権限を削除することは、これらの列挙された領域以外にもユーザビリティに影響を与える可能性がある。
...
![データ削除の環境設定」セクションで、ワークスペースごとにデータ削除の環境設定をカスタマイズするためのトグルがオンになった。]({% image_buster /assets/img/deletion_preferences_1.png %})
...
問題が解決しない場合は、Authyアプリで古いプロファイルを削除し、再度QRコードをスキャンして2FAを設定する。再セットアップを試みる前に、広告ブロッカーを無効にしているか、サードパーティのCookieをイネーブルメントにしているか、別のブラウザを使用していることを確認すること。