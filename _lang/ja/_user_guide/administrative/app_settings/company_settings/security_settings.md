...
![[このアカウントを 30 日間記憶する] チェックボックス][04]{: style="float:right;max-width:40%;margin-left:15px;"}
...
2 要素認証でのログインに問題があるユーザーは、会社の管理者に連絡して 2 要素認証をリセットすることができる。これを行うには、管理者に以下のステップを実行してもらう：
...
| カスタムイベント | プロパティ |  |
| 購入イベント | プロパティ |  |
| メッセージ送信 | message_extras | message_extras フィールドを含むイベントタイプがいくつかあります。この環境設定は、メッセージエクストラをサポートするすべてのメッセージ送信イベントタイプ (将来追加されるイベントタイプを含む) に適用されます。 |
