...
設定時には、アサーションコンシューマーサービス（ACS）のURLを入力するよう求められる。  
...
5. RelayStateフィールドに、生成されたRelay State API キーを入力して設定する。

{% alert important %}
**サインオンURL**フィールドを設定**してはならない**。このフィールドは空白のままにしておくと、IdP開始のSAMLシングルサインオンで問題が発生するのを防げる。
{% endalert %}
