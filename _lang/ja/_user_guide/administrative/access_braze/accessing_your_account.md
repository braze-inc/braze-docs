...
- カスタマーサクセスマネージャーまたはアカウントマネージャーに連絡し、各ダッシュボードの`saml_sso_custom_entity_id` 機能フリッパーをイネーブルメントする。
...
![ダッシュボードにログインすると、"パスワードをお忘れですか？"というプロンプトが表示される。]({% image_buster /assets/img_archive/enable_reset.png %}){: style="max-width:60%"}
