...
{% alert tip %}
完全に一致するテキストを検索するには、検索語を引用符 ("") で囲みます。たとえば、\["all ユーザー s"] を検索すると、名前に"all ユーザー s" という完全句を含むすべての項目が返されます。
{% endalert %}
...
**Show active and 下書き only**を選択すると、検索結果にアクティブコンテンツと下書きコンテンツを含めることができます。デフォルトでは、切り替えが有効になっており、アーカイブされた内容を含むすべての内容が表示されます。
