...
1. **ユーザープロファイル** **[ユーザー検索]({{site.baseurl}}/user_guide/engagement_tools/segments/user_profiles/)**ページのBraze ダッシュボードから個々のユーザープロファイルにアクセスできます。Eメールアドレス、電話番号、または外部ユーザーIDを介して）ユーザーのプロフィールを見つけた後、**Engagement**タブを選択してユーザーの購読状態を表示し、手動で調整することができる。
...
{% alert note %}
Android プッシュ通知を傍受できるのは、アプリがフォアグラウンドまたはバックグラウンドにある場合(ただし、実行中の場合)のみです。アプリが終了または完全に強制終了されたときに通知s を傍受することはできません。
{% endalert %}
...
{% alert note %}
iOSでは、アプリがプッシュ通知を表示する前にプッシュ通知を傍受することは許可されていません。つまり、アプリ s (およびBraze) は、通知の表示/非表示を切り替えてもコントロールできません。ユーザーは、装置設定sでアプリのためにプッシュ通知sをオプトアウトできますが、それはオペレーティングシステムによってコントロール主導されます。
{% endalert %}
