...
メールメッセージをキャンペーンとキャンバスのどちらで送るべきか迷っている？キャンペーンは単一のターゲットメッセージングキャンペーンに適しており、キャンバスはマルチステップユーザージャーニーに適している。
...
![メールの [言語] の値の設定]({% image_buster /assets/img/dnd/language_setting_dnd.png %})({: style="max-width:70%;"})
...
`<head>` タグの外側に追加されたものは、メールの`<body>` タグの後に追加される。これは、追加されたコンテンツがメールに表示されることを意味する。

##### タグによって許可されるタグと属性

| タグ名 | 説明 | 例 |
| --- | --- | --- |
| `base` | メッセージ内のすべての相対 URL のベース URL を指定する。 | `<base href="https://example.com" target="_blank">` |
| `link`| メッセージと外部リソースとの関係を定義する。 | `<link href="styles.css" rel="stylesheet" type="text/css">` |
| `meta` | ページの説明やキーワードなどのメタデータを提供する。 | `<meta name="description" content="Free Web tutorials">` |
| `style` | 内部CSSスタイルを埋め込む。 | `<style type="text/css" media="screen">body { font-size: 16px; }</style>` |
...
| `link` | `rel` | 現在のメッセージとリンクされたメッセージの関係を定義する。 | ```<link rel="stylesheet">``` |
| `link` | `type` | リンク先のリソースのタイプ。 | ```<link type="text/css">``` |
...
| `title` | 属性なし | `title` タグは属性を受け付けない。 | ```<title>Kitchenerie</title>``` |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation" }

{% alert note %}
リンク名は最大63バイトまでで、制限を超えると自動的に切り捨てられる。
{% endalert %}
