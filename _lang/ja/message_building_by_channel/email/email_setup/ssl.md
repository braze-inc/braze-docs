...
SSLのセットアップを開始するには、Brazeカスタマーサクセスマネージャーに連絡して、Brazeメールのフルセットアップを開始する。
...
4. この時点で、CDNを設定する。Braze は、CDN 設定のトラブルシューティングに役立ちません。CDNプロバイダーに問い合わせること。
5. カスタマー・サクセス・マネージャーに連絡し、SSLを有効にする。
...
{% alert important %}
CDN設定のトラブルシューティングの詳細については、CDNプロバイダーに問い合わせる必要がある。
{% endalert %}
