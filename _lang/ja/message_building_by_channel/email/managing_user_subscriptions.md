...
1. **Audience**> **Subscription Group Management**に移動します。
...
![ユーザーsを"Lapsed ユーザー s"Segmentとユーザーsのフィルターを"Weekly Emails" サブスクリプショングループに指定します。]({% image_buster /assets/img/segment_sub_group.png %}){: style="max-width:90%"}
...
Brazeは、アーカイブされたグループのユーザーの状態変更を処理しない。たとえば、Susie がサブスクライブされている間にSubscription Group 1 をアーカイブすると、Suscription Group 1 は"subscribed" のままになります。これは、Suscription Group 1 がアーカイブされ、それを使用してメッセージを送信できないため、Susie には関係ありません。
...
### ユーザーのメール サブスクリプショングループを確認する

- **ユーザープロフィール:**個々のユーザープロフィールは、[ユーザー検索]({{site.baseurl}}/user_guide/engagement_tools/segments/user_profiles/#access-profiles)ページのBrazeダッシュボードからアクセスできます。ここでは、メールアドレス、電話番号、または外部ユーザーIDでユーザープロフィールを検索できます。**Engagement**タブでユーザーのメール サブスクリプショングループを表示することもできます。
- Braze REST API[List ユーザーのサブスクリプショングループのエンドポイント]({{site.baseurl}}/api/endpoints/subscription_groups/get_list_user_subscription_groups/)または[List ユーザーのサブスクリプショングループ ステータス エンドポイント]({{site.baseurl}}/api/endpoints/subscription_groups/get_list_user_subscription_group_status/)を使用して、個々のユーザープロファイルのサブスクリプショングループを表示します。 
...
![プレビューとクォートが付いたカスタム配信停止ページです。ごめんなさい!&クォート;。]({% image_buster /assets/img/custom_unsubscribe.png %})
...
ユーザは、{% raw %}`{{${set_user_to_opted_in_url}}}`{% endraw %} タグを使用してこのページに移動します。
...
![メッセージ&quot を含むカスタムオプトインメール。まだ、我々" から返信したいと思っていることがわかりました。]({% image_buster /assets/img/custom_optin.png %})
