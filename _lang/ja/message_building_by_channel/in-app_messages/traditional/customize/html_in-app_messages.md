...
{% alert note %}
JavaScriptのブリッジメソッドに<code>customAttributesを</code>挿入するためにLiquidを参照することはできない。
{% endalert %}
...
モバイルアプリユーザーがアプリ内**カスタムコード**メッセージを受信するには、サポートされているSDKバージョンにアップグレードする必要がある。新しいBraze SDKバージョンに依存するキャンペーンを開始する前に、モバイルアプリを[アップグレードするようユーザーに促す]({{site.baseurl}}/user_guide/engagement_tools/campaigns/ideas_and_strategies/new_features/)ことを推奨する。
...
{% alert tip %}
HTMLエディター内で<i class="fa-solid fa-magnifying-glass"></i> **検索を**選択すると、コード内部を検索することができる！
{% endalert %}
...
{% alert tip %}
`onclick` 属性内部でJavaScriptメソッドを使用する場合、二重引用符で囲まれたHTML属性との衝突を避けるために、文字列値をシングルクォートで囲む。
{% endalert %}
...
3. ボタンIDに`?abButtonId=0` を使用していた自動クリックトラッキングと、閉じるボタンの「ボディクリック」トラッキングは削除された。以下のコード例は、新しいクリック・トラッキングJavaScriptメソッドを使用するためにHTMLを変更する方法を示している：
   ...
   | 前 | その後 |
   |:-------- |:------------|
   |<code><a href="braze://close">閉じるボタン</a></code>|[閉じる] ボタン|
   |<code><a href="braze://close?abButtonId=0">閉じるボタン</a></code>|[閉じる] ボタン|
   |<code><a href="app://deeplink?abButtonId=0">トラックボタン1</a></code>|<code><a href="app://deeplink" onclick="brazeBridge.logClick('0')">トラックボタン1</a></code>|
   |<code><script><br>location.href<br></script></code>|<code><script><br>関数<br>  4\.<br>  brazeBridge.closeMessage<br><br></script></code>|
