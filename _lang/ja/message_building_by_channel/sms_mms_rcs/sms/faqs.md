...
特定の国（例えば、スウェーデンや北欧諸国）にURLを含むSMSメッセージを送信する前に、これらのURLをキャリアに登録する必要があります。Braze 顧客の保守マネージャーに連絡してください。この処理には 5 日ほどかかります。  
