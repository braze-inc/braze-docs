...
{% alert important %}
アーカイブされたユーザーは永久に削除される。<br><br>キャンバスを使って、[ユーザーアーカイブポリシーをカスタマイズ](#customizing-your-user-archival-policy)できる。 顧客は、ユーザーが非アクティブか休止状態かを完全にコントロールできます。キャンバスでは、これを自動的に行う機能を提供しているため、一部またはすべての休止状態のユーザーに対して、この機能を効果的にオフにすることができる。
{% endalert %}
...
2. 再資格付与期間を6か月弱に設定します。<br><br>
      ![エントリーコントロールで、再資格をオンにし、再資格ウィンドウを23週に設定した][3]。<br><br>
