...
24 時間の間にワークスペースで100,000 を超えるWebhook またはConnected Content エンドポイントエラー(再試行を含む) が発生した場合、エラーの解決方法に関する次の情報を含むE メールが送信されます。 
...
これらのメールは、ワークスペースレベルで1 日に1 回のみ送信されます。これらのメールにサインアップするユーザがいない場合、すべての会社の管理者に通知されます。

これらのメールを受信するためにサインアップするには、次の手順を実行します。

1. [**設定**] > [**管理者設定**] > [**通知設定**] に移動します。
2. **Connected Content Errors**および**Webhook Errors**を**Canvas &Campaigns**セクションで選択します。
