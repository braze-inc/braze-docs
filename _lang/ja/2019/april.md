...
このセクションへの修正に加え、メッセージエンゲージメントイベントページに新しい[サブスクリプションイベント]が]({{ site.baseurl}}/user_guide/data_and_analytics/braze_currents/message_engagement_events/#subscription-events)追加された。 
...
{% alert important %}
これらの更新を活用するには、Currents コネクターの設定を編集し、使用するイベントを有効にする必要があります。質問がある場合は、担当のアカウントマネージャーに連絡すること。
{% endalert %}
