...
ユーザーがオーディエンス同期のステップに達すると、BrazeはSnapchatのAPIレート制限を尊重しながら、ほぼリアルタイムで同期を行う。Brazeは、5秒ごとにできるだけ多くのユーザーをバッチ処理してからSnapchatに送信する。

SnapchatのAPIレート制限では、1秒あたり10クエリ、1リクエストあたり10万ユーザーを超えてはならない。顧客がこの制限に達した場合、Brazeは最大13時間まで同期を再試行する。それでも同期できない場合、BrazeはこれらのユーザーをUsers Erroredメトリックの下にリストアップする。
