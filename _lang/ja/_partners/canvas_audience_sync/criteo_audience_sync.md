...
ユーザーがオーディエンス同期ステップに到達すると、BrazeはクリテオのAPIレート制限を尊重しながら、ほぼリアルタイムでそれらを同期します。バッチをBrazeし、5秒ごとにできるだけ多くのユーザーを処理してから、クリテオに送信します。

Criteo のAPI レート制限では、1 分あたり250 件を超えるリクエストは許可されません。顧客がこの制限に達すると、Braze は最大13 時間まで同期を再試行します。それでも同期できない場合、Braze はユーザーs エラーメトリクスにこれらのユーザーs を一覧表示します。 
