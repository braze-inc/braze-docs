...
{% alert note %}
**モバイル広告IDが見つからない？修正しよう：**iOSのIDFAまたはAndroidのGAIDを一致させるフィールドとして使用してオーディエンスに同期した場合、ステップエディタにこのメッセージが表示されることがある。これは**情報提供であり、エラーではない**。マッチングするモバイル広告IDフィールドがオーディエンスデータに存在すること（例えば、キャンバスパスのユーザーが対応する識別子を収集していること）を確認するようリマインドされる。データを確認した後で、それを解除することができる。
{% endalert %}
...
![カスタムオーディエンスキャンバスコンポーネントの展開ビュー。ここで、希望する広告アカウントと既存のオーディエンスを選択し、"Add user to Audience "ラジオボタンも選択する。]({% image_buster /assets/img/audience_sync/g_sync2.png %})
