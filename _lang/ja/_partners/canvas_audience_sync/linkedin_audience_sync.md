...
LinkedIn のAPI レート制限では、1 秒あたり最大10 個のクエリと1 リクエストあたり100,000 ユーザー を許可します。顧客がこの制限に達すると、Braze は最大で約13 時間同期を再試行します。それでも同期できない場合、Braze はユーザーs エラーメトリクスにこれらのユーザーs を一覧表示します。
...
![オーディエンスシンクステップメトリクスのサンプルで、10,000 個のユーザーs が入力されています。]({% image_buster /assets/img/linkedin/linkedin11.png %})
{% endalert %}
