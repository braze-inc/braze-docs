...
ユーザーがオーディエンス同期のステップに達すると、BrazeはTikTokのマーケティングAPIのレート制限を尊重しながら、ほぼリアルタイムで同期する。Brazeは、TikTokに送信する前に、5秒ごとにできるだけ多くのユーザーをバッチ処理する。

TikTokのセグメンテーションAPIのレート制限では、1秒あたり50クエリ、1リクエストあたり10kユーザーを超えることはできない。顧客がこの制限に達した場合、Brazeは最大13時間まで同期を再試行する。それでも同期できない場合、BrazeはこれらのユーザーをUsers Erroredメトリックの下にリストアップする。
...
### キャンバスにはオーディエンスが存在しません」という件名のメールが届くのはなぜか？

これは、同期を選択したオーディエンスがストリーミングオーディエンスではない場合に発生する可能性がある（たとえば、そっくりさんオーディエンスやユーザーファイルオーディエンスの場合）。Braze Audience Sync Canvasステップで新しいオーディエンスを作成してみる。