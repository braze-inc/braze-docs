...
{% alert important %}
Googleは、2024年3月6日より施行される[デジタルマーケット法（DMA](https://ads-developers.googleblog.com/2023/10/updates-to-customer-match-conversion.html)）の変更に対応し、[EUユーザー同意ポリシーを](https://www.google.com/about/company/user-consent-policy/)更新する。この新しい変更により、広告主はEEA、英国、スイスのエンドユーザーに特定の情報を開示し、必要な同意を得ることが求められる。次のドキュメントを確認して、詳細を学んでください。
{% endalert %}
...
キャンバスでGoogle オーディエンスのステップを設定する前に、以下の項目が作成され、完了していることを確認する。
...
開始するには、**パートナー統合** > **テクノロジーパートナー** > **Google 広告** に移動し、**Google 広告を接続** を選択します。モーダルが表示されるので、Google広告アカウントに関連付けられているメールを選択し、BrazeにGoogle広告アカウントへのアクセスを許可する。

Google広告アカウントとの接続に成功すると、Google広告のパートナーページに戻る。その後、Brazeワークスペースでアクセスしたい広告アカウントを選択するよう促される。

![]({% image_buster /assets/img/google_sync/googlesync.gif %}) Google広告アカウントとBrazeの接続が成功した場合のワークフローを示すGIF。{: style="max-width:85%;"}

#### iOSのIDFAまたはGoogle Advertising IDをエクスポートする

オーディエンスシンクでiOS IDFAまたはGoogle Advertising IDをエクスポートする場合、Googleはリクエスト内でiOSアプリIDとAndroidアプリIDを要求する。Google オーディエンス同期の下で、[**モバイル広告 ID を追加**] を選択し、iOS アプリ ID と Android アプリ ID (アプリパッケージ名) を入力して、それぞれ保存します。
...
1つのワークスペースに複数のアプリがある場合、ユーザーのモバイル広告IDは複数のアプリで同じになるため、セットアップでどのアプリIDを入力してもよい。これは、AndroidのGAIDとiOSのIDFAの両方がデバイス上のユニバーサル広告識別子であり、アプリ固有ではないからだ。特定のアプリからのユーザーのモバイル広告IDを同期するには、セグメントフィルター（「最後に使用した特定のアプリ」または「最新のアプリバージョン」）を使用して、これらのユーザーをターゲットにすることができる。

### ステップ2:キャンバスフローに Google オーディエンスステップを追加する

キャンバスにコンポーネントを追加し、**オーディエンス・シンクを**選択する。

![エディターでキャンバス・コンポーネントを選択するメニュー。][18]{: style="max-width:35%;"}![ユーザー・ジャーニーにオーディエンス・シンクのステップが追加された。][20]{: style="max-width:28%;"}

### ステップ3:同期セットアップ

1. **Custom Audienceを**選択し、コンポーネント・エディターを開封する。
2. オーディエンスシンクのパートナーとして**Googleを**選択する。

![オーディエンス同期ステップの設定には、同期を開始するパートナーを選択するオプションがある。][19]{: style="max-width:80%;"}

{: start="3"}
3\.希望するGoogle広告アカウントを選択してください。
4\.**Choose a New or Existing Audience（新規または既存のオーディエンスを選択**）ドロップダウンで、新規または既存のオーディエンスの名前を入力する。 

{% tabs %}
{% tab 新規オーディエンスの作成 %}

1. 新しいカスタムオーディエンスの名前を入力する。
2. **Add Users to Audienceを**選択する。
3. オーディエンスに送信するファーストパーティーのユーザーフィールドデータを選択する。次のいずれかを選択できます。

- **お客様の連絡先情報**:ユーザーのメールまたは電話番号（Brazeに存在する場合はその両方）を含む。グーグルは、このフィールドを別々の識別子ではなく、1つのフィールドとして同期することを要求している。識別子が1つしかない場合でも、この単一フィールドを使用することができる。
- **モバイル広告主 ID**:iOS IDFAまたはAndroid GAIDのいずれかを選択する。GoogleのCustomer Matchの要件により、同じ顧客リストに両方のモバイル広告主IDを持つことはできない。

{: start="4"}
4\.次に、ステップエディタの下部にある**Create Audience**ボタンを選択して、オーディエンスを保存する。
...
新しいオーディエンスでキャンバスを立ち上げると、Brazeはキャンバスの立ち上げと同時に新しいカスタムオーディエンスを作成し、その後ユーザーがGoogle Audienceのステップに入るとほぼリアルタイムで同期する。 
...
Braze では、これらのオーディエンスが最新の状態であるようにするために、既存の Google 顧客リストからユーザーを追加または削除することもできます。既存のオーディエンスとの同期を図る：

1. 同期する既存のカスタムオーディエンスを選択する。
2. **オーディエンスに追加**するか、**オーディエンスから削除**するかを選択する。
3. Brazeは、ユーザーがGoogle オーディエンスのステップに入ると、ほぼリアルタイムでユーザーの追加や削除を行う。 
4. Google オーディエンスのステップを設定したら、「**完了**」を選択する。Google オーディエンスステップには、新しいオーディエンスに関する詳細情報が含まれます。
...
### Google オーディエンスステップの設定で、マッチさせるフィールドを複数選択できないのはなぜか？
...
### Googleでオーディエンスが同期されるまでどのくらいかかるのか？

オーディエンスが Googleに同期されるまでには6～12時間かかることがあります。 

### オーディエンスを同期したのに、なぜGoogleのオーディエンスサイズがゼロなのか？

プライバシー保護のため、ユーザーリストのサイズは、リストのメンバーが1,000人以上になるまでゼロと表示される。その後、サイズは最も重要な2桁に丸められます。

### Googleにオーディエンスを同期させたが、広告が配信されない。

オーディエンスに少なくとも5,000人のユーザーが含まれていることを確認し、広告配信を開始する。

### Mobile Apps IDs Deleted」エラーを解決するには？

Googleにオーディエンスを同期している場合、同期の一部としてモバイル識別子を同期することを選択したにもかかわらず、GoogleパートナーページからモバイルアプリIDを削除すると、このエラーがトリガーされる。この問題を解決するには、GoogleパートナーページにiOSとAndroidの適切なモバイルアプリIDが追加されていることを確認する。
