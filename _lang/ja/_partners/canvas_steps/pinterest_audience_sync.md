...
**Confirmを**選択すると、Brazeに戻り、同期したいPinterest広告アカウントを選択する。 
...
### オーディエンスがPinterestに登録されるまでどのくらいかかるか？

オーディエンスサイズは、Pinterestの広告マネージャーの**オーディエンス**ページで24～48時間以内に更新される。

### Pinterestにユーザーを渡した後、ユーザーがマッチしたかどうかを知るには？

Pinterest は、独自のデータプライバシーポリシーにこの情報を提供しません。

### 無効なトークン・エラーが表示された場合、次に何をすればよいか？

同期したい広告アカウントに適切な権限があることをPinterest Business Hubの管理者に確認する。また、PinterestパートナーページでPinterestアカウントを切断し、再接続することもできる。 
...
### オーディエンスシンクのステップで広告アカウントを選択できないのはなぜか？

トークンが正しいアカウント権限で生成されたことを確認する。Pinterest広告アカウントにオーディエンスが多すぎる場合、広告アカウントを選択するドロップダウンがタイムアウトすることがあるので注意しよう。この場合、広告アカウントのオーディエンスの量を減らすことをお勧めする。
