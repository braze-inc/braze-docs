...
| 返信メッセージ    | キーワードが検出されたときに送信されるメッセージ。"A customer service rep will contact you shortly."（顧客サービス担当者がまもなくご連絡します）など。 |
{: .reset-td-br-1 .reset-td-br-2 }

![BrazeのSMSキーワードカテゴリーの例。]({% image_buster /assets/img/zendesk/instant_chat/chat11.png %}){: style="max-width:70%;"}
...
![最初のWebhookキャンペーンの「配信スケジュール」ページ。]({% image_buster /assets/img/zendesk/instant_chat/chat13.png %})
...
- 再適格性基準を設定します。
- 該当するオーディエンスを追加する。 **zendesk_ticket_open**は**true**である）。
