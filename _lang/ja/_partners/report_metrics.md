...
- SMS/MMS: SMS プロバイダがアップストリームの通信事業者と宛先デバイスから確認を受け取った後に「配信済み」。
...
{% apitags %}
メール、コンテンツカード、SMS、LINE
{% endapitags %}

配信されたメール、カード、またはメッセージ内でクリックしたユーザーの合計数 (および割合)。LINEの場合、これは1日あたり20メッセージの最小しきい値に達した後に追跡されます。
...
{% apitags %}
メール、iOSプッシュ、Androidプッシュ、Webプッシュ、LINE
{% endapitags %}

開封されたメッセージ数の合計。LINEの場合、これは1日あたり20メッセージの最小しきい値に達した後に追跡されます。
...
LINEの場合、これは1日あたり20メッセージの最小しきい値に達した後に追跡されます。 
...
メールの場合、これは 1 人のユーザーによって少なくとも 1 回開かれた配信済みメールの合計数です。メールの場合、これは 7 日間にわたって追跡されます。LINEの場合、これは1日あたり20メッセージの最小しきい値に達した後に追跡されます。
