...
![Group ID、Android App ID、iOS App ID の入力を促す Pilgrim コンソール。]({% image_buster /assets/img_archive/pilgrim-dev-console.png %}){: style="float:right;max-width:40%;margin-left:15px;"}
...
![配信ステップ内のアクションベースのキャンペーンで、"arrival"selected as the "perform カスタムイベント" option, where "locationType"equals "home"が表示されます。]({% image_buster /assets/img_archive/action-based-campaign.png %})
...
!["last_location"が"Pizza Place"であることを示す、対象ユーザーs ステップのアクションベースのキャンペーン。]({% image_buster /assets/img_archive/last-location-segment.png %})
