...
Braze と Amplitude の双方向統合により、[ Amplitude コホート]({{site.baseurl}}/partners/data_and_analytics/customer_data_platform/amplitude/amplitude_audiences/)、ユーザー特性、およびイベントを Braze に同期し、また Braze Currents を使用して [Amplitude に Braze イベントをエクスポートして](#data-export-integration)、製品データとマーケティングデータのより深い分析を実行できます。
