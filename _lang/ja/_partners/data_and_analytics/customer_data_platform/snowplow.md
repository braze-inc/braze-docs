...
> [Snowplow](https://snowplowanalytics.com) は、豊富で高品質、低レイテンシーのデータ収集のためのスケーラブルなオープンソースプラットフォームです。エンタープライズビジネスのための高品質で完全な行動データを収集するように設計されています。
...
| Google Tag Managerサーバーサイド | GTM-SSをデプロイし、[GTM-SS用のSnowplowクライアント](https://docs.snowplowanalytics.com/docs/forwarding-events-to-destinations/forwarding-events/google-tag-manager-server-side/snowplow-client-for-gtm-ss/)をセットアップする必要があります。 |
...
1. [`template.tpl`](https://github.com/snowplow/snowplow-gtm-server-side-braze-tag/blob/main/template.tpl) テンプレートファイルをダウンロードしてください。
...
| パラメータ | 説明 |
| --------- | ----------- |
| Braze REST API エンドポイント | これを Braze REST [エンドポイント]({{site.baseurl}}/developer_guide/rest_api/basics/#endpoints) の URL に設定します。 |
| Braze API キー | これを各リクエストに含まれるBraze [API キー]({{site.baseurl}}/developer_guide/rest_api/basics/#app-group-rest-api-keys)に設定します。 |
| Braze `external_id` | このキーをクライアントイベントプロパティに設定し、ユーザーの`external_id`に対応させ、[Brazeユーザー識別子]({{site.baseurl}}/developer_guide/rest_api/basics/#external-user-id-explanation)として使用します。 |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

### イベントマッピング

次の表は、[Snowplowクライアント](https://docs.snowplowanalytics.com/docs/forwarding-events-to-destinations/forwarding-events/google-tag-manager-server-side/snowplow-client-for-gtm-ss/)が主張するSnowplowイベントに関するイベントマッピングオプションを一覧にしたものです。
...
| イベントプロパティルール | 説明 |
| --------- | ----------- |
| 共通イベントプロパティを含める | デフォルトで有効になっているこのオプションは、Brazeイベントのプロパティに[共通イベント定義](https://developers.google.com/tag-platform/tag-manager/server-side/common-event-data)のイベントプロパティを自動的に含めるかどうかを設定します。 |
