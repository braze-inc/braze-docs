...
サイドバイサイドの統合とは異なり、サーバー間の統合では、アプリ内メッセージング、コンテンツカード、自動プッシュトークン登録などの Braze UI 機能はサポートされません。また、クラウドモードでは利用できない[自動取得]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/#user-data-collection)データ (匿名ユーザーやデバイスレベルのフィールドなど) も存在します。
