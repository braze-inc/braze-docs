...
| Brazeアプリ識別子キー | アプリ識別子キー。<br><br>これはダッシュボードの**Manage Settings**>**API Keyで**確認できる。 |
...
削除前に Braze ユーザーからオーディエンスを除去するには、オーディエンスフィルターを調整してオーディエンスのサイズを強制的に0にしてから、オーディエンスを削除します。オーディエンスの計算が完了して0人のユーザーが返された後に、オーディエンスを削除します。その後、オーディエンス・メンバーシップはBrazeで更新され、シングル属性オプションの場合は`false` 、配列フォーマットからオーディエンスIDを削除する。
