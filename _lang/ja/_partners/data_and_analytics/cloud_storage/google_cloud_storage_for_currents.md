...
{% alert note %}
ワークロード ID フェデレーション（WIF）は、カレントの認証方法としてサポートされていない。JSON秘密キーを持つサービス・アカウントを使用しなければならない。
{% endalert %}
...
次に、**GCS JSON認証**情報の下にJSON秘密キーをアップロードし、GCSバケット名とGCSプレフィックス（オプション）を入力する。この認証情報は、前のステップで説明したように、Google Cloud Platformを通じて生成する必要がある。
...
クラウド・データ・ストレージ・ソリューションを統合し、API、ダッシュボード・レポート、CSVレポートをエクスポートしようとしているユーザーには、次のような問題が発生する：
