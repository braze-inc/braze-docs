...
ユースケースによっては、Kickboxカスタム属性 "result "がユーザープロファイルに存在する、またはその値が "deliverable "に等しいセグメンテーションを作成することが適切な場合がある。このフィルターは、単独でセグメンテーションを作成するために使用することもできるし、将来的にすべてのセグメンテーションの一部にして、セグメンテーション内のすべてのユーザーを検証することもできる。 