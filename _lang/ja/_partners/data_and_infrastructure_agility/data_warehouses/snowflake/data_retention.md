...
> Brazeは、2年以上前のすべてのイベントデータから個人を特定できる情報（PII）を削除し、匿名化する。Snowflakeデータ共有を使用する場合、保持ポリシーが適用される前に、Snowflakeアカウントにコピーを保存することで、完全なイベントデータを環境に保持することを選択することができる。
...
{% alert warning %}
Brazeは、[データ保護テクニカルアシスタンスに]({{site.baseurl}}/dp-technical-assistance/)記載されているとおり、Brazeから削除されたユーザーのイベントデータを自動的に匿名化する。共有データベースの外部にコピーされたデータは、Brazeがもはや管理していないため、このプロセスには含まれない。
{% endalert %}
...
{: start="2"}
2\.Snowflakeアカウントで以下のコマンドのいずれかを実行し、プロシージャを実行する。

{% tabs %}
{% tab デフォルト %}

デフォルトでは、すべての`USERS_*` イベントタイプについて、2年以上前のデータをバックアップする。 
...
バックアップする年齢データを選択するためにフィルタを指定し、選択したイベント・テーブルのみをバックアップするためにテーブル名フィルタを指定する。 
...
{% alert note %}
このプロシージャは最新の`SF_CREATED_AT` をチェックし、それより新しいデータのみをバックアップするため、プロシージャを繰り返し実行しても重複レコードは作成されない。
{% endalert %}
...
1. `UNLOAD_BRAZE_SHARE` 、Brazeが共有する全データを指定したステージにコピーするための手続きを作成する。
...
{: start="2"}
2\.以下のいずれかのコマンドを実行し、プロシージャを実行する。 

{% tabs %}
{% tab デフォルト %}

デフォルトでは、`USERS_` 接頭辞を持つすべてのテーブルをコピーする。
