...
![Amazon IAM コンソールとポリシーのリスト。]({% image_buster /assets/img/fivetran_as3_iam.png %})
...
最後に、**Review Policy** を選択し、ポリシーに一意の名前を付け、説明を入力します。[**Create Policy**] を選択して、ポリシーを作成します。 
...
[**Next: Review**] を選択し、新しいロールに名前 (Fivetran など) を付け、[**Create Role]** を選択します。ロールが作成されたら、それを選択し、表示されているロール ARN をメモしておきます。
...
Fivetran で、**\+ Connector**、**Braze** コネクターの順に選択して設定フォームを起動します。フォーム内で、指定されたフィールドに適切な値を入力してください:
...
- `Bucket`:Braze アカウントで、[**Partner Integrations**] > [**Data Export**] > 利用している Current 名の順に移動します。
...
最後に、[**Save & Test**] を選択すると、あとは Fivetran が Braze アカウントのデータとの同期を処理します。
...
- `API URL`:あなたのBraze REST APIエンドポイント。
- `API Key`:あなたのBraze REST APIキー。 
- `Bucket Name`:Braze アカウントで、[**Partner Integrations**] > [**Data Export**] > 利用している Current 名の順に移動します。
