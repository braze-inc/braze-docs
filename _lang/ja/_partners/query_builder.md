...
この機能では一部の顧客データに直接アクセスできるため、クエリビルダーにアクセスできるのは「個人情報の表示」[権限がある場合のみです]({{site.baseurl}}/user_guide/administrative/manage_your_braze_users/user_permissions/)。
...
{:start="2"}
2\.\[**SQL クエリの作成**] を選択します。クエリの作成にヒントやヘルプが必要な場合は、\[**クエリテンプレート**] を選択し、リストからテンプレートを選択します。それ以外の場合は、\[**SQL エディター**] を選択してエディターに直接移動します。
...
![テンプレート化されたクエリ「過去 30 日間のチャネルエンゲージメントと収益」の結果を表示するクエリビルダー。]() {% image_buster /assets/img_archive/query_builder.png %}
...
2. プロンプトを入力するか、サンプルプロンプトを選択して \[**生成**] を選択し、プロンプトを SQL に変換します。
3. 生成された SQL が正しいことを確認し、\[**エディターに挿入**] を選択します。
...
実行に 6 分以上かかるレポートはタイムアウトします。これが、ある時点で実行する最初のクエリである場合、処理に時間がかかるため、タイムアウトする可能性が高くなります。タイムアウトした場合は、レポートをもう一度実行してみてください。

再試行してもレポートがタイムアウトしたり、エラーが発生したりした場合は、[サポートに連絡してください]({{site.baseurl}}/help/support#braze-support)。

## クエリテンプレート

すべてのテンプレートは、過去 60 日間のデータを読み込んで表示できます。クエリテンプレートにアクセスするには、最初にレポートを作成するときに \[**SQL クエリを作成**] > \[**クエリテンプレート**] を選択します。

[使用可能なテンプレートのリストについては、「クエリテンプレート]({{site.baseurl}}/user_guide/data_and_analytics/query_builder/query_templates/)」を参照してください。
...
| 日付範囲タイプ | 説明 | 必要な値 |
| --- | --- | --- |
| 相対 | 過去 X 日間を指定できます | 必要な `start_date` |
| 開始日 | 開始日を指定できます | 必要な `start_date` |
| 終了日 | 終了日を指定できます | 必要な `end_date` |
| 期間 | 開始日と終了日の両方を指定できます | `start_date`との両方が必要 `end_date` |
...
![現在の月に使用されたクレジットの量を示すクエリビルダー。][1]{: style="max-width:60%;"}
