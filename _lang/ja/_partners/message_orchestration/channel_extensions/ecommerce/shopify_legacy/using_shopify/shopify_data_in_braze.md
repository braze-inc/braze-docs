...
さらに、Braze で購入フィルタの幅を使用して、以下に基づいてユーザのセグメントを作成できます。
- 最初の購入 / 最後の購入
- 特定のアプリの最初の購入 / 最後の購入
- 過去30日以内に購入した製品
- 購入した製品の数
...
カスタムイベントプロパティのネストされたオブジェクトサポートを使用すると、ネストされたイベントプロパティを使用してキャンペーンおよびキャンバスをトリガできます。以下は、`shopify_created_order` カスタムイベントから特定の商品を使用してキャンペーンをトリガーする例である。`list_items[0].product_id` を使用してアイテムリストのインデックスを作成し、製品 ID にアクセスするようにしてください。
