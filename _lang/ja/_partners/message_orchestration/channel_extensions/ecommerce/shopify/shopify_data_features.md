...
**タイプ**：推奨イベント<br>
**トリガー**: 顧客によるショッピングカートの追加、削除、更新<br>
**ユースケース**: カート放棄
...
これで、以下のショッピングカートの Liquid タグをメッセージに追加できます。
...
次に、次の Liquid タグをメッセージに追加することで、チェックアウト時点のカート内の商品を参照できます。
...
**タイプ**：推奨イベント<br>
**トリガー**: ユーザーがチェックアウトプロセスを正常に完了し、注文を出すとき<br>
...
**トリガー**: ユーザーの注文が満たされ、発送の準備ができたとき<br>
...
**タイプ**：[カスタムイベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events)<br>
**トリガー**: ユーザーの注文の一部が履行され、発送の準備ができたとき<br> 
**ユースケース**: (トランザクション) フルフィルメントの更新 
...
**タイプ**：[カスタムイベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events)<br>
**トリガー**: ユーザの注文がShopify内で支払済みとマークされたとき<br>  
...
**タイプ**：[カスタムイベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events)<br>
**トリガー**: ユーザーの注文がキャンセルされた場合<br> 
**ユースケース**: (トランザクション) 注文キャンセルの確認
...
**タイプ**：[カスタムイベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events)<br>
**トリガー**: 利用者の注文が払い戻されたとき<br>
**ユースケース**: (トランザクション) 返金の確認
...
**タイプ**：[カスタムイベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events)<br>
**トリガー**: ユーザーが自分のアカウントにログインするとき<br>
**ユースケース**: ウェルカムシリーズ
...
{% alert note %}
現在、Shopify 統合では、Braze [購入イベント]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/purchase_events#purchase-events)への入力はサポートされていません。その結果、購入フィルタ、液体タグ、アクションベースのトリガ、およびアナリティックでは、ecommerce.order_placed イベントを使用する必要があります。
{% endalert %}
...
Braze SDK で収集されるデータの詳細については、[SDK データ収集]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/sdk_data_collection/)を参照してください。 

## 歴史的埋め戻し

Shopify ストアのオンボーディング中に、履歴バックフィルを介して初期データ同期を開始することで、顧客とすぐにやり取りすることができます。このバックフィルの一部として、Braze は、Shopify 統合接続の直前の 90 日分の、すべての顧客と注文の初期データ同期を実行します。 

### Shopify 履歴バックフィルの設定

1. **Shopify データを追跡**ステップで履歴バックフィルをオンにします。

![Shopify 統合の「Shopify データを追跡」ステップで、履歴バックフィルが選択されている画像。]({% image_buster /assets/img/Shopify/historical_data_backfill_sync.png %})

{: start="2"}

2. 統合設定が完了すると、Braze は初期データ同期を開始します。進捗状況は、統合設定の [**Shopify データ**] タブで確認できます。 

![Shopify の統合設定ページで、イベントがアクティブに同期されていることを示すスピナーがある。]({% image_buster /assets/img/Shopify/historical_data_backfill_syncing.png %})

### 同期データ 

初期データ同期の場合、Braze は、Shopify 統合接続の直前の 90 日分の顧客と注文をインポートします。Braze が Shopify の顧客をインポートする際に、設定で選択した `external_id` タイプを割り当てます。