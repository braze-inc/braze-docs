---
nav_title: Punchh
article_title: Punchh
alias: /partners/punchh/
noindex: true
layout: dev_guide
guide_top_header: "Punchh"
guide_top_text: "Punchhは、レストラン、小売店、コンビニエンスストアのブランドにとって、顧客ロイヤルティとエンゲージメントの主要なソリューションである。Punchhは10年にわたり、実店舗が顧客を理解し、リアルタイムのインサイトを活用して最高のサービスを提供できるよう、一貫性のある最新のロイヤルティ体験を創造してきた。"
description: "このランディング・ページは、統合ガイダンスやダイナミック・コード生成など、Punchhのすべてを紹介するページだ。"
guide_featured_list:
- name: Punchh
  link: /docs/partners/message_orchestration/channel_extensions/loyalty/punchh/punchh/
  image: /assets/img/braze_icons/bank-note-02.svg
- name: 動的コード生成
  link: /docs/partners/message_orchestration/channel_extensions/loyalty/punchh/code_generation/
  image: /assets/img/braze_icons/code-square-01.svg

---

<br><br>