...
![API トリガー配信][4]{: style="max-width:70%;"}

最後に、[**配信コントロール**] で「再適格性」をオンにします。

![[配信コントロール] で再適格性が有効になっている。][5]
...
![SMS Braze で応答します。][6]{: style="max-width:70%;"}
...
![SMS Braze JSON 本文経由で応答します。][7]{: style="max-width:70%;"}
...
![チケットがクローズされたときにユーザーを更新します。][8]{: style="max-width:70%;"}
...
![Zendesk に設定されたチケットを解決しました。][9]{: style="max-width:70%;"}
...
![2つの必須ヘッダーを含むリクエストの例。][12]({: style="max-width:70%;"})
...
![最初の Webhook キャンペーンの [配信コントロール] で再適格性が選択されている。][14]
