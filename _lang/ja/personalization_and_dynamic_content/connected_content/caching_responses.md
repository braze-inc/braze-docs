...
GET リクエストがキャッシュされます。これを設定するには、:no_cache パラメータをConnected Content コールに追加します。

POST リクエストはキャッシュされません。これは、:cache_max_age パラメータをConnected Content コールに追加することで強制できます。最小キャッシュ時間は5分、最大キャッシュ時間は4時間である。
