...
## アプリ内メッセージのスタイルをリセットするドラッグ&＆ドロップエディター

アプリ内メッセージ用のドラッグ&＆ドロップエディターでは、変更を加えた後、スタイルを素早くデフォルト設定にリセットできるようになった。詳細については、[スタイルをデフォルトにリセットする]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/drag_and_drop/create/#resetting-styles-to-default)を参照してください。
...
[Sisu Data]({{site.baseurl}}/partners/data_and_analytics/business_intelligence/sisu_data/)とBrazeの統合により、すべてのキャンペーンまたはキャンペーンレベルで、メトリクス（例えば、開封率、クリックスルー率、コンバージョン率など）が変化する理由と、最も最適な結果を導く要因を理解することができます。これらのセグメントが識別されたら、企業ユーザーはデータウェアハウスに出力を反映させるか、Sisuから直接Brazeに送信してユーザーをリターゲティングし、再エンゲージメントを図ることができる。
