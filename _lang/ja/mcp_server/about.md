...
[Braze MCP サーバーを設定]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/setup/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/setup/){% endif %}の後、エージェント、アシスタント、チャットボットなどのAI ツールをBraze に直接接続し、キャンバスやキャンペーン分析、カスタム属性 s、Segment s などの集計データを読み込むことができます。Braze MCP サーバーは次の用途に適しています。
...
クラウドやカーソルなどのツールを使用して、自然言語でBrazeを操作できます。他の例およびベストプラクティスについては、[Braze MCP サーバーの使用]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/usage/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/usage/){% endif %})を参照してください。
