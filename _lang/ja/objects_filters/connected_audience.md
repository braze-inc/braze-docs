...
接続オーディエンスは、デフォルト属性、カスタムイベント、セグメント、またはメッセージエンゲージメントイベントによってユーザーをフィルターすることはできません。これらのフィルターを使用するには、それらをオーディエンス Segmentに組み込み、[`/messages/send`エンドポイント]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages#request-parameters)の`segment_id`パラメータでそのSegmentを指定することをお勧めします。他のエンドポイントを使用する場合は、最初にBraze ダッシュボードのAPI-トリガーed キャンペーンまたはキャンバスにSegmentを追加する必要があります。
