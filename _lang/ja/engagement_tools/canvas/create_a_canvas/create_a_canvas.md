...
![キャンバスの詳細ページ。キャンバス名、説明、場所、タグのフィールドがあります。]({% image_buster /assets/img/canvas_details.png %})({: style="max-width:70%;"})
...
![[ユーザー検索] フィールド。外部ユーザー ID または Braze ID で検索できます。]({% image_buster /assets/img_archive/user_lookup.png %})({: style="max-width:100%;"}){: style="max-width:80%;"}
...
1. 乱数を保存するカスタム属性を作成します。"lottery_number" 、"random_assignment". のように、わかりやすい名前をつける。[ダッシュボードで]({{site.baseurl}}/user_guide/data/custom_data/managing_custom_data/)アトリビューションを作成することもできるし、[`/users/track` エンドポイントへの]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)APIコールで作成することもできる。<br><br>
...
キャンバスワークフローのどのステップも、コンポーネントを選択して編集できる。例えば、ワークフローの最初のステップである[遅延]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/)コンポーネントを特定の日に変更したいとします。ステップを選択して設定を表示し、3月1日までの遅延を調整する。すると、ユーザーは 3 月 1 日にキャンバスの次のステップに移行します。
