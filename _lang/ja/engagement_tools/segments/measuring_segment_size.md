...
![メール、iOS プッシュ、Android プッシュ、Web プッシュ、およびKindle プッシュで到達可能なユーザー別に、到達可能なユーザーの合計を表示するテーブル。]({% image_buster /assets/img_archive/segmenter_reachable_users.png %})()
...
![セッション数が正確に1であるユーザーを絞り込むフィルターが示されているフィルターグループ。]({% image_buster /assets/img_archive/segment_filter_stats.png %})({: style="max-width:80%;"})
...
![推定ユーザー数が2.3M—2.4M であることを示す [到達可能なユーザー] パネル。]({% image_buster /assets/img_archive/reachable_users_side_panel.png %})()
...
![[到達可能なユーザー] サイドパネル。]({% image_buster /assets/img_archive/reachable_users_panel.png %})()
...
{% alert note %}
[ランダムなバケット番号で]({{site.baseurl}}/user_guide/engagement_tools/testing/random_bucket_numbers/)均等に分割されたセグメントは、同じサイズにはならない。たとえば、**ランダムバケット#が5000未満の**フィルターで1つのセグメントを作成し、**ランダムバケット#が5000以上の**フィルターで1つのセグメントを作成した場合、セグメントのサイズが最大で数％ポイント異なることは可能であり、予想されることである。これは、アクティブでないユーザーが削除されたり、ユーザーに連絡がつかなくなったりといった状況が発生するためだ。
{% endalert %}

![内訳を表示するオプションがある [到達可能なユーザー] パネル。]({% image_buster /assets/img_archive/reachable_users_breakdown.png %})()
