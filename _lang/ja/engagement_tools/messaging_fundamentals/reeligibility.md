...
再適格性が0秒に設定されている場合、ユーザーは再入力する前に最初にキャンバスを終了する必要はありません。つまり、ユーザーが同じキャンバスに再入力することができます。フィルターs を追加して、ユーザーs が同じステップまたはメッセージを複数回受信しないようにすることができます。ただし、ユーザーが2回目にキャンバスに再び入ると、キャンバスで初めて受信したステップは、ユーザーには表示されません。これは、ユーザーが同じメッセージを再び受信できることを意味します。これを防ぐには、キャンバスの再エントリを防止するようにキャンバスを設定したり、キャンバスの最長継続時間に再適格性を設定したりできます。

また、[ユーザーアップデートコンポーネント]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/user_update/)を使用して、ステップを受信したユーザーがこれをカスタム属性としてログアウトすることもできます。これは、キャンバスの移動中にステップを受信したフィルターアウトに使用できます。
