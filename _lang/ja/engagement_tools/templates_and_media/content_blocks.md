...
| コンテンツサイズ | 最大50KB。 |
| 配置 | コンテンツブロックはメールのフッター内では使用できないが、[フッターを含むコンテンツブロックを作成して](#email-footers)メールで使用することができる。 |
...
![サイクリングとダンスのためのコンテンツブロック（"Workout_Promo" ）のプレビューで、6つのインクルージョンがある。]({% image_buster /assets/img/preview_tab_content_block.png %}){: style="max-width:60%;"} 
...
コンテンツブロックをメールフッター内で使用することはできないが、フッターコンテンツを含むコンテンツブロックを作成し、メール内で使用することはできる。そのためには、以下の手順に従ってほしい：

1. **設定**＞**メール設定**＞**カスタムフッターと**進み、フッターを作成する。
2. **コンテンツブロックライブラリーの**コンテンツブロックにフッターを追加する。
3. コンテンツブロックをメールテンプレートやメッセージに追加する。

これで、複数のメッセージで同じフッターを使うことができる！
...
![保存されたコンテンツブロックパネルでは、"Test_32" の設定ドロップダウンメニューが拡張され、3つのオプションが表示される：アーカイブ解除、複製、ワークスペースにコピー]({% image_buster /assets/img/unarchive-content-block.png %})
