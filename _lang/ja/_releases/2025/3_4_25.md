...
[メールメッセージ作成]({{site.baseurl}}/user_guide/message_building_by_channel/email/html_editor/#step-3-compose-your-email)画面では、プレーンテキストが同期されていない場合にのみ表示される「Regenerate from HTML」アイコンを選択することで、「Plaintext」タブで自動同期に戻すことができる。
