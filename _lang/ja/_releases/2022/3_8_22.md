...
Brazeは、Rokuデバイスでユーザーにアプリ内メッセージを送信することをサポートするようになりました！これはSDKの追加設定が必要であり、デフォルトでは利用できないことに注意されたい。Rokuのアプリ内メッセージの統合に関する詳細は、[Rokuのアプリ内メッセージ]({{site.baseurl}}/developer_guide/in_app_messages/?sdktab=roku)を参照してください。
...
Brazeと[Grouparooの]({{site.baseurl}}/help/release_notes/deprecations/grouparoo)統合は、データウェアハウスのデータをBrazeに同期する。自動同期スケジュールにより、顧客とのコミュニケーションは常に最新の情報に保たれる。
