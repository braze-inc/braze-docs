...
![ユーザーガイド > チャネル別メッセージ作成 > アプリ内メッセージ > テンプレート > 簡易アンケートから移動する一連のパンくず]({% image_buster /assets/img/doc-breadcrumbs.png %}){: style="max-width:55%"}
...
{% alert important %}
SQLエディターは早期アクセス中だ。早期アクセスへの参加に興味がある場合は、カスタマー・サクセス・マネージャーに連絡を。
{% endalert %}
...
## 百花繚乱イベント：users_campaigns_abort および users_canvas_abort
...
## Shopify 履歴バックフィル
[Shopify 履歴バックフィル]({{site.baseurl}}/partners/ecommerce/shopify_legacy/shopify_features/shopify_backfill)を使用すると、Braze で Shopify 統合接続前の過去 90 日間のすべての顧客、注文、購入イベントをインポートできます。
