...
[Content Optimizer]({{site.baseurl}}/user_guide/brazeai/content_optimizer)は、継続的で高バリアントなコンテンツテストキャンバスステップで、自動エンゲージメント最適化を実現します。メッセージステップと同様のドラッグアンドドロップ可能なインターフェイスを使用して、テストするコンポーネントを定義し、AI を使用してバリアントs を生成し(または手動で入力)、Liquid タグ s を使用してこれらのコンポーネントをメッセージコンテンツにマッピングできます。

コンテンツ最適化ルーチンは、非文脈に応じた or 状況に即したのマルチアームバンドトオプティマイザに組み込まれており、ユーザーごとに1つのメッセージを送信し、予測推奨に基づいて配信するコンポーネントバリアントの組合せを決定します。ステップが長期間にわたってデータを収集すると、パフォーマンスの高いバリアントは送信割り当てが自然に増え、パフォーマンスの低いバリアントは減ります。コンテンツオプティマイザは、継続的な最適化を可能にするために、一日のユーザー量が一貫している(1日に少なくとも数千ユーザー秒)繰り返し送信キャンバスで最適に動作します。
...
ロケールをワークスペースに追加した後、[は、1つのバナー内のすべての異なる言語]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/localization/locales#translating-locales)のユーザーを対象にすることができます。

#### ドラッグアンドドロップコンテンツブロックの設定幅

[ ナビゲーションメニューのボタンを選択して、コンテンツブロック]({{site.baseurl}}/user_guide/message_building_by_channel/email/drag_and_drop/dnd_content_blocks/#using-the-editor-to-add-a-content-block) の幅をAdjustします。デフォルトの幅は、メールのグローバルスタイル設定sで指定されていない場合は100%です。指定されていない場合は、グローバル設定sが適用されます。
...
[自動IPウォーミング]({{site.baseurl}}/user_guide/message_building_by_channel/email/email_setup/ip_warming/#automated-ip-warming)を使用すると、毎日の送信量を徐々に増やし、受信トレイプロバイダーが送信パターンを学習し、信頼することができます。Braze は、最もエンゲージメントの高いサブスクライバーに最初に送信します。これにより、ベストプラクティスに合ったペースで毎日のボリュームが増加します。

### パートナーシップ:

#### LinkedIn – Canvas Audience Sync

[ Braze Audience Sync to LinkedIn]({{site.baseurl}}/partners/canvas_audience_sync/linkedin_audience_sync/) を使用すると、BrazeインテグレーションのユーザーデータをLinkedIn 顧客一覧に追加して、動作トリガー、セグメンテーションなどに基づいたアドバタイズメントを配信できます。通常、メッセージをトリガーするために使用する基準(プッシュ、メール、SMS、Webhookなど)が、Brazeキャンバスでユーザーデータに基づいて、LinkedIn 顧客一覧のそのユーザーに広告をトリガーできるようになりました。

#### Oracle Crowdtwist - データ& 分析

[Oracle Crowdtwist]({{site.baseurl}}/partners/crowdtwist)は、ブランドがパーソナライズされた カスタマーエクスペリエンスを提供できるようにするための、主要なクラウドネイティブ顧客 ロイヤルティ ソリューションです。彼らのソリューションは100以上のエンゲージメント パスを提供し、マーケター sが顧客のより完全なビューを開発するための迅速な価値への時間を提供します。

#### フルストーリー- 動的コンテンツ

[Fullstoryの]({{site.baseurl}}/partners/fullstory/)行動データプラットフォームは、テクノロジーリーダーがより優れた、より情報に基づいた意思決定を行うのに役立ちます。デジタルビヘイビアーデータを分析スタックに注入することで、フルストーリーの特許取得済みのテクノロジーは、ビヘイビアーデータの質の力をスケールで解き放ち、すべてのデジタル訪問をアクション可能なインサイトに変換します。 

#### オープンロイヤルティ-データ&分析

[オープンロイヤルティ]({{site.baseurl}}/partners/openloyalty)は、顧客 ロイヤルティおよび報酬のsプログラムを構築および管理できるクラウドベースのロイヤルティプログラム プラットフォームです。Brazeとオープンロイヤルティインテグレーションは、ポイントバランス、ティア変更、有効期限ワーニングなどのロイヤルティデータをリアルタイムでBrazeに直接同期します。これにより、ユーザーのロイヤルティ ステータスが変更されたときに、メール(Eメール、プッシュ、SMS)をトリガー パーソナライズされたできます。

#### エクステンション

[DOTS.ECO]({{site.baseurl}}/partners/docs.eco) 追跡可能なディジタル証明書を通じて、現実世界の環境影響を報酬 ユーザーできる。それぞれの証明書には、共有可能な証明書 URL や"画像 URL などのメタデータを含めることができるため、ユーザーは影響の証明を表示(再訪問) できます。

### メールオーケストレーション

[Mailizio]({{site.baseurl}}/partners/mailizio/)は、直感的なビジュアルエディタを使って、再利用可能でブランドセーフなコンテンツを簡単にデザインできるメール作成・マネジメントプラットフォームです。Mailizio のBraze への統合により、コンテンツブロック s とメール テンプレート s をエクスポートし、それらの同じアセットからアプリ内メッセージs を自動的に生成することができ、迅速かつ完全にコントロール主導型のキャンペーン 展開が可能になります。
...
メディアライブラリアセットをAPI を使用して追加できるようになり、顧客s、パートナ、および代理店がメッセージ作成ワークフローをより自動化できるようになりました。[API]({{site.baseurl}}/api/endpoints/media_library/manage_assets/create) を使用すると、アセットファイルを直接アップロードしたり、既存のURL からファイルをコピーしたりできます。この機能は、インテグレーションとオートメーション機能のロックを解除します。

### Currentsとデータ

#### ストレージ送信先およびデータのエージェントコンソールイベント

{% multi_lang_include release_type.md release="General availability" %}

2 つの新しい[events](http://braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events) が、Storage 送信先 s (AWS S3、GCS、およびAzure Blob Storage) とSnowflake Datashare で使用可能になりました: `agentconsole.AgentExecuted` および`agentconsole.ToolInvocation`。これらのイベントを使用すると、ダウンストリームシステムのエージェントコンソールの使用状況と詳細を分析できるため、エージェントの使用状況を理解し、最大限に活用することができます。エージェントを使用すると、キャンバス es またはカタログ s でコンテンツを生成し、インテリジェントな決定に基づいてユーザーをさまざまなパスs にルーティングするなど、Braze間で特定のタスクを実行できるインテリジェントエージェントを作成およびデプロイできます。詳細については、[Currents変更ログ](https://www.braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/currents_changelogs#changes-in-version-5-release-date-2026-02-04)を参照してください。

#### 各チャネルの新しい「再試行」イベント

{% multi_lang_include release_type.md release="General availability" %}

新しい[再試行イベント](https://www.braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events) は、メール、LINE、プッシュ通知s、SMS、webhook、およびWhatsApp チャネルs で使用できます。これらのイベントは、フリークエンシーキャップによってスケジュールされたメッセージがアボートされるのではなく遅延される場合に可視性を提供します。メッセージが廃止されたり、頻度cがアプリしたりすると、設定された再試行ウィンドウ内で再試行できるようになり、メッセージの配信方法やフリークエンシーキャップへの影響をよりよくインサイトできるようになります。詳細については、[Currents変更ログ](https://www.braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/currents_changelogs#changes-in-version-5-release-date-2026-02-04)を参照してください。

#### TokenStateChange イベントに新しい'time_ms' フィールドを追加します

{% multi_lang_include release_type.md release="General availability" %}

新しい`time_ms` フィールドが[`users.behaviors.pushnotification.TokenStateChange`](https://www.braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/customer_behavior_events) イベントに追加され、"トラッキング プッシュトークンのステート変更のミリ秒レベルの粒度が提供されました。この精度の向上は、同じ秒内に複数の変更が発生した場合のプッシュトークンの最新ステータスを理解するのに役立ち、正しいサブスクリプション ステータスを持っている下流のシステムに信頼を置くことができます。詳細については、[Currents変更ログ](https://www.braze.com/docs/user_guide/data/distribution/braze_currents/event_glossary/currents_changelogs#changes-in-version-5-release-date-2026-02-04)を参照してください。

#### Tealium宛先への匿名ユーザーの送信

{% multi_lang_include release_type.md release="General availability" %}

外部ユーザー ID が定義されていないイベントは、[Tealium]({{site.baseurl}}/partners/data_and_analytics/customer_data_platform/tealium/tealium_for_currents?redirected=1#tealium-for-currents)送信先s にストリーミングできるようになりました。"匿名ユーザーのイベントを含める"チェックボックスをCurrentsインテグレーションで選択すると、外部ユーザー IDのないイベントが抑制されずに送信先に送信されます。この機能は、川下分析や、識別されていない匿名のユーザーを含むユースケースに不可欠です。

##### 匿名ユーザーをカスタムHTTP 宛先に送信する

{% multi_lang_include release_type.md release="Beta" %}

外部ユーザー ID が定義されていないイベントは、カスタムHTTP 送信先にストリーミングできるようになりました。"匿名ユーザーのイベントを含める"チェックボックスをCurrentsインテグレーションで選択すると、外部ユーザー IDのないイベントが抑制されずに送信先に送信されます。この機能は、川下分析や、識別されていない匿名のユーザーを含むユースケースに不可欠です。

#### メールオープンイベント- "machine_open" フィールド

[Email Open event]({{site.baseurl}}/user_guide/data/distribution/braze_currents/event_glossary/message_engagement_events#email-open-events) は"machine_open" フィールドを生成するようになりました。これにより、[_Machine Open_]({{site.baseurl}}/user_guide/analytics/reporting/report_metrics#machine-opens) メトリクスをレポートできます。 

### SDK

以下のSDKアップデートがリリースされた。Swift SDK v14.0.1 は、ユニバーサルリンクの処理に関する問題を修正します。Android SDK v40.2.0 は、潜在的なメモリリークを修正し、透明なアクティビティが存在する場合に開封される多重セッションの問題を解決します。Expo SDK v3.2.0 では、`forwardUniversalLinks` オプション(デフォルト: false) を追加して、ユニバーサルリンクのネイティブスウィフトSDK処理を設定します。
...
`Live Activities Push to Start Registered for App` フィルター Segmentは、指定したアプリのiOS プッシュ通知 s を介してLive Activity を起動するように登録されているかどうかによって、ユーザー を起動します。

#### RFM SQLセグメント拡張

[RFM (recency, frequency, monetary) Segment Extension]({{site.baseurl}}/rfm_segments/) を作成して、購買習慣を測定することで最良のユーザー を目標にすることができます。

RFM 解析は、カテゴリ(最新、度数、金額)ごとに0 ～3 のスケールでユーザー をスコア化することによって、最良のユーザー を識別するマーケティング手法です。ここで、3 が最良のスコアで、0 が最悪のスコアです。最近度、度数、金額はすべて、選択した特定の時間範囲のデータに基づいています。
...
[マルチルールフィーチャーフラグのロールアウト]({{site.baseurl}}/developer_guide/feature_flags/create/#multi-rule-feature-flag-rollouts)を使用して、ユーザーを評価するための一連のルールを定義します。これにより、正確なセグメンテーションとコントロール主導機能のリリースが可能になります。このメソッドは、同じ機能を多様なオーディエンスs にデプロイする場合にアイデアです。
...
[除外リスト]({{site.baseurl}}/user_guide/engagement_tools/segments/suppression_lists)は、自動的にキャンペーンsやキャンバスを受信しないユーザーのグループです。除外リストはSegment フィルター で定義され、ユーザー はフィルター基準を満たすときに除外リストに入り、終了します。
...
[SMSおよびRCSボットクリックフィルター ing]({{site.baseurl}}/user_guide/message_building_by_channel/sms_mms_rcs/bot_click_filtering/)は、ボットクリックの疑いを除外することでキャンペーン 分析とワークフローを強化します。「ボットクリック」とは、ウェブクローラ、AndroidおよびiOSリンクプレビュー、またはCPaSセキュリティソフトウェアからのものなど、SMSおよびRCSメッセージの短縮されたリンクを自動クリックすることを意味する。この機能により、AC キュレート レポート のing、セグメンテーション、およびオーケストレーションが実際のユーザー に関与しやすくなります。
...
私たちは[ベストプラクティスセクション]({{site.baseurl}})を追加しました。これは、あなたのファジィオプトアウトメッセージを思慮深く設定し、あなたのサブスクライバーのために明確で、準拠し、肯定的な体験を作るのに役立ちます。
...
- [Web SDK 5.9.0](https://github.com/braze-inc/braze-web-sdk/blob/master/CHANGELOG.md)
    ...
    - SDKが初期化されていない場合、特定のSDKメソッドが明示的に未定義を返し、実際の実行時の動作に合わせて型付けが整列することに注意するために、定義と更新 d型を明確にしました。これにより、以前の(不完全な)タイプに依存していたプロジェクトに新しいタイプスクリプトエラー が導入される可能性があります。
