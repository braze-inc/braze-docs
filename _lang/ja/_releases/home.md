...
[ Braze Audience Sync to LinkedIn]({{site.baseurl}}/partners/canvas_audience_sync/linkedin_audience_sync/) を使用すると、BrazeインテグレーションのユーザーデータをLinkedIn 顧客一覧に追加して、動作トリガーs、セグメンテーションなどに基づいたアドバタイズメントを配信できます。通常、メッセージをトリガーするために使用する基準(プッシュ、メール、SMS、Webhookなど)が、Brazeキャンバスでユーザーデータに基づいて、LinkedIn 顧客一覧のそのユーザーに広告をトリガーできるようになりました。
