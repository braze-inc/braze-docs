...
全てのクラスターで利用可能な、新しいBraze Facebook オーディエンスエクスポートプロセスは、明確な統合ステップによりワークフローを効率化する。カスタムオーディエンスを送信したり、Facebookアプリ設定を調整して統合したりするために、OAuthリダイレクトURIをホワイトリストに登録する必要はもうない。
...
テンプレート[content_block/list]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks/)／メール／リストとAPIエンドポイントが更新され、新しい`tags`フィールドが追加された。このフィールドは、現行のブロックまたはメール テンプレートにアプリする任意のタグs を配列として列挙します。
