...
<br>
{% alert important %}
基本的なアプリ内メッセージ開発者統合ガイドをお探しですか?それを見つける [here]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/in-app_messaging/overview/).
{% endalert %}
...
![メッセージ作成画面で検出された2 つのキーと値のペア。最初のキーと値のペアには"attribute_key" が"Favorite Team" として設定され、2 番目には"view_type" が"picker" として設定されます。]({% image_buster /assets/img/iam_implementation/dashboard2.png %}){: style="max-width:65%;"}
...
![メッセージ作成画面で見つかった3 つのキーと値のペア。最初のキーと値のペア"attribute_key" は"Push Tags" 2 番目の"subtitle_text" は"有効化通知も。。。" 3 番目の"view_type" は"table_list".]({% image_buster /assets/img/iam_implementation/dashboard3.png %} に設定されます。{: style="max-width:65%;"}
