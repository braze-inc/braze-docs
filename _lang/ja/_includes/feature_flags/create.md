...
**フィーチャーフラグの詳細**欄に、フィーチャーフラグの名前、ID、説明を入力する。

![フィーチャーフラグに名前、ID、説明、プロパティを追加できることを示すフォームだ。]({% image_buster /assets/img/feature_flags/create_ff_properties.png %}){: style="max-width:75%"}
...
| プロパティ   | フィーチャーフラグをリモートで設定するオプションのプロパティ。それらはキャンバスステップやフィーチャーフラグ実験で上書きできる。 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

### ステップ 2a: カスタムプロパティを作成する

**プロパティ**では、イネーブルメントの際にアプリがBraze SDKを通じてアクセスできるカスタムプロパティを任意で作成できる。各変数には文字列、ブール値、画像, 写真、タイムスタンプ、JSON、数値を割り当てられる。またデフォルト値を設定することもできる。

{% tabs local %}
{% tab example %}
次の例では、フィーチャーフラグが指定されたカスタムプロパティを使用して、e コマースストアに在庫切れバナーを表示する。 
...
フィーチャーフラグをロールアウトする前に、ターゲットとするユーザーの[セグメント]({{site.baseurl}}/user_guide/engagement_tools/segments/)を選択する必要があります。新しく作成したフラグで**「ルールを追加」**を選択し、フィルターグループとセグメントのドロップダウンメニューを使って、対象オーディエンスから除外するユーザーを絞り込む。複数のフィルターを追加して、オーディエンスをさらに絞り込む。

![「トラフィック展開」と表示されたテキストボックスで、セグメントとフィルターの追加が可能だ。]({% image_buster /assets/img/feature_flags/segmentation_ff.png %}){: style="max-width:75%;"}

### ステップ 5: ロールアウト・トラフィックを設定する {#rollout}

デフォルトでは、フィーチャーフラグは常に無効になっている。これにより、フィーチャーリリースの日付と全ユーザーへの有効化を分離できる。ロールアウトを開始するには、**ロールアウトトラフィック**セクションのテキストボックスにパーセンテージを入力する。これは、選択したセグメント内のランダムなユーザーの割合を決定し、この新機能をそのユーザーに提供する。
...
{% alert important %}
単一のルールで旗を展開する場合、または単一のオーディエンスに展開する場合、セグメンテーション基準と展開割合を選択した最初のルールを追加する。最後に、**他の全員**ルールがオフになっていることを確認し、設定を保存する。
{% endalert %}

## 複数ルールによるフィーチャーフラグの段階的導入

複数のルールを組み合わせたフィーチャーフラグの段階的導入を活用し、ユーザー評価のルールシーケンスを定義する。これにより、精密なセグメンテーションとコントロールされた機能リリースが可能となる。この方法は、同じ機能を多様なオーディエンスに展開するのに最適だ。 

### 評価順序

フィーチャーフラグのルールは、リストされている順序で上から下へ評価される。ユーザーは最初に満たしたルールに該当する。ユーザーがどのルールにも該当しない場合、その適格性はデフォルトの「その他全員」ルールによって決定される。

### ユーザー資格

- ユーザーが最初のルールの条件を満たした場合、そのユーザーは直ちにフィーチャーフラグを受け取る資格を得る。
- ユーザーが最初のルールに該当しない場合、次のルールで評価される。以下同様である。

順次評価は、ユーザーがルールの条件を満たすか、リストの一番下にある「その他全員」ルールに到達するまで続く。

### 「その他大勢」のルール

「その他全員」ルールはデフォルトとして機能する。ユーザーが先行するいずれのルールにも該当しない場合、そのフィーチャーフラグの適用可否は「その他全員」ルールのトグル設定によって決定される。例えば、「その他全員」ルールが「オフ」に設定されている場合、デフォルト状態では、他のどのルールの条件にも該当しないユーザーは、セッション開始時にフィーチャーフラグを受け取らない。

### 再順序付けのルール

デフォルトでは、ルールは作成された順序で並べられる。しかし、ダッシュボード上でルールをドラッグ＆ドロップすることで、これらの順序を変更できる。

![ユーザーがフィーチャーフラグにルールを追加できることを示す写真。]({% image_buster /assets/img/feature_flags/add_rule.png %}){: style="max-width:80%;"}

![複数のルールが追加され、さらに「その他全員」ルールが設定されたフィーチャーフラグの概要を示す写真。]({% image_buster /assets/img/feature_flags/mr_rules_overview.png %}){: style="max-width:80%;"}

### 複数ルールによるフィーチャーフラグのユースケース

#### チェックアウトページを徐々に公開する

例えば、君がe コマースブランドで働いていて、新しいチェックアウトページを安定性を確保するために、異なる地域に展開したい場合を考えてみよう。マルチルールフィーチャーフラグを使用すると、以下の設定が可能だ：

- **ルール 1:**君の米国セグメントは100%に設定されている。
- **ルール 2:**あなたのセグメントはブラジルユーザーの50％に設定されているため、全員が同時にフローを受け取るわけではない。 
- **ルール 3 (Everyone Else):**その他の全ユーザーに対しては、「その他の全員」ルールを有効にし、15%に設定する。これにより、全ユーザーの一部が新しいフローでチェックアウトできるようになる。

#### まず内部テスターに連絡する

例えば、製品マネージャーとして、新製品をリリースする際に、内部テスターが常にフィーチャーフラグを受け取れるようにしたい場合を考えてみよう。内部テスターのセグメントを最初のルールに追加し、100%に設定すれば、内部テスターは全ての機能リリース時に対象となる。

## 機能フラグの"enabled"フィールドの使用 {#enabled}

フィーチャーフラグを定義したら、アプリやサイトを設定して、特定のユーザーに対してそのフィーチャーフラグのイネーブルメント状態を確認するようにする。有効になったら、ユースケースに応じて何らかのアクションを設定するか、機能フラグの変数プロパティを参照する。Braze SDKは、機能フラグのステータスとそのプロパティをアプリに取り込むためのゲッターメソッドを提供する。 
...
指定したキーに対応する型のそのようなプロパティが存在しない場合、これらのメソッドは返す`null`。
...
Brazeでユーザーがどのフィーチャーフラグを利用できるか確認するには、**オーディエンス**＞**ユーザー検索**に移動し、ユーザーを検索して選択する。

**フィーチャーフラグ**の**適格性**タブでは、プラットフォーム、アプリケーション、またはデバイスごとに適格なフィーチャーフラグのリストをフィルターできる。フィーチャーフラグの横にある<i class="fa-solid fa-eye"></i>を選択することで、ユーザーに返されるペイロードをプレビューすることもできる。

![ユーザーが利用可能なフィーチャーフラグの一覧表を示す画像, 写真。]({% image_buster /assets/img/feature_flags/eligibility.png %}){: style="max-width:85%;"}

## 変更履歴を見る

フィーチャーフラグの変更ログを見るには、フィーチャーフラグを開き、[**変更ログ**] を選択します。

![フィーチャーフラグの「編集」ページで、「変更履歴」ボタンが強調表示されている。]({% image_buster /assets/img/feature_flags/changelog/open_changelog.png %}){: style="max-width:60%;"}

ここでは、いつ変更されたのか、誰が変更したのか、どのカテゴリーに属するのか、などを確認できます。

![選択したフィーチャーフラグの変更履歴だ。]({% image_buster /assets/img/feature_flags/changelog/changelog.png %}){: style="max-width:90%;"}
