...
![Live Chat Widget という名前の例の機能フラグの詳細。このenable_live_chat.フィーチャーフラグの説明は、サポートページにライブチャットウィジェットが表示されることを示している。]({% image_buster /assets/img/feature_flags/feature-flags-use-case-livechat-1.png %})
...
![ロイヤルティ報酬プログラムという名前のフィーチャーフラグ。IDは  でありshow_loyalty_program,、これはホーム画面とプロファイルページに新しいロイヤルティプログラムが表示されることを示す説明である。]({% image_buster /assets/img/feature_flags/feature-flags-use-case-loyalty.png %})

次に、キャンバスで「高価値顧客」セグメント向けの`show_loyalty_program`フィー[チャー]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/feature_flags/)フラグをイネーブルメントする[フィーチャーフラグ]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/feature_flags/)ステップを作成する：
...
さて、50%のユーザーが旧来の体験を見て、残りの50%が新しい体験を見てくれるでしょう。その後、二つのバリアントを分析し、どちらのチェックアウトフローがより高いコンバージョン率をもたらしたかを判断できる。 {% multi_lang_include analytics/metrics.md metric='Conversion Rate' %}
