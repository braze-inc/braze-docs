...
次に、SQL を反転する必要があるかどうかを判断します。0 個のイベントを持つユーザー を直接クエリーすることはできませんが、**SQL** を反転してこれらのユーザー をターゲットにすることができます。
...
{% alert important %}
0 個のイベントを持つユーザーを特に対象にする場合を除き、SQL を反転する必要はありません。**Invert SQL**が選択されている場合は、機能が必要であり、Segmentが目的のオーディエンスと一致していることを確認します。例えば、照会が少なくとも1 つのイベントを持つユーザー を対象とする場合、逆にすると、0 のイベントを持つユーザー のみを対象とします。
{% endalert %}
