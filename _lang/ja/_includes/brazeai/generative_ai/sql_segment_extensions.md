...
>
> 標準のセグメントエクステンションと同様に、SQL セグメントエクステンションでも過去 2 年間 (730 日) までのイベントをクエリできます。標準のセグメントエクステンションとは異なり、SQLセグメントエクステンションは[クレジットを消費する](#credits)。

## 前提条件

この機能を通じて個人識別情報（PII）データにアクセスできるため、SQLセグメントクエリを実行するにはPII権限が必要だ。
...
- **完全リフレッシュ：**セグメントが更新されるたびに、Braze は利用可能なすべてのデータをクエリしてセグメントを更新します。これにより、増分更新よりも多くのクレジットが使用されます。完全更新エクステンションでは、メンバーシップを毎日自動的に更新できますが、増分更新を使用して更新することはできません。
- **増分更新:**増分更新はクエリを設定するより費用対効果の高い方法だが、設定には少し[ステップ](#step-2-write-your-sql)が多い。セグメントを構築する際にこれらの追加ステップを完了できるなら、このオプションを選択する価値がある。クエリの実行に必要なクレジットが少なくなるからだ。 
- **AI SQL生成ツール：**AI SQLジェネレーターは、平易な言語でプロンプトを入力すると、それを対象セグメント向けのSQLクエリに変換する。自分でSQLを書く必要なく、すぐに始められる方法だ。
...
2. **新規拡張機能の作成**を選択し、次に**完全な更新**を選択する。<br><br>
   ![]({% image_buster /assets/img/segment/segment_extension_modal.png %}){: style="max-width:50%" }<br><br>
3. セグメントエクステンションの名前を追加し、SQL を入力します。要件とリソースについては[ステップ2](#step-2-write-your-sql)を参照せよ。<br><br>
   ![SQLエディタがSQLセグメントエクステンションの例を示している。]({% image_buster /assets/img_archive/sql_segments_editor.png %}){: style="max-width:60%" }<br><br>
...
{:start="2"}
2\.**新規拡張機能の作成**を選択し、**増分更新**を選択する。<br><br>
   ![]({% image_buster /assets/img/segment/segment_extension_modal.png %}){: style="max-width:50%" }<br><br>
3\.セグメントエクステンションの名前を追加し、SQL を入力します。要件とリソースについては、[[SQL の作成](#writing-sql)] セクションを参照してください。<br><br>
   ![SQLエディタが示しているのは、増分SQLセグメントエクステンションの例だ。]({% image_buster /assets/img_archive/sql_segments_editor_incremental.png %}){: style="max-width:60%" }<br><br>
4. 必要に応じて、[**エクステンションを毎日再生成する**] を選択します。<br><br>
   ![毎日拡張機能を再生成するチェックボックス。]({% image_buster /assets/img_archive/sql_segments_regenerate.png %}){: style="max-width:60%" }<br><br>
...
![「先月通知を受け取ったユーザー」というプロンプトを持つAI SQL生成器]({% image_buster /assets/img/ai_sql_generator.png %}){: style="max-width:70%;"}

AI SQL ジェネレーターを使用するには、以下を実行します。

1. フル更新または増分更新のいずれかを使用して[SQLセグメント]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments)を作成した後**、AI SQLジェネレータ**を**起動**する。
2. プロンプトを入力し、**[生成]**を選択すると、プロンプトがSQLに変換される。
...
拡張機能の処理が完了したら、セグメントエクステンションを使って[セグメントを作成]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension#step-5-use-your-extension-in-a-segment)できる。この新しいセグメントを、キャンペーンやキャンバスでターゲットに設定できる。
...
さらに、標準的なSQLクエリは以下の規則に従わなければならない：

- \`statements\` `DECLARE`は使用できない。
{% endtab %}
...
2. エディターの上のフィールドから [**演算子**]、[**回数**]、および [**期間**] を選択して、スキーマの詳細を追加します。クエリでは、集計列の合計が {% raw %}`{{operator}}` および `{{number of times}}`{% endraw %} プレースホルダーで指定された特定の条件を満たすかどうかが確認されます。これは、従来のセグメントエクステンションを作成するワークフローと同様に機能します。<br><br>
   ...
   ![演算子フィールドで「より大きい」が選択されている。]({% image_buster /assets/img_archive/sql_segments_operator.png %})<br><br>
...
![SQLエディタが示しているのは、増分SQLセグメントエクステンションの例だ。]({% image_buster /assets/img_archive/sql_segments_editor_incremental.png %}){: style="max-width:65%" }
...
次に、SQLを反転させる必要があるかどうかを判断する。イベントがゼロのユーザーを直接クエリすることはできないが、**Invert SQL** を使ってこれらのユーザーをターゲットにできる。

{% alert note %}
デフォルトでは、**SQLの反転は**オンになっていない。ただし、AI SQLジェネレーターを使って否定形にする必要があるSQL文を生成する場合、ChatGPTはこの機能を自動的に有効にする出力を返す可能性がある。
{% endalert %}

例えば、購入回数が3回未満のユーザーをターゲットにするには、まず購入回数が3回以上のユーザーを選択するクエリを書く。次に、**SQL**を**反転させて、**購入回数が3回未満のユーザー（購入回数が0回のユーザーを含む）を対象に選択する。

{% alert important %}
特にイベントがゼロのユーザーをターゲットにする場合を除けば、SQLを反転させる必要はない。**SQLを反転する**を選択した場合、その機能が本当に必要か、またセグメントがオーディエンスに合致しているかを確認せよ。例えば、クエリが少なくとも1つのイベントを持つユーザーを対象とする場合、それを反転させるとイベントがゼロのユーザーのみを対象とする。
{% endalert %}
...
![SQLエディタの「メッセージング使用」セクションは、SQLセグメントが使用されている箇所を示す。]({% image_buster /assets/img_archive/sql_segments_usage.png %}){: style="max-width:70%;"}
