...
**キャンペーン分析**の期間設定は、デフォルトで現在時刻から過去90日間を表示する。これは、キャンペーンが90日以上前に開始された場合、指定した期間の分析データは「0」と表示されることを意味する。古いキャンペーンの全分析データを見るには、レポートの時間範囲を調整する。
...
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/cc-campaign-details.png %})

{% elsif include.channel == "banner" %}
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/banners/campaign_details.png %})

{% elsif include.channel == "email" %}
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/campaign_details_email.png %})

{% elsif include.channel == "push" %}
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/campaign_details_push.png %})

{% elsif include.channel == "SMS" %}
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/campaign_details_sms.png %})

{% elsif include.channel == "in-app message" %}
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/campaign_details_iam.png %})
...
![キャンペーン詳細パネルには、キャンペーンのパフォーマンスを判断するために使用される指標の概要が表示される。]({% image_buster /assets/img/campaign_details_webhook.png %})
...
![プッシュ通知のパフォーマンス分析]({% image_buster /assets/img_archive/push_message_performance.png %})

{% elsif include.channel == "SMS" %}
### SMS/MMS/RCS のパフォーマンス

**SMS/MMS/RCSパフォーマンス**パネルは、メッセージが様々な側面でどれほど良好なパフォーマンスを示したかを示す。このパネルの指標は、選択したメッセージングチャネルや、多変量テストを実行しているかどうかに応じて異なります。<i class="fa fa-eye preview-icon"></i>[**プレビュー**] アイコンをクリックすると、バリアントやチャネルごとにメッセージを表示できます。

![SMS/MMS/RCSパフォーマンスパネル。コントロールグループ、バリアント1、バリアント2のメトリクス表を含む。]({% image_buster /assets/img_archive/sms_message_performance.png %})
...
![SMS/MMSパフォーマンスパネルには、コントロールグループ、バリアント1、バリアント2のメトリクス表が含まれている。]({% image_buster /assets/img/banners/banner_performance.png %})
...
![Webhookのパフォーマンスパネルには、コントロールグループとバリアント1のメトリクス表が含まれている。]({% image_buster /assets/img/webhook_message_performance.png %})
...
![WhatsAppのパフォーマンスパネルで、バリアント1のメトリクス一覧表が含まれている。]({% image_buster /assets/img/whatsapp_message_performance.png %})
...
ヒートマップを使えば、1つのメールキャンペーンで異なるリンクがどれだけ成功したかを見ることができる。[**メッセージ分析**] セクションから、[**メールパフォーマンス**] パネルに移動します。**プレビュー&とヒートマップ**を選択して、メールキャンペーンとヒートマップのプレビューを表示する。または、バリアント名のハイパーリンクを選択してヒートマップを表示することもできます。
...
![プレビューとヒートマップページの例。メールキャンペーンと、リンクエイリアスの例とその総クリック数を示すパネルが含まれている。]({% image_buster /assets/img_archive/email_heatmap_example.png %})
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} 同じユーザーに対して複数回増加する可能性がある。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data/report_metrics/#unique-impressions">ユニークインプレッション数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Impressions' %} <span style="white-space: nowrap">このカウントは、</span>ユーザーがコンテンツカードを2回目以降に閲覧しても増加しない。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data/report_metrics/#unique-recipients">ユニーク受信者数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Recipients' %}<br><br> ユーザーは毎日ユニークな受信者となり得るため、<i>ユニークインプレッション</i>よりもこの数値が高くなることを想定すべきだ。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data/report_metrics/#unique-clicks">ユニーククリック数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} これにはBrazeが提供する配信停止リンクのクリックも含まれる。</td>
        </tr>
        <tr>
...
メッセージの可視性をカバーする指標はいくつかある。これには_ユニークな_受信者と_ユニ__ークな__イン_プレ_ッション_が含まれる。これらの指標をよりよく理解するために、いくつかのシナリオ例を使ってみよう。
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} バナー広告については、インプレッションはユーザーセッションごとに1回記録される。同一のセッション内で同じバナーが複数回表示された場合、インプレッションは1回のみ記録される。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-impressions">ユニークインプレッション数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Impressions' %} <span style="white-space: nowrap">各ユーザーは一度だけカウントされる。</span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks No Dispatch ID' %} 各ユーザーは一度だけカウントされる。</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Recipients' %}<br><br> 閲覧者は毎日のユニーク受信者となる可能性があるため、<i>ユニークインプレッション数</i>よりもこの数値の方が高くなると予想されます。</td>
        </tr>
        <tr>
...
メッセージの可視性をカバーする指標はいくつかある。これには_ユニークな_受信者と_ユニ__ークな__イン_プレ_ッション_が含まれる。これらの指標をよりよく理解するために、いくつかのシナリオ例を使ってみましょう。

例えば、今日バナーを閲覧し、翌日同じバナーを閲覧し、さらにその翌日も閲覧した場合、あなたは_ユニークな_受信者として3回カウントされる。ただし、_ユニークインプレッション_は1回のみカウントされます。

別の例として、バナー広告キャンペーンでFIVE回の_ユニークインプレッション_を確認したとしよう。これは、以下の全ステップを実行したユーザー端末がわずか5台のみであることを意味する。

1. セッションを開始した、またはアプリが明示的にバナーの同期を要求した（またはその両方）
2. バナービューに移動した
3. SDK がインプレッションを記録し、サーバーにログを記録しました

_ユニークな受信者とは、_実際に表示されたバナーを指す。
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-clicks">ユニーククリック数</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} これはメールについて7日間の期間でトラッキングされ、によって測定される<a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a>。これには、Brazeが提供する配信停止リンクのクリックも含まれる。この数値は5から10% の範囲内でなければなりません。10%を超えるものは例外です。
            </td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-opens">ユニーク開封数</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Opens' %} メールについては、7日間の期間でトラッキングされる。この数値は30から40% の範囲内でなければなりません。40%を超えるものは例外です。
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Spam' %} この指標が0.08を超える場合、メッセージの文面が売り込み色が強すぎるか、あるいはメールアドレスの収集方法を見直すべきだという兆候かもしれない（つまり、実際にメッセージングを行いたいと思っている相手にメッセージを送っているか確認する必要がある）。
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split"> {% multi_lang_include analytics/metrics.md metric='Estimated Real Opens' %} 詳細は次のセクションを参照せよ。</td>
        </tr>
        <tr>
...
この統計は、Brazeが独自に作成した分析モデルを使用して、機械開封が存在しないかのようにキャンペーン独自の開封率の推定値を再構築する。メール送信者から一部の開封イベントについて「*機械*による*開封*」というラベルを受け取る場合がある（上記参照）。しかし、このラベルは実際の開封を機械による開封と誤って分類することが多い。言い換えれば、「*その他の開封*」は（実際のユーザーによる）実際の開封を過小評価している可能性が高い。その代わり、Brazeは各キャンペーンのクリックデータを使って、実際の人間がメッセージを開封した率を推測している。これにより、Apple の MPP を含むさまざまなマシン開封メカニズムが補われます。

_推定実質開封率_はメール送信開始から36時間後に算出され、その後24時間ごとに再計算されます。キャンペーンが繰り返される場合、推定は別の送信が発生してから36時間後に再計算されます。

この指標は継続的に再計算されるため、_推定実際の開封率は_時間の経過とともに変化する可能性がある。新しいエンゲージメント信号（開封やクリックなど）が受信され、モデルに組み込まれるにつれて、その値は変動するのだ。実際には、_推定実際の開封_率は、キャンペーンがアクティブな間、毎日更新され続けることがある。
...
結局のところ、メールが当社のサーバーを離れた後は、その後何が起きるかについて把握できる範囲は限られている。ただし、結果に影響するNHIを管理するための推奨事項は以下の通りだ：

1. この事象は、すべての送信者とほぼすべての受信者に発生する可能性があることに注意してください。クリック数は、開封数と同様に、メッセージに対する人間のインタラクションを示す完全に信頼できる指標ではないため、NHI を防ぐことはできません。
2. より高いポジティブなエンゲージメントは、より低いNHIと相関する傾向がある。したがって、メールメッセージングの[ベストプラクティス]({{site.baseurl}}/user_guide/message_building_by_channel/email/best_practices)に従うことが重要だ。これには、通常のケイデンスでメールとSunsetting (配信停止)の未調整のサブスクライバーを送信するための明示的な権限をユーザーから取得することも含まれます。 
3. 可能な限り、メールにHTTPSリンクを使用する。NHI は、安全なリンクを使用する送信者に対してはそれほど一般的に発生しません。
4. ワンクリックで配信停止できる仕組みを使うなら、ユーザーが通知設定を編集・管理できるページへ誘導する[ユーザー設定センター]({{site.baseurl}}/user_guide/message_building_by_channel/email/preference_center/overview)を作成することを検討すべきだ。NHI によって誤ってユーザーの登録を解除してしまう可能性があるため、これは有用です。
...
6. メールキャンペーンに隠しリンクを追加する。このリンクは、白地に白のテキストや句読点など、人間が気づかないようなものだろう。ボットはリンクを全てクリックする傾向がある。したがって、見えないリンクでクリックイベントを生成しているユーザーは、実際にはNHI（非人間的インタラクション）の結果だと結論付けられる。つまり、開封やクリックが必ずしも積極的なエンゲージメントを示しているわけではない。
...
![「レポート用識別子」フィールドの値が「0」である。]({% image_buster /assets/img/identifier_for_reporting.png %}){: style="max-width:50%;"}
{% endalert %}
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#bounces">バウンス数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Bounces' %} <a href="#bounced-push">バウンスしたプッシュ通知を</a>参照せよ。</td>
        </tr>
        <tr>
...
プッシュ通知の配信停止はキャンペーン分析の指標に含まれておらず、AppleやGoogleなどのプロバイダーによるユーザーのプッシュ通知ステータスの更新に依存する。これらの更新は、頻度が低く、予測不可能な場合があります。そのため、プッシュの配信停止は、プッシュキャンペーン分析の指標として含まれません。 

ただし、手動でプッシュの配信停止をトラッキングすると、通知の頻度とコンテンツの関連性に対するユーザー応答に関する重要なインサイトが得られます。プッシュ通知の配信停止のトラッキング, 追跡方法は以下の2つだ：セグメントフィルターやカスタムフィルターを使う。

{% tabs local %}
{% tab Segment filters %}

プッシュ通知をイネーブルメントしていないユーザーを識別するセグメントを作成できる。つまり、サブスクライバーまたはオプトインしておらず、[フォアグラウンドプッシュトークン]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_registration/#push-tokens)を持っていないユーザーだ。例えば、アプリ内の配信停止者数を確認するには、以下のセグメントを「OR」条件で組み合わせる。 

- `Background or Foreground Push Enabled is false`
- `Has Uninstalled`

![アプリに対する「バックグラウンドまたは前景プッシュのイネーブルメント」フィルターが偽であり、「アンインストール済み」フィルターが選択されているセグメントビルダーセクション。]({% image_buster /assets/img/push_unsub_segment_example.png %})

セグメンテーションフィルターはおおよその目安であり、特定の日付やキャンペーンに結びつけることはできない。

{% endtab %}
{% tab Custom filters %}

{% alert important %}
サブスクリプション変更のカスタムイベントをログに記録すると[、データポイントが]({{site.baseurl}}/user_guide/data_and_analytics/data_points#consumption-count)記録される。または、セグメントフィルターを使用して、プッシュが有効になっていないユーザーを識別し、ターゲットにします。
{% endalert %}
...
![2021年2月から2022年5月までのメールに関する統計例を示す、履歴パフォーマンスパネルのグラフ。]({% image_buster /assets/img/cc-historical-performance.png %})
...
![キャンペーンレベルのSMS/MMS/RCSキーワード応答パネルには、時間経過に伴うキーワード分布の折れ線グラフと、キーワードカテゴリセクションが含まれる。このセクションには、オプトイン、オプトアウト、ヘルプ、その他、詳細、コーチングのチェックボックスが選択されている。]({% image_buster /assets/img/sms/keyword_responses.png %})
...
![折れ線グラフの下にある表には、キーワードカテゴリ、レスポンス分布、リターゲティングの列がある。ここでキーワードカテゴリでセグメントを作成する選択肢が与えられる。]({% image_buster /assets/img/sms/keyword_segments.png %})
...
![1次コンバージョンイベントAからのユーザー属性と行動に関する分析を含むコンバージョン相関パネル。]({% image_buster /assets/img/convcorr.png %})
