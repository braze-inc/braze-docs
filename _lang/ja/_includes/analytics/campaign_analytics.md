...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} これは、同じユーザーに対して何度も増分することができます。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data/report_metrics/#unique-impressions">ユニークインプレッション数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Impressions' %} <span style="white-space: nowrap">この数</span>は、ユーザーがコンテンツカードを表示する2回目の時間を増やしません。</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} これには、Braze提供の配信停止リンクのクリックも含まれます。</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} バナーの場合、インプレッションはユーザー セッションごとに1 回ログに記録されます。同じセッション内で同じバナーが複数回表示される場合、1 つのインプレッションのみがログに記録されます。</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-impressions">ユニークインプレッション数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Impressions' %} <span style="white-space: nowrap">それぞれのユーザーは1 回だけカウントされます。</span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks No Dispatch ID' %} それぞれのユーザーは1 回だけカウントされます。</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-clicks">ユニーククリック数</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} これは、メールの7 日間にわたって追跡され、<a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a> によって測定されます。これには、Brazeが提供する配信停止リンクのクリックも含まれる。この数値は5から10% の範囲内でなければなりません。10%を超えるものは例外です。
            </td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-opens">ユニーク開封数</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Opens' %} メールの場合、これは7 日間にわたって追跡されます。この数値は30から40% の範囲内でなければなりません。40%を超えるものは例外です。
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Spam' %} このメトリクスが 0.08 より大きい場合は、メッセージコピーが売れすぎるか、メールのアドレス収集方法を再検討する必要があります (通信に興味を持つ人物がメッセージングいることを確認するため)。
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split"> {% multi_lang_include analytics/metrics.md metric='Estimated Real Opens' %} 詳細については、次のセクションを参照してください。</td>
        </tr>
        <tr>
...
このメトリクスは進行中のケイデンスで再計算されるため、_推定実質開封率_値は、新しいエンゲージメントシグナル(開封sやクリックなど)が受信されてモデルに組み込まれると、時とともに変化する可能性があります。実際には、_推定実質開放率_は、キャンペーンが有効な間、毎日更新し続けることができます。
...
4. シングルクリックの配信停止処理を使用する場合は、ページにユーザー を渡して通知の環境設定を編集および管理する[ユーザー設定センター]({{site.baseurl}}/user_guide/message_building_by_channel/email/preference_center/overview) を作成することを検討してください。NHI によって誤ってユーザーの登録を解除してしまう可能性があるため、これは有用です。
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#bounces">バウンス数</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Bounces' %} <a href="#bounced-push">バウンスプッシュ通知s</a>を参照してください。</td>
        </tr>
        <tr>
...
セグメンテーション フィルター はアプリ近接であり、日付とキャンペーンに特に結びつけることはできません。
