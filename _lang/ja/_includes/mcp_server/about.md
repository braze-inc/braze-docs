...
[Braze MCP サーバー]{% if include.section == "user" %}({{site.baseurl}}/user_guide//user_guide/brazeai/mcp_server/setup/){%elsif include.section=="developer" %}({{site.baseurl}}/developer_/developer_guide/mcp_server/setup/){%endif %}などを設定すると、エージェント、アシスタント、チャットボットなどのAI ツールをBraze に直結し、キャンバスやキャンペーン分析、カスタム属性s、SegmentsBraze MCP サーバーは次の用途に適しています。
