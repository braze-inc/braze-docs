...
フォアグラウンド通知の処理は、プラットフォームやセットアップによって異なる。自分の統合に合ったアプローチを選択する：

{% tabs local %}
{% tab iOS %}
iOSの場合、フォアグラウンド通知の処理は、ネイティブのSwift統合と同じだ。`UNUserNotificationCenterDelegate.userNotificationCenter(_:willPresent:withCompletionHandler:)` の実装内部で`handleForegroundNotification(notification:)` を呼び出す。

完全な詳細とコード例については、Swift のプッシュ通知ドキュメントの[Handling foreground notifications]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift#handling-foreground-notifications)を参照のこと。
{% endtab %}

{% tab Android %}
Androidの場合、フォアグラウンド通知の処理は、Androidネイティブの統合と同じである。`FirebaseMessagingService.onMessageReceived` メソッド内部で`BrazeFirebaseMessagingService.handleBrazeRemoteMessage` を呼び出す。

詳細とコード例については、Androidプッシュ通知ドキュメントの[Handling foreground notificationsを]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=android#handling-foreground-notifications)参照のこと。
{% endtab %}

{% tab Expo %}
Expoマネージャーのワークフローでは、ネイティブの通知ハンドラを直接呼び出すことはない。代わりに、Expo Notifications APIを使用してフォアグラウンド表示をコントロールし、Braze Expo Pluginがネイティブ処理を自動的に処理する。
...
{% alert note %}
Expoマネージャーのワークフローでは、Braze Expo Pluginがネイティブプッシュ処理を自動的に行う。フォアグラウンドUIのコントロールは、上に示したExpo Notificationsの表示オプションで行う。
{% endalert %}

素のワークフロー統合については、代わりにiOSやAndroidのネイティブ・アプローチに従おう。
{% endtab %}
{% endtabs %}

### ステップ 5: テストプッシュ通知を送信する
...
{% alert tip %}
iOS統合については、[プッシュ通知のセットアップチュートリアルを](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/b1-standard-push-notifications)参照し、プロジェクトの依存関係との潜在的な競合を特定するのに役立てることができる。
{% endalert %}
