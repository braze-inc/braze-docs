...
- **コードベースの統合：**お好みのパッケージマネージャーまたはBraze CDNを使用して、Web Braze SDKをコードベースに直接統合する。これにより、SDKをどのように読み込んで設定するかを完全にコントロールすることができる。
...
{% alert important %}
[NPMの統合方法を]({{site.baseurl}}/developer_guide/sdk_integration/?subtab=package%20manager&sdktab=web)使うことをお勧めする。SDKライブラリーをWebサイトにローカルに保存できる、広告ブロッカーの拡張機能から保護される、バンドラーサポートの一環として読み込み時間の短縮に貢献する、などの利点がある。
{% endalert %}
...
{% alert important %}
Safariのデフォルトの**Prevent Cross-Site Tracking**設定は、CDN統合方法を使用する際に、バナーやコンテンツカードのようなアプリ内メッセージタイプが表示されないようにすることができる。この問題を回避するには、NPM統合メソッドを使用し、Safariがこれらのメッセージをクロスサイトトラフィックとして分類しないようにし、Webユーザーがすべてのサポートブラウザでメッセージを見ることができるようにする。
{% endalert %}
...
{% alert note %}
**Web SDKリクエスト用のカスタムドメインはサポートされていない**：Web SDK`baseUrl` はBraze SDKエンドポイントでなければならない（例：`sdk.iad-05.braze.com` ）。Brazeは、CNAMEレコードによる顧客所有ドメイン経由のWeb SDKトラフィックのルーティングをサポートしていない。Web SDKリクエストを独自ドメインから発信する必要がある場合は、Brazeサポートに連絡すること。
{% endalert %}
...
{% alert important %}
**アプリ内メッセージ表示**：アプリ内メッセージがトリガーされたときに自動的に表示するには、`braze.automaticallyShowInAppMessages()` 。この呼び出しがないと、アプリ内メッセージは自動的に表示されない。メッセージ表示を手動で管理したい場合は、この呼び出しを削除し、代わりに`braze.subscribeToInAppMessage()` 。詳しくは[アプリ内メッセージ配信を]({{site.baseurl}}/developer_guide/in_app_messages/delivery/)参照のこと。
{% endalert %}
...
Braze Web SDKを当社のコンテンツデリバリーネットワーク、例えば`https://js.appboycdn.com/web-sdk/a.a/braze.min.js` （当社のデフォルトの統合手順で推奨されている）から参照すると、ユーザーがサイトを更新する際に、マイナー更新（バグフィックスと下位互換機能、上記の例ではバージョン`a.a.a` ～`a.a.z` ）を自動的に受け取る。

ただし、弊社が大きな変更をリリースする際には、変更点がお客様の統合に影響しないように、Braze Web SDKを手動でアップグレードしていただく必要があります。さらに、SDKをダウンロードして自分でホスティングする場合、バージョン更新は自動的には行われないので、手動でアップグレードして最新の機能やバグ修正を受ける必要がある。
...
AMP Web Pushコンポーネントは、サブスクリプションを処理するためのポップアップを作成するので、この機能をイネーブルメントするには、プロジェクトに以下のヘルパーファイルを追加する必要がある：
...
詳細や詳細なTealium設定サポートについては、[インテグレーションドキュメントを]({{site.baseurl}}/partners/data_and_infrastructure_agility/customer_data_platform/tealium/#about-tealium)ご覧になるか、Tealiumアカウントマネージャーにお問い合わせください。
