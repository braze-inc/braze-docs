...
`configuration.push.automation` 設定オプションを使用して[プッシュ通知]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift) を設定した場合、Braze はデフォルトのプッシュカテゴリのアクションボタンを自動的に登録し、プッシュアクションボタンのクリックアナリティクスとURL ルーティングを処理します。
