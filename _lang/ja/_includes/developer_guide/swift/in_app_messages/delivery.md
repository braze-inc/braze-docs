...
SDKのセッション開始の仕組みについて詳しくは、[セッションのライフサイクルを参照せよ]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/?tab=swift)。

### デフォルトのレート制限

デフォルトでは、アプリ内メッセージは 30 秒に 1 回送信できます。

これを上書きするには、Brazeインスタンスが初期化される前に、Braze設定に`triggerMinimumTimeInterval`プロパティを追加する。任意の正の整数に設定でき、最小の時間間隔を秒単位で表す。以下に例を示します。
...
また、アプリ内でアプリ内メッセージをリアルタイムに表示することもできる。その場合は、アプリ内で手動でメソッド[`present(message:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/brazeinappmessagepresenter/present(message:))を呼び出す必要がある`inAppMessagePresenter`。以下に例を示します。
