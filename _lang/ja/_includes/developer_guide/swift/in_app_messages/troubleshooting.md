...
Braze とサードパーティのネットワークロギングライブラリを統合する場合、開発者はドメインコード `-1008` の `NSURLError` に遭遇することがよくあります。このエラーは、画像やフォントなどのアセットが取得できなかったか、キャッシュできなかったことを示しています。このような場合を回避するには、これらのライブラリで無視するドメインのリストにBraze CDN URL を登録する必要があります。
...
以下は、Braze アセットキャッシュと競合することが知られているライブラリと、この問題を回避するためのサンプルコードです。使用できないリソース・エラーを引き起こすライブラリを使用しているプロジェクトで、以下にリストアップされていない場合は、そのライブラリのドキュメントを参照して、同様の使用APIを確認してほしい。
