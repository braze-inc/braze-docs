...
<br><br>![[ロールの作成] ボタン。]({% image_buster /assets/img/create_role_1_list.png %})()<br><br>
...
{: start="5"}
5. 作成したロールのARNと生成した外部IDは、Cloud Data Ingestion統合を作成するために必要なので、控えておくこと。
...
![新しいインポート同期を作成するために S3で表示されるセキュリティ認証情報の例。]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_1.png %})()

{: start="3"}
3\.統合に名前を付け、この統合のデータ型を選択する。 

<br><br>![データタイプとしてユーザー属性を選択して「cdi-s3-as-source-integration」の同期の詳細を設定する。]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_2.png %})()<br><br>

{: start="4"}
4\.アクセスや権限の問題で同期が切れた場合に通知を受け取る連絡先メールアドレスを追加します。オプションで、ユーザーレベルのエラーと同期の成功の通知をオンにします。 

<br><br> ![同期エラー通知の通知設定を設定する。]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_3.png %})()<br><br>
