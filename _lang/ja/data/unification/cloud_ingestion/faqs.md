...
## 将来のUPDATED_AT が統合と同期された場合、どのアプリが終了しますか?
...
CDI は `UPDATED_AT` を使用して、同期中に取得するレコードを決定します。どのように機能するかは、[このイラストを]({{site.baseurl}}/user_guide/data_and_analytics/cloud_ingestion/overview/#what-gets-synced)ご覧いただきたい。同期実行の開始時に、CDI はウェアハウスに問い合わせて、以前に処理された`UPDATED_AT` タイムスタンプと同じかそれ以降の`UPDATED_AT` を持つすべてのレコードを取得します。クエリー実行時にピックアップされたレコードはすべてBrazeに同期される。以下は、レコードが同期されない可能性のある一般的なケースである：
