...
> Braze クラウドデータ取り込み(CDI)を使用すると、データストレージソリューションから直接接続を設定して、Brazeに関連するユーザーデータや他の非ユーザーデータを同期できます。このデーターは、パーソナライゼーションまたはセグメンテーションに使用してマーケティング ユースケースに電力を供給できます。Cloud Data Ingestion の柔軟な統合は、ネストされたJSON やオブジェクトの配列などの複雑なデータ構造をサポートします。
...
Braze クラウドデータインジェスト機能を使用すると、次のことができます。

- お使いのデータウェアハウスやファイルストレージソリューションから Braze へのシンプルな直接統合を構築できます。所要時間はわずか数分です。
- \- 属性、イベント、購入などのユーザーデータを、データウェアハウスから Braze に安全に同期します。
- \- クラウドデータ取り込みを Currents または Snowflake のデータ共有と組み合わせることで、Braze でデータループを閉じます。

また、[Connected Sources]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/connected_sources) はゼロコピーの選択肢です。Brazeは、基礎となるデータをBrazeにコピーせずに、データウェアハウスまたはファイルストレージソリューションに直接照会して、CDI Segments -allを構築できます。

## サポートされるデータソース

:\*\*クラウドデータ取り込みは、以下からのデータを同期できます。**
