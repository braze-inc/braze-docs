...
{% multi_lang_include analytics/metrics.md metric='Bounces' %} これは、有効なプッシュトークンがないか、キャンペーンが起動された後のユーザー 配信停止d、またはメールアドレスがキュレートされていないか非アクティブになっているために発生する可能性があります。
...
詳細については、{% multi_lang_include analytics/metrics.md metric='Body Clicks' %}iOS{% multi_lang_include analytics/metrics.md metric='Body Clicks' %}および[Android]({{site.baseurl}}/developer_guide/platform_integration_guides/ios/changelog/objc_changelog#3310)のSDK変更ログを参照してください。
...
{% multi_lang_include analytics/metrics.md metric='Button 1 Clicks' %} _ボタン1クリック_のレポートは、アプリ内メッセージで**Reporting**の識別子を"0"として指定した場合にのみ機能します。
...
{% multi_lang_include analytics/metrics.md metric='Button 2 Clicks' %} _Button 2 Clicks_のレポートは、アプリ内メッセージで**Reporting**の識別子を"1"として指定した場合にのみ機能します。
...
Braze のお客様の場合、配信数は SMS 割り当てを消費します。 
...
この定義されたイベントは、キャンペーンを作成するときにあなたが決定します。 
...
<a href="/docs/braze_support/"> Braze サポート</a> に問い合わせて、配信失敗の理由を確認してください。
...
{% multi_lang_include analytics/metrics.md metric='Errors' %} エラーは<i>送信</i>カウントに含まれますが、<i>固有受信者</i>カウントには含まれません。
...
{% multi_lang_include analytics/metrics.md metric='Failures' %} 失敗は<i>Sends</i>カウントに含まれますが、<i>Deliveries</i>カウントには含まれません。</td>
...
{% multi_lang_include analytics/metrics.md metric='Help' %} ユーザー応答は、ユーザーがメッセージを受信してから4時間以内に受信メッセージを送信するたびに測定されます。
...
この指標の追跡は、SendGrid の場合は 2021 年 11 月 11 日から、SparkPost の場合は 2021 年 12 月 2 日から開始されます。Amazon SES の場合、分析は_開封数_として表示されます。ただし、クリックに対するボットフィルターはサポートされる予定です。
...
{% multi_lang_include analytics/metrics.md metric='Opt-Out' %} ユーザー応答は、ユーザーがメッセージを受信してから4時間以内に受信メッセージを送信するたびに測定されます。
...
ユーザーは、マシン開封数が記録される前に、({% multi_lang_include analytics/metrics.md metric='Other Opens' %}他の開封{% multi_lang_include analytics/metrics.md metric='Other Opens' %}に対する開封数などの) メールも開封できることに注意してください。あるユーザーが、マシン開封イベント後に Apple Mail 以外の受信トレイメールを1回 (またはそれ以上) 開封した場合、ユーザーによるメール開封数がその他の開封数に加算され、ユニーク開封数には1回のみが加算されます。
...
Braze のお客様の場合、拒否数は SMS 割り当てを消費します。
...
{% multi_lang_include analytics/metrics.md metric='Soft Bounce' %} _ソフトバウンス_は_Deferrals_とは異なる点に注意してください。この再試行期間中にメールが正常に配信されなかった場合、Brazeは送信されたキャンペーンごとに1つのソフトバウンスイベントを送信する。2025年2月25日以前は、これらの再試行は1回のキャンペーン送信に対して複数のソフトバウンスとしてカウントされていた。
...
{% multi_lang_include analytics/metrics.md metric='Total Dismissals' %} ユーザーが同じキャンペーンから2つの別々のカードsを受け取り、両方を消去した場合、この数は2つ増えます。再適格性により、ユーザーがカードを受け取るたびに_却下数の合計_を増やすことができます。各カードは異なるメッセージです。
...
この値は、Braze が SDK から受け取ったインプレッションイベント数の合計です。
...
{% multi_lang_include analytics/metrics.md metric='Total Revenue' %} このメトリクスは、<a href='https://braze.com/docs/user_guide/data_and_analytics/reporting/report_builder/'>レポートビルダ</a>からキャンペーン比較レポートでのみ使用できます。
