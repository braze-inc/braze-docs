...
- "r_details" をプロパティとして一覧表示するAPI リクエスト
- その順序のネストされたプロパティ
...
![プロパティ フィルターr_details.name を追加するユーザーは、カスタムイベントのマクドナルドです。]({% image_buster /assets/img/nested_object1.png %})
...
### ネストされたオブジェクトを使用すると、追加のデータポイントs が記録されますか?

この機能を追加しても、データポイントs の記録方法に変更はありません。ネストされたオブジェクトに基づくセグメンテーションでは、追加のデータポイントを使用しないSegment Extensions が使用されます。
