...
利用レポートを表示する際、「**値」**タブを選択すると、約25万ユーザーのサンプルに基づき、選択したカスタム属性のトップ値が表示される。結果はユーザーのサブセットからサンプリングされるため、サンプルには既存の値がすべて含まれるわけではないことに注意。つまり、「**値」**タブは、トラブルシューティングや全ユーザーのデータを取り込む必要のあるユースケースには使うべきではないということだ。

![US "や "PR "などの国属性値の円グラフを表示する "Values "タブが開封された、選択されたカスタム属性の使用状況レポート。]({% image_buster /assets/img/usage_report_values.png %}){: style="max-width:80%;"}
