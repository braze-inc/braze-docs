...
<b>SMS:</b>セグメント<br>
<b>MMS:</b>セグメント<br><br>
つまり、WhatsAppメッセージあたりのクレジットは会話の開始時に計算され、SMS および MMS 両方のメッセージあたりのクレジットは送信されたセグメントに基づいて計算されます。
<br><br>
...
| **1回の送信あたりのクレジット** | 1つのメッセージを送信するためのメッセージクレジットの正確な数<br> (メッセージあたりのクレジット = チャネルのクレジット比率 x 宛先係数） |
...
<table>
    ...
    <tr>
        ...
        <th><b>送信先</b></th>
        <th><b>乗数</b></th>
        <th class="credits-column"><b>1回の送信あたりのクレジット</b></th>
    </tr>
    <tbody><tr>
        <td>SMS - 米国 / カナダ</td>
        <td>0.7</td>
...
{% alert note %}
2024年11月1日現在、サービスタイプの会話は無料で、購入クレジットの割り当てから差し引かれません。
{% endalert %}
