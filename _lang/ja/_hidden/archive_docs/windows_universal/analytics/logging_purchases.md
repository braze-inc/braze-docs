---
nav_title: 購入のロギング
article_title: Windows Universal の購入の記録
platform: Windows Universal
page_order: 4
description: "このリファレンス記事では、Windows ユニバーサルプラットフォームで購入を記録する方法について説明します。"
hidden: true
---
...
実施にあたっては、[ベストプラクティス][3]の記事で、カスタムイベント s、カスタム属性 s、購買イベントが提供するセグメンテーションの選択肢の事例を必ず確認してください。また、[イベントの命名規則]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/event_naming_conventions/)についてもよく理解しておくことをお勧めします。

この機能を使用するには、アプリ内購入が正常に完了した後でこのメソッド呼び出しを追加します。

購入は、`EventLogger` を使用してログに記録されます。これは、IAppboy で公開されるプロパティです。`EventLogger` への参照を取得するには、`Appboy.SharedInstance.EventLogger` を呼び出します。
