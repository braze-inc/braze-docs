...
> [LiftIgniterは](https://www.liftigniter.com/)、あらゆるタッチポイントにおけるリアルタイムのパーソナライゼーションを通じて、企業のカスタマーエクスペリエンスの変革を支援するパーソナライゼーションプラットフォームのリーディングカンパニーである。

_この統合は Liftigniter によって管理されます。_

## 統合について

LiftIgniterとBrazeの統合はコネクテッドコンテンツを使用し、ニュース記事、衣料品、その他の小売店（店鋪）や動画などの興味深いトピックをレコメンドできるようにする。
...
![LiftIgniter 固有のコネクテッドコンテンツ呼び出しを含むプッシュキャンペーン。画像フィールドにはコネクテッドコンテンツロジックも追加されている。]({% image_buster /assets/img/liftigniter.png %})

このメッセージを作成画面の本文に入力すると、メッセージをプレビューできます。以下に示すように、画像を取得することもできます。

![送信後に表示されるメッセージのプレビュー画像。]({% image_buster /assets/img/liftigniter2.png %})
