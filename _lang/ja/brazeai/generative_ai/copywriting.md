...
プッシュを出力の長さとして使用する場合、**過去のキャンペーンデータを参照するを**選択すると、ランダムに選択された過去のモバイルプッシュキャンペーンがOpenAIに送信され、GPTはそれらをコピー生成のベースとして使用することができる。Currents, AIコピーライターはLiquid構文を持たないOpenAIにプッシュキャンペーンを送る。この機能を活用しない場合は、このボックスをオフのままにします。Braze および OpenAI でのデータの使用方法の詳細については、次のセクションを参照してください。 
