...
これで、認証されていないログイン・ユーザーがアプリのAPIキーを使って、他のユーザーに偽装したりなりすまして悪意のあるアクションを実行するのを防ぐことができる。
...
{% endtab %}
{% tab React Native %}
SDK認証は、ネイティブSDKの初期化中にイネーブルメントにする必要がある。iOSとAndroidのネイティブコードに以下の設定を追加する：
...
ネイティブレイヤーでSDK認証を有効にしたら、以下のステップで示すReact Native JavaScriptのメソッドを使用できる。
{% endtab %}
{% tab Java %}
...
{% endtab %}
{% tab Flutter %}
SDK認証は、iOSやAndroidのネイティブコードでSDKを初期化する際にイネーブルメントにする必要がある。ネイティブレイヤーでイネーブルメントにすると、Flutter SDKのメソッドを使ってJWTシグネチャを渡すことができる。

**iOS**

SDK 認証を有効にするには、iOS ネイティブコードで`configuration.api.sdkAuthentication` プロパティを`true` に設定する：
...
ネイティブレイヤーでSDK認証をイネーブルメントにしたら、次のステップで示すFlutter SDKのメソッドを使うことができる。
{% endtab %}
{% tab Unity %}
SDK認証は、ネイティブSDKの初期化中にイネーブルメントにする必要がある。iOSとAndroidのネイティブコードに以下の設定を追加する：
...
設定ファイルで、`SDKAuthenticationEnabled` プロパティを`true` に設定する：
...
ネイティブレイヤーでSDK認証をイネーブルメントした後、以下のステップで示すUnity C#メソッドを使用できる。
{% endtab %}
{% tab Cordova %}
SDK認証は、ネイティブSDKの初期化中にイネーブルメントにする必要がある。iOSとAndroidのネイティブコードに以下の設定を追加する：
...
SDK 認証を有効にするには、`config.xml` の`enableSDKAuthentication` プロパティを`true` に設定する：
...
ネイティブレイヤーでSDK認証をイネーブルメントした後、以下のステップで示すCordova JavaScriptのメソッドを使用することができる。
{% endtab %}
{% tab .NET MAUI (Xamarin) %}
SDK認証は、ネイティブSDKの初期化中にイネーブルメントにする必要がある。SDK認証をiOSとAndroidで別々に設定する：

**iOS**

SDK 認証を有効にするには、SDK の初期化時に`configuration.Api.SdkAuthentication` プロパティを`true` に設定する：
...
SDK認証を有効にした後、以下のステップで示す.NET MAUIメソッドを使用できる。
{% endtab %}
{% tab Expo %}
Braze Expoプラグインを使用する場合は、アプリの設定で`enableSdkAuthentication` プロパティを`true` に設定する。これにより、手動でネイティブコードを変更することなく、ネイティブiOSおよびAndroidレイヤーのSDK認証が自動的に設定される。
...
アプリの設定でSDK認証をイネーブルメントにした後、React Nativeタブに表示されているReact Native JavaScriptメソッドを以下のステップで使用できる。

{% alert note %}
完全な実装例については、GitHubの[Braze Expoプラグインサンプルアプリを](https://github.com/braze-inc/braze-expo-plugin/blob/main/example/components/Braze.tsx)参照のこと。
{% endalert %}
{% endtab %}
{% endtabs %}
...
Braze Expoプラグインを使用する場合は、同じReact Native SDKのメソッドを使用する。`changeUser` を呼び出す際に JWT を提供する：
...
認証エラーを受け取ったら、エラーの`userId` 、Currentsログインユーザーと一致することを確認し、サーバーから新しい署名を取得し、Braze SDKに提供する。これらのエラーは、モニタリングサービスやエラーレポートサービスに記録することもできる。
...
`Braze` インスタンスに SDK 認証デリゲートを設定する：
...
{% endtab %}
{% tab Expo %}
Braze Expoプラグインを使用する場合は、同じReact Native SDKのメソッドを使用する：
