...
{% alert tip %}
より詳細な調査のためには、詳細なSDK出力を取得するために[詳細ログ記録をイネーブルメントすることも]({{site.baseurl}}/developer_guide/sdk_integration/verbose_logging)できる。また、特定のチャネルに関する[詳細ログの読み方についても学習することができる]({{site.baseurl}}/developer_guide/sdk_integration/reading_verbose_logs)。
{% endalert %}

## 前提条件

Braze SDKデバッガーを使用するには、「個人識別情報（PII）の閲覧」および「ユーザープロファイルのPII準拠閲覧」権限が必要だ。デバッグセッションのログをダウンロードするには、「ユーザーデータのエクスポート」権限も必要だ。さらに、Braze SDK は、次の最小バージョンを満たしているか、参照している必要があります。 
...
![「SDKデバッガー」が強調表示された「セットアップとテスト」セクション。]({% image_buster /assets/img/sdk_debugger/select_sdk_debugger.png %})
...
![選択したユーザー向けのデバッグページ。]({% image_buster /assets/img/sdk_debugger/search_and_select_user.png %}){: style="max-width:85%;"}
...
![「セッション終了」ボタンが表示されているデバッグセッション。]({% image_buster /assets/img/sdk_debugger/close_debugging_session.png %}){: style="max-width:85%;"}
...
![セッション終了後に表示されるデバッグページには、「ログのエクスポート」と「セッションIDのコピー」が表示される。]({% image_buster /assets/img/sdk_debugger/copy_id_and_export_logs.png %})
