...
Brazeでキャンペーンを作成する際、キーと値のペアをパラメータとして設定できる。このパラメータは、アプリ内`extras`メッセージングオブジェクトがデータをアプリに送信する際に使用できる。
...
メッセージのタイミングをより高度にコントロールする方法（トリガーメッセージの遅延や復元を含む）については、チュートリアル[を参照せよ。トリガーメッセージの延期と]({{site.baseurl}}/developer_guide/in_app_messages/tutorials/deferring_triggered_messages)復元。
{% endtab %}

{% tab android %}
1. カスタムリスナーを設定するために[`IInAppMessageManagerListener`](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?sdktab=android&tab=global%20listener#android_step-1-implement-the-custom-manager-listener)実装する。
2. メソッド[`beforeInAppMessageDisplayed()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-displayed.html)を更新して、を返すようにしろ[`InAppMessageOperation.DISCARD`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-operation/-d-i-s-c-a-r-d/index.html)。

メッセージのタイミングをより高度にコントロールするには、後表示や再キューイングを含む詳細については、[メッセージングのカスタマイズ](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?tab=global%20listener&subtab=kotlin#android_step-2-instruct-braze-to-use-the-custom-manager-listener)ページを参照せよ。
{% endtab %}

{% tab swift %}
1. `BrazeInAppMessageUIDelegate` デリゲートをアプリに実装する。完全な手順については、チュートリアル[を参照せよ。アプリ内メッセージ UI ](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/c1-inappmessageui).
...
特定のサイレントプッシュ通知をリッスンするには、カスタムプッシュコールバックを登録します。詳細については、[プッシュ通知の設定]({{site.baseurl}}/developer_guide/push_notifications#android_setting-up-push-notifications)を参照せよ。
...
![二組のキーと値のペア：IS_SERVER_EVENT一つは「true」に設定され、もう一つは「例：キャンペーンCAMPAIGN_NAME名」に設定される。]({% image_buster /assets/img_archive/kvpConfiguration.png %}){: style="max-width:70%;" }
...
![アクションベースの配信キャンペーンで、アプリ内メッセージがトリガーされる条件は、キャンペーン名が「IAMキャンペーン名例」と"campaign_name"等しい場合である。]({% image_buster /assets/img_archive/iam_event_trigger.png %})
...
![カスタムイベント/assets/img_archive/iosServerSentPush.pngimage_buster"server_event".]({%    %}) をユーザープロファイルに持つユーザーに配信される、アクションベースのアプリ内メッセージキャンペーンだ。

プッシュキャンペーンにはキーと値のペアエクストラを含める必要があります。これは、このプッシュキャンペーンが SDK カスタムイベントを記録するために送信されることを示します。このイベントは次のアプリ内メッセージをトリガーするために使用されます。

![アクションベースの配信アプリ内メッセージキャンペーンには、2つのキーと値のペアがある。「アプリ内"CAMPAIGN_NAME"メッセージ名例」として設定され、「true」に設定"IS_SERVER_EVENT"される。]({% image_buster /assets/img_archive/iOSServerPush.png %})
...
![カスタムイベント「アプリ内メッセージトリガー」を実行したユーザーに対して配信される、アクションベースのアプリ内メッセージキャンペーンである。ここで、"campaign_name"は「IAMキャンペーン名例」に等しい。]({% image_buster /assets/img_archive/iosIAMeventTrigger.png %})
...
{% tabs %}
{% tab web %}
Web SDKでは、アプリ内メッセージを表示`braze.showInAppMessage(inAppMessage)`するには``を使用する。詳細と例については、[リアルタイムでのメッセージング表示を](#displaying-a-message-in-real-time)参照のこと。
{% endtab %}
