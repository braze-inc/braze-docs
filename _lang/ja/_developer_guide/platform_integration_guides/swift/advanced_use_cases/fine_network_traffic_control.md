...
- Brazeは、アプリ内メッセージのリクエスト、アプリ内メッセージでのLiquidテンプレート、ジオフェンス、位置情報の追跡など、内部機能のために自動ネットワークリクエストを引き続き実行します。詳細については、`Braze.Configuration.Api.RequestPolicy.manual` の[ドキュメント][2]を参照してください。これらの内部リクエストが実行されると、リクエストのタイプによっては、ローカルに保存されたカスタム属性とカスタムイベントデータが Braze サーバーにフラッシュされる場合があります。
