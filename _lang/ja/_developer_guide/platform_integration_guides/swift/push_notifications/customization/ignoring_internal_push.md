...
platform: Swift
page_order: 6
description: "ここでは、スウィフトSDKのBraze 内部 プッシュ通知s を無視する方法について説明します。"
channel:
  - push
...
> Braze は、特定の高度な機能の内部インプリメンテーションに[サイレントプッシュ通知s][2] を使用します。ほとんどの統合では、アプリに代わって変更を行う必要はありません。ただし、Braze機能を内部 プッシュ通知に依存する機能(アンインストール追跡 やジオフェンス s など)に統合する場合は、アプリを更新して、Brazeからの内部プッシュを無視することができます。

アプリがアプリのライケーションの起動時またはバックグラウンドのプッシュ時に自動的にアクションを実行する場合は、アクティビティがトリガー当社の内部 プッシュ通知sによって制御されないようにゲートを設定することを検討してください。たとえば、すべてのバックグラウンドプッシュまたはアプリケーションの起動時に新しいコンテンツのためにサーバーを呼び出すロジックがある場合、不要なネットワークトラフィックが発生するため、Braze の内部プッシュトリガーを使用しないことがあります。さらに、Braze は特定の種類の内部プッシュをすべてのユーザーにほぼ同時に送信するため、起動時に内部プッシュからのネットワーク呼び出しをゲートしないと、サーバーに重大な負荷がかかる可能性があります。

## アプリの自動アクションを確認する

次の箇所で自動的にアクション s が使用されているかどうかをアプリライケーションで確認し、コードを更新してBrazeの内部プッシュを無視します。
...
スタティックユーティリティメソッドを`Braze.Notifications` で使用して、アプリがBraze 内部プッシュによって受信または起動されたかどうかを確認できます。[`Braze.Notifications.isInternalNotification(_:)`][1] は、アンインストール追跡、フィーチャーフラグ s s s sync、およびジオフェンス s s s s sync 通知s を含むすべてのBraze 内部 プッシュ通知s で`true` を返します。
