...
Firebase メッセージングサービスがすでに登録されている場合、\[`RemoteMessage`][75] オブジェクトを Braze に \[`BrazeFirebaseMessagingService.handleBrazeRemoteMessage()`][74] 経由で渡すことができます。このメソッドは \[`RemoteMessage`][75] オブジェクトが Braze から発信された場合にのみ通知を表示し、そうでない場合は無視します。
...
Braze Android SDK は \[Android 通知チャネル]][62] をサポートしています。Braze の通知に通知チャネルの ID が含まれていない場合、または Braze の通知に無効なチャネル ID が含まれている場合、Braze は SDK で定義されているデフォルトの通知チャネルで通知を表示します。Brazeユーザーはプラットフォーム内で\[Android通知チャンネル][61]を使用して通知をグループ化します。
...
\[Android プッシュオブジェクト][63]パラメーターを持つ API キャンペーンを更新して、`notification_channel` フィールドが含まれるようにします。このフィールドが指定されていない場合、Braze は \[ダッシュボードフォールバック][64]チャネル ID を使用して通知ペイロードを送信します。
...
Braze は Braze プッシュ通知からのデータを含む \[`BrazeNotificationPayload`][77] を渡します。
