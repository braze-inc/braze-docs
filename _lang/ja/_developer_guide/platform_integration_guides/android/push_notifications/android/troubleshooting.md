...
1. \[Firebase Cloud Messaging][21]] からBraze に有効なプッシュトークンを渡してください。
...
\[カスタム通知ファクトリ][16]を実装した場合は、`null`を返さないようにしてください。null が返されると、通知が表示されなくなります。
...
![「&quot」、「Settings&quot」、「&quot」、「Cloud Messaging&quot」のFirebase プラットフォームには、サーバーID とサーバーキーが表示されます。][20]
...
ディープリンク\[ADB][17]で正しく動作するが、Brazeプッシュで動作しない場合は、\[カスタムプッシュ開封処理][18]が実装されているかどうかを確認します。実装されている場合は、カスタム処理コードが受信ディープリンクを適切に処理していることを確認してください。

#### バックスタック動作を無効にする

ディープリンク\[ADB][17] で正しく動作するが、Brazeプッシュで動作しない場合は、\[back stack][22]] を無効にしてみてください。そのためには、**braze.xml** を更新して以下を含めます。
