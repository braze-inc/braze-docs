...
## セッショントラッキングのデバッグ
セッション・トラッキングが予期せぬ振る舞いをする場合、\[verbose logging][56] ] をオンにし、セッション・トリガーの手順を再現しながらアプリを観察する。llogcat 内の Braze ステートメントを観察して、アクティビティ内の `openSession` および `closeSession` 呼び出しのログ記録を見逃している可能性がある場所を検出します。
