...
| バージョン | タイプ | 主な変更点 |
|---------|------|-------------|
| **6.0.0** | **メジャー** | `<img>` タグ s、`imageAltText`、または`language` フィールド s、全般的なUI アクセシビリティの向上 |
| **3.5.0** | 軽微 | スクロール可能なテキストアクセシビリティの向上 |
| **3.4.0** | 修正する | コンテンツカード`article` ロールの修正 |
| **3.2.0** | 軽微 | ボタン用の45x45px 最小タッチターゲット |
| **3.1.2** | 軽微 | "画像s のデフォルトアルトテキスト |
| **2.4.1** | **メジャー** | セマンティックHTML(`h1`または`button`)、ARIA 属性 s、キーボードナビゲーション、フォーカスマネジメント |
