...
Braze は、Snowflake、Kafka、MongoDB、および Redis を活用したストリーミングデータアーキテクチャ上に構築されています。SDKやAPIを通じて、多くのデータソースからBrazeにデータを読み込むことができる。プラットフォームは、データがどのようにネストまたは構造化されているかに関係なく、あらゆるデータをリアルタイムで処理できます。データはBrazeにユーザープロファイルに保存されます。 
...
Brazeは様々な機能に対して異なるデータストレージシステムを使用している。どの機能がどのデータソースを使用しているかを理解することは、データ管理とトラブルシューティングのために重要である。

#### MongoDB搭載の機能
- カスタムイベント（SDKおよびAPIでトラッキング、追跡される）
- カスタム属性
- ユーザープロファイル
- 購入イベント
...
- [予測スイート]({{site.baseurl}}/user_guide/brazeai/predictive_suite/)
- [パーソナライズされたパスと]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/experiment_step/personalized_paths/) [パーソナライズされたバリアント]({{site.baseurl}}/user_guide/engagement_tools/testing/multivariant_testing/optimizations/#personalized-variant)
- [AIがパーソナライズされたアイテムを推薦する]({{site.baseurl}}/user_guide/brazeai/recommendations/creating_recommendations/ai/)
- [推定実質開封率]({{site.baseurl}}/user_guide/message_building_by_channel/email/reporting_and_analytics/email_reporting#estimated-real-open-rate)（カスタムイベントは使用しない）

{% alert important %}
**データ削除の考慮事項：**カスタムイベントはMongoDBに保存され、Snowflakeデータとは別になる。誤ったカスタムイベントデータを削除する必要がある場合は、MongoDBで対処する必要がある。Snowflakeを利用した機能（SQLセグメントエクステンションやその他のSnowflakeを利用した機能など）は、Snowflakeのデータを利用するが、これは別に扱われる。一方のシステムからデータを削除しても、もう一方のシステムから自動的にデータが削除されるわけではない。
{% endalert %}

### Braze APIによるバックエンドデータソース
Braze は、[REST API]({{site.baseurl}}/api/endpoints/user_data) を使用して、ユーザーデータベース、オフライントランザクション、およびデータウェアハウスからデータを取得できます。 

### Braze SDKによるフロントエンドデータソース
...
### Braze Cloud Data Ingestionによるデータウェアハウスへの直接接続
...
パーソナライゼーション層は、メッセージ内でダイナミックなコンテンツを提供する機能を表します。広く使用されているパーソナライゼーション言語である Liquid を使用すると、チームは既存のデータを動的に取得して、各受信者に合わせたメッセージを表示できます。さらに、[コネクテッドコンテンツを]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content)使えば、プッシュ通知やメールなどの送信メッセージに、ウェブサーバーやAPIからアクセス可能な情報を直接挿入することができる。コネクテッドコンテンツは Liquid の上に構築され、おなじみの構文を使用します。
