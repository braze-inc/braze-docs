...
{% alert note %}
セキュリティのレイヤーを追加するには、[SDK認証を]({{site.baseurl}}/developer_guide/sdk_integration/authentication/)有効にして、不正なSDKリクエストを防ぐことができる。この機能は、Web、iOS、Android、React Native、Flutter、Unity、Cordova、.NET MAUI（Xamarin）、Expoなど、すべての主要プラットフォームで利用できる。
{% endalert %}
