...
{% alert note %}
セキュリティを強化するため、[SDK認証を]({{site.baseurl}}/developer_guide/sdk_integration/authentication/)イネーブルメントすれば不正なSDKリクエストを防ぐことができる。この機能は、Web、iOS、Android、React Native、Flutter、Unity、Cordova、.NET MAUI (Xamarin)、Expo を含むすべての主要プラットフォームで利用できる。
{% endalert %}
...
### 月間アクティブユーザー数（CY 24-25）、ユニバーサルMAU、Web MAU、モバイルMAU  

2024-2025年度（CY）の月間アクティブユーザー（MAU）、ユニバーサルMAU、Web MAU、モバイルMAUを購入した顧客に対して、Brazeはセッション、ユーザー属性、イベント、その他のユーザープロファイルデータを更新するために当社のSDKが使用するAPIリクエストに対し、サーバーサイドのレート制限を適用する。これはプラットフォームの安定性を確保し、高速で信頼性の高いサービスを維持するためである。 
...
* 定常レート制限は、バーストウィンドウよりも長いローリングウィンドウ（例えば数分間）における持続的なリクエスト量をコントロールし、バースト制限と時間単位のレート制限の間で継続的なトラフィックを平滑化するのに役立つ。
...
より高いBrazeのレート制限が必要な場合は、Brazeサポートまたは顧客サクセスマネージャーに連絡し、以下の詳細を記載すること：

* 一時的な増加か恒久的な増加か、どちらが必要か。
* なぜ増額が必要なのか。
* どのエンドポイントと環境が影響を受けるのか。
* おおよその交通量とスケジュール。開始日、期間、ピーク時間帯を含む。
* 通話をまとめて処理できるか、それともトラフィックを時間をかけて分散できるか。

リクエストを送信した後、Brazeがそれを確認し、結果を更新する。
