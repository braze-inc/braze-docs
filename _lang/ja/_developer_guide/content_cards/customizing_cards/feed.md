...
{% alert important %}
Braze SDKは、パフォーマンスと信頼性のためにレート制限も適用する。自動テストを実行する時や手動QAを行う時は、この点を心に留めておくことだ。詳細については[、Braze SDK のレート制限を]({{site.baseurl}}/developer_guide/sdk_integration/rate_limits/)参照せよ。
{% endalert %}
