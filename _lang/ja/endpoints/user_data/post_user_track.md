...
{% alert note %}
Braze は、API を通過したデータを額面で処理します。顧客 s は、不必要なデータポイントログを最小限に抑えるために、差分(データの変更) のみを渡す必要があります。続きを読むには、[データポイント]({{site.baseurl}}/user_guide/data/data_points/)を参照してください。
{% endalert %}
...
リクエストが致命的なエラーに遭遇した場合に返されるステータス コードsおよび関連するエラーメッセージについては、[致命的なエラーs &レスポンス]({{site.baseurl}}/api/errors/#fatal-errors)を参照してください。

エラー &quot を受け取った場合、external_id はブラックリストに登録され、許可されていない" で、リクエストに"ダミーユーザー." が含まれている可能性があります。詳細については、「[スパムのブロック]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_archival/#spam-blocking)」を参照してください。 
...
### 月間稼働ユーザーの料金制限ヘッダー(24-25年)

rate-limited 以外のすべてのレスポンス(`429` など) には、クライアントへの時間単位のレート制限ウィンドウの状態を示す以下のHTTP レスポンスヘッダーs が含まれます。以下のヘッダーを使用して、リクエストレートを管理することをお勧めします。

| ヘッダ名             | 説明                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------- |
| `X-RateLimit-Limit`     | 期間ごとに許可されるリクエストの数                                              |
| `X-RateLimit-Remaining` | ウィンドウ内に残っているリクエストのアプリ最大数                                |
| `X-RateLimit-Reset`     | 現在のウィンドウがリセットされるまでの残り秒数                                    |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

HTTP `RateLimit-Limit`、`RateLimit-Remaining`、および`RateLimit-Reset` ヘッダーs は、HTTP `429` エラーを押しても返されないことに注意してください。エラーが発生すると、これらのヘッダーs は`X-Ratelimit-Retry-After` ヘッダーに置き換えられ、秒数を示す整数が返されてからリクエストを実行できます。
