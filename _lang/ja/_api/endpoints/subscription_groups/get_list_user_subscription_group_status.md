...
{% alert note %}
ユーザーがグローバルに配信停止すると、各サブスクリプショングループから配信停止される。このエンドポイントは、各サブスクリプショングループの最後のサブスクリプションステータスを返す。ユーザーがグローバルに再サブスクリプションを決定した場合、Brazeは各サブスクリプションのステータスを戻すので、これは予想された動作である。
{% endalert %}
