---
nav_title: "POST:Eメールテンプレートを更新する"
...
## 前提条件
このエンドポイントを使用するには、`templates.email.update` 権限を持つ [API キー]({{site.baseurl}}/api/api_key/)が必要です。
...
| --------- | ---------| --------- | ----------- |
|`email_template_id`| 必須 |string|[メールテンプレートのAPI識別子]({{site.baseurl}}/api/identifier_types/)|
|`template_name`|オプション|string|メールテンプレートの名前|
|`subject`|オプション|string|メールテンプレートの件名|
|`body`|オプション|string|HTMLを含む可能性のあるメールテンプレート本文。|
|`plaintext_body`|オプション|string|メールテンプレート本文のプレーンテキストバージョン。|
|`preheader`|オプション|string|一部のクライアントでプレビューを生成するために使用されるメールプレヘッダー。|
|`tags`|オプション|string|[タグ]({{site.baseurl}}/user_guide/administrative/app_settings/manage_app_group/tags/)はすでに存している必要があります。|
