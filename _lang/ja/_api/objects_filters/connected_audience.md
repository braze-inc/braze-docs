...
> 接続されたオーディエンスオブジェクトは、メッセージのオーディエンスを識別します。たとえば、カスタム属性アレイを使用して、ユーザーがお気に入りにしたすべてのカテゴリとテレビ番組を追跡する場合、接続されたオーディエンスsを使用して、新しいエピソードがリリースされるたびに、番組がお気に入りになった人にプッシュ通知またはメールを自動的に送信できます。そのため、それぞれの番組にセグメントを設定する必要はありません。
...
複数のカスタム属性 フィルターs を`AND` および`OR` 演算子と組み合わせると、接続オーディエンス フィルターが作成されます。
...
このフィルターでは、ユーザーが最後にアプリを使用した時間に基づいてセグメントできます。これらのフィルターには2つのフィールドがある：
...
接続オーディエンスは、デフォルト属性、カスタムイベント、セグメント、またはメッセージエンゲージメントイベントによってユーザーをフィルターすることはできません。これらのフィルターを使用するには、それらをオーディエンス セグメントに組み込み、[`/messages/send`エンドポイント]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages#request-parameters)の`segment_id`パラメータでそのセグメントを指定することをお勧めします。他のエンドポイントを使用する場合は、最初にBraze ダッシュボードのAPI-トリガーed キャンペーンまたはキャンバスにセグメントを追加する必要があります。
