...
!!step
lines-MainApplication.kt=9-21

#### 2\.バナーの更新をサブスクライバーする

`subscribeToBannersUpdates()` 、バナーが更新されるたびに実行されるハンドラを登録する。

!!step
lines-MainActivity.kt=9-21

#### 3\.プレースメントをリフレッシュする

Braze SDKを初期化した後、`requestBannersRefresh(["PLACEMENT_ID"])` を呼び出し、その配置の最新のBannerコンテンツを取得する。

!!step
lines-banners.xml=9-21

#### 4\.`BannerView` を定義する。 `banners.xml`

`banners.xml` で、`<com.braze.ui.banners.BannerView>` 要素を`app:placementId="PLACEMENT_ID"` と宣言する。Brazeはこのエレメントを使って、あなたのBannerをUIに挿入する。
