...
{% alert note %}
iOS プロジェクトに SDWebImage の依存関係が必要かどうかを確認するには、[iOS アプリ内メッセージドキュメント[](https://github.com/SDWebImage/SDWebImage)を参照してください。(]({{ site.baseurl }}/developer_guide/platform_integration_guides/swift/in-app_messaging/overview/)]
{% endalert %}
{% endtab %}
{% endtabs %}
...
4. APIキーを再度設定する。完全なウォークスルーは、[ステップ3.1を参照のこと：API キーの設定
