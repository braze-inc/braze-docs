...
まず、[アプリ用にADM API Keyを生成](https://developer.amazon.com/public/apis/engage/device-messaging/tech-docs/02-obtaining-adm-credentials)し、そのキーを`api_key.txt` という名前のファイルに保存して、プロジェクトの [`Assets/`](https://docs.unity3d.com/Manual/AndroidAARPlugins.html)ディレクトリに追加する。
...
{% tab Amazon Device Messaging %}
[前のステップで](#unity_step-21-update-androidmanifestxml) `AndroidManifest.xml` を更新し、以下の行を追加すると、プッシュ・リスナーが自動的に設定された。だから、これ以上の設定は必要ない。
