...
glossary_top_text: "Braze には、ユーザーs の追跡、メッセージの送信、データのエクスポートなどを可能にする高パフォーマンス RESTなAPI が用意されています。このページでは、利用可能な Braze API エンドポイントとその用途を示します。"
page_type: glossary
...
glossaries:
  ...
  - name: "<a href='/docs/api/endpoints/messaging/schedule_messages/post_update_scheduled_messages/'>/messages/schedule/update</a>"
    description: スケジュールされたメッセージを更新する。このエンドポイントは、<code>schedule</code> か <code>messages</code> パラメータのどちらか、または両方の更新を受け付けます。
    tags:
      - Schedule Messages
