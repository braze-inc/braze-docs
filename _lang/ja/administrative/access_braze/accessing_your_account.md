...
6. それでも問題が解決しない場合は、アカウントの再作成を検討してほしい。無料トライアル期間中にBraze Learningにアクセスしたユーザーは、現在アクセスしづらい可能性がある。

### 2要素認証（2FA）の問題

ユーザーが2要素認証（2FA）に問題があり、Brazeダッシュボードにアクセスできない場合、いくつかの原因が考えられる。最も一般的なのは、登録した電話番号やAuthyアプリがインストールされているデバイスにアクセスできなくなることだ。
