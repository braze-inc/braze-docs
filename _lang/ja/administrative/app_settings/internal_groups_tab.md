...
![「Email test group」という名前の内部グループ。]({% image_buster /assets/img_archive/internal_group.png %})()
...
![コンテンツテストグループを選択するためのアプリ内メッセージエディターの [テスト] セクション。]({% image_buster /assets/img_archive/content_test_preview.png %})({: style="max-width:60%" })
...
シード送信は、内部の品質保証と確認のために設計されているため、意図的にシードダッシュボード ユーザーのサブスクリプション検査を省略します。つまり、シードグループの一部である有効なメールアドレスを持つユーザーsは、サブスクライブされていない場合でもメッセージを受信します。ただし、そのグループにシードコピーを送信するようにメッセージを設定する必要があります。
...
![バリアント1のメールキャンペーンの送信先として「Email seed test」というシードグループが選択されている。]({% image_buster /assets/img_archive/seed_group_campaign.png %})()
