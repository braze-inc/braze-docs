...
{% alert important %}
**Format de sortie d'exportation** : Lorsqu'une exportation est réussie et que vous n'avez pas configuré d'identifiants de stockage cloud, la réponse HTTP inclut une URL permettant de télécharger une archive compressée (fichier ZIP ou GZIP). Lorsque les identifiants de stockage cloud (S3, Azure ou Google cloud storage) sont configurés, Braze enregistre l'exportation directement dans votre compartiment, et la réponse ne comprend pas d'URL de téléchargement. Si l'exportation échoue, vous recevrez une notification par e-mail. La configuration des informations d'identification pour le stockage dans le cloud réduit le risque d'échecs lors d'exportations volumineuses.
{% endalert %}
...
| `phone`               | Chaîne de caractères          | Numéro de téléphone de l'utilisateur dans le format importé dans Braze. Par exemple, si une demande d'ajout d'un numéro de téléphone est reçue sous la forme `1234567890`, elle sera exportée dans le même format.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- Les champs ,`custom_events` `purchases`,`campaigns_received` et ne`canvases_received` contiennent que les données des 90 derniers jours.
- `custom_events` et `purchases` contiennent tous les deux des champs pour `first` et `count`. Ces deux champs reflètent les informations de toutes les périodes et ne se limitent pas aux données des 90 derniers jours. Par exemple, si un utilisateur particulier a participé à l'événement pour la première fois il y a 90 jours, cela est reflété avec précision dans le`first`champ, et le`count`champ prend également en compte les événements qui ont eu lieu avant les 90 derniers jours.
- Le nombre d’exportations de segments simultanées qu’une entreprise peut exécuter au niveau de l’endpoint est plafonné à 100. Toute tentative dépassant cette limite entraînera une erreur.
...
Si vous observez`object_prefix`  dans votre réponse API et qu'aucune URL n'est fournie pour télécharger les données, cela signifie qu'un compartiment Amazon S3 est déjà configuré pour cet endpoint. Toutes les données exportées à l'aide de cet endpoint sont directement transférées vers votre compartiment S3.
