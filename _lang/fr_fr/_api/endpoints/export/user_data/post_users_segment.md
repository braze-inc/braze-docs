...
{% alert important %}
Lors de l'utilisation de cet endpoint, notez les points suivants :<br><br>1. Le champ `fields_to_export` de cette requête API est **obligatoire**.<br>2. Les champs `custom_events`, `purchases`, `campaigns_received` et `canvases_received` contiennent uniquement les données des 90 derniers jours.
{% endalert %}
...
{% alert important %}
**Format de sortie de l'exportation** : Lorsqu'une exportation réussit et que vous n'avez pas configuré d'identifiants de stockage cloud, la réponse HTTP inclut une URL permettant de télécharger une archive compressée (fichier ZIP ou GZIP). Lorsque des identifiants de stockage cloud (S3, Azure ou Google Cloud Storage) sont configurés, Braze enregistre l'exportation directement dans votre compartiment, et la réponse ne comprend pas d'URL de téléchargement. Si l'exportation échoue, vous recevez une notification par e-mail. La configuration d'identifiants de stockage cloud réduit le risque d'échec lors d'exportations volumineuses.
{% endalert %}
...
| `created_from`        | Chaîne de caractères          | Méthode utilisée pour créer le profil utilisateur (par exemple, SDK, API REST ou importation CSV).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `devices`             | Tableau           | Informations sur l'appareil de l'utilisateur, pouvant inclure les éléments suivants selon la plateforme :<br><br>- `model` : nom du modèle de l'appareil<br>- `os` : système d'exploitation de l'appareil<br>- `carrier` : fournisseur de services de l'appareil, si disponible<br>- `idfv` : (iOS) identifiant de l'appareil Braze, l'identifiant Apple pour le vendeur (IDFV), s'il existe<br>- `idfa` : (iOS) identifiant publicitaire (IDFA), s'il existe<br>- `device_id` : (Android) identifiant de l'appareil Braze<br>- `google_ad_id` : (Android) identifiant publicitaire Google Play, s'il existe<br>- `roku_ad_id` : (Roku) identifiant publicitaire Roku<br>- `ad_tracking_enabled` : indique si le suivi publicitaire est activé sur l'appareil, peut être true ou false |
...
| `phone`               | Chaîne de caractères          | Numéro de téléphone de l'utilisateur dans le format importé dans Braze. Par exemple, si un numéro de téléphone est ajouté sous la forme `1234567890`, il sera exporté dans le même format.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `total_revenue`       | Float           | Chiffre d'affaires total attribué à cet utilisateur. Le chiffre d'affaires total est calculé à partir des achats réalisés par l'utilisateur pendant les fenêtres de conversion des campagnes et Canvas qu'il a reçus.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
