...
Pour plus d'informations, consultez les articles suivants en fonction de votre plateforme : [iOS][9], [Android][10] et [Web.][13]
...
![Panneau des clés API REST sur la page Clés API.][27]
...
Les autorisations de clés API sont des autorisations que vous pouvez affecter à un utilisateur ou un groupe pour limiter leur accès à certains appels API. Pour afficher la liste des autorisations de votre clé API, allez dans **Paramètres** > **Clés API**, et sélectionnez votre clé API.
...
| Autorisation | Endpoint | Description |
|---|---|---|
| `feed.list` | [`/feed/list`]({{site.baseurl}}/api/endpoints/export/news_feed/get_news_feed_cards/) | Extraire la liste des cartes de fil d’actualité. |
| `feed.data_series` | [`/feed/data_series`]({{site.baseurl}}/api/endpoints/export/news_feed/get_news_feed_card_analytics/) | Extraire les analyses du fil d’actualité sur une période donnée. |
| `feed.details` | [`/feed/details`]({{site.baseurl}}/api/endpoints/export/news_feed/get_news_feed_card_details/) | Extraire les données d'un fil d’actualité spécifique. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3}
...
Les clés API REST ne peuvent pas être modifiées après leur création. Toutefois, vous pouvez afficher les détails des clés API REST existantes ou les supprimer à partir de la page **Clés API.**  La liste **Clés API Rest** affiche les informations suivantes en un coup d’œil pour chaque clé :
...
Pour afficher les détails d’une clé spécifique, sélectionnez celle-ci dans la liste. Vous pouvez alors voir toutes les autorisations dont dispose cette clé, les adresses IP inscrites sur la liste blanche (le cas échéant) et si cette clé est inscrite sur la liste blanche des adresses IP de Braze.

![][30]

Notez que lors de la [suppression d'un utilisateur]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/adding_users_to_your_dashboard/), les clés API associées qu'il a créées ne seront pas supprimées. Pour supprimer une clé, cliquez sur <i class="fas fa-gear" alt="Settings"></i> l’option correspondante et sélectionnez-la.
