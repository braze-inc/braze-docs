...
Reportez-vous à ce qui suit pour connaître les limites de débit par défaut de l'API pour différents types de demandes. Ces limites par défaut peuvent être augmentées sur demande. Contactez votre gestionnaire satisfaction client pour plus d'informations.

### Demandes avec différentes limites de débit

| Type de demande                                                                                                                                                                                                                                           | Limite de débit par défaut de l’API                                                                                                                                                                                                                                                                                                                                                                    |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)                                                                                                                                                                                                                                   | **Demandes :** 3 000 demandes par trois secondes.<br><br>**Traitement par lot :** 75 événements, 75 achats et 75 attributs par requête d'API. Pour en savoir plus, consultez la rubrique [Mise en lot des demandes de suivi des utilisateurs](#batch-user-track).<br><br>**Limites pour les utilisateurs actifs par mois CY 24-25, MAU universels, MAU Web et MAU mobiles :** veuillez consulter [les directives relatives aux limites ici]({{site.baseurl}}/api/endpoints/user_data/post_user_track/#monthly-active-users-cy-24-25). |
...
| [`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/)<br>[`/campaigns/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns/)<br>[`/canvas/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)                                                                                                                                                          | Pour les appels diffusés (lorsque le ciblage porte sur des segments, des filtres ou une audience connectée), 250 requêtes par minute pour toutes les audiences **et** 10 requêtes par minute par [audience unique]({{site.baseurl}}/api/api_limits/#what-counts-as-the-same-unique-audience) (la limite atteinte en premier étant prise en compte).<br><br>Dans le cas contraire, lors du ciblage de destinataires individuels, la requête est incluse dans la [limite de débit]({{site.baseurl}}/api/api_limits/#requests-with-shared-rate-limits) de 250 000 requêtes par heure [partagée]({{site.baseurl}}/api/api_limits/#requests-with-shared-rate-limits).                                                                                                                                                                                                                    |
...
Cela s'applique aux trois endpoints suivants : [`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/), et[`/campaigns/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns/) [`/canvas/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/).

Pour ces endpoints, les demandes de diffusion sont considérées comme ayant un ciblage sur la même audience unique lorsque tous les éléments suivants correspondent :

- La campagne ou le canvas déclenché (le`campaign_id`ou`canvas_id`dans votre requête API, si spécifié)
- Le public ciblé (les segments ou filtres, ou pour les campagnes API, les`segment_id`paramètres de votre requête API)
- Les filtres d'audience connectés (`audience`l'objet dans votre requête API, s'il est spécifié)

Chaque combinaison unique de ces attributs est considérée comme une audience distincte. Par conséquent, la limite de débit supplémentaire pour chaque audience unique s'applique indépendamment à chaque combinaison.
...
Dans des conditions normales, le délai nécessaire pour que nos données atteignent leur cohérence finale est de 10 à 100 ms (1/10 de seconde). Toutefois, dans certains cas, il faut plus de temps pour que cette cohérence se produise. Nous vous recommandons donc de prévoir un délai de 5 minutes entre les appels suivants afin de minimiser la probabilité d'une erreur.
