...
excerpt_separator: ""
page_type: glossary
description: "Ce glossaire répertorie les différents événements d'engagement aux messages que Braze peut suivre et envoyer aux entrepôts de données choisis à l'aide de Currents."
tool: Currents
search_rank: 6
---
...
Contactez votre gestionnaire de compte ou ouvrez un [ticket d'assistance]({{site.baseurl}}/braze_support/) si vous avez besoin d'accéder à des droits d'événements supplémentaires. Si vous ne trouvez pas ce dont vous avez besoin dans cet article, consultez notre [bibliothèque d'événements du comportement des clients]({{site.baseurl}}/user_guide/data/braze_currents/event_glossary/customer_behavior_events/) ou nos [exemples de données comportement](https://github.com/Appboy/currents-examples/tree/master/sample-data) [ales]({{site.baseurl}}/user_guide/data/braze_currents/event_glossary/customer_behavior_events/) [Currents](https://github.com/Appboy/currents-examples/tree/master/sample-data).

{% details Explanation of message engagement event structure and platform values %}

### Structure de l'événement

Cette répartition des événements montre quel type d'information est généralement inclus dans un événement d'engagement lié aux messages. Avec une solide compréhension de ses composants, vos développeurs et votre équipe de stratégie d'aide à la décision peuvent utiliser les données d'événement entrantes de Teams pour réaliser des rapports et des graphiques axés sur les données, et tirer parti d'autres indicateurs de données précieux.

Ventilation d'un événement d'engagement aux messages montrant un événement de désinscription par e-mail avec les propriétés énumérées regroupées par propriétés spécifiques à l'utilisateur, propriétés de suivi de la campagne ou du canvas, et propriétés spécifiques à l'événement.]({% image_buster /assets/img/message_engagement_event.png %})
...
### Schéma de l'ID de l'utilisateur

Notez les conventions d'appellation pour les ID d'utilisateurs.

| Schéma de Braze | Schéma actuel | Description |
| ----------- | ----------- | ----------- |
| `braze_id` | `"USER_ID"` | L'identifiant unique qui est automatiquement attribué par Braze. |
| `external_id` | `"EXTERNAL_USER_ID"` | L'identifiant unique du profil d'un utilisateur qui est défini par le client. |
...
Cet événement se produit lorsqu'un utilisateur désinstalle une application. Utilisez ces données pour savoir quand les utilisateurs désinstallent une appli. Bien qu'il s'agisse actuellement d'un événement d'engagement lié aux messages, il s'agira à l'avenir d'un événement lié au comportement de l'utilisateur.

{% alert important %}
Cet événement n'est pas déclenché lorsque l'utilisateur désinstalle réellement l'application, car il est impossible de le suivre exactement. Braze envoie un push silencieux quotidien pour déterminer si l'application existe toujours sur l'appareil de votre utilisateur, et si nous obtenons une erreur lors de ce push silencieux, nous supposons que l'application a été désinstallée.
{% endalert %}
...
| Étape de mise à jour de l'utilisateur de Canvas | Lorsqu'un utilisateur est mis à jour par l'étape du canvas de mise à jour de l'utilisateur |
| Enregistrement du jeton de poussée | Lorsqu'un utilisateur est mis à jour par le processus d'enregistrement des jetons |
| List-Unsubscribe | Lorsqu'un utilisateur se désabonne via un mailto de Braze ou une liste en un clic, l'en-tête "unsubscribe" est affiché. |
...
Cet événement se produit lorsque l'état de l'abonnement d'un utilisateur dans un groupe d'abonnement change.
...
Cet événement se produit lorsqu'un utilisateur effectue une action qui a été définie comme un événement de conversion dans une campagne.
...
{% alert important %}
Notez que l'événement de conversion est encodé dans le champ `conversion_behavior`, qui comprend le type d'événement de conversion, la fenêtre (délai) et des informations supplémentaires en fonction du type d'événement de conversion. Le champ `conversion_behavior_index` représente l'événement de conversion, par exemple 0 = A, 1 = B, 2 = C, 3 = D.
{% endalert %}
...
Cet événement se produit lorsqu'un utilisateur est inscrit à un ensemble de variantes de contrôle dans le cadre d'une campagne à variantes multiples. Cet événement est généré car il n'y aura pas d'événement d'envoi de canal pour cet utilisateur.
...
Cet événement se produit lorsqu'un utilisateur effectue une action qui a été définie comme un événement de conversion dans Canvas.
...
Cet événement se produit lorsqu'un utilisateur a quitté un canvas en correspondant à une audience.
...
Cet événement se produit lorsqu'un utilisateur a quitté un canvas en effectuant un événement.
...
Cet événement se produit lorsqu'un utilisateur entre dans un chemin d'étape des chemins d'expérience Canvas.
...
- Pour `ad_id`, `ad_id_type`, et `ad_tracking_enabled`, vous devez collecter explicitement l'IDFA pour iOS et l'ID publicitaire Google pour Android via les SDK natifs. En savoir plus sur cette configuration pour [iOS]({{site.baseurl}}/developer_guide/analytics/managing_data_collection/?sdktab=swift) et [Android]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=android#android_google-advertising-id).
...
{% apitags %}
Cartes de contenu, fermeture de contenu
{% endapitags %}

Cet événement se produit lorsqu'un utilisateur ferme une carte de contenu.
...
Cet événement se produit lorsqu'un utilisateur consulte une carte de contenu.
...
- `message_extras` vous permettent d'annoter vos événements d'envoi avec des données dynamiques issues du contenu connecté, des attributs personnalisés (tels que la langue ou le pays) et des propriétés d'entrée dans Canvas. Pour en savoir plus, reportez-vous aux [suppléments de messages.]({{site.baseurl}}/message_extras_tag/) 
{% endapi %}
...
Cet événement se produit lorsqu'un fournisseur de services Internet renvoie un échec d'envoi définitif. Un échec d'envoi définitif signifie un échec de livrabilité définitif.
...
- Le comportement de `dispatch_id` diffère entre Canvas et les campagnes car Braze traite les étapes de Canvas (à l'exception des étapes d'entrée, qui peuvent être planifiées) comme des événements déclenchés, même lorsqu'elles sont planifiées. En savoir plus sur le [comportement de l'ID de répartition]({{site.baseurl}}/help/help_articles/data/dispatch_id/).
{% endapi %}
...
Cet événement se produit lorsqu'un utilisateur clique sur un e-mail. Plusieurs événements peuvent être générés pour la même campagne si un utilisateur clique plusieurs fois ou clique sur différents liens dans l'e-mail.
...
{% apitags %}
E-mail, réception/distribution
{% endapitags %}

Cet événement se produit lorsqu'un e-mail envoyé est parvenu jusqu'à la boîte de réception de l'utilisateur final.
...
Cet événement se produit lorsque l'utilisateur final appuie sur le bouton "spam" de l'e-mail. Notez que cela ne conseille pas d'envoyer l'e-mail dans le dossier spam, car Braze n'en tient pas compte.
...
Cet événement se produit lorsqu'une demande d'envoi d'e-mail a été communiquée avec succès entre Braze et Sendgrid. Cela ne signifie toutefois pas que l'e-mail a été reçu dans la boîte de réception de l'utilisateur.
...
- Le comportement de `dispatch_id` diffère entre Canvas et les campagnes car Braze traite les étapes de Canvas (à l'exception des étapes d'entrée, qui peuvent être planifiées) comme des événements déclenchés, même lorsqu'elles sont planifiées. En savoir plus sur le [comportement de l'ID de répartition]({{site.baseurl}}/help/help_articles/data/dispatch_id/).
- `message_extras` vous permettent d'annoter vos événements d'envoi avec des données dynamiques issues du contenu connecté, des attributs personnalisés (tels que la langue, le pays) et des propriétés d'entrée dans le Canvas. Pour en savoir plus, reportez-vous aux [suppléments de messages.]({{site.baseurl}}/message_extras_tag/) 
{% endapi %}

{% api %}
## Événements de l'échec provisoire d'envoi par e-mail {#email-soft-bounce-events}

{% apitags %}
E-mail, rebond
{% endapitags %}

Cet événement se produit lorsqu'un fournisseur de services Internet renvoie un échec provisoire d'envoi. Un échec provisoire d'envoi signifie qu'un e-mail n'a pas pu être livré en raison d'un problème temporaire de livrabilité.
...
Cet événement se produit lorsque l'utilisateur final a cliqué sur "se désabonner" de l'e-mail.
...
Cet événement se produit lorsqu'un utilisateur consulte un message in-app.
...
{% api %}
## Abandon d'événements {#abort-events}
...
{% api %}
## Cliquez sur les événements {#click-events}
...
{% api %}
## Envoyer des événements {#send-events}
...
Cet événement se produit lorsqu'une erreur est reçue du service de notification push d'Apple ou de l'envoi messages de Fire Cloud. Cela signifie que le message push a été renvoyé et qu'il n'a donc pas été transmis à l'appareil de l'utilisateur.
...
Cet événement n'est pas pris en charge par notre [SDK Swift](https://github.com/braze-inc/braze-swift-sdk) et est désormais déprécié à l'aide de notre [SDK Obj-C.](https://github.com/Appboy/appboy-ios-sdk)
...
Cet événement se produit lorsqu'un utilisateur clique directement sur la notification push pour ouvrir l'application. Actuellement, les événements "Push Open" font spécifiquement référence aux "ouvertures directes" plutôt qu'aux "ouvertures totales". Cela n'inclut pas les statistiques affichées au niveau de la campagne des "ouvertures influencées", car celles-ci ne sont pas attribuées au niveau de l'utilisateur.
...
Cet événement se produit lorsque Braze traite un message push pour un utilisateur, en le communiquant au service de notification push d'Apple ou à Fire Cloud Messaging. Cela ne signifie pas que le message a été transmis à l'appareil, mais simplement qu'un message a été envoyé.
...
{% api %}
## Événements de réception/distribution {#delivery-events}
...
{% api %}
## Réception d'événements {#inbound-receive-events}
...
{% api %}
## Lire les événements {#read-events}
...
{% api %}
## Événements de rejet {#rejection-events}
...
Cet événement se produit lorsqu'un SMS subit un échec de réception/distribution. Utilisez cet événement et les codes d'erreur fournis pour résoudre les problèmes de réception/distribution des SMS.
...
Cet événement se produit lorsqu'un de vos utilisateurs envoie un SMS à un numéro de téléphone figurant dans l'un de vos groupes d'abonnement SMS de Braze.

Lorsque Braze reçoit un SMS entrant, nous attribuons ce message entrant à tout utilisateur partageant ce numéro de téléphone. Par conséquent, vous pouvez recevoir plusieurs événements par message entrant si plusieurs utilisateurs de votre instance Braze partagent le même numéro de téléphone. Si vous souhaitez attribuer des ID d'utilisateurs spécifiques en fonction des messages précédemment envoyés à cet utilisateur, vous pouvez utiliser l'événement SMS délivré pour attribuer les événements reçus entrants à l'ID d'utilisateur qui a reçu le plus récemment un message en provenance de votre numéro Braze.

Si nous détectons que ce message entrant est une réponse à une campagne sortante ou à un composant Canvas envoyé depuis Braze, nous inclurons également les métadonnées de la campagne ou de Canvas avec l'événement. Braze définit une réponse comme un envoi de messages entrants dans les quatre heures suivant un message sortant. Toutefois, il existe un cache d'une minute pour les informations relatives à la campagne attribuée au dernier SMS sortant reçu.
...
Cet événement se produit lorsqu'un envoi de SMS est rejeté par l'opérateur, ce qui peut se produire pour plusieurs raisons. Utilisez cet événement et les codes d'erreur fournis pour résoudre les problèmes de réception/distribution des SMS.
...
Cet événement se produit lorsqu'un webhook a été traité et envoyé au tiers spécifié dans ce webhook. Notez que cela ne signifie pas que la demande a été reçue ou non.
...
Cet événement se produit lorsque WhatsApp ne peut pas envoyer le message à l'utilisateur. Un échec d'envoi définitif signifie un échec de livrabilité définitif.
