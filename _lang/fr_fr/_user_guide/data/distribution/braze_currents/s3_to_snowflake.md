...
page_order: 7
page_type: tutorial
description: "Cet article pratique vous guidera dans le transfert de données d'un stockage dans le cloud (comme Amazon S3) vers un entrepôt (comme Snowflake) à l'aide du processus Extraire, Transformer, Charger (ETL)."
tool: Currents
...
-   [Tuyaux à neige Auto-Ingest](#auto-ingest-snowpipes)
...
2. Créez les éléments suivants dans l'ordre indiqué :
  2.1 Dans AWS, créez une nouvelle **paire de clés publique-privée** pour le compartiment S3 souhaité, avec des subventions conformes aux exigences de sécurité de votre organisation.
  ...
  2.3. Créez une scène S3 Snowflake (appelée `braze_data`) :
...
## Configuration des tuyaux d'arrosage auto-ingest {#auto-ingest-snowpipes}
...
{: start="2"}
2\. Créez le tuyau de chargement continu AUTO et spécifiez-le :
  2.1. Quel tableau charger
  2.2 Comment charger le tableau suivant
...
Pour voir les types d'analyse/analytique que vous pouvez effectuer à l'aide de Braze Currents, consultez nos [blocs Looker](https://github.com/llooker?q=braze).
