...
Vous accorderez à Braze l'accès pour se connecter à votre instance Fabric. Dans votre portail d'administration Fabric, accédez à **Paramètres** > **Gouvernance et informations** > **Portail d'administration** > **Paramètres des locataires**.    
...
Dans le tableau de bord de Braze, accédez à **Paramètres des données** > **Ingestion de données cloud**, sélectionnez **Créer une nouvelle synchronisation des données**, puis sélectionnez **Importation Snowflake**.
...
{% alert note %}
Dans le champ **Localisateur de compte Snowflake**, saisissez l'[identifiant de compte](https://docs.snowflake.com/en/user-guide/admin-account-identifier) Snowflake, qui suit généralement un format tel que `xy12345.us-east-1.aws`. Il ne s'agit pas d'un nom de base de données ou d'entrepôt.
{% endalert %}
...
Dans le tableau de bord de Braze, accédez à **Paramètres des données** > **Ingestion de données cloud**, sélectionnez **Créer une nouvelle synchronisation des données**, puis sélectionnez **Importation Amazon Redshift**.
...
Dans le tableau de bord de Braze, accédez à **Paramètres des données** > **Ingestion de données cloud**, sélectionnez **Créer une nouvelle synchronisation des données**, puis sélectionnez **Importation Google BigQuery**.
...
Dans le tableau de bord de Braze, accédez à **Paramètres des données** > **Ingestion de données cloud**, sélectionnez **Créer une nouvelle synchronisation des données**, puis sélectionnez **Importation Databricks**.
...
Vous allez créer une nouvelle synchronisation de données pour Microsoft Fabric. Dans le tableau de bord de Braze, accédez à **Paramètres des données** > **Ingestion de données cloud**, sélectionnez **Créer une nouvelle synchronisation des données**, puis sélectionnez **Importation Microsoft Fabric**.
...
Après avoir saisi tous les détails de configuration de votre synchronisation, sélectionnez **Tester la connexion**. En cas de succès, vous verrez un aperçu des données. Si la connexion échoue, un message d'erreur s'affichera pour vous aider à résoudre le problème.
...
{% alert note %}
Vous devez tester une intégration avec succès avant qu'elle ne puisse passer de l'état Brouillon à l'état Actif. Si vous devez fermer la page de création, votre intégration sera sauvegardée et vous pourrez revenir à la page de détails pour effectuer des modifications et tester.  
{% endalert %}
