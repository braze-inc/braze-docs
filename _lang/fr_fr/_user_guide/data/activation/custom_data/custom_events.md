...
Voici quelques cas d'utilisation courants d'événements personnalisés :
...
- Segmentation des utilisateurs en fonction du nombre de fois qu'ils ont effectué un événement personnalisé, de la date à laquelle l'événement s'est produit pour la dernière fois, etc.
- L'utilisation du tableau de bord d'[analyse/analytique personnalisé des événements]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events#custom-event-analytics) permet de visualiser un agrégat de la fréquence de chaque événement.
...
Vous pouvez gérer, créer ou mettre en liste de blocage des événements personnalisés dans le tableau de bord en allant dans **Paramètres des données** > Événements personnalisés.
...
Vous pouvez ajouter une description à un événement personnalisé après sa création si vous disposez de l' [autorisation utilisateur]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/) `Manage Events, Attributes, Purchases`. Sélectionnez **Modifier la description de** l'événement personnalisé et **modifiez** les informations que vous souhaitez, par exemple une note à l'intention de votre équipe.

## Ajout d'étiquettes

Vous pouvez ajouter des tags à un événement personnalisé après sa création si vous disposez de l'autorisation utilisateur "Gérer les événements et les attributs, les achats". Les tags peuvent ensuite être utilisés pour filtrer la liste des événements.
...
Le tableau suivant présente les filtres disponibles pour segmenter les utilisateurs en fonction des événements personnalisés.

| Options de segmentation | Filtre déroulant | Options d'entrée |
| ---------------------| --------------- | ------------- |
| Vérifier si l'événement personnalisé s'est produit **plus de X fois.** | **PLUS QUE** | **NUMÉRO** |
| Vérifier si l'événement personnalisé s'est produit **moins de X fois.** | **MOINS DE** | **NUMÉRO** |
| Vérifier si l'événement personnalisé s'est produit **exactement X fois.** | **EXACTEMENT** | **NUMÉRO** |
| Vérifier si l'événement personnalisé s'est produit pour la dernière fois **après la date X** | **APRÈS** | **TEMPS** |
| Vérifier si l'événement personnalisé s'est produit pour la dernière fois **avant la date X** | **AVANT** | **TEMPS** |
| Vérifier si l'événement personnalisé a eu lieu **il y a plus de X jours.** | **PLUS QUE** | **NOMBRE DE JOURS DERNIERS** (Nombre positif) |
| Vérifier si l'événement personnalisé a eu lieu **il y a moins de X jours.** | **MOINS DE** | **NOMBRE DE JOURS DERNIERS** (Nombre positif) |
...
!Filtres personnalisés pour le graphe des événements]({% image_buster /assets/img/custom_events_report_filters.png %}){: style="max-width:40%;"}
...
{% alert important %}
Chaque événement personnalisé ou achat peut avoir jusqu'à 256 propriétés d'événement personnalisé distinctes. Si un événement personnalisé ou un achat est enregistré avec plus de 256 propriétés, seules les 256 premières seront saisies et utilisables.
{% endalert %}

### Format attendu

Les valeurs des propriétés doivent être un objet dont les clés sont les noms des propriétés et les valeurs les valeurs des propriétés. Les noms de propriétés doivent être des chaînes non vides de 255 caractères au maximum, sans signe de dollar (`$`).

Les valeurs des propriétés peuvent correspondre à l'un des types de données suivants :
...
| Dates | Formatés sous forme de chaînes de caractères au format [ISO-8601](https://en.wikipedia.org/wiki/ISO_8601) ou `yyyy-MM-dd'T'HH:mm:ss:SSSZ`. Non pris en charge dans les tableaux. |
| Chaînes de caractères | 255 caractères ou moins. |
| Tableaux | Les tableaux ne peuvent pas contenir de dates. |
| Objets imbriqués | Objets qui se trouvent à l'intérieur d'autres objets. Pour en savoir plus, consultez la section de cet article consacrée aux [objets imbriqués](#nested-objects).
...
### Utilisation des propriétés d'événement personnalisées
...
!Filtres personnalisés de propriétés d'événement pour une carte abandonnée. Deux filtres sont combinés avec un opérateur AND pour envoyer cette campagne aux utilisateurs qui ont abandonné leur carte avec un prix d'article compris entre 100 et 200 dollars.]({% image_buster /assets/img_archive/customEventProperties.png %} "customEventProperties.png")
...
!Filtres personnalisés de propriétés d'événement pour une carte abandonnée. Un filtre est sélectionné si l'un des articles du panier a un prix supérieur à 100 dollars.]({% image_buster /assets/img_archive/customEventPropertiesNested.png %} "customEventPropertiesNested.png")

#### Personnaliser les messages

Vous pouvez également utiliser des propriétés d'événement personnalisées pour la personnalisation du modèle d'envoi de messages. Toute campagne utilisant la [livraison par]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/triggered_delivery/) événement avec un événement déclencheur peut utiliser les propriétés d'événement personnalisées de cet événement pour la personnalisation des messages.
...
Pour obtenir une liste complète des étiquettes Liquid qui entraîneront l'envoi de messages in-app sous forme de messages in-app modélisés, reportez-vous à la rubrique [Foire aux questions.]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/faq/#what-are-templated-in-app-messages/)

##### Considérations relatives aux filtres

- **Appels API :** Lorsque vous effectuez des appels API et que vous utilisez le filtre "is blank", une propriété d'événement personnalisé est considérée comme "vide" si elle est exclue de l'appel. Par exemple, si vous incluez `"event_property": ""`, vos utilisateurs seront considérés comme "non vierges".
- **Entiers :** Lorsque vous filtrez pour un nombre de propriétés d'événement personnalisé et que ce nombre est très grand, n'utilisez pas le filtre "exactement". Si un nombre est trop grand, il peut être arrondi à une certaine longueur, de sorte que votre filtre ne fonctionnera pas comme prévu.
...
Les propriétés des événements personnalisés sont mises à jour en temps réel pour tout segment qui les utilise. Vous pouvez gérer les propriétés en allant dans **Paramètres des données** > **Événements personnalisés** et en sélectionnant **Gérer les propriétés de** l'événement personnalisé associé. Les propriétés d'événement personnalisées utilisées dans certains filtres de segmentation ont un historique maximum de 30 jours.
...
\![Un groupe de filtre qui a 'Abandon de panier' avec la propriété 'number of itmes' et la valeur 2 plus d'une fois au cours des 30 derniers jours calendaires.]({% image_buster /assets/img/nested_object3.png %})
...
En ce qui concerne l'utilisation des abonnements, les propriétés d'événement personnalisé activées pour la segmentation avec les filtres suivants sont toutes comptées comme des points de données distincts en plus du point de données compté par l'événement personnalisé lui-même :
...
Les propriétés d'événement personnalisées sont conçues pour vous aider à améliorer la précision du ciblage et à rendre les messages encore plus personnalisés. Les propriétés d'événements personnalisés peuvent être stockées dans Braze à court et à long terme.
