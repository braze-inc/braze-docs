...
| Auto Opt-In par enregistrement de jeton | Lorsqu'un utilisateur est mis à jour par le [processus d'enregistrement des jetons]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_registration/) |
{: .reset-td-br-1 .reset-td-br-2 role="presentation"}
...
{% alert important %}
Notez que l’événement de conversion est encodé dans le champ`conversion_behavior`, qui inclut le type d’événement de conversion, la fenêtre (période) et des informations supplémentaires en fonction du type d’événement de conversion. Le champ `conversion_behavior_index` représente l'événement de conversion, tel que `0 = A`, `1 = B`, `2 = C`, `3 = D`.
{% endalert %}
