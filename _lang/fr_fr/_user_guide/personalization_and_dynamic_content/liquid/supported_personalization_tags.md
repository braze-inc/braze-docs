...
Un résumé des balises de personnalisation prises en charge est fourni. 
...
{% raw %}
-  Pour en savoir plus, reportez-vous à [Comportement du Dispatch ID][50].
...
## Informations sur les appareils les plus récemment utilisées
 ...
 Si un utilisateur n'a pas utilisé votre application (par exemple, vous avez importé l'utilisateur via l'API REST), ces valeurs seront toutes `null`.
...
|`{{most_recently_used_device.${id}}}` | L'identifiant de l'appareil de Braze. Sur iOS, il s’agit de l’identifiant Apple pour le fournisseur (IDFV) ou un UUID.  |
...
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
 ...
 Ces valeurs seront `null` si elles ne sont pas disponibles sur un appareil particulier.
...
## Informations sur les dispositifs ciblés
 ...
 C’est-à-dire, une notification push ou un message in-app peut inclure des attributs d'appareil de l’appareil sur lequel le message est lu. Notez que ces attributs ne fonctionnent pas pour les cartes de contenu. 

|Balise | Description |
|------------------|---|
| `{{targeted_device.${id}}}` | Ceci est l'identifiant de l'appareil Braze. Sur iOS, il s’agit de l’identifiant Apple pour le fournisseur (IDFV) ou un UUID.  |
...
Vous pouvez utiliser la balise `assign` pour créer une variable dans le compositeur de message.  
...
## Envoi de messages en fonction de la langue, des paramètres régionaux les plus récents et du fuseau horaire
 ...
 Par exemple, le portugais brésilien est généralement différent du portugais européen.
