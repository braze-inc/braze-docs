...
- **Identifiant de l'utilisateur :** Chaque profil utilisateur est identifié de manière unique par un ID utilisateur, appelé `external_id`. Cet identifiant permet à Braze de suivre et d'associer les données des utilisateurs sur différents canaux et appareils, offrant ainsi une vue unifiée des interactions de chaque utilisateur avec votre marque.
