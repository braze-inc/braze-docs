...
Braze propose trois types d'abonnement internationaux pour les utilisateurs d'e-mail. Ces états contrôlent vos messages provenant des utilisateurs. Par exemple, les utilisateurs de `unsubscribed`l'État ne reçoivent pas de messages ciblés vers`subscribed`ou `opted-in`.
...
Braze désabonne automatiquement tout utilisateur qui se désabonne manuellement via un [pied de page personnalisé]({{site.baseurl}}/user_guide/message_building_by_channel/email/custom_email_footer). Si l'utilisateur met à jour son adresse e-mail et **que l'option « Réinscrire les utilisateurs lorsqu'ils mettent à jour leur adresse e-mail** » est activée dans **la configuration d'envoi**, l'envoi normal reprend.

Si un utilisateur marque un ou plusieurs de vos e-mails comme spam, Braze n'enverra plus que des e-mails transactionnels à cet utilisateur. Les e-mails transactionnels font référence à l'option **Envoyer à tous les utilisateurs, y compris les utilisateurs désabonnés** dans **Public cible**.
...
Lorsqu'un e-mail provoque un échec d'envoi définitif, Braze ne définit pas automatiquement le statut d'abonnement de l'utilisateur sur « désabonné ». Si une adresse provoque un échec d'envoi définitif (invalide ou inexistante), Braze la marque comme invalide et ne tente plus d'envoyer de messages. Si l'utilisateur modifie son adresse de e-mail, Braze reprend l'envoi. Braze effectue des tentatives de renvoi pour éviter les échecs provisoires d'envoi pendant 72 heures.
...
Veuillez utiliser [`/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)l'[endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) pour mettre à jour [`email_subscribe`]({{site.baseurl}}/api/objects_filters/user_attributes_object)l'[attribut]({{site.baseurl}}/api/objects_filters/user_attributes_object) d'un utilisateur.

#### Profil utilisateur

1. Trouvez l'utilisateur par le biais de la **recherche d'utilisateurs**. 
2. Sous **Engagement**, veuillez sélectionner **Désabonné**, **Abonné** ou **Abonnement** pour modifier le statut d'abonnement de l'utilisateur. 
...
Veuillez inclure [le centre de préférences](#email-preference-center) Liquid au bas de vos e-mails afin de permettre aux utilisateurs d'effectuer un abonnement ou de se désabonner. Braze gère les mises à jour de l'état des abonnements à partir du centre de préférences.
...
Vous pouvez vérifier l'état de l'abonnement à l'e-mailing d'un utilisateur de différentes manières :
...
Vous pouvez consulter le nombre d'utilisateurs ayant modifié leur statut d'abonnement (abonnés ou désabonnés) à partir d'une campagne d'e-mails spécifique sur la page d'analyse de cette campagne.
...
### Vérification du groupe d'abonnement aux e-mails d'un utilisateur
...
- **API REST Braze :** Veuillez utiliser l'[endpoint « Lister les groupes d'abonnement de l'utilisateur]({{site.baseurl}}/api/endpoints/subscription_groups/get_list_user_subscription_groups/) » ou [l'endpoint « Lister le statut du groupe d'abonnement de l'utilisateur]({{site.baseurl}}/api/endpoints/subscription_groups/get_list_user_subscription_group_status/) » pour afficher les groupes d'abonnement du profil utilisateur. 

## Centre de préférence des e-mails

Le centre de préférences d'e-mail vous permet de gérer les utilisateurs qui reçoivent les newsletters des groupes d'abonnement. Vous pouvez le trouver dans le tableau de bord sous **« Subscription groups** ». Chaque groupe d’abonnement que vous créez est ajouté à la liste du centre de préférences. 
...
Dans la plupart des cas, les utilisateurs gèrent leur abonnement par e-mail via les liens inclus dans les e-mails qu'ils reçoivent. Veuillez insérer un pied de page conforme à la législation avec un lien de désabonnement au bas de chaque e-mail. Lorsque les utilisateurs cliquent sur l'URL de désabonnement, Braze procède à leur désabonnement et affiche une page d'accueil confirmant la modification. Veuillez inclure cette étiquette Liquid : {%raw%}`${set_user_to_unsubscribed_url}`{%endraw%}.

Lorsqu'un utilisateur sélectionne « Se désabonner de tous les types d'e-mails ci-dessus » dans le centre de préférences, Braze définit son statut d'abonnement global aux e-mails sur « `unsubscribed`désabonné » et le désabonne de tous les groupes.

### Création de pieds de page personnalisés {#custom-footer}

Si vous ne souhaitez pas utiliser le pied de page par défaut, veuillez créer un pied de page personnalisé pour l'ensemble de l'espace de travail et l'intégrer à chaque e-mail à l'aide de {% raw %}`{{${email_footer}}}`{% endraw %}.

Cela vous évite de devoir créer un nouveau pied de page pour chaque modèle d'e-mail ou campagne d'e-mail. Pour connaître la procédure à suivre, veuillez consulter [la section « Pied de page personnalisé pour les e-mails]({{site.baseurl}}/user_guide/message_building_by_channel/email/custom_email_footer/) ».

#### Gestion des abonnements pour les adresses IP chinoises

Si vous prévoyez d'utiliser des adresses IP chinoises, veuillez ne pas vous fier uniquement à un lien de désabonnement pour gérer`unsubscribed`vos listes. Veuillez proposer d'autres moyens de se désabonner, tels qu'un ticket d'assistance ou l'adresse e-mail d'un conseiller du service clientèle. 

### Créer une page de désabonnement personnalisée

Lorsque les utilisateurs cliquent sur un lien de désabonnement, Braze affiche une page d'accueil par défaut confirmant la modification.

Pour créer une page d'accueil personnalisée (au lieu de la page par défaut) qui s'affiche après s'être abonné :
...
Veuillez inclure un lien de réinscription (tel que {% raw %}`{{${set_user_to_subscribed_url}}}`{% endraw %}) afin que les utilisateurs puissent se réinscrire s'ils se sont désinscrits par inadvertance.
...
Veuillez utiliser une page d'abonnement personnalisée pour permettre aux utilisateurs de confirmer et de contrôler leurs préférences de notification avant de s'abonner. Cette communication supplémentaire peut contribuer à éviter que les campagnes par e-mail ne soient classées dans les dossiers de courrier indésirable.
...
Les utilisateurs accèdent à cette page via {% raw %}`{{${set_user_to_opted_in_url}}}`{% endraw %}l'étiquette.

{% alert tip %}
Veuillez utiliser un processus d'abonnement double pour améliorer la portée. Braze envoie un e-mail de confirmation supplémentaire dans lequel l'utilisateur confirme ses préférences de notification via un lien. Après confirmation, l'utilisateur bénéficie d'un abonnement.
{% endalert %}
...
Par défaut, Braze cible les campagnes avec des messages push ou des e-mails aux utilisateurs abonnés ou qui ont donné leur consentement. Veuillez modifier cela dans **Audience** en sélectionnant le menu déroulant à côté de **Envoyer à ces utilisateurs :******.
...
Veuillez utiliser les filtres « Statut d'abonnement par e-mail » et « Statut d'abonnement push » pour segmenter les utilisateurs en fonction de leur statut d'abonnement.

Veuillez utiliser cette option pour le ciblage des utilisateurs qui n'ont ni accepté ni refusé un abonnement et les encourager à accepter explicitement. Veuillez créer un segment avec le filtre « Statut d'abonnement aux e-mails/notifications push : abonné » et envoyer des campagnes aux utilisateurs abonnés mais qui n'ont pas donné leur consentement.
