...
|---|---|---|
| Une IP dédiée (protocole Internet)| Une adresse IP dédiée est une adresse Internet unique fournie exclusivement à un seul compte d’hébergement. | Braze vous fournit des adresses IP dédiées afin de garantir le contrôle de la réputation de votre expéditeur d'e-mails. L’onboarding de Braze vous permettra de le configurer.|
...
[Le réchauffement IP]({{site.baseurl}}/user_guide/message_building_by_channel/email/email_setup/ip_warming/) consiste à envoyer un nombre relativement restreint d'e-mails lors de votre premier envoi, puis, au fil du temps, à augmenter légèrement le volume des envois suivants jusqu'à atteindre votre volume quotidien habituel. Cette opération s'effectue à la toute fin de la procédure de configuration de votre e-mail.
