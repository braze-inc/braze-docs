...
![Une publicité Facebook de Calorie Rocket qui annonce la réception/distribution gratuite, et la conversation WhatsApp correspondante qui se produit lorsqu'un utilisateur sélectionne le bouton de la publicité.]({% image_buster /assets/img/whatsapp/ads_that_click_whatsapp.png %}){: style="max-width:70%;"}
...
![Le modèle de compositeur du gestionnaire d'annonces avec un message pré-rempli "Je veux une réception/distribution gratuite".]({% image_buster /assets/img/whatsapp/pre_filled_message.png %})
...
![Planification d'entrée pour une livraison par événement de Braze Canvas, avec l'événement déclencheur "Envoyer un message entrant WhatsApp" et un corps de message qui correspond à l'expression régulière "réception/distribution gratuite".]({% image_buster /assets/img/whatsapp/action_based_free_delivery.png %})
...
![Étape du canvas avec un groupe "Opting in" avec un événement déclencheur de "Sent inbound WhatsApp to subscription group" et un mot déclencheur de "YES".]({% image_buster /assets/img/whatsapp/opting_in_step.png %})
...
![Mise à jour de l'utilisateur Étape du canvas qui utilise l'éditeur JSON avancé pour mettre à jour le profil utilisateur.]({% image_buster /assets/img/whatsapp/user_update_step_json.png %})

![Canevas montrant le flux de travail pour l'envoi de publicités qui cliquent à WhatsApp, y compris trois parcours d'action : L'abonnement, l'exclusion et tous les autres.]({% image_buster /assets/img/whatsapp/ads_that_click_canvas.png %})
