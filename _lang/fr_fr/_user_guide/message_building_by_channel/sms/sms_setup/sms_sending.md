...
Pour pallier cette limitation, lors de la configuration des groupes d'abonnement (][5]), des groupes peuvent être constitués pour contenir des codes longs et courts provenant de plusieurs pays différents. Une fois cette opération terminée, les numéros de téléphone d'envoi ayant le même code pays que le numéro de téléphone de l'utilisateur cible seront automatiquement utilisés lors du lancement d'une campagne. Vous n’aurez pas à créer de campagnes distinctes pour les utilisateurs ayant des numéros de téléphone avec des codes pays différents, ce qui vous permet de lancer une campagne ou d’utiliser un composant de Canvas pour cibler les utilisateurs concernés.
...
[5]: {{site.baseurl}}/user_guide/message_building_by_ (en anglais)channel/sms/sms_subscription_group/#setup-process
