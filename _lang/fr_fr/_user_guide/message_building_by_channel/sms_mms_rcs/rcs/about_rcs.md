...
| --- | --- |
| Envoi de messages | Contactez votre gestionnaire de compte Braze pour confirmer que vous avez acheté des crédits de messages dans votre contrat. Les crédits de messages sont un élément contractuel flexible qui vous permet d'acheter et d'allouer un volume d'envoi de messages sur différents canaux, tels que les SMS, les MMS, le RCS et WhatsApp. |
...
| Liste des utilisateurs avec numéros de téléphone | Pour pouvoir envoyer des messages, vous devez ajouter des utilisateurs à votre compte. Vous devez par ailleurs connaître la taille approximative de votre public. Les utilisateurs et les numéros de téléphone peuvent être ajoutés à Braze de différentes manières. Les numéros de téléphone doivent avoir un format de 10 chiffres et inclure un indicatif régional. Pour en savoir plus, consultez la rubrique [Numéros de téléphone des utilisateurs]({{site.baseurl}}/user_phone_numbers/). |
...
| RCS texte uniquement | Cette catégorie comprend les messages RCS simples qui se limitent à du texte, comme les SMS traditionnels. Ces messages peuvent contenir jusqu'à 160 caractères et fournissent un niveau de communication de base sans aucun élément de média enrichi.<br><br> Vous pouvez envoyer des messages textuels d'une longueur maximale de 3 072 caractères. Toutefois, lorsqu'ils dépassent 160 caractères, ils seront facturés de la même manière qu'un message RCS riche (un message RCS simple). |
| Envoi de messages RCS enrichis | Les messages RCS enrichis tirent parti des fonctionnalités les plus attrayantes et les plus riches offertes par RCS, telles que les médias, les boutons, etc. |
| "de base" et "unique" | Les messages RCS sont principalement facturés de deux manières : en tant que messages de base et messages uniques. L'option "Basic" correspond au RCS texte seul, tandis que l'option "Single" correspond au RCS riche. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
