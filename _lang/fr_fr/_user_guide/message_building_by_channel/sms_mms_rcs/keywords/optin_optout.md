...
|-|-------|---|
|Inscription| `START`<br> `YES`<br> `UNSTOP` | Toute demande entrante contenant l'un de ces mots-clés `Opt-In` entraînera un changement d'état du groupe d'abonnement, qui passera à `subscribed`. En outre, les expéditeurs associés à ce groupe d'abonnement pourront désormais envoyer un message SMS, MMS ou RCS à ce client (en fonction du type d'envoi de messages pris en charge par les expéditeurs). <br><br>L’utilisateur reçoit votre réponse automatique d’inscription définie.  |
|Désabonnement| `STOP`<br> `STOPALL`<br> `UNSUBSCRIBE`<br> `CANCEL`<br> `END`<br> `QUIT` | Toute demande entrante contenant l'un de ces mots-clés `Opt-Out` entraînera un changement d'état du groupe d'abonnement, qui passera à `unsubscribed`. En outre, le pool de numéros associé à ce groupe d'abonnement ne pourra plus envoyer de messages à ce client.<br><br>L’utilisateur reçoit votre réponse automatique de désabonnement définie. |
