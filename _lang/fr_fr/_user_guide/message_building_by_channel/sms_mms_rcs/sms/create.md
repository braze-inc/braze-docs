---
nav_title: Créer un message SMS
article_title: Créer un message SMS
page_order: 5
...
![Envoi d'un SMS au compositeur dans Braze avec le message suivant : "Salut first_name,, nous apprécions votre soutien ! Pourquoi ne pas passer dans l’un de nos magasins et leur montrer ce SMS pour bénéficier d’une remise exclusive ? Répondez STOP pour ne plus recevoir des messages de notre part. »]({% image_buster /assets/img/sms_campaign_compose.png %})
...
![Lancez le bouton Copywriter de l'intelligence artificielle, situé dans le champ Message du compositeur de SMS.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_sms.png %}){: style="max-width:60%"}
...
Au cours de cette étape, vous sélectionnez l'audience la plus large à partir de vos segments et, si vous le souhaitez, vous réduisez encore ce segment à l'aide de nos filtres. Vous recevez automatiquement un aperçu de ce à quoi ressemble la population de ce segment approximatif. N'oubliez pas que l'appartenance exacte à un segment est toujours calculée avant l'envoi du message.
