...
Si vous souhaitez un peu de fantaisie, la carte bannière est faite pour vous. Il est entièrement personnalisé en fonction de vos souhaits. Créez simplement votre contenu ailleurs et chargez-le pour avoir une carte magnifique qui vous ressemble !

![Image d'une bannière avec des détails recommandés et un exemple de bannière]({% image_buster /assets/img/content_card_banner.png %}){: style="max-width:45%;border:0;"}
...
![Image montrant le swipe ou la fermeture d’une carte pour la refuser][5]
...
![Image d'un flux de cartes de contenu vide][6]{: style="max-width:45%"}
...
| --- | --- |---|
| Par défaut, le SDK Android ne prend pas en charge les GIF animés.  | Par défaut, le SDK Swift ne prend pas en charge les GIF animés. Pour plus de détails sur l'activation de la prise en charge du format GIF, reportez-vous au [didacticiel sur la prise en charge du format GIF](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/c3-gif-support). | La prise en charge du format GIF est incluse par défaut dans l'intégration SDK Web. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
