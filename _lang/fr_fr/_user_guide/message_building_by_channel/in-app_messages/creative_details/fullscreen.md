...
Lors de la prévisualisation d’un message in-app plein écran dans la plateforme Braze, vous pouvez activer la zone de sécurité d’image dans la zone du message, sans danger à l’affichage d’un message sur divers appareils. En plus de tester la zone de sécurité de l'image dans le volet de prévisualisation, nous vous recommandons de [tester votre message]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/sending_test_messages/) comme d'habitude.

![Aperçu d’un message in-app en Braze avec l’option « Show Image Safe Zone » (Afficher la zone sécurisée d’image) activée. La zone sécurisée d’image est une incrustation sur l’image qui montre les parties de l’image non sujettes au rognage.]({% image_buster /assets/img/image-safe-zone-full-screen-in-app-message.png %})
