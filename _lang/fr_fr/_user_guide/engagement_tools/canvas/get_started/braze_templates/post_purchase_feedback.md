...
Cet article vous guidera à travers un cas d'utilisation du modèle **Feedback post-achat**, conçu pour l'étape de conversion du cycle de vie de l'utilisateur. Lorsque vous aurez terminé, vous aurez créé un Canvas qui encourage les utilisateurs à donner leur avis sur votre application.
...
\!["Paramètres d'envoi" étape de ciblage des utilisateurs qui sont abonnés ou opt-in. Les heures calmes sont activées.]({% image_buster /assets/img/canvas_templates/post_purchase_feedback/send_settings_with_quiet_hours.png %}){: style="max-width:90%;"}
...
### Mettre en place des cas de support webhook
