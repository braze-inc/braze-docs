...
Imaginons que nous travaillions pour Steppington, une application de santé connue pour ses fonctionnalités telles que le suivi des calories, les cours d'exercices numériques et les marathons flash-mob. Avant de créer le Canvas, nous avons mis [en place des messages in-app et in-browser de plusieurs pages]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/drag_and_drop/create#multi-page) qui comprennent une série de questions engageantes pour déterminer l'expérience et l'impression d'un utilisateur lors de sa première utilisation de l'application.
