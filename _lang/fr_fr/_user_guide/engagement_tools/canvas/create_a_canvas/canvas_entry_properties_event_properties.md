...
| **Persistance** | Peut être référencé par toutes les étapes du [message]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step/) pendant la durée d'un canvas créé à l'aide de Canvas. | \- Elle ne peut être référencée qu’une seule fois. <br> \- Elle ne peut pas être référencée par des étapes de messagerie suivantes. |
...
- Pour les canaux de messages in-app, vous pouvez faire référence à `canvas_entry_properties` et `event_properties` dans un canvas. `event_properties` est accessible lorsqu'il est inclus dans la première étape du canvas car il est basé sur un déclencheur.
...
![Un parcours d'action suivi d'un délai et d'un message pour les utilisateurs qui ont ajouté un article à leur liste de souhaits, et un parcours pour tous les autres.]({% image_buster /assets/img_archive/canvas_entry_properties1.png %}){: style="float:right;max-width:30%;margin-left:15px;"}

Pour mieux comprendre les différences entre `canvas_entry_properties` et `event_properties`, examinons le scénario suivant : les utilisateurs entrent dans un canvas basé sur des actions s'ils effectuent l'événement personnalisé "ajouter un article à la liste de souhaits". 

Les adresses `canvas_entry_properties` sont configurées à l'étape de la [planification de la]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas#step-2b-set-your-canvas-entry-schedule) création d'un canvas et correspondent au moment où un utilisateur entre dans un canvas. Ces `canvas_entry_properties` peuvent également être référencés dans n'importe quelle étape du message.

Dans ce canvas, nous avons un parcours utilisateur qui commence par une étape Parcours d'action pour déterminer si un utilisateur a ajouté un article à sa liste de souhaits. À partir de là, si l'utilisateur a ajouté un article, il subit une expérience de communication avant de recevoir un message "Nouvel article dans votre liste de souhaits !" à partir de l'étape Message. 

La première étape Message d'un parcours client a accès à l'adresse `event_properties` personnalisée de votre étape Chemins d'action. Dans ce cas, nous sommes en mesure d'inclure ``{% raw %} {{event_properties.${property_name}}} {% endraw %}`` dans cette étape Message en tant que partie du contenu de notre message. Si un utilisateur n'ajoute pas d'article à sa liste de souhaits, il passe par le chemin "Everyone Else", ce qui signifie que le site `event_properties` ne peut pas être référencé et renvoie une erreur de paramètres non valides.

Prenez en compte le fait que vous n’aurez accès aux `event_properties` que si votre étape de message peut être remontée jusqu’à un parcours n’étant pas « Tous les autres » dans l’étape du parcours d’action. Si l'étape du message est liée à un parcours Everyone Else mais peut être retracée jusqu'à une étape Action Paths dans le parcours de l'utilisateur, alors vous avez toujours accès à `event_properties`. Pour plus d'informations sur ces comportements, reportez-vous à la rubrique [Étape du message.]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/message_step/)
