...
> Lorsqu'un Canvas est déclenché par un événement personnalisé, un achat ou un appel API, vous pouvez utiliser les métadonnées de l'appel API, de l'événement personnalisé ou de l'événement d'achat pour la personnalisation à chaque étape de votre flux de travail Canvas. Vous pouvez utiliser ces propriétés pour envoyer des messages plus personnalisés.

{% alert important %}
Les propriétés d'entrées persistantes sont un vestige de l'éditeur Canvas d'origine. Par conséquent, certaines références obsolètes à des termes tels que « propriétés d'entrées Canvas » subsistent à titre historique. Pour obtenir des informations sur la dernière version de l'éditeur Canvas, veuillez consulter [les propriétés]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties) de [contexte et d'événement]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties).
{% endalert %}
...
Dans cette demande, la valeur globale pour « allergies alimentaires » est « aucune ». Customer_123,La valeur est « produits laitiers ». Les messages dans ce Canvas contenant l'extrait de code {%raw%}`{{canvas_entry_properties.${food_allergies}}}`{%endraw%}Liquid utiliseront le modèle « dairy » pourCustomer_123et « none » pour tous les autres. 
