...
Braze évalue les variables de contexte censées être de type JSON (ou Object) des réponses au contenu connecté en chaînes de caractères. Pour éviter que les variables de contexte ne soient évaluées comme des chaînes de caractères, entrez ces résultats dans ce filtre Liquid : `as_json_string`. Voici un exemple :
