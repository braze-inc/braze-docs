...
> L'arbre décisionnel de Canvas vous permet d'offrir des expériences de personnalisation et en temps réel à vos utilisateurs.

![Une étape de l'arbre décisionnel intitulée « Push activé ? » pour les utilisateurs qui n'ont pas activé la fonction push et ceux qui l'ont activée.]({% image_buster /assets/img/decision-split-1.png %}){: style="float:right;max-width:40%;margin-left:15px;margin-top:15px;margin-bottom:15px;"}
...
![Une étape de l'arbre décisionnel avec le filtre « Foreground Push Enabled is true » (Activation de la mise en avant) sélectionné.]({% image_buster /assets/img/define-split-2.png %})
...
![Une étape de l'arbre décisionnel intitulée "Push enabled ?" pour les utilisateurs qui ne sont pas équipés de la fonction Push et ceux qui le sont. Les utilisateurs qui ne disposent pas de la fonction "push" verront un délai de trois jours avant de recevoir un message par e-mail. Les utilisateurs qui ont activé les notifications push subiront un retard d'un jour, recevront une notification push suivie d'un retard de deux jours, puis recevront le même e-mail que les utilisateurs qui n'ont pas activé les notifications push.]({% image_buster /assets/img/use-split-onboarding-3.png %}){: style="max-width:60%"}
