...
Bien que Braze ne fournisse pas de suivi d'attribution d'installation, nous pouvons intégrer des [services]({{site.baseurl}}/partners/message_orchestration/attribution) tels que Branch et AppsFlyer pour vous fournir de façon fluide des données d'installation.

## Segmenter vos utilisateurs

Une fois que votre utilisateur installe votre application, vous pouvez commencer à le segmenter en fonction des [filtres d'attribution d'installation][2] suivants. Par exemple, une application de voyage pourrait ajouter des utilisateurs provenant d’une publicité liée aux offres de vacances sur la plage dans un segment « Aiment la plage ». De même, une application musicale peut segmenter les utilisateurs en fonction du genre de musique affiché dans la publicité qui a entraîné l’installation.
