---
nav_title: Données segmentation
article_title: Visualisation et compréhension des données de segmentation
page_order: 4
page_type: reference
...
## Accès aux données relatives à vos segments et à vos membres
...
!page des segments]({% image_buster /assets/img_archive/segments.png %})

Après avoir sélectionné le nom d'un segment, vous pouvez afficher les statistiques et les filtres du segment, et modifier le segment en ajoutant ou en supprimant des filtres. Veillez à enregistrer les modifications apportées !
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">Nombre total d'utilisateurs de votre application.</td>
        </tr>
        <tr>
            <td class="no-split">Utilisateurs sélectionnés</td>
            <td class="no-split">Combien d'utilisateurs se trouvent dans votre segmentation et quel pourcentage de votre base totale d'utilisateurs ils représentent.</td>
        </tr>
        <tr>
            <td class="no-split">LTV (utilisateurs payants)</td>
            <td class="no-split">La valeur vie client par utilisateur (LTV) dans ce segment et la valeur vie client par utilisateur payant dans ce segment. La LTV est calculée en divisant votre chiffre d'affaires à vie par le nombre d'utilisateurs à vie.</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">Le terme "Push enabled" désigne le nombre d'utilisateurs disposant d'au moins un jeton push. Certains utilisateurs peuvent avoir plusieurs jetons push (par exemple, s'ils possèdent un iPhone et un iPad), de sorte que le nombre de notifications push que vous envoyez à ce segment peut être supérieur au nombre d'utilisateurs "push enabled". " Opted In " fait référence au nombre d'utilisateurs qui ont explicitement opté pour les notifications push. Les utilisateurs doivent toujours donner leur accord explicite pour que vous leur envoyiez des pushs.</td>
        </tr>
    </tbody>
</table>
...
Vous pouvez voir les performances d'un segment par rapport à un autre en fonction d'un ensemble d'indicateurs clés de performance présélectionnés en consultant la page [Statistiques des segments de]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_insights/) votre tableau de bord.
...
La section **Historique de l'adhésion** montre comment la taille de votre segmentation a évolué dans le temps. Utilisez le menu déroulant pour filtrer l'appartenance à un segment par plage de dates.
...
Sur cette page, vous pouvez consulter un certain nombre d'attributs propres à l'utilisateur, tels que le sexe, l'âge, le nombre de sessions, et s'il a opté pour le push et l'e-mail.
...
## Création d'un rapport de segmentation à l'aide du générateur de rapports

Pour créer un rapport à partir d'un modèle [Query]({{site.baseurl}}/user_guide/analytics/query_builder/) **Builder**, accédez à **Query Builder** et procédez comme suit :

1. Sélectionnez **Créer une requête SQL** > **Modèle de requête.**
2. Filtrez les modèles pour ceux dont les indicateurs comprennent des "segmentations".
...
- **Plage de temps :** Identifiez la période dont vous souhaitez extraire les données. Si aucun intervalle de temps n'est spécifié, l'intervalle de temps sera par défaut les 30 derniers jours.
...
- **Segmentations :** Identifier les segments par lesquels ventiler les données. S'il n'est pas spécifié, le rapport sera exécuté pour tous les segments pour lesquels le suivi analytique est activé.
- **Tags :** Spécifiez des tags dans les **Variables** pour exécuter votre rapport pour toutes les campagnes ou les Canevas avec certains tags. Vous pouvez inclure plusieurs étiquettes. Si vous ajoutez à la fois des étiquettes et des campagnes ou canevas spécifiques à un rapport, votre rapport inclura les données de vos étiquettes et des campagnes ou canevas spécifiés. 
