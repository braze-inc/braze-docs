...
![Tableau présentant le nombre total d'utilisateurs accessibles, ventilé par utilisateurs accessibles par e-mail, notifications push iOS, notifications push Android, notifications push Web et notifications push Kindle.]({% image_buster /assets/img_archive/segmenter_reachable_users.png %})
...
![Un groupe de filtres avec un filtre pour les utilisateurs qui ont eu exactement une session.]({% image_buster /assets/img_archive/segment_filter_stats.png %})
...
![Le panneau « Utilisateurs accessibles » indique qu'il y a entre 2,3 et 2,4 millions d'utilisateurs estimés.]({% image_buster /assets/img_archive/reachable_users_side_panel.png %})
...
![Le panneau latéral « Utilisateurs accessibles » affiche un nombre exact de 31 utilisateurs.]({% image_buster /assets/img_archive/reachable_users_panel.png %})
...
#### Filtres appliqués pour les utilisateurs accessibles spécifiques à un canal

Les filtres suivants sont appliqués à chaque canal lors de la détermination des utilisateurs accessibles.

| Canal | Filtre |
| --- | --- |
| E-mail | **L'adresse e-mail** est **valide**. |
| Notification push | **L'option « Foreground Push Enabled** » est activée. |
| SMS | **Un groupe d'abonnement** désigne tout groupe d'abonnement SMS. **Le numéro de téléphone** n'est pas valide. |
...
{% alert note %}
Les segments répartis de manière égale par [des numéros de compartiments aléatoires]({{site.baseurl}}/user_guide/engagement_tools/testing/random_bucket_numbers/) ne seront pas de taille identique. Par exemple, si vous créez un segment avec le filtre **« Random Bucket # inférieur à 5000** » et un segment avec le filtre **« Random Bucket # supérieur ou égal à 5000** », il est possible et prévisible que la taille des segments varie de quelques points de pourcentage. Cela est dû à des situations telles que la suppression des utilisateurs inactifs et l'inaccessibilité de certains utilisateurs.
{% endalert %}

![Le panneau « Utilisateurs accessibles » avec une option permettant d'afficher la répartition.]({% image_buster /assets/img_archive/reachable_users_breakdown.png %})
...
Par exemple, supposons que votre espace de travail compte 100 millions d'utilisateurs et que votre segment en compte environ 700. Il est possible que certains jours, aucun utilisateur ne se trouve dans le segment et qu'aucun utilisateur n'atterrisse dans le compartiment aléatoire utilisé pour l'estimation historique de l'adhésion, ce qui se traduit par un nombre d'adhésions journalier de 0.
