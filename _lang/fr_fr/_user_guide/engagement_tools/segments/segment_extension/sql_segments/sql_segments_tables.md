...
Tableau | Description
------|------------
[CATALOGS_ITEMS_SHARED](#CATALOGS_ITEMS_SHARED) | Articles du catalogue non supprimés
...
[USERS_BEHAVIORS_LIVEACTIVITY_PUSHTOSTARTTOKENCHANGE_SHARED](#USERS_BEHAVIORS_LIVEACTIVITY_PUSHTOSTARTTOKENCHANGE_SHARED) | Lorsqu'un jeton Live Activity « push-to-start » change
[USERS_BEHAVIORS_LIVEACTIVITY_UPDATETOKENCHANGE_SHARED](#USERS_BEHAVIORS_LIVEACTIVITY_UPDATETOKENCHANGE_SHARED) | Lorsqu'un jeton de mise à jour d'activité en ligne/en production/instantané est modifié
...
[USERS_MESSAGES_BANNER_ABORT_SHARED](#USERS_MESSAGES_BANNER_ABORT_SHARED) | Un message initialement prévu dans la bannière a été annulé pour une raison quelconque.
...
[USERS_MESSAGES_LINE_ABORT_SHARED](#USERS_MESSAGES_LINE_ABORT_SHARED) | Lorsqu'un message LINE programmé ne peut être délivré, avant l'envoi vers LINE
...
[USERS_MESSAGES_LIVEACTIVITY_OUTCOME_SHARED](#USERS_MESSAGES_LIVEACTIVITY_OUTCOME_SHARED) | Lorsqu'une activité en ligne/en production/instantanée est associée à un événement de résultat
[USERS_MESSAGES_LIVEACTIVITY_SEND_SHARED](#USERS_MESSAGES_LIVEACTIVITY_SEND_SHARED) | Lorsqu'un message d'activité en ligne/en production/instantané est envoyé
[USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED](#USERS_MESSAGES_NEWSFEEDCARD_ABORT_SHARED) | Un message initialement prévu dans la carte de fil d'actualité a été interrompu pour une raison quelconque.
...
[USERS_MESSAGES_PUSHNOTIFICATION_OPEN_SHARED](#USERS_MESSAGES_PUSHNOTIFICATION_OPEN_SHARED) | Lorsqu'un utilisateur ouvre une notification push ou clique sur un bouton de notification push (y compris un bouton CLOSE qui n'ouvre PAS l'appli). <br><br> Les boutons d'action push ont des résultats multiples. Les actions « Non », « Refuser » et « Annuler » correspondent à des « clics », et les actions « Accepter » correspondent à des « ouvertures ». Les deux sont représentés dans ce tableau, mais ils peuvent être distingués dans la**BUTTON_ACTION_TYPE**colonne. Par exemple, une requête peut être utilisée pour regrouper en fonction d'un `BUTTON_ACTION_TYPE` qui n'est ni Non, ni Refuser, ni Annuler.
...
[USERS_MESSAGES_RCS_ABORT_SHARED](#USERS_MESSAGES_RCS_ABORT_SHARED) | Lorsqu'un envoi RCS est interrompu en raison d'une erreur détectée dans Braze et que le message est supprimé
[USERS_MESSAGES_RCS_CLICK_SHARED](#USERS_MESSAGES_RCS_CLICK_SHARED) | Lorsque l'utilisateur final interagit avec un message RCS en appuyant ou en cliquant sur un élément de l'interface utilisateur
[USERS_MESSAGES_RCS_DELIVERY_SHARED](#USERS_MESSAGES_RCS_DELIVERY_SHARED) | Lorsqu'un message RCS est livré avec succès sur l'appareil mobile d'un utilisateur final
...
[USERS_MESSAGES_RCS_REJECTION_SHARED](#USERS_MESSAGES_RCS_REJECTION_SHARED) | Lorsqu'un message RCS ne peut être délivré en raison d'une intervention de l'opérateur
[USERS_MESSAGES_RCS_SEND_SHARED](#USERS_MESSAGES_RCS_SEND_SHARED) | Lorsqu'un message RCS est envoyé depuis les systèmes Braze vers les partenaires de livraison du dernier kilomètre
...
[USERS_MESSAGES_WEBHOOK_FAILURE_SHARED](#USERS_MESSAGES_WEBHOOK_FAILURE_SHARED) | Lorsqu'un message webhook est envoyé mais échoue avec une réponse d'erreur provenant de l'endpoint
...
`global_control_group` | `null, boolean` | Avec cette modification, le numéro du compartiment est inclus dans le groupe de contrôle global.
...
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`altitude` | `null, float` | [PII] altitude de l’emplacement enregistré
...
`time` | `int` | Horodatage unix auquel l'événement s'est produit
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`activity_attributes_type` | `null,` `string` | Type d'attribut « Activité en ligne/en production/instantané »
`push_to_start_token` | `null,` `string` | Envoi d'un jeton pour démarrer l'activité en ligne/en production/instantanée
`device_id` | `null,` `string` | ID de l’appareil sur lequel l’événement s’est produit
`sdk_version` | `null,` `string` | Version du SDK de Braze utilisée lors de l'événement
`ios_push_token_apns_gateway` | `null, int` | Passerelle APN du jeton push, applicable uniquement aux jetons push iOS, 1 pour le développement, 2 pour la production.
`push_token_state_change_type` | `null,` `string` | Description du type de modification de l'état du jeton push
`app_group_api_id` | `null,` `string` | ID API du groupe d'applications auquel appartient cet utilisateur
`app_api_id` | `null,` `string` | ID API de l’application sur laquelle cet événement s’est produit
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`time` | `int` | Horodatage unix auquel l'événement s'est produit
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`activity_id` | `null,` `string` | Identifiant d'activité en ligne/en production/instantané
`update_token` | `null,` `string` | Jeton de mise à jour des activités en ligne/en production/instantanées
`device_id` | `null,` `string` | ID de l’appareil sur lequel l’événement s’est produit
`sdk_version` | `null,` `string` | Version du SDK de Braze utilisée lors de l'événement
`ios_push_token_apns_gateway` | `null, int` | Passerelle APN du jeton push, applicable uniquement aux jetons push iOS, 1 pour le développement, 2 pour la production.
`push_token_state_change_type` | `null,` `string` | Description du type de modification de l'état du jeton push
...
`push_token_updated_at` | `null, int` | Horodatage unix à laquelle le jeton push a été mis à jour pour la dernière fois
`push_token_foreground_push_disabled` | `null, boolean` | Indicateur de désactivation de la poussée au premier plan du jeton de poussée
`push_token_device_id` | `null,` `string` | Identifiant de l'appareil du jeton push
`push_token_provisionally_opted_in` | `null, boolean` | Drapeau avec abonnement provisoire pour le jeton push
`ios_push_token_apns_gateway` | `null, int` | Passerelle APN du jeton push, applicable uniquement aux jetons push iOS, 1 pour le développement, 2 pour la production.
`web_push_token_public_key` | `null,` `string` | Clé publique du jeton push, applicable uniquement aux jetons push Web.
`web_push_token_user_auth` | `null,` `string` | Authentification utilisateur du jeton push, applicable uniquement aux jetons push Web.
`web_push_token_vapid_public_key` | `null,` `string` | Clé publique VAPID du jeton push, applicable uniquement aux jetons push Web.
`push_token_state_change_type` | `null,` `string` | Description du type de modification de l'état du jeton push
`app_group_api_id` | `null,` `string` | ID API du groupe d'applications auquel appartient cet utilisateur
`app_api_id` | `null,` `string` | ID API de l’application sur laquelle cet événement s’est produit
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
...
`external_user_id` | `null,` `string` | [PII] ID externe de l'utilisateur
`email_address` | `null,` `string` | Adresse de e-mail personnelle de l'utilisateur
`state_change_source` | `null,` `string` | source du changement d’état (REST, SDK, tableau de bord, etc.)
`subscription_status` | `string` | Statut de l'abonnement : « Utilisateur abonné », « Désabonné » ou « Abonnement »
...
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`channel_identifier` | `null,` `string` | [PII] Identifiant de l'utilisateur sur le canal auquel l'événement est destiné.
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
| `canvas_api_id`           | `string`, `null`    | ID API du Canvas auquel appartient cet événement                           |
| `canvas_variation_api_id` | `string`, `null`    | ID API de la variation de Canvas à laquelle appartient cet événement                 |
| `canvas_step_api_id`      | `string`, `null`    | [Obsolète] ID API de l'étape du canvas à laquelle cet événement est associé         |
| `gender`                  | `string`, `null`    | Sexe de l'utilisateur                                             |
| `country`                 | `string`, `null`    | Pays de l'utilisateur                                            |
| `timezone`                | `string`, `null`    | Fuseau horaire de l'utilisateur                                                 |
| `language`                | `string`, `null`    | Langue de l'utilisateur                                           |
...
| `conversion_behavior_index` | `int`, `null`       | Type d'événement de conversion effectué par l'utilisateur, où "0" correspond à une conversion principale et "1" à une conversion secondaire. |
| `sf_created_at`             | `timestamp`, `null` | Lorsque cet événement a été repris par le Snowpipe                                                                   |
| `experiment_split_api_id` | `string`, `null` | L'ID API de l'expérience à laquelle l'utilisateur est inscrit. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`browser` | `null,` `string` | Navigateur de l'appareil - extrait deuser_agent\- sur lequel l'ouverture s'est produite
`ad_id` | `null,` `string` | [PII] Identifiant publicitaire
`ad_id_type` | `null,` `string` | L'un des ['ios_idfa', 'google_ad_id', 'windows_ad_id', 'roku_ad_id']
`ad_tracking_enabled` | `null, boolean` | Si le suivi publicitaire est activé pour l’appareil ou non
`abort_type` | `null,` `string` | Type d'interruption de grossesse, l'un des ['liquid_abort_message', 'quiet_hours', 'rate_limit']
`abort_log` | `null,` `string` | Message de journal décrivant les détails de l'interruption (jusqu'à 128 caractères)
`banner_placement_id` | `null,` `string` | ID de l'emplacement de la bannière personnalisé par le client
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`language` | `null,` `string` | Langue de l'utilisateur
`abort_type` | `null,` `string` | Type d'interruption de grossesse, l'un des ['liquid_abort_message', 'quiet_hours', 'rate_limit']
`abort_log` | `null,` `string` | Message de journal décrivant les détails de l'interruption (maximum de 2 000 caractères)
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`content_card_id` | `string` | ID de la carte qui a généré cet événement
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`message_extras` | `null,` `string` | [PII] Une chaîne de caractères JSON des paires clé-valeur balisées lors du rendu liquide.
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`timezone` | `null,` `string` | Fuseau horaire de l'utilisateur
`language` | `null,` `string` | Langue de l'utilisateur
`email_address` | `string` | Adresse de e-mail personnelle de l'utilisateur
...
`suspected_bot_click_reason` | `null, object` | Pourquoi cet événement a-t-il été classé comme un bot ?
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`canvas_step_message_variation_api_id` | `null,` `string` | ID API de la variation de message de l'étape de Canvas que l’utilisateur a reçue
`dispatch_id` | `null,` `string` | ID de l’envoi auquel appartient ce message
`email_address` | `null,` `string` | Adresse e-mail de l'utilisateur
`recipient_domain` | `null,` `string` | Domaine d'e-mail du destinataire
`esp` | `null,` `string` | ESP lié à l'événement (Sparkpost, Sendgrid ou Amazon SES)
`from_domain` | `null,` `string` | Domaine d'envoi de l'e-mail
...
`canvas_step_message_variation_api_id` | `null,` `string` | ID API de la variation de message de l'étape de Canvas que l’utilisateur a reçue
`canvas_variation_api_id` | `null,` `string` | ID API de la variation de Canvas à laquelle appartient cet événement
`feature_flag_id_name` | `null,` `string` | Identifiant du déploiement du drapeau de fonctionnalité
`message_variation_api_id` | `null,` `string` | ID API de la variation de message que l’utilisateur a reçue
`external_user_id` | `null,` `string` | [PII] ID externe de l'utilisateur
`device_id` | `null,` `string` | ID de l’appareil sur lequel l’événement s’est produit
`time` | `int` | Horodatage unix auquel l'événement s'est produit
...
`device_id` | `null,` `string` | ID de l’appareil sur lequel l’événement s’est produit
`dispatch_id` | `null,` `string` | ID de l’envoi auquel appartient ce message
`line_channel_id` | `null,` `string` | L'ID du canal LINE auquel le message a été envoyé ou depuis lequel il a été reçu.
`line_channel_name` | `null,` `string` | Nom du canal LINE auquel le message a été envoyé ou depuis lequel il a été reçu
`message_variation_api_id` | `null,` `string` | ID API de la variation de message que l’utilisateur a reçue
`native_line_id` | `null,` `string` | [PII] L'ID Line de l'utilisateur à partir duquel le message a été envoyé ou reçu.
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`subscription_group_api_id` | `string` | ID d’API du groupe d'abonnement
`timezone` | `null,` `string` | Fuseau horaire de l'utilisateur
...
`line_channel_id` | `null,` `string` | L'ID du canal LINE auquel le message a été envoyé ou depuis lequel il a été reçu.
`line_channel_name` | `null,` `string` | Nom du canal LINE auquel le message a été envoyé ou depuis lequel il a été reçu
`media_id` | `null,` `string` | L'ID généré par LINE qui peut être utilisé pour récupérer les médias entrants depuis LINE.
...
`push_to_start_token` | `null,` `string` | Envoi d'un jeton pour démarrer l'activité en ligne/en production/instantanée
`update_token` | `null,` `string` | Jeton de mise à jour des activités en ligne/en production/instantanées
`live_activity_event_type` | `null,` `string` | Type d'événement « Activité en ligne/en production/instantanée ». L'un des éléments suivants : ['début', 'mise à jour', 'fin']
`live_activity_event_outcome` | `null,` `string` | Résultats de l'événement en ligne/en production/instantané
`app_group_api_id` | `null,` `string` | ID API du groupe d'applications auquel appartient cet utilisateur
`app_api_id` | `null,` `string` | ID API de l’application sur laquelle cet événement s’est produit
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`language` | `null,` `string` | Langue de l'utilisateur
`platform` | `null,` `string` | Plate-forme de l'appareil
`ad_id` | `null,` `string` | ID publicitaire [PII] de l'appareil auquel nous avons tenté d'effectuer une opération de réception/distribution.
...
`carrier` | `null,` `string` | Opérateur de l'appareil
`browser` | `null,` `string` | Navigateur de l'appareil
`button_string` | `null,` `string` | Identifiant(button_string)du bouton de notification push sur lequel l'utilisateur a cliqué. Valeur nulle si l'action ne provient pas d'un clic sur un bouton.
`button_action_type` | `null,` `string` | Type d’action du bouton de la notification push. L'un des éléments suivants : [URI,DEEP_LINK,NONE, CLOSE]. Valeur null si l'action n'est pas déclenchée par un clic sur un bouton.
...
`app_group_id` | `null,` `string` | ID BSON du groupe d'applications auquel appartient cet utilisateur
`message_extras` | `null,` `string` | [PII] Une chaîne de caractères JSON des paires clé-valeur balisées lors du rendu liquide.
`is_sampled` | `null,` `string` | Indique si l'envoi push a été échantillonné et s'il était prévu qu'un événement de réception/distribution se produise.
`locale_key` | `null,` `string` | [PII] La clé correspondant aux traductions (par exemple « en-us ») utilisées pour composer ce message (null par défaut).
`sf_created_at` | `timestamp`, `null` | lorsque cet événement a été détecté par le Snowpipe
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
...
`message_variation_name` | `null,` `string` | Nom de la variante du message
`subscription_group_api_id` | `string` | ID d’API du groupe d'abonnement
`message_variation_api_id` | `null,` `string` | ID API de la variation de message que l’utilisateur a reçue
`canvas_variation_api_id` | `null,` `string` | ID API de la variation de Canvas à laquelle appartient cet événement
`canvas_step_api_id` | `null,` `string` | ID API de l'étape de Canvas auquel appartient cet événement
...
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`short_url` | `null,` `string` | URL raccourcie sur laquelle l'utilisateur a cliqué
`suspected_bot_click_reason` | `null,` `string` | Pourquoi cet événement a-t-il été classé comme un bot ?
`user_agent` | `null,` `string` | Agent utilisateur sur lequel le signalement du courrier indésirable a eu lieu
...
`canvas_step_api_id` | `null,` `string` | ID API de l'étape de Canvas auquel appartient cet événement
`canvas_step_message_variation_api_id` | `null,` `string` | ID API de la variation de message de l'étape de Canvas que l’utilisateur a reçue
`interaction_type` | `null,` `string` | Le type d'interaction qui a généré le clic. Exemples de valeurs de chaîne de caractères : URL du texte, Répondre, Ouvrir l'URL
`element_label` | `null,` `string` | Informations facultatives sur l'élément cliqué, telles que le texte d'une réponse suggérée ou d'un bouton
`element_type` | `null,` `string` | Indique si uninteraction_typeélément commun aux suggestions et aux boutons provient d'une suggestion ou d'un bouton. Exemples : Suggestion, Bouton
`campaign_api_id` | `null,` `string` | ID API de la campagne à laquelle appartient cet événement
`url` | `null,` `string` | URL sur laquelle l’utilisateur a cliqué
`subscription_group_api_id` | `string` | ID d’API du groupe d'abonnement
`canvas_variation_name` | `null,` `string` | Nom de la variante canvas reçue par cet utilisateur
...
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`subscription_group_api_id` | `string` | ID d’API du groupe d'abonnement
`to_phone_number` | `null,` `string` | [PII] Numéro de téléphone de l'utilisateur recevant le message aue.164format (par exemple +14155552671)
`message_variation_api_id` | `null,` `string` | ID API de la variation de message que l’utilisateur a reçue
`canvas_variation_api_id` | `null,` `string` | ID API de la variation de Canvas à laquelle appartient cet événement
`canvas_step_api_id` | `null,` `string` | ID API de l'étape de Canvas auquel appartient cet événement
`canvas_step_message_variation_api_id` | `null,` `string` | ID API de la variation de message de l'étape de Canvas que l’utilisateur a reçue
...
`time` | `int` | Horodatage unix auquel l'événement s'est produit
`user_id` | `string` | ID Braze de l'utilisateur ayant effectué cet événement
`action` | `null,` `string` | Mesure prise en réponse à ce message (par exemple, s'abonner, se désabonner ou aucune mesure).
`campaign_name` | `null,` `string` | Nom de la campagne
`canvas_id` | `null,` `string` | ID BSON du canvas auquel cet événement est associé
`canvas_name` | `null,` `string` | Nom du canvas
`canvas_step_name` | `null,` `string` | Nom de l'étape du canvas
...
`from_rcs_sender` | `null,` `string` | L'ID de l'expéditeur RCS ou le nom de l'agent utilisé pour envoyer le message.
`is_sms_fallback` | `null, boolean` | Indique si une solution de repli SMS a été tentée pour ce message RCS rejeté. Il est associé à l'événement « Réception/distribution de SMS ».
`message_variation_name` | `null,` `string` | Nom de la variante du message
...
`canvas_step_name` | `null,` `string` | Nom de l'étape du canvas
`canvas_variation_name` | `null,` `string` | Nom de la variante canvas reçue par cet utilisateur
`category` | `null,` `string` | Nom de la catégorie de mot-clé, uniquement renseigné pour les messages de réponse automatique : « abonnement », « opt-out », « help » ou valeur personnalisée.
`device_id` | `null,` `string` | ID de l’appareil sur lequel l’événement s’est produit
`dispatch_id` | `null,` `string` | ID de l’envoi auquel appartient ce message
`from_rcs_sender` | `null,` `string` | L'ID de l'expéditeur RCS ou le nom de l'agent utilisé pour envoyer le message.
`message_extras` | `null,` `string` | Une chaîne de caractères JSON des paires clé-valeur balisées lors du rendu liquide
`message_variation_name` | `null,` `string` | Nom de la variante du message
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`subscription_group_api_id` | `string` | ID d’API du groupe d'abonnement
`to_phone_number` | `null,` `string` | [PII] Numéro de téléphone de l'utilisateur recevant le message aue.164format (par exemple +14155552671)
...
`timezone` | `null,` `string` | Fuseau horaire de l'utilisateur
`language` | `null,` `string` | Langue de l'utilisateur
`to_phone_number` | `null,` `string` | Numéro de téléphone du destinataire
...
------|------|------------
`id` | `string` | ID unique au niveau mondial pour cet événement
`user_id` | `null,` `string` | ID Braze de l'utilisateur ciblé parshort_url,null si le suivi des clics utilisateurshort_url n'a pas été utilisé
`external_user_id` | `null,` `string` | [PII] ID externe de l'utilisateur ciblé parshort_urlsi celui-ci existe, null sishort_urln'a pas utilisé le suivi des clics des utilisateurs.
`app_group_api_id` | `null,` `string` | ID d'API de l'espace de travail utilisé pour générer short_url
`time` | `int` | Horodatage Unix du moment où la short_url a été cliqué
`timezone` | `null,` `string` | Fuseau horaire de l'utilisateur
`campaign_id` | `null,` `string` | L'ID Braze de la campagne pour laquelleshort_url il a été généré, null s'il ne provient pas d'une campagne.
...
`canvas_id` | `null,` `string` | L'ID Braze du canvasshort_urla été généré pour, null s'il ne provient pas d'un canvas.
`canvas_api_id` | `null,` `string` | L'ID API du canvasshort_urla été généré pour, null s'il ne provient pas d'un canvas.
`canvas_variation_api_id` | `null,` `string` | L'ID API de la variante short_urlcanvas a été généré pour, null s'il ne provient pas d'un canvas.
`canvas_step_api_id` | `null,` `string` | L'ID API de l'étape du canvasshort_url a été généré pour, null si elle ne provient pas d'un canvas.
`canvas_step_message_variation_api_id` | `null,` `string` | ID API de la variante de message de l'étape dushort_url canvas pour laquelle il a été généré, null s'il ne provient pas d'un canvas.
...
`dispatch_id` | `null,` `string` | ID de l’envoi auquel appartient ce message
`external_user_id` | `null,` `string` | [PII] ID externe de l'utilisateur
`host` | `null,` `string` | L'hôte pour la requête
`id` | `string` | ID unique au niveau mondial pour cet événement
`message_variation_api_id` | `null,` `string` | ID API de la variation de message que l’utilisateur a reçue
`raw_response` | `null,` `string` | Réponse brute tronquée provenant de l'endpoint
`retry_count` | `null, int` | Le nombre de tentatives effectuées
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`time` | `int` | Horodatage unix auquel l'événement s'est produit
`url_path` | `null,` `string` | Le chemin d'accès de l'URL demandée
`user_id` | `string` | ID Braze de l'utilisateur ayant effectué cet événement
`webhook_duration` | `null, int` | Durée totale de cette requête en millisecondes
`webhook_failure_source` | `null,` `string` | Pour déterminer si une erreur a été générée par Braze ou par l'endpoint lui-même. Le champ source peut être Point d'endpoint externe, Traiter aucun code d'état comme hôte inaccessible.
...
`sf_created_at` | `timestamp`, `null` | Lorsque cet événement a été repris par le Snowpipe      
`send_id` | `null,` `string` | ID d'envoi de message auquel ce message appartient
`flow_id` | `null,` `string` | ID unique du flux dans WhatsApp gestionnaire. Présent si l'utilisateur répond à un flux WhatsApp.
`template_name` | `null,` `string` | [PII] Nom du modèle dans le gestionnaire WhatsApp. Veuillez indiquer si vous envoyez un message type.
...
`action` | `string` | Mesures prises en réponse à ce message. Par exemple, `Subscribed`, `Unsubscribed`, ou `None`.
`sf_created_at` | `timestamp`, `null` | Lorsque cet événement a été repris par le Snowpipe      
`catalog_id` | `null,` `string` | ID catalogue d'un produit si celui-ci est mentionné dans le message entrant. Sinon, veuillez laisser vide.
...
`message_id` | `null,` `string` | ID unique généré par Meta pour ce message
`in_reply_to` | `null,` `string` | Lemessage_idcontenu du message auquel ce message répondait
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
