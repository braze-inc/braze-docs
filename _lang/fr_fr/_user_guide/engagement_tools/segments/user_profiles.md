...
{% alert important %}
Lorsqu'un numéro de téléphone est utilisé dans la recherche, il est converti au format [`E.164`](https://en.wikipedia.org/wiki/e.164) format. Les utilisateurs dont les numéros de téléphone ne peuvent pas être convertis au format `E.164` (par exemple, parce que le numéro de téléphone comporte un code pays ou un code régional non valide) ne peuvent pas faire l'objet d'une recherche par numéro de téléphone.
{% endalert %}
...
Pour plus d'informations sur ces données, voir la [collecte de données du SDK]({{site.baseurl}}/user_guide/data/unification/user_data/sdk_data_collection/).
...
| Statistiques de communication | Date à laquelle cet utilisateur a reçu pour la dernière fois des messages de votre part dans chaque canal. |
...
![L'onglet "Historique des messages" indique les campagnes et les canvas qu'un utilisateur a reçues.]({% image_buster /assets/img_archive/profiles_messaging_history_tab.png %})
...
| --- | --- |
| E-mail | Rebond<br>Clic<br>Événements de report<br>Réception<br>Marqué comme courrier indésirable<br>Ouvert (voir la [note sur l'événement d’ouverture d’e-mail](#note-on-email-open-event))<br>Envoyer<br>Échec provisoire de livraison<br>Se désabonner |
...
- [Cycle de vie du profil de l'utilisateur]({{site.baseurl}}/user_guide/data/unification/user_data/user_profile_lifecycle/)
