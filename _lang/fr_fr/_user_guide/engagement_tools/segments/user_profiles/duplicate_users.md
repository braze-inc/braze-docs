---
nav_title: Utilisateurs en double
article_title: Utilisateurs en double
description: "Découvrez comment trouver et fusionner les utilisateurs en double dans votre tableau de bord de Braze."
page_order: 0
---

# Utilisateurs en double

> Apprenez à trouver et à fusionner les utilisateurs en double, afin de maximiser l'efficacité de vos campagnes et de vos Canevas.

{% alert tip %}
Pour fusionner des utilisateurs en double à l'aide de l'API REST de Braze, consultez [POST : Fusionner les utilisateurs]({{site.baseurl}}/api/endpoints/user_data/post_users_merge/).
{% endalert %}
...
### Étape 1 : Recherche d'un profil en double

Dans Braze, sélectionnez **Audience** > **Recherche d'utilisateurs.**

!La tuile "Recherche d'utilisateurs" est mise en évidence dans le menu de navigation.]({% image_buster /assets/img/audience_management/duplicate_users/individual_merging/select_search_users.png %}){: style="max-width:60%;"}
...
!Un des profils utilisateurs dupliqués.]({% image_buster /assets/img/audience_management/duplicate_users/individual_merging/select_merge_duplicates.png %}){: style="max-width:50%;"}

Choisissez le profil utilisateur à conserver et celui à fusionner, puis sélectionnez **Fusionner les profils**. Répétez ce processus jusqu'à ce que vous ayez fusionné tous les profils en double.

La page de fusion individuelle d'un profil dupliqué.]({% image_buster /assets/img/audience_management/duplicate_users/individual_merging/select_merge_profiles.png %}){: style="max-width:80%;"}
...
Lorsque vous fusionnez en masse des utilisateurs en double, Braze trouve des profils avec des identifiants correspondants (comme une adresse e-mail) et fusionne toutes leurs données dans le profil le plus récemment mis à jour avec une adresse `external_id`. S'il n'y a pas de profil avec `external_id`, le profil le plus récemment mis à jour sans `external_id` sera utilisé à la place.

### Étape 1 : Allez dans Gestion de l'audience

Dans le tableau de bord de Braze, sélectionnez **Audience** > Gérer l'audience.

La tuile "Gérer l'audience" a été mise en évidence dans le menu de navigation.]({% image_buster /assets/img/audience_management/duplicate_users/bulk_merging/select_manage_audience.png %}){: style="max-width:60%;"}
...
La page "Gérer l'audience" avec l'option "Générer une liste de doublons" en surbrillance.]({% image_buster /assets/img/audience_management/duplicate_users/bulk_merging/select_generate_list.png %})

Braze génère votre aperçu et l'envoie à votre adresse e-mail sous la forme d'un fichier CSV.

Un e-mail de Braze avec un lien vers le fichier CSV généré.]({% image_buster /assets/img/audience_management/duplicate_users/bulk_merging/example_email.png %}){: style="max-width:60%;"}
...
### Étape 3 : Fusionnez vos doublons

Si vous êtes satisfait des résultats de votre aperçu, sélectionnez **Fusionner tous les doublons**.

{% alert warning %}
Les profils utilisateurs dupliqués ne peuvent pas être récupérés après la fusion.
{% endalert %}

La page "Gestion de l'audience" avec l'option "Fusionner tous les doublons" en surbrillance.]({% image_buster /assets/img/audience_management/duplicate_users/bulk_merging/select_merge_profiles.png %}){: style="max-width:70%;"}
...
### Étape 1 : Définissez vos règles

1. Allez dans **Audience** > **Gérer l'audience** > **Modifier les règles**.
...
- **Résolvez les liens en utilisant**: Date de création, Date de mise à jour, Dernière session
...
Si vous êtes satisfait des résultats de votre aperçu, revenez à la page **Gérer l'audience** et sélectionnez **Fusionner tous les doublons**.
...
!La page "Gérer l'audience" avec le bouton "planification".]({% image_buster /assets/img/audience_management/duplicate_users/bulk_merging/select_scheduled_merge_rules.png %})
