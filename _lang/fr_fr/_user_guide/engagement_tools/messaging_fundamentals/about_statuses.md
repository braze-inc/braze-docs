...
![Une liste des canevas dans le tableau de bord de Braze, avec le menu ouvert pour l'un des canevas.]({% image_buster /assets/img/messaging_fundamentals/change-status.png %})
...
| Arrêté | Les campagnes et les toiles arrêtées sont en pause, mais vous pouvez toujours les modifier. Pour reprendre un canvas, veuillez vous rendre à l'étape **Résumé** du générateur de canevas et sélectionner **Reprendre le canvas**. Pour les campagnes, veuillez sélectionner le<i class="fas fa-ellipsis-vertical"></i>menu, puis **Reprendre**. Pour plus d'informations, reportez-vous à la section [Comportement des toiles à l'arrêt.](#stopped-canvas-behavior) |
...
- **Modifications proposées :** Toute modification apportée au canvas sera supprimée lorsque celui-ci sera arrêté.

Pour reprendre le canvas, veuillez vous rendre à l'étape **Résumé** du générateur de canevas et sélectionner **Reprendre le canvas**. Lorsqu'il est réactivé, tous les messages précédemment interrompus sont envoyés conformément à la planification, tant que l'heure prévue n'est pas dépassée.
