...
| Catalogue Braze | Vous devez créer un catalogue Braze qui comprend les champs suivants, que vous utilisez dans la configuration de votre bloc produit :{::nomarkdown}<code><ul><li>product_title</li><li>product_url</li><li>variant_image_url</li></ul></code>{:/} |
| Sélection du catalogue | Pour les blocs de produits statiques, vous devez créer une [sélection de catalogue]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/) pour spécifier les produits à inclure dans votre bloc de produits. |
...
| Statique | Personnalisez les produits à l'aide des données stockées dans un catalogue Braze. Vous devez utiliser une [sélection de catalogue]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/) pour spécifier les produits à inclure. | Parfait pour présenter le lancement de nouveaux produits ou des offres spécifiques à une catégorie.| |
...
Les styles existants pour les paragraphes et les boutons s'appliquent automatiquement aux éléments de texte et de bouton dans le bloc produit. Cela signifie que votre bloc produit utilise de manière cohérente tout formatage que vous avez défini pour les paragraphes et les boutons, ce qui permet de conserver un aspect cohérent dans l'ensemble de votre e-mail.
...
Pour utiliser les blocs de produits glisser-déposer, vous devez configurer un catalogue Braze qui inclut des valeurs de champs spécifiques. Vous utilisez ces champs dans la configuration de votre bloc de produits. Assurez-vous que votre catalogue comprend les champs suivants :
...
![Un exemple de fichier CSV contenant les champs obligatoires ainsi que d'autres.]({% image_buster /assets/img/ecommerce/sample_product_catalog.png %})

#### Mappage aux champs du catalogue

Dans l'onglet **Paramètres** de votre catalogue, vous pouvez basculer les **blocs de produits** vers des champs et des informations spécifiques de votre catalogue. Cela vous permet de sélectionner les champs à utiliser pour le titre du produit, l'URL du produit et l'URL de l'image. Notez que les champs du catalogue Shopify sont mappés par défaut et ne peuvent pas être modifiés.

{% alert note %}
Si vous n'utilisez pas Shopify, vous pouvez contacter votre gestionnaire de compte pour activer le mappage de champs, ce qui vous permet de connecter n'importe quel catalogue à des blocs de produits et de mapper ses champs aux `product_title`, `product_url`, et `variant_image_url`.
{% endalert %}
...
Sélectionnez le catalogue que vous souhaitez utiliser pour votre bloc produit. Vous devez sélectionner un catalogue pour spécifier les produits qui s'affichent dans votre bloc de produits.
...
![Bloc produit avec un côté dollar ajouté au prix de l'article.]({% image_buster /assets/img/product_blocks/liquid.png %}){: style="max-width:45%;"}
...
1. Dans la section **Prévisualisation & Test**, prévisualisez le message en tant qu'utilisateur personnalisé.
...
![L'onglet "Aperçu en tant qu'utilisateur" avec une section déroulante "Bloc de produits dynamiques" qui spécifie d'afficher 4 éléments.]({% image_buster /assets/img/product_blocks/preview_as_a_user.png %}){: style="max-width:40%;"}
...
![Compositeur d'e-mail par glisser-déposer montrant un bloc de produits générés avec différentes tuiles d'articles.]({% image_buster /assets/img/product_blocks/static_block_preview.png %})
