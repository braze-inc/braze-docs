...
- **Placement et taux de bannière :** Chaque placement de bannière peut être utilisé dans un maximum de 25 messages dans un espace de travail.  
...
- **Sessions SDK et création de cartes**: Les cartes de contenu ne sont pas créées pour les utilisateurs qui n'ont pas de session SDK, même si ces utilisateurs répondent aux critères de segmentation. Toutefois, si un utilisateur dispose déjà d'une session Android, les cartes de contenu avec des actions de clic spécifiques à iOS seront toujours créées, et l'utilisateur pourra consulter ces cartes de contenu sur iOS une fois qu'il y aura ouvert une session. Pour plus d'informations sur la création des cartes, reportez-vous à la section [Création des cartes]({{site.baseurl}}/user_guide/message_building_by_channel/content_cards/create/card_creation/).  
