...
La facturation des points de données pour Cloud Data Ingestion équivaut à la facturation des mises à jour via l’[endpoint `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track#user-track). Pour plus d'informations, reportez-vous à la section [Points de données]({{site.baseurl}}/user_guide/data_and_analytics/data_points/). 
