...
> Découvrez les différents événements de Braze et leur raison d'être.

Braze s'appuie sur quelques types d'événements différents pour fournir une compréhension complète du comportement des utilisateurs et de leur engagement envers votre marque. Chaque type d'événement a un objectif unique :
...
Pour en savoir plus, consultez notre article consacré aux [événements d'achat]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/purchase_events/).

{% details Exemple de mise en œuvre %}

Veuillez noter que l’implémentation effective des événements d'achat nécessitera certaines connaissances techniques, car elle implique l'intégration du SDK Braze à votre appli. Votre gestionnaire de la satisfaction client guidera votre équipe tout au long de ce processus dans le cadre de votre onboarding, mais les étapes générales sont les suivantes :

1. **Intégrez le SDK Braze :** Avant d'enregistrer des événements, vous devez intégrer le SDK Braze dans votre application.
...
Pour en savoir plus, consultez notre article consacré aux [événements personnalisés]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/).
...
1. **Intégrez le SDK Braze :** Avant de pouvoir enregistrer des événements, vous devez intégrer le SDK Braze dans votre application.
