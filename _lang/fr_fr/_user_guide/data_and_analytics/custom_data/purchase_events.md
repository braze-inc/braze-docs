...
> Découvrez les événements et propriétés d'achat, leur utilisation, la segmentation, l'endroit où afficher les analyses pertinentes, et plus encore.

Les événements d’achat sont des actions d’achat effectuées par vos utilisateurs. Ces événements permettent d'enregistrer les achats in-app et d'établir la valeur vie (LTV) pour chaque profil utilisateur. Ces événements d’achat doivent être configurés par votre équipe. L'enregistrement des événements d'achat vous permet d'ajouter des propriétés telles que la quantité et le type, ce qui vous aide à mieux cibler vos utilisateurs en fonction de ces propriétés.
...
{% alert tip %}
Si vous souhaitez segmenter le nombre de fois qu'un achat spécifique a été effectué, vous devez enregistrer cet achat individuellement en tant qu'[attribut personnalisé incrémentiel.]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview/#custom-attribute-storage)
{% endalert %}
...
Vous pouvez également personnaliser votre message déclenché avec Liquid.
...
Dans cet exemple, `${purchase_product_name}` est un attribut personnalisé que vous remplacerez par le nom de l'attribut qui stocke le nom du produit acheté dans votre configuration Braze.

### Analyse

En plus du suivi des indicateurs d'achat pour la segmentation, Braze note également le nombre d'achats pour chaque produit et le chiffre d'affaires généré au fil du temps. Cela peut être utile pour savoir quels produits sont les plus populaires ou mesurer l'impact d'une campagne promotionnelle sur les ventes.

Vous pouvez consulter ces données sur la page du [rapport des affaires]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/exporting_revenue_data/#revenue-data).
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include metrics.md metric='Lifetime Revenue' (chiffre d'affaires à vie) %}</td>
        </tr>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include metrics.md metric='Lifetime Value Per User' (valeur vie client par utilisateur) %}</td>
        </tr>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include metrics.md metric='Chiffre d'affaires quotidien moyen' %}</td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics#daily-purchases">Achats quotidiens</a></td>
            <td class="no-split">{% multi_lang_include metrics.md indicateurs="Achats quotidiens %}</td>
        </tr>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include metrics.md metric='Daily Revenue Per User' (Chiffre d'affaires quotidien par utilisateur) %}</td>
        </tr>
    </tbody>
</table>
...
Lorsque vous utilisez des événements d'achat pour suivre les données d'achat, vous devez suivre les remboursements en enregistrant un événement d'achat Braze avec une propriété d'achat négative `price`. Cette approche permet de maintenir un chiffre d'affaires précis pour l'ensemble de la durée de vie.
...
Par exemple, si une application de commerce électronique souhaite envoyer un message à un utilisateur après avoir effectué un achat, elle peut en outre améliorer son audience cible et permettre une personnalisation accrue de la campagne en ajoutant une propriété d'événement d'achat de `brand_name`.
...
- A effectué l'événement personnalisé avec la propriété Y avec la valeur V X fois au cours des Y derniers jours.
...
Vous pouvez tirer parti des `canvas_entry_properties` et des `event_properties` dans les parcours utilisateur de votre Canvas. Pour plus d'informations et d'exemples, consultez les [propriétés d'entrée et les propriétés d'événement de Canvas.]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_entry_properties_event_properties/)
...
Dans le tableau de bord de Braze, vous pouvez gérer la mise en liste de blocage à partir de **Paramètres des données** > **Produits**. Pour en savoir plus, consultez la rubrique [Gérer les données personnalisées]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/managing_custom_data/).
