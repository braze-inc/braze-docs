...
> Cet article décrit les événements/ propriétés personnalisés, les filtres de segmentation associés, les propriétés d'entrée dans Canvas, les analyses pertinentes, etc. Pour en savoir plus sur les événements de Braze en général, consultez la rubrique [Événements.]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/events)
...
- Trouver des analyses/analytiques supplémentaires en utilisant les rapports d'[entonnoir]({{site.baseurl}}/user_guide/data_and_analytics/reporting/funnel_reports/#step-2-select-events-for-funnel-steps) et de [rétention]({{site.baseurl}}/user_guide/data_and_analytics/reporting/retention_reports/).
- Exploitation des [propriétés d'entrées persistantes]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_persistent_entry_properties/) pour utiliser les métadonnées de votre événement client à des fins de personnalisation dans vos étapes du canvas.
- Générer des analyses/analytiques plus sophistiquées avec [Currents (si]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents) utilisé comme adjectif)
...
Vous pouvez mettre en liste de blocage des événements personnalisés individuels via le menu d'actions, ou sélectionner et mettre en liste de blocage jusqu'à 10 événements en bloc. 
...
{% alert note %}
Si vous souhaitez créer des segments basés sur la récurrence et la fréquence des propriétés d'événement, contactez votre gestionnaire de satisfaction client pour activer la segmentation pour des propriétés d'événement personnalisées spécifiques. Lorsque cette option est activée, vous pouvez accéder à des options de filtrage supplémentaires lors de la segmentation.
{% endalert %}
...
Pour en savoir plus, consultez notre page dédiée aux [objets imbriqués]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/nested_objects/).
