...
{% alert important %}
Les recommandations basées sur des règles sont fondées sur une logique fixe que vous devez définir manuellement. Cela signifie que vos recommandations ne s'ajusteront pas à l'historique d'achat et aux goûts d'un utilisateur, à moins que vous ne mettiez à jour la logique.<br><br>Pour créer des recommandations [d']({{site.baseurl}}/user_guide/brazeai/recommendations/creating_recommendations/ai/)intelligence artificielle personnalisées qui s'adaptent automatiquement à l'historique de l'utilisateur, consultez les [recommandations d'articles d'intelligence artificielle.]({{site.baseurl}}/user_guide/brazeai/recommendations/creating_recommendations/ai/)
{% endalert %}
...
<table style="text-align: center;">
  ...
  <tbody>
    <tr>
      ...
      <td>✔</td>
      <td>Oui, si les recommandations <strong>ne</strong> sont <strong>pas</strong> mises à jour fréquemment.</td>
      <td>✔</td>
      <td>✔</td>
    </tr>
    <tr>
      <td><strong>Catalogue API</strong></td>
      <td>✔</td>
      <td></td>
      <td>✔</td>
...
4. Obtenez la valeur `product_recommendations` pour cet article de catalogue et divisez-la par le délimiteur à l'aide d'un filtre de division liquide.
...
|-----|-----------|
| **ID** | Un numéro unique en corrélation avec le nombre de jours écoulés depuis l'inscription de l'utilisateur à votre appli. Par exemple, `3` correspond à trois jours. |
| **type** | La catégorie de recette, telle que `comfort`, `fresh`, et autres. |
| **titre** | Le titre de la carte de contenu qui sera envoyée pour chaque ID, par exemple "Préparez le déjeuner de cette semaine" ou "Taco, parlons-en". |
| **lien** | Le lien vers l'article de la recette. |
| **image_url** | L'image qui correspond à la recette. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

Une fois le catalogue téléchargé dans Braze, vérifiez l'aperçu d'un certain nombre d'articles du catalogue pour confirmer que les informations importées sont exactes. Les éléments peuvent être aléatoires dans l'aperçu, mais cela n'affectera pas le résultat du moteur de recommandation.
...
\![Un exemple d'envoi de messages composé à partir d'une campagne de cartes contenu.]({% image_buster /assets/img/recs/content_card_preview.png %})
...
{% alert note %}
Il s'agit d'un exemple de base que vous devrez peut-être modifier en fonction de vos besoins spécifiques et de la structure de vos données. Pour des conseils plus détaillés, reportez-vous à la [documentation Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/) ou consultez un développeur.
{% endalert %}

### Exemple

Supposons que vous souhaitiez extraire des recommandations de restaurants de la base de données Zomato Restaurants et enregistrer le résultat dans une variable locale appelée `restaurants`. Vous pouvez effectuer les appels de contenu connecté suivants :
...
1. Utilisez `assign` pour créer des tableaux vierges pour les catégories d'évaluation "excellent", "très bon" et "bon".
...
- Si l'évaluation est "Très bon", ajoutez le nom du restaurant à la chaîne de caractères `very_good_restaurants`, puis ajoutez un caractère * à la fin.
- Si l'évaluation est "Bonne", ajoutez le nom du restaurant à la chaîne de caractères `good_restaurants`, puis ajoutez un caractère * à la fin.
...
Rendement d'une liste de restaurants généré par l'exemple de l'appel final.]({% image_buster /assets/img/recs/sample_response.png %}){: style="max-width:30%;"}
{% endtab %}
{% endtabs %}
