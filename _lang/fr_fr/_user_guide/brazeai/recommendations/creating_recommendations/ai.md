...
article_title: "Créer des recommandations d'articles d'intelligence artificielle"
...
# Créer des recommandations d'articles d'intelligence artificielle
...
!["Définissez votre recommandation" étape avec "Intelligence artificielle personnalisée" comme type et l'option "Ne pas recommander des éléments avec lesquels les utilisateurs ont déjà interagi" sélectionnée.]({% image_buster /assets/img/item_recs_2-3.png %})
...
{% alert note %}
Les événements personnalisés doivent comporter suffisamment de données pour apparaître dans la liste des événements. Si votre événement personnalisé n'apparaît pas, c'est peut-être parce que le backend de Braze ne l'a pas encore traité ou qu'il manque de données pour l'entraînement du modèle. Les recommandations de l'intelligence artificielle s'appuient sur les données historiques pour générer des informations, de sorte que les événements nouvellement créés ou rarement déclenchés ne seront pas disponibles tant que davantage de données n'auront pas été collectées.
{% endalert %}
...
- **Si vous utilisez des [événements de commerce électronique]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events/) pour former des recommandations d'articles :** Ajoutez `products.product_id` pour accéder à l'ID du produit à partir des événements.
