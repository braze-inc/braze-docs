...
description: "Cet article explique comment utiliser les recommandations d'articles dans vos messages."
page_order: 1.2
---
...
Basculez sur **Image with Liquid**, puis ajoutez ce qui suit dans le champ **Dynamic URL** (le champ URL ne prend pas en charge les retours à la ligne, assurez-vous donc que le code apparaît sur une seule ligne) :
...
{% tab HTML %}
Pour les références d'images en HTML, définissez l'attribut `src` de l'image sur le champ URL de l'image dans le catalogue. Vous pouvez utiliser un autre champ, tel que le nom ou la description d'un produit, comme texte alternatif.
