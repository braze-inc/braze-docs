...
Dans le cadre de la fonctionnalité [de recommandations d'articles par intelligence artificielle]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/), les recommandations de personnalisation par intelligence artificielle utilisent l'apprentissage profond pour prédire ce qui est susceptible d'intéresser vos utilisateurs à l'avenir, en fonction de ce qui les a intéressés par le passé. Cette méthode fournit un système de recommandation dynamique et personnalisé qui s'adapte au comportement de l'utilisateur.

Les recommandations personnalisées de l'intelligence artificielle utilisent les 6 derniers mois de données d'interaction avec les articles, comme les achats ou les événements personnalisés, pour créer le modèle de recommandation. Pour les utilisateurs qui ne disposent pas de suffisamment de données pour réaliser la personnalisation de la liste, les articles les plus populaires servent de solution de repli, afin que vos utilisateurs continuent de recevoir des suggestions pertinentes.
...
- Une méthode pour suivre les achats, qu'il s'agisse d'un objet acheté ou d'un événement personnalisé.
{% enddetails %}

{% details Setting it up %}
1. Créez une [recommandation d'article d'intelligence artificielle]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/).
...
7. [Utilisez la recommandation dans les messages.]({{site.baseurl}}/user_guide/brazeai/item_recommendations/using_recommendations/)
{% enddetails %}
{% endtab %}
{% endtabs %}
...
La fonctionnalité de recommandation « Tendances » présente les articles qui ont connu la plus forte dynamique positive dans les interactions récentes des utilisateurs. Nous calculons cela à l'aide d'une analyse pondérée d'environ 10 semaines d'historique des événements, la pondération la plus importante étant appliquée aux deux dernières semaines approximatives. Afin d'éviter que de petites fluctuations n'affectent la qualité des recommandations, nous appliquons un seuil d'activité et des techniques de lissage statistique.
...
6. Former la recommandation.
7. [Utilisez la recommandation dans les communications.]({{site.baseurl}}/user_guide/brazeai/item_recommendations/using_recommendations)
{% enddetails %}
{% endtab %}
...
{% alert note %}
Si vous utilisez des sélections, le champ de tri et les limites ne seront pas pris en compte dans les recommandations d'articles par l'intelligence artificielle. Cela signifie que si vous créez une sélection avec un champ de tri spécifique et limitez le nombre d'éléments renvoyés, ces contraintes ne seront pas utilisées lors du traitement des recommandations d'éléments par l'intelligence artificielle.
{% endalert %}
...
Veuillez noter que les recommandations fonctionnent de manière indépendante et n'ont aucune connaissance des recommandations des autres modèles. Cela signifie que chaque section peut contenir des éléments déjà affichés dans d'autres sections de recommandations d'intelligence artificielle du même e-mail.
...
### Comment puis-je faire en sorte que toutes les recommandations valables pendant plusieurs jours expirent en même temps ?

Si vous souhaitez faire expirer toutes les recommandations sur plusieurs jours à une date précise (afin que toutes les recommandations actives reçoivent simultanément de nouvelles prédictions), veuillez contacter le service d'assistance Braze ou votre gestionnaire de la satisfaction client pour obtenir de l'aide. Les experts de Braze en intelligence artificielle effectuent cette opération manuellement afin de garantir des performances optimales du modèle.
