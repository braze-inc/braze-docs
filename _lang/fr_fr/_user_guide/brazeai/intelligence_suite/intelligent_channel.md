...
Pour que Braze détermine quel canal est « le meilleur », il doit posséder assez de données. Cela signifie qu'un utilisateur doit avoir reçu au moins trois messages ou plus par canal sur au moins deux des trois canaux disponibles. Les messages n’ont pas nécessairement besoin d’avoir été ouverts. 
...
Plutôt que de laisser Braze sélectionner le canal le plus approprié pour un utilisateur, vous pouvez utiliser le [filtre de segmentation « Probabilité d'ouverture du message »]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#message-open-likelihood) pour filtrer les utilisateurs en fonction de leur probabilité d'ouvrir un message sur un canal spécifique que vous avez choisi. Ce filtre est calculé en divisant le pourcentage d'interactions par le nombre total de messages reçus pour les 100 derniers messages envoyés par canal.

Veuillez noter qu'un utilisateur doit avoir reçu au moins trois messages sur un canal spécifique avant de pouvoir obtenir un score de probabilité pour ce canal. Les utilisateurs sans données suffisantes pour mesurer une probabilité pour une chaîne peuvent être sélectionnés en utilisant "est vide."
