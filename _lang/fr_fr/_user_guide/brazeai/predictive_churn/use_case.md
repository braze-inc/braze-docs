...
![La définition du désabonnement montre que le désabonnement est défini comme un utilisateur qui n'a pas effectué d'événement personnalisé "stream_started" au cours des 14 derniers jours.]({% image_buster /assets/img/ai_use_cases/churn_definition.png %})
...
Pour déterminer un seuil de départ pour le ciblage, Camila utilise le curseur d'audience de prédiction pour prévisualiser combien d'utilisateurs se situent dans chaque plage de score et quelle est la précision de la prédiction à ce niveau. Elle établit un équilibre entre la couverture et la précision en fonction des résultats positifs escomptés. Sur cette base, elle décide de cibler les scores de risque supérieurs à 70. 
...
![Segmentation du filtrage pour les utilisateurs désabonnés dont le score de risque de désabonnement est supérieur à 70.]({% image_buster /assets/img/ai_use_cases/churn_risk_score.png %})
...
- **Corps :** Vous n'avez pas joué depuis longtemps ? Ne vous inquiétez pas, nous avons fait quelques choix rien que pour vous. Des thrillers aux documentaires primés, il y a quelque chose qui porte votre nom.
...
Sur la base de ces informations, Camila prévoit de faire des tests A/B sur les lignes d'objet, de tester différentes fenêtres temporelles et d'expérimenter des formats de contenu tels que des recommandations de type carrousel dans un message in-app.
