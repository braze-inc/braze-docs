...
![Le site de Braze Documentation affichant l'interface japonaise]({% image_buster /assets/img/braze-docs-japan.png %}){: style="max-width:70%;"}
...
Nous avons ajouté la prise en charge des [propriétés WhatsApp et SMS Liquid dans le flux Canvas.]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_entry_properties_event_properties) Désormais, lorsqu'une étape du parcours action contient un déclencheur "Envoi d'un message SMS entrant" ou "Envoi d'un message WhatsApp entrant", les étapes du canvas suivantes peuvent inclure une propriété SMS ou WhatsApp Liquid. Cela reflète le fonctionnement des propriétés d'événement dans Canvas. Vous pouvez ainsi tirer parti de vos messages pour enregistrer et référencer des données first-party sur les profils utilisateurs et les envois de messages conversationnels.
