...
Nous avons exposé de nouveaux endpoints d'API, /email/hard_bounces, qui vous permet d'extraire les échecs définitifs par adresse e-mail ou dans une plage de dates donnée, et /messages/scheduled_broadcasts, qui vous permet d'extraire la prochaine heure de début des campagnes planifiées et des toiles à entrée planifiée. Ces nouveaux endpoints vous permettent de personnaliser et d’optimiser davantage vos campagnes. En savoir plus sur nos [endpoints API]({{site.baseurl}}/developer_guide/rest_api/basics/#what-is-a-rest-api).
