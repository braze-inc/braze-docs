...
- Les champs `custom_events`, `purchases`, `campaigns_received`, et `canvases_received` ne contiendront que les données des 90 derniers jours.
...
Si vous voyez `object_prefix` dans votre réponse API et aucune URL pour télécharger les données, cela signifie que vous avez un compartiment Amazon S3 déjà configuré pour cet endpoint. Toutes les données exportées à l'aide de cet endpoint iront directement dans votre compartiment S3.
