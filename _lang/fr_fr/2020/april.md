...
Disponible pour tous les clusters, le nouveau processus d'exportation d'audience Facebook de Braze simplifie le flux de travail grâce à des étapes d'intégration claires. Il n'est plus nécessaire d'ajouter les URI de redirection OAuth à la liste blanche pour envoyer des audiences personnalisées ou de modifier les paramètres de l'application Facebook pour l'intégration.
...
Le [modèle/e-mail/liste]({{site.baseurl}}/api/endpoints/templates/email_templates/get_list_email_templates/) et les [content_block/list]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks/)endpoints API ont été mis à jour afin d'inclure un nouveau`tags`champ. Ce champ s’affichera sous forme de tableau, toutes les balises qui s’appliquent au modèle de bloc ou d’e-mail actuel.
