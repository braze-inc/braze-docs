...
![Détails de l’indicateur de fonctionnalité pour un exemple nommé Widget d’assistance en direct. L'ID estenable_live_chat.  Cette description de la fonctionnalité indique que le widget de chat en ligne s'affichera sur la page d'assistance.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-livechat-1.png %})
...
![Indicateur de fonctionnalité portant le nom de Programme de fidélité. L'ID estshow_loyalty_program,et la description indique que le nouveau programme de fidélité s'affiche sur l'écran d'accueil et la page de profil.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-loyalty.png %})

Ensuite, dans Canvas, nous allons créer une [étape Feature Flag]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/feature_flags/) qui active le drapeau`show_loyalty_program` de fonctionnalité pour notre segment « Clients à forte valeur ajoutée » :

![Exemple de Canvas avec une étape de segmentation de l'audience où le segment des clients à forte valeur active le drapeaushow_loyalty_program de fonctionnalité.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-canvas-flow.png %})
...
Désormais, 50 % des utilisateurs verront l'ancienne expérience, tandis que les 50 % restants verront la nouvelle expérience. Nous pouvons ensuite analyser les deux variantes afin de déterminer quel processus de paiement a généré le taux de conversion le plus élevé. {% multi_lang_include analytics/metrics.md metric='Conversion Rate' %}
