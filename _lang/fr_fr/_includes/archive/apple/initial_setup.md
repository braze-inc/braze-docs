...
Le SDK Braze pour {{include.platform}} doit être installé ou mis à jour à l’aide de [CocoaPods](http://cocoapods.org/), un gestionnaire de dépendances pour les projets Objective-C et Swift. CocoaPods offre une simplicité supplémentaire pour l’intégration et la mise à jour.

## {{include.platform}} Intégration SDK CocoaPods

### Étape 1 : Installer CocoaPods

L'installation du SDK via le site {{include.platform}} [CocoaPods](http://cocoapods.org/) automatise la majeure partie du processus d'installation pour vous. Avant de commencer ce processus, vérifiez que vous utilisez la [version 2.0.0 de Ruby](https://www.ruby-lang.org/en/installation/) ou une version ultérieure. Notez qu'il n'est pas nécessaire de connaître la syntaxe de Ruby pour installer ce SDK.
...
**Remarque** : Si vous êtes invité à remplacer l’exécutable `rake`, reportez-vous à la rubrique [Instructions de démarrage sur CocoaPods.org](http://guides.cocoapods.org/using/getting-started.html) pour plus de détails.

**Remarque** : Si vous avez des problèmes concernant CocoaPods, veuillez consulter le [guide de résolution des problèmes de CocoaPods](http://guides.cocoapods.org/using/troubleshooting.html).
...
![Nouvel espace de travail]({% image_buster /assets/img_archive/podsworkspace.png %})
