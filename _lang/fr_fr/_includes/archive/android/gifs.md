...
Le SDK de Braze utilisera tout chargeur d'images personnalisé défini via [`setBrazeImageLoader`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.images/-i-braze-image-loader/index.html). Nous vous recommandons de définir le chargeur d'images personnalisé dans une sous-classe personnalisée de votre application :
...
Pour personnaliser le chargement des images avec Jetpack Compose, vous pouvez passer une valeur à `imageComposable`. Cette fonction reçoit une `Card` et effectue le rendu de l'image avec les modificateurs nécessaires. Vous pouvez également utiliser `customCardComposer` de `ContentCardsList` pour personnaliser le rendu de la carte entière.
