...
Nous avons mis à disposition de nouveaux points de terminaison API qui/email/hard_bounces, vous permettent d'extraire les échecs d'envoi définitifs par adresse e-mail ou pour une période donnée, et/messages/scheduled_broadcasts,qui vous permettent d'extraire la prochaine date de début des campagnes de planification et des canevas à entrée planifiée. Ces nouveaux endpoints vous permettent de personnaliser et d’optimiser davantage vos campagnes. En savoir plus sur nos [endpoints API]({{site.baseurl}}/developer_guide/rest_api/basics/#what-is-a-rest-api).
