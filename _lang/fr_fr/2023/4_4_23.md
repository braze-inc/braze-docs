...
![Un fil d’Ariane de navigation depuis User Guide > Message Building by Channel > in-app Messages > Templates > Simple Survey (Guide de l’utilisateur > Création de messages par canal > Messages in-app > Modèles > Enquête simple)]({% image_buster /assets/img/doc-breadcrumbs.png %}){: style="max-width:55%"}
...
{% alert important %}
L’éditeur SQL est en accès anticipé. Si vous souhaitez participer à l'accès anticipé, veuillez contacter votre gestionnaire de la satisfaction client.
{% endalert %}
...
[Shopify Historical Backfill]({{site.baseurl}}/partners/ecommerce/shopify_legacy/shopify_features/shopify_backfill) permet à Braze d'importer tous les clients, commandes et événements d'achat des 90 derniers jours précédant votre connexion à l'intégration Shopify.
