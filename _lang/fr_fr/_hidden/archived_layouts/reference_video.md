...
>  Ce type d’article permet d’expliquer un concept et regroupe des informations spécifiques sur des processus techniques et le contenu de certains produits (étapes de Canvas, segmentation, un type d’objet donné, etc.). Le second type de modèle de référence est celui du glossaire. Ce format n’est pas utilisé pour notre glossaire dédié aux API ou notre documentation de référence, à moins qu’un concept spécifique doive être expliqué. Précisez qu’ils apprendront [ceci](#what-is-x-concept), [cela](#topic-1-regarding-this-concept) et [plus encore](#topic-2-regarding-this-concept) sur cette page. Ceci est un [bon exemple d'un document de référence](https://guide.meteor.com/code-style.html). Ceci est un bon exemple d'un [document de référence très technique](https://www.w3schools.com/html/html_intro.asp).
>
...
Ce sujet doit expliquer un aspect spécifique du concept, par exemple, comment un certain type d'étape de Canvas est utilisé avec un canal. Cliquez sur le [lien vers ce tutoriel]() pour découvrir comment cela peut être fait en situation réelle.
