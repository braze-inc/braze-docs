---
nav_title: Cartes de contenu personnalisées
...
# Cartes de contenu personnalisées
...
Vous pouvez également afficher les cartes de contenu de manière totalement personnalisée en créant votre propre interface utilisateur de présentation alimentée par les données des modèles Braze. Analysez les objets des cartes de contenu et extrayez-en les données utiles. Ensuite, utilisez les données du modèle résultant pour alimenter votre interface utilisateur personnalisée - la phase d'exécution de l'[approche "crawl, walk, run" (ramper, marcher, exécuter).]({{site.baseurl}}/developer_guide/getting_started/customization_overview)
...
Dans le SDK Android et FireOS, la logique du centre de messages est pilotée par la valeur `class_type` qui est fournie par les paires clé-valeur de Braze. En utilisant la méthode [`createContentCardable`]({{site.baseurl}}/developer_guide/platforms/android/content_cards/examples/), vous pouvez filtrer et identifier ces types de classes.
