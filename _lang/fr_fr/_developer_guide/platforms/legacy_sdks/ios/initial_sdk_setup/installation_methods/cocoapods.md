...
Si vous avez des problèmes concernant CocoaPods, reportez-vous au [guide de résolution des problèmes de](http://guides.cocoapods.org/using/troubleshooting.html) CocoaPods.

{% alert note %}
Si vous êtes invité à écraser l'exécutable `rake`, reportez-vous aux instructions de [démarrage](http://guides.cocoapods.org/using/getting-started.html) sur CocoaPods.org pour plus de détails.
{% endalert %}
...
![Un dossier d'exemple d'Appboy agrandi pour montrer le nouveau \`AppbpyExample.workspace\`.]({% image_buster /assets/img_archive/podsworkspace.png %})
