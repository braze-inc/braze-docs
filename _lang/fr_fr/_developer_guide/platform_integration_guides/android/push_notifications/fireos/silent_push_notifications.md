...
Des notifications silencieuses sont disponibles via [l’API d’envoi de messages]({{site.baseurl}}/api/endpoints/messaging/) de Braze. Pour les utiliser, vous devez définir le drapeau `send_to_sync` sur `true` dans l'[objet push d'Android]({{site.baseurl}}/api/objects_filters/messaging/android_object/) et vous assurer qu'aucun champ `title` ou `alert` n'est défini, car l'utilisation de ce drapeau avec `send_to_sync` entraînera des erreurs. Vous pouvez toutefois inclure les données `extras` dans l’objet.
