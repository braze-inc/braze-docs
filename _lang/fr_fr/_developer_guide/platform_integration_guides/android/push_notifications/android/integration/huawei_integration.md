...
> Les téléphones récents fabriqués par [Huawei](https://huaweimobileservices.com/) sont équipés des services mobiles Huawei (HMS), un service utilisé pour envoyer des notifications push au lieu de recourir à Firebase Cloud Messaging (FCM) de Google.<br><br>Ce guide vous montrera comment configurer votre intégration Huawei pour Android pour envoyer des notifications push via Braze et profiter de toutes les fonctionnalités existantes de Braze, y compris la segmentation, l’analyse, Canvas, et bien plus encore !
...
Une fois votre campagne lancée, vous verrez l’analyse de votre campagne ou agrégée par Canvas pour les notifications push Android. Consultez notre [guide de l'utilisateur push]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_reporting/) pour plus d'informations sur l'analyse/analytique push Android (si utilisé en tant qu'adjectif).
