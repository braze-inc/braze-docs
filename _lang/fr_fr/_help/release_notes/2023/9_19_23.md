...
[L'archivage des messages]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/message_archiving/) vous permet de sauvegarder une copie des messages envoyés aux utilisateurs à des fins d'archivage ou de conformité dans votre bucket S3. Les champs suivants ont été ajoutés à la charge utile JSON livrée à votre bucket S3 chaque fois qu'un message est envoyé :
...
- [SDK Android 28.0.0](https://github.com/braze-inc/braze-android-sdk/blob/master/CHANGELOG.md#2800)
    ...
    - Les fonctions des drapeaux de fonctionnalité ont été modifiées.
...
- [SDK Roku 2.0.0](https://github.com/braze-inc/braze-roku-sdk/blob/main/CHANGELOG.md#200)
    - `getFeatureFlag` renverra invalide lorsque le drapeau n'existe pas.
    - `BrazeTask` observe maintenant `BrazeFeatureFlagsUpdated` pour savoir quand les actualisations des drapeaux de fonctionnalité réussissent ou échouent. Les valeurs de données peuvent ne pas toujours être différentes.
