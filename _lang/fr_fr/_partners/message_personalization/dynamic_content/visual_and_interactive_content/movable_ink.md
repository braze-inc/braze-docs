...
|| Vidéo | ✔* | ✔* | \* Uniquement les GIFs animés, <br>Pour Android, Braze exige la [prise en charge du format GIF]({{site.baseurl}}/developer_guide/platform_integration_guides/android/in-app_messaging/customization/gifs/) dans l'implémentation |
...
Pour plus d'informations sur l'utilisation de la plate-forme Movable Ink, visitez le [centre d'assistance Movable Ink](https://support.movableink.com/).
...
- **Messages in-app et cartes de contenu**: Veillez à ce que l'URL de l'image soit unique pour chaque impression. Pour ce faire, il suffit d'ajouter la balise Liquid appropriée pour différencier toutes les URL. Voir les [instructions relatives aux messages in-app et aux cartes de contenu]({{site.baseurl}}/partners/message_personalization/dynamic_content/visual_and_interactive_content/movable_ink/#step-4-braze-experience). 
...
- Android exige la prise en charge du format GIF dans la mise en œuvre. Suivez l'article sur [la personnalisation des messages in-app d']({{site.baseurl}}/developer_guide/platform_integration_guides/android/in-app_messaging/customization/gifs/) Android si vous n'avez pas cette configuration.
