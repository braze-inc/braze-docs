...
* Les données sont prises en compte toutes les huit heures, ce qui signifie que plusieurs mises à jour dans cette fenêtre sont combinées en un seul enregistrement. Les modifications individuelles au cours de cette période ne sont pas conservées séparément.
...
* Les requêtes sur `USER_LATEST_STATE_DEFAULT_ATTRIBUTES_VIEW_SHARED` pour un seul utilisateur donnent des résultats en moins d'une minute, mais elles ne sont pas très efficaces sans le filtrage de `USER_ID`.
