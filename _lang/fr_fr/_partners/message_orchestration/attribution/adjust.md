...
Si vous disposez d'une application Android, vous devez transmettre un ID unique de l'appareil Braze à Adjust. Cet ID peut être défini dans la méthode `addGlobalPartnerParameter()` du SDK Adjust. L’extrait de code suivant doit être inclus avant d'initialiser le SDK sur `Adjust.initSdk.`
