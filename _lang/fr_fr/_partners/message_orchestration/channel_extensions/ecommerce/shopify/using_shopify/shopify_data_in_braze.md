...
Vous pouvez filtrer les événements de Shopify avec tous les [filtres personnalisés existants]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/) dans la segmentation. 
...
De plus, vous pouvez également utiliser le filtre de largeur d'achat de Braze pour créer des segments d'utilisateurs basés sur :
...
Avec la prise en charge des objets imbriqués pour les propriétés d'événement personnalisé, les clients peuvent désormais déclencher des campagnes et des Canvases en utilisant une propriété d'événement imbriquée. L'exemple suivant montre comment déclencher une campagne en utilisant un produit spécifique de l’événement personnalisé '`shopify_created_order`. Assurez-vous d'utiliser `list_items[].product_id` pour indexer votre liste d'articles et accéder à l'ID du produit.
