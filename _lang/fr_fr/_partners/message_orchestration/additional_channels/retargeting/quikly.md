...
> [Quikly][1], plateforme de marketing d'urgence, s'appuie sur la psychologie pour motiver les consommateurs, afin que les marques puissent immédiatement augmenter la réponse autour de leurs initiatives marketing clés.
...
| ----------- | ----------- |
| Compte Quikly | Un compte partenaire de la marque [Quikly][1] est nécessaire pour bénéficier de ce partenariat. |
...
Par exemple:
 - Créez de l'anticipation et de l'engagement au fil des jours, les consommateurs s'abonnant pour avoir une chance de remporter des récompenses excitantes avec [Quikly Hype.][3] Les données first-party sont automatiquement transmises à Braze.
 - Accélérez l'acquisition de nouveaux abonnés par e-mail et par SMS en utilisant des offres uniques en temps réel basées sur la rapidité de réponse du consommateur, son classement par rapport aux autres, au hasard, ou avant que le temps ou les quantités ne soient épuisés avec [Quikly Swap.][4]
...
2. Enregistrez ou mettez à jour l'utilisateur.
  ...
  - Si l'utilisateur n'existe pas :
    - Quikly crée un profil d'alias uniquement via l' [endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) Braze [`/users/track`, en définissant l'e-mail de l'utilisateur comme alias de l'utilisateur pour le référencer à l'avenir (car l'utilisateur n'aura pas d'ID externe).]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)
...
2. Quikly effectue une recherche d'utilisateur à l'aide de l'[endpoint Exporter le profil utilisateur par identifiant]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier/) pour voir s'il existe un profil utilisateur avec un `email_address` donné. Si aucun utilisateur n'existe, créez un profil d'alias uniquement via l' [endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) Braze [`/users/track`, en définissant l'e-mail de l'utilisateur comme alias de l'utilisateur pour faire référence à cet utilisateur à l'avenir (car l'utilisateur n'aura pas d'ID externe).]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)
