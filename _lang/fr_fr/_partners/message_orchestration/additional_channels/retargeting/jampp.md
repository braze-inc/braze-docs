...
| Collecte de l'ID publicitaire de Google via un attribut personnalisé | La collecte de l'ID publicitaire de Google est facultative pour les clients et peut être collectée sous la forme d'un [attribut personnalisé][5].
...
- Insérez [Liquid][1] pour l'attribut personnalisé approprié que vous suivez en tant qu'ID de publicité Google. Notez que l'ID publicitaire de Google est indiqué comme `aaid` dans cet exemple, mais vous devrez le remplacer par le nom de l'attribut personnalisé défini par vos développeurs.

![L'URL du webhook et l'envoi du message s'affichent dans le générateur de webhooks de Braze.][2]
...
![Les en-têtes de la requête, la méthode HTTP et l'envoi du message sont affichés dans le générateur de webhooks Braze.][3]
