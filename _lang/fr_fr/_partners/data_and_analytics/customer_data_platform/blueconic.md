...
> [BlueConic](https://www.blueconic.com/), la plateforme spécialisée dans l’exploitation des données clients, extrait des données first-party de systèmes disparates et les rend accessibles au moment et à l’endroit où les entreprises en ont besoin pour transformer leurs relations avec les clients et stimuler leur croissance. 
...
| --- | --- |
| Compte BlueConic | Un [compte BlueConic](https://www.blueconic.com/) est nécessaire pour profiter de ce partenariat. Vous aurez besoin d'un accès pour [voir et modifier les connexions](https://support.blueconic.com/hc/en-us/articles/202607121-BlueConic-Roles) dans votre compte BlueConic pour accéder aux plugins. |
...
2. Sélectionnez l'emplacement des données dans Braze. Ici, vous pouvez indiquer à BlueConic où trouver les données à importer en sélectionnant votre audience Braze.<br>![L'audience de BlueConic Braze est définie comme "BlueConic utilisateurs test".]({% image_buster /assets/img/blueconic/braze4.png %}){: style="max-width:80%;"}<br><br>
3. Ensuite, mappez les identifiants entre Braze et BlueConic. <br>![Le champ "ID externe" de Braze est mappé sur le champ "ID externe de Braze" de BlueConic.]({% image_buster /assets/img/blueconic/braze5.png %}){: style="max-width:80%;"}<br><br> Pour relier les données des clients entre les deux systèmes, saisissez un ou plusieurs identifiants de client.<br>Utilisez la case à cocher **Autoriser la création...** pour permettre à BlueConic de créer de nouveaux profils pour les données qui ne correspondent pas à un profil BlueConic existant.<br><br>
...
2. Choisissez un segment BlueConic pour l'exportation. Seuls les profils de ce segment dont les identifiants correspondent dans Braze seront exportés.<br>![Un segment BlueConic de 20k profils.]({% image_buster /assets/img/blueconic/braze8.png %}){: style="max-width:80%;"}<br><br>
3. Ensuite, reliez les identifiants entre les profils BlueConic et les champs Braze. Vous pouvez choisir de laisser BlueConic créer de nouveaux enregistrements si aucune correspondance n'est trouvée.<br>![Le champ "ID externe" de Braze est mappé sur le champ "ID externe de Braze" de BlueConic.]({% image_buster /assets/img/blueconic/braze7.png %}){: style="max-width:80%;"}<br><br>
