...
```json
{
  ...
  "plaintext_body" : (optionnel, plaintext valide, par défaut pour générer automatiquement le texte brut à partir de "body" lorsque ce n'est pas défini),
  "preheader": (optionnel*, chaîne) Longueur recommandée de 50-100 caractères,
  "email_template_id": (optionnel, chaîne) Si fourni, nous utiliserons les valeurs subject/body/should_inline_css du modèle d'email donné UNIQUEMENT elles sont spécifiées ici, auquel cas nous remplacerons le modèle fourni,
  ...
  "pièces jointes": (optionnel, tableau), table d'objets JSON qui définissent les fichiers dont vous avez besoin, défini par "file_name" et "url" ci-dessous,
    "file_name": (requis, ) le nom du fichier que vous voulez attacher à votre e-mail, à l'exclusion de l'extension (e. ., ".pdf"). Vous pouvez joindre n'importe quel nombre de fichiers jusqu'à 2 Mo. Ceci est requis si vous utilisez "pièces jointes",
...
{% alert warning %}
Braze vous recommande d'éviter d'utiliser les liens Google Drive pour l'url `de votre pièce jointe`, car cela peut bloquer les appels de nos serveurs pour obtenir le fichier et résulter en un message de courriel qui ne sera pas envoyé.
{% endalert %}

Pour plus d'informations et de meilleures pratiques sur les pré-en-têtes, consultez notre article d'aide sur le [style du corps de l'e-mail][46].
...
## Exemple d'objet de courriel avec pièce jointe
