...
**속성** 아래에서 해당 기능이 인에이블먼트되면 앱이 Braze 소프트웨어 개발 키트를 통해 액세스할 수 있는 커스텀 속성을 선택적으로 생성할 수 있습니다. 각 변수에 문자열, 부울 값, 이미지, 타임스탬프, JSON 또는 숫자 값을 할당하고 기본값을 설정할 수 있습니다.
...
![세그먼트 및 필터를 추가할 수 있는 롤아웃 트래픽이라는 레이블이 지정된 텍스트 상자.]({% image_buster /assets/img/feature_flags/segmentation_ff.png %}){: style="max-width:75%;"}
...
![여러 규칙이 추가된 기능 플래그와 모든 사용자 규칙의 요약을 보여주는 이미지.]({% image_buster /assets/img/feature_flags/mr_rules_overview.png %}){: style="max-width:80%;"}

### 다중 규칙 기능 플래그 사용 사례

#### 결제 페이지 점진적 공개

전자상거래 브랜드에서 일하고 있으며 안정성을 보장하기 위해 여러 지역에 걸쳐 새 결제 페이지를 배포하고자 한다고 가정해 보겠습니다. 다중 규칙 기능 플래그를 사용하여 다음을 설정할 수 있습니다:
...
![사용자가 받을 수 있는 기능 플래그 표를 보여주는 이미지.]({% image_buster /assets/img/feature_flags/eligibility.png %}){: style="max-width:85%;"}
