...
**기능 플래그 세부정보** 아래에 이름, ID 및 설명을 입력하세요.

![이름, ID, 설명 및 속성을 기능 플래그에 추가할 수 있음을 보여주는 양식.]({% image_buster /assets/img/feature_flags/create_ff_properties.png %}){: style="max-width:75%"}
...
| 등록정보   | 기능 플래그를 원격으로 구성하는 선택적 속성입니다. 이들은 캔버스 단계나 기능 플래그 실험에서 덮어쓸 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

### Step 2a: 사용자 지정 속성 만들기

**속성** 아래에서 기능이 활성화될 때 앱이 Braze SDK를 통해 접근할 수 있는 사용자 정의 속성을 선택적으로 생성할 수 있습니다. 각 변수에 문자열, 불리언, 이미지, 타임스탬프, JSON 또는 숫자 값을 할당할 수 있으며 기본값을 설정할 수 있습니다.

{% tabs local %}
{% tab example %}
다음 예제에서 기능 플래그는 나열된 사용자 정의 속성을 사용하여 전자상거래 상점에 품절 배너를 표시합니다: 
...
기능 플래그를 롤아웃하기 전에 타겟팅할 사용자 [세그먼트]({{site.baseurl}}/user_guide/engagement_tools/segments/)를 선택해야 합니다. 새로 생성된 플래그에서 **규칙 추가**를 선택한 다음 필터 그룹 및 세그먼트 드롭다운 메뉴를 사용하여 대상 오디언스에서 사용자를 필터링합니다. 여러 필터를 추가하여 오디언스를 더욱 좁힙니다.
...
기본적으로 기능 플래그는 항상 비활성 상태이며, 이는 기능 출시 날짜를 전체 사용자 활성화와 분리할 수 있게 해줍니다. 롤아웃을 시작하려면 **롤아웃 트래픽** 섹션을 사용하여 텍스트 상자에 비율을 입력하세요. 이것은 선택한 세그먼트의 무작위 사용자 중 이 새로운 기능을 받을 비율을 선택합니다.
...
{% alert important %}
하나의 규칙으로 플래그를 롤아웃하거나 단일 오디언스에 롤아웃하려면 세분화 기준과 롤아웃 비율을 선택하여 첫 번째 규칙을 추가하세요. 마지막으로 **다른 모든 사람** 규칙이 꺼져 있는지 확인하고 플래그를 저장하세요.
{% endalert %}

## 다중 규칙 기능 플래그 롤아웃

다중 규칙 기능 플래그 롤아웃을 사용하여 사용자를 평가하기 위한 규칙의 순서를 정의하여 정밀한 세분화 및 제어된 기능 출시를 가능하게 합니다. 이 방법은 다양한 오디언스에 동일한 기능을 배포하는 데 이상적입니다. 

### 평가 순서

기능 플래그 규칙은 나열된 순서대로 위에서 아래로 평가됩니다. 사용자는 그들이 충족하는 첫 번째 규칙에 자격이 있습니다. 사용자가 어떤 규칙도 충족하지 않으면, 그들의 자격은 기본 "모든 다른 사람" 규칙에 의해 결정됩니다.

### 사용자 자격

- 사용자가 첫 번째 규칙의 기준을 충족하면, 즉시 기능 플래그를 받을 자격이 있습니다.
- 사용자가 첫 번째 규칙에 자격이 없으면, 두 번째 규칙에 대해 평가되고, 계속해서 진행됩니다.

순차적 평가는 사용자가 규칙에 자격이 있거나 목록의 맨 아래에 있는 "모든 다른 사람" 규칙에 도달할 때까지 계속됩니다.

### "모든 다른 사람" 규칙

"모든 다른 사람" 규칙은 기본값으로 작용합니다. 사용자가 이전의 어떤 규칙에도 자격이 없으면, 기능 플래그에 대한 그들의 자격은 "모든 다른 사람" 규칙의 토글 설정에 의해 결정됩니다. 예를 들어, "모든 다른 사람" 규칙이 "꺼짐"으로 설정되어 있다면, 기본 상태에서 다른 규칙의 기준을 충족하지 않는 사용자는 세션 시작 시 기능 플래그를 받지 못합니다.

### 규칙 재정렬

기본적으로 규칙은 생성된 순서대로 정렬되지만, 대시보드에서 드래그 앤 드롭하여 이러한 규칙을 재정렬할 수 있습니다.

![사용자가 기능 플래그에 규칙을 추가할 수 있음을 보여주는 이미지.]({% image_buster /assets/img/feature_flags/add_rule.png %}){: style="max-width:80%;"}

![여러 규칙이 추가된 기능 플래그의 요약과 모든 다른 사람 규칙을 보여주는 이미지.]({% image_buster /assets/img/feature_flags/mr_rules_overview.png %}){: style="max-width:80%;"}

### 다중 규칙 기능 플래그 사용 사례

#### 체크아웃 페이지를 점진적으로 출시하기

당신이 전자상거래 브랜드에서 일하고 있으며, 안정성을 보장하기 위해 다양한 지역에 걸쳐 출시하고자 하는 새로운 체크아웃 페이지가 있다고 가정해 보겠습니다. 다중 규칙 기능 플래그를 사용하여 다음을 설정할 수 있습니다:

- **규칙 1:** 귀하의 미국 세그먼트는 100%로 설정되어 있습니다.
- **규칙 2:** 귀하의 세그먼트는 브라질 사용자 50%로 설정되어 있으므로 모든 사용자가 한 번에 흐름을 받지는 않습니다. 
- **규칙 3 (다른 모든 사용자):** 모든 다른 사용자에 대해서는 "모든 다른 사용자" 규칙을 토글하여 15%로 설정하여 모든 사용자가 새로운 흐름을 확인할 수 있도록 합니다.

#### 먼저 내부 테스터에게 도달하십시오.

당신이 새로운 제품을 출시할 때 내부 테스터가 항상 기능 플래그를 받을 수 있도록 하고 싶은 제품 매니저라고 가정해 보겠습니다. 내부 테스터 세그먼트를 첫 번째 규칙에 추가하고 100%로 설정하여 내부 테스터가 모든 기능 롤아웃 동안 자격을 갖추도록 할 수 있습니다.

## 기능 플래그에 '활성화됨' 필드 사용 {#enabled}

기능 플래그를 정의한 후, 특정 사용자에게 활성화되어 있는지 확인하기 위해 앱이나 사이트를 구성하십시오. 이 기능이 활성화되면 사용 사례에 따라 몇 가지 작업을 설정하거나 기능 플래그의 변수 속성을 참조하게 됩니다. Braze SDK는 기능 플래그의 상태와 해당 속성정보를 앱으로 가져오는 getter 메서드를 제공합니다. 
...
제공한 키에 대해 해당 유형의 속성이 없으면 이러한 메서드는 `null`을 반환합니다.
...
Braze에서 사용자가 어떤 기능 플래그에 자격이 있는지 확인하려면 **오디언스** > **사용자 검색**로 이동한 다음 사용자를 검색하고 선택하십시오.

**기능 플래그 자격** 탭에서 플랫폼, 애플리케이션 또는 기기에 따라 자격이 있는 기능 플래그 목록을 필터링할 수 있습니다. 기능 플래그 옆에 있는 <i class="fa-solid fa-eye"></i>를 선택하여 사용자에게 반환될 페이로드를 미리 볼 수도 있습니다.

![사용자가 자격이 있는 기능 플래그의 테이블을 보여주는 이미지입니다.]({% image_buster /assets/img/feature_flags/eligibility.png %}){: style="max-width:85%;"}
...
![필터 검색창에 "기능 플래그"가 입력된 "필터" 섹션입니다.]({% image_buster /assets/img/feature_flags/feature-flags-filter-name.png %}){: style="max-width:75%;"}
