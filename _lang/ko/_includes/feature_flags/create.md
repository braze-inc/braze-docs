...
![기존 기능 플래그와 새 기능 플래그를 만드는 방법을 보여주는 데이터테이블입니다.]({% image_buster /assets/img/feature_flags/create_ff.png %}){: style="max-width:75%"}

### 2단계: 세부 정보 입력

**기능 플래그 세부정보에서** 기능 플래그의 이름, ID 및 설명을 입력합니다.

![기능 플래그에 이름, ID, 설명 및 속성을 추가할 수 있는 양식을 보여줍니다.]({% image_buster /assets/img/feature_flags/create_ff_properties.png %}){: style="max-width:75%"}
...
| 등록정보   | 기능 플래그를 원격으로 구성하는 선택적 속성입니다. 캔버스 단계에서 덮어쓰거나 플래그 실험 기능을 사용할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

### Step 2a: 사용자 지정 속성 만들기

**속성** 아래에서 해당 기능이 인에이블먼트되면 앱이 Braze 소프트웨어 개발 키트를 통해 액세스할 수 있는 커스텀 속성을 선택적으로 만들 수 있습니다. 각 변수에 문자열, 부울 값, 이미지, 타임스탬프, JSON 또는 숫자 값을 할당하고 기본값을 설정할 수 있습니다.

{% tabs local %}
{% tab example %}
다음 예제에서는 기능 플래그가 나열된 커스텀 속성을 사용하여 이커머스 스토어의 품절 배너를 표시합니다: 
...
기능 플래그를 롤아웃하기 전에 타겟팅할 사용자 [세그먼트]({{site.baseurl}}/user_guide/engagement_tools/segments/)를 선택해야 합니다. 새로 만든 플래그에서 **규칙 추가를** 선택한 다음 필터 그룹 및 세그먼트 드롭다운 메뉴를 사용하여 타겟팅 대상에서 사용자를 필터링합니다. 여러 필터를 추가하여 오디언스 범위를 더욱 좁힐 수 있습니다.

![세그먼트와 필터를 추가할 수 있는 롤아웃 트래픽이라는 레이블이 붙은 텍스트 상자입니다.]({% image_buster /assets/img/feature_flags/segmentation_ff.png %}){: style="max-width:75%;"}

### 5단계: 롤아웃 트래픽 설정 {#rollout}

기본값으로 기능 플래그는 항상 비활성 상태이므로 기능 릴리스 날짜를 전체 사용자 활성화와 구분할 수 있습니다. 롤아웃을 시작하려면 **롤아웃 트래픽** 섹션을 사용하여 텍스트 상자에 백분율을 입력합니다. 이렇게 하면 선택한 세그먼트에서 이 새로운 기능을 받을 무작위 사용자의 비율이 선택됩니다.
...
{% alert important %}
하나의 규칙으로만 플래그를 롤아웃하거나 단일 오디언스에게 롤아웃하려면 세분화 기준 및 롤아웃 비율을 선택한 상태에서 첫 번째 규칙을 추가하세요. 마지막으로 **다른 모든 사용자** 규칙이 토글되어 있는지 확인하고 플래그를 저장합니다.
{% endalert %}

## 다중 규칙 기능 플래그 롤아웃

다중 규칙 기능 플래그 롤아웃을 사용하여 사용자를 평가하기 위한 일련의 규칙을 정의하면 정확한 세분화와 제어된 기능 릴리스가 가능합니다. 이 방법은 다양한 오디언스에게 동일한 기능을 배포하는 데 이상적입니다. 

### 평가 순서

기능 플래그 규칙은 나열된 순서대로 위에서 아래로 평가됩니다. 사용자는 첫 번째 규칙을 충족하는 자격을 얻습니다. 사용자가 규칙을 충족하지 않는 경우 기본값인 '기타 모든 사용자' 규칙에 따라 자격이 결정됩니다.

### 사용자 자격

- 사용자가 첫 번째 규칙의 기준을 충족하면 즉시 기능 플래그를 받을 수 있습니다.
- 사용자가 첫 번째 규칙에 해당하지 않으면 두 번째 규칙에 따라 평가되는 식으로 평가됩니다.

순차적 평가는 사용자가 규칙에 대한 자격을 갖추거나 목록 하단의 '기타 모든 사용자' 규칙에 도달할 때까지 계속됩니다.

### "다른 모든 사용자" 규칙

'다른 모든 사용자' 규칙이 기본값으로 사용됩니다. 사용자가 앞의 규칙에 해당하지 않는 경우 '기타 모든 사용자' 규칙의 토글 설정에 따라 기능 플래그를 사용할 수 있는 자격이 결정됩니다. 예를 들어 '다른 모든 사용자' 규칙이 기본값인 '꺼짐'으로 토글되어 있으면 다른 규칙의 기준을 충족하지 않는 사용자는 세션 시작 시 해당 기능 플래그를 받지 못합니다.

### 재주문 규칙

기본값으로 규칙은 생성된 순서대로 정렬되지만 대시보드에서 드래그 앤 드롭으로 규칙의 순서를 변경할 수 있습니다.

![사용자가 기능 플래그에 규칙을 추가할 수 있음을 보여주는 이미지입니다.]({% image_buster /assets/img/feature_flags/add_rule.png %}){: style="max-width:80%;"}

![여러 규칙이 추가된 기능 플래그와 모든 사용자 규칙의 요약을 보여주는 이미지입니다.]({% image_buster /assets/img/feature_flags/mr_rules_overview.png %}){: style="max-width:80%;"}

### 다중 규칙 기능 플래그 사용 사례

#### 결제 페이지 점진적 공개

전자상거래 브랜드에서 일하면서 안정성을 보장하기 위해 여러 지역에 배포하려는 새 결제 페이지가 있다고 가정해 보겠습니다. 다중 규칙 기능 플래그를 사용하여 다음을 설정할 수 있습니다:

- **규칙 1:** 미국 세그먼트가 100%로 설정되어 있습니다.
- **규칙 2:** 세그먼트가 브라질 사용자의 50%로 설정되어 있으므로 모든 사용자가 한 번에 플로우를 수신하지는 않습니다. 
- **규칙 3(그 외 모든 사람):** 다른 모든 사용자의 경우 '기타 모든 사용자' 규칙을 토글하고 15%로 설정하여 전체 사용자의 일부가 새 플로우로 결제할 수 있도록 합니다.

#### 내부 테스터에게 먼저 연락하기

신제품을 출시할 때 내부 테스터가 항상 기능 플래그를 받도록 하고 싶은 제품 매니저라고 가정해 보겠습니다. 첫 번째 규칙에 내부 테스터 세그먼트를 추가하고 100%로 설정하면 모든 기능 롤아웃 시 내부 테스터가 자격을 갖추게 됩니다.

## 기능 플래그에 '활성화됨' 필드 사용 {#enabled}

기능 플래그를 정의한 후에는 앱 또는 사이트를 구성하여 특정 사용자에 대해 해당 기능이 인에이블먼트되었는지 여부를 확인합니다. 이 기능이 활성화되면 사용 사례에 따라 몇 가지 작업을 설정하거나 기능 플래그의 변수 속성을 참조하게 됩니다. Braze SDK는 기능 플래그의 상태와 해당 속성정보를 앱으로 가져오는 getter 메서드를 제공합니다. 
...
앱에 새로운 유형의 고객 프로필을 배포한다고 가정합니다. `ID`를 `expanded_user_profile`로 설정할 수 있습니다. 그런 다음, 앱에서 이 새 고객 프로필을 특정 사용자에게 표시해야 하는지 확인합니다. For example:
...
제공한 키에 해당하는 유형의 속성이 없는 경우 이 메서드는 `null` 을 반환합니다.
...
사용자가 Braze에서 어떤 기능 플래그를 사용할 수 있는지 확인하려면 **오디언스** > **사용자 검색으로** 이동한 다음 사용자를 검색하여 선택합니다.

**기능 플래그 적격성** 탭에서 플랫폼, 애플리케이션 또는 기기별로 적격 기능 플래그 목록을 필터링할 수 있습니다. 기능 플래그 옆의 <i class="fa-solid fa-eye"></i> 을 선택하여 사용자에게 반환될 페이로드를 미리 볼 수도 있습니다.

![사용자가 받을 수 있는 기능 플래그 표를 보여주는 이미지입니다.]({% image_buster /assets/img/feature_flags/eligibility.png %}){: style="max-width:85%;"}

## 변경 로그 보기

기능 플래그의 체인지로그를 보려면 기능 플래그를 열고 **체인지로그**를 선택합니다.

![기능 플래그의 '편집' 페이지에 '체인지로그' 버튼이 강조 표시된 모습입니다.]({% image_buster /assets/img/feature_flags/changelog/open_changelog.png %}){: style="max-width:60%;"}

여기에서 변경이 발생한 시기, 변경을 수행한 사람, 변경이 속한 카테고리 등을 검토할 수 있습니다.

![선택한 기능 플래그의 체인지로그입니다.]({% image_buster /assets/img/feature_flags/changelog/changelog.png %}){: style="max-width:90%;"}
...
![필터 검색창에 '기능 플래그'를 입력하면 '필터' 섹션이 나타납니다.]({% image_buster /assets/img/feature_flags/feature-flags-filter-name.png %}){: style="max-width:75%;"}
