...
이제 50%의 사용자에게는 이전 환경이 표시되고 나머지 50%에게는 새로운 환경이 표시됩니다. 그런 다음 두 배리언트를 분석하여 어떤 결제 흐름이 더 높은 전환율을 가져왔는지 확인할 수 있습니다. {% multi_lang_include 분석/metrics.md metric='전환율' %}
