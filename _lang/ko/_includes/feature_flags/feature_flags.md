...
> 기능 플래그를 사용하면 특정 사용자나 무작위로 선택한 사용자에 대해 원격으로 기능을 활성화 또는 비활성화할 수 있습니다. 중요한 점은 추가 코드 배포나 앱 스토어 업데이트 없이 프로덕션 환경에서 기능을 켜고 끌 수 있다는 점입니다. 이를 통해 새로운 기능을 안심하고 안전하게 출시할 수 있습니다.
...
![실시간 채팅 위젯이라는 예제에서 기능 플래그 세부 정보. ID는 enable_live_chat.입니다. 이 기능 플래그 설명은 라이브 채팅 위젯이 지원 페이지에 표시된다고 읽습니다.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-livechat-1.png %})
...
![일반 판매 페이지로 연결되는 링크 및 텍스트 속성이 있는 기능 플래그입니다.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-navigation-link-1.png %})
...
![감사제 판매 페이지로 연결되는 링크 및 텍스트 속성이 있는 기능 플래그입니다.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-navigation-link-2.png %})
...
![Loyalty Rewards Program이라는 이름의 기능 플래그. ID는 show_loyalty_program,이며, 이는 홈 화면과 프로필 페이지에 새로운 로열티 보상 프로그램이 표시된다고 설명합니다.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-loyalty.png %})

그런 다음, 캔버스에서 [기능 플래그 단계]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/feature_flags/)를 생성하여 "고객 가치가 높은" 세그먼트에 대해 `show_loyalty_program` 기능 플래그를 활성화합니다:

![고객 가치가 높은 세그먼트가 show_loyalty_program 기능 플래그를 켜는 오디언스 분할 단계가 있는 캔버스의 예입니다.]({% image_buster /assets/img/feature_flags/feature-flags-use-case-canvas-flow.png %})
...
이제 50%의 사용자에게는 이전 환경이 표시되고 나머지 50%에게는 새로운 환경이 표시됩니다. 그런 다음 두 가지 변형을 분석하여 어떤 체크아웃 흐름이 더 높은 전환율을 가져왔는지 확인할 수 있습니다. {% multi_lang_include analytics/metrics.md metric='Conversion Rate' %}
