...
반면에 이 모델에서 20%의 사용자에게만 메시지를 보낼 수 있고, 그 과정에서 "진짜" 이탈자만 포착하고 다른 이탈자는 포착하지 못한다면 상승률은 100%/20% = 5가 됩니다. 메시지를 보낼 가능성이 높은 이탈 고객의 모든 비율에 대해 이 비율을 차트화하면 이탈률 [곡선](https://en.wikipedia.org/wiki/Lift_(data_mining))을 얻을 수 있습니다. 
