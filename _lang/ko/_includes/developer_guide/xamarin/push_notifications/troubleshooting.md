...
앱이 작업 전환기에서 닫힌 후 푸시 알림이 더 이상 나타나지 않는다면, 앱이 디버그 모드에 있을 가능성이 높습니다. .NET MAUI는 프로세스가 종료된 후 앱이 푸시를 수신하지 못하도록 디버그 모드에서 스캐폴딩을 추가합니다. 릴리스 모드에서 앱을 실행하는 경우 작업 전환기에서 앱을 닫은 후에도 푸시가 표시되어야 합니다.
