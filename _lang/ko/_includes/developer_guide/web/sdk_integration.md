...
- **코드 기반 통합:** 선호하는 패키지 매니저 또는 Braze CDN을 사용하여 코드베이스에 직접 Web Braze SDK를 통합하세요. 이를 통해 소프트웨어 개발 키트를 로드하고 구성하는 방법을 완전히 제어할 수 있습니다.
...
{% alert important %}
[NPM 통합 방법을]({{site.baseurl}}/developer_guide/sdk_integration/?subtab=package%20manager&sdktab=web) 사용하는 것이 좋습니다. 웹사이트에 소프트웨어 개발 키트 라이브러리를 로컬에 저장하고, 광고 차단기 확장 프로그램에 대한 내성을 제공하며, 번들러 지원의 일부로 로드 시간을 단축하는 등의 이점을 누릴 수 있습니다.
{% endalert %}
...
{% alert important %}
Safari의 기본값인 **사이트 간 추적 방지** 설정을 사용하면 CDN 통합 방식을 사용할 때 배너 및 콘텐츠 카드와 같은 인앱 메시지 유형이 표시되지 않도록 할 수 있습니다. 이 문제를 방지하려면 NPM 통합 방법을 사용하여 Safari가 이러한 메시지를 교차 사이트 트래픽으로 분류하지 않고 웹 사용자가 지원되는 모든 브라우저에서 메시지를 볼 수 있도록 하세요.
{% endalert %}
...
{% alert important %}
**인앱 메시지 표시**: 인앱 메시지가 트리거될 때 자동으로 표시하려면 `braze.automaticallyShowInAppMessages()` 으로 전화해야 합니다. 이 호출이 없으면 인앱 메시지가 자동으로 표시되지 않습니다. 메시지 표시를 수동으로 관리하려면 이 호출을 삭제하고 대신 `braze.subscribeToInAppMessage()` 을 사용하세요. 자세한 내용은 [인앱 메시지 전달을]({{site.baseurl}}/developer_guide/in_app_messages/delivery/) 참조하세요.
{% endalert %}
...
예를 들어, 기본값 통합 지침에서 권장하는 대로 콘텐츠 전달 네트워크( `https://js.appboycdn.com/web-sdk/a.a/braze.min.js` )에서 Braze 웹 SDK를 참조하면 사용자가 사이트를 새로고침할 때 자동으로 사소한 업데이트(버그 수정 및 이전 버전과의 호환 기능, 위의 예에서 `a.a.a` ~ `a.a.z` 버전)를 받게 됩니다.

그러나 주요 변경 사항을 릴리스할 때는 변경 사항이 통합에 영향을 미치지 않도록 Braze 웹 SDK를 수동으로 업그레이드해야 합니다. 또한 소프트웨어 개발 키트를 다운로드하여 직접 호스팅하는 경우 버전 업데이트가 자동으로 제공되지 않으므로 최신 기능과 버그 수정을 받으려면 수동으로 업그레이드해야 합니다.
...
제대로 작동하려면 이 두 파일을 서로 조정하여 업데이트해야 합니다.
...
자세한 내용이나 심층적인 Tealium 구성 지원은 [통합 설명서를]({{site.baseurl}}/partners/data_and_infrastructure_agility/customer_data_platform/tealium/#about-tealium) 확인하거나 Tealium 계정 매니저에게 문의하세요.
