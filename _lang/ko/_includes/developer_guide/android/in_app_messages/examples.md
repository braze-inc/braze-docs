...
- [구현 예시](#implementation-examples)
  - [코드 고려 사항](#code-considerations)
    - [커스텀 팩토리](#custom-factories)
  - [사용 사례](#use-cases)
    - [사용자 지정 슬라이드업 인앱 메시지](#custom-slide-up-in-app-message)
      - [사용자 지정 뷰 래퍼](#custom-view-wrapper)
    - [커스텀 Modal 인앱 메시지](#custom-modal-in-app-message)
    - [커스텀 전체 인앱 메시지](#custom-full-in-app-message)
      - [인앱 메시지 터치 가로채기](#intercepting-in-app-message-touches)
...
#### 인앱 메시지 터치 가로채기
![설정 및 토글 행을 표시하는 안드로이드 기기입니다. 커스텀 보기는 버튼을 처리하며, 버튼 컨트롤 외부의 모든 터치는 인앱 메시지에 의해 처리되고 메시지를 해제합니다.]({% image_buster /assets/img/iam_implementation_guide_android.png %}){: style="float:right;max-width:30%;margin-left:10px;border:0"}
