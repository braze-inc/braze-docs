...
{% tabs %}
{% tab global listener %}
`BrazeInAppMessageManager` 리스너는 인앱 메시지의 표시 및 생명주기를 자동으로 처리할 수 있지만, 메시지를 완전히 커스터마이징하려면 커스텀 매니저 리스너를 구현해야 합니다.
{% endtab %}

{% tab html listener %}
Braze SDK에는 커스텀 리스너가 정의되어 있지 않은 경우 사용되는 기본 `DefaultHtmlInAppMessageActionListener` 클래스가 있으며, 자동으로 적절한 동작을 수행합니다. 커스텀 HTML 인앱 메시지 내에서 사용자가 다양한 버튼과 상호 작용하는 방식을 더 세밀하게 제어해야 하는 경우 커스텀 `IHtmlInAppMessageActionListener` 클래스를 구현하세요.
...
`IInAppMessageManagerListener`의 콜백은 인앱 메시지 생명주기의 다양한 시점에서 호출됩니다. 예를 들어, Braze에서 인앱 메시지를 수신할 때 커스텀 매니저 리스너를 설정하면 [`beforeInAppMessageDisplayed()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-displayed.html) 메서드가 호출됩니다. 이 메서드의 구현이 [`InAppMessageOperation.DISCARD`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-operation/-d-i-s-c-a-r-d/index.html)를 반환하면, 인앱 메시지가 호스트 앱에서 처리되며 Braze에서 표시하지 않아야 함을 Braze에 알립니다. [`InAppMessageOperation.DISPLAY_NOW`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-operation/-d-i-s-p-l-a-y_-n-o-w/index.html)가 반환되면, Braze는 인앱 메시지를 표시하려고 시도합니다. 인앱 메시지를 커스텀 방식으로 표시하려는 경우 이 메서드를 사용해야 합니다.
...
1. [`beforeInAppMessageViewOpened`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-view-opened.html): 인앱 메시지가 활동의 뷰에 추가되기 직전에 호출됩니다. 이 시점에서 인앱 메시지는 아직 사용자에게 표시되지 않습니다.
2. [`afterInAppMessageViewOpened`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/after-in-app-message-view-opened.html): 인앱 메시지가 활동의 뷰에 추가된 직후에 호출됩니다. 이 시점에서 인앱 메시지가 사용자에게 표시됩니다.
3. [`beforeInAppMessageViewClosed`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-view-closed.html): 인앱 메시지가 활동의 뷰에서 제거되기 직전에 호출됩니다. 이 시점에서 인앱 메시지는 여전히 사용자에게 표시됩니다.
4. [`afterInAppMessageViewClosed`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/after-in-app-message-view-closed.html): 인앱 메시지가 활동의 뷰에서 제거된 직후에 호출됩니다. 이 시점에서 인앱 메시지는 더 이상 사용자에게 표시되지 않습니다.

[`afterInAppMessageViewOpened`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/after-in-app-message-view-opened.html)와 [`beforeInAppMessageViewClosed`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-view-closed.html) 사이의 시간이 인앱 메시지 뷰가 화면에 표시되어 사용자에게 보이는 시간입니다.

{% alert note %}
이러한 메서드를 반드시 구현할 필요는 없습니다. 인앱 메시지 뷰 생명주기를 추적하고 알리기 위해 제공되는 것입니다. 이 메서드 구현을 비워둘 수 있습니다.
{% endalert %}
{% endtab %}
...
`IHtmlInAppMessageActionListener`의 콜백은 사용자가 HTML 인앱 메시지 내에서 다음 동작 중 하나를 수행할 때마다 호출됩니다:

- 닫기 버튼 클릭
- 커스텀 이벤트 실행
- HTML 인앱 메시지 내의 URL 클릭
...
그러나 표시 중인 인앱 메시지가 없으면 `IInAppMessageManagerListener`의 다음 위임 메서드가 호출됩니다:
...
`InAppMessageOperation()` 반환 값으로 메시지 표시 시기를 제어할 수 있습니다. 이 메서드의 권장 사용법은 인앱 메시지가 사용자의 앱 경험을 방해할 수 있는 앱의 특정 부분에서 `DISPLAY_LATER`를 반환하여 메시지를 지연시키는 것입니다.
...
{% alert tip %}
인앱 메시지를 `DISCARD`하고 자체 인앱 메시지 뷰로 대체하는 경우, 인앱 메시지 클릭 수와 노출 횟수를 수동으로 기록해야 합니다.
{% endalert %}

Android에서는 인앱 메시지에서 `logClick` 및 `logImpression`을 호출하고, 몰입형 인앱 메시지에서 `logButtonClick`을 호출하면 됩니다.

{% alert tip %}
인앱 메시지가 스택에 배치된 후에는 언제든지 [`BrazeInAppMessageManager.getInstance().requestDisplayInAppMessage()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-braze-in-app-message-manager/request-display-in-app-message.html)를 호출하여 검색 및 표시를 요청할 수 있습니다. 이 메서드는 스택에서 사용 가능한 다음 인앱 메시지를 표시하도록 Braze에 요청합니다.
{% endalert %}
{% endtab %}

{% tab html listener %}
`IHtmlInAppMessageActionListener`를 생성한 후, `BrazeInAppMessageManager.getInstance().setCustomHtmlInAppMessageActionListener()`를 호출하여 `BrazeInAppMessageManager`에게 기본 동작 리스너 대신 커스텀 `IHtmlInAppMessageActionListener`를 사용하도록 지시합니다.

Braze에 대한 다른 호출보다 먼저 [`Application.onCreate()`](https://developer.android.com/reference/android/app/Application.html#onCreate())에서 `IHtmlInAppMessageActionListener`를 설정하는 것이 좋습니다. 이렇게 하면 인앱 메시지가 표시되기 전에 커스텀 동작 리스너가 설정됩니다:
...
{% tabs %}
{% tab view %}
Braze 인앱 메시지 유형은 대부분의 커스텀 사용 사례를 충분히 다룰 수 있을 만큼 다양합니다. 그러나 기본 유형을 사용하는 대신 인앱 메시지의 시각적 모양을 완전히 정의하고 싶다면, Braze에서 커스텀 뷰 팩토리를 설정하여 이를 구현할 수 있습니다.
{% endtab %}

{% tab view wrapper %}
`BrazeInAppMessageManager`는 기본적으로 [`DefaultInAppMessageViewWrapper`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-default-in-app-message-view-wrapper/index.html)를 사용하여 인앱 메시지 모델을 기존 활동 뷰 계층 구조에 자동으로 배치합니다. 인앱 메시지가 뷰 계층 구조에 배치되는 방식을 커스터마이징해야 하는 경우 커스텀 [`IInAppMessageViewWrapperFactory`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-i-in-app-message-view-wrapper-factory/index.html)를 사용해야 합니다.
{% endtab %}

{% tab animation %}
인앱 메시지에는 사전 설정된 애니메이션 동작이 있습니다. `Slideup` 메시지는 화면으로 슬라이드되고, `full` 및 `modal` 메시지는 페이드 인/아웃됩니다. 인앱 메시지에 커스텀 애니메이션 동작을 정의하려면, Braze에서 커스텀 애니메이션 팩토리를 설정하면 됩니다.
{% endtab %}
{% endtabs %}
...
`IInAppMessageView`를 구현하면 커스텀 뷰의 특정 부분을 클릭 가능한 영역으로 정의할 수 있습니다. [`IInAppMessageImmersiveView`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.views/-i-in-app-message-immersive-view/index.html)를 구현하면 메시지 버튼 뷰와 닫기 버튼 뷰를 정의할 수 있습니다.
{% endtab %}

{% tab view wrapper %}
[`IInAppMessageViewWrapper`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-i-in-app-message-view-wrapper/index.html)를 생성한 후, [`BrazeInAppMessageManager.getInstance().setCustomInAppMessageViewWrapperFactory()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-manager-base/set-custom-in-app-message-view-factory.html)를 호출하여 `BrazeInAppMessageManager`에게 기본 뷰 래퍼 팩토리 대신 커스텀 [`IInAppMessageViewWrapperFactory`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-i-in-app-message-view-wrapper-factory/index.html)를 사용하도록 지시합니다.

Braze에 대한 다른 호출보다 먼저 [`Application.onCreate()`](https://developer.android.com/reference/android/app/Application.html#onCreate())에서 [`IInAppMessageViewWrapperFactory`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-i-in-app-message-view-wrapper-factory/index.html)를 설정하는 것이 좋습니다. 이렇게 하면 인앱 메시지가 표시되기 전에 커스텀 뷰 래퍼 팩토리가 설정됩니다:
...
예를 들어 인앱 메시지 버튼 텍스트의 폰트를 업데이트하려면 `Braze.InAppMessage.Button` 스타일을 재정의하고 커스텀 폰트 패밀리를 참조합니다. 속성 값은 `res/font` 디렉토리에 있는 폰트 패밀리를 가리켜야 합니다.

다음은 마지막 줄에서 커스텀 폰트 패밀리(`my_custom_font_family`)를 참조하는 축약된 예제입니다:
...
버튼 텍스트의 `Braze.InAppMessage.Button` 스타일 외에도, 메시지 텍스트의 스타일은 `Braze.InAppMessage.Message`이고 메시지 헤더의 스타일은 `Braze.InAppMessage.Header`입니다. 가능한 모든 인앱 메시지 텍스트에 커스텀 폰트 패밀리를 사용하려면, 모든 인앱 메시지의 상위 스타일인 `Braze.InAppMessage` 스타일에서 폰트 패밀리를 설정하면 됩니다.
...
- **왼쪽 또는 오른쪽 스와이프:** 위치에 관계없이 슬라이드업을 해제합니다.
- **하단에서 슬라이드업:** 위에서 아래로 스와이프하면 메시지가 해제됩니다. 아래에서 위로 스와이프하면 해제되지 않습니다.
- **상단에서 슬라이드업:** 아래에서 위로 스와이프하면 메시지가 해제됩니다. 위에서 아래로 스와이프하면 해제되지 않습니다.

이 스와이프 동작은 기본 [`DefaultInAppMessageViewWrapper`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-default-in-app-message-view-wrapper/index.html)에 내장되어 있으며 슬라이드업 인앱 메시지에만 적용됩니다. 모달 및 전체 화면 인앱 메시지는 스와이프 해제를 지원하지 않습니다. 이 동작을 커스터마이징하려면 [커스텀 뷰 래퍼 팩토리](#android_setting-custom-factories)를 구현할 수 있습니다.
...
## 다크 테마 비활성화 {#android-in-app-message-dark-theme-customization}

기본적으로 `IInAppMessageManagerListener`의 `beforeInAppMessageDisplayed()`는 시스템 설정을 확인하고 다음 코드를 사용하여 메시지에 다크 테마 스타일링을 조건부로 활성화합니다:
