...
인앱 메시지는 SDK가 다음의 커스텀 이벤트 유형 중 하나를 기록할 때 자동으로 트리거됩니다: `Any Purchase`, `Specific Purchase`, `Session Start`, `Custom Event`, 및 `Push Click`. `Specific Purchase` 및 `Custom Event` 트리거는 강력한 속성 필터도 포함하고 있습니다.
...
모든 적격 인앱 메시지는 사용자의 세션 시작 시 기기로 전달됩니다. 전달 시, SDK는 자산을 미리 가져와서 트리거 시점에 사용할 수 있도록 하여 표시 지연을 최소화합니다. 트리거 이벤트에 적격 인앱 메시지가 두 개 이상 있는 경우 우선순위가 가장 높은 메시지만 전달됩니다.

SDK의 세션 시작 의미에 대한 자세한 내용은 [세션 생명주기]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/?tab=android)를 참조하십시오.
...
Braze에서 캠페인을 생성할 때, 인앱 메시징 객체가 앱에 데이터를 전송하는 데 사용할 수 있는 `extras`으로 키-값 쌍을 설정할 수 있습니다. For example:
...
특정 무음 푸시 알림을 수신하기 위해 [커스텀 푸시 이벤트 콜백]({{site.baseurl}}/developer_guide/push_notifications/customization/?sdktab=android#push-callback)을 등록하십시오.

다음 예제에서는 인앱 메시지가 전달되기 위해 두 개의 이벤트가 기록됩니다. 하나는 서버에서, 다른 하나는 커스텀 푸시 콜백 내에서 발생합니다. 동일한 이벤트가 중복되지 않도록 하려면 푸시 콜백 내에서 기록된 이벤트는 일번적인 명명 규칙을 따라야 하고(예: '인앱 메시지 트리거 이벤트'), 서버에서 보낸 이벤트와 같은 이름이 아니어야 합니다. 이를 준수하지 않으면 단일 사용자 작업에 대해 기록된 중복 이벤트가 세분화 및 사용자 데이터에 영향을 미칠 수 있습니다.
...
![두 세트의 키-값 쌍: IS_SERVER_EVENT는 "true"로 설정되고, CAMPAIGN_NAME는 "예시 캠페인 이름"으로 설정됩니다.]({% image_buster /assets/img_archive/kvpConfiguration.png %}){: style="max-width:70%;" }
...
![인앱 메시지가 "campaign_name"가 "IAM 캠페인 이름 예시."와 같을 때 트리거되는 실행 기반 전달 캠페인입니다.]({% image_buster /assets/img_archive/iam_event_trigger.png %})
