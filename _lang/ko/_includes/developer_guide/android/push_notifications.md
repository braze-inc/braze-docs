...
다음 기능들은 Braze Android 소프트웨어 개발 키트에 구축되어 있습니다. 다른 푸시 알림 기능을 사용하려면 앱에 [푸시 알림을 설정해야](#android_setting-up-push-notifications) 합니다.

|Feature|설명|
|-------|-----------|
|푸시 스토리|Android 푸시 스토리는 기본값으로 Braze Android 소프트웨어 개발 키트에 내장되어 있습니다. 자세한 내용은 [푸시 스토리를]({{site.baseurl}}/user_guide/message_building_by_channel/push/advanced_push_options/push_stories/) 참조하세요.|
...
다음 플로우 차트는 Braze가 푸시 알림 라이프사이클(예: 권한 요청, 토큰 생성, 메시지 전달)을 처리하는 방식을 보여줍니다.
...
{% alert tip %}
Braze Android 소프트웨어 개발 키트를 사용한 FCM 샘플 앱을 확인하려면 Braze를 [참조하세요: Firebase 푸시 샘플 앱](https://github.com/braze-inc/braze-android-sdk/tree/master/samples/firebase-push).
{% endalert %}
...
**역할** 필드에서 역할 목록에서 **Firebase Cloud Messaging API 관리자**를 찾아 선택합니다. 보다 제한적인 액세스를 원하면 `cloudmessaging.messages.create` 권한으로 [커스텀 역할](https://cloud.google.com/iam/docs/creating-custom-roles)을 생성한 다음, 목록에서 해당 역할을 선택합니다. 완료했으면 **완료**를 선택합니다.

{% alert warning %}
**Firebase 클라우드 메시징 관리자**가 아닌 **Firebase 클라우드 메시징 _API_ 관리자**를 선택해야 합니다.
{% endalert %}

!["이 서비스 계정에 프로젝트 접근 권한 부여" 양식에서 역할로 "Firebase Cloud Messaging API 관리자"가 선택된 상태.]({% image_buster /assets/img/android/push_integration/create_a_service_account/add-fcm-api-admin.png %})

### 5단계: JSON 자격 증명 생성 {#json}

다음으로 FCM 서비스 계정에 대한 JSON 자격 증명을 생성합니다. Google Cloud IAM&관리자에서 **서비스 **계정으로 이동한 후 프로젝트를 선택하세요. [이전에 만든](#android_service-account) FCM 서비스 계정을 찾은 다음, <i class="fa-solid fa-ellipsis-vertical"></i> **작업** > **키 관리**를 선택합니다.

![프로젝트의 서비스 계정 홈페이지에 "작업" 메뉴가 열린 상태입니다.]({% image_buster /assets/img/android/push_integration/generate_json_credentials/select-manage-keys.png %})
...
!["설정" 메뉴가 Braze에서 열리며 "앱 설정"이 강조 표시됩니다.]({% image_buster /assets/img/android/push_integration/upload_json_credentials/select-app-settings.png %})

Android 앱의 **푸시 알림 설정**에서 **Firebase**를 선택한 다음, **JSON 파일 업로드**를 선택하고 [앞서 생성](#android_json)한 자격 증명을 업로드합니다. When you're finished, select **Save**.
...
![Firebase 프로젝트의 "Cloud 메시징" 페이지에서 "발신자 ID"가 강조 표시된 상태.]({% image_buster /assets/img/android/push_integration/set_up_automatic_token_registration/copy-sender-id.png %})
...
새로 생성하거나 기존에 존재하는 Firebase 메시징 서비스를 생성하거나 Braze와 연동되지 않은 서비스를 생성할 수 있습니다. 여러분의 특정 요구 사항에 가장 잘 맞는 것을 선택하십시오.
...
또한 알림 코드는 `BrazeFirebaseMessagingService` 를 사용하여 열기 및 클릭 동작 추적을 처리합니다. 이 서비스가 제대로 작동하려면 `AndroidManifest.xml` 에 등록해야 합니다. 또한 Braze는 시스템에서 보낸 알림만 렌더링하도록 시스템의 알림에 고유한 키를 접두사로 추가합니다. 추가 서비스를 별도로 등록하여 다른 FCM 서비스에서 전송되는 알림을 표시할 수 있습니다. Firebase 푸시 샘플 앱에서 [`AndroidManifest.xml`](https://github.com/braze-inc/braze-android-sdk/blob/master/samples/firebase-push/src/main/AndroidManifest.xml)확인하세요.
...
알림용 작은 아이콘 자산을 올바르게 표시되도록 생성하려면:
...
!["Hey I'm on my way to the bar..."라는 메시지 옆의 큰 아이콘 하단 모서리에 작은 아이콘이 나타납니다.]({% image_buster /assets/img_archive/large_and_small_notification_icon.png %} "Large and Small Notification Icon")

### 3단계: 알림 아이콘 설정 {#configure-icons}
...
딥링크의 커스텀 처리를 설정하려면 Braze에서 푸시를 수신하고 여는 의도를 수신 대기하는 푸시 콜백을 만들어야 합니다. 자세한 내용은 [푸시 이벤트에 대한 콜백 사용을]({{site.baseurl}}/developer_guide/push_notifications/customization#android_using-a-callback-for-push-events) 참조하십시오.

## 전경 알림 처리

기본값으로, Android에서 앱이 전경에 있을 때 푸시 알림이 도착하면 시스템이 자동으로 표시합니다. Braze가 푸시 알림 페이로드를 처리하도록 하려면(분석 추적, 딥링크 처리 및 커스텀 처리를 위해), `FirebaseMessagingService.onMessageReceived`메서드 내에서 수신 푸시 데이터를 Braze로 라우팅하십시오.

### 작동 방식

호출 시`BrazeFirebaseMessagingService.handleBrazeRemoteMessage`, Braze는 페이로드가 Braze 푸시 알림인지 판단하고, 해당할 경우  `NotificationManagerCompat`메서드를 통해 알림을 생성 및 표시합니다. iOS와 달리 Android는 앱이 전경에 있든 배경에 있든 상관없이 알림을 표시합니다.
...
자세한 내용은 Braze Android SDK 저장소의 [Firebase SDK 통합 샘플을](https://github.com/braze-inc/braze-android-sdk/blob/master/samples/firebase-push/src/main/java/com/braze/firebasepush/FirebaseMessagingService.kt) 참조하십시오.

### 전경 동작 커스텀

시스템 알림을 억제하거나 대신 앱 내 UI를 표시하는 등 커스텀 전경 동작을 원한다면 다음과 같이 할 수 있습니다:

- 를`subscribeToPushNotificationEvents`사용하여 푸시 이벤트에 반응하고  `BrazeNotificationUtils.routeUserWithNotificationOpenedIntent`메서드로 딥링크를 처리하세요. 자세한 내용은 [Firebase 푸시 ](https://github.com/braze-inc/braze-android-sdk/blob/master/samples/firebase-push/src/main/java/com/braze/firebasepush/FirebaseApplication.kt)샘플을 참조하세요.
- 커스텀 를 사용하여 자체 알림을`IBrazeNotificationFactory` 구축하고 게시하거나, 처리 경로에서 를 `notificationManager.notify`호출하지 않음으로써 알림을 억제할 수 있습니다.

알림 커스텀 방법에 대한 자세한 내용은 [커스텀 알림 팩토리]({{site.baseurl}}/developer_guide/push_notifications/customization/?sdktab=android#custom-notification-factory) 문서를 참조하십시오.

#### 사용자 지정 딥링크 만들기

앱에 아직 딥링킹을 추가하지 않았다면, [Android 개발자 설명서](http://developer.android.com/training/app-indexing/deep-linking.html) 내 딥링킹 관련 지침을 따르십시오. 딥링크 개념에 대한 자세한 내용은 [FAQ 문서]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/deep_linking_to_in-app_content/#what-is-deep-linking)를 참조하세요.

#### 딥링크 추가하기

Braze 대시보드는 푸시 알림 캠페인과 캔버스에서 알림을 클릭할 때 열 딥링크 또는 웹 URL 설정을 지원합니다.

![Braze 대시보드의 '클릭 시 동작' 설정에서 드롭다운 메뉴에서 '앱으로의 딥링크'를 선택한 상태.]({% image_buster /assets/img_archive/deep_link_click_action.png %} "Deep Link Click Action")
...
Braze 안드로이드 SDK는 [안드로이드 알림 채널을](https://developer.android.com/preview/features/notification-channels.html) 지원합니다. Braze 알림에 알림 채널의 ID가 포함되어 있지 않거나 Braze 알림에 유효하지 않은 채널 ID가 포함되어 있는 경우, Braze는 SDK에 정의된 기본 알림 채널로 알림을 표시합니다. 기업 사용자는 플랫폼 내에서 [Android 알림 채널을]({{site.baseurl}}/user_guide/message_building_by_channel/push/android/notification_channels/) 사용하여 알림을 그룹화합니다.
...
Firebase FCM의 할당량 한도를 초과하면 Google에서 "할당량 초과" 오류를 반환합니다. FCM의 기본값 제한은 분당 600,000건의 요청입니다. Braze는 Google이 권장하는 모범 사례에 따라 재전송을 시도합니다. 그러나 이러한 오류가 대량으로 발생할 경우 전송 시간이 수 분간 지연될 수 있습니다. 잠재적 영향을 완화하기 위해 Braze는 속도 제한 초과 알림을 발송하며, 오류를 방지하기 위한 조치 방법을 안내해 드립니다.

현재 한도를 확인하려면 **Google Cloud Console** > **API 및& 서비스** > **Firebase FCM API** > **할당량 및& 시스템 제한으로** 이동하거나 [FCM API 할당량 페이지를](https://console.cloud.google.com/apis/api/fcm.googleapis.com/quotas) 방문하세요.

### Best practices

이러한 오류 발생량을 낮게 유지하기 위해 다음과 같은 모범 사례를 권장합니다.

#### FCM에 속도 제한 증가 요청

FCM에 대한 속도 제한 증량을 요청하려면 [Firebase 지원팀에](https://firebase.google.com/support) 직접 문의하거나 다음 단계를 수행하세요:

1. [FCM API 할당량 페이지](https://console.cloud.google.com/apis/api/fcm.googleapis.com/quotas)로 이동하세요.
2. **분당 요청 전송** 할당량을 찾으십시오.
3. **할당량 편집을** 선택하십시오. 
4. 새 값을 입력하고 요청을 제출하세요.

#### Braze를 통한 글로벌 속도 제한 요청

Android 푸시 알림에 대한 작업 공간 전체 제한을 적용하려면 [Braze 지원팀에]({{site.baseurl}}/help/support#access-the-support-portal) 문의하십시오.