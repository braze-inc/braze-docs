...
{% alert important %}
기본 콘텐츠 카드 개발자 통합 가이드를 찾고 계신가요? [여기]({{site.baseurl}}/developer_guide/content_cards/?sdktab=android)에서 확인하세요.<br><br>콘텐츠 카드 사용자 지정에 대한 자세한 내용은 [사용자 지정 가이드]({{site.baseurl}}/developer_guide/content_cards/)에서 확인할 수 있습니다.
{% endalert %}
...
콘텐츠 카드를 빌드할 때는 단일 매니저 싱글톤을 통해 Braze SDK를 공개해야 합니다. 이 패턴은 사용 사례에 적합한 공유 추상화 이면의 Braze 구현 세부 정보으로부터 애플리케이션 코드를 보호합니다. 또한 이를 통해 코드를 더 쉽게 추적, 디버그 및 변경할 수 있습니다. 예제 매니저 구현에 대한 내용은 GitHub의 데모 앱에서 [`BrazeManager.kt`](https://github.com/braze-inc/braze-growth-shares-android-demo-app/blob/main/app/src/main/java/com/braze/advancedsamples/BrazeManager.kt)을 참조하세요.

### 사용자 지정 객체로서의 콘텐츠 카드

앱에 이미 있는 커스텀 객체를 확장하여 콘텐츠 카드 데이터를 저장할 수 있으므로, 데이터 소스를 코드와 이미 호환되는 형식으로 추상화할 수 있습니다. 이것은 서로 다른 데이터 백엔드를 교환 가능하고 함께 작업할 수 있는 유연성을 제공합니다.

다음 예제에서는 기존 데이터(로컬 JSON 파일에서 제공됨)와 새로운 데이터(Braze SDK에서 제공됨)를 모두 나타내기 위해 `ContentCardable` 추상 기본 클래스를 정의했습니다. 기본 클래스는 원본 `Card` 구현에 액세스해야 하는 소비자를 위해 원시 콘텐츠 카드 데이터도 공개합니다.
...
이 코드 고려 사항을 탐색한 후, [사용 사례](#sample-use-cases)를 확인하여 자신의 커스텀 객체를 구현하기 시작할 수 있습니다.
