...
앱에 이미 있는 커스텀 객체를 확장하여 콘텐츠 카드 데이터를 저장할 수 있으므로 데이터 소스를 코드와 호환되는 형식으로 추상화할 수 있습니다. 이것은 서로 다른 데이터 백엔드를 유연하게 교환하고 함께 작업할 수 있게 해줍니다.

다음 예제에서는 기존 데이터(로컬 JSON 파일에서 제공됨)와 새로운 데이터(Braze SDK에서 제공됨)를 나타내기 위해 `ContentCardable` 추상 기본 클래스를 정의했습니다. 기본 클래스는 원본 `Card` 구현에 액세스해야 하는 소비자를 위해 원시 콘텐츠 카드 데이터도 공개합니다.
