...
Braze 콘텐츠 카드를 사용하기 전에 [Braze Android SDK]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=android)를 앱에 통합해야 합니다. 그러나 추가 설정은 필요하지 않습니다.
...
활동에 프래그먼트를 추가하는 방법을 배우려면 [구글의 프래그먼트 설명서](https://developer.android.com/guide/fragments#Adding)를 참조하세요.

## 카드 유형 및 속성

콘텐츠 카드 데이터 모델은 Android SDK에서 사용할 수 있으며 다음과 같은 고유한 콘텐츠 카드 유형을 제공합니다. 각 유형은 기본 모델을 공유하며, 이를 통해 고유한 속성을 가지는 것 외에도 기본 모델에서 공통 속성을 상속받을 수 있습니다. 전체 참조 설명서는 [`com.braze.models.cards`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.models.cards/index.html)를 참조하세요.
...
|`isDismissibleByUser()`  | 카드가 사용자가 닫을 수 있는지 여부를 반영하는 불리언 값을 반환합니다.|
|`isClicked()` | 이 카드의 클릭 상태를 반영하는 불리언 값을 반환합니다.|
|`isDismissed` | 카드가 닫혔는지 여부를 반영하는 불리언 값을 반환합니다. 카드를 닫힌 것으로 표시하려면 `true`로 설정합니다. 카드가 이미 해제됨 상태로 표시된 경우 다시 해제됨 상태로 표시할 수 없습니다.|
|`isControl()` | 이 카드가 제어 카드이고 렌더링되지 않아야 하는 경우 불리언 값을 반환합니다.|
...
### 캡션이 있는 이미지 {#captioned-image-card-for-android}
