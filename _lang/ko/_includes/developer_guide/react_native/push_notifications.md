...
[React Native Linking에서](https://reactnative.dev/docs/linking) 처리하는 기본 시나리오 외에도 `Braze.getInitialPushPayload` 메서드를 구현하고 `url` 값을 검색하여 앱이 실행되지 않을 때 앱을 여는 푸시 알림의 딥링킹을 고려하세요. For example:
...
포그라운드 알림 처리는 플랫폼과 설정에 따라 다르게 작동합니다. 통합에 맞는 접근 방식을 선택하세요:

{% tabs local %}
{% tab iOS %}
iOS의 경우 포그라운드 알림 처리는 기본 Swift 통합과 동일합니다. `UNUserNotificationCenterDelegate.userNotificationCenter(_:willPresent:withCompletionHandler:)` 구현 내에서 `handleForegroundNotification(notification:)` 으로 전화하세요.

자세한 내용과 코드 예시는 Swift 푸시 알림 설명서에서 [포그라운드 알림 처리하기를]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift#handling-foreground-notifications) 참조하세요.
{% endtab %}

{% tab Android %}
Android의 경우 포그라운드 알림 처리는 기본 Android 통합과 동일합니다. `FirebaseMessagingService.onMessageReceived` 메소드 내에서 `BrazeFirebaseMessagingService.handleBrazeRemoteMessage` 으로 전화하세요.

자세한 내용과 코드 예시는 Android 푸시 알림 설명서에서 [포그라운드 알림 처리하기를]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=android#handling-foreground-notifications) 참조하세요.
{% endtab %}

{% tab Expo %}
엑스포 관리 워크플로에서는 기본 알림 핸들러를 직접 호출하지 않습니다. 대신 엑스포 알림 API를 사용하여 포그라운드 프레젠테이션을 제어하고, Braze 엑스포 플러그인은 기본 처리를 자동으로 처리합니다.
...
{% alert note %}
엑스포 관리 워크플로우에서 Braze 엑스포 플러그인은 기본 푸시 처리를 자동으로 처리합니다. 위에 표시된 엑스포 알림 프레젠테이션 옵션을 통해 전경 UI를 제어할 수 있습니다.
{% endalert %}

베어 워크플로 통합의 경우, 대신 기본 iOS 및 Android 접근 방식을 따르세요.
{% endtab %}
{% endtabs %}
...
문제가 지속되면 별도의 종속성이 Braze 푸시 알림 구성을 방해하고 있을 수 있습니다. 대신 제거하거나 `Braze.registerPushToken` 으로 직접 문의하세요.
