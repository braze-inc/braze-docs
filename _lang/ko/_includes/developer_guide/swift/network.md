...
{% tabs local %}
{% tab automatic %}
기본값으로 `RequestPolicy` 열거형 값은 `automatic` 으로 설정되어 있습니다. 설정하면 인앱 메시지와 같은 Braze 기능에 사용자 데이터가 필요할 때 즉각적인 서버 요청이 수행됩니다.
