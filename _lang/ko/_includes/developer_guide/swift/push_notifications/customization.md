...
## 실행 버튼 커스텀하기 {#push-action-buttons-integration}
...
{% alert important %}
푸시 실행 버튼을 수동으로 등록하는 것은 권장하지 않습니다.
{% endalert %}

`configuration.push.automation` 설정 옵션을 사용하여 [푸시 알림을 설정하면]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift) 기본값 푸시 카테고리에 대한 실행 버튼을 자동으로 등록하고 푸시 실행 버튼 클릭 분석 및 URL 라우팅을 처리합니다.

그러나 대신 푸시 실행 버튼을 수동으로 등록하도록 선택할 수 있습니다.
...
## 푸시 카테고리 커스텀하기 {#customizing-push-categories}
