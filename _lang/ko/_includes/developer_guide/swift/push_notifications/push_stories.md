{% multi_lang_include developer_guide/prerequisites/swift.md %} 또한 [푸시 알림을 설정]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift)해야 하며, 여기에는 프레임워크`UNNotification` 구현이 포함됩니다.

푸시 스토리를 수신하려면 다음 최소 소프트웨어 개발 키트 버전이 필요합니다:
...
Xcode에서 메인 앱 타깃에 **서명&기능** 패널을 사용하여 배경 모드 기능을 추가하십시오. **백그라운드 가져오기** 및 **원격 알림** 체크박스를 모두 선택합니다.

![]({% image_buster /assets/img/swift/push_story/enable_background_mode.png %})

#### 앱 그룹 추가

또한 Xcode의 **'서명&기능'** 패널에서 메인 앱 타깃과 알림 콘텐츠 확장 타깃에 앱 그룹 기능을 추가하십시오. 그런 다음 **+** 버튼을 클릭합니다. 앱의 번들 ID를 사용하여 앱 그룹을 만드십시오. 예를 들어 앱의 번들 ID가 `com.company.appname`인 경우 앱 그룹 이름을 `group.com.company.appname.xyz`로 지정할 수 있습니다.
...
또한, 동일한`Info.plist`파일에 다음 최상위`Braze`사전(dictionary)을 추가하고, [단계 2](#enable-capabilities)에서 생성한 앱 그룹으로  를 `REPLACE_WITH_APPGROUP`대체하십시오:
...
### Step 6: 주요 앱 {#update-braze}에서 Braze 통합 업데이트
