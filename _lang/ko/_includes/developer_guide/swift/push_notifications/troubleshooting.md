...
- 사용자가 다른 기기에 앱을 설치하면 동일한 방식으로 또 다른 토큰이 생성되고 캡처됩니다. 
- 사용자가 앱을 재설치하면 새 토큰이 생성되어 Braze에 전달됩니다. 그러나 원본 토큰은 여전히 APN과 Braze에서 유효한 것으로 기록될 수 있습니다.
- 사용자가 앱을 제거해도 Braze는 이 사실을 즉시 알리지 않으며, 토큰은 APN이 폐기할 때까지 유효한 것으로 표시됩니다. 
- 어느 시점이 되면 APN은 오래된 토큰을 폐기할 것입니다. Braze는 이에 대한 통제권이나 가시성을 가지고 있지 않습니다. 

### 3단계: Braze 푸시 캠페인 시작

푸시 캠페인이 시작되면 Braze는 APN에 메시지 전달을 요청합니다. 특히, **사용자의 가장 최근 기기로 보내기를** 선택하지 않으면 현재 유효한 각 푸시 토큰에 대해 요청이 APN으로 전달됩니다. 사용자가 다음과 같은 이유로 실제 메시지를 받지 못했을 수도 있지만, Braze가 APN으로부터 성공적인 응답을 받으면 고객 프로필에 성공적인 전달로 기록됩니다:
- 기기의 전원이 꺼져 있습니다.
...
{% alert note %}
APN은 토큰 무효화 이벤트를 즉시 보고하지 않기 때문에 토큰이 등록되지 않더라도 처음에는 성공 상태를 반환하는 것이 정상입니다. APN은 사용자 개인정보를 보호하고 앱 제거 추적을 방지하기 위해 무작위 일정에 따라 유효하지 않은 토큰에 대해 `410` 상태 반환을 의도적으로 지연시킵니다. APN이 `410` 상태를 반환할 때까지 등록되지 않은 토큰에 알림을 계속 보낼 수 있습니다.
{% endalert %}
