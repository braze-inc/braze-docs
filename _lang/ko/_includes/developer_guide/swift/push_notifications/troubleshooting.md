...
- 사용자가 다른 기기에 앱을 설치하면, 또 다른 토큰이 생성되어 같은 방식으로 캡처됩니다. 
- 사용자가 앱을 재설치하면, 새로운 토큰이 생성되어 Braze에 전달됩니다. 그러나 원래의 토큰은 여전히 APNs와 Braze에 의해 유효한 것으로 기록될 수 있습니다.
- 사용자가 앱을 제거하면, Braze는 즉시 이 사실을 통보받지 않으며, 토큰은 APNs에 의해 폐기될 때까지 여전히 유효한 것으로 나타납니다. 
- 어느 시점에서 APNs는 오래된 토큰을 폐기합니다. Braze는 이에 대한 제어권이나 가시성을 가지고 있지 않습니다. 

### 3단계: Braze 푸시 캠페인 시작

푸시 캠페인이 시작되면 Braze는 APN에 메시지 전달을 요청합니다. 특히, 요청은 **사용자의 가장 최근 기기로 전송**이 선택되지 않는 한, 현재 유효한 푸시 토큰마다 APNs에 전달됩니다. Braze가 APNs로부터 성공적인 응답을 받으면, 사용자 프로필에 성공적인 전달을 기록하지만, 사용자가 실제 메시지를 받지 못했을 수 있는 이유는 다음과 같습니다:
- 그들의 기기가 꺼져 있습니다.
- 그들의 기기가 인터넷(와이파이 또는 셀룰러)에 연결되어 있지 않습니다.
- 그들이 최근에 앱을 제거했습니다.
...
{% alert note %}
APNs는 토큰이 등록 해제되더라도 처음에는 성공 상태를 반환하는 것이 일반적입니다. APNs는 즉시 토큰 무효화 이벤트를 보고하지 않기 때문입니다. APNs는 사용자 프라이버시를 보호하고 앱 제거 추적을 방지하기 위해 무효 토큰에 대한 `410` 상태를 무작위 일정으로 반환하는 것을 의도적으로 지연시킵니다. APNs가 `410` 상태를 반환할 때까지 등록 해제된 토큰에 안전하게 알림을 계속 보낼 수 있습니다.
{% endalert %}
...
모든 채널에 걸친 포괄적인 문제 해결을 위해—유니버설 링크, 커스텀 스킴, 이메일 및 Branch와 같은 제3자 제공업체를 포함하여—[딥링킹 문제 해결]({{site.baseurl}}/developer_guide/push_notifications/deep_linking_troubleshooting)을 참조하십시오.

### 푸시 클릭으로 인한 웹 링크가 열리지 않음

푸시 알림의 링크를 웹 보기에서 열려면 ATS를 준수해야 합니다. 웹 링크가 HTTPS를 사용하는지 확인합니다. 자세한 내용은 [ATS 준수]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/advanced_use_cases/linking/#app-transport-security-ats)를 참조하십시오.

### 푸시 클릭으로 인한 딥링크가 열리지 않음

딥링크를 처리하는 대부분의 코드는 푸시 오픈도 처리합니다. 먼저 푸시 오픈이 로깅되고 있는지 확인합니다. 그렇지 않다면, 그 문제를 해결하십시오(수정이 종종 링크 처리를 수정합니다).
