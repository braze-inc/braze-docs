...
스위프트 패키지 매니저(SPM), CocoaPods 또는 수동 통합 방법을 사용하여 Braze 스위프트 SDK를 통합하고 사용자 정의할 수 있습니다. 다양한 SDK 기호에 대한 자세한 정보는 [Braze Swift 참조 설명서](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/)를 참조하십시오.

### 필수 조건

시작하기 전에, [최신 Braze Swift SDK 버전](https://github.com/braze-inc/braze-swift-sdk#version-information)이 지원되는지 확인하십시오.

### 1단계: Braze 스위프트 SDK 설치

Braze 스위프트 SDK를 설치하기 위해 [스위프트 패키지 매니저(SwiftPM)](https://swift.org/package-manager/) 또는 [CocoaPods](http://cocoapods.org/)를 사용하는 것을 권장합니다. 대안으로, SDK를 수동으로 설치할 수 있습니다.
...
전체 안내서는 CocoaPods의 [시작하기 가이드](https://guides.cocoapods.org/using/getting-started.html)를 참조하십시오. 그렇지 않으면, 빠르게 시작하기 위해 다음 명령을 실행할 수 있습니다:

```bash
$ sudo gem install cocoapods
```

문제가 발생하면 CocoaPods의 [문제 해결 가이드](http://guides.cocoapods.org/using/troubleshooting.html)를 확인하십시오.

#### 1.2단계: Podfile 구성

다음으로, Xcode 프로젝트 디렉토리에 `Podfile`라는 파일을 만드십시오.
...
![Braze 예제 폴더가 새 \`BrazeExample.workspace\`를 보여주도록 확장되었습니다.]({% image_buster /assets/img/braze_example_workspace.png %})
...
!["Braze 라이브러리가 'Embed & Sign.'으로 설정된 예제 Xcode 프로젝트입니다."]({% image_buster /assets/img/swift/sdk_integration/embed-and-sign.png %})

{% alert note %}
Swift SDK 12.0.0부터는 Braze XCFrameworks의 정적 및 동적 변형 모두에 대해 항상 **Embed & Sign**을 선택해야 합니다. 이것은 프레임워크 리소스가 앱 번들에 올바르게 포함되도록 보장합니다.
{% endalert %}

{% alert tip %}
GIF 지원을 활성화하려면 `SDWebImage.xcframework`를 추가하세요. 이는 `braze-swift-sdk-prebuilt/static` 또는 `braze-swift-sdk-prebuilt/dynamic`에 위치합니다.
{% endalert %}
...
{% alert note %}
이것은 Braze의 푸시 알림에만 적용됩니다. 다른 푸시 알림은 시스템 대리자에 의해 정상적으로 처리됩니다.
{% endalert %}
...
지연 초기화를 사용할 때 푸시 알림 자동화가 암묵적으로 활성화됩니다. [푸시 자동화](#swift_step-23-customize-push-automation-optional) 구성을 `pushAutomation` 매개변수를 전달하여 사용자 정의할 수 있습니다.
...
푸시 분석을 명시적으로 큐에 추가하려면(기본 동작) `.queue`을 `analyticsBehavior` 매개변수에 전달하세요. 초기화 전에 대기 중인 푸시 분석 이벤트는 초기화 시 서버로 처리되고 전송됩니다.
...
SDK 초기화 전에 수신된 푸시 분석을 삭제하려면 `.drop`을 `analyticsBehavior` 매개변수로 전달하십시오. 이 옵션을 사용하면 SDK가 초기화되지 않은 동안 발생하는 모든 푸시 분석 이벤트는 무시됩니다.
...
`pushAutomation` 매개변수를 전달하여 푸시 자동화 구성을 사용자 정의할 수 있습니다. 기본적으로 모든 자동화 기능이 활성화되어 있으며 `requestAuthorizationAtLaunch`는 제외됩니다.
...
{% alert note %}
SDK가 초기화되면 모든 대기 중인 푸시 알림, 푸시 토큰 및 딥 링크가 자동으로 처리됩니다.
{% endalert %}

### 3단계: 앱 대리자를 업데이트하세요

{% alert important %}
다음은 기본적으로 생성되지 않는 `AppDelegate`를 프로젝트에 이미 추가했다고 가정합니다. 지연 초기화 기능을 사용하지 않는 경우입니다. `AppDelegate`을 사용할 계획이 없다면, 앱 시작 시 브레이즈 SDK를 가능한 한 빨리 초기화하십시오. 지연 초기화 기능을 사용하는 경우, SDK 초기화에 대한 [2.4단계](#swift_step-24-initialize-the-sdk)를 참조하고 이 단계를 무시하십시오.
{% endalert %}
...
모든 플랫폼에서 중앙 집중식 개요를 보려면 [상세 로깅]({{site.baseurl}}/developer_guide/sdk_integration/verbose_logging)을 참조하십시오. 로그 출력을 해석하는 방법을 배우려면 [상세 로그 읽기]({{site.baseurl}}/developer_guide/sdk_integration/reading_verbose_logs)를 참조하십시오.

#### 로그 레벨

브레이즈 스위프트 SDK의 기본 로그 수준은 `.error`입니다. 로그가 활성화될 때 최소 지원 수준이기도 합니다. 다음은 로그 수준의 전체 목록입니다:
...
런타임에 `Braze.Configuration` 객체에서 로그 수준을 할당할 수 있습니다. 자세한 사용 내용은 [`Braze.Configuration.Logger`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/configuration-swift.class/logger-swift.class)를 참조하십시오.
