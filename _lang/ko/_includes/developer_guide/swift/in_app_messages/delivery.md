...
소프트웨어 개발 키트의 세션 시작 시맨틱에 대한 자세한 내용은 세션[수명주기를]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/?tab=swift) 참조하세요.

### 기본값 비율 제한

기본적으로 30초에 한 번씩 인앱 메시지를 보낼 수 있습니다.

이를 재정의하려면 Braze 인스턴스가 초기화되기 전에 `triggerMinimumTimeInterval` 속성을 Braze 설정에 추가하세요. 임의의 양의 정수로 설정할 수 있으며 최소 시간 간격(초)을 담당자가 지정합니다. For example:
