...
소프트웨어 개발 키트가 다음 커스텀 이벤트 유형 중 하나를 기록할 때 인앱 메시지가 자동으로 트리거됩니다: `Any Purchase`, `Specific Purchase`, `Session Start`, `Custom Event`, 및 `Push Click`. 참고로  및`Custom Event`  `Specific Purchase`트리거에도 강력한 속성 필터가 포함되어 있습니다.
...
모든 적격 인앱 메시지는 사용자의 세션 시작 시점에 해당 사용자의 기기로 전달됩니다. 소프트웨어 개발 키트가 제공되면 자산을 미리 가져와 트리거 시점에 사용할 수 있도록 하여 표시 지연을 최소화합니다. 트리거 이벤트에 적격 인앱 메시지가 두 개 이상 있는 경우 우선순위가 가장 높은 메시지만 전달됩니다.

소프트웨어 개발 키트의 세션 시작 의미론에 대한 자세한 내용은 [세션 수명 주기를]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/?tab=swift) 참조하십시오.

### 기본값 속도 제한

기본적으로 30초에 한 번씩 인앱 메시지를 보낼 수 있습니다.

이를 재정의하려면 Braze 인스턴스가 초기화되기 전에 Braze 구성에  속성을`triggerMinimumTimeInterval` 추가하십시오. 양의 정수로 설정할 수 있으며, 초 단위의 최소 시간 간격을 나타냅니다. For example:
...
Braze에서 캠페인을 생성할 때 키-값 페어를 설정할 수 있으며`extras`, 인앱 메시징 객체가 이를 활용하여 데이터를 앱으로 전송할 수 있습니다. For example:
...
앱 내 인앱 메시지가 자동으로 트리거되는 것을 방지하려면:
...
또한 인앱 메시지를 실시간으로 표시하려면 앱의  에서  [`present(message:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/brazeinappmessagepresenter/present(message:))메서드를 `inAppMessagePresenter`수동으로 호출하면 됩니다. For example:
