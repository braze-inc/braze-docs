...
더 이상 사용되지 않는 [`Original In-App Messaging`][in_app_message_14] 제품]에 대한 설명서를 참조할 수도 있습니다.
...
{% alert note %}
트리거된 인앱 메시지는 Braze SDK를 통해 기록된 커스텀 이벤트에서만 작동합니다. 인앱 메시지는 API 또는 API 이벤트(예: 구매 이벤트)를 통해 트리거할 수 없습니다. Android로 작업하는 경우 [Android에서 커스텀 이벤트를 로그하는 방법][in_app_message_24]을 참고하세요. iOS로 작업하는 경우 [iOS에서 커스텀 이벤트를 로그하는 방법][in_app_message_25]을 참고하세요.
{% endalert %}
