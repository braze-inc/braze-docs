...
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/cc-campaign-details.png %})

{% elsif include.channel == "banner" %}
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/banners/campaign_details.png %})

{% elsif include.channel == "email" %}
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/campaign_details_email.png %})

{% elsif include.channel == "push" %}
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/campaign_details_push.png %})

{% elsif include.channel == "SMS" %}
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/campaign_details_sms.png %})

{% elsif include.channel == "in-app message" %}
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/campaign_details_iam.png %})
...
![캠페인 세부 정보 패널에는 캠페인 성능/성과를 결정하는 데 사용되는 측정기준에 대한 개요가 표시됩니다.]({% image_buster /assets/img/campaign_details_webhook.png %})
...
![콘텐츠 카드 메시지 성능/성과 분석]({% image_buster /assets/img/cc-message-performance.png %})
...
![이메일 메시지 성능/성과 분석]({% image_buster /assets/img_archive/email_message_performance.png %})
...
![인앱 메시지 성능/성과 분석]({% image_buster /assets/img_archive/iam_message_performance.png %})
...
![푸시 메시지 성능/성과 분석]({% image_buster /assets/img_archive/push_message_performance.png %})

{% elsif include.channel == "SMS" %}
### SMS/MMS/RCS 성능

**SMS/MMS/RCS 성능** 패널에는 다양한 측면에서 메시징이 얼마나 잘 수행되었는지 요약되어 있습니다. 이 패널의 측정기준은 선택한 메시징 채널과 다변량 테스트를 실행하는지 여부에 따라 달라집니다. <i class="fa fa-eye preview-icon"></i> **미리보기** 아이콘을 클릭하여 각 이형 상품 또는 채널에 대한 메시지를 확인할 수 있습니다.

![대조군, 배리언트 1, 배리언트 2에 대한 측정기준 표가 포함된 SMS/MMS/RCS 성능 패널입니다.]({% image_buster /assets/img_archive/sms_message_performance.png %})
...
![대조군, 배리언트 1 및 배리언트 2에 대한 측정기준 표가 포함된 SMS/MMS 성능 패널입니다.]({% image_buster /assets/img/banners/banner_performance.png %})
...
![대조군 및 배리언트 1에 대한 측정기준 표가 포함된 웹훅 성능 패널.]({% image_buster /assets/img/webhook_message_performance.png %})
...
![배리언트 1에 대한 측정기준 표가 포함된 WhatsApp 성능 패널.]({% image_buster /assets/img/whatsapp_message_performance.png %})
...
히트맵을 사용하여 단일 이메일 캠페인에서 다양한 링크가 얼마나 성공적인지 확인할 수 있습니다. **메시지 분석** 섹션에서 **이메일 성능** 패널로 이동하세요. ** & 히트맵 미리** 보기를 선택하면 이메일 캠페인과 히트맵의 미리 보기를 볼 수 있습니다. 대안적으로, 배리언트 이름의 하이퍼링크를 선택하여 히트맵을 볼 수 있습니다.
...
![이메일 캠페인이 포함된 미리보기 및 히트맵 페이지의 예와 총 클릭 수와 함께 링크 별칭 지정 예가 있는 패널입니다.]({% image_buster /assets/img_archive/email_heatmap_example.png %})
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} 동일한 사용자에 대해 여러 번 증가시킬 수 있습니다.</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} 여기에는 Braze가 제공한 탈퇴 링크 클릭이 포함됩니다.</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Impressions' %} 배너의 경우 노출 횟수는 사용자 세션당 한 번만 기록됩니다. 동일한 세션 내에서 동일한 배너가 여러 번 조회된 경우 노출 횟수는 한 번만 기록됩니다.</td>
        </tr>
        <tr>
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-clicks">고유 클릭 수</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Clicks' %} 이는 이메일의 경우 7일 동안 추적되며 측정은 <a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a>. This includes clicks on Braze-provided unsubscribe links. 이 숫자는 5–10% 사이여야 합니다. 10%보다 큰 것은 예외적입니다!
            </td>
        </tr>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#unique-opens">고유 열람</a></td>
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Unique Opens' %} 이메일의 경우 7일 동안 추적됩니다. 이 숫자는 30–40% 사이여야 합니다. 40%보다 큰 것은 예외적입니다!
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="no-split">
                {% multi_lang_include analytics/metrics.md metric='Spam' %} 이 측정기준이 0.08보다 크다면 메시지 문구가 너무 판매적인 것이거나 이메일 주소 수집 방법을 재고해야 한다는 신호일 수 있습니다(서신에 관심이 있는 사람들에게 메시징을 보내고 있는지 확인하기 위해).
            </td>
        </tr>
        <tr>
...
이 측정기준은 지속적인 주기로 재계산되므로 새로운 참여 신호(예: 열기 및 클릭)가 수신되어 모델에 통합되면 시간이 지남에 따라 _예상 실제 열람율_ 값이 변경될 수 있습니다. 실제로 _예상 실제 열람율은_ 캠페인이 활성화되어 있는 동안 매일 계속 업데이트될 수 있습니다.
...
궁극적으로 이메일이 서버를 떠난 후에는 다음에 어떤 일이 발생하는지 파악할 수 없지만, 결과에 영향을 미치는 NHI 관리를 위한 권장 사항은 다음과 같습니다:
...
4. 원클릭 탈퇴 프로세스를 사용하는 경우 사용자를 알림 환경설정을 편집하고 관리할 수 있는 페이지로 안내하는 [환경설정 센터를]({{site.baseurl}}/user_guide/message_building_by_channel/email/preference_center/overview) 만드는 것도 고려해 보세요. 이는 NHI가 실수로 사용자의 구독을 취소할 수 있기 때문에 유용할 수 있습니다.
...
<table>
    ...
    <tbody>
        <tr>
            <td class="no-split"><a href="/docs/user_guide/data_and_analytics/report_metrics/#bounces">반송 수</a></td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Bounces' %} <a href="#bounced-push">반송된 푸시 알림을</a> 참조하세요.</td>
        </tr>
        <tr>
...
푸시 인에이블먼트가 되지 않은 사용자, 즉 가입하거나 옵트인하지 않았고 [포그라운드 푸시 토큰이]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_registration/#push-tokens) 없는 사용자를 식별하는 세그먼트를 만들 수 있습니다. 예를 들어 앱의 탈퇴 수를 확인하려면 다음 세그먼트의 "OR" 조합을 사용하면 됩니다: 

- `Background or Foreground Push Enabled is false`
- `Has Uninstalled`

![세그먼트 빌더 섹션에서 앱에 대해 "앱에 대해 백그라운드 또는 포그라운드 푸시 사용" 필터가 거짓이고 "제거됨" 필터가 선택되어 있습니다.]({% image_buster /assets/img/push_unsub_segment_example.png %})
...
![2021년 2월부터 2022년 5월까지의 이메일에 대한 예시 통계가 포함된 과거 실적 패널의 그래프입니다.]({% image_buster /assets/img/cc-historical-performance.png %})
...
![시간 경과에 따른 키워드 분포의 선 그래프와 옵트인, 옵트아웃, 도움말, 기타, 추가 및 코칭에 대한 선택된 체크박스가 있는 키워드 카테고리 섹션이 포함된 캠페인 수준 SMS/MMS/RCS 키워드 응답 패널을 볼 수 있습니다.]({% image_buster /assets/img/sms/keyword_responses.png %})
...
![선 그래프 아래에는 키워드 카테고리, 응답 분포, 리타겟팅에 대한 열이 있으며, 여기서 키워드 카테고리로 세그먼트를 생성할 수 있는 옵션이 제공됩니다.]({% image_buster /assets/img/sms/keyword_segments.png %})
...
![전환 이벤트 세부 정보 패널입니다.]({% image_buster /assets/img/cc-conversion.png %})
...
![주요 전환 이벤트의 사용자 속성 및 행동에 대한 분석이 포함된 전환 상관관계 패널 - A.]({% image_buster /assets/img/convcorr.png %})
