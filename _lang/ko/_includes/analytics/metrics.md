...
<i>총 노출 횟수</i>는 메시지가 조회된 횟수입니다. Braze는 메시지가 사용자 화면에 보일 때만 노출 횟수를 기록합니다. 예를 들어, 메시지가 페이지 하단에 배치된 경우, 사용자가 아래로 스크롤하여 메시지가 보일 때까지 노출 횟수는 기록되지 않습니다. 사용자가 동일한 메시지를 두 번 보게 되면, 두 번의 노출 횟수로 계산됩니다.
{% endif %}
...
<i>고유 노출 횟수</i>는 특정 캠페인에서 메시지를 본 총 사용자 수입니다. 노출 횟수는 메시지가 사용자 화면에 보일 때만 기록됩니다.
{% endif %}
