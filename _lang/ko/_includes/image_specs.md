...
모든 인앱 메시지의 권장 이미지 크기는 500KB, 최대 이미지 크기는 5MB이며 PNG, JPEG, GIF 파일 유형을 지원합니다. WebP 이미지는 모든 기기나 브라우저에서 지원되는 것은 아니므로 인앱 메시지에 추가하기 전에 WebP 이미지를 PNG 또는 JPEG로 변환하는 것이 좋습니다.
...
| 파일 유형 | PNG, JPEG, GIF<br><br> WebP 이미지 지원은 이메일 클라이언트마다 다릅니다. 안정적인 렌더링을 위해 WebP 이미지를 이메일 메시징에 추가하기 전에 PNG 또는 JPEG로 변환하세요. |
{: .reset-td-br-1 .reset-td-br-2}
