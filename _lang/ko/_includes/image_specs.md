...
모든 인앱 메시지의 권장 이미지 크기는 500KB, 최대 이미지 크기는 5MB이며 PNG, JPEG, GIF 파일 유형을 지원합니다. WebP 이미지는 모든 장치나 브라우저에서 지원되지 않으므로, 인앱 메시지에 추가하기 전에 WebP 이미지를 PNG 또는 JPEG로 변환하는 것이 좋습니다.
...
| 유형 | 종횡비 | 이미지 품질 | 참고 |
| --- | --- | --- | --- |
| Modal(이미지만 해당) | 1:1 | 권장 최대 해상도: 1200 x 2000 px <br> 최소 해상도: 600 x 600 px | 메시지는 대부분의 가로 세로 비율에 맞게 이미지 크기를 조정합니다. 권장 최대 해상도는 3:5 종횡비를 가지며, 이는 최적의 결과를 제공하지 않을 수 있습니다. 더 큰 이미지는 사용 가능하지만, 로드 시간이 길어질 수 있습니다. <br> 이미지에 대한 이상적인 종횡비는 1:1이며, 이 비율을 충족하지 않으면 업로드 중에 경고가 발생할 수 있습니다. 이 경고는 최상의 결과를 위한 제안이며, 더 큰 이미지의 업로드를 방해하지 않습니다. |
...
| 파일 유형 | PNG, JPEG, GIF<br><br> WebP 이미지 지원은 이메일 클라이언트마다 다릅니다. 신뢰할 수 있는 렌더링을 위해, 이메일 메시지에 추가하기 전에 WebP 이미지를 PNG 또는 JPEG로 변환하세요. |
{: .reset-td-br-1 .reset-td-br-2}
