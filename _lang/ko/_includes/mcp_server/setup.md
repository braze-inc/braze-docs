...
> Claude 및 Cursor와 같은 도구를 사용하여 자연어를 통해 Braze 데이터와 상호 작용할 수 있도록 Braze MCP 서버를 설정하는 방법을 알아보세요. 자세한 내용은 [Braze MCP 서버]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/){% endif %}.
...
| MCP 클라이언트 | [클로드](https://claude.ai/), [커서](https://cursor.com/), [구글 제미니 CLI가](https://docs.cloud.google.com/gemini/docs/codeassist/gemini-cli) 공식 지원됩니다. 이러한 클라이언트 중 하나에 대한 계정이 있어야 Braze MCP 서버를 사용할 수 있습니다. |
...
![Braze의 'API 키'는 새로 생성된 API 키와 사용자의 REST API 엔드포인트를 보여줍니다.]({% image_buster /assets/img/mcp_server/get_indentifer_and_endpoint.png %}){: style="max-width:85%;"}
...
{% tabs %}
{% tab Claude %}
[클로드 데스크톱](https://claude.ai/download) 커넥터 디렉토리를 사용하여 MCP 서버를 설정합니다. 

1. Claude Desktop에서 **설정** > **커넥터** > **커넥터 찾아보기** > **데스크톱 확장** 프로그램 > **Braze MCP 서버** > **설치로** 이동합니다.
2. API 키와 기본 URL을 입력합니다.
3. 구성을 저장하고 Claude Desktop을 다시 시작합니다.
...
Braze MCP 서버를 설정한 후 MCP 클라이언트에 테스트 프롬프트를 전송해 보세요. 다른 예제 및 모범 사례는 [Braze MCP 서버 사용]{% if include.section == "user" %}({{site.baseurl}}/user_guide/brazeai/mcp_server/usage/){% elsif include.section == "developer" %}({{site.baseurl}}/developer_guide/mcp_server/usage/){% endif %}.
...
{% tab Gemini CLI %}
![내가 사용할 수 있는 Braze 기능은 무엇인가요? Gemini CLI에서 질문하고 답변합니다.]({% image_buster /assets/img/mcp_server/gemini_cli/what_are_my_available_braze_functions.png %})
{% endtab %}
{% endtabs %}
