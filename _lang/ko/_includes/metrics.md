...
<i>오디언스</i>는 특정 메시지를 수신한 사용자의 비율입니다. 이 수치는 Braze에서 제공됩니다.
{% endif %}
...
<i>예상 실제 열람</i>은 기계 열람이 존재하지 않을 경우 발생했을 고유 열람 수에 대한 추정치이며, Braze의 독자적인 통계 모델을 기반으로 산출됩니다.
{% endif %}

{% if include.metric == "Help" %}
<i>도움말</i>은 사용자가 <a href="https://braze.com/docs/user_guide/message_building_by_channel/sms/keywords/keyword_handling/#default-opt-in-opt-out-keywords">도움말 키워드</a>로 메시지에 답장하여 도움말 자동 응답이 발송된 경우입니다.
{% endif %}

{% if include.metric == "Hard Bounce" %}
<i>하드바운스</i>는 영구적인 전달 오류로 인해 이메일이 수신자에게 전달되지 못하는 경우를 말합니다. 도메인 이름이 존재하지 않거나 수신자를 알 수 없는 경우 하드바운스가 발생할 수 있습니다.
{% endif %}

{% if include.metric == "Soft Bounce" %}
<i>소프트바운스</i>는 수신자의 이메일 주소가 유효함에도 불구하고 일시적인 전달 오류로 인해 이메일이 수신자에게 전달되지 못하는 경우를 말합니다. 소프트바운스는 수신자의 받은편지함이 꽉 찼거나, 서버가 다운되었거나, 메시지가 수신자의 받은편지함에 비해 너무 큰 경우 발생할 수 있습니다.
{% endif %}

{% if include.metric == "Deferral" %}
<i>연기</i>란 이메일이 즉시 전달되지 않았지만, 특정 캠페인에 대한 시도가 중지되기 전에 전달 성공 가능성을 극대화하기 위해 이 일시적인 전달 실패 후 최대 72시간 동안 Braze가 이메일 재전송을 시도하는 것을 말합니다.
{% endif %}

{% if include.metric == "Body Click" %}
Push Stories 알림은 알림을 클릭하면 <i>본문 클릭</i>을 기록합니다. 메시지가 확장되거나 실행 버튼을 클릭한 경우에는 기록되지 않습니다.
{% endif %}

{% if include.metric == "Body Clicks" %}
<i>본문 클릭</i>은 사용자가 버튼(버튼 1, 버튼 2)이 없고 기존 편집기로 작성된 메시지를 클릭하는 경우와 HTML 편집기 또는 드래그 앤 드롭 편집기를 사용하여 작성된 메시지가 인수 없이 <code>brazeBridge.logClick()</code>을 사용하는 경우에 발생합니다.
{% endif %}
...
<i>클릭 후 열람율</i>은 단일 사용자 또는 기계가 한 번 이상 열어본 전달된 이메일의 비율이며, <a href='https://braze.com/docs/user_guide/data_and_analytics/reporting/report_builder/'>보고서 빌더</a>에서만 사용할 수 있습니다.
{% endif %}

{% if include.metric == "Close Message" %}
<i>메시지 닫기</i>는 메시지의 닫기 버튼을 클릭한 총 횟수입니다. 이는 기존 편집기가 아닌 드래그 앤 드롭 편집기에서 만든 인앱 메시지에만 존재합니다.
{% endif %}

{% if include.metric == "Confirmed Deliveries" %}
<i>전달 확인</i>은 이동통신사가 메시지가 대상 전화번호로 전달되었음을 확인한 경우입니다.
{% endif %}

{% if include.metric == "Confidence" %}
<i>신뢰도</i>는 메시지의 특정 배리언트가 대조군보다 성능이 우수하다는 확신의 백분율입니다.
{% endif %}
...
<i>전환 기간</i>이란 메시지를 수신한 후 사용자 행동을 추적하여 전환 이벤트에 귀속시키는 기간(일수)입니다. 이 기간 이후에 발생하는 전환은 전환 이벤트에 귀속되지 않습니다.
{% endif %}

{% if include.metric == "Conversions (B, C, D)" %}
<i>전환(B, C, D)</i>은 주요 전환 이벤트 이후에 추가된 추가 전환 이벤트입니다. Braze 캠페인에서 수신한 메시지와 상호작용하거나 메시지를 본 후 정의된 이벤트가 발생한 횟수입니다.
{% endif %}
...
<i>전달</i>은 수신 서버가 수락한 총 메시지 요청 수입니다. 이는 메시지가 기기에 전달되었다는 것을 의미하지 않으며, 메시지가 서버에 의해 수락되었다는 것만을 의미합니다.
{% endif %}

{% if include.metric == "Deliveries %" %}
<i>전달 %</i>는 이메일 수신 가능 대상에게 성공적으로 전송 및 수신된 총 메시지 수(전송)의 백분율입니다.
{% endif %}
...
<i>전송 실패</i>는 대기줄이 넘쳐서 RCS를 전송할 수 없는 경우(RCS 인증 발신자가 처리할 수 있는 속도보다 높은 속도로 RCS를 전송하는 경우)입니다.
{% endif %}

{% if include.metric == "Failed Delivery Rate" %}
<i>전달 실패율</i>은 메시지를 보낼 수 없어 실패한 전송의 백분율입니다. 이는 대기줄 오버플로, 계정 일시 정지, MMS의 경우 미디어 오류 등 다양한 이유로 발생할 수 있습니다.
{% endif %}
...
<i>이메일 가능</i> 수는 이메일 주소가 기록되어 있고 명시적으로 옵트인 또는 구독한 총 사용자 수입니다.
{% endif %}

{% if include.metric == "Errors" %}
<i>오류 수</i>는 웹훅 이벤트가 반환한 오류의 수입니다(전송 프로세스 중에 증가).
{% endif %}

{% if include.metric == "Failures" %}
<i>실패</i>는 인터넷 서비스 공급자가 하드바운스를 반환하여 WhatsApp 메시지를 보낼 수 없는 경우입니다. 하드바운스는 영구적인 전달 가능성 실패를 의미합니다.
{% endif %}

{% if include.metric == "Influenced Opens" %}
<i>영향받은 열람</i>은 푸시 알림이 전송된 후 푸시를 직접 열지 않고 앱을 실행한 사용자의 총 수(및 백분율)입니다.
{% endif %}

{% if include.metric == "Lifetime Revenue" %}
<i>생애주기 매출</i>은 시작 이후 수신된 <code>PurchaseEvents</code> 가격 값(USD)의 총합입니다.
{% endif %}

{% if include.metric == "Lifetime Value Per User" %}
<i>사용자당 생애주기 가치</i>는 <i>생애주기 매출</i>을 총 <i>사용자</i> 수로 나눈 값입니다(홈 페이지에 위치).
{% endif %}

{% if include.metric == "Average Daily Revenue" %}
<i>일일 평균 매출</i>은 해당 일의 캠페인과 캔버스 매출 합계의 평균입니다.
{% endif %}

{% if include.metric == "Daily Purchases" %}
<i>일일 구매</i>는 해당 기간 동안의 총 고유 <code>PurchaseEvents</code>의 평균입니다.
{% endif %}
...
<i>열람</i>은 <i>직접 열람</i>과 <i>영향받은 열람</i>을 모두 포함하는 인스턴스로, Braze SDK가 독자적인 알고리즘을 사용하여 푸시 알림으로 인해 사용자가 앱을 열었다고 판단한 경우입니다.
{% endif %}

{% if include.metric == "Opt-Out" %}
<i>옵트아웃</i>은 사용자가 <a href="https://braze.com/docs/user_guide/message_building_by_channel/sms/keywords/keyword_handling/#default-opt-in-opt-out-keywords">옵트아웃 키워드</a>로 메시지에 답장하여 SMS 또는 RCS 프로그램에서 수신 거부된 경우입니다.
{% endif %}
...
<i>주요 전환(A)</i> 또는 <i>주요 전환 이벤트</i>는 Braze 캠페인에서 메시지를 수신하거나 본 후 정의된 이벤트를 수행한 고유 사용자 수입니다. 이 이벤트는 캠페인을 설정할 때 선택하며, 보고 및 최적화를 위한 주요 성공 지표로 사용됩니다.
{% endif %}

{% if include.metric == "Reads" %}
<i>읽음</i>은 사용자가 메시지를 읽은 경우입니다. Braze에서 읽음을 추적하려면 사용자의 읽음 확인이 "켜짐" 상태여야 합니다.
{% endif %}

{% if include.metric == "Read Rate" %}
<i>읽음률</i>은 읽음으로 이어진 전송의 백분율입니다. 읽음 확인을 켠 사용자에게만 제공됩니다.
{% endif %}
...
<i>보낸 메시지</i> 수는 캠페인에서 보낸 총 메시지 수입니다. 예약된 캠페인을 시작한 후, 이 측정기준에는 사용량 제한으로 인해 아직 전송되지 않은 메시지를 포함하여 전송된 모든 메시지가 포함됩니다. 이는 메시지가 기기에 수신 또는 전달되었음을 의미하는 것이 아니라 메시지가 전송되었다는 의미입니다.
{% endif %}

{% if include.metric == "Sent" %}
<i>전송</i>은 캠페인 또는 캔버스 단계가 시작되거나 트리거되어 Braze에서 SMS 또는 RCS가 전송될 때마다 발생합니다. 오류로 인해 SMS 또는 RCS가 사용자의 기기에 도달하지 못했을 수 있습니다.
{% endif %}

{% if include.metric == "Sends" %}
<i>전송</i> 수는 캠페인에서 보낸 총 메시지 수입니다. 예약된 캠페인을 시작한 후, 이 측정기준에는 사용량 제한으로 인해 아직 전송되지 않은 메시지를 포함하여 전송된 모든 메시지가 포함됩니다. 이는 메시지가 기기에 수신 또는 전달되었음을 의미하는 것이 아니라 메시지가 전송되었다는 의미입니다.
{% endif %}

{% if include.metric == "Sends to Carrier" %}
<i>이동통신사로 전송</i>은 더 이상 사용되지 않지만, 이미 사용 중인 사용자에게는 계속 지원됩니다. 이동통신사에서 전달 또는 거부가 확인되지 않은 <i>전달 확인</i>, <i>거부</i> 및 <i>전송</i>의 합계입니다. 일부 이동통신사는 전달 확인을 제공하지 않거나 전송 시점에 확인을 제공할 수 없기 때문에, 이동통신사가 전달 또는 거부 확인을 제공하지 않는 경우도 포함됩니다.
{% endif %}

{% if include.metric == "Sends to Carrier Rate" %}
<i>이동통신사로 전송 비율</i>은 전송된 전체 메시지 중 <i>이동통신사로 전송</i>으로 분류된 메시지의 비율입니다. 일부 이동통신사는 전달 확인을 제공하지 않거나 전송 시점에 확인을 제공할 수 없기 때문에, 이동통신사가 전달 또는 거부 확인을 제공하지 않는 경우도 포함됩니다. 이 측정기준은 더 이상 사용되지 않지만 이미 해당 측정기준을 보유한 사용자에게는 계속 지원됩니다.
{% endif %}

{% if include.metric == "Spam" %}
<i>스팸</i>은 수신자가 '스팸'으로 표시한 총 전달된 이메일 수입니다. Braze는 이러한 사용자의 구독 상태를 변경하지 않지만, '탈퇴한 사용자를 포함한 모든 사용자에게 보내기'로 구성된 트랜잭션 이메일을 보내지 않는 한 향후 이메일에서 해당 사용자는 자동으로 제외됩니다.
{% endif %}
...
<i>총 닫기</i> 횟수는 캠페인의 콘텐츠 카드가 닫기된 횟수입니다.
{% endif %}
...
<i>고유 클릭 수</i>는 메시지 내의 링크를 최소한 한 번 클릭한 수신자의 고유 수이며, <a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a>로 측정됩니다.
{% endif %}

<!-- Pull channels like Banners that don't have a Dispatch ID-->
{% if include.metric == "Unique Clicks No Dispatch ID" %}
<i>고유 클릭 수</i>는 메시지 내의 링크를 최소한 한 번 클릭한 수신자의 고유 수입니다.
{% endif %}

{% if include.metric == "Unique Dismissals" %}
<i>고유 닫기</i> 횟수는 캠페인에서 콘텐츠 카드를 닫은 고유 수신자 수입니다. 사용자가 캠페인에서 콘텐츠 카드를 여러 번 닫더라도 하나의 고유 닫기로 계산됩니다.
{% endif %}
...
<i>고유 수신자</i>는 일일 고유 수신자 수, 즉 하루에 새 메시지를 받은 사용자의 수입니다. 사용자에 대해 이 횟수가 두 번 이상 증가하려면 해당 사용자가 다른 날에 새 메시지를 수신해야 합니다.
{% endif %}
...
<i>수신 거부자</i> 또는 <i>수신 취소</i>는 수신 거부로 이어진 메시지 수입니다. 수신 거부는 사용자가 Braze 수신 거부 링크를 클릭하면 발생합니다.
{% endif %}
...
<i>배리언트</i>는 작성자가 정의한 대로 서로 다른 캠페인 배리언트의 수입니다.
{% endif %}