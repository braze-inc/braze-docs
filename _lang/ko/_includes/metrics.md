...
<i>직접 열람</i>은 해당 푸시에서 직접 열람한 푸시 알림의 총 개수(및 백분율)입니다.
{% endif %}
...
<i>사용자당 생애주기 가치</i>는 특정 날짜의 캠페인과 캔버스 수익의 합계를 평균한 값입니다.
{% endif %}

{% if include.metric == "Average Daily Revenue" %}
<i>평균 일일 수익은</i> <i>평생 수익을</i> 총 <i>사용자</i> 수(홈 페이지에 있음)로 나눈 값입니다.
{% endif %}
...
<i>스팸</i>은 "스팸"으로 표시된 전달된 이메일의 총 개수입니다. Braze는 이메일을 스팸으로 표시한 사용자의 구독을 자동으로 취소하며, 해당 사용자는 향후 이메일의 타겟이 되지 않습니다.
{% endif %}
...
<i>총 클릭</i> 수는 동일한 사용자가 여러 번 클릭했는지 여부와 관계없이 전달된 메시지 내에서 클릭한 사용자의 총 수(및 백분율)입니다.
{% endif %}

{% if include.metric == "Total Dismissals" %}
<i>총 닫기</i> 횟수는 캠페인에서 콘텐츠 카드가 닫기된 횟수입니다. 사용자가 메시지를 두 번 닫으면 한 번만 계산됩니다.
{% endif %}
...
<i>고유 클릭</i> 수는 메시지 내에서 한 번 이상 클릭한 수신자의 고유한 수이며 <a href='https://braze.com/docs/help/help_articles/data/dispatch_id/'>dispatch_id</a>로 측정됩니다.
{% endif %}

{% if include.metric == "Unique Dismissals" %}
<i>고유 닫기</i> 수는 캠페인에서 콘텐츠 카드를 닫은 사용자 수입니다. 사용자가 캠페인에서 콘텐츠 카드를 여러 번 해제하는 것은 하나의 고유한 해제를 나타냅니다.
{% endif %}
