...
## 이메일용 드래그 & 드롭 편집기의 링크에 대한 HTML 속성
이제 이메일용 드래그 & 드롭 편집기에서 `Image`, `Button` 또는 `Text` 편집기 블록 내의 모든 URL에 [HTML 속성을 추가]({{site.baseurl}}/user_guide/message_building_by_channel/email/drag_and_drop/overview/#creative-details/)할 수 있습니다. 사용자 지정 속성을 사용하면 이메일의 HTML 태그에 추가 정보를 쉽게 추가할 수 있습니다. 이는 메시지 개인화, 세분화 및 스타일링에 특히 유용할 수 있습니다. 
...
## 새로운 비디오 블록
이메일용 드래그 & 드롭 편집기에 새로운 [동영상용]({{site.baseurl}}/user_guide/message_building_by_channel/email/drag_and_drop/dnd_editor_blocks/#video) 콘텐츠 블록이 추가되었습니다.
...
자세한 내용을 알아보려면 고객 성공 관리자에게 문의하세요.
...
- **Dynamic Content Blocks**: Target different promotions and messages to different users. Targeting can be based on either affinity or audience. 동적 수익률은 이메일이 열렸을 때 제공할 개인화된 경험을 결정합니다. 
...
### ThoughtSpot - 분석
[ThoughtSpot과]({{site.baseurl}}/partners/data_and_analytics/business_intelligence/thoughtspot/) Braze의 통합은 회사 사용자가 미리 빌드된 워크시트와 모델 템플릿으로 사용자 행동 분석을 가속화할 수 있도록 ThoughtSpot TML 블록을 활용합니다. This integration enables users to limitlessly search across their Braze interaction data and uncover actionable insights. 
