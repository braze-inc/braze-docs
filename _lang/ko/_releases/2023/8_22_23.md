...
{% include early_access_beta_alert.md feature='이메일별로 사용자를 병합하고 식별자가 일치하지 않는 `/users/merge` 엔드포인트를 사용하는 기능' %}
...
{% include early_access_beta_alert.md feature='Control over card creation in Canvas steps' %}
...
{% include early_access_beta_alert.md feature='Copying campaigns across workspaces' %}
...
{% include early_access_beta_alert.md feature='Push Max' %}

## SDK 업데이트

다음 SDK 업데이트가 릴리스되었습니다. 주요 업데이트는 아래에 나열되어 있으며, 그 외의 모든 업데이트는 해당 SDK 체인지로그를 확인하면 확인할 수 있습니다.

- [Xamarin SDK 1.26.0](https://github.com/braze-inc/braze-xamarin-sdk/blob/master/CHANGELOG.md)
