...
![사용자 가이드 > 채널별 메시지 작성 > 인앱 메시지 > 템플릿 > 간단한 설문조사로 이동하는 이동 경로]({% image_buster /assets/img/doc-breadcrumbs.png %}){: style="max-width:55%"}
...
{% alert important %}
SQL 편집기는 초기 액세스 중입니다. 얼리 액세스에 참여하고 싶으시면 고객 성공 매니저에게 문의하세요.
{% endalert %}
...
## 신규 커런츠 이벤트: users_campaigns_abort 및 users_canvas_abort
...
## Shopify 히스토리컬 백필
[Shopify Historical Backfill]({{site.baseurl}}/partners/ecommerce/shopify_legacy/shopify_features/shopify_backfill)은 Braze가 Shopify 통합 연결 이전 90일 동안의 모든 고객, 주문 및 구매 이벤트를 가져올 수 있도록 합니다.
