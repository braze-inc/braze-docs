...
nav_title: Grouparoo
page_order: 1
description: "이 글에서는 데이터 웨어하우스의 데이터로 마케팅, 영업, 지원 도구를 강화하는 데 사용되는 오픈 소스 리버스 ETL 도구인 Braze와 Grouparoo의 파트너십에 대해 간략하게 설명합니다."
page_type: update
...
> [Grouparoo는](https://www.grouparoo.com/) 데이터 웨어하우스에서 마케팅, 영업, 지원 도구로 데이터를 동기화하는 오픈 소스 리버스 ETL 도구입니다. 모델 중심의 UI를 통해 기술 전문가가 아닌 팀원도 데이터 동기화를 구성하고 예약할 수 있습니다.

Braze와 Grouparoo 통합은 데이터 웨어하우스 데이터를 Braze에 동기화합니다. 자동 동기화 일정은 고객 커뮤니케이션을 최신 정보로 최신 상태로 유지합니다.
...
![예제 속성 매핑 필드. Grouparoo userID는 external_id. 이메일, 이름 및 성은 동등한 "이메일", "first_name", 및 "last_name" 그룹아루 필드에 매핑되도록 설정합니다.]({% image_buster /assets/img/grouparoo/mapping.png %}){: style="max-width:80%;"}
