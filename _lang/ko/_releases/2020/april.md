...
모든 클러스터에서 사용할 수 있는 새로운 Braze Facebook 오디언스 내보내기 프로세스는 명확한 통합 단계로 워크플로우를 간소화합니다. 더 이상 커스텀 오디언스를 전송하기 위해 OAuth 리디렉션 URI를 화이트리스트에 추가하거나 통합을 위해 Facebook 앱 설정을 조정할 필요가 없습니다.
...
[템플릿/이메일/목록]({{site.baseurl}}/api/endpoints/templates/email_templates/get_list_email_templates/) 및 [content_block/list]({{site.baseurl}}/api/endpoints/templates/content_blocks_templates/get_list_email_content_blocks/) API 엔드포인트가 새로운 `tags` 필드를 포함하도록 업데이트되었습니다. 이 필드는 현재 블록 또는 이메일 템플릿에 적용되는 모든 태그를 배열로 나열합니다.
...
![개인화된 From Address]({% image_buster /assets/img/personalized-from-name.png %}){: style="max-width:80%"}
