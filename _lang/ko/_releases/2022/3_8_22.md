...
이제 Braze는 Roku 기기에서 사용자에게 인앱 메시지 전송을 지원합니다! 이 기능은 추가 소프트웨어 개발 키트 구성이 필요하며 기본값으로 제공되지 않는다는 점에 유의하세요. Roku용 인앱 메시지 통합에 대한 자세한 내용은 [Roku 인앱 메시지]({{site.baseurl}}/developer_guide/in_app_messages/?sdktab=roku)를 참조하세요.
...
Braze와 [Grouparoo]({{site.baseurl}}/help/release_notes/deprecations/grouparoo) 통합은 데이터 웨어하우스 데이터를 Braze에 동기화합니다. 자동 동기화 일정은 고객 커뮤니케이션을 최신 정보로 최신 상태로 유지합니다.
