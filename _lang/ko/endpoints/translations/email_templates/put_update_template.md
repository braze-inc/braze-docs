...
> 이 엔드포인트를 사용하여 [이메일 템플릿]({{site.baseurl}}/user_guide/message_building_by_channel/email/templates)에 대한 번역을 업데이트합니다.
...
| `locale_id` | Required | 문자열 | 로캘의 ID입니다. |
| `translations` | Required | 문자열 | 이메일 템플릿에 대한 번역의 맵입니다. |
...
| `Something went wrong. Translation IDs are mismatched or translated text exceeds limits.`| 이것은 포괄적인 오류입니다. 추가 지원이 필요한 경우 [Braze 지원팀에]({{site.baseurl}}/user_guide/administrative/access_braze/support) 문의하세요. |
