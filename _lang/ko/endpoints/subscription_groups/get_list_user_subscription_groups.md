...
> 이 엔드포인트를 사용하여 특정 사용자의 구독 기록과 함께 구독 그룹을 나열하고 가져올 수 있습니다.
...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
|---|---|---|---|
| `external_id`  | Required | 문자열 | 사용자의 `external_id` (최소 1개, 최대 50개 `external_ids`)를 포함해야 합니다. |
...
사용자의 기록에서 구독 상태 업데이트가 있었던 구독 그룹만 성공적인 응답에 포함됩니다. 이는 새로 생성된 구독 그룹이 나열되지 않음을 의미합니다.
