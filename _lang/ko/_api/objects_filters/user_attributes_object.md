...
> 속성 개체에 필드가 있는 API 요청은 지정된 고객 프로필에 지정된 값으로 해당 이름의 속성을 만들거나 업데이트합니다.
...
Braze에서 기존 사용자 프로필만 업데이트하려면 요청 본문에 `true` 값과 함께 `_update_existing_only` 키를 전달해야 합니다. 이 값을 생략하면 `external_id` 주소가 없는 경우 Braze에서 새 고객 프로필을 생성합니다.

{% alert note %}
`/users/track` 엔드포인트를 통해 별칭 전용 사용자 프로필을 생성하는 경우 `_update_existing_only` 을 `false` 으로 설정해야 합니다. 이 값을 생략하면 Braze는 별칭 전용 프로필을 만들지 않습니다.
{% endalert %}
...
* 토큰이 이미 Braze에 존재하는 경우 요청은 무시되며, 그렇지 않으면 각 토큰에 대해 임시 익명 사용자 프로필을 생성하여 해당 개인에게 메시지를 계속 보낼 수 있도록 합니다.

가져오기 후, 각 사용자가 앱의 Braze 지원 버전을 실행하면 Braze는 가져온 푸시 토큰을 자동으로 Braze 사용자 프로필로 이동하고 임시 프로필을 정리합니다.

Braze는 한 달에 한 번씩 푸시 토큰이 없는 `push_token_import` 플래그가 있는 익명 프로필을 확인합니다. 익명 프로필에 더 이상 푸시 토큰이 없는 경우 Braze는 해당 프로필을 삭제합니다. 그러나 익명 프로필에 푸시 토큰이 남아 있어 실제 사용자가 해당 푸시 토큰으로 기기에 아직 로그인하지 않았다는 것을 암시하는 경우, Braze는 아무 조치도 취하지 않습니다.
...
| 데이터 유형 | 참고 |
| --- | --- |
| 배열 | 사용자 지정 속성 배열이 지원됩니다. 사용자 지정 속성 배열에 요소를 추가하면 해당 요소가 이미 존재하지 않는 한 배열의 끝에 추가되며, 이 경우 현재 위치에서 배열의 끝으로 이동합니다.<br><br>예를 들어 `['hotdog','hotdog','hotdog','pizza']` 배열을 가져온 경우 고유 값만 지원되므로 배열 속성에 `['hotdog', 'pizza']` 으로 표시됩니다.<br><br>`"my_array_custom_attribute":[ "Value1", "Value2" ]` 와 같이 배열의 값을 설정하는 것 외에도 `"my_array_custom_attribute" : { "add" : ["Value3"] },` 과 같이 기존 배열에 추가하거나 다음과 같이 배열에서 값을 제거할 수 있습니다. `"my_array_custom_attribute" : { "remove" : [ "Value1" ]}`<br><br>커스텀 속성 배열의 최대 요소 개수는 기본값이 25개이지만 개별 배열의 경우 최대 100개까지 늘릴 수 있습니다. 자세한 내용은 [배열을]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview/#arrays) 참조하세요. |
...
| 부울 | `true` 또는 `false` |
| 날짜 | [ISO 8601](http://en.wikipedia.org/wiki/ISO_8601) 형식 또는 다음 형식 중 하나로 저장해야 합니다: <br>- `yyyy-MM-ddTHH:mm:ss:SSSZ` <br>- `yyyy-MM-ddTHH:mm:ss` <br>- `yyyy-MM-dd HH:mm:ss` <br>- `yyyy-MM-dd` <br>- `MM/dd/yyyy` <br>- `ddd MM dd HH:mm:ss.TZD YYYY` <br><br>'T'는 자리 표시자가 아닌 시간 지정자이므로 변경하거나 제거해서는 안 됩니다. <br><br>표준 시간대가 없는 시간 속성은 기본값이 자정(UTC)으로 설정됩니다(대시보드에서는 회사 표준 시간대의 자정에 해당하는 시간으로 표시됨). <br><br> 타임스탬프가 미래인 이벤트는 기본값이 현재 시간으로 설정됩니다. <br><br> 일반 사용자 지정 속성의 경우, 연도가 0보다 작거나 3000보다 크면 Braze는 이 값을 사용자에게 문자열로 저장합니다. |
...
| 국가 | (문자열) [ISO-3166-1 알파-2 표준에](http://en.wikipedia.org/wiki/ISO_3166-1) 따라 국가 코드를 Braze에 전달해야 합니다. API는 다양한 형식으로 수신된 국가를 매핑하기 위해 최선을 다하고 있습니다. 예를 들어 '오스트레일리아'는 'AU'로 매핑될 수 있습니다. 그러나 입력값이 지정된 [ISO-3166-1 알파-2 표준과](http://en.wikipedia.org/wiki/ISO_3166-1) 일치하지 않으면 국가 값이 `NULL` 으로 설정됩니다. <br><br>CSV 가져오기 또는 API로 사용자에 대해 `country` 을 설정하면 Braze가 소프트웨어 개발 키트를 통해 이 정보를 자동으로 캡처하지 못합니다. |
| current_location | (객체) {"경도": -73.991443, "위도" 형식입니다: 40.753824} |
...
| 이메일 | (문자열) |
| email_subscribe | (문자열) 사용 가능한 값은 "opted_in" (이메일 메시지를 수신하도록 명시적으로 등록됨), "탈퇴됨"(이메일 메시지를 명시적으로 탈퇴함), "구독함"(옵트인하지도 탈퇴하지도 않음)입니다.  |
...
| external_id | (문자열) 사용자 프로필의 고유 식별자입니다. `external_id` 을 할당하면 Braze는 사용자의 기기에서 고객 프로필을 식별합니다. 알 수 없는 사용자 프로필에 external_id 을 처음 할당하는 경우, Braze는 기존의 모든 사용자 프로필 데이터를 새 사용자 프로필로 마이그레이션합니다. |
| Facebook | `id`(문자열), `likes`(문자열 배열), `num_friends`(정수) 중 하나를 포함하는 해시입니다. |
| first_name | (문자열) |
| 성별 | (문자열) "M", "F", "O"(기타), "N"(해당 없음), "P"(말하지 않음) 또는 nil(알 수 없음)입니다. |
| home_city | (문자열) |
| language | (문자열)의 경우 [ISO-639-1 표준에](http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) 따라 해당 언어가 Braze에 전달되어야 합니다. 지원되는 언어에 대해서는 [허용되는 언어 목록을]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/language_codes/) 참조하세요.<br><br>CSV 가져오기 또는 API로 사용자에 대해 `language` 을 설정하면 Braze가 소프트웨어 개발 키트를 통해 이 정보를 자동으로 캡처하지 못합니다. |
| last_name | (문자열) |
...
| push_subscribe | (문자열) 사용 가능한 값은 "opted_in" (푸시 메시지를 수신하도록 명시적으로 등록됨), "탈퇴됨"(푸시 메시지를 명시적으로 탈퇴함), "구독함"(옵트인하거나 탈퇴하지 않음)입니다.  |
...
| time_zone | (문자열) [IANA 표준 시간대 데이터베이스의](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) 표준 시간대 이름(예: "America/New_York" 또는 "동부 표준시(미국 & 캐나다)"). 유효한 시간대 값만 설정됩니다. |
| 트위터 | `id`(정수), `screen_name`(문자열, X(옛 트위터) 핸들), `followers_count`(정수), `friends_count`(정수), `statuses_count`(정수) 중 하나를 포함하는 해시입니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

이 API를 통해 명시적으로 설정한 언어 값은 Braze가 기기로부터 자동으로 수신하는 로캘 정보보다 우선합니다.
...
### 소프트웨어 개발 키트를 통한 자동 마이그레이션

[Braze SDK를 통합하면]({{site.baseurl}}/developer_guide/sdk_integration/) 옵트인한 사용자의 푸시 토큰은 다음에 앱을 열 때 자동으로 마이그레이션됩니다. 그 전까지는 해당 사용자에게 Braze를 통해 푸시 알림을 보낼 수 없습니다.
...
|Consideration|세부 정보|
|----------------------|------------|
| **서비스 작업자**  | `manageServiceWorkerExternally` 또는 `serviceWorkerLocation` 와 같은 다른 옵션을 지정하지 않는 한 기본값으로 웹 소프트웨어 개발 키트는 `./service-worker` 에서 서비스 종사자를 찾습니다. 서비스 워커가 제대로 설정되어 있지 않으면 사용자의 푸시 토큰이 만료될 수 있습니다. |
| **만료된 토큰**   | 사용자가 60일 이내에 웹 세션을 시작하지 않으면 푸시 토큰이 만료됩니다. Braze는 만료된 푸시 토큰을 마이그레이션할 수 없으므로 재참여하려면 [푸시 프라이머를]({{site.baseurl}}/user_guide/message_building_by_channel/push/best_practices/push_primer_messages) 보내야 합니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation"}
...
다른 시스템에서 푸시 토큰을 가져올 때 `external_id`를 항상 사용할 수 있는 것은 아닙니다. 이 경우 `push_token_import` 플래그를 `true`로 설정하고 `app_id` 및 `token` 값을 지정합니다. Braze는 각 토큰에 대해 임시 익명 사용자 프로필을 생성하여 이러한 개인에게 메시지를 계속 보낼 수 있도록 합니다. 토큰이 이미 Braze에 존재하는 경우 요청은 무시됩니다.
...
사용자 가져오기 후 익명 사용자가 앱의 Braze 지원 버전을 실행하면 Braze는 가져온 푸시 토큰을 자동으로 Braze 고객 프로필로 이동하고 임시 프로필을 정리합니다.

Braze는 한 달에 한 번씩 푸시 토큰이 없는 `push_token_import` 플래그가 있는 익명 프로필을 확인합니다. 익명 프로필에 더 이상 푸시 토큰이 없는 경우 Braze는 해당 프로필을 삭제합니다. 그러나 익명 프로필에 푸시 토큰이 남아 있어 실제 사용자가 아직 해당 푸시 토큰으로 기기에 로그인하지 않았다는 것을 시사하는 경우 Braze는 아무 조치도 취하지 않습니다.
{% endtab %}
{% endtabs %}
...
{% alert note %}
일부 푸시 알림 제공업체의 경우, Braze는 키-값 페어를 평평하게 만들어야 제대로 해석할 수 있습니다. 특정 Android 앱의 키-값 페어를 평평하게 하려면 고객 성공 매니저에게 문의하세요.
{% endalert %}