...
Braze는 시간이 지남에 따라 발신자와 서버가 변경될 수 있으므로 일관되고 예측 가능한 동작을 위해 `request_headers` 필드에서 `Content-Type`에 대한 명시적인 가치를 제공하는 것이 좋습니다. `Content-Type` 헤더에 값을 지정하지 않으면 시스템에서 요청 본문에서 값을 유추합니다.
