...
{% alert important %}
**미디어 전송 제한:** 미디어 전송(설명서, 동영상 및 기타 미디어 유형)은 `messages/send` API에서 지원되지 않습니다. API를 통해 전송되는 템플릿 메시지에는 텍스트 및 이미지 헤더 유형만 지원됩니다. WhatsApp 템플릿에 문서, 동영상 또는 기타 미디어 유형 헤더를 사용하는 경우 `messages/send` API를 사용하여 보낼 수 없습니다. [캠페인 트리거 API]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns/) 또는 Braze 대시보드를 사용하여 미디어 헤더가 포함된 템플릿을 전송하세요.
{% endalert %}
...
{% alert note %}
**헤더 유형 요구 사항:** `header_variables` 은 텍스트형 헤더가 있는 템플릿에만 사용할 수 있습니다. 이미지 헤더의 경우 `header_image_uri` 대신 를 사용합니다. 문서, 비디오 및 기타 미디어 헤더 유형은 `messages/send` API에서 지원되지 않습니다.
{% endalert %}
