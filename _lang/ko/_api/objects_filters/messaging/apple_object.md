...
{% alert note %}
Apple 푸시 객체를 라이브 활동 페이로드의 일부로 전송하는 경우 `alert` 객체에 `sound` 문자열을 포함해야 합니다.
{% endalert %}
...
iOS 푸시 액션 버튼을 사용하려면 Apple 푸시 개체에 `category` 필드를 포함해야 합니다. `category` 필드를 포함하면 관련된 모든 푸시 실행 버튼이 표시되며, 버튼의 개별 클릭 동작을 추가로 정의하려는 경우에만 `buttons` 필드를 포함합니다. Braze SDK는 다음 표와 같이 사용할 수 있는 기본값 푸시 실행 버튼 세트를 제공합니다. 앱에 등록되어 있는 경우 직접 만든 버튼을 사용할 수도 있습니다.
