...
> 연결된 오디언스 개체는 메시징의 오디언스를 식별합니다. 예를 들어 커스텀 속성 배열을 사용하여 사용자가 즐겨찾기를 지정한 모든 카테고리와 TV 프로그램을 추적하는 경우, 연결된 오디언스를 사용하면 각 프로그램에 대해 세그먼트를 설정할 필요 없이 새 에피소드가 공개될 때마다 해당 프로그램을 즐겨찾기한 모든 사용자에게 푸시 알림이나 이메일을 자동으로 보낼 수 있습니다.
...
이 필터를 사용하면 사용자가 마지막으로 앱을 사용한 시간을 기준으로 세그먼트를 세분화할 수 있습니다. 이 필터에는 두 개의 필드가 포함되어 있습니다:
