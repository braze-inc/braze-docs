...
> 연결된 오디언스 객체는 메시지의 오디언스를 식별자로 식별합니다. 예를 들어, 사용자별 커스텀 속성 배열을 활용해 사용자가 즐겨찾기한 모든 카테고리와 TV 프로그램을 추적한다면, 연결된 오디언스를 통해 새 에피소드가 공개될 때마다 해당 프로그램을 즐겨찾기한 모든 사용자에게 자동으로 푸시 알림이나 이메일을 발송할 수 있습니다. 각 프로그램별로 세그먼트를 설정할 필요 없이 말이죠.
...
여러 커스텀 속성 필터를`AND`  및`OR`  연산자와 결합하면 연결된 오디언스 필터가 생성됩니다.
...
이 필터를 사용하면 사용자가 앱을 마지막으로 사용한 시점을 기준으로 세그먼트를 만들 수 있습니다. 이 필터에는 두 개의 필드가 포함되어 있습니다:
...
연결된 오디언스는 기본 속성, 사용자 지정 이벤트, 세그먼트 또는 메시지 참여 이벤트를 기준으로 사용자를 필터링할 수 없습니다. 이러한 필터를 사용하려면, 해당 필터를 오디언스 세그먼트에 통합한 후 해당 세그먼트를  [`/messages/send`엔드포인트의]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages#request-parameters)  `segment_id`매개변수에 지정하는 것을 권장합니다. 다른 엔드포인트를 사용할 때는 먼저 Braze 대시보드에서 API 트리거 캠페인 또는 캔버스에 세그먼트를 추가해야 합니다.
