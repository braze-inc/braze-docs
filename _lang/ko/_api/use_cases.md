...
이 작업을 수행하기 위해 개발자 팀은 `email.spam.remove` 권한이 있는 API 키가 필요하며 `/email/spam/remove` 엔드포인트를 사용해야 합니다. 이 엔드포인트는 Braze 스팸 목록과 MovieCanon의 이메일 제공자가 유지하는 스팸 목록에서 이메일 주소를 제거합니다.
...
이 페이로드를 성공적으로 전송한 후, 이 응답은 Braze가 MovieCanon의 스팸 목록에서 이메일을 제거했음을 확인합니다.
