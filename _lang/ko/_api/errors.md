...
메시지를 전송하지 않는 [`/users/identify`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify)와 같은 엔드포인트의 경우, 성공 메시지는 Braze가 처리 요청을 수신했음을 의미합니다. 처리 후 별칭에 대한 일치가 없으면 요청이 중단됩니다.
