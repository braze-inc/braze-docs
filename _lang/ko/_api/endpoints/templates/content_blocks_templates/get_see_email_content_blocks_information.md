...
| `include_inclusion_data` | 선택 사항 | 부울 | `true` 로 설정하면 API는 이 콘텐츠 블록이 포함된 캠페인 및 캔버스의 메시지 변형 API 식별자를 반환하여 후속 호출에 사용합니다.  결과에는 보관되거나 삭제된 캠페인 또는 캔버스가 제외됩니다. |
...
| 오류 | 문제 해결 | 문제 해결
| --- | --- |
| `Content Block ID cannot be blank` | 요청에 콘텐츠 블록이 나열되어 있고 따옴표로 묶여 있는지 확인합니다(`""`). |
...
| `Include Inclusion Data—error` | 이 매개변수는 부울 값(참 또는 거짓)만 허용합니다. `include_inclusion_data` 값이 따옴표(`""`)로 묶이지 않았는지 확인하여 값이 문자열로 대신 전송되도록 합니다. 자세한 내용은 [요청 매개변수](#request-parameters)를 참조하세요. |
