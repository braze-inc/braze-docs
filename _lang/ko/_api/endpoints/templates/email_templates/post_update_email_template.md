...
| --------- | ---------| --------- | ----------- |
|`email_template_id`| 필수 | [문자열|이메일 템플릿의 API 식별자]({{site.baseurl}}/api/identifier_types/).|
|`template_name`|선택 사항| 문자열| 이메일 템플릿 이름
|`subject`|선택 사항| 문자열| 이메일 템플릿 제목.|
|`body`|선택 사항|문자|HTML을 포함할 수 있는 이메일 템플릿 본문입니다.|
|`plaintext_body`|선택 사항| 문자열| 이메일 템플릿 본문의 일반 텍스트 버전입니다.|
|`preheader`|선택적| 문자열|일부 클라이언트에서 미리 보기를 생성하는 데 사용되는 이메일 프리헤더입니다.|
|`tags`[|선택적|]({{site.baseurl}}/user_guide/administrative/app_settings/manage_app_group/tags/) 문자열|태그가 이미 존재해야 합니다.|
|`should_inline_css`|선택적|부울|템플릿별로 `inline_css` 기능을 활성화 또는 비활성화합니다. 제공하지 않으면 Braze는 앱그룹에 대한 기본 설정을 사용합니다. `true` 또는 `false` 중 하나가 예상됩니다.
...
다음 표에는 해당되는 경우 반환될 수 있는 오류와 관련 문제 해결 단계가 나열되어 있습니다.

| 오류 | 문제 해결 | 문제 해결
| --- | --- |
| 템플릿 이름은 필수입니다 | 템플릿 이름을 입력합니다. |
| 태그는 배열이어야 합니다 | 태그는 문자열 배열로 형식을 지정해야 합니다(예: `["marketing", "promotional", "transactional"]`. |
| 모든 태그는 문자열이어야 합니다 | 태그가 따옴표(`""`)로 묶여 있는지 확인합니다. |
| 일부 태그를 찾을 수 없습니다 | 이메일 템플릿을 만들 때 태그를 추가하려면 해당 태그가 이미 Braze에 존재해야 합니다. |
| `should_inline_css` 에 대한 잘못된 값입니다. `true` 또는 `false` 중 하나가 예상되었습니다 | 이 매개변수는 부울 값(true 또는 false)만 허용합니다. `should_inline_css` 값이 따옴표(`""`)로 묶이지 않았는지 확인하여 값이 문자열로 대신 전송되도록 합니다. |
