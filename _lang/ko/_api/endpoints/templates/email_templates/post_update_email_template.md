...
이메일 템플릿의 `email_template_id`에 접근하려면 **템플릿 & 미디어** 페이지로 이동하세요. [이메일 템플릿 만들기 엔드포인트는]({{site.baseurl}}/api/endpoints/templates/email_templates/post_create_email_template/) `email_template_id` 참조도 반환합니다.
