...
> 이 엔드포인트를 사용하여 번역된 캠페인 메시지를 보고 이 메시지가 사용자에게 어떻게 보이는지 확인할 수 있습니다.

{% alert important %}
API를 통해 번역된 캠페인 메시지를 보는 기능은 현재 얼리 액세스 중입니다. 얼리 액세스에 참여하려면 Braze 계정 매니저에게 문의하세요.
{% endalert %}
...
모든 번역 ID는 보편적인 고유 식별자(UUID)로 간주되며, 이는 **다국어 지원** 설정 또는 요청 응답에서 찾을 수 있습니다.
...
이 엔드포인트에는 네 가지 상태 코드 응답이 있습니다: `200`, `400`, `404`, 및 `429`.
