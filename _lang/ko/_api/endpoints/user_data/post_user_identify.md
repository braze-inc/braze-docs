...
`external_id` 을 가진 사용자가 없는 경우 `external_id` 이 사용자 별칭 지정 기록에 추가되며 해당 사용자는 식별된 것으로 간주됩니다. 사용자는 특정 라벨에 대해 하나의 별칭만 지정할 수 있습니다. 사용자가 이미 `external_id` 에 존재하고 별칭 전용 프로필과 동일한 라벨을 가진 기존 별칭이 있는 경우 사용자 프로필이 결합되지 않습니다.
...
기본값으로 이 엔드포인트는 익명 **사용자에게만** 있는 다음 필드 목록을 식별된 사용자에게 병합합니다.
...
- 앱이 두 사용자 프로필에 모두 존재하는 경우 세션 데이터
  - 예를 들어, 타겟 사용자에게는 'ABCApp'에 대한 앱 요약이 없지만 원래 사용자에게는 있는 경우, 병합 후 타겟 사용자의 프로필에 'ABCApp' 앱 요약이 표시됩니다.
{% enddetails %}
