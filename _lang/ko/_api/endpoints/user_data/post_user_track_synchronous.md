...
동기 호출에서 API는 상태 코드 `201`을 반환하여 요청이 성공적으로 수신되고 이해되었으며 수락되고 완료되었음을 나타냅니다. 호출 응답은 작업의 결과로 선택된 사용자 프로필 필드를 보여줍니다.
...
동기 호출에서는 API가 Braze가 요청을 완료할 때까지 응답을 반환하지 않습니다. 그 결과, 동기 요청은 평균적으로 `/users/track`에 대한 비동기 요청보다 더 오래 걸립니다. 대부분의 요청에 대해 몇 초 내에 응답을 받을 수 있습니다.
