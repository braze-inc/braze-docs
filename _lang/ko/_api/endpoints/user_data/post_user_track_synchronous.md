---
nav_title: "POST: 사용자 만들기 및 업데이트(동기식)"
article_title: "POST: 사용자 만들기 및 업데이트(동기식)"
...
{% alert important %}
이 엔드포인트는 현재 **제한된 베타** 버전입니다. 지금은 베타 버전에 신규 고객을 추가하고 있지 않지만, 이 기능이 Braze 통합에 유용하다고 생각되면 계정 매니저에게 알려주세요.
{% endalert %}
...
동기 호출에서 API는 요청이 성공적으로 수신, 이해, 수락 및 완료되었음을 나타내는 상태 코드 `201` 를 반환합니다. 통화 응답에는 작업의 결과로 선택된 고객 프로필 필드가 표시됩니다.
...
메시징이 성공하면 다음과 같은 응답이 반환되며, 여기에는 Braze가 업데이트한 사용자 프로필 데이터에 대한 정보가 포함됩니다.
...
대부분의 프로필 업데이트의 경우 속도 제한이 높고 요청을 일괄 처리할 수 있는 유연성이 있는 `/users/track` 엔드포인트가 가장 적합합니다. 그러나 `/users/track/sync` 엔드포인트는 동일한 사용자에 대한 빠른 연속 요청으로 인해 경합 조건이 발생하는 경우 유용합니다.

### 응답 시간이 `/users/track` 엔드포인트와 다른가요?

동기 호출을 사용하면 API는 Braze가 요청을 완료할 때까지 기다렸다가 응답을 반환합니다. 결과적으로 동기식 요청은 `/users/track` 에 대한 비동기식 요청보다 평균적으로 더 오래 걸립니다. 대부분의 요청에 대해 몇 초 내에 응답을 받을 수 있습니다.
