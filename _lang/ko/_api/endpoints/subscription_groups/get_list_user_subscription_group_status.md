...
| `external_id` | 필수* | 문자열 | 사용자의 `external_id`(최소 1개에서 최대 50개까지 `external_ids`를 포함해야 합니다). <br><br>`external_id` 및 `email`/`phone`을 모두 제출하면 제공된 `external_id`만 결과 쿼리에 적용됩니다. |
| `email` | 필수* | 문자열 | 사용자의 이메일 주소입니다. 최대 50개의 문자열 배열로 전달할 수 있습니다.<br><br> 이메일 주소와 전화번호를 모두 제출하면( `external_id` 없이) 오류가 발생합니다. |
| `phone` | 필수* | [E.164](https://en.wikipedia.org/wiki/E.164) 형식의 문자열 | 사용자의 전화 번호입니다. 이메일이 포함되지 않은 경우 전화번호를 하나 이상 포함해야 합니다(최대 50개까지).<br><br> 이메일 주소와 전화번호를 모두 제출하면( `external_id` 없이) 오류가 발생합니다. |
