...
|---|---|---|---|
| `catalogs` | 필수 | 배열 | 카탈로그 개체가 포함된 배열입니다. 이 요청에는 하나의 카탈로그 개체만 허용됩니다. |
...
| 매개변수 | 필수 | 데이터 유형 | 설명 | 설명
|---|---|---|---|
| `name` | 필수 | 문자열 | 만들려는 카탈로그의 이름입니다. |
| `description` | 필수 | 문자열 | 만들려는 카탈로그에 대한 설명입니다. |
| `fields` | 필수 | 배열 | 개체에 키 `name` 및 `type` 가 포함된 개체의 배열입니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4}
...
| 오류 | 문제 해결 | 문제 해결
| --- | --- |
| `catalog-array-invalid` | `catalogs` 은 객체의 배열이어야 합니다. |
...
| `id-not-first-column` | `id`는 배열의 첫 번째 필드여야 합니다. 유형이 문자열인지 확인합니다. |
...
| `invalid-fields` | `fields` 형식이 올바르지 않습니다. |
