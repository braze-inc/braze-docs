...
`content-available`이 설정되지 않은 경우 기본 Apple 푸시 알림 서비스(APNs) 우선 순위는 10입니다. `content-available`가 설정된 경우 이 우선 순위는 5입니다. 자세한 내용은 [Apple 푸시 객체]({{site.baseurl}}/api/objects_filters/messaging/apple_object)를 참조하십시오.
