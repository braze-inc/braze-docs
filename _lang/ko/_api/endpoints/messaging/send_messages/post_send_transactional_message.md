...
| 매개변수 | 필수 | 데이터 유형 | 설명 |
| --------- | ---------| --------- | ----------- |
|`external_send_id`| Optional | 문자열 |  Base64 호환 문자열입니다. 다음 정규식에 대해 유효성을 검사합니다.<br><br> `/^[a-zA-Z0-9-_+\/=]+$/` <br><br>이 선택적 필드를 사용하면 트랜잭션 HTTP 이벤트 포스트백에서 전송되는 이벤트에 포함되는 특정 전송에 대한 내부 식별자를 전달할 수 있습니다. 이 식별자는 통과되면 중복 제거 키로도 사용되며, 이 키는 Braze에서 24시간 동안 저장됩니다. <br><br>다른 요청에서 동일한 식별자를 전달해도 24시간 동안은 Braze에 의한 새로운 인스턴스 전송이 발생하지 않습니다.|
|`trigger_properties`|선택 사항|객체|트리거 속성을 참조하십시오. 이 요청에서 사용자에게 적용되는 개인화 키-값 페어입니다. |
|`recipient`|필수|객체| 이 메시지를 타겟팅하는 사용자입니다. `attributes` 및 단일 `external_user_id` 또는 `user_alias`를 포함할 수 있습니다.<br><br>Braze에 아직 존재하지 않는 외부 사용자 ID를 제공하는 경우 `attributes` 개체에 필드를 전달하면 이 사용자 프로필이 Braze에 생성되고 새로 생성된 사용자에게 이 메시지가 전송됩니다. <br><br>`attributes` 객체의 서로 다른 데이터를 사용하여 동일한 사용자에게 여러 요청을 보내는 경우 `first_name`, `last_name`, `email` 속성이 동기적으로 업데이트되고 메시징에 템플릿이 적용됩니다. 사용자 지정 속성에는 이와 같은 보호 기능이 없으므로 이 API를 통해 사용자를 업데이트하고 다른 사용자 지정 속성 값을 빠르게 연속해서 전달할 때는 주의해서 진행하세요.|
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3  .reset-td-br-4 role="presentation" }
...
| `The external reference has been queued.  Please retry to obtain send_id.` | external_send_id 주소가 최근에 생성되었으므로 새 메시지를 보내려면 external_send_id 주소를 새로 만들어 보세요. |
...
Braze의 대부분의 엔드포인트에는 너무 많은 요청을 한 경우 429 응답 코드를 반환하는 속도 제한이 구현되어 있습니다. 트랜잭션 전송 엔드포인트는 다르게 작동합니다. 할당된 전송률 한도를 초과하면 시스템이 계속해서 API 호출을 수집하고 성공 코드를 반환하며 메시지를 전송하지만, 해당 메시지는 해당 기능에 대해 계약된 SLA의 적용을 받지 않을 수 있습니다. 이 기능에 대한 자세한 정보가 필요한 경우 Braze 지원팀에 문의하세요.
