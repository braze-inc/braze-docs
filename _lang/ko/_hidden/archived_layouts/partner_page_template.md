...
여기에서 통합을 단계별로 세분화할 수 있습니다. 마케터와 개발자 모두가 통합을 시작하고 실행하는 데 사용할 기술 문서이므로 끝없는 단락을 작성하지 마세요. 이 섹션의 유일한 목표는 Braze 사용자가 작업을 완료하는 데 도움이 되는 설명서를 작성하는 것입니다. '유형 통합'이라는 섹션 제목은 이것이 병렬 통합인지, 서버 간 통합인지, 또는 기본값인지 여부를 나타내기 위한 것입니다. 이 파트너와 통합할 수 있는 방법이 두 가지 이상인 경우 여러 개의 통합 섹션을 사용할 수 있습니다.
