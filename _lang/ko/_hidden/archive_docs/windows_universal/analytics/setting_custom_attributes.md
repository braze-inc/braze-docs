...
article_title: Windows 유니버설용 커스텀 속성 설정하기
...
구현하기 전에 커스텀 이벤트, 커스텀 속성 및 구매 이벤트가 제공하는 세분화 옵션의 예시를 [모범 사례]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview/#user-data-collection)에서 검토하세요.
...
```
>  Braze에 전달된 날짜는 [ISO 8601](http://en.wikipedia.org/wiki/ISO_8601) 형식, e.g `2013-07-16T19:20:30+01:00` 또는 `yyyy-MM-dd'T'HH:mm:ss:SSSZ` 형식 e.g `2016-12-14T13:32:31.601-0800`이어야 합니다
{% endtab %}
{% tab Array %}
```csharp
...
>  이러한 유형은 `AppboyPlatform.PCL.Models.NotificationSubscriptionType`에 해당합니다. 자세한 내용은 [사용자 구독 관리하기를]({{site.baseurl}}/user_guide/message_building_by_channel/email/managing_user_subscriptions/#managing-user-subscriptions) 참조하세요.
