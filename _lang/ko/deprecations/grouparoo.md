...
nav_title: Grouparoo
page_order: 1
description: "이 글에서는 Braze와 Grouparoo 간의 파트너십을 설명합니다. Grouparoo는 데이터 웨어하우스의 데이터를 사용하여 마케팅, 영업 및 지원 도구를 강화하는 오픈 소스 리버스 ETL 도구입니다."
page_type: update
...
> [Grouparoo](https://www.grouparoo.com/)는 데이터 웨어하우스에서 마케팅, 영업 및 지원 도구로 데이터를 동기화하는 오픈 소스 리버스 ETL 도구입니다. 모델 중심의 UI는 비기술 팀원이 데이터 동기화를 구성하고 예약할 수 있게 해줍니다.

Braze와 Grouparoo 통합은 웨어하우스 데이터를 Braze에 동기화합니다. 자동 동기화 일정은 고객 커뮤니케이션을 최신 정보로 유지합니다.
...
- **동기화**: 필요에 따라 회사 사용자를 추가, 업데이트 및 제거합니다. 이 옵션은 새 레코드, 기존 레코드의 변경 사항 및 삭제를 찾습니다.
- **첨가제**: 필요에 따라 회사 사용자를 추가 및 업데이트하지만, 아무도 제거하지 마십시오. 이 옵션은 Braze에 추가할 새로운 사용자와 기존 회사 사용자의 변경 사항을 찾지만 삭제는 추적하지 않습니다.
...
![예제 속성 매핑 필드. Grouparoo userID는 external_id. 이메일, firstName 및 lastName에 매핑되도록 설정되며, "email", "first_name", 및 "last_name" grouparoo 필드가 동등하게 설정됩니다.]({% image_buster /assets/img/grouparoo/mapping.png %}){: style="max-width:80%;"}
...
![Grouparoo 대상 구성 창의 "Braze 구독 그룹"에서 "최근 자동차 구매와 함께 높은 가치" Grouparoo 그룹이 "최근 자동차 구매와 함께 높은 가치" Braze 구독 그룹에 추가됩니다.]({% image_buster /assets/img/grouparoo/lists.png %}){: style="max-width:80%;"}
