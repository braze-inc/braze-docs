...
| 정적 | Braze 카탈로그에 저장된 데이터를 사용하여 제품을 개인화하세요. [카탈로그 선택을]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/) 사용하여 포함할 제품을 지정해야 합니다. | 신제품 출시 또는 카테고리별 제공을 보여주기에 완벽합니다.| |
...
![제품 방향, 정렬, 행당 최대 제품 수 및 제품 간격에 대한 레이아웃 옵션입니다.]({% image_buster /assets/img/product_blocks/layout_options.png %}){: style="max-width:50%;"}
...
단락 및 버튼에 대한 기존 스타일이 제품 블록 내의 텍스트 및 버튼 요소에 자동으로 적용됩니다. 즉, 제품 블록은 단락과 버튼에 설정한 서식을 일관되게 사용하므로 이메일 전체에서 일관된 모양을 유지할 수 있습니다.
...
드래그 앤 드롭 제품 블록을 사용하려면 특정 필드 값을 포함하는 Braze 카탈로그를 설정해야 합니다. 제품 블록 구성에서 이러한 필드를 사용합니다. 카탈로그에 다음 필드가 포함되어 있는지 확인하세요:
...
카탈로그의 **설정** 탭에서 **제품 블록** 토글을 선택하여 카탈로그의 특정 필드 및 정보에 매핑할 수 있습니다. 이를 통해 제품 제목, 제품 URL, 이미지 URL로 사용할 필드를 선택할 수 있습니다. 참고: Shopify 카탈로그 필드는 기본값으로 매핑되며 변경할 수 없습니다.

{% alert note %}
Shopify를 사용하지 않는 경우 계정 매니저에게 문의하여 필드 매핑을 사용하도록 설정하면 카탈로그를 제품 블록에 연결하고 해당 필드를 `product_title`, `product_url`, `variant_image_url` 에 매핑할 수 있습니다.
{% endalert %}
...
![품목 가격에 달러 쪽이 앞에 붙은 제품 블록입니다.]({% image_buster /assets/img/product_blocks/liquid.png %}){: style="max-width:45%;"}
...
!['사용자로 미리 보기' 탭에 4개의 항목을 표시하도록 지정하는 드롭다운 섹션인 '동적 제품 블록'이 있습니다.]({% image_buster /assets/img/product_blocks/preview_as_a_user.png %}){: style="max-width:40%;"}
