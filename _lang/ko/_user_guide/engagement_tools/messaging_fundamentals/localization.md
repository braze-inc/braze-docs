...
로캘 정보는 [Braze 소프트웨어 개발 키트]({{site.baseurl}}/developer_guide/sdk_integration/) (자동)를 사용하여 수집한 데이터를 기반으로 사용자 프로필에 저장되거나 [REST API]({{ site.baseurl }}/api/endpoints/user_data/post_user_track). 로캘에는 언어와 지역 식별자가 포함되어 있습니다. 이 정보는 **국가** 및 **언어** 아래의 Braze 세분화 도구에서 확인할 수 있습니다.
...
사용자의 언어, 로캘 또는 커스텀 속성을 기반으로 개인화된 메시지를 보내려면 다음 방법 중 하나를 사용하세요.

### 번역 Liquid 태그(권장) {#translation-liquid-tag}

Braze는 단일 메시지로 다양한 언어의 사용자를 타겟팅할 수 있는 {% raw %}`{% translation salutation %}Hello!{% endtranslation %}`{% endraw %} Liquid 태그를 지원합니다. 

전체 안내는 [번역 태그 사용 가이드를]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/localization/locales) 참조하세요.
