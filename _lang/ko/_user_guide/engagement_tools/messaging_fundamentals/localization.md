...
이 접근 방식에서는 [캔버스 여정과]({{site.baseurl}}/user_guide/engagement_tools/canvas/get_started/the_basics/#building-the-customer-journey) 리퀴드 내에서 로컬라이제이션을 처리하여 각 사용자에 대한 메시지를 정의합니다. 
...
번역 대행사와 협력하여 번역을 Google 스프레드시트에 저장한 다음 [Braze 커넥티드 콘텐츠를]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content) 사용하여 이 콘텐츠를 쿼리할 수 있습니다. 메시지를 보내면 선택한 언어에 따라 각 사용자에 대한 관련 번역이 캠페인 본문으로 가져옵니다. 
