...
로케일 정보는 사용자가 수집한 데이터를 기반으로 사용자의 프로필에 저장됩니다 [Braze SDK]({{site.baseurl}}/developer_guide/sdk_integration/) (자동으로) 또는 [REST API]({{ site.baseurl }}/api/endpoints/user_data/post_user_track). 로캘에는 언어와 지역 식별자가 포함되어 있습니다. 이 정보는 **국가** 및 **언어** 아래의 Braze 세분화 도구에서 확인할 수 있습니다.

{% alert tip %}
로케일이 우리 SDK에 의해 수집되는 방법에 대한 기술적 세부정보는 공식 [iOS](https://developer.apple.com/library/ios/documentation/MacOSX/Conceptual/BPInternational/LanguageandLocaleIDs/LanguageandLocaleIDs.html), [Android](http://developer.android.com/reference/java/util/Locale.html), 및 [Web](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/language) 설명서를 참조하십시오.
{% endalert %}
...
사용자의 언어, 로케일 또는 커스텀 속성을 기반으로 개인화된 메시지를 보내려면 다음 방법 중 하나를 사용하십시오.

### 번역 Liquid 태그 (권장) {#translation-liquid-tag}

Braze는 단일 메시지로 다양한 언어의 사용자를 타겟팅하기 위해 {% raw %}`{% translation salutation %}Hello!{% endtranslation %}`{% endraw %} Liquid 태그를 지원합니다. 

전체 안내서는 [번역 태그 사용에 대한 가이드]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/localization/locales)를 참조하십시오.
...
{% tab Catalogs %}
[카탈로그]({{site.baseurl}}/user_guide/data/activation/catalogs/)를 사용하면 API 및 CSV 파일을 통해 가져온 JSON 객체의 데이터에 액세스하여 Liquid를 통해 커스텀 속성이나 커스텀 이벤트 속성과 유사하게 메시지를 보강할 수 있습니다. For example:
...
이러한 카탈로그 항목은 아래 표시된 [개인화]({{site.baseurl}}/user_guide/data/activation/catalogs/catalog/#using-catalogs-in-a-message)를 사용하여 참조하거나 데이터 그룹을 만들 수 있는 [선택]({{site.baseurl}}/user_guide/data/activation/catalogs/selections) 항목을 사용하여 참조할 수 있습니다. 
...
일부 사용자는 SheetDB의 [검색 방법](https://docs.sheetdb.io/#get-search-in-document)을 구현하여 JSON 객체를 필터링하여 Liquid 및 연결된 블록 종속성을 줄이는 것을 선호할 수 있습니다. 이를 통해 단일 언어에 대한 결과를 자동으로 반환할 수 있습니다.
