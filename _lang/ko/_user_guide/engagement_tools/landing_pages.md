...
{% alert note %}
랜딩 페이지 및 커스텀 도메인 가용성은 귀하의 Braze 패키지에 따라 다릅니다. 시작하려면 계정 매니저 또는 고객 성공 매니저에게 문의하십시오.
{% endalert %}
...
랜딩 페이지에 Google 태그 관리자를 추가하려면 드래그 앤 드롭 편집기에서 랜딩 페이지에 **커스텀 코드** 블록을 추가한 다음, 블록에 태그 관리자 코드를 삽입하십시오. 태그 관리자 코드 전에 데이터 레이어를 추가해야 합니다. 예를 들어 다음과 같이 추가하십시오:
...
Google 태그 관리자 구현에 대한 자세한 내용은 [Google의 설명서](https://developers.google.com/tag-platform/tag-manager/datalayer#installation)를 참조하십시오.

## Frequently asked questions

### What's the maximum size for landing pages?

랜딩 페이지 본문 크기는 최대 500 KB까지 가능합니다.
