...
입력 필드와 표시되는 텍스트 컴포넌트 간 공백 처리는 CSS 스타일로 인해 다릅니다. 기본값이 `white-space: normal` CSS인 텍스트 컴포넌트에서는 여러 개의 연속된 공백이 표시될 때 하나의 공백으로 축소됩니다. 이는 렌더링된 텍스트에 대한 표준 HTML 동작입니다. 
