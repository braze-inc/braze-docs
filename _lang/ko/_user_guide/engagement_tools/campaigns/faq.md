...
### 왜 다채널 캠페인에서 전환 수가 고유 사용자 수를 초과할 수 있습니까?

다채널 캠페인에서는 Braze가 사용자별이 아니라 채널별로 전환을 계산합니다. 사용자가 전환 창 내에서 단일 전환 작업을 수행하면 Braze는 해당 전환을 사용자가 메시지를 받은 각 채널에 귀속시킵니다. 즉, 사용자가 여러 채널(예: 이메일과 푸시 모두)에서 메시지를 받고 전환하면 Braze는 각 채널에 대해 하나씩 여러 전환을 계산합니다. 그 결과, 총 전환 수는 전환한 고유 사용자 수를 초과할 수 있습니다.

예를 들어, 다채널 캠페인이 사용자에게 이메일과 푸시 알림을 모두 보내고, 그 사용자가 두 메시지를 모두 받은 후 전환 창 내에서 하나의 전환 작업을 수행하면, Braze는 이를 두 개의 전환으로 계산합니다. 하나는 이메일에 귀속되고 다른 하나는 푸시에 귀속됩니다. 같은 사용자의 단일 작업임에도 불구하고 말입니다.
...
사용자는 시작하기 24시간 전에 세그먼트에 있어야 합니다. 사용자가 첫 번째 검사에서 자격이 없으면 Braze는 두 번째 검사를 시도하지 않습니다.
...
캠페인이 예정되면 메시지를 전송하기 위해 메시지 구성을 제외한 모든 것을 편집해야 합니다. 모든 캠페인에 따라, 시작 후 전환 이벤트를 편집할 수 없습니다.
...
- Braze는 발송 시간을 1시간 이상 놓친 사용자에게 메시지를 보내지 않습니다.
- Pre-enqueued messages may still be sent at the originally enqueued time, rather than the adjusted time.

변경이 필요하면 현재 캠페인을 중지하는 것이 좋습니다(이렇게 하면 대기 중인 메시지가 취소됩니다). 그런 다음 캠페인을 복제하고 필요에 따라 변경한 다음 새 캠페인을 시작할 수 있습니다. 이미 첫 번째 캠페인을 받은 사용자를 이 캠페인에서 제외해야 할 수도 있습니다. 시간대 전송이 가능하도록 캠페인 일정 시간을 다시 조정해야 합니다.

### 일광 절약 시간(DST) 날에 왜 아무 사용자도 내 일일 예정 캠페인에 참여하지 않았습니까?

일광 절약 시간(DST) 전환일에는 일일 예정 캠페인이 일반적으로보다 최대 1시간 더 일찍 또는 늦게 실행될 수 있습니다. 시계가 앞으로 나가거나 뒤로 떨어지는지에 따라 다릅니다. 세그먼트가 사용자 정의 속성이나 예정된 발송 시간의 1시간 이내에 타임스탬프가 있는 이벤트에 의존하는 경우, 캠페인이 DST 날에 자격을 평가할 때 해당 사용자가 아직 자격을 갖추지 못할 수 있습니다.

예를 들어, 사용자가 일반적으로 UTC 오후 3시에 사용자 정의 속성 업데이트를 받고, 캠페인이 뉴욕(동부 시간)에서 매일 오전 10시 30분에 실행된다고 가정해 보십시오. 뉴욕이 표준 시간(UTC-5)일 때, 오전 10:30 ET는 UTC 오후 3:30에 해당하므로 캠페인은 속성이 기록된 후에 실행됩니다. 뉴욕이 일광 절약 시간(UTC-4)으로 전환되면, 오전 10:30 ET는 UTC 오후 2:30에 해당하므로, 봄으로 전환되는 DST 날에는 캠페인이 오후 3시 UTC 속성 업데이트 전에 실행될 수 있습니다. 자격이 있는 속성이 아직 존재하지 않기 때문에, 해당 사용자들은 필터링됩니다. 재자격이 꺼져 있으면, 이전 날에 들어온 사용자들은 다시 들어올 수 없으며, 그 날의 항목 수는 0이 됩니다.

이를 피하려면, 캠페인의 예정된 전송 시간보다 1시간 이상 전에 사용자 정의 속성이나 이벤트 업데이트가 발생하도록 하십시오.
...
### 내 캠페인 분석 페이지에서 CSV 내보내기 사용자 데이터와 CSV 내보내기 이메일 주소 옵션의 차이는 무엇입니까?

**CSV 내보내기 이메일 주소** 옵션을 선택하면 이메일 주소가 있는 사용자에 대한 데이터만 다운로드됩니다. 예를 들어, 100,000명의 사용자 세그먼트가 있지만 그 중 50,000명만 이메일 주소를 가지고 있고, **CSV 내보내기 이메일 주소**를 클릭하면, 내보내기에는 50,000개의 데이터 행만 포함됩니다. 비교적으로, **CSV 내보내기 사용자 데이터**를 선택하면 모든 사용자 데이터가 내보내집니다.
...
### 입력 필드와 표시된 텍스트에서 공백이 다르게 나타나는 이유는 무엇입니까? 

공백 처리는 CSS 스타일링 때문에 입력 필드와 표시된 텍스트 구성 요소 간에 다릅니다. 기본 `white-space: normal` CSS가 적용된 텍스트 구성 요소에서는 여러 개의 연속된 공백이 표시될 때 하나의 공백으로 축소됩니다. 이는 렌더링된 텍스트에 대한 표준 HTML 동작입니다. 

입력 필드는 정확한 데이터 입력을 위해 입력한 대로 여러 개의 공백을 그대로 유지합니다. 이는 여러 개의 공백이 있는 텍스트가 입력 필드(모든 공백이 유지됨)에서 볼 때와 대시보드의 다른 부분(여기서 CSS가 여러 개의 공백을 축소할 수 있음)에서 표시될 때 다르게 나타날 수 있음을 의미합니다. 

예를 들어, 입력 필드에 여러 개의 공백이 있는 캠페인 이름이나 UTM 매개변수를 입력하면 모든 공백이 유지됩니다. 그러나 동일한 텍스트가 검색 결과, 캠페인 목록 또는 다른 텍스트 구성 요소에 나타날 때, CSS 공백 처리로 인해 여러 개의 공백이 하나의 공백으로 나타날 수 있습니다. 
