...
{% alert warning %}
CDI 세그먼트 확장은 데이터 웨어하우스를 직접 쿼리하므로, 이러한 쿼리 실행과 관련된 모든 비용이 데이터 웨어하우스에서 발생합니다. CDI 세그먼트 확장은 [SQL 세그먼트 크레딧을]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments/#monitoring-your-sql-segments-usage) 소모하지 않으며, 세그먼트 확장 한도에 포함되지 않고, 데이터 포인트를 기록하지 않습니다.
{% endalert %}

## 필수 조건

To use your data warehouse data for segmentation within your Braze workspace, you'll need to create a [connected source]({{site.baseurl}}/user_guide/data/cloud_ingestion/connected_sources/), then create a CDI segment within your [Segment Extensions]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/). CDI 세그먼트 확장을 사용하면 CDI 연결을 통해 제공되는 데이터를 활용하여 자체 데이터 웨어하우스를 직접 쿼리하는 SQL을 작성하고, Braze 내에서 타겟팅할 수 있는 사용자 그룹을 생성할 수 있습니다.

## 세그먼트 생성

### 1단계: 소스 설정

첫 번째 CDI 세그먼트 확장을 생성하기 전에, [연결된 소스]({{site.baseurl}}/user_guide/data/cloud_ingestion/connected_sources/) 단계에 따라 데이터 웨어하우스를 사용한 새 연결된 소스를 설정하십시오.
...
CDI 설정의 일환으로, CDI 세그먼트 확장에서 사용할 다양한 연결 방식 중에서 선택할 수 있습니다. 각 연결에는 특정 데이터 테이블 세트가 있습니다. 개발 팀은 CDI 설정 중에 연결 및 데이터 테이블을 구성할 수 있습니다.

사용 가능한 데이터 테이블(스키마 및 설명 포함)을 보려면 **참조를** 선택하십시오. 준비가 되면 연결을 선택하세요.
...
모든 CDI 세그먼트 확장 기능은 선택된 열로`external_user_id`  를 사용해야 하며, 귀하의  는 Braze에서 `external_user_id`사용자에게 설정된 것과 일치해야 합니다.

{% alert important %}
`external_user_id` **문자열** 값이어야 합니다. 소스 ID가 숫자(예: 정수`client_id`)로 저장된 경우, Braze의 데이터`external_id`유형과 일치하도록 [SQL에서 문자열로 형변환하십시오](https://www.w3schools.com/sql/func_sqlserver_cast.asp).
{% endalert %}

쿼리 결과에 Braze에 존재하지 않는 사용자가 포함된 경우, 해당 사용자는 무시됩니다. Braze는 CDI 세그먼트 확장 기능의 출력 결과를 기반으로 새 사용자를 생성하지 않습니다.

{% alert tip %}
세그먼트 확장 기능을 미리 보고 관리하며 자동화 된 멤버십 새로고침을 실행하는 방법은 [SQL 세그먼트 확장]({{site.baseurl}}/user_guide/engagement_tools/segments/sql_segments/) 기능을 참조하십시오.
{% endalert %}
