...
Braze 대시보드의 **세그먼트** 페이지에는 모든 세그먼트에 대한 요약이 포함되어 있으며 각 세그먼트에 대한 자세한 데이터를 살펴볼 수 있습니다. 이 페이지에서 세그먼트의 이름을 검색하고 선택하여 해당 데이터를 편집하고 확인합니다. 세그먼트를 만드는 방법을 배우려면 [세그먼트 만들기]({{site.baseurl}}/user_guide/engagement_tools/segments/creating_a_segment/#creating-a-segment)를 확인하세요.
...
세그먼트에 대한 [분석 추적]({{site.baseurl}}/user_guide/analytics/tracking/segment_analytics_tracking/)을 켜면 이 세그먼트에 대한 세션, 커스텀 이벤트 및 매출을 시간에 따라 볼 수 있습니다.
...
세그먼트의 구성원 수와 크기를 모니터링하는 방법에 대해 더 알아보려면 [세그먼트 크기 측정]({{site.baseurl}}/user_guide/engagement_tools/segments/measuring_segment_size/)을 참조하세요.
...
세그먼트가 작업 공간 크기에 비해 매우 작을 경우 사용자 미리 보기가 0명의 사용자를 반환할 수 있습니다. 이것이 반드시 세그먼트에 사용자가 0명이라는 것을 의미하지는 않습니다. [정확한 통계 계산]({{site.baseurl}}/user_guide/engagement_tools/segments/measuring_segment_size/#statistics-for-segment-size)을 실행하여 세그먼트의 정확한 크기를 확인하세요.

![사용자 미리 보기]({% image_buster /assets/img_archive/user_preview.png %})
