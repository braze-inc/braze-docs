...
glossaries:
  ...
  - name: 캠페인에서 메시지 수신
    ...
    tags:
      - Retargeting
  - name: 수신한 캠페인 배리언트
    description: "사용자가 수신한 다변량 캠페인의 배리언트을 기준으로 사용자를 세분화합니다.<br><br> 콘텐츠 카드 및 인앱 메시지의 경우, 사용자가 노출을 기록하는 시점은 카드 또는 인앱 메시지가 전송되는 시점이 아니라 사용자가 노출을 기록하는 시점입니다.<br><br>푸시 및 웹훅의 경우 사용자에게 메시지가 전송되는 시점입니다.<br><br> WhatsApp의 경우, 이는 메시지가 사용자의 기기로 전달되는 시점이 아니라 마지막 메시지 API 요청이 WhatsApp으로 전송되는 시점입니다. <br><br>이메일의 경우, 이메일 요청이 이메일 서비스 제공업체에 전송되는 시점입니다(실제로 배달되는지 여부와 관계없이). 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 최초 전송 시에는 특정 타겟 사용자의 프로필만 업데이트됩니다. <br>- 이메일이 전달되거나 사용자가 이메일 또는 이메일의 링크를 열면 해당 이메일 주소를 공유하는 모든 사용자가 메시지를 받은 것으로 표시됩니다.<br><br>SMS의 경우 마지막 메시지가 SMS 제공업체에 전달된 시점입니다. 그렇다고 해서 메시지가 사용자의 디바이스에 전달되었다는 보장은 없습니다."
    tags:
      - Retargeting
  ...
  - name: 캔버스 변형 진입
    ...
    tags:
      - Retargeting
  - name: 마지막으로 수신한 메시지
    description: "마지막으로 수신한 메시지를 확인하여 사용자를 세그먼트화합니다. (24시간 기간)<br><br> 콘텐츠 카드 및 인앱 메시지의 경우, 사용자가 마지막으로 노출을 기록한 시점이 아니라 카드 또는 인앱 메시지가 마지막으로 전송된 시점이 기준이 됩니다.<br><br>푸시 및 웹훅의 경우 사용자에게 메시지가 전송된 때입니다.<br><br> WhatsApp의 경우, 이는 메시지가 사용자의 기기로 전달된 시점이 아니라 마지막 메시지 API 요청이 WhatsApp으로 전송된 시점입니다. <br><br>이메일의 경우, 이메일 요청이 이메일 서비스 제공업체에 전송되는 시점입니다(실제로 배달되는지 여부와 관계없이). 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 최초 전송 시에는 특정 타겟 사용자의 프로필만 업데이트됩니다. <br>- 이메일이 전달되거나 사용자가 이메일 또는 이메일의 링크를 열면 해당 이메일 주소를 공유하는 모든 사용자가 메시지를 받은 것으로 표시됩니다.<br><br>SMS의 경우 마지막 메시지가 SMS 제공업체에 전달된 시점입니다. 그렇다고 해서 메시지가 사용자의 디바이스에 전달되었다는 보장은 없습니다.<br><br>예시:<br>마지막으로 받은 메시지 1일 전 미만 = 24시간 전 미만<br><br>시간대:<br>회사 표준 시간대"
    tags:
      - Retargeting
  - name: 마지막 메시지 참여
