...
![드래그 앤 드롭 편집기에서 생성 중인 예시 랜딩 페이지.]({% image_buster /assets/img/landing_pages/template.png %})

편집기는 랜딩 페이지 구성을 위해 두 가지 유형의 구성 요소를 사용합니다: 기본 블록과 양식 블록입니다. 모든 블록은 한 줄에 배치되어야 합니다.

!['구축' 섹션에는 '행'과 '폼 블록'이 포함됩니다.]({% image_buster /assets/img/landing_pages/dnd.png %}){: style="max-width:35%;"}
...
| 체크박스 그룹| 사용자는 제시된 여러 선택지 중에서 선택할 수 있습니다. 값들은 정의된 배열 커스텀 속성에 설정되거나 추가됩니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
!['페이지 컨테이너' 섹션으로 배경 이미지, 색상, 테두리 세부 사항 및 콘텐츠 스타일을 커스텀할 수 있는 옵션이 포함되어 있습니다.]({% image_buster /assets/img/landing_pages/page_container.png %}){: style="max-width:40%;"}

#### 사용자 기기에 반응하는

작은 화면에서는 열을 세로로 쌓아 배치함으로써 사용자의 기기 크기에 반응하는 랜딩 페이지를 만들 수 있습니다. 이를 인에이블하려면 반응형으로 설정하려는 행에 열을 추가한 후, **'열 커스텀'** 섹션에서 **'작은 화면에서 세로로 쌓기**'를 토글하십시오.

인에이블먼트 시, 작은 화면에서 다중 열 콘텐츠의 수직 순서를 제어하기 위해 열 스택을 반전시킬 수도 있습니다. 이 기능은 커스텀 코드 없이도 모바일에서 페이지의 시각적 효과와 사용감을 개선합니다.

!["컬럼 커스텀" 섹션의 "작은 화면에서 세로로 쌓기" 토글.]({% image_buster /assets/img/landing_pages/device_responsive_toggle.png %}){: style="max-width:50%;"}
...
{% alert note %}
확인 페이지가 새 탭에서 열릴 경우, 사용자가 원래 랜딩 페이지로 돌아가 업데이트된 정보로 재제출하면 이전 제출 내용을 덮어쓸 수 있어 데이터 불일치가 발생할 수 있습니다.
{% endalert %}

{% alert important %}
버튼** 클릭 시 '양식 제출**'을 인에이블하고 **'클릭 시 동작'을** 사용하여 새 탭에서 웹 URL을 열도록 설정할 경우, iOS Safari의 내장 팝업 차단 기능으로 인해 새 탭 열기가 작동하지 않을 수 있습니다. 이는 양식 제출 후 새 탭을 여는 동작이 팝업으로 간주되기 때문입니다.<br><br>이 문제를 방지하려면, 양식 제출 버튼을 설정하여 결과 URL을 새 탭이 아닌 같은 탭에서 열도록 하십시오. 폼 제출이 없는 버튼은 새 탭에서 URL을 문제없이 열 수 있습니다.
{% endalert %}
...
![메뉴가 열려 "URL 복사" 옵션이 표시되는 랜딩 페이지.]({% image_buster /assets/img/landing_pages/copy-url.png %})
...
![랜딩 페이지를 저장, 변경, 삭제할 수 있는 옵션이 있는 드롭다운 메뉴.]({% image_buster /assets/img/landing_pages/manage-lp-template.png %}){: style="max-width:60%;"}
