...
description: "이 문서에서는 드래그 앤 드롭 편집기로 Braze 랜딩 페이지를 만들고 커스텀하는 방법을 다룹니다."
page_order: 0
---

# 랜딩 페이지 만들기

> 드래그 앤 드롭 편집기를 사용하여 랜딩 페이지를 만들고 커스텀하는 방법을 알아보고 Braze에서 직접 오디언스를 늘리고 선호도를 수집할 수 있습니다.
...
메타태그를 설정하여 페이지가 브라우저 탭에 표시되는 방식을 커스텀하고 검색 엔진 결과에 최적화할 수 있습니다. 이러한 내용은 고객에게 표시됩니다.

다음 모범 사례를 따르는 것이 좋습니다:

| 필드 | 설명 | 권장 사항 |
| --- | --- |
| 사이트 제목 | 브라우저 탭에 표시되는 제목입니다. | 최대 60자까지 사용할 수 있습니다. |
| 메타 설명 | 검색 결과에 표시되는 텍스트 스니펫입니다. | 140-160자 사이를 사용합니다.|
| 파비콘 | 브라우저 탭의 사이트 제목 옆에 표시되는 아이콘입니다. | 종횡비는 1:1, 지원되는 파일 형식은 PNG, JPEG 또는 ICO를 사용하세요. |
| 페이지 URL | 랜딩 페이지의 URL 경로입니다. 이 값은 양식을 제출할 때 자동으로 식별할 수 있도록 메시징에 임베드할 수 있는 [랜딩 페이지 Liquid 태그를]({{site.baseurl}}/user_guide/engagement_tools/landing_pages/tracking_users) 사용할 때도 참조됩니다.| 이 값은 워크스페이스 전체에서 고유해야 합니다. |
...
이러한 블록을 사용하여 콘텐츠를 추가하고 랜딩 페이지의 레이아웃을 커스텀할 수 있습니다.
...
| 라디오 버튼 | 사용자가 선택할 수 있는 옵션 목록을 추가합니다. 제출되면 고객 프로필에 관련 커스텀 속성이 기록됩니다. |
...
| 스페이서      | 요소 사이에 세로 간격을 추가하여 레이아웃과 가독성을 개선하는 보이지 않는 블록입니다. |
| 커스텀 코드 | 고급 커스터마이징을 위해 커스텀 HTML, CSS 또는 JavaScript를 삽입하고 실행할 수 있는 블록입니다. |
...
| 블록 유형     | 설명 |
|---------------|-------------|
| 이메일 캡처 | 이메일 주소를 입력하는 양식 필드입니다. 이메일 주소가 제출되면 해당 이메일 주소가 Braze의 해당 사용자 프로필에 추가됩니다. |
| 전화 캡처 | 전화번호를 입력하는 양식 필드입니다. 제출하면 해당 사용자는 SMS 또는 WhatsApp 구독 그룹에 가입하게 됩니다. |
| 입력 필드   | 표준 속성(예: 이름 및 성) 또는 원하는 커스텀 속성 문자열을 지원하는 양식 필드입니다. |
| 드롭다운      | 사용자는 미리 정의된 목록에서 항목을 선택할 수 있습니다. 목록에 커스텀 속성 문자열을 추가할 수 있습니다. |
| 체크박스      | 사용자가 이 확인란을 선택하면 블록의 속성이 `true` 로 설정됩니다. 체크하지 않으면 속성이 `false` 로 설정됩니다. |
| 체크박스 그룹| 사용자는 제시된 여러 선택지 중에서 선택할 수 있습니다. 값은 정의된 배열 커스텀 속성에 설정되거나 추가됩니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

{% alert important %}
양식이 있는 랜딩 페이지를 만든 후에는 메시징에 해당 [랜딩 페이지 Liquid 태그를]({{site.baseurl}}/user_guide/engagement_tools/landing_pages/tracking_users) 임베드해야 합니다. 이 태그를 사용하면 Braze는 양식을 제출할 때 기존 고객 프로필을 자동으로 식별하고 업데이트할 수 있습니다.
{% endalert %}
...
**페이지 컨테이너** 탭에서 랜딩 페이지의 모든 관련 컴포넌트 블록에 적용될 스타일을 설정할 수 있습니다. 이러한 스타일은 특정 블록으로 재정의하는 경우를 제외하고 페이지의 모든 곳에서 사용됩니다.

블록 수준에서 스타일을 커스텀하기 전에 페이지 컨테이너 수준 스타일을 설정하는 것이 좋습니다. 전체 페이지에 배경 이미지를 추가할 수도 있습니다.

배경 이미지, 색상, 테두리 세부 정보, 콘텐츠 스타일을 커스텀할 수 있는 옵션이 있는 '페이지 컨테이너' 섹션입니다.]({% image_buster /assets/img/landing_pages/page_container.png %}){: style="max-width:40%;"}
...
사용자가 입력이 누락되었거나 지원되지 않는 양식을 제출하려고 하면 일반적인 오류 메시지가 표시되며 제출할 수 없습니다.
...
사용자에게 표시되는 오류 메시지는 커스텀할 수 없습니다. 게시하기 전에 랜딩 페이지를 미리 보고 필드 동작을 확인합니다. 
