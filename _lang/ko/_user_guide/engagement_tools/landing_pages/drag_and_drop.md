...
{% alert important %}
랜딩 페이지는 현재 초기 액세스 중입니다. 회사는 최대 다섯 개의 랜딩 페이지를 가질 수 있습니다. 랜딩 페이지에 기록된 최종 사용자 세션은 월간 활성 사용자(MAU) 계산에 포함됩니다.
{% endalert %}
...
다양한 형식 블록 구성 요소를 사용하여 커스텀 및 표준 프로필 속성과 커스텀 이벤트를 기록합니다. 입력 필드 양식 블록은 사용자에 대한 표준 및 커스텀 속성을 모두 기록할 수 있으며, 전화 캡처 및 이메일 캡처 양식 블록은 사용자의 양식 제출을 위한 전화 및 이메일 필드를 캡처할 수 있습니다. 버튼 작업은 양식 제출 시 커스텀 속성, 커스텀 이벤트 또는 둘 다로 기록될 수 있습니다. 
...
모든 채널에 랜딩 페이지 링크를 포함할 수 있으며, 링크를 복사하여 Braze 메시지나 소셜 미디어 캠페인에 붙여넣으면 됩니다.
...
랜딩 페이지에 접근하고, 생성하고, 게시하려면 관리자 권한 또는 다음 모든 권한이 필요합니다:
...
랜딩 페이지의 HTML을 커스텀 코드 블록을 사용하여 편집할 수 있습니다.
...
### 랜딩 페이지에 대한 로드맵에는 어떤 기능이 있나요? {#roadmap}

개발 중인 랜딩 페이지용 추가 기능도 곧 출시할 계획입니다. 여기에는 다음이 포함될 수 있습니다:

* 새로운 Liquid 태그로 Braze 메시징 채널에서 랜딩 페이지를 연결합니다.
* 랜딩 페이지가 Braze 채널을 통해 전송될 때 자동 사용자 병합
* 기본 보고서 페이지
* 체크박스와 드롭다운을 위한 드래그 앤 드롭 양식 블록
* 양식 제출을 기반으로 한 추적 및 리타겟팅을 위한 표준 이벤트
