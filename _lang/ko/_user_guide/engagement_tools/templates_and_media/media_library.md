...
모든 이미지는 미디어 라이브러리에 업로드된 5MB 미만이어야 합니다.  메시징 채널별 특정 이미지 추천은 다음 섹션을 참조하십시오.
