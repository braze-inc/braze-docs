...
콘텐츠 블록을 생성하는 데 사용되는 편집기에는 클래식과 드래그 앤 드롭의 두 가지 유형이 있습니다. 이 두 가지 유형의 편집기는 콘텐츠 블록 유형에 해당합니다: HTML 및 드래그 앤 드롭. 
...
{% alert tip %}
 이 줄 바꿈이 전송 중에 남아 있으면 블록이 렌더링되는 방식에 영향을 줄 수 있는 불필요한 공백이 생길 위험이 있습니다. 이를 피하려면 블록에 **Capture** 태그를 사용하고 **| strip** 필터를 함께 사용하십시오.
{% raw %}
```
