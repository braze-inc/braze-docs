...
| 콘텐츠 크기 | 최대 50KB. |
| 배치 | 콘텐츠 블록은 이메일 바닥글 내에서 사용할 수 없지만, 이메일에서 사용할 바닥글을 포함하는 콘텐츠 블록을 [생성할 수 있습니다.](#email-footers) |
...
이 미리보기에는 콘텐츠 블록에 대한 정보가 포함되어 있습니다. 예를 들어, 누가 생성했는지, 태그, 생성 날짜, 마지막 수정 날짜, 설명, 편집기 유형, 포함 수와 세부 사항(콘텐츠 블록을 사용하는 메시지 또는 콘텐츠 블록의 클릭 가능한 목록) 및 콘텐츠 블록의 실제 미리보기가 포함됩니다.

![사이클링 및 댄스를 위한 콘텐츠 블록의 미리보기 "Workout_Promo"가 하나의 포함을 가지고 있습니다.]({% image_buster /assets/img/preview_tab_content_block.png %}){: style="max-width:60%;"} 
...
콘텐츠 블록은 이메일 바닥글 내에서 사용할 수 없지만, 이메일에서 사용할 바닥글 콘텐츠를 포함하는 콘텐츠 블록을 생성할 수 있습니다. To do so, follow these steps:

1. **설정** > **이메일 기본 설정** > **커스텀 바닥글**로 이동하여 바닥글을 생성합니다.
2. **콘텐츠 블록 라이브러리**에서 콘텐츠 블록에 바닥글을 추가합니다.
3. 그 콘텐츠 블록을 이메일 템플릿이나 메시지에 추가합니다.

이제 여러 메시지에서 동일한 바닥글을 사용할 수 있습니다!

### 콘텐츠 블록 보관

![확장된 설정 드롭다운 메뉴에는 세 가지 옵션이 표시됩니다: 워크스페이스로 아카이브, 복제 및 복사.]({% image_buster /assets/img/template_archive_cog.png %}){: style="max-width:20%;float:right;margin-left:15px;" }

콘텐츠 블록 사용이 끝나면 [템플릿 & 미디어]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/managing_templates/) 페이지에서 아카이브할 수 있습니다. 보관된 콘텐츠 블록은 읽기 전용이므로 편집하기 전에 콘텐츠 블록의 보관을 해제하십시오. 콘텐츠 블록은 메시지에서 사용되는 경우 보관할 수 없습니다.
...
- 블록이 이 목록의 첫 번째 제안으로 합리적으로 관리할 수 있는 메시지보다 더 많이 사용될 때, 블록에서 모든 콘텐츠를 제거하는 것이 좋습니다. 이것은 모든 메시지에서 구식 정보의 포함을 방지합니다.
- 콘텐츠 블록을 실수로 보관하면 보관 해제할 수 있습니다.

![설정 드롭다운 메뉴가 "Test_32"에 대해 확장된 저장된 콘텐츠 블록 패널로 세 가지 옵션을 보여줍니다: 보관 해제, 복제 및 작업 공간으로 복사]({% image_buster /assets/img/unarchive-content-block.png %})
