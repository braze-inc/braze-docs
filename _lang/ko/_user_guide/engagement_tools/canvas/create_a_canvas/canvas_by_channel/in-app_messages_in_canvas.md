...
동작 트리거 항목이 있는 캔버스 단계의 경우 사용자는 세션 중간에 캔버스에 들어갈 수 있습니다. 인앱 메시지는 세션이 시작될 때까지 활성화되지 않으므로, 사용자가 메시지 단계에 도달할 때 세션 중간에 있다면, 다른 세션을 시작하고 관련 트리거를 수행할 때까지 인앱 메시지를 받지 못합니다.
...
![The Message Controls section of a Message step for an in-app message. 인앱 메시지는 단계가 사용 가능해진 후 3일 후에 만료됩니다.]({% image_buster /assets/img_archive/canvas_expiration2.png %}){: style="max-width:90%"}

| 옵션 | 설명 | 예시 |
|---|---|---|
| **이 단계 이후의 기간을 사용할 수 있음** | 사용자가 단계를 사용할 수 있게 된 시점을 기준으로 인앱 메시지가 만료되도록 설정합니다. | 2일 만료가 있는 인앱 메시지는 사용자가 메시지 단계에 들어가고 오디언스 옵션이 체크될 때 사용 가능해집니다. 이 단계에 도달하기 전의 지연은 캔버스의 이전 지연 단계에서 발생합니다. 인앱 메시지는 사용자가 단계에 들어간 시점부터 2일(48시간) 동안 사용 가능하며, 그 2일 동안 사용자가 앱을 열면 인앱 메시지를 볼 수 있습니다. |
...
![ "환영 IAM" 및 "후속 IAM"이라는 두 단계가 있는 우선 순위 정렬기입니다.]({% image_buster /assets/img_archive/canvas_priority2.png %}){: style="max-width:85%"}
...
**메시지 전송 시 진행** 옵션을 사용하려면, 이전 단계를 받지 않은 사용자를 필터링하기 위해 별도의 [오디언스 경로]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/audience_paths/)를 추가하십시오.
...
원본 편집기에서 만든 캔버스는 캔버스 구성 요소를 통해 발전하는 기준인 진행 동작을 지정해야 합니다. [인앱 메시지만 있는 단계는](#steps-iam-only) 푸시 또는 이메일 같이 [여러 메시지 유형이 있는 단계](#steps-multiple-channels)와 다른 진행 옵션이 있습니다. 현재 캔버스 워크플로우의 인앱 메시지에 대해 이 옵션은 항상 즉시 오디언스를 진행하도록 설정되어 있습니다.
...
- **사용자 지정 이벤트를 수행합니다:** 선택한 커스텀 이벤트를 수행하는 사용자 타겟팅(커스텀 이벤트는 SDK를 사용하여 전송되어야 합니다).
...
![ "특정 구매하기"가 트리거 동작으로 선택되었습니다.]({% image_buster /assets/img_archive/canvas_trigger_actions.png %}){: style="max-width:90%"}
...
- 사용자는 동일한 캔버스 단계 내에서 최대 10개의 인앱 메시지에 적격이 될 수 있습니다. 예를 들어, 캔버스가 재입장을 허용하고 사용자가 캔버스에 11번 들어가면, 만료되지 않은 경우 10개의 인앱 메시지만 전송됩니다.
