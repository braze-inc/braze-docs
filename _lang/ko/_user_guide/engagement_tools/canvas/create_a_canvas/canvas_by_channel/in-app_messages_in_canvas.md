...
![The Message Controls section of a Message step for an in-app message. 인앱 메시지는 해당 단계를 사용할 수 있게 된 후 3일 후에 만료됩니다.]({% image_buster /assets/img_archive/canvas_expiration2.png %}){: style="max-width:90%"}

| 옵션 | 설명 | 예시 |
|---|---|---|
| **이 단계 이후의 기간을 사용할 수 있음** | 사용자가 단계를 사용할 수 있게 된 시점을 기준으로 인앱 메시지가 만료되도록 설정합니다. | 사용자가 메시지 단계로 들어가 오디언스 옵션을 선택하면 2일 만료되는 인앱 메시지를 사용할 수 있게 됩니다. 이 단계에 도달하기 전의 지연은 캔버스의 앞선 지연 단계에서 발생합니다. 인앱 메시지는 사용자가 해당 단계에 진입한 시점부터 2일(48시간) 동안 표시되며, 이 2일 동안 사용자가 앱을 열면 인앱 메시지를 볼 수 있습니다. |
...
!["환영 IAM"과 "후속 IAM"의 두 단계가 있는 우선순위 정렬기입니다.]({% image_buster /assets/img_archive/canvas_priority2.png %}){: style="max-width:85%"}
...
원본 편집기에서 만든 캔버스는 캔버스 구성 요소를 통해 발전하는 기준인 진행 동작을 지정해야 합니다. [인앱 메시지만 있는 단계는](#steps-iam-only) 푸시 또는 이메일 같이 [여러 메시지 유형이 있는 단계](#steps-multiple-channels)와 다른 진행 옵션이 있습니다. 현재 캔버스 워크플로우의 인앱 메시지의 경우, 이 옵션은 항상 오디언스에게 즉시 진행되도록 설정되어 있습니다.
...
- **사용자 지정 이벤트를 수행합니다:** 선택한 커스텀 이벤트를 수행하는 사용자를 타겟팅합니다(커스텀 이벤트는 소프트웨어 개발 키트를 사용하여 전송해야 합니다).
...
![트리거 동작으로 "특정 구매하기"를 선택합니다.]({% image_buster /assets/img_archive/canvas_trigger_actions.png %}){: style="max-width:90%"}
...
- 사용자는 동일한 캔버스 단계 내에서 최대 10개의 인앱 메시지를 받을 수 있습니다. 예를 들어, 캔버스에 재입장이 허용되고 사용자가 캔버스에 11번 입장한 경우 만료된 메시지가 없는 경우 10개의 인앱 메시지만 전송됩니다.
