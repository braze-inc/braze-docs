...
> 캔버스가 커스텀 이벤트, 구매 또는 API 호출에 의해 트리거될 때, API 호출, 커스텀 이벤트 또는 구매 이벤트의 메타데이터를 사용하여 캔버스 워크플로의 각 단계에서 개인화를 할 수 있습니다. 이 속성을 사용하여 더 선별된 메시지를 보낼 수 있습니다.

{% alert important %}
지속적인 항목 속성은 원래 캔버스 편집기의 유물로, 역사적 참조를 위해 남아 있는 캔버스 항목 속성과 같은 용어에 대한 사용 중단된 참조가 있습니다. 현재 업데이트된 캔버스 편집기에 대해서는 [Context and event properties]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties)를 참조하십시오.
{% endalert %}
...
이 요청에서 "음식 알레르기"의 전역 값은 "없음"입니다. Customer_123,의 값은 "유제품"입니다. 이 캔버스의 메시지는 Liquid 스니펫 {%raw%}`{{canvas_entry_properties.${food_allergies}}}`{%endraw%}와 함께 "유제품"을 Customer_123에 템플릿하고 다른 모든 사람에게는 "없음"으로 설정됩니다. 
