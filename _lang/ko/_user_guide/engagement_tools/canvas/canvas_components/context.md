---
nav\_title: 컨텍스트 글\_제목: 컨텍스트 별칭: /context/ page\_order: 1.5 페이지 유형: 참조 설명: "이 참조 문서에서는 캔버스에서 컨텍스트 단계를 만들고 사용하는 방법에 대해 설명합니다." 도구: 캔버스
...
{% 경고 중요 %} 컨텍스트 단계는 현재 얼리 액세스 중입니다. 이 얼리 액세스에 참여하려면 Braze 계정 관리자에게 문의하세요. {% endalert %}

## 작동 방식

각 컨텍스트 단계는 변수 이름과 연결된 데이터 유형 또는 컨텍스트 변수(이전에는 캔버스 항목 속성이라고 함)로 구성됩니다. 이러한 변수는 캔버스를 통해 사용자를 따라다니며 Liquid `컨텍스트를` 사용하여 액세스할 수 있습니다.

캔버스의 첫 번째 단계인 컨텍스트 단계]\[1]{:: style="float:right;max-width:40%;margin-left:15px;"}
