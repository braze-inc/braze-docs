...
> 실험 경로를 사용하면 사용자 여정의 어느 시점에서든 여러 캔버스 경로를 서로 비교하고 대조군을 테스트할 수 있습니다. 이 구성 요소를 사용하면 경로 성능/성과를 추적하여 캔버스 여정에 대한 정보에 기반한 결정을 내릴 수 있습니다.
...
{% alert important %}
실험 오염을 방지하기 위해 캔버스에 활성 또는 진행 중인 우승 경로 또는 개인화된 경로 실험이 있고 활성 캔버스를 업데이트하는 경우 실험 경로 단계 자체를 업데이트하더라도 진행 중인 실험이 종료되고 실험 단계에서 우승 경로 또는 개인화된 경로가 결정되지 않습니다. To restart the experiment, you can disconnect the existing Experiment Path and launch a new one, or duplicate the Canvas and launch a new Canvas. 그렇지 않으면 사용자는 최적화 방법을 선택하지 않은 것처럼 실험 경로를 통과하게 됩니다. 또한 실험 경로 단계가 있는 이미 활성화된 캔버스에 대해서는 개인화된 경로 또는 위닝 경로를 켤 수 없습니다.<br><br>For more information, refer to [Editing Canvases after launch]({{site.baseurl}}/post-launch_edits/).
{% endalert %}

## 성과 추적

**캔버스 분석** 페이지에서 실험 경로를 선택하면 **배리언트 분석** 탭과 동일한 [세부 테이블이]({{site.baseurl}}/user_guide/engagement_tools/canvas/get_started/measuring_and_testing_with_canvas_analytics/#performance-breakdown-by-variant) 열리고 경로 간의 자세한 성능/성과 및 전환 통계를 비교할 수 있습니다. 또한 CSV를 통해 테이블을 내보내고 선택한 경로 또는 컨트롤을 기준으로 관심 있는 지표의 변경 비율을 비교할 수도 있습니다.

Each step in each path will display statistics in the [Canvas Analytics]({{site.baseurl}}/user_guide/engagement_tools/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics/) view, just like any Canvas step. 그러나 개별 단계에 대한 분석은 실험의 구조를 **고려하지 않는다는** 점에 유의하세요. 실험 단계의 분석은 여러 경로를 비교하는 데 사용해야 합니다.
...
실험 경로는 각 단계를 입력하고 지정된 경로에 있는 동안 변환하는 사용자를 기록합니다. 이렇게 하면 캔버스 설정에 지정된 모든 전환 이벤트를 추적할 수 있습니다. **추가 설정** 탭에서 이 실험에서 전환을 추적할 일수(1~30일 사이)를 입력합니다. 여기에서 지정하는 시간 창에 따라 실험에서 전환 이벤트(캔버스 설정에서 선택)를 추적할 기간이 결정됩니다. 캔버스 설정에 지정된 이벤트별 전환 이벤트 기간은 이 단계의 추적에 적용되지 않으며 이 전환 기간으로 대체됩니다.
