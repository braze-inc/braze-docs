...
![두 그룹이 있는 오디언스 경로: 참여한 사용자와 나머지 모든 사용자.]({% image_buster /assets/img/audience_path/audience_path.png %}){: style="float:right;max-width:45%;margin-left:15px;margin-top:15px;"}
...
![메시지 단계 후 24시간 지연을 보여주는 캔버스, 그 뒤에 오디언스 경로가 이어집니다.]({% image_buster /assets/img/audience_path/audience_path5.png %}){: style="float:right;max-width:40%;margin-left:15px;"}

사용자는 오디언스 경로 단계에 도달할 때 필터 및 세그먼트 멤버십 **기준으로 평가됩니다**—캔버스에 들어갔을 때가 아닙니다. 평가 후, 그들은 즉시 일치하는 경로로 진행합니다. 사용자가 오디언스 그룹에 배치되면, 이후 사용자 프로필이 변경되더라도 그 그룹에 남아 있습니다.

{% alert important %}
오디언스 경로는 평가 시 사용자의 현재 속성, 필터 및 세그먼트 멤버십을 기준으로 평가합니다. 특정 이벤트에 따라 캔버스에 들어간 것을 기준으로 평가하지 않습니다. 사용자가 수행하는 행동(예: 커스텀 이벤트)에 따라 라우팅하려면 [액션 경로]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/action_paths/)를 대신 사용하세요.
{% endalert %}

### 사용자 평가를 위한 시간을 허용합니다.

평가가 즉각적이기 때문에, 경로 기준이 이전 단계와의 사용자 상호작용에 의존하는 경우 오디언스 경로 전에 지연을 추가하는 것이 중요합니다.
...
사용자가 보낸 메시지와 상호작용할 시간을 주기 위해 메시지 단계와 오디언스 경로 사이에 지연을 추가하세요. 예를 들어, 24시간 지연은 메시지 A가 전송된 후 24시간 동안 사용자가 상호작용할 수 있는 시간을 제공합니다.
...
!["아시아 요리를 좋아하는 사람", "라틴 요리를 좋아하는 사람", "유럽 요리를 좋아하는 사람" 및 "모든 사람" 그룹이 있는 확장된 오디언스 경로.]({% image_buster /assets/img/audience_path/audience_path3.png %})

오디언스 경로 단계가 완료되면 각 오디언스 그룹은 별도의 분기를 갖게 됩니다. 오디언스 경로를 계속 사용하여 오디언스를 추가로 필터링하거나 표준 캔버스 단계를 사용하여 캔버스 여정을 계속할 수 있습니다. 

![참여도를 기준으로 한 서로 다른 그룹이 있는 두 개의 오디언스 경로.]({% image_buster /assets/img/audience_path/audience_path4.png %}){: style="max-width:50%"}
...
![“대형 브랜드 신발을 좋아하는 사람”, “대형 브랜드를 좋아하는 사람” 및 "모든 사람" 그룹이 있는 오디언스 경로.]({% image_buster /assets/img/audience_path/audience_path2.png %}){: style="float:right;max-width:50%;margin-left:15px;margin-bottom:15px;"}
