...
다른 팀원이 캔버스를 업데이트한 횟수는 캔버스 개요 페이지의 *마지막으로 본 이후 변경사항* 측정기준을 통해 추적할 수 있습니다. **마지막으로 본 이후 변경사항을** 선택하면 캔버스 이름, 일정, 태그, 메시지, 오디언스, 승인 상태 또는 팀 액세스 구성에 대한 업데이트 체인지로그를 볼 수 있습니다. 각 업데이트마다 누가 업데이트를 수행했는지와 언제 수행했는지 확인할 수 있습니다. 이 체인지로그를 사용하여 캔버스에 대한 변경 사항을 감사할 수 있습니다.
...
![캔버스 쿠성요소의 성능 세부 정보에 대한 두 가지 예시입니다. 왼쪽에는 하나의 캔버스 구성요소가 있는 사용자 경로에 대한 성능 세부 정보가 표시됩니다. 오른쪽에는 확장된 캔버스 구성요소의 실적 세부 정보와 인앱 메시지 노출 수를 표시하는 중첩 단계가 표시됩니다.]({% image_buster /assets/img_archive/Journey_6.png %})
...
사용자는 캔버스 항목당 전환 이벤트당 한 번만 전환할 수 있습니다. 전환은 해당 항목에 대해 사용자가 가장 최근에 수신한 메시지에 할당됩니다. 캔버스 요약에는 해당 경로에서 사용자가 수행한 모든 전환 성능/성과와 메시지 수신 여부가 반영됩니다. 이후의 각 단계는 사용자가 가장 최근에 받은 단계인 동안에 발생한 전환만 표시합니다. 
...
마지막 예로, 재인증이 켜져 있는 캔버스가 있다고 가정해 보겠습니다. 재적격 사용자가 첫 번째 항목과 두 번째 항목에서 전환 이벤트를 수행하면 두 번의 전환이 계산됩니다.
