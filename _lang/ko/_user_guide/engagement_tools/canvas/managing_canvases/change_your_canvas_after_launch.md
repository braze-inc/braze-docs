...
현지 전송 시간에 시작하도록 예약된 캔버스는 예약된 전송 시간 24시간 전까지 편집할 수 있습니다. 이 창은 "안전 구역"이라고 합니다. 
...
{% alert important %}
다음과 같은 문제는 피할 수 있습니다. 캔버스를 시작한 후 캔버스를 편집해야 하는 경우 먼저 캔버스에 이미 들어온 모든 사용자가 사용자 여정을 완료했는지 확인하는 것이 좋습니다. 또한 적어도 한 명의 사용자가 이미 처리한 단계는 삭제하지 않는 것이 좋습니다.
{% endalert %}
...
- 기존 요금 제한으로 인해 사용자가 메시지를 수신하지 못하는 경우
  - 사용자가 캔버스에 파견되면 사용자가 파견될 때 캔버스에 적용된 요금 제한이 사용자에게 적용됩니다. 캔버스가 전송된 후에는 해당 사용자에 대한 요금 제한을 편집할 수 없으므로 전송 후 요금 제한을 늘리거나 줄여도 이미 전송된 사용자에게는 영향을 미치지 않습니다.
