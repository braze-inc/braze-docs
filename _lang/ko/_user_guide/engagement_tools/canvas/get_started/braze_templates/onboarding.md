...
- 앱 사용 방법에 대한 팁이 담긴 이메일
- 사용자 설문조사가 포함된 피드백 이메일

## 필요에 맞게 템플릿 조정하기

팬츠래비린스에서 일하며 사용자 참여도를 높이고, 사용자와의 신뢰와 로열티를 구축하며, 지속적인 참여를 유도하는 것이 목표라고 가정해 보겠습니다. 이를 위해 아직 앱과 상호작용하지 않은 신규 사용자를 타겟팅하는 메시지를 제작하는 데 집중하고자 합니다.

온보딩 템플릿에 액세스하려면 새 캔버스를 만들 때 **캔버스 템플릿 사용** > **Braze 템플릿을** 선택합니다. 그런 다음 **온보딩** 옆의 **템플릿 적용을** 선택합니다. 사용 사례에 맞게 이 템플릿을 커스텀해 보겠습니다.
...
팬츠라비린스의 신규 사용자를 타겟팅하는 것이 목표이므로 캔버스는 액션 기반으로 유지합니다. **세션 시작의** 경우 모든 **앱에서** 세션 시작을 선택하여 모든 앱에서 세션을 시작한 사용자가 캔버스에 입장할 수 있도록 허용합니다.
...
적절한 오디언스를 타겟팅하면 신규 사용자의 참여를 효과적으로 유도할 수 있습니다. 예를 들어 이 템플릿은 앱을 처음 사용한 지 하루가 되지 않은 모든 사용자를 타겟팅하므로 사용 사례에 정확합니다. 따라서 이 섹션은 그대로 두겠습니다.
...
2. **메시지 편집을** 선택하여 템플릿의 이메일을 환영 이메일로 바꿉니다.
3. **완료를** 선택합니다.

이제 사용자는 앱에서 세션을 시작한 후 이 환영 이메일을 받게 됩니다. 반복되는 메시징으로 사용자에게 부담을 주지 않으려면 사용자 여정의 일부로 지연 단계를 사용하는 것이 좋습니다.
