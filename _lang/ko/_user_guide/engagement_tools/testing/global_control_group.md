...
> 글로벌 컨트롤 그룹을 사용하여 캠페인이나 캔버스를 받지 말아야 하는 전체 사용자의 비율을 지정하면 시간에 따른 메시징 활동의 전반적인 영향을 분석할 수 있습니다. 

메시징을 받은 사용자와 그렇지 않은 사용자의 행동을 비교함으로써 마케팅 캠페인과 캔버스가 세션 및 커스텀 이벤트의 향상에 어떻게 기여하는지 더 잘 이해할 수 있습니다.

## 글로벌 제어 그룹의 작동 방식

글로벌 제어 그룹을 사용하면 모든 사용자의 비율을 제어 그룹으로 설정할 수 있습니다. 저장되면 그룹의 사용자는 캠페인이나 캔버스를 받지 않습니다. 

{% alert important %}
Your Global Control Group applies to all channels, campaigns, and Canvases, except for [API campaigns]({{site.baseurl}}/api/api_campaigns). 즉, 대조군의 사용자는 여전히 API 캠페인을 받게 됩니다. However, this exception doesn't apply to Content Cards. If you're using an API-triggered Content Card campaign, users in your control group won't receive them.
{% endalert %}
...
글로벌 컨트롤 그룹이 생성되면 무작위 버킷 번호를 가진 모든 사용자가 이 그룹에 속하게 됩니다. 또한 이 시점 이후에 가입한 신규 사용자(글로벌 컨트롤 그룹이 생성된 이후에 가입한 사용자)도 이러한 무작위 버킷 번호를 가지고 있으면 글로벌 컨트롤 그룹에 추가됩니다. 마찬가지로 많은 사용자가 삭제되면 삭제된 사용자 중 일부가 이 그룹에 속하게 되므로 글로벌 컨트롤 그룹의 크기가 줄어들 것으로 예상할 수 있습니다. 이렇게 하면 전체 사용자 기반에 비해 그룹의 크기가 일정한 비율로 유지됩니다.
...
치료 그룹은 글로벌 컨트롤 그룹과 규모가 비슷하지만 정확히 같은 규모는 아닐 수 있습니다. [보고를](#reporting) 위해 Braze는 대조군에 속한 사용자와 치료 샘플에 속한 사용자의 행동을 측정합니다. 각 작업 공간에는 최대 하나의 글로벌 대조군 그룹과 하나의 치료 샘플 그룹이 있습니다. 치료 샘플 그룹은 글로벌 제어 보고를 구성하는 방법에 관계없이 동일한 사용자 그룹입니다.
...
대조군에 대한 백분율을 입력하고 **저장을** 선택합니다. 입력하면 Braze는 글로벌 컨트롤, 치료 및 치료 샘플에 해당하는 사용자 수 추정치를 표시합니다. 워크스페이스에 사용자가 많을수록 이 추정치는 더 정확해진다는 점에 유의하세요. 

The number of users in your Global Control Group automatically updates after its initial setup to remain proportionate to this percentage when more users are added to your workspace. 또한 글로벌 컨트롤 그룹이 설정된 후 가입한 사용자 중 무작위 버킷 번호를 가진 사용자도 글로벌 컨트롤 그룹에 추가됩니다. 많은 사용자가 추가되면 글로벌 컨트롤 그룹의 크기가 커져 전체 사용자 기반에 비해 일정한 비율을 유지하게 됩니다. 글로벌 컨트롤 그룹의 규모가 커져도 이전에 그룹에 속해 있던 사용자는 그룹을 비활성화하고 새 그룹을 만들어 그룹을 변경하지 않는 한 그룹에 계속 남아 있습니다.

For percentage guidelines, refer to [Testing best practices](#percentage-guidelines).

![오디언스 설정이 "모든 사용자의 5%를 글로벌 컨트롤 그룹에 할당"으로 설정된 글로벌 컨트롤 그룹 설정입니다.]({% image_buster /assets/img/control_group/control_group4.png %})
...
**글로벌 컨트롤 그룹 설정** 탭에서 언제든지 글로벌 컨트롤 그룹을 비활성화할 수 있지만, 비활성화하면 이 그룹의 사용자가 즉시 캠페인과 캔버스를 사용할 수 없게 됩니다.
...
대조군을 비활성화한 후 새 제어 그룹을 저장할 수 있습니다. 백분율을 입력하고 저장하면 Braze에서 무작위로 선택된 새로운 사용자 그룹을 생성합니다. 이전과 동일한 비율을 입력하면 Braze에서 대조군 및 치료군에 대한 새 사용자 그룹을 생성합니다.

![글로벌 컨트롤 그룹을 비활성화하면 더 이상 신규 또는 활성 캠페인이나 캔버스에서 제외되지 않는다는 경고 텍스트와 함께 "글로벌 메시지 설정을 변경하고 있습니다"라는 제목의 대화 상자가 표시됩니다.]({% image_buster /assets/img/control_group/control_group2.png %}){: style="max-width:60%" }
...
![사용자가 무작위 버킷 번호 6356을 가지고 있으며 글로벌 컨트롤 그룹에 속하지 않음을 알리는 '기타' 섹션입니다.]({% image_buster /assets/img/control_group/control_group1.png %}){: style="max-width:50%;"}
...
글로벌 컨트롤 그룹은 무작위 버킷 번호를 사용하여 구성되므로 무작위 버킷 번호 세그먼트 필터를 사용하여 다른 테스트를 실행하는 경우 생성한 세그먼트와 글로벌 컨트롤 그룹 사용자 간에 중복될 수 있다는 점에 유의하세요.

#### 중복 이메일 주소

외부 사용자 ID가 다른 두 사용자의 이메일 주소가 같고 그 중 한 사용자는 대조군에 속하고 다른 사용자는 대조군에 속하지 않은 경우, 대조군이 아닌 사용자가 이메일을 받을 자격이 있는 경우에도 해당 이메일 주소로 이메일이 전송됩니다. 이 경우 두 고객 프로필 모두 해당 이메일이 포함된 캠페인 또는 캔버스를 수신한 것으로 표시합니다.

#### 글로벌 제어 그룹 및 메시지별 제어 그룹

글로벌 컨트롤 그룹을 사용할 수도 있고 캠페인별 또는 캔버스별 대조군을 사용할 수도 있습니다. 캠페인별 또는 캔버스별 대조군을 사용하면 특정 메시지의 영향력을 측정할 수 있습니다.

글로벌 관리 그룹의 사용자는 태그 예외가 있는 메시지 외에는 수신이 보류되며, 캠페인이나 캔버스에 관리 기능을 추가하면 Braze는 글로벌 관리 그룹의 일부가 해당 특정 캠페인이나 캔버스를 수신하지 못하도록 보류합니다. 즉, 글로벌 컨트롤 그룹의 구성원이 특정 캠페인 또는 캔버스를 받을 자격이 없는 경우 해당 캠페인 또는 캔버스의 대조군에 포함되지 않습니다.
...
총 오디언스가 10,000명 미만인 경우 비율을 높여 1,000명 이상의 사용자 그룹을 만들어야 하며, 이 경우 비율을 15%보다 높게 늘리지 않아야 합니다. 전체 작업 공간의 크기가 작을수록 통계적으로 엄격한 테스트를 실행하기가 더 어렵다는 점을 명심하세요.
