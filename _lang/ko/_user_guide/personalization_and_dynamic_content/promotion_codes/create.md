...
각 프로모션 코드 목록에는 생성 시 설정되는 해당 만료 날짜와 시간이 있습니다. 최대 만료 기간은 목록을 생성하거나 편집한 날로부터 6개월입니다.
...
임계값 알림을 선택 사항으로 설정하고 사용자 지정할 수도 있습니다. 설정된 경우, 이러한 알림은 목록에 있는 사용 가능한 프로모션 코드가 부족하거나 프로모션 코드 목록이 만료에 가까워질 때 지정된 수신자에게 이메일을 보냅니다. 수신자는 하루에 한 번 알림을 받습니다.

![프로모션 코드 목록이 5일 후에 만료될 때 "marketing@abc.com"에 알리기 위한 임계값 알림의 예입니다.]({% image_buster /assets/img/promocodes/promocode5.png %}){: style="max-width:80%"}
...
{% alert important %}
최대 파일 크기는 100MB이고 최대 목록 크기는 2000만 개의 사용되지 않은 코드입니다. 잘못된 파일이 업로드된 경우, 이전 파일을 대체할 새 파일을 업로드하세요.
{% endalert %}
...
![업로드 중인 프로모션 코드입니다.]({% image_buster /assets/img/promocodes/promocode8.png %})

{% alert note %}
더 큰 파일은 가져오는 데 몇 분이 걸립니다. 기다리는 동안 페이지에서 나가서 가져오기가 진행되는 동안 다른 작업을 할 수 있습니다. 가져오기가 완료되면 상태가 테이블의 **완료**로 변경됩니다.
{% endalert %}

## 프로모션 코드 목록 업데이트

목록을 업데이트하려면 기존 목록 중 하나를 선택합니다. 이름, 설명, 목록 만료 및 임계값 알림을 변경할 수 있습니다. 새 파일을 업로드하고 **목록 업데이트**를 선택하여 목록에 코드를 더 추가할 수도 있습니다. 목록의 모든 코드는 가져오기 날짜와 관계없이 동일한 만료일을 가집니다.
...
- 전체 목록 사용 중단: 현재 프로모션 코드 목록을 캠페인, 캔버스 또는 템플릿에서 사용하지 마세요. 그런 다음, 올바른 코드가 포함된 CSV 파일을 업로드하고 메시징에 사용하세요.
- 잘못된 코드 사용하기: 잘못된 프로모션 코드 목록에서 프로모션 코드를 자리 표시자에 보내는 캠페인을 생성하여 모든 잘못된 코드가 사용될 때까지 기다립니다. 그런 다음, 동일한 목록에 올바른 프로모션 코드를 업로드하세요.
