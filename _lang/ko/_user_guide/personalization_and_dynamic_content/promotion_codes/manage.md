...
사용자가 여러 채널에서 코드 자격을 갖추면, 각 채널에서 동일한 코드를 받습니다. 예를 들어, 이메일과 푸시로 메시지를 받는 경우, 동일한 코드가 두 곳에 전송됩니다. 보고서에도 단일 코드가 반영됩니다.

{% alert note %}
사용 가능한 프로모션 코드가 없으면, 코드에 의존하는 테스트 또는 실시간 메시지가 전송되지 않습니다.
{% endalert %}
...
둘째, 사용자 정의 속성(이 예에서는 {% raw %}`{{custom_attribute.${Promo Code}}}`{% endraw %})을 메시지에 추가합니다. 할인 코드는 템플릿에 포함되어 있습니다.
