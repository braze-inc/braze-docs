...
편의를 위해 지원되는 개인화 태그에 대한 요약이 제공됩니다. 각 태그 유형 및 모범 사례에 대한 자세한 내용은 계속 읽어보세요.
...
{% raw %}
- `dispatch_id` 동작이 다른 이유는 Braze가 캔버스 단계가 '예약된' 경우에도 트리거된 이벤트로 취급하기 때문입니다(예약 가능한 입력 단계 제외). 자세히 알아보려면 [디스패치 ID 동작을]({{site.baseurl}}/help/help_articles/data/dispatch_id/) 참조하세요.
...
디바이스 사업자, 모델명, 운영체제의 종류가 매우 다양하므로 이러한 값에 조건부로 의존하는 모든 Liquid를 철저하게 테스트하는 것이 좋습니다. 이 값은 특정 기기에서 사용할 수 없는 경우 `null`입니다.
...
| `{{targeted_device.${google_ad_id}}}` | Android 기기의 경우, 이 값은 애플리케이션이 [선택적 Google Play 광고 ID 수집]으로 구성된 경우 Google Play 광고 식별자가 됩니다. Android 이외의 기기의 경우 이 값은 null이 됩니다. |
...
또한, 푸시 알림의 경우, Braze가 푸시 토큰이 API를 통해 가져온 경우와 같은 특정 상황에서 푸시 알림에 연결된 기기를 식별할 수 없을 수 있으며, 이로 인해 해당 메시지의 값이 `null`이 될 수 있습니다.
...
예를 들어, 고객에게 보상 잔액 알림을 보내고 있다고 가정해 보겠습니다. 기본값을 사용하여 잔액이 낮거나 없는 고객을 고려하는 좋은 방법이 없습니다.
...
2. 이 고객에게는 다음과 같은 완전히 다른 메시지를 보냅니다:
...
이 사용 사례에서는 빈 또는 null인 이름을 가진 사용자가 "다운로드해 주셔서 감사합니다"라는 메시지를 받습니다. 이름에 대한 [기본값을]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/setting_default_values/) 포함해야 실수로 고객이 Liquid를 보지 않도록 할 수 있습니다.
...
{% raw %}
반복 태그는 코드 블록을 반복적으로 실행하는 데 사용할 수 있습니다. 아래 사용 사례에는 `for` 태그가 포함되어 있습니다.
...
이 사용 사례에서는 스니커즈 브랜드 조회 배열의 처음 다섯 항목을 확인합니다. 그 항목 중 하나가 컨버스인 경우, `converse_viewer` 변수를 생성하고 true로 설정합니다.

그런 다음, `converse_viewer`이 true일 때 판매 메시지를 보냅니다. 그렇지 않으면 메시지를 중단합니다.
...
구문 태그를 사용하여 Liquid가 렌더링되는 방식을 제어할 수 있습니다. `echo` 태그를 사용하여 표현식을 반환할 수 있습니다. 이 태그는 중괄호를 사용하여 표현식을 래핑하는 것과 동일하지만 Liquid 태그 내에서 이 태그를 사용할 수 있다는 점이 다릅니다. `liquid` 태그를 사용하여 각 태그에 구분 기호 없이 Liquid 블록을 만들 수도 있습니다. `liquid` 태그를 사용할 때는 각 태그가 고유한 줄에 있어야 합니다. 자세한 내용과 예제는 [구문 태그에](https://shopify.dev/api/liquid/tags#syntax-tags) 대한 Shopify 문서를 확인하십시오.
...
먼저 로컬 변수로 저장한 다음 `__http_status_code__` 키를 사용하여 [커넥티드 콘텐츠]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content/) 호출의 HTTP 상태를 활용할 수 있습니다. 예를 들어, 다음과 같습니다.
...
{% alert note %}
이 키는 엔드포인트가 JSON 개체를 반환하는 경우에만 연결된 콘텐츠 개체에 자동으로 추가됩니다. 엔드포인트가 배열 또는 다른 유형을 반환하는 경우, 해당 키는 응답에서 자동으로 설정될 수 없습니다.
{% endalert %}
...
이 사용 사례에서는 가장 최근 로케일이 `pt_BR`인 고객에게 브라질 포르투갈어로 메시지가 전송되고, 가장 최근 로케일이 `pt_PT`인 고객에게는 유럽 포르투갈어로 메시지가 전송됩니다. 앞의 두 조건을 충족하지 않지만 언어를 포르투갈어로 설정한 고객에게는 기본 포르투갈어 유형이 무엇이든 원하는 언어로 메시지가 표시됩니다.

### Use case: 시간대별로 사용자 타겟팅하기

시간대별로 사용자를 타겟팅할 수도 있습니다. 예를 들어, 상대방의 기준이 동부 표준시인 경우 한 메시지를 보내고 태평양 표준시인 경우 다른 메시지를 보내세요. 이를 위해 현재 시간을 UTC로 저장하고, 사용자의 현재 시간과 비교하여 올바른 시간대에 맞는 메시지를 전송하기 위해 if/else 문을 사용합니다. 사용자의 로컬 시간대에 캠페인을 설정하여 적절한 시간에 캠페인을 제공해야 합니다. 

오후 2시에서 3시 사이에 전송될 메시지를 작성하는 방법에 대한 다음 사용 사례를 참조하십시오.
