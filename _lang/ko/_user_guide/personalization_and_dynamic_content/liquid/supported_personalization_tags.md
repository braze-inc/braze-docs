...
| 이벤트 등록정보 <br> (워크스페이스에 맞게 사용자 지정됩니다.)| `{{event_properties.${your_custom_event_property}}}` |
| 캔버스 진입 등록정보| `{{canvas_entry_properties}}` |
| 커스텀 속성 <br> (워크스페이스에 맞게 사용자 지정됩니다.) | `{{custom_attribute.${your_custom_attribute}}}` |
...
{% raw %}
- `dispatch_id` 는 캔버스 단계가 '예약된' 경우에도 트리거된 이벤트로 취급하기 때문에 캔버스와 캠페인이 다릅니다(예약 가능한 엔트리 단계는 제외). 자세한 내용은 \[디스패치 ID 동작][50] 을 참조하세요.
- Canvas에 `{{campaign.${name}}}` 태그를 사용하면 Canvas 구성 요소 이름이 표시됩니다. 이 태그를 캠페인에 사용하면 캠페인 이름이 표시됩니다.
{% endraw %}
...
|`{{most_recently_used_device.${id}}}` | 이것은 Braze 디바이스 식별자입니다. iOS에서는 공급업체용 Apple 식별자(IDFV) 또는 UUID가 될 수 있습니다. Android 및 기타 플랫폼의 경우 무작위로 생성된 UUID입니다. |
...
| `{{most_recently_used_device.${idfa}}}` | iOS 디바이스의 경우, 애플리케이션이 \[선택적 IDFA 컬렉션][40]]으로 구성된 경우 이 값은 광고용 식별자(IDFA)가 됩니다. iOS가 아닌 디바이스의 경우 이 값은 null이 됩니다. |
...
| `{{most_recently_used_device.${roku_ad_id}}}` | Roku 디바이스의 경우, 이 값은 애플리케이션이 Braze로 구성될 때 수집되는 Roku 광고 식별자가 됩니다. Roku가 아닌 디바이스의 경우 이 값은 null이 됩니다. |
...
디바이스 사업자, 모델명, 운영체제가 매우 다양하므로 이러한 값에 조건부로 의존하는 모든 Liquid를 철저하게 테스트하는 것이 좋습니다. 이 값은 특정 디바이스에서 사용할 수 없는 경우 `null` 입니다.
...
| `{{app.${name}}}` | 메시지를 요청하는 앱의 이름(Braze 대시보드에 정의된 대로)|
{: .reset-td-br-1 .reset-td-br-2}
...
푸시 알림 및 인앱 메시지 채널의 경우 메시지가 전송되는 디바이스에 대해 다음 속성을 템플릿으로 만들 수 있습니다. 즉, 푸시 알림 또는 인앱 메시지에는 메시지를 읽고 있는 디바이스의 디바이스 속성이 포함될 수 있습니다. 이러한 속성은 콘텐츠 카드에는 작동하지 않습니다. 
...
| `{{targeted_device.${idfa}}}` | iOS 디바이스의 경우, 애플리케이션이 \[선택적 IDFA 컬렉션][40]]으로 구성된 경우 이 값은 광고용 식별자(IDFA)가 됩니다. iOS가 아닌 디바이스의 경우 이 값은 null이 됩니다. |
| `{{targeted_device.${google_ad_id}}}` | Android 디바이스의 경우, 애플리케이션이 \[선택 사항인 Google Play 광고 ID 수집]]으로 구성된 경우 이 값은 Google Play 광고 식별자가 됩니다. Android 이외의 기기의 경우 이 값은 null이 됩니다. |
| `{{targeted_device.${roku_ad_id}}}` | Roku 디바이스의 경우, 이 값은 애플리케이션이 Braze로 구성될 때 수집되는 Roku 광고 식별자가 됩니다. Roku가 아닌 디바이스의 경우 이 값은 null이 됩니다. |
...
| `{{targeted_device.${foreground_push_enabled}}}` | 이 값은 대상 디바이스가 포그라운드 푸시를 사용하도록 설정된 경우 `true`, 그렇지 않은 경우 `false` 입니다. |
{: .reset-td-br-1 .reset-td-br-2}
...
디바이스 사업자, 모델명, 운영 체제가 매우 다양하므로 이러한 값에 조건부로 의존하는 로직을 철저하게 테스트하는 것이 좋습니다. 이 값은 특정 디바이스에서 사용할 수 없는 경우 `null` 입니다. 또한 푸시 알림의 경우, API를 통해 푸시 토큰을 가져온 경우와 같이 특정 상황에서 Braze가 푸시 알림에 연결된 디바이스를 식별하지 못해 해당 메시지에 대해 `null` 값이 표시될 수 있습니다.

![푸시 메시지에서 이름 변수를 사용할 때 기본값인 "there"를 사용하는 예시입니다.][4]

경우에 따라 기본값을 설정하는 대신 \[조건부 로직][17] ]을 사용할 수도 있습니다. 조건부 논리를 사용하면 사용자 지정 속성의 값에 따라 달라지는 메시지를 보낼 수 있습니다.

또한 조건부 논리를 사용하여 \[null 또는 빈 속성 값을 가진 고객에게][18] 메시지를 중단]할 수 있습니다.

예를 들어 고객에게 리워드 잔액 알림을 보내는 경우 기본값을 사용하여 잔액이 낮거나 0인 고객을 설명할 수 있는 좋은 방법이 없습니다.
...
2. 이러한 고객에게 완전히 다른 메시지, 예를 들어 다음과 같은 메시지를 보내세요:
...
이 예에서 이름이 비어 있거나 0인 사용자에게는 "다운로드해 주셔서 감사합니다"라는 메시지가 표시됩니다. 이름에 \[기본값][47] ]을 포함해야 실수 시 고객에게 Liquid가 표시되지 않습니다.
