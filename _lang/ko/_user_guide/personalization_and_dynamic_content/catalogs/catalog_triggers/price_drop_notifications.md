...
{% alert important %}
카탈로그의 가격 하락 알림은 현재 초기 액세스 중입니다. 관심이 있으시면 계정 매니저에게 연락하세요.
{% endalert %}
...
3. 글로벌 카탈로그 설정이 구성되지 않은 경우, 알림을 트리거하는 데 사용할 커스텀 이벤트 및 속성을 설정하라는 메시지가 표시됩니다:
    ...
    ```
{% alert note %}
재입고 및 가격 인하 트리거는 동일한 이벤트를 사용하여 사용자가 알림에 가입하도록 합니다. `type`를 `price_drop`으로 설정하여 가격 인하 알림을 생성하세요. 가격 인하와 재입고 알림을 동시에 설정할 수 없습니다.
{% endalert %}

{: start="4"}
4\. **저장**을 선택하고 카탈로그의 **설정** 페이지로 계속 진행하십시오.
5\. 알림 규칙을 설정하세요. 두 가지 옵션이 있습니다:
    **모든 구독 사용자에게 알림**은(는) 상품의 가격이 하락할 때 대기 중인 모든 고객에게 알립니다.
    **지정된 수의 고객에게 설정된 알림 기간에 따라 알림을 보냅니다.** Braze는 알림을 받을 고객 수를 지정된 수만큼 순차적으로 알립니다. 더 이상 알릴 고객이 없거나, 해당 품목의 가격이 다시 오를 때까지 알림을 보냅니다. 알림 속도는 분당 10,000명의 사용자에게 알림을 보낼 수 없습니다.
