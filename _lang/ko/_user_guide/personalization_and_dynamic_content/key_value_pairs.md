...
메시지에 구조화된 메타데이터를 추가하려면 키-값 페어를 사용하세요. 이 추가 데이터 페이로드는 메시지를 렌더링하거나 처리하는 방식에 영향을 줄 수 있는 추가적인 상황별 정보로 메시지를 풍부하게 할 수 있습니다.

키-값 페어가 메타데이터이기 때문에 이 데이터는 반드시 수신자에게 보이지 않지만, 연결된 시스템이나 프로세스에서 메시지 처리를 사용자 정의하는 데 사용할 수 있습니다. 
...
키-값 페어로 메타데이터를 추가하는 몇 가지 사용 사례는 다음과 같습니다:

1. **추적 매개변수:** 분석 목적을 위한 UTM 매개변수 첨부
   - 키: `utm_campaign`
   - 값: `spring_sale`
2. **커스텀 태그:** 내부 라우팅 또는 분류를 위한 태그 추가
   - 키: `priority`
   - 값: `high`
3. **행동 유발기:** 인앱 행동을 트리거하거나 사용자 정의하는 데 사용되는 메타데이터
   - 키: `deep_link`
   - 값: `app://promo-page`

## 푸시 알림

키-값 페어는 Android, iOS 및 웹 푸시 알림에 추가할 수 있습니다. 키-값 페어를 사용하여 내부 측정기준 및 앱 콘텐츠를 업데이트하거나 알림 우선 순위, 현지화 및 소리와 같은 푸시 알림 속성을 사용자 정의할 수 있습니다.
...
Braze를 사용하면 `extras`로 알려진 커스텀 정의 문자열 키-값 페어를 보낼 수 있습니다. API 트리거 및 예약된 API 트리거 캠페인에서 추가 항목에 접근하려면 대시보드에서 키를 "example_key",로 설정하고 값을 {% raw %}`"$json:{"foo": 1, "bar": 1}"`{% endraw %}로 설정하세요. 이로 인해 `"extras": { "test": { "foo": 1, "bar": 1 }`의 개발자 콘솔 출력이 발생합니다
...
[전통적인 편집기]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/)에서 인앱 메시지에 키-값 페어를 추가하려면 **설정** 탭을 선택하고 **새 페어 추가**를 선택한 다음 키-값 페어를 지정하세요.

{% alert note %}
인앱 메시지에 대해 드래그 앤 드롭 편집기를 통해 키-값 페어를 설정할 수 없습니다.
{% endalert %}
...
![Braze의 이메일 메시지 작성기의 보내기 정보 탭.]({% image_buster /assets/img_archive/keyvalue_email.png %})

## 콘텐츠 카드

To add a key-value pair to a Content Card, go to the **Settings** tab in the Braze message composer and select **Add New Pair**.

![콘텐츠 카드에 키-값 페어 추가]({% image_buster /assets/img_archive/kvp_content_cards.png %}){: style="max-width:70%;"}
