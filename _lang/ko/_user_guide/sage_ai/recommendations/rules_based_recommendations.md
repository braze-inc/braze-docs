...
> 규칙 기반 추천 엔진은 사용자 데이터와 제품 정보를 사용하여 메시지 내에서 사용자에게 관련 항목을 제안합니다. [Liquid와]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/) Braze [카탈로그]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/) 또는 [커넥티드 콘텐츠를]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/connected_content/) 사용하여 사용자 행동과 속성을 기반으로 콘텐츠를 동적으로 개인화합니다.

Liquid, 카탈로그 및 커넥티드 콘텐츠에 대해 자세히 알아보려면 다음 Braze 학습 과정을 확인하세요:

- [이메일을 통한 맞춤형 추천](https://learning.braze.com/personalized-recommendations-with-email)
- [리퀴드를 통한 동적 개인화](https://learning.braze.com/path/dynamic-personalization-with-liquid)
- [커넥티드 콘텐츠 기본 사항](https://learning.braze.com/path/dynamic-personalization-with-liquid/connected-content-fundamentals)

{% alert important %}
규칙 기반 권장 사항은 수동으로 설정해야 하는 고정 로직을 기반으로 합니다. 즉, 로직을 업데이트하지 않으면 사용자의 구매 내역과 취향에 맞게 추천이 조정되지 않습니다.<br><br>사용자의 기록에 따라 자동으로 조정되는 개인화된 AI 추천을 만들려면 [AI 항목 추천을]({{site.baseurl}}/ai_item_recommendations) 확인하세요.
{% endalert %}
...
1. 다음 정보가 포함된 CSV를 통해 카탈로그를 생성하고 업로드합니다:<br>- **id:** 사용자가 앱에 가입한 후 경과한 일수와 관련된 고유 번호입니다. 예를 들어 `3` 은 3일과 관련이 있습니다.<br>- **유형입니다:** `comfort`, `fresh` 등과 같은 레시피 카테고리.<br>- **제목:** "이번 주 점심 미리 만들어 놓기" 또는 "타코 먹자"와 같이 각 ID에 대해 전송될 콘텐츠 카드의 제목입니다.<br>- **링크:** 레시피 문서로 연결되는 링크입니다.<br>- **image_url:** 레시피에 해당하는 이미지입니다.
...
{: start="3"}
3\. 콘텐츠 카드 캠페인을 만듭니다. 작성기에서 리퀴드 로직을 입력하여 캠페인을 수신할 사용자와 표시할 레시피 및 이미지를 결정합니다. 이 사용 사례에서 Braze는 사용자의 `start_date` (또는 가입 날짜)를 가져와 현재 날짜와 비교합니다. 날짜 차이에 따라 전송되는 콘텐츠 카드가 결정됩니다. 
...
{: start="4"}
4\. **클릭 시 동작** 섹션에서 사용자가 iOS, Android 및 웹 장치에서 콘텐츠 카드를 클릭할 때 리디렉션해야 하는 위치에 대한 리퀴드 로직을 입력합니다. 
...
{: start="5"}
5\. **테스트** 탭으로 이동하여 **사용자로 메시지 미리보기에서 사용자** **지정 사용자를** 선택합니다. **사용자 지정 속성** 필드에 날짜를 입력하면 해당 날짜에 가입한 사용자에게 전송될 콘텐츠 카드를 미리 볼 수 있습니다. <br><br>

![][4]

## 커넥티드 콘텐츠 추천 엔진 만들기

1. 다음 방법 중 하나로 커넥티드 콘텐츠 엔드포인트를 만듭니다:
- SheetDP와 같은 서비스를 사용하여 스프레드시트를 JSON API 엔드포인트로 변환하고, 이렇게 생성되는 API URL을 기록해 두세요.
- 맞춤형 사내 엔드포인트 구축, 호스팅 및 유지 관리
- [아마존 퍼스널라이즈]({{site.baseurl}}/partners/message_personalization/dynamic_content/amazon_personalize/), [서토나]({{site.baseurl}}/partners/message_personalization/dynamic_content/certona/), [다이나믹 일드]({{site.baseurl}}/partners/message_personalization/dynamic_content/dynamic_yield/) 등 [Alloy 파트너와]({{site.baseurl}}/partners/message_personalization/dynamic_content) 같은 타사 파트너를 통해 추천 엔진을 구매하세요.

2. 연결된 콘텐츠 리퀴드를 메시지 본문 또는 콘텐츠 블록 HTML 편집기에 작성하면 엔드포인트를 호출하여 데이터베이스를 검색할 수 있습니다.
3. 특정 사용자의 프로필에서 찾은 사용자 지정 속성 값에 맞춰 리퀴드를 정렬합니다.
...
|`RECOMMENDED_ITEM_IDS` | 추천 항목의 ID가 포함된 사용자 지정 속성의 실제 이름으로 바꿉니다. 이 속성은 세미콜론으로 구분된 ID 문자열이 될 것으로 예상됩니다. |
|`ITEM_ID` | API 응답에서 항목 ID에 해당하는 속성의 실제 이름으로 바꿉니다. |
{: .reset-td-br-1 .reset-td-br-2}

{% alert note %}
이것은 기본적인 예시이며, 특정 요구 사항과 데이터 구조에 따라 추가로 수정해야 할 수도 있습니다. 자세한 안내는 [Liquid 설명서를]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid) 참조하거나 개발자에게 문의하세요.
{% endalert %}

## 커넥티드 콘텐츠 사용 사례

Zomato 레스토랑 데이터베이스에서 레스토랑 추천을 가져와 그 결과를 `restaurants` 이라는 로컬 변수로 저장한다고 가정해 보겠습니다. 다음과 같은 커넥티드 콘텐츠 호출을 할 수 있습니다:
...
다음으로 사용자의 도시와 음식 유형에 따라 레스토랑 추천을 가져오고 싶다고 가정해 보겠습니다. 사용자의 도시 및 음식 유형에 대한 사용자 지정 속성을 통화 시작 부분에 동적으로 삽입한 다음 `restaurants` 값을 `city_food.restaurants` 변수에 할당하면 됩니다.

커넥티드 콘텐츠 호출은 다음과 같이 표시됩니다:
...
아래 스크린샷에서 사용자 디바이스에 응답이 표시되는 방식에 대한 예를 참조하세요.
...
| 코드 솔루션 없음 | 지원되지 않음 | 사전 생성된 리퀴드의 경우 지원 | 지원되지 않음 | 지원되지 않음 |
| 고급 액체가 자주 필요함 | 지원 | 지원되지 않음 | 지원되지 않음 | 지원 |
