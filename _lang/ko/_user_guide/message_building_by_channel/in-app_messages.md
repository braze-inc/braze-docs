...
> 인앱 메시지는 푸시 알림으로 사용자의 하루를 방해하지 않고 콘텐츠를 사용자에게 전달하는 데 도움을 줍니다. 이러한 메시지는 사용자의 앱 외부로 전달되지 않으며 홈 화면을 방해하지 않습니다. 

커스텀 인앱 메시지는 사용자 경험을 향상시키고 오디언스가 앱에서 최대한의 가치를 얻을 수 있도록 도와줍니다. 다양한 레이아웃과 사용자 지정 도구를 선택할 수 있는 인앱 메시지는 그 어느 때보다 사용자의 참여를 유도할 수 있습니다. 이 메시지는 맥락을 가지고 있으며 긴급성이 낮고 사용자가 앱 내에서 활동할 때 전달됩니다. 인앱 메시지의 예시는 [고객 사례](https://www.braze.com/customers/)를 확인하세요.
...
이 기본 메시지 템플릿 외에도 커스텀 HTML 인앱 메시지, CSS가 포함된 웹 모달 또는 웹 이메일 캡처 양식을 사용하여 메시징을 추가로 사용자 정의할 수 있습니다. For more information, refer to [Customization]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/customize/).
...
Braze에서 중단은 사용자가 메시지를 받을 자격이 있는 행동을 취하지만 Liquid 로직이 그들을 자격이 없다고 표시하여 메시지를 받지 못할 때 발생합니다. For example:
...
다시 말해, 인앱 메시지를 중단해야 하는지 여부를 결정하는 논리는 트리거가 발생하기 **전에** 발생합니다. 이를 설명하기 위해 이메일 예제의 샘이 푸시 알림을 구독하고 있다고 가정해 보겠습니다.
...
3. 샘은 이러한 메시지를 트리거할 수 있는 어떤 행동도 **하지** 않았지만, 세션에서 해당 메시지를 받을 수 있습니다.
4. 두 개의 인앱 메시지에 있는 Liquid는 샘이 메시지를 받지 못하도록 하는 규칙이 있습니다(예: 점수 커스텀 속성이 충분히 높지 않음).
5. 샘은 자신을 제외하는 두 개의 인앱 메시지를 받지 않지만, 나머지 세 개의 메시지는 받습니다.
6. 중단 이벤트는 기록되지 않습니다.
