...
이러한 기본 메시지 템플릿 외에도 커스텀 HTML 인앱 메시지, CSS가 포함된 웹 모달 또는 웹 이메일 캡처 양식을 사용하여 메시징을 추가로 사용자 정의할 수 있습니다. For more information, refer to [Customization]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/traditional/customize/).

## 템플릿 인앱 메시지

인앱 메시지는 **캠페인 자격 재평가 후 표시**가 선택되거나 메시지에 다음 Liquid 태그가 존재하는 경우 템플릿 인앱 메시지로 전달됩니다:
...
Braze에서 중단은 사용자가 메시지를 받을 자격이 있는 행동을 취했지만 Liquid 로직이 그들을 자격이 없다고 표시하여 메시지를 받지 못할 때 발생합니다. For example:

1. 샘은 이메일 캠페인을 트리거해야 하는 행동을 수행합니다.
2. 이메일 본문에는 커스텀 속성 점수가 50 미만인 경우 이 이메일을 보내지 말라는 Liquid 로직이 포함되어 있습니다.
...
인앱 메시지는 세션 시작 시 기기에 의해 가져와지고 기기에 캐시되므로 인터넷 연결 품질에 관계없이 메시지가 사용자에게 즉시 전달될 수 있습니다. 예를 들어, 사용자가 세션 내에서 다섯 개의 인앱 메시지를 받으면 세션 시작 시 다섯 개 모두를 받게 됩니다. 메시지는 로컬에 캐시되며 정의된 트리거 이벤트(세션 시작, 사용자가 커스텀 이벤트를 기록하는 버튼 클릭 등)가 발생할 때 나타납니다.

다시 말해, 인앱 메시지를 중단해야 하는지를 결정하는 논리는 트리거가 발생하기 **전에** 발생합니다. 이를 설명하기 위해, 이메일 예제의 샘이 푸시 알림을 구독하고 있다고 가정해 보겠습니다.

1. 샘은 자신의 전화에서 Braze 기반 앱을 실행하여 세션을 시작합니다.
2. 활성 캠페인의 청중 기준에 따라 샘은 다섯 가지 다른 캠페인에 적합할 수 있습니다. 모든 다섯 개가 그의 전화로 가져와지고 캐시됩니다.
3. 샘 **는** 이러한 메시지를 트리거할 수 있는 어떤 행동도 수행하지 않았지만, 세션에서 해당 메시지를 받을 수 있습니다.
4. 두 개의 인앱 메시지에 있는 Liquid는 샘이 메시지를 받지 못하도록 하는 규칙이 있습니다(예: 그의 점수 커스텀 속성이 충분히 높지 않음).
5. 샘은 그를 제외하는 두 개의 인앱 메시지를 받지 않지만, 나머지 세 개의 메시지는 받습니다.
6. 중단 이벤트는 기록되지 않습니다.

Braze는 샘의 경우 중단 이벤트를 기록하지 않는데, 이는 중단의 정의를 충족하지 않기 때문입니다; 샘 **는** 메시지를 트리거할 수 있는 어떤 행동도 수행하지 않았습니다. 인앱 메시지의 경우, 사용자는 Braze가 메시지를 보여주지 않아야 한다고 판단하기 전에 실제로 트리거를 수행하지 않습니다.

#### 템플릿 인앱 메시지 중단 동작

[템플릿 인앱 메시지](#templated-in-app-messages)는 트리거 이벤트가 발생할 때 메시지를 표시해야 하는지 SDK가 재평가하도록 강제합니다. 이는 다른 중단 동작을 가집니다. 이를 설명하기 위해, 이 예제를 고려해 보겠습니다:
...
| --- | --- |
| Standard | 샘이 메시지를 트리거할 수 있는 행동을 수행하지 않았기 때문에 중단 이벤트가 기록되지 않았습니다.<br><br>표준 인앱 메시지는 중단을 기록하지 않는데, 중단의 정의는 "트리거 동작을 수행했음에도 불구하고 메시지를 보지 못했다"입니다. 인앱 메시지는 트리거 동작이 발생하기 전에 기기로 전달되므로 Liquid 논리로 인해 인앱 메시지가 생략되었다고 간주하는 것은 의미가 없습니다. |
