...
Braze를 사용하면 대시보드 폴백 채널을 지정할 수 있습니다. 대시보드 폴백 채널의 목적은 명시적인 채널 선택 없이 레거시 푸시 메시지에 대한 채널 ID를 제공하는 것입니다. 채널 선택은 Android 푸시 작성기에서 채널을 선택하는 것으로 정의합니다.
...
대시보드 폴백 채널이 '마케팅'이고 채널을 선택하지 않은 Android 푸시 메시지가 10개 있습니다. '마케팅' 채널이 대시보드 대체 채널이기 때문에 이러한 캠페인은 '마케팅' 채널을 통해 전송됩니다.
...
Braze에 잘못된 채널 ID를 제공한 경우(예: 개발자가 SDK에서 생성하지 않은 채널 ID를 제공한 경우), 당사는 귀하의 SDK 기본 채널을 통해 알림을 전달합니다. 따라서 개발 중에 Braze의 대시보드를 통해 알림 채널을 테스트해 보실 것을 적극 권장합니다.
...
|**회사 XYZ가** Android O를 지원하는 SDK로 업데이트합니다. <br>**회사 XYZ는** Braze 대시보드에 채널을 추가하지 않습니다.<br>**회사 XYZ가** SDK 기본 채널의 이름을 "마케팅"으로 변경합니다. | Android O 디바이스로 전송되는 푸시 알림은 '마케팅'이라는 채널을 생성하고 '마케팅' 채널을 통해 알림이 전송됩니다.
|**회사 LMN이** Android O를 지원하는 SDK로 업데이트합니다. <br>**회사 LMN은** 애플리케이션 코드에 "프로모션"과 "주문 업데이트"라는 두 가지 채널을 정의합니다. <br>**회사 LMN이** Braze 대시보드에 "프로모션" 및 "주문 업데이트"에 대한 채널 ID를 추가합니다. <br>**회사 LMN은** 대시보드 대체 채널로 '프로모션'을 지정합니다.<br>**회사 LMN이** SDK 기본 채널의 이름을 "마케팅"으로 변경합니다. | Android O 디바이스로 전송된 푸시 알림은 채널을 만들지 않습니다.<br><br>마케팅 담당자가 '주문 업데이트' 또는 '마케팅' 채널을 통해 알림을 보내도록 명시적으로 지정하지 않는 한, 채널이 대시보드에 추가되기 전에 생성된 모든 알림은 '프로모션' 채널을 통해 전송됩니다.<br><br>SDK 기본 채널인 '마케팅'은 회사가 잘못된 채널 ID를 통해 알림을 보내려고 하거나 명시적으로 선택한 경우에만 생성되어 사용됩니다.
|**회사 HIJ가** Android O로 업데이트하지만 Braze Android SDK를 2.1.0 이상으로 업데이트하지 않습니다. | Android O 이상을 실행하는 사용자에게 보낸 알림이 표시되지 않습니다. |
...
3. **알림 채널 관리를** 클릭합니다. 여기에 추가된 모든 채널은 모든 캠페인과 캔버스에서 전 세계적으로 사용할 수 있습니다. 채널을 관리하려면 워크스페이스에 대한 '앱 관리' [권한이]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/#limited-and-team-role-permissions) 있어야 합니다.
