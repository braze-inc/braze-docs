...
애플리케이션에 채널을 추가한 후에는 채널을 제거하도록 선택할 수 있습니다. 그러나 소비자는 항상 [제거한] 채널 수를 볼 수 있습니다.[3] Braze 대시보드에는 프로그래밍 방식으로 채널을 만드는 기능이 포함되어 있지 않으며, 원활한 경험을 제공하려면 애플리케이션 코드에서 채널을 만들고 정의해야 합니다.

다시 한 번 강조하지만, 엔지니어링 팀과 협력하여 Android O 타겟팅으로 원활하게 전환할 수 있도록 하는 것이 좋습니다.

## 대시보드 폴백 채널

Braze를 사용하면 대시보드 폴백 채널을 지정할 수 있습니다. 대시보드 대체 채널의 목적은 명시적인 채널 선택 없이 레거시 푸시 메시지에 대한 채널 ID를 제공하는 것입니다. 채널 선택은 Android 푸시 작성기에서 채널을 선택하는 것으로 정의합니다.

채널이 선택되어 있지 않은 메시지는 대시보드 대체 채널 ID로 전송됩니다. 대시보드 대체 채널을 변경하면 채널이 명시적으로 선택되어 있지 않은 모든 메시지가 새 대체 채널의 ID로 전송됩니다.

다음은 대시보드 폴백 채널 예상 동작의 예입니다:

대시보드 대체 채널이 "마케팅"이고 채널을 선택하지 않은 Android 푸시 메시지가 10개 있습니다. '마케팅' 채널이 대시보드 대체 채널이기 때문에 이러한 캠페인은 '마케팅' 채널을 통해 전송됩니다.

또한 '소셜 알림' 채널을 통해 보내도록 선택한 15개의 메시지와 '마케팅' 채널을 통해 보내도록 선택한 5개의 메시지가 있습니다.

그런 다음 대시보드 기본 채널을 '마케팅'에서 '업데이트'로 변경하기로 결정합니다.

이 경우 이전에 '마케팅' 채널을 통해 전송되었던 채널 선택이 없는 10개의 캠페인은 이제 모두 '업데이트' 채널을 통해 전송되므로 이러한 메시지는 대체 채널을 통해 전송됩니다. "소셜 알림" 채널을 통해 전송된 15개의 메시지는 "소셜 알림" 채널을 통해 계속 전송됩니다. "마케팅" 채널을 통해 전송된 5개의 메시지는 계속해서 "마케팅" 채널을 통해 전송됩니다.

Braze에 잘못된 채널 ID를 제공한 경우(예: 개발자가 SDK에서 생성하지 않은 채널 ID를 제공한 경우), 당사는 SDK 기본 채널을 통해 알림을 전달합니다. 따라서 개발 중에 Braze의 대시보드를 통해 알림 채널을 테스트해 보실 것을 적극 권장합니다.

채널에 대한 예상 동작을 더 잘 이해하려면 다음 표를 참조하세요:

|시나리오 |결과  |    
| ---|-------------
|**회사 ABC가** Android O를 지원하는 SDK로 업데이트합니다.<br>**회사 ABC는** Braze 대시보드에 채널을 추가하지 않습니다.<br>**회사 ABC는** SDK 기본 채널의 이름을 변경하지 않습니다. | Android O 디바이스로 전송되는 푸시 알림은 '일반'이라는 채널을 생성하고 '일반' 채널을 통해 알림이 전송됩니다.
|**회사 XYZ가** Android O를 지원하는 SDK로 업데이트합니다. <br>**회사 XYZ는** Braze 대시보드에 채널을 추가하지 않습니다.<br>**회사 XYZ**가 SDK 기본 채널의 이름을 "마케팅"으로 변경합니다. | Android O 디바이스로 전송되는 푸시 알림은 '마케팅'이라는 채널을 생성하고 '마케팅' 채널을 통해 알림이 전송됩니다.
|**회사 LMN**이 Android O를 지원하는 SDK로 업데이트합니다. <br>**회사 LMN**은 애플리케이션 코드에 "프로모션"과 "주문 업데이트"라는 두 가지 채널을 정의합니다. <br>**회사 LMN**이 Braze 대시보드에 "프로모션" 및 "주문 업데이트"에 대한 채널 ID를 추가합니다. <br>**회사 LMN**은 대시보드 대체 채널로 "프로모션"을 지정합니다.<br>**회사 LMN**이 SDK 기본 채널의 이름을 "마케팅"으로 변경합니다. | Android O 디바이스로 전송된 푸시 알림은 채널을 만들지 않습니다.<br><br>마케팅 담당자가 "주문 업데이트" 또는 "마케팅" 채널을 통해 알림을 보내도록 명시적으로 지정하지 않는 한, 채널이 대시보드에 추가되기 전에 생성된 모든 알림은 "프로모션" 채널을 통해 전송됩니다.<br><br>SDK 기본 채널인 "마케팅"은 회사가 잘못된 채널 ID를 통해 알림을 보내려고 하거나 명시적으로 선택한 경우에만 생성되어 사용됩니다.
|**회사 HIJ**가 Android O로 업데이트하지만 Braze Android SDK를 2.1.0 이상으로 업데이트하지 않습니다. | Android O 이상을 실행하는 사용자에게 보낸 알림이 표시되지 않습니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

## Braze 대시보드에 채널 추가하기

1. Android 푸시가 포함된 캠페인 또는 캔버스를 열고 **캠페인 편집**을 클릭합니다.
2. Android 푸시 메시지 작성기로 이동합니다.
3. **알림 채널 관리**를 클릭합니다. 여기에 추가된 모든 채널은 모든 캠페인과 캔버스에서 전 세계적으로 사용할 수 있습니다. 채널을 관리하려면 워크스페이스에 대한 "앱 관리" [권한이]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/#limited-and-team-role-permissions) 있어야 합니다.
...
대체 채널은 메시지에 대한 채널을 선택하지 않은 경우 Braze가 Android 메시지를 보내려고 시도하는 채널입니다. 채널 선택 없이 Android 메시지가 표시되는 유일한 캠페인과 캔버스는 팀이 Braze 대시보드에 채널을 추가하기 전에 생성한 캠페인과 캔버스입니다. 대체 채널을 변경하면 명시적으로 채널을 선택하지 않아도 변경 사항이 모든 캠페인과 캔버스에 전역적으로 적용됩니다.
