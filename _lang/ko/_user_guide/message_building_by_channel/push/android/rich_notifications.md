...
- 푸시 알림에 작업 버튼을 추가하면 표시할 수 있는 이미지 영역에 영향을 미칠 수 있습니다. 대시보드 프리뷰와 라이브 기기로 테스트하여 결과가 예상대로 나오는지 확인하세요.
...
알림의 이미지 앞에 표시할 **요약 텍스트를** 추가합니다.

![Dog라는 반려동물 사료 앱에서 요약 텍스트와 함께 사료를 더 주문할 때가 되었음을 알리는 리치 푸시 알림을 받았습니다.]({% image_buster /assets/img_archive/android_rich_summarytext.png %})

### 3단계: 미디어 추가

메시지 작성기의 **Android 알림 이미지** 필드에 이미지를 추가합니다. 이미지는 대시보드를 통해 직접 업로드하거나 다른 곳에 호스팅된 콘텐츠 URL을 지정하여 업로드할 수 있습니다.

자세한 내용은 [이미지 사양]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/media_library/#push)을 확인하세요.

![Android 알림 이미지 섹션에서 이미지를 추가하거나 이미지 URL을 입력할 수 있습니다.]({% image_buster /assets/img_archive/android_rich_image.png %})
