...
|------------------------------------|-----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       |                          |   |
|  |                           | 모든 사용자는 푸시 알림을 받으려면 명시적으로 옵트인해야 합니다.                                      |
...
- **사용자 프로필**:  이메일 주소, 전화번호 또는 외부 사용자 ID를 통해 사용자의 프로필을 찾은 후 **참여** 탭을 선택하여 사용자의 가입 상태를 확인하고 수동으로 조정할 수 있습니다.
-   Braze는 디바이스별 푸시 활성화 정보가 포함된 푸시 토큰 객체를 반환합니다.
...
| 푸시 오류 발생 | 토큰 제거로 이어지는 몇 가지 일반적인 푸시 오류로는 `MismatchSenderId`, `InvalidRegistration`, 기타 유형의 푸시 반송 등이 있습니다. <br><br> |
