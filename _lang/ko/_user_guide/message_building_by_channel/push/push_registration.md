...
{% alert tip %}
**연락처 설정** > **푸시 체인지로그에** 오류 메시지가 표시되면 [일반적인 푸시 오류 메시지에]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_error_codes/) 대한 설명과 다음 단계를 참조하세요.
{% endalert %}
...
| **iOS 11 이전 버전** | 아니요                          | 모든 사용자는 푸시 알림을 받으려면 명시적으로 옵트인해야 합니다. 푸시 토큰은 권한이 부여된 후에만 생성됩니다.                                     |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
{% endtab %}
{% endtabs %}
...
![앞서 언급한 푸시 프로세스를 Braze, 고객, Apple 푸시 알림 서비스 또는 Firebase Cloud 메시징 간에 매핑하는 플로우 차트입니다.]({% image_buster /assets/img/push_process.png %})
