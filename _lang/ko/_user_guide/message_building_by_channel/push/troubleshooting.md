...
푸시 알림 전달에 어려움을 겪고 계신가요? 다음 항목을 확인하여 이 문제를 해결할 수 있습니다:
...
푸시는 구독하거나 옵트인한 사용자에게만 보낼 수 있습니다. **고객 프로필** 섹션의 [참여]({{site.baseurl}}/user_guide/engagement_tools/segments/using_user_search/#engagement-tab) 탭에서 고객 프로필을 확인하여 테스트 중인 워크스페이스에 대해 푸시 등록이 되어 있는지 확인하세요. 여러 앱에 등록되어 있는 경우 **푸시 등록 대상** 필드에 해당 앱이 나열됩니다:
...
두 엔드포인트 모두 기기별 푸시 인에이블먼트 정보가 포함된 푸시 토큰 오브젝트를 반환합니다.

#### 세그먼트

타겟팅하는 세그먼트에 속하는지 확인하세요(테스트가 아닌 라이브 캠페인인 경우). **고객 프로필**에서 사용자가 현재 속해 있는 세그먼트 목록을 확인할 수 있습니다. 세분화는 실시간으로 업데이트되므로 계속 변화하는 변수라는 점을 기억하세요.

![세그먼트 목록]({% image_buster /assets/img_archive/trouble2.png %})

세그먼트를 생성할 때 **사용자 조회**를 사용하여 사용자가 세그먼트에 포함되어 있는지 확인할 수도 있습니다.
...
대시보드에서 [글로벌 최대 게재빈도 설정]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/rate-limiting/#freq-cap-feat-over)을 확인하여 이를 수행할 수 있습니다. 캠페인이 최대 게재빈도 설정 규칙을 준수하도록 설정된 경우, 이러한 설정의 영향을 받는 사용자가 다수 발생할 수 있습니다.

![캠페인 세부 정보]({% image_buster /assets/img_archive/trouble3.png %})

#### 사용량 제한

캠페인 또는 캔버스에 사용량 제한이 설정되어 있는 경우 이 한도를 초과하여 메시징 수신이 중단될 수 있습니다. 자세한 내용은 [사용량 제한]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/rate-limiting/#rate-limiting)을 참조하세요.

#### 대조군 상태

단일 채널 캠페인이거나 캔버스에 대조군이 있는 경우 대조군에 속할 수 있습니다.

  1. [배리언트 분포]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/multivariate_testing/#step-5-distribute-users-among-your-variants)를 확인하여 대조군이 있는지 확인합니다.
  2. 대조군이 있다면 [캠페인 대조군 내]({{site.baseurl}}/user_guide/engagement_tools/campaigns/ideas_and_strategies/retargeting_campaigns/#in-campaign-control-group-filter) 필터로 세그먼트를 생성한 다음 [세그먼트를 내보내고]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/segment_data_to_csv/#exporting-to-csv) 사용자 ID가 이 목록에 있는지 확인하세요.
...
캠페인 또는 캔버스 단계에서 **모바일 앱 내에서 웹 URL 열기**가 선택되어 있지 않은지 다시 확인하세요. 선택되어 있다면 선택을 해제하고 다시 시작하세요. 

!["클릭 시 동작" 필드에서 푸시를 구성할 때 "웹 URL 열기"가 선택되고 "모바일 앱 내에서 웹 URL 열기"가 선택 해제되어 있습니다.]({% image_buster /assets/img/push_on_click.png %})

클릭 시 동작 "웹 URL 열기"의 기본 상호 작용은 SDK 버전에 따라 다릅니다. SDK 버전 iOS 2.29.0 및 Android 2.0.0 이상의 경우 이 옵션이 기본적으로 선택되어 있으며 웹 URL이 앱 내 웹 뷰에서 열립니다. 이 버전 이전에는 이 옵션이 기본적으로 선택 해제되어 있으며 웹 URL이 기기의 기본 웹 브라우저에서 열립니다.

이 문제가 아니라면 푸시 구현에 문제가 있을 수 있습니다. 

### 푸시 통합 재확인

푸시 알림의 링크가 앱에서 예기치 않게 열리는 경우 푸시 알림 통합 또는 커스텀 설정에 문제가 있을 수 있습니다. 다음 단계에 따라 문제를 해결하세요:
...
브라우저에서 푸시 알림에 문제가 있는 경우 사이트의 알림 권한을 재설정하고 사이트의 저장소를 지워야 할 수 있습니다. 다음 단계를 참조하여 도움을 받으세요.
...
2. **알림** 아래에서 **권한 재설정**을 선택합니다.
3. Chrome DevTools를 엽니다. 다음은 운영체제별 관련 단축키입니다.
...
5. 사이드바에서 **Storage**를 선택합니다.
6. **Clear site data**를 선택합니다.
7. Chrome이 업데이트된 설정을 적용하기 위해 페이지를 새로고침하라는 메시지를 표시합니다. **Reload**를 선택합니다.

푸시 권한이 이제 재설정되었습니다. 새 탭을 열어 사이트에 접속하여 시도해 보세요.

### Android에서 Chrome 재설정

사이트에서 Android 알림 드로어에 알림이 표시되는 경우:

1. 푸시 알림에서 <i class="fas fa-cog" title="설정"></i>을 탭하고 **사이트 설정**을 선택합니다.
2. **사이트 설정**에서 **지우기 및 재설정**을 탭합니다.

사이트에서 알림을 열지 않은 경우:

1. Android에서 Chrome을 엽니다.
2. <i class="fas fa-ellipsis-vertical"></i> 메뉴를 탭합니다.
3. **설정** > **사이트 설정** > **알림**으로 이동합니다.
4. 알림이 **전송 전에 요청(권장)**으로 설정되어 있는지 확인합니다.
5. 목록에서 사이트를 찾습니다.
6. 항목을 선택하고 **지우기 및 재설정**을 탭합니다.
...
1. 사이트 URL 옆에서 <i class="fa-solid fa-circle-info" alt="info icon"></i> 또는 <i class="fas fa-lock" alt="lock icon"></i>을 선택합니다.
...
일반적인 푸시 오류 메시지(`DEVICE_UNREGISTERED`, `Unregistered`, `NotRegistered` 등)에 대한 자세한 내용은 [일반적인 푸시 오류 메시지]({{site.baseurl}}/user_guide/message_building_by_channel/push/push_error_codes/)를 참조하세요.

아직도 도움이 필요하신가요? [고객지원 티켓]({{site.baseurl}}/braze_support/)을 여세요.