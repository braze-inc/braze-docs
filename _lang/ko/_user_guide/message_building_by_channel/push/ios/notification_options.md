...
![iOS 푸시 알림 그룹을 관리하는 창입니다.]({% image_buster /assets/img_archive/managenotgroups.png %}){: style="max-width:70%;" }
...
전통적인 iOS 푸시 토큰과 달리, 임시 푸시 토큰은 브랜드가 사용자가 Apple의 기본 푸시 옵트인 안내를 보고 클릭하기 전에 새로운 사용자에게 도달할 수 있도록 하는 "체험 패스" 역할을 합니다. 이 기능을 사용하면 푸시 알림이 새 사용자의 알림 트레이로 바로 전달되며, 향후 알림을 '유지' 또는 '끄기'할 수 있는 옵션이 제공됩니다. 사용자는 '옵트인' 여정을 경험하는 대신 '옵트아웃' 여정에 더 가까운 경험을 하게 됩니다.
...
![푸시 구독 상태가 있는 연락처 설정입니다.]({% image_buster /assets/img/profile-push-prov-auth.png %}){: width="50%"}
...
{% alert important %}
추가 푸시 안내 또는 [인앱 푸시 프라이머](https://www.braze.com/resources/glossary/priming-for-push/) (사용자가 푸시 알림에 옵트인하도록 유도하는 인앱 메시지)를 사용하는 경우, 추가 지침을 위해 Braze 담당자에게 문의하십시오.
{% endalert %}
...
![즉시 전달을 위해 알림이 활성화되고 시간 민감 알림이 활성화된 iOS 알림 설정 페이지입니다.]({% image_buster /assets/img/ios/ios15-notification-settings.png %}){: style="max-width:40%"}
...
![세 개의 알림이 포함된 "당신의 저녁 요약"이라는 제목의 iOS 알림 요약입니다.]({% image_buster /assets/img/ios/ios15-notification-summary.png %}){: style="float:right;max-width:25%;margin-left:15px;border:0"}
...
!["0.5"의 관련성 점수입니다.]({% image_buster /assets/img/ios/relevance-score.png %}){: style="max-width:80%;"}
