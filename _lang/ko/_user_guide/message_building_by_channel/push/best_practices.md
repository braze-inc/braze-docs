...
사용자가 앱에 유효한 포그라운드 푸시 토큰이 없는 경우(즉, 설정을 통해 기기 수준에서 푸시 토큰을 끄고 알림을 받지 않기로 선택한 경우) 구독 상태는 여전히 `subscribed`로 간주되어 푸시할 수 있습니다. 그러나 이 사용자는 포그라운드 푸시 토큰이 유효하지 않으므로 Braze에서 `Foreground Push Enabled for App`이 아닙니다.
