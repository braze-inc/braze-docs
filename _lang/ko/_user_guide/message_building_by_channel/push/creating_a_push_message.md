...
{% alert tip %}
캠페인을 사용할지 캔버스를 사용할지 고민 중이신가요? 캠페인은 단일 타겟팅 메시징 캠페인에 더 적합하며, 캔버스는 다단계 사용자 여정에 더 적합합니다.
{% endalert %}
...
| Canvas | 메시지 단계에서 **\+ 추가를** 선택하여 푸시 플랫폼을 추가합니다. 멀티채널 캠페인과 마찬가지로 플랫폼 선택은 각 변형에 따라 다릅니다. |
...
![푸시 알림을 작성하는 작성 탭입니다.]({% image_buster /assets/img_archive/push_compose.png %})
...
다음으로 세그먼트 또는 필터를 선택하여 오디언스 범위를 좁혀 [사용자를 타겟팅해야]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/) 합니다. 대략적인 세그먼트 인구가 어떻게 생겼는지 자동으로 미리 보기를 받을 수 있습니다. 캠페인이 타겟팅하는 채널에 대한 자세한 오디언스 통계는 바닥글에서 확인할 수 있습니다. 타겟팅 중인 사용자 기반 비율과 이 세그먼트의 생애주기 가치를 확인하려면 **추가 통계 표시를** 선택합니다.
...
정확한 세그먼트 멤버십은 항상 메시지를 보내기 전에 계산된다는 점에 유의하세요.
...
캠페인 또는 캔버스의 마지막 제작을 완료한 후에는 세부 정보를 검토합니다. 캠페인의 경우 마지막 페이지에서 디자인한 캠페인에 대한 요약 정보를 확인할 수 있습니다. 모든 관련 세부 정보를 확인하고 메시지를 테스트한 다음 메시지를 전송하고 데이터가 들어오는 것을 지켜보세요!
