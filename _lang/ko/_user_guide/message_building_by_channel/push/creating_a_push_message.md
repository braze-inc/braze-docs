...
{% alert tip %}
캠페인 또는 캔버스를 사용할지 확실하지 않으신가요? 캠페인은 단일 타겟 메시징 캠페인에 더 적합하고, 캔버스는 다단계 사용자 여정에 더 적합합니다.
{% endalert %}

{% tabs %}
{% tab Campaign %}
1. Go to **Messaging** > **Campaigns**, then select **Create campaign**.
2. 여러 채널을 타겟팅하는 캠페인의 경우 **멀티채널**을 선택하세요. Otherwise, select **Push notification**. 아직 확실하지 않다면 아래의 **정규 또는 멀티채널 푸시 캠페인 결정하기**를 참조하세요.
...
| 멀티채널 캠페인 | 추가 푸시 플랫폼을 추가하려면 **메시징 채널 추가**를 선택하세요. 플랫폼 선택은 각 배리언트에 따라 다르므로 플랫폼별로 메시지 인게이지먼트를 테스트해 볼 수 있습니다!
| Canvas | 메시지 단계에서 **\+ 더 추가**를 선택하여 추가 푸시 플랫폼을 추가하세요. 멀티채널 캠페인과 마찬가지로 플랫폼 선택은 각 변형에 따라 다릅니다. |
...
![푸시 알림 생성의 구성 탭.]({% image_buster /assets/img_archive/push_compose.png %})
...
**제목** 필드를 사용하여 헤드라인을 추가하세요. 푸시를 개인화 및 타겟팅하기 위해 [Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/)를 포함할 수 있습니다.
{% endtab %}
...
{% alert important %}
제목 없이 Android 푸시 메시지를 보낼 수 **없습니다**—하지만 대신 단일 공백을 입력할 수 있습니다. 메시지가 단일 공백만 포함되어 있다면, 이는 무음 푸시 알림으로 전송됩니다. 자세한 내용은 [무음 푸시 알림을]({{site.baseurl}}/developer_guide/push_notifications/silent/?sdktab=android) 참조하세요.
{% endalert %}
{% endtab %}
{% endtabs %}
...
![푸시 작성기의 본문 필드에 위치한 AI 카피라이터 버튼을 실행하세요.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_push.png %}){: style="max-width:60%"}
{% endalert %}
...
푸시 알림 본문을 사용자가 선택할 때 발생하는 일을 **클릭 행동**으로 지정하세요. 예를 들어 고객에게 애플리케이션을 열라는 메시지를 표시하거나, 고객을 지정된 웹 URL로 리디렉션하거나, [딥링크]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/deep_linking_to_in-app_content/)를 통해 애플리케이션의 특정 페이지를 열 수 있습니다.
...
![기기 옵션 확인란을 선택하면 사용자가 가장 최근에 사용한 기기로만 푸시를 보낼 수 있습니다.]({% image_buster /assets/img_archive/push_recent_device.png %}){: style="max-width:70%;" }

이 설정에는 약간의 뉘앙스가 있습니다. 이 옵션을 선택하면 캠페인이 iOS와 Android 등 여러 플랫폼을 타겟팅하는 경우를 제외하고는 Braze에서 다중 전송이 발생하지 않도록 제한합니다. If the user has your app on both an iOS and an Android device, they'll receive a push for both platforms. 사용자가 가장 최근에 사용한 기기가 [푸시를 사용하도록]({{site.baseurl}}/user_guide/message_building_by_channel/push/users_and_subscriptions/#foreground-push-enabled) 설정되어 있지 않으면 메시지가 전송되지 않습니다.

iOS의 경우 푸시 알림을 iPad 기기로만 보내거나 iPhone 및 iPod 기기로만 보내도록 설정하여 메시징을 더욱 제한할 수 있습니다.

## 5단계: 메시지를 미리 보고 테스트하세요 (선택 사항)
...
다음으로, 세그먼트 또는 필터를 선택하여 [사용자 타겟팅]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)을 해야 합니다. 해당 세그먼트 인구가 어떻게 생겼는지 미리보기를 자동으로 받습니다. 캠페인이 타겟팅하는 채널에 대한 자세한 오디언스 통계는 바닥글에서 확인할 수 있습니다. 사용자 기반의 몇 퍼센트가 타겟팅되고 있는지 및 이 세그먼트의 생애주기 가치를 보려면 **추가 통계 보기**를 선택하세요.
...
![도달 가능한 사용자에 대한 자세한 통계 테이블.]({% image_buster /assets/img_archive/multi_channel_footer.png %})

메시지가 전송되기 전에 정확한 세그먼트 구성원이 항상 계산된다는 점을 염두에 두세요.
...
캠페인 또는 캔버스의 마지막 제작을 완료한 후에는 세부 정보를 검토합니다. 캠페인의 경우, 최종 페이지는 당신이 설계한 캠페인의 요약을 제공합니다. 모든 관련 세부 정보를 확인하고 메시지를 테스트한 다음 메시지를 전송하고 데이터가 들어오는 것을 지켜보세요!
