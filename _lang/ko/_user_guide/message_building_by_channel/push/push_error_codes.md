...
### 페이로드가 유효하지 않아 푸시 전송 중 오류가 발생했습니다.

이 메시지는 잘못된 페이로드로 인해 Apple 푸시 알림 서비스(APN)가 푸시 요청을 거부할 때 **연락처 설정** > **푸시 체인지로그** 아래의 고객 프로필 **참여** 탭에 표시될 수 있습니다.

Braze에서 이 대시보드 메시지는 다음 APN 오류 원인 중 하나에 매핑될 수 있습니다:

- `PayloadEmpty`: 페이로드에 전송되는 푸시 유형에 필요한 콘텐츠가 누락되었습니다.
- `PayloadTooLarge`: 페이로드가 APN의 최대 페이로드 크기를 초과했습니다.

일반적인 원인은 다음과 같습니다:

- 커스텀 키(및 해당 값)로 인해 페이로드가 너무 커지는 경우(여기에는 예기치 않게 큰 Liquid 렌더링 값이 포함될 수 있음).
- 필요한 경우 비어 있거나 누락된 알림 또는 본문(또는 잘못된 형태의 `aps` 페이로드)이 있습니다.

Next steps:

- 커스텀 키를 트리밍하고 큰 동적 값을 단축하여 페이로드 크기를 줄이세요.
- API를 통해 전송하는 경우 전송하기 전에 최종 JSON 페이로드(크기 포함)의 유효성을 검사하세요.
