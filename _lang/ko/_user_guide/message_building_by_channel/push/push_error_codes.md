...
### 푸시를 보내는 중 오류가 발생했습니다. 페이로드가 유효하지 않습니다.

이 메시지는 사용자 프로필 **참여** 탭의 **연락처 설정** > **푸시 체인지로그** 아래에 나타날 수 있습니다. Apple 푸시 알림 서비스(APNs)가 유효하지 않은 페이로드로 인해 푸시 요청을 거부할 때입니다.
...
- 커스텀 키(및 해당 값)가 페이로드를 너무 크게 만드는 경우(예상치 못하게 큰 Liquid 렌더링 값이 포함될 수 있습니다).
- 필요한 경우 빈 또는 누락된 알림 또는 본문(또는 잘못 형성된 `aps` 페이로드).

Next steps:

- 커스텀 키를 잘라내고 큰 동적 값을 줄여 페이로드 크기를 줄이십시오.
- API를 통해 전송하는 경우, 전송하기 전에 최종 JSON 페이로드(크기 포함)를 검증하십시오.
