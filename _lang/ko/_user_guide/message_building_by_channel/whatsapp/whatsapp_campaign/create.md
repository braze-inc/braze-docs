...
![맥스라는 사용자에 대한 미리보기 메시지입니다.]({% image_buster /assets/img/whatsapp/whatsapp8.png %}){: style="max-width:80%;"}
...
다음으로, [사용자 타겟팅]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)을 선택하여 세그먼트나 필터를 선택하여 오디언스를 좁혀야 합니다. 이미 구독 그룹을 선택했어야 하며, 이는 사용자를 원하는 커뮤니케이션 수준이나 카테고리로 좁힙니다. 이 단계에서는 세그먼트에서 더 큰 오디언스를 선택하고 필터를 사용하여 해당 세그먼트를 더 좁힙니다. 해당 세그먼트 인구의 대략적인 모습에 대한 스냅샷을 자동으로 받습니다. 정확한 세그먼트 멤버십은 메시지가 전송되기 전에 항상 계산된다는 점을 기억하세요.
...
{% alert important %}
메타는 특정 인코딩 또는 컨테이너 설정으로 인해 일부 MP4 비디오가 Android 장치에서 재생되지 않도록 방지할 수 있는 알려진 문제가 있습니다. 영구적인 수정이 가능해질 때까지, MP4 파일을 재포맷하면 대부분의 발신자에게 문제를 해결합니다. 모든 비디오를 Android 기기에서 테스트하여 올바른 전달 가능성을 확인하십시오. <br><br>웹 도구를 사용하여 MP4 파일을 MP4로 다시 포맷할 수 있습니다. 예: [CloudConvert](https://cloudconvert.com/mp4-converter). MP4 파일을 도구에 업로드하고, 다시 MP4로 변환한 후 변환된 파일을 다운로드하십시오.
{% endalert %}
