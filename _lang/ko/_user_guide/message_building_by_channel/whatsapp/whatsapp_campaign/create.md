...
{% alert tip %}
작업 기반 캔버스가 수신된 WhatsApp 메시지에 의해 트리거되면, 다음 작업 경로까지 모든 캔버스 단계에서 WhatsApp 속성을 참조할 수 있습니다.
{% endalert %}
...
![메시지 변형 섹션에서는 구독 그룹과 두 가지 메시지 유형 중 하나를 선택할 수 있습니다: WhatsApp 템플릿 메시지 및 응답 메시지.]({% image_buster /assets/img/whatsapp/whatsapp_message_variants.png %}){: style="max-width:80%;"}
...
![템플릿 목록에는 메시지 미리보기, 할당된 언어 및 승인 상태가 포함됩니다.]({% image_buster /assets/img/whatsapp/whatsapp_templates.png %}){: style="max-width:80%;"}
...
Meta Business 관리자에서 WhatsApp 템플릿을 만들 때 변수를 추가한 경우, 해당 변수는 메시지 작성기에 빈 공간으로 표시됩니다. 이 빈칸을 Liquid 또는 일반 텍스트로 바꾸세요. 일반 텍스트를 사용하려면, 이중 중괄호로 둘러싸인 "여기에 텍스트" 형식을 사용하세요. 템플릿을 만들 때 이미지를 포함하기로 선택한 경우, 미디어 라이브러리에서 이미지를 업로드하거나 이미지 URL을 참조하여 이미지를 추가할 수 있습니다. 가능하면 일관성과 신뢰성 보장을 위해 이미지를 바로 미디어 라이브러리에 업로드하는 것이 좋습니다.
...
![속성 "first_name"와 기본값 "당신"이 있는 개인화 도구 추가.]({% image_buster /assets/img/whatsapp/whatsapp7.png %}){: style="max-width:80%;"}

### 동적 링크 

클릭 유도 URL에는 변수가 포함될 수 있지만, Meta는 URL의 끝에 `{% raw %}https://example.com/{{variable}}{% endraw %}`와 같이 변수가 있어야 한다고 요구합니다. 그런 다음 변수는 Braze에서 Liquid로 대체될 수 있습니다. 링크는 템플릿의 일부로 본문 텍스트로도 포함될 수 있습니다. 이 두 링크는 [클릭 추적]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/whatsapp_campaign/click_tracking/)를 사용하여 단축하고 추적할 수 있습니다.
...
![새 사용자에게 할인 코드를 제공하는 환영 메시지에 대한 응답 메시지 작성기.]({% image_buster /assets/img/whatsapp/whatsapp_response_messages.png %}){: style="max-width:80%;"}
...
![Max라는 사용자에 대한 미리보기 메시지.]({% image_buster /assets/img/whatsapp/whatsapp8.png %}){: style="max-width:80%;"}
...
![An alert that says, "To test, first open a conversation window by sending a WhatsApp message to +1 217-582-9414. 그런 다음, 테스트 사용자에게 응답 메시지를 보내십시오."]({% image_buster /assets/img/whatsapp/whatsapp_test_phone_number.png %}){: style="max-width:70%;"}
...
다음으로, [대상 사용자]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)를 선택하여 세그먼트나 필터를 선택하여 오디언스를 좁혀야 합니다. 이미 구독 그룹을 선택했어야 하며, 이는 사용자가 당신과 갖고자 하는 커뮤니케이션의 수준이나 범주에 따라 사용자를 좁힙니다. 이 단계에서는 세그먼트에서 더 큰 오디언스를 선택하고, 우리의 필터로 그 세그먼트를 더 좁힙니다. 해당 세그먼트 인구의 대략적인 모습에 대한 스냅샷을 자동으로 받습니다. 정확한 세그먼트 구성원 자격은 메시지가 전송되기 전에 항상 계산된다는 것을 기억하세요.
...
{% alert important %}
메타는 특정 인코딩 또는 컨테이너 설정으로 인해 일부 MP4 비디오가 Android 장치에서 재생되지 않도록 방지하는 알려진 문제가 있습니다. 영구적인 수정이 가능해질 때까지, MP4 파일을 재포맷하면 대부분의 발신자에게 문제를 해결합니다. 모든 비디오를 Android 기기에서 테스트하여 올바른 전달 가능성을 확인하십시오. <br><br>웹 도구를 사용하여 MP4 파일을 MP4로 다시 포맷할 수 있습니다. 예: [CloudConvert](https://cloudconvert.com/mp4-converter). MP4 파일을 도구에 업로드하고, 다시 MP4로 변환한 후 변환된 파일을 다운로드하십시오.
{% endalert %}
...
| Video | 오직 H.264 비디오 코덱과 AAC 오디오 코덱만 지원됩니다. 비디오는 단일 오디오 스트림이 있거나 오디오 스트림이 없어야 합니다. | `.mp4`, `.3gp` |
| CTA 링크 | 다양한 클릭 유도 문안(CTA) 유형이 지원됩니다. 자세한 내용은 [클릭 유도 문안 유형](#ctas)을 참조하십시오. | — |
...
| 마케팅 옵트아웃 버튼 | 기본적으로 구독 상태는 자동으로 업데이트되지 않습니다. 전체 안내는 [Opt-ins & Opt-Outs]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/message_processing/opt-ins_and_opt-outs/#marketing-opt-out-selection)을 참조하십시오. |
| 쿠폰 코드 메시지 템플릿 | 메시지 템플릿에서만 사용 가능합니다. <br>이 템플릿은 다른 메시지 템플릿처럼 열고 편집할 수 있으며, Liquid 및 Braze 프로모션 코드와 호환됩니다. |
| CTA 응답 메시지  | 실행 버튼을 포함하는 응답 메시지를 만드십시오. |
