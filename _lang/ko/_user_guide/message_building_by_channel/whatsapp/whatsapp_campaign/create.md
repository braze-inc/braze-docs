...
{% alert tip %}
인바운드 WhatsApp 메시지에 의해 트리거된 액션 기반 캔버스인 경우, 다음 액션 경로까지 모든 캔버스 단계에서 WhatsApp 속성을 참조할 수 있습니다.
{% endalert %}
...
Meta Business 관리자에서 WhatsApp 템플릿을 만들 때 변수를 추가한 경우, 해당 변수는 메시지 작성기에 빈 공간으로 표시됩니다. 이 빈칸을 Liquid 또는 일반 텍스트로 바꾸세요. 일반 텍스트를 사용하려면, 이중 중괄호로 둘러싸인 "여기에 텍스트" 형식을 사용하세요. 템플릿을 만들 때 이미지를 포함하기로 선택한 경우, 미디어 라이브러리에서 이미지를 업로드하거나 이미지 URL을 참조하여 이미지를 추가할 수 있습니다. 가능한 경우, 일관성과 신뢰성을 보장하기 위해 이미지를 미디어 라이브러리에 직접 업로드하는 것을 권장합니다.
...
![속성 "first_name"와 기본값 "당신"이 있는 개인화 도구 추가입니다.]({% image_buster /assets/img/whatsapp/whatsapp7.png %}){: style="max-width:80%;"}
...
![Max라는 사용자에 대한 미리보기 메시지입니다.]({% image_buster /assets/img/whatsapp/whatsapp8.png %}){: style="max-width:80%;"}
...
{% alert important %}
Meta는 특정 인코딩 또는 컨테이너 설정으로 인해 일부 MP4 비디오가 Android 장치에서 재생되지 않도록 방지할 수 있는 알려진 문제가 있습니다. 영구적인 수정이 가능해질 때까지 MP4 파일을 재포맷하면 대부분의 발신자에게 문제가 해결됩니다. 모든 비디오를 Android 기기에서 테스트하여 올바른 전달 가능성을 확인하세요. <br><br>웹 도구를 사용하여 MP4 파일을 MP4로 재포맷할 수 있습니다, 예를 들어 [CloudConvert](https://cloudconvert.com/mp4-converter). 도구에 MP4 파일을 업로드하고, 다시 MP4로 변환한 후 변환된 파일을 다운로드하세요.
{% endalert %}
...
| 마케팅 옵트아웃 버튼 | 기본적으로 구독 상태는 자동으로 업데이트되지 않습니다. 전체 절차는 [Opt-ins & Opt-Outs]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/message_processing/opt-ins_and_opt-outs/#marketing-opt-out-selection)을 참조하세요. |
| 쿠폰 코드 메시지 템플릿 | 메시지 템플릿에서만 사용 가능합니다. <br>이들은 다른 메시지 템플릿처럼 열고 편집할 수 있으며, Liquid 및 Braze 프로모션 코드와 호환됩니다. |
| CTA 응답 메시지  | 행동 버튼을 포함하는 응답 메시지를 만드세요. |
