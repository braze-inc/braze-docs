...
![기본 속성이 "소스"인 `ecommerce.cart_updated` 커스텀 이벤트 트리거의 캔버스 단계( `whats_app`.]({% image_buster /assets/img/whatsapp/product_message_canvas_step.png %}))
...
![연결된 콘텐츠 호출에 대한 체크아웃 경험 워크플로우를 보여주는 다이어그램: Meta는 주문 수신 메시지를 이커머스 플랫폼과 주고받는 통화 기능이 있는 Braze에 보낸 다음 WhatsApp 메시지를 보냅니다.]({% image_buster /assets/img/whatsapp/connected_content_checkout.png %})
