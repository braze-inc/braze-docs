...
이전에 Braze 내에서 사용하지 않았던 플로우를 사용하는 경우 **메시지 작성기의** **플로우 커스텀 속성** 섹션을 볼 때 아무런 정보가 표시되지 않을 수 있습니다. 이는 스키마가 아직 생성되지 않았음을 의미합니다.
...
- **흐름이 변경됩니다:** 스키마가 생성된 후 Flow를 변경하는 경우 추가 테스트 메시지를 보내야만 Braze가 Flow 응답의 모양이 변경되었음을 이해하고 그에 따라 속성 구조를 조정할 수 있습니다. 이 작업은 24시간에 한 번으로 제한됩니다. 
- **일관성:** 생성된 플로우 커스텀 속성은 일관되며 사용되는 캔버스에 관계없이 이 특정 플로우에 대해 동일한 속성이 됩니다.
- **수동 옵션:** **플로우 응답을 커스텀 속성으로 저장** 확인란을 선택할 필요는 없습니다. 사용자 단계 중복을 피하기 위해 [플로우 응답의 특정 필드를 특정 커스텀 속성에 저장하여](#saving-specific-fields-from-flow-responses-to-a-specific-custom-attribute) 커스텀 속성을 수동으로 생성할 수 있습니다.
