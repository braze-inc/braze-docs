...
> WhatsApp 플로우는 기존 WhatsApp 채널을 개선한 기능으로, 대화형 및 동적 메시징 경험을 만들 수 있습니다. 이 페이지에서는 WhatsApp 플로우 사용에 대한 단계별 지침을 제공합니다.
...
응답 구조에 대한 정보를 Braze에 제공하는 가장 간단한 방법은 플로우 응답을 커스텀 속성으로 저장하고 테스트 전송을 완료하는 것입니다.

#### Braze에서 사용되지 않은 Flow 사용

이전에 Braze 내에서 사용하지 않은 플로우를 사용하는 경우 **메시지 작성기의** **플로우 커스텀 속성** 섹션을 볼 때 아무런 정보가 표시되지 않을 수 있습니다. 이는 스키마가 아직 생성되지 않았음을 의미합니다.

![메타 플로우 섹션에서 플로우 커스텀 속성을 볼 수 있는 옵션이 있습니다.]({% image_buster /assets/img/whatsapp/flows/flow_custom_attribute.png %}){: style="max-width:70%;"}

이 문제를 해결하려면 다음을 수행하세요:

1. WhatsApp 메시지 설정 단계를 완료합니다.
2. **플로우 응답을 커스텀 속성으로 저장을** 선택했는지 확인합니다.

![메타 플로우 섹션에서 플로우 응답을 커스텀 속성으로 저장하는 확인란을 선택합니다.]({% image_buster /assets/img/whatsapp/flows/save_flow_responses_checkbox.png %}){: style="max-width:80%;"}

{: start="3"}
3\. 자신에게 테스트 메시지를 보내고 사용자로서 플로우를 완료합니다.

이제 Braze는 플로우 응답 JSON의 형태를 가지며 커스텀 속성을 생성할 수 있습니다.

{% endtab %}
{% tab Alternative methods %}

진행 중인 JSON 편집기를 사용하여 플로우 응답의 속성을 커스텀 속성에 저장하거나 다단계 캔버스를 사용하여 응답을 중첩된 고객 속성에 저장할 수 있습니다. 
...
![사용자 업데이트 단계는 고급 JSON 편집기로 진행합니다.]({% image_buster /assets/img/whatsapp/flows/user_update_advanced_json_editor.png %})
...
흐름이 완료되면 Braze는 자동으로 흐름 ID를 기반으로 한 이름으로 흐름 커스텀 속성을 생성합니다. 그런 다음 **고객** 프로필로 이동하여 **커스텀 속성** 섹션에서 저장된 플로우 응답을 중첩된 개체로 볼 수 있습니다.

스키마가 생성되면 플로우 **커스텀 속성** 섹션에 각 응답에 대해 예상되는 데이터 유형(예: "문자열" 또는 "문자열 배열")을 포함한 예상 구조가 표시됩니다.

![스키마 드롭다운이 있는 플로우 커스텀 속성 세부 정보 창입니다.]({% image_buster /assets/img/whatsapp/flows/flow_custom_attribute_details.png %}){: style="max-width:80%;"}

### 고려 사항

- **기존 속성:** 특정 플로우에 대한 커스텀 속성이 이미 생성되어 있는 경우 해당 플로우는 사용 가능한 속성 정보와 함께 로드됩니다. 이러한 경우 Braze가 이미 예상 응답 메시지를 인식하고 있으므로 스키마를 생성하기 위해 테스트 메시지를 보낼 필요가 없습니다.
- **흐름이 변경됩니다:** 스키마가 생성된 후 Flow를 변경하는 경우 추가 테스트 메시지를 보내야 Braze가 Flow 응답의 모양이 변경되었음을 이해하고 그에 따라 속성 구조를 조정할 수 있습니다. 이 작업은 24시간에 한 번으로 제한됩니다. 
- **일관성:** 생성된 플로우 커스텀 속성은 일관되며 사용되는 캔버스에 관계없이 이 특정 플로우에 대해 동일한 속성이 됩니다.
- **수동 옵션:** **플로우 응답을 커스텀 속성으로 저장** 확인란을 선택할 필요는 없습니다. [플로우 응답의 특정 필드를 특정 커스텀 속성에 저장하여](#saving-specific-fields-from-flow-responses-to-a-specific-custom-attribute) 수동으로 커스텀 속성을 생성할 수 있으므로 사용자 단계의 중복을 피할 수 있습니다.
...
![속성을 선택할 수 있는 "flow_1" 창이 표시됩니다.]({% image_buster /assets/img/whatsapp/flows/attribute_key.png %})
...
!["개인화 추가" 구성요소가 있는 WhatsApp 메시지 작성기를 사용하여 커스텀 속성으로 WhatsApp 속성 개인화를 삽입 `inbound_flow_response`.]({% image_buster /assets/img/whatsapp/flows/parsed_json.png %}))
...
[지원되는 개인화 태그와]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/supported_personalization_tags/) 함께 Liquid를 통해 플로우 응답을 사용하려면 다음 단계를 완료하세요:

1. WhatsApp 메시지를 작성할 때 더하기 아이콘을 선택하여 **개인화 추가** 창을 엽니다.
2. 개인화 유형은 **WhatsApp 속성을** 선택하고 사용자 지정 **속성은** **inbound_flow_response** 을 선택합니다. 정보를 고객 프로필에 저장하거나 메시징에 포함하거나 웹훅과 같은 다른 서비스로 전달하는 데 사용할 수 있습니다.

!["개인화 추가" 구성요소가 있는 WhatsApp 메시지 작성기를 사용하여 커스텀 속성으로 WhatsApp 속성 개인화를 삽입 inbound_flow_response.]({% image_buster /assets/img/whatsapp/flows/inbound_flow_response.png %})){: style="max-width:80%;"}
