...
Braze와 외부 AI 또는 커뮤니케이션 시스템 간의 통합은 양방향으로 작동하며, Braze는 커뮤니케이션 채널, 외부 시스템은 메시지를 처리하고 응답을 공식화하는 '인텔리전스'가 됩니다.
...
| Prerequisite | 설명 |
| - | - |
| 외부 시스템 | 챗봇을 구축하고 관리할 수 있는 타사 AI 또는 커뮤니케이션 시스템, API를 사용하는 자동화된 클라이언트 서비스 시스템 또는 둘 다를 사용할 수 있습니다. |
| Braze와 WhatsApp 통합 | Braze가 관리하는 WhatsApp 번호 |
| Braze REST API 키 | `campaigns.trigger.send` 권한이 있는 REST API 키입니다. **설정** > **API 키로** 이동하여 Braze 대시보드에서 생성할 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 role=”presentation” }

## 통합 구성하기

### 1단계: 인바운드 메시징을 위한 웹훅 캠페인 만들기
...
![WhatsApp 인바운드 메시지 전송을 트리거하는 실행 기반 전달.]({% image_buster /assets/img/whatsapp/inbound_message_trigger.png %})

{: start="6"}
6\. 캠페인 구성을 완료한 다음 캠페인을 저장하고 실행합니다. 캠페인을 시작한 후 메시지가 수신될 때마다 Braze는 외부 시스템으로 웹훅을 보냅니다.
...
![메시지 유형과 메시지 레이아웃을 선택할 수 있는 옵션이 있는 메시지 작성기입니다.]({% image_buster /assets/img/whatsapp/response_message_layout.png %})
...
2. `/messages/send` 요청 본문에는 [2단계의](#step-2) `campaign_id`, 사용자의 `external_id`, 외부 시스템의 응답 내용을 포함합니다.
