...
2019년 이후, 통신사들은 상업적 메시징을 위해 10DLC를 채택하기 시작했으며, 현재 Verizon과 AT&T이 10DLC를 지원하고 있으며, 모든 주요 통신사들이 곧 따를 것으로 예상합니다. 단기적으로는 불편을 초래할 수 있지만, 장기적으로는 원치 않는 메시지로부터 소비자를 보호하면서 전달률을 높일 수 있습니다. 
...
아직 10DLC에 등록하지 않은 고객은 고객 성공 매니저와 협력하여 긴 코드를 등록해야 합니다. **고객이 긴 코드를 등록하지 않으면 2021년 10월 1일부터 긴 코드를 사용하는 모든 A2P 발신자에게 모든 메시지에 대한 필터링이 강화됩니다.** 10DLC 등록을 시작하려면 고객 성공 매니저에게 연락하십시오.
