...
- **국제 번호:** 모든 국제 번호는 `+`로 시작해야 하며, 그 뒤에 국가 코드와 전화번호가 와야 합니다. For example, `+442071838750`.
...
{% alert important %}
사용자 전화번호는 Braze에서 숫자의 문자열로 나타납니다. 선행 {% raw %}`+`{% endraw %}을 제외한 비숫자(예: `,`, `-` 또는 `(`)가 포함된 번호를 가져오면 Braze에서 렌더링될 때 비숫자가 제거됩니다. 예를 들어, `+1 (724) 123-4567`을 가져오면 `+17241234567`로 나타납니다.
{% endalert %}
...
![Braze에서 유효하지 않은 전화번호에 대한 오류 메시지 예입니다.]({% image_buster /assets/img/sms/invalid_banner.png %}){: style="max-width:50%;border: 0;"}
