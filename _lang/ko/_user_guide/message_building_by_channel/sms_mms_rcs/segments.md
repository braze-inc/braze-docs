...
RCS 메시지는 메시지당 요금이 부과됩니다. 청구 가능한 RCS 메시지 유형을 이해하는 것은 청구를 이해하는 데 핵심입니다.

### RCS 청구 가능한 메시지 유형

RCS 메시지는 몇 가지 다른 방식으로 청구됩니다. Braze는 현재 두 가지 유형의 청구를 지원합니다: 기본 RCS 및 단일 RCS. 

- **기본 RCS 메시지**: 텍스트 전용이며 최대 160자 길이의 메시지입니다. 
- **단일 RCS 메시지:** 텍스트 전용이며 160자보다 긴 메시지 또는 모든 리치 요소가 포함된 메시지입니다. 풍부한 요소에는 이미지와 버튼(예: 제안된 답변 또는 제안된 작업)이 포함됩니다.

해당 청구 유형은 RCS 메시지 작성기 내에서 두 가지 값 중 하나를 가진 레이블로 표시됩니다: **텍스트 전용 RCS** (기본 RCS) 및 **RCS** (단일 RCS).

귀하의 RCS 청구 유형 데이터는 [메시지 사용 대시보드]({{site.baseurl}}/message_usage_dashboard/)에 채워지며, 이는 귀하의 메시지 크레딧 소비를 크레딧 비율과 사용된 메시지 크레딧 수를 지정하여 표시합니다. 