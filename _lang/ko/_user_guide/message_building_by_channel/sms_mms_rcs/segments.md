...
function getCharacterEncoding(char, type) {
  ...
  // 자동 감지를 위해, 문자가 GSM-7 세트에 있는지 확인
...
우리 플랫폼은 두 가지 주요 청구 모델을 지원합니다: 글로벌 모델과 미국 모델.
...
단일 RCS 메시지는 160자를 초과하거나 버튼이나 미디어와 같은 리치 요소를 포함하는 메시지입니다. 이들은 메시지 길이에 관계없이 단일 메시지로 청구됩니다.
