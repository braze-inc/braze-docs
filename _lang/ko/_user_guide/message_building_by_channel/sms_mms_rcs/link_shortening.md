...
- **고급 추적은** 캠페인 수준 및 사용자 수준의 클릭 추적을 가능하게 하며, 클릭에 의존하는 세분화 및 리타겟팅 기능을 사용할 수 있게 해줍니다. Clicks will also generate an [SMS click event]({{site.baseurl}}/user_guide/data/braze_currents/event_glossary/message_engagement_events/) sent through Currents. 추적이 진행되는 정적 URL의 길이는 27~28자로, URL을 클릭한 사용자의 세그먼트를 생성할 수 있습니다. 개인화된 URL의 길이는 32~33자입니다.

링크는 공유 쇼트 도메인(`brz.ai`)을 사용하여 단축됩니다. 예시 URL은 `https://brz.ai/8jshX` (기본, 정적) 또는 `https://brz.ai/p/8jshX/2dj8d` (고급, 개인화)와 같이 보일 수 있습니다. 자세한 내용은 [테스트](#testing)를 참조하세요.
...
Braze는 `http://` 또는 `https://` 로 시작하는 URL만 인식합니다. URL이 인식되면 **미리 보기** 섹션이 자리 표시자 URL로 업데이트됩니다. Braze는 단축 후 URL 길이를 추정하지만, 보다 정확한 추정치를 위해 테스트 사용자를 선택하고 메시지를 초안으로 저장하라는 경고 메시지가 표시됩니다.
...
Liquid에서 렌더링하는 URL은 API 트리거 프로퍼티에 포함된 URL도 단축합니다. 예를 들어 {% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %} 이 유효한 URL을 담당하는 경우 메시지를 보내기 전에 해당 URL을 단축하고 추적합니다. 

### `/messages/send` 엔드포인트의 URL 단축
...
테스트 메시지를 보내기 전에 캠페인 또는 캔버스를 저장하여 메시지에 전송되는 단축 URL의 표현을 받아보세요. 테스트 전송 전에 캠페인 또는 캔버스가 저장되지 않은 경우, 테스트 전송에는 입력 안내 URL이 포함됩니다.

캔버스가 '클릭한 단축 SMS 링크' 필터에 표시되려면 단축 링크가 포함된 캔버스 단계에도 사용자 수준의 클릭 추적을 허용하는 고급 추적이 인에이블먼트되어 있어야 합니다. 기본 추적을 사용하여 짧은 링크를 구성한 경우 SMS 짧은 링크 클릭 이벤트를 필터링하는 옵션을 사용할 수 없습니다.
...
링크 단축이 켜져 있으면 **SMS/MMS/RCS 성능/성과** 테이블에 **총 클릭** 수라는 제목의 열이 포함되어 배리언트별 클릭 이벤트 수와 관련 클릭률을 표시합니다. 메트릭에 대한 자세한 내용은 [메시지 성능을]({{site.baseurl}}/sms_mms_rcs_reporting/) 참조하세요.
...
{% alert note %}
현재 커런츠에서는 RCS 클릭 이벤트를 사용할 수 없습니다.
{% endalert %}

### 링크 단축은 딥링크 또는 유니버설 링크에서 작동하나요?

링크 단축은 딥링크에서는 작동하지 않습니다. 또는 Branch 또는 Appsflyer와 같은 타사 제공업체의 유니버설 링크를 단축할 수 있지만 사용자에게 짧은 리디렉션 또는 "깜박임" 현상이 발생할 수 있습니다. 이는 단축 링크가 앱 열기를 지원하는 유니버설 링크로 확인되기 전에 웹을 먼저 통과하기 때문에 발생합니다. 또한 Braze는 유니버설 링크를 단축할 때 발생할 수 있는 문제, 예를 들어 기여도가 깨지거나 예기치 않은 리디렉션이 발생하는 등의 문제를 해결할 수 없습니다.
