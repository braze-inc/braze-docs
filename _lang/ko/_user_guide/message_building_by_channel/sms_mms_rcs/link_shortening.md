...
!['메시지' 상자에 긴 URL을 입력하고 미리 보기에서 단축 링크를 생성하는 메시지 작성기]({% image_buster /assets/img/link_shortening/shortening3.png %})
...
Liquid에서 렌더링하는 URL은 API 트리거 프로퍼티에 포함된 URL도 단축합니다. 예를 들어 {% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %} 이 유효한 URL을 나타내는 경우, 메시지를 보내기 전에 해당 URL을 단축하여 추적합니다. 
...
링크 단축이 켜져 있으면 **SMS/MMS/RCS 실적** 테이블에 이형 상품별 클릭 이벤트 수와 관련 클릭률을 표시하는 **총 클릭** 수라는 열이 포함됩니다. 메트릭에 대한 자세한 내용은 [메시지 성능을]({{site.baseurl}}/sms_mms_rcs_reporting/) 참조하세요.

![SMS 및 MMS 성능 지표 표.]({% image_buster /assets/img/link_shortening/shortening4.png %})

**과거 실적** 및 **SMS/MMS/RCS 실적** 테이블에는 **총 클릭 수** 옵션도 포함되어 있으며 클릭 이벤트의 일일 시계열을 표시합니다. 클릭 수는 리디렉션 시(예: 사용자가 링크를 방문할 때) 증가하며, 사용자당 한 번 이상 증가될 수 있습니다.
...
{% alert note %}
현재 RCS 클릭 이벤트는 커런츠를 통해 이용할 수 없습니다.
{% endalert %}
