...
- **첫인상(권장):** Braze는 사용자가 캔버스 단계에 진입하면 오디언스 자격을 계산합니다. 사용자가 다음에 앱을 열면(새 [세션](https://www.braze.com/resources/articles/whats-an-app-session-anyway) 시작) Braze는 Liquid 또는 연결된 콘텐츠와 같은 개인화를 템플릿화한 다음 카드를 생성합니다. 이 옵션은 더 나은 카드 전달 성능/성과와 최신 개인화를 제공합니다.
...
<tbody>
  ...
  <td class="tg-0pky"><em>보낸 메시지는</em> 세션 시작 후 Braze가 사용자에게 보낸 카드 수를 나타냅니다. 캔버스에서는 사용자가 세션을 시작하지 않고 단계에 진입하면 Braze가 카드를 보내지 않으므로 이 측정기준이 단계에 진입하는 사용자 수와 일치하지 않을 수 있습니다.<br><br>도달 가능한 사용자와 노출 횟수는 변하지 않지만, 첫 노출 시 카드를 만들면 캠페인 시작 또는 캔버스 단계 진입에 비해 전송량<em>(전송된 메시지 수)이</em> 감소할 수 있습니다.</td>
  </tr>
  <tr>
...
첫 노출 횟수에서 카드가 생성되면 카드가 처리되는 데 1~2초가 소요될 수 있습니다. 이 처리 시간은 카드 크기 및 메시지 템플릿 옵션의 복잡성 등 다양한 요인에 따라 달라집니다. 예를 들어, 커넥티드 콘텐츠를 사용하는 카드의 처리 시간은 적어도 연결된 콘텐츠 응답 시간만큼 길어집니다.

### 이전 SDK 버전

사용자의 앱이 이전 소프트웨어 개발 키트 버전을 실행하는 경우에도 회원님이 보낸 콘텐츠 카드를 받을 수 있습니다. 그러나 카드가 표시되는 데 시간이 오래 걸리며 다음 콘텐츠 카드 동기화 때까지 표시되지 않을 수 있습니다.
