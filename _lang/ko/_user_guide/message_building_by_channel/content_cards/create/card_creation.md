...
![콘텐츠 카드 제어 섹션은 예약된 콘텐츠 카드의 전달을 편집할 때입니다.]({% image_buster /assets/img_archive/card_creation.png %})

다음 옵션을 사용할 수 있습니다:

- **캠페인 시작 시:** 콘텐츠 카드의 이전 기본 동작입니다. Braze는 캠페인이 시작될 때 잠재 고객 자격 및 개인화를 계산한 다음, 사용자가 앱을 열 때까지 카드를 생성하고 저장합니다. 
- **첫인상(권장):** 사용자가 다음에 앱을 열 때(새 [세션](https://www.braze.com/resources/articles/whats-an-app-session-anyway) 시작), Braze는 사용자가 자격이 있는 콘텐츠 카드를 결정하고 Liquid 또는 연결된 콘텐츠와 같은 개인화를 템플릿화한 후 카드를 생성합니다. 이 옵션은 일반적으로 더 나은 성능을 제공합니다.
...
![콘텐츠 카드 제어 섹션은 예약된 콘텐츠 카드의 전달을 편집할 때입니다.]({% image_buster /assets/img_archive/card_creation_canvas.png %})

다음 옵션을 사용할 수 있습니다:

- **단계 진입 시:** 콘텐츠 카드의 이전 기본 동작입니다. Braze는 사용자가 캔버스 단계에 진입하면 오디언스 자격을 계산한 다음, 카드를 생성하고 사용자가 앱을 열 때까지 저장합니다.
- **첫인상(권장):** Braze는 사용자가 캔버스 단계에 진입하면 오디언스 자격을 계산합니다. 사용자가 다음에 앱을 열 때(새 [세션](https://www.braze.com/resources/articles/whats-an-app-session-anyway) 시작), Braze는 Liquid 또는 연결된 콘텐츠와 같은 개인화를 템플릿화한 후 카드를 생성합니다. 이 옵션은 카드 전달에서 더 나은 성능과 더 최신의 개인화를 제공합니다.
...
### 카드를 시작할 때 또는 첫 노출 시 생성하는 것의 차이 {#differences}
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-0pky">Braze는 사용자가 다음에 앱을 열 때(세션 시작, <a href="#campaign_note">캠페인만 해당*</a>) 멤버십을 평가합니다.<br><br> 이 설정은 신규 또는 익명 사용자가 카드를 보려고 할 때 항상 적격성 여부를 평가하기 때문에 더 많은 오디언스에게 도달할 수 있습니다. <br><br>또한, 비율 제한(카드를 받을 사람 수 제한)은 첫 노출 시 설정할 때 적용되지 않습니다.</td>
  </tr>
  <tr>
  ...
  <td class="tg-0pky"><em>전송된 메시지</em>는 Braze가 생성하고 제공한 카드 수를 나타냅니다. 이것은 사용자가 카드를 보았는지 여부를 계산하지 않습니다.</td>
  <td class="tg-0pky"><em>전송된 메시지</em>는 세션 시작 후 Braze가 사용자에게 보내는 카드 수를 나타냅니다. 캔버스에서 사용자가 세션을 시작하지 않고 단계에 들어가면 Braze는 카드를 보내지 않으므로 이 지표는 단계에 들어가는 사용자 수와 일치하지 않을 수 있습니다.<br><br>도달 가능한 사용자와 노출 수는 변하지 않지만, 캠페인 시작 또는 캔버스 단계 진입에 비해 첫 노출 시 카드를 생성할 때 전송량(<em>전송된 메시지</em>)이 낮아질 것으로 예상됩니다.</td>
  </tr>
  <tr>
    <td class="leftHeader">처리 시간</td>
  <td class="tg-0pky">Braze는 시작 시간에 세그먼트의 모든 자격이 있는 사용자에 대해 카드를 생성합니다. 대규모 청중의 경우, <b>첫 노출 시</b>를 선택하여 카드가 시작 후 더 빨리 제공되도록 합니다.</td>
  <td class="tg-0pky">Braze는 사용자가 카드를 처음 보려고 할 때 카드를 생성하므로 첫 노출 시 표시되는 데 1-2초가 걸릴 수 있습니다.</td>
  </tr>
</tbody>
</table>
...
대규모 청중의 경우, 카드가 시작 후 빠르게 제공되도록 첫 노출 시 카드를 생성하는 옵션을 선택합니다. 세션 시작 시 트리거된 캠페인은 성능을 개선하기 위해 첫 노출 시 생성으로 이동하는 것의 이점을 누릴 수 있습니다(예약된 전달을 통해 사용 가능).

카드가 첫 노출 시 생성되면 카드가 처리되는 데 1-2초가 걸릴 수 있습니다. 이 처리 시간은 카드 크기 및 메시지 템플릿 옵션의 복잡성 등 다양한 요인에 따라 달라집니다. 예를 들어, 커넥티드 콘텐츠를 사용하는 카드의 처리 시간은 적어도 연결된 콘텐츠 응답 시간만큼 길어집니다.

### 이전 SDK 버전

사용자의 앱이 이전 SDK 버전을 실행하는 경우에도 전송하는 콘텐츠 카드를 여전히 받습니다. 그러나 카드는 나타나는 데 더 오랜 시간이 걸리며 다음 콘텐츠 카드 동기화까지 표시되지 않을 수 있습니다.
