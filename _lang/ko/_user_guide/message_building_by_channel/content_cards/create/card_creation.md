...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-0pky"><em>전송된 메시지는</em> 세션 시작 후 사용자에게 전송된 카드 수를 나타냅니다. 캔버스에서는 세션을 시작하지 않고 단계에 입장하는 사용자에게는 카드가 전송되지 않으므로 이 지표가 단계에 입장하는 사용자 수와 일치하지 않을 수 있습니다.<br><br>도달 가능한 사용자와 노출 수는 변하지 않지만, 캠페인 시작 또는 캔버스 단계 진입 시 동일한 카드를 만들었을 때와 비교하여 첫 노출 시 카드를 만들면 전송량<em>(전송된 메시지 수<em>)이 감소할 것으로 예상할 수 있습니다.</td>
  </tr>
  <tr>
