...
먼저 메시지를 수신할 플랫폼을 선택합니다. 이 선택을 사용하면 특정 앱 집합으로 캠페인 전송을 제한할 수 있습니다. 예를 들어, 사용자가 모바일 앱을 다운로드하도록 유도하는 인브라우저 메시지에 **웹 브라우저를** 선택하여 이미 앱을 다운로드한 후 메시지를 받지 않도록 할 수 있습니다. 플랫폼 선택은 각 배리언트에 따라 다르므로 플랫폼별로 메시지 참여를 테스트해 볼 수 있습니다.
...
![신규 고객을 환영하고 고객 프로필을 설정하도록 유도하는 브랜드의 인앱 메시지 예시입니다.]({% image_buster /assets/img_archive/iam_compose.png %}){: style="max-width:85%" }
...
메시지 유형에 따라 **이미지 업로드**, **배지 선택** 또는 **멋진 글꼴**을 사용할 수 있습니다. 이미지를 업로드하려면 **이미지 추가를** 선택하거나 이미지 URL을 입력합니다. **이미지 추가를** 선택하면 이전에 업로드한 이미지를 선택하거나 새 이미지를 추가할 수 있는 **미디어 라이브러리가** 열립니다. 메시지 유형과 플랫폼마다 권장 비율과 요구 사항이 다를 수 있으므로 이미지를 커미셔닝하거나 처음부터 만들기 전에 이를 확인해야 합니다.
...
이 설정은 사용자 지정 코드 메시지 유형에만 적용됩니다. HTML을 복사하여 사용 가능한 공간에 붙여넣고 ZIP 파일을 사용하여 자산을 업로드합니다.
...
{% alert important %}
일부 인앱 메시지 유형에는 ZIP 파일을 사용하여 커스텀 HTML(또는 CSS 또는 JavaScript) 및 자산을 업로드하는 것 외에 스타일링할 수 있는 옵션이 없습니다. [CSS가 포함된 웹 모달을]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/customize/#web-modal-css) 사용하면 사용자 지정 CSS를 업로드하거나 작성하여 모든 기능을 갖춘 멋진 사용자 지정 스타일의 메시지를 만들 수 있습니다.
{% endalert %}
...
다음으로 세그먼트 또는 필터를 선택하여 오디언스 범위를 좁혀 [사용자를 타겟팅해야]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/) 합니다. 대략적인 세그먼트 인구가 어떤 모습인지에 대한 스냅샷을 자동으로 받게 됩니다. 정확한 세그먼트 멤버십은 항상 메시지를 보내기 전에 계산된다는 점에 유의하세요.
...
!['표시하기 전에 캠페인 자격 재평가' 확인란을 선택합니다.]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %}){:style="max-width:60%"}
...
Braze는 안정성과 속도를 중시합니다. 필요한 데이터만 Braze에 전송하고 더 이상 브랜드에 가치를 더하지 않는 캠페인은 해제하는 것이 좋습니다.
...
사용자의 현지 시간대를 사용하여 인앱 메시지 캠페인을 예약하면 캠페인의 시작 및 종료 시간에 대한 평가가 기기 자체에서 처리됩니다.

인앱 메시지 캠페인은 일반적으로 앱 세션이 시작되거나 새로고침될 때 사용자의 기기에 푸시됩니다. 그 순간:

1. 소프트웨어 개발 키트는 사용자가 트리거 기반 인앱 메시지를 받을 자격이 있는지 여부를 평가합니다.
2. 기기는 사용자의 트리거 이벤트가 캠페인의 시작 및 종료 시간(사용자의 현지 시간대로 정의된 시간) 내에 발생했는지 확인합니다.
3. 두 조건이 모두 충족되면 인앱 메시지가 표시될 수 있습니다.

#### 고려 사항

- 인앱 메시지가 전달된 직후 사용자가 이벤트를 트리거(예: 버튼 탭)하면 모든 자격 기준이 충족된다는 가정 하에 다음 세션을 새로고침할 때까지 메시지가 표시되지 않을 수 있습니다.
- 다른 채널 유형과 마찬가지로 인앱 메시지 캠페인은 24~48시간 전에 시작하는 것이 이상적입니다. 이 버퍼는 사용자가 자격 요건을 충족하고 세션을 시작하여 메시지를 평가하고 표시할 수 있는 충분한 시간을 제공합니다.