...
메시지를 캠페인으로 보내야 할지 캔버스로 보내야 할지 잘 모르시겠어요? 캠페인은 단일, 타겟 메시징 캠페인에 더 적합하고, 캔버스는 다단계 사용자 여정에 더 적합합니다.
...
먼저 메시지를 수신할 플랫폼을 선택합니다. 이 선택을 사용하면 특정 앱 집합으로 캠페인 전송을 제한할 수 있습니다. 예를 들어, 사용자가 모바일 앱을 다운로드하도록 유도하는 인브라우저 메시지에 **웹 브라우저를** 선택하여 이미 앱을 다운로드한 후 메시지를 받지 않도록 할 수 있습니다. 플랫폼 선택은 각 배리언트에 특정하므로, 플랫폼별 메시지 참여를 테스트해 볼 수 있습니다.

| 플랫폼                        | 메시지 전달        |
|---------------------------------|-------------------------|
| 모바일 앱                     | iOS & Android SDKs      |
| 웹 브라우저                    | 웹 SDK                 |
| 모바일 앱 & 웹 브라우저 | iOS, Android & 웹 SDKs |
...
![예시 브랜드의 인앱 메시지로, 새로운 고객을 환영하고 사용자 프로필 설정을 유도합니다.]({% image_buster /assets/img_archive/iam_compose.png %}){: style="max-width:85%" }

**작성** 탭의 내용은 이전 단계에서 선택한 메시지 옵션에 따라 달라지지만 다음 옵션 중 하나를 포함할 수 있습니다:

### Language

**언어 추가**를 선택하고 제공된 목록에서 원하는 언어를 선택하세요. 이렇게 하면 [Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/conditional_logic/#conditional-logic)가 메시지에 삽입됩니다. 콘텐츠를 작성하기 전에 언어를 선택하여 Liquid에서 원하는 위치에 텍스트를 채울 수 있도록 하는 것이 좋습니다. See our [full list of available languages]({{site.baseurl}}/developer_guide/platform_integration_guides/android/advanced_use_cases/localization/#languages-supported).

### 이미지

메시지 유형에 따라 **이미지 업로드**, **배지 선택** 또는 **멋진 글꼴**을 사용할 수 있습니다. 이미지를 업로드하려면 **이미지 추가**를 선택하거나 이미지 URL을 제공하세요. **이미지 추가**를 선택하면 **미디어 라이브러리**가 열리며, 이전에 업로드한 이미지를 선택하거나 새 이미지를 추가할 수 있습니다. 각 메시지 유형과 플랫폼은 고유한 비율과 요구 사항이 있을 수 있으므로, 이미지를 의뢰하거나 처음부터 만들기 전에 확인하세요.
...
![인앱 메시지 작성기 메시지 필드에 위치한 AI 카피라이터 버튼을 실행하세요.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_iam.png %}){: style="max-width:60%"}
...
이 설정은 사용자 지정 코드 메시지 유형에만 적용됩니다. HTML을 복사하여 사용 가능한 공간에 붙여넣고 ZIP 파일을 사용하여 자산을 업로드하세요.
...
메시지를 보내기 전에 항상 [미리 보고 테스트하세요]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/sending_test_messages/).

{% alert important %}
일부 인앱 메시지 유형은 사용자 정의 HTML(또는 CSS 또는 JavaScript) 및 ZIP 파일을 사용하여 자산을 업로드하는 것 외에는 스타일링 옵션이 없습니다. [CSS가 포함된 웹 모달을]({{site.baseurl}}/user_guide/message_building_by_channel/in-app_messages/customize/#web-modal-css) 사용하면 사용자 지정 CSS를 업로드하거나 작성하여 모든 기능을 갖춘 멋진 사용자 지정 스타일의 메시지를 만들 수 있습니다.
{% endalert %}
...
{% alert important %}
사용자 정의 이벤트를 기반으로 인앱 메시지를 트리거하려는 경우, 해당 사용자 정의 이벤트는 SDK를 사용하여 전송되어야 합니다.
{% endalert %}
...
- 구매하기
- 앱/웹페이지 열기
- 사용자 정의 이벤트 수행(오직 SDK를 사용하여 전송된 이벤트에서만 작동)
- 특정 푸시 메시지 열기
...
트리거된 메시지 우선순위의 높은, 중간 및 낮은 옵션은 버킷이며, 따라서 여러 메시지가 동일한 선택된 우선순위를 가질 수 있습니다. 여러 메시지가 동일한 우선 순위를 공유할 때, 가장 최근에 생성되거나 할당된 메시지가 우선하며 먼저 표시됩니다:

- **기본 우선 순위 버킷:** 두 캠페인이 동일한 트리거를 공유하고 기본(중간) 우선 순위를 사용할 때, 마지막에 생성된 캠페인이 트리거를 받습니다.
- **특정 우선 순위 버킷:** 여러 캠페인이 동일한 트리거를 공유하고 특정 우선 순위 버킷에 할당될 때, 가장 최근에 해당 버킷에 할당된 캠페인이 트리거를 받습니다.

이 버킷 내에서 우선 순위를 설정하려면 **정확한 우선 순위 설정**을 클릭하고, 캠페인을 드래그 앤 드롭하여 올바른 우선 순위로 정렬할 수 있습니다.

![인앱 메시지 캠페인과 캔버스의 우선 순위 설정 예시입니다.]({% image_buster /assets/img_archive/bucket_prioritization.png %}){: style="max-width:70%"}

#### 타겟팅할 사용자 선택

다음으로, [사용자 타겟팅]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)을 위해 세그먼트나 필터를 선택하여 오디언스를 좁혀야 합니다. 해당 세그먼트 인구의 대략적인 모습에 대한 스냅샷을 자동으로 받게 됩니다. 정확한 세그먼트 구성원 자격은 메시지가 전송되기 전에 항상 계산된다는 점을 염두에 두세요.
...
!["표시하기 전에 캠페인 자격 재평가" 체크박스가 선택되었습니다.]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %}){:style="max-width:60%"}
...
{% alert note %}
이 옵션을 활성화하면 사용자가 인앱 메시지를 트리거할 때와 메시지가 표시될 때 사이에 약간의 지연(100ms)이 발생합니다. 이는 추가된 자격 및 템플릿 요청 때문입니다.
<br><br>
...
캠페인 또는 캔버스의 마지막 제작을 완료한 후에는 세부 정보를 검토하고 [테스트한]({{site.baseurl}}/user_guide/engagement_tools/campaigns/testing_and_more/sending_test_messages/) 다음 전송하세요!
...
Braze는 안정성과 속도를 중시합니다. 브레이즈에 필요한 데이터만 전송하고 더 이상 브랜드에 가치를 추가하지 않는 캠페인은 끄는 것이 좋습니다.
...
{% alert important %}
메시지 전달 속도를 최적화하고 타임아웃을 방지하기 위해 작업 공간당 최대 200개의 활성 액션 기반 인앱 메시지 캠페인을 가질 수 있습니다. 이것은 캔버스에는 적용되지 않습니다.
{% endalert %}
...
사용자의 현지 시간대를 사용하여 인앱 메시지 캠페인이 예약될 때, 캠페인의 시작 및 종료 시간 평가는 장치 자체에서 처리됩니다.

인앱 메시지 캠페인은 일반적으로 앱 세션이 시작되거나 새로 고쳐질 때 사용자의 장치로 푸시됩니다. 그 순간:

1. SDK는 사용자가 트리거 기반 인앱 메시지에 자격이 있는지 평가합니다.
2. 기기가 사용자의 트리거 이벤트가 캠페인의 시작 및 종료 시간(사용자의 현지 시간대에 따라 정의됨) 내에 발생했는지 확인합니다.
3. 두 조건이 모두 충족되면 인앱 메시지가 표시될 수 있습니다.

#### 고려 사항

- 사용자가 인앱 메시지가 전달된 직후에 이벤트(예: 버튼 탭)를 트리거하면 메시지는 다음 세션 새로 고침까지 나타나지 않을 수 있습니다. - 모든 적격 기준이 여전히 충족된다고 가정할 때.
- 다른 채널 유형과 유사하게, 인앱 메시지 캠페인은 이상적으로 24-48시간 전에 시작되어야 합니다. 이 버퍼는 사용자가 적격성을 충족하고 메시지가 평가되고 표시될 세션을 시작할 수 있는 충분한 시간을 제공합니다.