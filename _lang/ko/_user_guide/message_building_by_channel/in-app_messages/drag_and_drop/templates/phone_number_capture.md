...
드래그 앤 드롭 인앱 메시지를 만들 때, 템플릿으로 **SMS 가입** (이것은 RCS 가입을 수용합니다) 또는 **WhatsApp 가입**을 선택한 다음 **메시지 구축**을 선택하세요. 이 템플릿은 모바일 앱과 웹 브라우저 모두에서 지원됩니다.
...
![커스텀 글꼴을 업로드하고 선택하는 워크플로우.]({% image_buster /assets/img_archive/dnd_iam_phone_capture_custom_font.gif %})
...
![선택된 구독 그룹이 있는 구독 그룹 드롭다운.]({% image_buster /assets/img_archive/dnd_iam_phone_capture_subscription.png %}){: style="max-width:40%"}
...
![번호를 수집하고자 하는 국가를 선택하는 국가 드롭다운.]({% image_buster /assets/img_archive/dnd_iam_phone_capture_countries.png %}){: style="max-width:40%"}

#### 유효하지 않은 전화번호

사용자가 허용되지 않는 특수 문자가 포함된 전화번호를 입력하면 사용자에게 사용자 지정할 수 없는 일반 오류 표시기가 표시되며 양식을 제출할 수 없습니다. 테스트 장치와 **미리보기 & 테스트** 탭에서 오류 동작을 볼 수 있습니다. 이 문서를 참조하여 [Braze가 전화번호를 형식화하는 방법]({{site.baseurl}}/user_guide/message_building_by_channel/sms/phone_numbers/user_phone_numbers/#importing-phone-numbers)을 배우십시오.
...
- 귀하의 약관 & 조건 및 SMS 및 RCS 개인정보 보호정책에 대한 링크.
