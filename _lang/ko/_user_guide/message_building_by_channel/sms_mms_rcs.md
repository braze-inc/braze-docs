---
nav_title: "SMS, MMS, 및 RCS"
article_title: "SMS, MMS, 및 RCS"
page_order: 4
layout: dev_guide
channel:
  - SMS
  - MMS
  - RCS
search_rank: 3
guide_top_header: "SMS, MMS, 및 RCS"
guide_top_text: "SMS (단문 메시지 서비스), MMS (멀티미디어 메시지 서비스), 및 RCS (풍부한 커뮤니케이션 서비스)는 대부분의 다른 메시징 채널보다 사용자에게 더 직접적으로 도달할 수 있는 방법을 제공합니다. 이는 사용자의 전화번호를 활용하여 실시간으로 도달할 수 있게 합니다. Braze SMS, MMS 또는 RCS를 시작하기 위해 다음 주제를 확인하세요!<br><br>다음 주제에 대한 준비가 되지 않았나요? Braze의 SMS <a href='/docs/about_sms/'>설정</a> 컬렉션으로 이동하여 SMS에 대해 알아보세요. <br><br>**SMS, MMS 및 RCS 사용 가능 여부는 귀하의 Braze 패키지에 따라 다릅니다.** **귀하의 계정 매니저 또는 고객 성공 매니저에게 연락하여 시작하십시오.**"
description: "이 랜딩 페이지는 SMS (단문 메시지 서비스), MMS (멀티미디어 메시지 서비스) 및 RCS (풍부한 커뮤니케이션 서비스)의 홈입니다. 이러한 서비스는 사용자의 전화번호를 활용하기 때문에 다른 대부분의 메시징 채널보다 더 직접적으로 사용자에게 다가갈 수 있는 방법을 제공하며, 실시간으로 사용자에게 연락할 수 있습니다."

guide_featured_title: "일반 기사"
guide_featured_list:
- name: "법률 및 규정"
  link: /docs/sms_mms_rcs_laws/
  image: /assets/img/braze_icons/briefcase-02.svg
- name: "구독 그룹"
  link: /docs/sms_rcs_subscription_groups/
  image: /assets/img/braze_icons/table.svg
- name: "지리적 권한"
  link: /docs/geographic_permissions/
  image: /assets/img/braze_icons/globe-04.svg
- name: "링크 단축"
  link: /docs/link_shortening/
  image: /assets/img/braze_icons/link-03.svg
- name: "리타겟팅"
  link: /docs/sms_mms_rcs_user_retargeting/
  image: /assets/img/braze_icons/users-01.svg
- name: "키워드 처리 및 사용 사례"
  link: /docs/keyword_processing/
  image: /assets/img/braze_icons/key-01.svg
- name: "사용자 전화번호"
  link: /docs/user_phone_numbers/
  image: /assets/img/braze_icons/phone-02.svg
- name: "SMS 및 RCS 발신자"
  link: /docs/sending_phone_numbers/
  image: /assets/img/braze_icons/phone-02.svg
- name: "보고"
  link: /docs/sms_mms_rcs_reporting/
  image: /assets/img/braze_icons/phone-02.svg
- name: "SMS 및 RCS 봇 클릭 필터링"
  link: /docs/sms_rcs_bot_click_filtering/
  image: /assets/img/braze_icons/phone-02.svg
- name: "청구 계산기"
  link: /docs/sms_rcs_billing_calculators/
  image: /assets/img/braze_icons/calculator.svg
- name: "메시지 사용 대시보드"
  link: /docs/message_usage_dashboard/
  image: /assets/img/braze_icons/file-07.svg

guide_menu_title: "SMS"
guide_menu_list:
- name: "SMS 정보"
  link: /docs/about_sms/
  image: /assets/img/braze_icons/check-square-broken.svg
- name: "알아두어야 할 SMS 약관"
  link: /docs/sms_terms_to_know/
  image: /assets/img/braze_icons/book-closed.svg
- name: "SMS 메시지 전송"
  link: /docs/sms_message_sending/
  image: /assets/img/braze_icons/phone-02.svg
- name: "SMS 메시지 만들기"
  link: /docs/create_sms_message/
  image: /assets/img/braze_icons/table.svg
- name: "SMS 자주 묻는 질문"
  link: /docs/sms_faq/
  image: /assets/img/braze_icons/annotation-question.svg

guide_menu_title2: "MMS"
guide_menu_list2:
- name: "MMS 정보"
  link: /docs/about_mms/
  image: /assets/img/braze_icons/brush-02.svg
- name: "MMS 짧은 코드 및 긴 코드"
  link: /docs/mms_short_long_codes/
  image: /assets/img/braze_icons/phone-02.svg
- name: "MMS 캠페인 만들기"
  link: /docs/create_mms_message/
  image: /assets/img/braze_icons/table.svg
- name: "연락처 카드"
  link: /docs/mms_contact_cards/
  image: /assets/img/braze_icons/user-circle.svg
- name: "MMS 자주 묻는 질문"
  link: /docs/mms_faq/
  image: /assets/img/braze_icons/annotation-question.svg

guide_menu_title3: "RCS"
guide_menu_list3:
- name: "RCS에 대하여"
  link: /docs/about_rcs/
  image: /assets/img/braze_icons/brush-02.svg
- name: "RCS 캠페인 만들기"
  link: /docs/create_rcs_message/
  image: /assets/img/braze_icons/table.svg
- name: "RCS 설정"
  link: /docs/rcs_setup/
  image: /assets/img/braze_icons/phone-02.svg
---
