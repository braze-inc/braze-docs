...
{% alert note %}
하나의 작업 공간에는 하나의 LINE 계정만 사용할 수 있습니다. LINE 계정이 여러 개 있는 경우, 각 계정을 다른 작업 공간에서 사용하는 것을 권장합니다.
{% endalert %}
...
### 2.2 단계: Braze에서 LINE 구독 그룹 생성하기
...
위에서 설명한 바와 같이, 동일한 `native_line_id`을 가진 여러 사용자 프로필이 존재할 가능성이 있습니다. 업데이트 방법으로 중복된 고객 프로필이 생성되는 경우 `/user/merge` 엔드포인트를 사용하여 식별되지 않은 사용자 프로필을 식별된 사용자 프로필에 병합할 수 있습니다. 
