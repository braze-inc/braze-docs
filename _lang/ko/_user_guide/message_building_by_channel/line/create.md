...
세그먼트에서 더 많은 오디언스를 선택하고 [필터]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/)를 사용하여 해당 세그먼트를 더 좁힐 수 있습니다. 해당 대략적인 세그먼트 인구가 어떻게 생겼는지의 스냅샷을 자동으로 받습니다. 정확한 세그먼트 구성원 자격은 메시지가 전송되기 전에 항상 계산된다는 점을 기억하세요.
...
아직 완료하지 않았다면, 캔버스의 나머지 섹션을 완료하세요. 캔버스의 나머지를 구축하는 방법, 다변량 테스트 및 지능형 선택 등을 포함한 추가 세부정보는 [캔버스 만들기]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/)를 참조하세요.
