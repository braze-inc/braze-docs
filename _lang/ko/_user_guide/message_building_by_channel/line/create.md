...
필요에 따라 개인화(예: Liquid 또는 연결된 콘텐츠)를 사용하여 메시지를 작성하세요. LINE은 각 메시지에 최대 다섯 개의 메시지 버블을 허용하며, 이는 텍스트, 이미지, 리치 또는 카드 기반의 사용 가능한 메시지 레이아웃 중 하나일 수 있습니다.

![미리보기에서 표시된 메시지가 있는 LINE 작성기.]({% image_buster /assets/img/line/line_composer.png %})
...
![테스트 메시지의 미리 보기를 표시하는 '테스트' 탭입니다.]({% image_buster /assets/img/line/test_preview.png %})
...
세그먼트에서 더 많은 오디언스를 선택하고 [필터]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/)를 사용하여 해당 세그먼트를 더 좁힐 수 있습니다. 해당 대략적인 세그먼트 인구가 어떻게 생겼는지의 스냅샷을 자동으로 받습니다. 메시지가 전송되기 전에 정확한 세그먼트 멤버십이 항상 계산된다는 점을 기억하세요.
...
아직 완료하지 않았다면, 캔버스의 나머지 섹션을 완료하세요. 캔버스의 나머지를 구축하는 방법, 다변량 테스트 및 지능형 선택 등을 포함한 자세한 내용은 []({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/)캔버스 만들기]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/)를 참조하세요.
