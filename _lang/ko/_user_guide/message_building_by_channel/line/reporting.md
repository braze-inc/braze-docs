...
> 캠페인 또는 캔버스를 시작한 후 캠페인 세부 정보 페이지 또는 캔버스 분석에서 주요 측정기준을 확인할 수 있습니다. 이 문서에서는 이러한 측정기준을 찾을 수 있는 위치와 해당 측정기준의 담당자에 대해 설명합니다.
...
!['LINE 성과' 패널에는 두 가지 변형에 대한 지표가 표시됩니다.]({% image_buster /assets/img/line/line_performance.png %})
...
| 기간 | Definition |
| --- | --- |
| Sends | Braze와 LINE 간에 성공적으로 통신한 총 전송 횟수입니다. 이는 사용자가 메시지를 수신했음을 의미하지는 않습니다. |
| 고유 열람 | 하루 최소 기준인 20개 메시징에 도달한 후 사용자가 열어본 LINE 메시징의 총 개수입니다. |
| 총 열람 수 | 하루 최소 기준인 20개 메시징에 도달한 후 사용자가 보낸 LINE 메시지를 열어본 총 횟수입니다. |
