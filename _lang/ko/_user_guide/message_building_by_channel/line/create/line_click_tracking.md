...
![긴 URL이 포함된 문자 메시지가 포함된 LINE 메시지 작성기입니다: https://braze.com/docs/user_guide/message_building_by_channel/line/create/]({% image_buster /assets/img/line/click_tracking_text_message.png %})
...
![LINE 메시지 작성기 '설정' 탭에서 '클릭 추적'이 켜져 있고 단축 URL이 포함된 미리보기 문자 메시지: https://olaf.brz.ai/p/9rcfdqdD]({% image_buster /assets/img/line/click_tracking_settings.png %})
...
![각각 URL이 있는 두 개의 탭 가능한 영역이 있는 리치 메시지가 있는 LINE 메시지 작성기.]({% image_buster /assets/img/line/rich_message_click_tracking.png %})
...
LINE 클릭 추적을 사용하면 자체 도메인을 사용하여 단축 URL의 모양과 느낌을 개인화할 수 있어 일관된 브랜드 이미지를 전달할 수 있습니다. 자세한 내용은 [사용자 정의 도메인을]({{site.baseurl}}/user_guide/message_building_by_channel/sms_mms_rcs/link_shortening/custom_domains) 참조하세요.
...
Braze는 API 트리거 프로퍼티에 포함된 URL을 포함하여 Liquid에서 렌더링하는 URL을 단축합니다. 예를 들어 {% raw %}`{{api_trigger_properties.${url_value}}}`{% endraw %} 이 유효한 URL을 담당하는 경우, 해당 URL을 단축하여 추적한 후 LINE 메시지를 전송합니다.

## 테스트

캠페인이나 Canvas를 시작하기 전에 먼저 메시지를 미리 보고 테스트하는 것이 가장 좋습니다. **테스트** 탭으로 이동하여 콘텐츠 테스트 그룹 또는 개별 사용자에게 LINE 메시지를 미리 보고 전송할 수 있습니다.
...
![세 가지 세분화 필터를 모두 표시하는 필터 그룹입니다: "클릭/열린 캠페인", "클릭/열린 캠페인 또는 태그가 있는 캔버스", "클릭/열린 단계".]({% image_buster /assets/img/line/line_segmentation_filters.png %})

## Frequently asked questions

### 테스트 전송 시 받은 링크가 실제 URL인가요?

예, 테스트 전송 시 실제 URL이 생성됩니다. 그러나 실제 캠페인에서 전송된 정확한 URL은 테스트 전송에서 전송된 URL과 다를 수 있습니다.
