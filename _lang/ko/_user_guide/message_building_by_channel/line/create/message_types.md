...
메시지 유형 패널에는 텍스트, 이미지, 리치 메시지 및 카드 기반 메시지를 작곡 편집기로 드래그할 수 있는 메시지 유형이 포함됩니다.
...
블랙 프라이데이 파티를 잊지 말라는 알림 메시지와 자정 전에 최대 80%를 절약할 수 있는 가능성.
...
작곡가 편집기에서 다양한 샌드위치 사진이 포함된 4분할 풍부한 메시지.
...
LINE 카드 기반 메시지는 사용자가 여러 메시지를 회전목마처럼 스크롤할 수 있게 하며, 카드나 카드의 버튼을 선택하여 자신에게 가장 관련 있는 메시지에 대해 조치를 취할 수 있게 합니다.
...
카드 기반 메시지로, 작곡가 편집기에서 빵과 샌드위치를 홍보하는 최소 두 개의 카드가 포함되어 있습니다.
