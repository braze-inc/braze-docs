...
> 여러 프로필이 동일한 이메일 주소를 공유하고 하나의 프로필이 구독을 취소하면, Braze는 해당 주소를 가진 다른 프로필(최대 100개)을 동일한 구독 상태로 업데이트합니다. 이는 구독 취소 및 글로벌 구독 상태와 개별 구독 그룹 상태와 같은 다른 변경 사항에 적용됩니다.

## 이메일 구독 업데이트

Braze는 이메일 캠페인을 보낼 때 중복 이메일 주소를 자동으로 확인하고 제거합니다. 이것은 여러 사용자 프로필이 주소를 공유하더라도 Braze가 이메일을 한 번 이상 보내는 것을 방지합니다.
...
사용자가 이메일 주소를 공유하고 하나의 프로필을 업데이트하면, Braze는 해당 사용자들(최대 100명) 간에 구독 변경 사항을 전파합니다.
