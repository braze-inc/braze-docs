...
모바일 및 웹 보기의 패딩을 단독으로 편집할 수 없으므로 편집한 내용은 두 보기 모두에 반영됩니다. 그러나 HTML 편집기에서 다양한 화면 크기에 따라 패딩을 설정하는 CSS 로직을 추가할 수 있습니다. 드래그 앤 드롭 편집기에서는 이 기능이 지원되지 않으므로 HTML 파일을 내보내고 대신 HTML 편집기를 사용할 수 있습니다.

### 데스크톱과 모바일에서 버튼 행을 가로로 유지하도록 최적화하려면 어떻게 해야 하나요?

드래그 앤 드롭 편집기를 사용하여 이메일을 구축할 때 콜투액션 버튼을 가로로 만들면 모바일에서 버튼이 세로 방향으로 변경되는 것을 볼 수 있습니다. 

기기 크기에서 동일한 형식을 유지하려면 모바일에 최적화된 패딩이 있고 데스크톱 기기에서 행을 숨기도록 설정된 CTA 버튼이 있는 별도의 행을 만드는 것이 좋습니다. 두 개의 행이 분리되어 있으므로 데스크톱과 모바일 기기에서 최상의 텍스트 렌더링을 위해 원하는 패딩을 설정할 수 있습니다.
...
아니요, **이메일 템플릿에서** 이메일을 다시 만들어 저장해야 합니다.

### 드래그 앤 드롭 편집기에 이메일 첨부파일을 추가할 수 있나요?

아니요, 드래그 앤 드롭 편집기는 이메일에 첨부파일 추가를 지원하지 않습니다.
