...
{% alert tip %}
모든 메시징 채널에서 링크 유형을 비교하고 AASA 파일이 필요한 시기에 대한 안내는 [iOS 딥링킹 가이드]({{site.baseurl}}/developer_guide/push_notifications/ios_deep_linking_guide)를 참조하십시오.
{% endalert %}
...
#### Step 1c: Xcode 프로젝트에서 연결된 도메인을 켭니다 {#step-1c}
...
- `paths`: 포함되거나 제외되는 경로를 지정하는 문자열 배열. 경로를 비활성화하려면 경로 앞에 `NOT`을 사용할 수 있습니다. 이 예제에서는 이 경로의 모든 링크가 앱을 여는 대신 웹으로 이동합니다. 디렉토리의 모든 경로를 활성화하려면 `*`를 와일드카드로 사용하고, 단일 문자를 일치시키려면 `?`를 사용하십시오(예: /archives/201?/는 2010-2019의 모든 숫자와 일치).
...
귀하의 `AndroidManifest.xml` 파일에서 애플리케이션 요소 내에 메타 데이터 요소를 추가하십시오. 메타데이터 요소는 `android:name` 속성이 "asset_statements"이고, 문자열 배열이 포함된 리소스 파일을 가리키는 `android:resource` 속성을 가져야 합니다.
...
{% alert note %}
클릭-추적 링크는 일반적으로 이메일 온보딩의 일부로 설정됩니다. 고객 온보딩 중에 완료되지 않았다면, 도움을 받으려면 계정 매니저에게 문의하십시오.
{% endalert %}
...
![텍스트 링크에 대한 커스텀 속성.]({% image_buster /assets/img/text_click_tracking_off.png %}){: style="max-width:60%;"}
...
![버튼에 대한 커스텀 속성.]({% image_buster /assets/img/button_click_tracking_off.png %}){: style="max-width:60%;"}
