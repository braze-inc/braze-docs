...
Braze에서 DNS 레코드를 제공한 후 DNS 또는 IT 팀이 가능한 한 빨리 추가하세요. 도메인 확인은 시간 제한이 있으며 레코드가 너무 늦게 추가되면 나중에 DNS 레코드가 올바르게 확인되더라도 확인이 실패할 수 있습니다. DNS 레코드가 올바르게 표시되지만 확인에 실패한 경우, Braze 온보딩 또는 지원팀에 문의하여 확인을 다시 시작하세요.
...
확인에 실패했지만 DNS 레코드가 정확하다고 생각되면 Braze 지원팀에 문의하여 도움을 받으세요.
