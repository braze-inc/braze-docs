...
Braze는 귀하의 IP와 도메인을 구성할 때 SPF 레코드를 설정합니다. 저희가 제공하는 DNS 레코드를 추가하는 것 외에는 추가적인 조치가 필요하지 않습니다.
...
Braze는 IP 주소와 도메인을 설정할 때 DKIM 레코드를 구성합니다. 저희가 제공하는 DNS 레코드를 추가하는 것 외에는 추가적인 조치가 필요하지 않습니다.

### 도메인 기반 메시지 인증, 보고 및 적합성(DMARC)

[도메인 기반 메시지 인증, 보고](https://dmarc.org/) 및[& 준수(DMARC)](https://dmarc.org/)는 이메일 발신자가 자신의 메일 합법성을 증명하기 위한 이메일 인증 프로토콜로, 수신자의 신뢰를 확보하고 메일 수락을 촉진합니다. 이메일 발신자는 DMARC를 통해 발신자 정책 프레임워크(SPF) 또는 도메인 키 식별 메일(DKIM)을 사용하여 인증되지 않은 이메일을 처리하는 방법을 지정할 수 있습니다. 이는 SPF 및 DKIM 검사가 모두 통과되었는지 확인하여 이루어집니다. 

발신자는 서명 또는 인증 검사에 실패한 메일을 처리하는 방법을 메일박스 제공자에게 지시합니다. 실패는 스푸핑을 나타낼 수 있습니다. 서비스 제공자에게 실패한 메일을 거부하거나 격리하도록 지시하고 자동화된 보고서를 전송하도록 할 수 있습니다. 이를 통해 제공업체는 스패머를 식별하고, 악성 이메일을 차단하며, 오탐을 최소화하고, 인증 보고의 투명성을 개선할 수 있습니다.
...
루트 도메인에 DMARC 정책을 설정하여 모든 하위 도메인에 적용되도록 하십시오. 이는 현재 및 향후 하위 도메인에 대한 추가 설정을 피합니다. 다음 정책 중 하나를 설정할 수 있습니다:
