...
> 이 참조 문서에서는 이메일 주소의 로컬 및 호스트 부분 유효성 검사 규칙을 다룹니다. 검증은 대시보드 이메일 주소, 최종 사용자 이메일 주소(고객), 및 이메일 메시지의 발신자 및 회신 주소에 사용됩니다.
...
이 검증은 구문 검사이며, 검증 서비스가 아닙니다. 이 과정의 목표 중 하나는 이메일 주소의 로컬 부분에서 국제 문자를 지원하는 것입니다(예: UTF-8).

Braze는 이메일 주소의 로컬 부분과 호스트 부분 모두에 대해 구문을 검증합니다. 로컬 부분은 앳 기호(@) 이전의 모든 것이며, 호스트 부분은 이후의 모든 것입니다. 로컬 부분은 마침표(.)를 제외한 모든 허용된 문자로 시작하고 끝날 수 있습니다. 이 과정은 도메인에 유효한 MX 서버가 있는지 또는 해당 도메인에 사용자가 존재하는지를 고려하지 않습니다.
