...
Braze는 사용자가 Google에 등록하거나 필요한 보안 요건을 충족하는 것에 대해 책임을 지지 않습니다. 이메일용 AMP는 스팍포스트와 센드그리드에서만 사용할 수 있습니다.
...
!['동적 이메일 사용' 확인란이 선택된 Gmail 설정의 예입니다.]({% image_buster /assets/img/dynamic-content.png %})
...
| Component | 설명 |
|---------|--------------|
| [데이터 바인딩 & 표현식](https://amp.dev/documentation/components/amp-anim?format=email) <br><br> `amp-bind`| 데이터 바인딩과 자바스크립트 유사 표현식을 통해 AMP 페이지에 커스텀 상태 저장형 상호 작용을 추가합니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
사용자는 이 구성요소를 사용하여 필드 값을 제출하거나 지울 수 있습니다. 또한 이메일 설정에 따라 사용자에게 설문 제출이 성공했는지 여부와 같은 추가 프롬프트를 제공하거나 사용자로부터 설문 결과(예: 투표 캠페인)를 보여주는 응답을 렌더링할 수 있습니다.
...
긴 교육 기사나 개인화된 추천을 보내는 경우, 시청자가 콘텐츠 개요를 한눈에 살펴보고 원하는 섹션이나 특정 제품 추천으로 이동하여 자세한 내용을 확인할 수 있습니다. 이것은 섹션에 몇 문장만 있어도 스크롤이 필요한 모바일 사용자에게 특히 유용할 수 있습니다.
{% endtab %}
{% tab Image Heavy Emails %}

소매 브랜드와 같은 많은 전문 사진이 포함된 이메일을 보내는 경향이 있다면, 사용자가 매력적으로 느끼는 이미지와 상호작용할 수 있도록 하는 `<amp-image-lightbox>` 구성 요소를 사용할 수 있습니다. 사용자가 이미지를 클릭하면 이 컴포넌트는 메시지 중앙에 이미지를 표시하여 라이트박스 효과를 만듭니다. 
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Opens' %} AMP 이메일의 경우, 이는 HTML 및 일반 텍스트 버전의 총 열람 수입니다.</td>
        </tr>
        <tr>
            <td class="no-split">총 클릭 수</td>
            <td class="no-split">{% multi_lang_include analytics/metrics.md metric='Total Clicks' %} AMP 이메일의 경우, 이는 HTML 및 일반 텍스트 버전의 총 클릭 수입니다.</td>
        </tr>
        <tr>
...
총 클릭 수와 고유 클릭 수에는 AMP 메시지에서 발생한 클릭은 포함되지 않습니다(HTML 및 일반 텍스트만 해당). AMP 특정 클릭은 *amp_click* 메트릭에 귀속됩니다.
...
이러한 조건 중 어느 것도 오류를 유발하지 않는 경우, [지원]({{site.baseurl}}/support_contact/)에 문의하십시오.
