...
사용자는 이 구성요소를 사용하여 필드 값을 제출하거나 지울 수 있습니다. 또한 이메일 설정에 따라 사용자에게 추가 프롬프트를 제공할 수 있습니다. 예를 들어 설문 제출이 성공했는지 여부나 사용자 응답을 렌더링하여 설문 결과(예: 투표 캠페인)를 보여줄 수 있습니다.
...
긴 교육 기사나 개인화된 추천을 보내는 경우, 시청자가 콘텐츠 개요를 한눈에 살펴보고 원하는 섹션이나 특정 제품 추천으로 이동하여 자세한 내용을 확인할 수 있습니다. 이것은 특히 몇 문장만으로도 스크롤이 필요한 모바일 사용자에게 유용할 수 있습니다.
{% endtab %}
{% tab Image Heavy Emails %}

소매 브랜드와 같은 많은 전문 사진이 포함된 이메일을 보내는 경향이 있다면, 사용자가 매력적으로 느끼는 이미지와 상호작용할 수 있는 `<amp-image-lightbox>` 구성 요소를 사용할 수 있습니다. 사용자가 이미지를 클릭하면 이 컴포넌트는 메시지 중앙에 이미지를 표시하여 라이트박스 효과를 만듭니다. 
