...
| IP 풀 | 이는 서로 다른 유형의 이메일(예: "프로모션" 및 "거래")의 평판을 분리하여 한 이메일의 평판이 다른 이메일에 영향을 미치는 것을 방지하고 더 높은 전달 가능성을 지원하는 데 사용되는 선택적 구성입니다. | 온보딩 팀에서 풀을 설정해 드립니다. 그런 다음 이메일을 작성할 때 **대상 사용자** 페이지의 **IP 풀** 드롭다운에서 이메일의 IP 풀을 선택합니다.|
