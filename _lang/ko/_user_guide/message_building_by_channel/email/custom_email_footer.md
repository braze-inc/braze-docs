...
구독 취소 링크 태그 `{{${set_user_to_unsubscribed_url}}}`{% endraw %} 없이 사용자 정의 바닥글 {% raw %}`{{${email_footer}}}`가 있는 템플릿을 사용할 때 매우 주의해야 합니다. 경고가 표시되지만, 구독 취소 링크가 있는지 없는지에 따라 이메일을 보낼지는 귀하의 선택입니다.
