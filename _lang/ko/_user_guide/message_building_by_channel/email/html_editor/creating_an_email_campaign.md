---
nav_title: 이메일 만들기
article_title: 사용자 정의 HTML로 이메일 만들기
page_order: 1
...
{% alert tip %}
이메일 캠페인을 처음 만드는 경우, Braze 학습 과정을 확인하는 것을 강력히 권장합니다:<br><br>
...
단일, 간단한 메시징을 위해 캠페인을 사용하세요. 다단계 사용자 여정을 위해 캔버스를 사용하세요.
...
Braze는 이메일 캠페인을 만들 때 [드래그 앤 드롭 편집기와]({{site.baseurl}}/dnd/) 표준 HTML 편집기 등 두 가지 편집 환경을 제공합니다. 원하는 편집 경험에 적합한 타일을 선택하세요. 
...
{% alert tip %}
이메일 캠페인당 하나의 편집 환경을 선택하는 것이 좋습니다. 예를 들어, 단일 이메일 캠페인에서 **HTML 클래식** 또는 **블록 편집기** 중 하나를 선택하고 편집기를 전환하지 마세요.
{% endalert %}
...
Braze는 HTML 버전에서 평문 버전으로 자동으로 업데이트하며, 평문에 대한 편집을 감지할 때까지 계속합니다. Braze가 편집을 감지한 후에는 의도적인 변경을 했다고 가정하고 평문 업데이트를 중지합니다. 자동 동기화를 복원하려면 **평문**으로 가서 **HTML에서 재생성**를 선택하세요(평문이 동기화되지 않을 때만 표시됨).
...
{% alert important %}
Braze는 속성으로 참조된 HTML 이벤트 핸들러를 자동으로 제거합니다. 이것은 HTML을 수정하므로, 작업을 마친 후 이메일을 다시 확인하세요. [HTML 핸들러](https://www.w3schools.com/tags/ref_eventattributes.asp)에 대해 자세히 알아보세요.
{% endalert %}
...
![이메일 작성기 본문 탭에 위치한 AI 카피라이터 버튼을 실행하세요.]({% image_buster /assets/img/ai_copywriter/ai_copywriter_email.png %}){: style="max-width:80%"}
{% endalert %}
...
이메일 메시지를 디자인하고 작성한 후, **발송 설정**에 발송 정보를 추가하세요.
...
오른쪽 패널의 미리보기가 추가한 전송 정보로 채워집니다. 이 정보는 **설정** > **이메일 기본 설정** > **발송 구성**으로 가서 업데이트할 수도 있습니다.

#### Advanced

**발송 설정** > **고급**에서 인라인 CSS를 켜고 이메일 헤더 및 이메일 추가에 대한 개인화를 추가하여 다른 이메일 서비스 제공업체에 추가 데이터를 전송하세요.

##### 이메일 헤더

이메일 헤더를 추가하려면 **새 헤더 추가**를 선택합니다. 이메일 헤더에는 전송되는 이메일에 대한 정보가 포함되어 있습니다. 이 [키-값 쌍]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/key_value_pairs/)에는 일반적으로 발신자, 수신자, 인증 프로토콜 및 라우팅 정보가 포함됩니다. Braze는 이메일이 받은편지함 제공업체에 도달하기 위해 RFC에서 요구하는 헤더 정보를 자동으로 추가합니다.
...
이메일 작성을 마친 후, 발송하기 전에 테스트하세요. From the bottom of the overview screen, select **Preview and Test**. 
...
최종 확인을 할 준비가 되면 **테스트 전송**을 선택하고 테스트 메시지를 자신이나 테스터 그룹에 보내 이메일이 장치와 클라이언트에서 제대로 표시되는지 확인하세요.

![이메일 작성 시 테스트 보내기 옵션과 이메일 미리보기 예시를 확인하세요.]({% image_buster /assets/img_archive/newEmailTest.png %})

이메일에 문제가 있거나 변경 사항을 원하면 **이메일 편집**을 선택하여 편집기로 돌아가세요.
...
{% tabs %}
{% tab Campaign %}
다음으로, 나머지 캠페인을 구축하십시오. Braze 도구를 사용하여 이메일 캠페인을 구축하는 방법에 대한 자세한 내용은 다음 섹션을 참조하세요.

#### 배송 일정 또는 트리거 선택

예정된 시간, 작업 또는 API 트리거에 따라 이메일을 전송하세요. 자세한 내용은 [캠페인 일정 잡기]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/)을 참조하세요.
...
다음으로, [대상 사용자]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/)를 선택하여 세그먼트 또는 필터를 선택하세요. Braze는 이메일을 통해 도달할 수 있는 사용자 수를 포함하여 세그먼트 인구의 라이브 미리보기를 보여줍니다. 정확한 세그먼트 구성원 자격은 전송 직전에 계산됩니다.
...
{% alert note %}
새 이메일 캠페인을 만들 때 대조군은 기본적으로 20%로 설정되며 캠페인에 필요에 따라 조정하거나 제거할 수 있습니다.
{% endalert %}
...
사용자가 지정된 작업을 수행하면 Braze가 전환을 계산하는 최대 30일의 기간을 허용할 수 있습니다. Braze가 자동으로 열기 및 클릭을 추적하는 동안, [지능형 선택]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/)을 사용하기 위해 전환 이벤트를 열기 또는 클릭으로 설정할 수 있습니다.
{% endtab %}
...
마지막 섹션은 설계한 캠페인을 요약합니다. 모든 관련 세부 정보를 확인하고 **캠페인 시작**을 선택하세요. 

이메일 캠페인의 결과에 액세스하는 방법을 알아보려면 [이메일 보고]({{site.baseurl}}/user_guide/message_building_by_channel/email/reporting_and_analytics/email_reporting/)를 확인하세요.
