...
단일하고 간단한 메시징을 위해 캠페인을 사용하세요. 다단계 사용자 여정에 캔버스를 사용하세요.
...
Braze는 HTML 버전에서 일반 텍스트에 대한 편집이 감지될 때까지 일반 텍스트 버전을 자동으로 업데이트합니다. Braze는 편집이 감지되면 사용자가 의도적으로 변경한 것으로 간주하여 일반 텍스트 업데이트를 중지합니다. 자동 동기화를 복원하려면 **일반 텍스트로** 이동하여 **HTML에서 재생성을** 선택합니다(일반 텍스트가 동기화되지 않을 때만 표시됨).
...
{% alert important %}
Braze는 속성으로 참조된 HTML 이벤트 핸들러를 자동으로 제거합니다. 이렇게 하면 HTML이 수정되므로 완료한 후 이메일을 다시 확인하세요. [HTML 핸들러](https://www.w3schools.com/tags/ref_eventattributes.asp)에 대해 자세히 알아보세요.
{% endalert %}
...
이메일 메시지 디자인 및 구축을 완료한 후에는 **전송 설정에서** 전송 정보를 추가합니다.
...
**전송 설정** > **고급에서** 인라인 CSS를 켜고 이메일 헤더 및 이메일 추가에 대한 개인화를 추가하여 다른 이메일 서비스 제공업체에 추가 데이터를 다시 전송합니다.

##### 이메일 헤더

이메일 헤더를 추가하려면 **새 헤더 추가**를 선택합니다. 이메일 헤더에는 전송되는 이메일에 대한 정보가 포함되어 있습니다. 이러한 [키-값 페어에는]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/key_value_pairs/) 일반적으로 발신자, 수신자, 인증 프로토콜 및 라우팅 정보가 포함됩니다. Braze는 이메일이 받은편지함 제공업체에 도달할 수 있도록 RFC 필수 헤더 정보를 자동으로 추가합니다.
...
이메일 작성을 완료한 후에는 보내기 전에 테스트하세요. From the bottom of the overview screen, select **Preview and Test**. 
...
최종 확인 준비가 완료되면 **테스트 보내기를** 선택하고 본인 또는 테스터 그룹에게 테스트 메시지를 전송하여 기기와 클라이언트에서 이메일이 제대로 표시되는지 확인합니다.
...
예약된 시간, 작업 또는 API 트리거에 따라 이메일을 전송하세요. 자세한 내용은 [캠페인 일정 잡기]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/)을 참조하세요.
...
그런 다음 세그먼트 또는 필터를 선택하여 [사용자를 타겟팅합니다]({{site.baseurl}}/user_guide/engagement_tools/messaging_fundamentals/targeting_users/). Braze는 이메일을 통해 도달할 수 있는 사용자 수를 포함하여 세그먼트 인구에 대한 라이브 프리뷰를 보여줍니다. 정확한 세그먼트 멤버십은 전송 직전에 계산됩니다.
...
{% alert note %}
새 이메일 캠페인을 만들 때 대조군은 기본값이 20%로 설정되며 캠페인에 따라 필요에 따라 조정하거나 제거할 수 있습니다.
{% endalert %}
...
사용자가 지정된 작업을 수행할 경우 최대 30일의 기간을 허용하여 Braze에서 전환을 계산할 수 있습니다. Braze는 열기 및 클릭을 자동으로 추적하지만, 전환 이벤트를 열기 또는 클릭으로 설정하여 [지능형 선택을]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_selection/) 사용할 수 있습니다.
{% endtab %}
...
마지막 섹션에는 디자인한 캠페인이 요약되어 있습니다. 모든 관련 세부 정보를 확인하고 **캠페인 시작을** 선택합니다. 
