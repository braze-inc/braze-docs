...
푸시 메시지는 고객의 전화나 브라우저로 직접 전달되는 침입적인 유형의 메시징이기 때문에, 앱과 사이트를 통해 푸시 메시지를 전송하기 위한 가이드라인이 있습니다.
