...
아니요. S3 인증정보를 추가하지 않으면, 내보내기가 Braze에 속하는 S3 버킷에 나타납니다.

### 대시보드에서 S3 인증정보를 설정했지만 "기본값 데이터 내보내기 대상"을 선택하지 않으면 어떻게 됩니까?

**이 기본값 데이터 내보내기 대상** 체크박스는 내보내기가 S3 또는 Azure로 가는지에 영향을 미칩니다. 두 가지 모두에 대한 자격 증명을 추가했다고 가정합니다.
...
### 왜 세그먼트를 통해 REST API로 사용자를 내보낼 때 중복이 보이나요?

이것은 데이터베이스 공급자의 기본 아키텍처로 인해 발생하는 매우 드문 사건입니다. 중복 항목은 매주 정리되지만, 대부분의 주에는 중복 항목이 정리되지 않습니다.
