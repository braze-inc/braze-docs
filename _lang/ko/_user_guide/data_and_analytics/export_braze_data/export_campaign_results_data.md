...
> 대시보드의 **캠페인** 페이지에서 보고 싶은 캠페인을 선택하고, 내보낼 수 있는 역사적 성능 그래프까지 스크롤 다운하세요.<br><br>이 페이지에서는 단일, 다중 채널 및 다변량 캠페인에서 캠페인 결과 데이터를 내보내는 방법과 수신자로부터 사용자 데이터를 내보내는 방법을 다룹니다.

## 멀티채널 캠페인

멀티채널 캠페인의 경우, 내보낼 수 있는 데이터는 사용한 메시징 채널에 따라 다릅니다. 다음은 iOS 푸시, Android 푸시, 이메일 및 인앱 메시지를 사용한 캠페인에서 내보낼 수 있는 모든 데이터의 목록입니다.

- 날짜별로 보낸 메시지
    - 보낸 총 메시지 수
    - 캠페인의 채널을 통해 전송된 메시지(푸시, 이메일 및 인앱 메시지를 포함할 수 있음)
- 날짜별 이메일 메시지 인게이지먼트
    - 전달된 이메일 수
    - 전송된 이메일 수
    - 열람 이메일 수
...
다변량 캠페인의 경우, 단일 메시징 채널을 사용하는 경우, 특정 메시징 채널의 분석에서 각 배리언트가 시간에 따라 어떻게 수행되었는지를 보여주는 데이터를 내보낼 수 있습니다. 이 데이터를 통계별로 그룹화하거나 메시지 배리언트별로 그룹화하여 볼 수 있습니다.
...
내보낸 파일에는 세그먼트][40]에 대한 사용자 데이터를 내보낼 때 포함되는 동일한 사용자 데이터 필드가 포함되어 있습니다. 이러한 데이터 필드 외에도 "모든 수신자 데이터 내보내기"를 선택하면 내보낸 파일에 각 사용자에 대한 다음 데이터도 포함됩니다.
...
{% alert tip %}
CSV 및 API 내보내기와 관련된 도움말은 [문제 해결]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/export_troubleshooting/)를 확인하세요.
{% endalert %}
