...
{% alert note %}
이전 탐색을 사용하는 경우 **앱 설정은** **설정**이며 **설정 관리** 아래에 있습니다.
{% endalert %}
...
iOS 애플리케이션에 대한 제거 추적을 구성하려면 [유틸리티 방법][iOS 설명서]를 사용하세요. Android 애플리케이션의 경우 [`BrazeNotificationUtils.isUninstallTrackingPush()`][8]를 사용합니다. 제거 추적 또는 일반 푸시 캠페인 전송을 통해 제거가 감지되면, Braze는 사용자의 제거 예상 시간을 기록합니다. 이 시간은 사용자 프로필에 표준 속성으로 저장되며 윈백 캠페인의 사용자 세그먼트를 정의하는 데 사용할 수 있습니다.
...
제거에 대한 일일 통계는 **홈** 페이지에서 확인할 수 있습니다. 그래프는 Braze가 제공하는 다른 통계와 마찬가지로 앱과 세그먼트별로 세분화할 수 있습니다. **실적 개요** 섹션에서 날짜 범위와 원하는 경우 앱을 선택합니다. 그런 다음 **성능/성과** 그래프로 스크롤하여 다음을 수행하십시오:
...
### 왜 앱 삭제 수가 APNs에 있는 것과 다른가요?

차이는 예상됩니다. APNs는 이러한 토큰에 대해 모호한 일정으로 410 상태를 반환하기 시작할 것입니다.
