...
2. Databricks 계정에서 Braze와 동기화하려는 테이블 또는 뷰를 설정하십시오.   
3. Braze 대시보드에서 새 통합을 생성합니다.  
4. 통합을 테스트하고 동기화를 시작하십시오.
...
- **사용자 식별자 열** \- 테이블에는 하나 이상의 사용자 식별자 열이 포함될 수 있습니다. 각 행에는 하나의 식별자( `external_id`, `alias_name`과 `alias_label`의 조합 또는 `braze_id` 중 하나)만 포함되어야 합니다. 소스 테이블에는 하나, 둘 또는 세 가지 식별자 유형에 대한 열이 있을 수 있습니다. 
    - `EXTERNAL_ID` - 업데이트하려는 사용자를 식별합니다. 이것은 Braze에서 사용된 `external_id` 값과 일치해야 합니다. 
    - `ALIAS_NAME` 및 `ALIAS_LABEL` \- 이 두 열은 사용자 별칭 객체를 생성합니다. `alias_name`은(는) 고유한 식별자여야 하며, `alias_label`은(는) 별칭의 유형을 지정합니다. 사용자는 여러 레이블이 있는 여러 별칭을 가질 수 있지만 `alias_label`당 하나의 `alias_name`만 가질 수 있습니다.
    - `BRAZE_ID` - Braze 사용자 식별자. 이것은 Braze 소프트웨어 개발 키트에 의해 생성되었으며, 새로운 사용자는 Braze ID를 통해 클라우드 데이터 수집을 사용하여 생성될 수 없습니다. 새 사용자를 만들려면 외부 사용자 ID 또는 사용자 별칭을 지정하십시오.
    ...
    - `PHONE` - 사용자의 이메일 주소. 동일한 전화번호를 가진 여러 프로필이 존재하는 경우, 가장 최근에 업데이트된 프로필이 업데이트에 우선됩니다. 
...
{% alert note %}
창고에는 **자동 재개** 플래그가 켜져 있어야 합니다. 그렇지 않으면, 쿼리를 실행할 때 이를 켜기 위해 창고에 대한 추가 `OPERATE` 권한을 Braze에 부여해야 합니다.
{% endalert %}
...
| 예를 들어 `US-01`, `US-02`, `US-03`, `US-04`, `US-05`, `US-06`, `US-07` | 인스턴스 `EU-01` 및 `EU-02`의 경우 |
|---|---|
...
선택적으로 소스 테이블을 보유할 새 데이터베이스 및 스키마를 설정하십시오
```json
CREATE DATABASE BRAZE_CLOUD_PRODUCTION;
CREATE SCHEMA BRAZE_CLOUD_PRODUCTION.INGESTION;
```
...
- `UPDATED_AT` - 이 행이 테이블에 업데이트되거나 추가된 시간. 우리는 마지막 동기화 이후에 추가되거나 업데이트된 행만 동기화할 것입니다.
- 사용자 식별자 열. 귀하의 테이블에는 하나 이상의 사용자 식별자 열이 포함될 수 있습니다. 각 행에는 하나의 식별자( `external_id`, `alias_name`와 `alias_label`의 조합 또는 `braze_id`)만 포함되어야 합니다. 소스 테이블에는 하나, 둘 또는 세 가지 식별자 유형에 대한 열이 있을 수 있습니다. 
    - `EXTERNAL_ID` - 업데이트하려는 사용자를 식별합니다. 이것은 Braze에서 사용된 `external_id` 값과 일치해야 합니다. 
    - `ALIAS_NAME` 및 `ALIAS_LABEL` \- 이 두 열은 사용자 별칭 객체를 생성합니다. `alias_name`은(는) 고유한 식별자여야 하며, `alias_label`은(는) 별칭의 유형을 지정합니다. 사용자는 여러 레이블이 있는 여러 별칭을 가질 수 있지만 `alias_label`당 하나의 `alias_name`만 가질 수 있습니다.
    - `BRAZE_ID` - Braze 사용자 식별자. 이것은 Braze 소프트웨어 개발 키트에 의해 생성되었으며, 새로운 사용자는 Braze ID를 통해 클라우드 데이터 수집을 사용하여 생성될 수 없습니다. 새 사용자를 만들려면 외부 사용자 ID 또는 사용자 별칭을 지정하십시오.
    - `EMAIL` - 사용자의 이메일 주소. 동일한 이메일 주소를 가진 여러 프로필이 존재하는 경우, 가장 최근에 업데이트된 프로필이 업데이트 우선순위가 됩니다. 이메일과 전화번호를 모두 포함하면 이메일을 기본 식별자로 사용하겠습니다.
...
다음 필드를 사용하여 CDI 통합에 사용할 하나 이상의 테이블을 만드십시오:
...
3. 이 토큰을 식별하는 데 도움이 되는 "Braze CDI"와 같은 주석을 입력하고, Lifetime (days) 상자를 비워 두어 토큰의 수명을 무수명으로 변경하십시오.
4. **생성**을 선택하십시오.
5. 표시된 토큰을 복사한 다음 **완료**를 선택합니다.

자격 증명 생성 단계에서 Braze 대시보드에 입력할 때까지 토큰을 안전한 장소에 보관하십시오.
...
{% alert note %}
[older navigation]({{site.baseurl}}/navigation)을 사용 중인 경우 **기술 파트너**로 이동하십시오.
{% endalert %}

#### 1단계: Snowflake 연결 정보 및 소스 테이블 추가

Snowflake 데이터 웨어하우스 및 소스 테이블에 대한 정보를 입력한 후 다음 단계로 진행하십시오.
...
추가 정보는 [Snowflake 설명서](https://docs.snowflake.com/en/user-guide/key-pair-auth.html)를 참조하십시오. 키를 언제든지 회전시키고 싶다면, 저희가 새로운 키 쌍을 생성하여 새로운 공개 키를 제공해 드릴 수 있습니다.
...
데이터브릭스 데이터 웨어하우스 및 소스 테이블에 대한 정보를 입력한 다음, 다음 단계로 진행하십시오.
...
활성화되면 동기화가 설정 중 구성된 일정에 따라 실행됩니다. 정상적인 테스트 일정 외에 동기화를 실행하거나 최신 데이터를 가져오려면 **지금 동기화**를 선택하십시오. 이 실행은 정기적으로 예정된 향후 동기화에 영향을 미치지 않습니다.
