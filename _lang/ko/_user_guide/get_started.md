...
{% alert tip %}
동료 함께 플랫폼을 탐색할 수 있도록 [Braze에 동료를 추가하세요]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/).
{% endalert %}
...
- **이메일 전송량:** 이메일을 보내려면 IP를 워밍업해야 합니다. [IP warming]({{site.baseurl}}/user_guide/message_building_by_channel/email/email_setup/ip_warming/) is the process of building sender reputation with your newly assigned IP addresses. 하루에 200-300만 통 미만의 이메일을 보내는 경우, IP 워밍은 30일 이내에 완료되어야 합니다. 최대 전송량을 염두에 두세요. 평소에는 하루에 2백만 개의 이메일을 보내지만 계절에 따라 7백만 개의 이메일을 보낼 계획이라면 이 '피크' 전송에 대비해야 합니다. 대량 발송자는 여러 IP를 사용하여 워밍업 프로세스의 속도를 높일 수 있습니다.
