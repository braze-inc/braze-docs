...
### 2단계: Set up your webhook template
...
![액션 기반이며 트리거 액션이 '이메일 주소 추가'인 캠페인을 만드는 2단계입니다.]({% image_buster /assets/img/b2b/salesforce_create_campaign.png %}){: style="max-width:70%;"}
...
![`lead_stage` 은 "리드"와 같고 `lead_score` 은 "50"보다 큰 필터가 있는 "MQL 임계값" 대상 경로 그룹입니다.]({% image_buster /assets/img/b2b/salesforce_check_mql.png %}){: style="max-width:70%;"}

{: start="3" }
3\. 사용자의 `lead_stage` 속성 값을 "MQL"로 업데이트하는 **사용자 업데이트** 단계를 추가합니다.

![`lead_stage` 속성이 "MQL" 값을 갖도록 업데이트하는 "MQL로 업데이트" 사용자 업데이트 단계입니다.]({% image_buster /assets/img/b2b/salesforce_update_mql.png %}){: style="max-width:70%;"}

{: start="4" }
4\. 새 MQL 단계로 Salesforce를 업데이트하는 웹후크 단계를 추가합니다.

![완료된 세부 정보가 포함된 'Salesforce 업데이트' 웹훅 단계입니다.]({% image_buster /assets/img/b2b/salesforce_webhook.png %}){: style="max-width:70%;"}

이제 캔버스 흐름이 MQL 임계값을 넘은 사용자를 업데이트합니다!

![사용자가 MQL 임계값을 통과하는지 확인하고, 통과할 경우 Salesforce를 업데이트하는 Canvas 사용자 업데이트 단계입니다.]({% image_buster /assets/img/b2b/salesforce_canvas.png %}){: style="max-width:50%;"}
