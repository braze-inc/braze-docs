...
앱에 대해 제거 추적을 활성화하면, Braze는 24시간 동안 세션을 기록하지 않거나 푸시를 받지 않은 사용자에게 매일 밤 백그라운드 푸시 메시지를 보냅니다.
...
Braze는 사용자의 디바이스로 전송된 푸시 메시지가 Firebase 클라우드 메시징(FCM) 또는 Apple 푸시 알림 서비스(APN)로부터 앱이 더 이상 설치되지 않았다는 신호를 반환하는 시점을 관찰하여 제거를 추적합니다. 앱에 대해 글로벌 제거 추적을 활성화하면, Braze는 사용자가 제거했는지 감지하기 위해 매일 조용한 푸시 메시지를 보냅니다. Braze는 이 "조용한" 푸시를 모든 사용자에게 보냅니다(사용자가 앱 설정에서 조용한 푸시를 비활성화하지 않은 경우); 푸시는 사용자에게 표시되지 않습니다. Braze가 사용자가 제거한 것을 감지하면, 우리는:

* 앱의 총 제거 횟수를 1씩 늘립니다.
* 지난 24시간 동안 사용자가 성공적으로 수신한 모든 캠페인의 제거 횟수를 하나씩 늘립니다.
* 사용자가 24시간 동안 세 개의 캠페인을 수신한 후 삭제하면 세 개의 캠페인 모두에 대한 '삭제' 횟수가 증가합니다.

FCM과 APNs는 제거 추적에 제한을 둡니다. Braze는 FCM 또는 APNs가 사용자가 제거했다고 알려줄 때만 제거 수를 증가시키지만, 이러한 제3자 시스템은 언제든지 제거를 알릴 수 있습니다. 정확한 통계보다는 방향성 추세를 감지하기 위해 제거 추적을 사용하십시오.
...
{% alert note %}
개인정보 보호 이유로, Braze의 푸시 제공자는 불규칙한 간격으로 토큰을 취소할 수 있으며, 이는 특정 기간 동안 제거 수가 급증할 수 있음을 의미합니다.<br><br>이러한 변화를 검증하기 위해, 직접 푸시 열람율과 같은 사용자 행동 지표와 함께 제거 추적을 모니터링하십시오. 제거가 급증하지만 직접 푸시 열람이 안정적이라면, 이 급증은 실제 사용자 행동이 아니라 파트너가 오래된 토큰을 취소한 것을 반영할 가능성이 높습니다.
{% endalert %}
