...
{% alert note %}
개인정보 보호를 위해 Braze의 푸시 제공업체는 불규칙한 간격으로 토큰을 해지할 수 있으므로 특정 기간에 제거 횟수가 급증할 수 있습니다.<br><br>이러한 변경 사항을 확인하려면 직접 푸시 열람율과 같은 사용자 행동 측정기준과 함께 제거 추적을 모니터링하세요. 제거가 급격히 증가하지만 직접 푸시 오픈이 안정적으로 유지되는 경우, 이는 실제 사용자 행동보다는 파트너가 기존 토큰을 해지하는 것을 반영하는 것일 수 있습니다.
{% endalert %}
