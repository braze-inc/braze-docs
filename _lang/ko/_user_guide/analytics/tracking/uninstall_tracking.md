...
{% alert note %}
Braze 대시보드에서 추적 제거를 옵트인해야 합니다. 이 기능은 iOS, Android 및 Fire OS의 앱에서 사용할 수 있습니다.
{% endalert %}
...
앱에 대한 제거 추적을 켜면 24시간 동안 세션을 녹화하지 않았거나 푸시를 받지 않은 사용자에게 Braze가 매일 밤 백그라운드 푸시 메시지를 보냅니다.
...
Braze는 사용자의 디바이스로 전송된 푸시 메시지가 Firebase 클라우드 메시징(FCM) 또는 Apple 푸시 알림 서비스(APN)로부터 앱이 더 이상 설치되지 않았다는 신호를 반환하는 시점을 관찰하여 제거를 추적합니다. 앱에 대해 글로벌 제거 추적을 사용 설정하면 Braze는 사용자에게 매일 무음 푸시 메시지를 보내 사용자가 앱을 제거했는지 여부를 감지합니다. 사용자가 앱 설정에서 무음 푸시를 비활성화하지 않은 경우 Braze는 모든 사용자에게 이 '무음' 푸시를 보내며, 푸시는 사용자에게 표시되지 않습니다. 사용자가 삭제한 것을 Braze에서 감지하면 당사는

* 앱의 총 제거 횟수를 1씩 늘립니다.
* 지난 24시간 동안 사용자가 성공적으로 수신한 모든 캠페인의 제거 횟수를 하나씩 늘립니다.
* 사용자가 24시간 동안 세 개의 캠페인을 수신한 후 삭제하면 세 개의 캠페인 모두에 대한 '삭제' 횟수가 증가합니다.

FCM 및 APN은 제거 추적에 제한을 두고 있습니다. Braze는 FCM 또는 APN이 사용자가 제거했다고 알려줄 때만 제거 횟수를 증가시키지만, 이러한 타사 시스템은 언제든지 제거를 알릴 수 있습니다. 제거 추적을 사용하면 정확한 통계보다는 방향성 추세를 파악할 수 있습니다.
