...
| Endpoint | API key | 응답 코드 | 임계값 조건 | 임계값 볼륨 | 범위 |
| --- | --- | --- | --- | --- | --- |
...
| Endpoint | API key | 응답 코드 | 임계값 조건 | 임계값 볼륨 | 범위 |
| --- | --- | --- | --- | --- | --- |
| {::nomarkdown}<ul><li><code>/campaigns/trigger/send</code></li><li><code>/canvas/trigger/send</code></li><li><code>/messages/send</code></li></ul>{:/} | 모든 API 키 | `4XX` and `5XX` | 다음보다 크거나 같음 | 1 | 1시간 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 .reset-td-br-5 .reset-td-br-6 role="presentation" }
{% endtab %}

{% tab partner integrations %}
다음 알림 구성을 사용하여 파트너 통합이 Braze로 데이터 전송을 중단할 때 알림을 받도록 설정하세요.
