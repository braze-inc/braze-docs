...
- **계산 타이밍**: 30일 스냅샷으로 매일 12:05 UTC에 한 번씩 계산되며, 카운트는 소급하여 변경되지 않습니다.
- **익명 프로필**: 하나 이상의 세션이 기록된 **경우에만** 계산합니다.
- **식별된 프로필**: 존재하면 자동으로 계산됩니다.
- **고아 프로필**: 다른 사용자에게 병합된 중복은 **계산되지 않습니다**.
- **CSV 업로드**: CSV로 업로드한 사용자는 `date_of_first_session` 또는 `date_of_last_session` 을 제공하거나 나중에 세션을 기록하는 경우에만 집계됩니다.
- **API 삭제**: API를 통해 사용자를 삭제해도 MAU가 즉시 업데이트되지 않으며, 다음 월 주기에 카운트가 자체 수정됩니다.
...
다음 예는 다양한 사용자 행동을 통해 MAU 계산이 어떻게 이루어지는지 보여줍니다:

| 단계 | Action | 즉각적인 MAU 변경 | 결과 합계 |
|------|--------|----------------------|-----------------|
| 1 | **익명 사용자 1** 만들기 및 세션 기록하기 | +1 | 1 |
| 2 | **익명 사용자** 식별자 **1** (프로필이 식별자로 전환됨) | 0 | 1 |
| 3 | **익명 사용자 2** 만들기 및 세션 기록하기 | +1 | 2 |
| 4 | **익명 사용자 2를** 사용자 1과 **동일한 사람으로** 식별합니다(사용자 2는 고아가 됩니다). | -1 | 1 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation"}

MAU 스냅샷은 하루에 한 번 계산되며 소급하여 변경되지 않습니다. 이 예에서는 사용자 2가 나중에 고아가 되더라도 3단계 다음 날의 MAU 카운트는 영구적으로 2로 유지됩니다. 그러나 이후 일수의 MAU 수에는 고아가 아닌 사용자만 반영됩니다. 30일 동안 이 흐름은 고아가 아닌 단 한 명의 사용자만 남기 때문에 궁극적으로 1 MAU를 소비합니다.
...
{% alert important %}
둘 이상의 앱과 연결된 사용자는 각 앱에 대해 별도로 계산됩니다. 즉, 한 사용자가 워크스페이스의 여러 앱에서 세션을 시작하면 *새 사용자* 수에 여러 번 기여할 수 있습니다.
{% endalert %}
