...
{% alert note %}
**중단한 곳에서 계속하기** 섹션은 캠페인, 캔버스 또는 세그먼트를 편집하거나 생성한 후에 나타납니다.
{% endalert %}
...
**시간에 따른 성능** 그래프는 지정된 앱에 대해 지정된 날짜 범위 동안 각 통계의 값을 보여줍니다.

![시간에 따른 성능 그래프는 30일 동안의 신규 사용자 통계를 보여줍니다.]({% image_buster /assets/img/dashboards/performance_over_time.png %})

다음에 대한 통계를 차트로 만들 수 있습니다:
- Banners
- 콘텐츠 카드
- 일간 활성 사용자 수
  - (선택 사항) 세그먼트별 분석
...
- 인앱 메시지
- KPI 산출식
  - **KPI 공식 관리**을 선택하여 공식을 만들거나 기존 공식을 편집합니다.
- LINE
...
- **계산 시점**: 매일 12:05 UTC에 30일 스냅샷으로 한 번 계산되며, 카운트는 소급하여 변경되지 않습니다.
- **익명 프로필**: 세션이 하나 이상 기록될 때만 **카운트**됩니다.
- **식별된 프로필**: 존재하는 즉시 자동으로 카운트됩니다.
- **고아 프로필**: 다른 사용자와 병합된 중복은 **카운트되지** 않습니다.
- **CSV 업로드**: CSV로 업로드된 사용자는 `date_of_first_session` 또는 `date_of_last_session`이 제공되거나 나중에 세션을 기록할 때만 카운트됩니다.
- **API 삭제**: API를 통해 사용자를 삭제해도 MAU는 즉시 업데이트되지 않으며, 카운트는 다음 월 주기에서 자동으로 수정됩니다.

{% alert note %}
익명 사용자도 MAU에 포함됩니다. For mobile devices, anonymous users are device-dependent. For web users, anonymous users are browser cache-dependent.
{% endalert %}

#### MAU 계산 예시

다음 예시는 다양한 사용자 행동을 통해 MAU 계산이 어떻게 작동하는지를 보여줍니다:

| 단계 | Action | 즉각적인 MAU 변화 | 결과 총계 |
|------|--------|----------------------|-----------------|
| 1 | **익명 사용자 1**를 생성하고 세션을 기록합니다. | +1 | 1 |
| 2 | **익명 사용자 1**을 식별합니다 (프로필이 식별된 것으로 변환됨) | 0 | 1 |
| 3 | **익명 사용자 2**를 생성하고 세션을 기록합니다. | +1 | 2 |
| 4 | **익명 사용자 2**을 사용자 1과 **같은 사람**으로 식별합니다 (사용자 2는 고아가 됨) | –1 | 1 |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation"}

MAU 스냅샷은 하루에 한 번 계산되며, 결코 소급적으로 변경되지 않습니다. 이 예시에서, 3단계 이후의 하루 MAU 카운트는 영구적으로 2로 유지되며, 사용자 2가 나중에 고아가 되더라도 그렇습니다. 그러나 이후 날의 MAU 카운트는 고아가 아닌 사용자만 반영합니다. 30일 기간 내에서, 이 흐름은 궁극적으로 1 MAU를 소모합니다. 왜냐하면 단 하나의 뚜렷한 고아가 아닌 사용자만 남기 때문입니다.
...
{% alert important %}
하나 이상의 앱과 연결된 사용자들은 각 앱에 대해 별도로 계산됩니다. 이는 단일 사용자가 작업 공간의 서로 다른 앱에서 세션을 시작할 경우 *신규 사용자* 카운트에 여러 번 기여할 수 있음을 의미합니다.
{% endalert %}
