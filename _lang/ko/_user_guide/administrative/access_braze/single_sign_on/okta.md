...
| 요구 사항 | 세부 정보 |
| ----------- | ------- |
| Okta가 계정에 활성화되었습니다 | 계정에 이 기능을 사용하려면 Braze 계정 매니저에게 문의하세요. |
...
![Okta SAML SSO가 보안 설정 페이지에서 활성화되었습니다.]({% image_buster/assets/img/Okta/okta1.png %})

### 1b 단계: SAML SSO 설정 편집

Okta 관리자 대시보드에서 타겟팅 URL(로그인 URL)과 `x.509` 인증서를 제공하며, 이를 Braze 계정의 **보안 설정** 페이지에 입력해야 합니다.
...
![SSO 탭의 Okta 기본 릴레이 상태.]({% image_buster /assets/img/Okta/okta2.png %}){: style="max-width:75%"}
...
![Okta SSO가 사용 설정된 Braze 대시보드 로그인.]({% image_buster /assets/img/Okta/okta4.png %}){: style="max-width:60%"}
