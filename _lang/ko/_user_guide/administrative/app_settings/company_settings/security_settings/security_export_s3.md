...
> 보안 이벤트를 Amazon S3로 자동으로 내보낼 수 있으며, 이는 자정 UTC에 실행되는 일일 작업을 통해 이루어집니다. 설정 후에는 대시보드에서 보안 이벤트를 수동으로 내보낼 필요가 없습니다. 작업은 지난 24시간 동안의 보안 이벤트를 CSV 형식으로 구성된 S3 저장소에 내보냅니다. CSV 파일은 수동으로 내보낸 보고서와 동일한 구조를 가지고 있습니다.
...
1. 사용자에 대한 권한을 추가하려면 **IAM**(Identity and Access Management) > **정책** > **정책 생성**으로 이동하십시오. 
...
2. AWS 콘솔에서 **IAM**(Identity and Access Management) 섹션 > **정책**으로 이동한 다음 **정책 생성**을 선택하십시오.
...
![정책을 검토하고 생성할 수 있는 페이지입니다.]({% image_buster /assets/img/security_export/review_and_create.png %})
...
4. AWS 콘솔에서 **IAM**(Identity and Access Management) 섹션 > **역할** > **역할 생성**으로 이동하십시오. 
5. 신뢰할 수 있는 엔터티 선택기 유형으로 **다른 AWS 계정**을 선택하십시오. 
6. Braze 계정 ID를 제공하고 **외부 ID 필요** 상자를 체크한 후 Braze 외부 ID를 입력하십시오. 
7. 완료되면 **다음**을 선택합니다.

![신뢰할 수 있는 엔터티 유형을 선택하고 AWS 계정에 대한 정보를 제공할 수 있는 옵션이 있는 페이지입니다.]({% image_buster /assets/img/security_export/select_trusted_entity.png %})
...
![정책의 목록으로, 각 정책의 유형과 설명을 위한 열이 있습니다.]({% image_buster /assets/img/security_export/add_permissions.png %})

{: start="3"}
3\. 역할에 이름과 설명을 입력하고 **역할 만들기를** 선택합니다.

![역할 세부정보를 제공하기 위한 필드로, 이름, 설명, 신뢰 정책, 권한 및 태그가 포함됩니다.]({% image_buster /assets/img/security_export/name_review_create.png %})
...
!["보안 이벤트 다운로드" 섹션에서 "AWS S3로 내보내기"를 위한 토글이 켜져 있습니다.]({% image_buster /assets/img/security_export/security_event_download3.png %})
