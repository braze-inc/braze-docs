...
> 매일 자정(UTC)에 실행되는 작업을 통해 보안 이벤트를 클라우드 스토리지 제공업체인 Amazon S3로 자동으로 내보낼 수 있습니다. 설정 후에는 대시보드에서 보안 이벤트를 수동으로 내보낼 필요가 없습니다. 이 작업은 지난 24시간 동안의 보안 이벤트를 CSV 형식으로 구성된 S3 스토리지로 내보냅니다. CSV 파일은 수동으로 내보낸 보고서와 동일한 구조를 가집니다.

{% alert note %}
10,000행 제한은 대시보드에서 수동으로 CSV 보고서를 다운로드하는 경우에만 적용됩니다. S3로의 보안 이벤트 내보내기에는 이 행 제한이 적용되지 않습니다.
{% endalert %}
...
### 1단계: ID 및 액세스 관리(IAM) 사용자 만들기

비밀 액세스 키와 액세스 키 ID를 검색하려면 [AWS 계정 설정하기의](https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started-account-iam.html#create-an-admin) 안내에 따라 IAM 사용자를 만들어야 합니다.
...
1. **IAM** (ID 및 액세스 관리) > **정책** > **정책 만들기로** 이동하여 사용자에 대한 권한을 추가합니다. 
...
!["변경 사항 저장" 버튼을 클릭합니다.]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:50%;"}
...
2. AWS 콘솔에서 **IAM** (ID 및 액세스 관리) 섹션 > **정책으로** 이동한 다음 **정책 만들기를** 선택합니다.
...
4. AWS 콘솔에서 **IAM** (ID 및 액세스 관리) 섹션 > **역할** > **역할 만들기로** 이동합니다. 
5. 신뢰할 수 있는 엔터티 선택기 유형으로 **다른 AWS 계정**을 선택하십시오. 
6. Braze 계정 ID를 제공하고 **외부 ID 필요** 상자를 체크한 후 Braze 외부 ID를 입력하십시오. 
7. 완료되면 **다음**을 선택합니다.

![신뢰할 수 있는 엔티티 유형을 선택하고 AWS 계정에 대한 정보를 제공하는 옵션이 있는 페이지입니다.]({% image_buster /assets/img/security_export/select_trusted_entity.png %})
...
!["변경 사항 저장" 버튼을 클릭합니다.]({% image_buster /assets/img/security_export/save_changes_button.png %}){: style="max-width:40%;"}
