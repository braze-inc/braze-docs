...
page_order: 23
page_type: reference
description: "이 페이지는 사용자 추가 및 삭제, 사용자 권한 설정, 팀 생성 및 회사 설정 관리와 같은 회사 사용자 관리에 대해 다룹니다."
---
...
1. **설정** > **회사 사용자**.
2. **\+ 새 사용자 추가**를 선택합니다.
3. 이메일, 부서 및 [사용자 역할]({{site.baseurl}}/user_guide/administrative/manage_your_braze_users/user_permissions/#creating-a-role)을 포함하여 요청된 정보를 입력합니다.
...
![사용자 정의 권한 필드 섹션이 있는 워크스페이스 수준 권한.]({% image_buster /assets/img/add_new_user_3.png %})
...
보안상의 이유로 사용자는 Braze 계정과 연결된 이메일 주소를 변경할 수 없습니다. 사용자가 이메일 주소를 업데이트하려면 관리자가 [새 계정 생성](#adding-braze-users)을 해야 합니다.
...
사용자를 일시 중지하면 계정이 비활성 상태가 되어 더 이상 로그인할 수 없지만 계정과 관련된 데이터는 보존됩니다. 오직 관리자만 회사 사용자를 일시 중지하거나 일시 중지 해제할 수 있습니다. 일시 중지된 사용자는 여전히 Braze로부터 알림을 받을 수 있습니다.

사용자를 일시 중지하려면 **설정** > **회사 사용자**로 가서 사용자 이름을 찾고 <i class="fa-solid fa-user-lock"></i> **일시 중지**를 선택합니다.

![사용자를 일시 중지하는 옵션.]({% image_buster /assets/img_archive/suspend_user.png %})

관리자는 목록에서 이름을 선택하고 바닥글에서 **사용자 일시 중지**를 선택하여 사용자를 일시 중지할 수도 있습니다.

![사용자 세부 정보를 편집할 때 사용자를 일시 중지합니다.]({% image_buster /assets/img_archive/suspend_user2.png %}){: style="max-width:70%;"}

## 회사 사용자 삭제

사용자를 삭제하려면 **설정** > **회사 사용자**로 가서 사용자 이름을 찾고 <i class="fa fa-trash-can"></i> **사용자 삭제**를 선택합니다.
...
대시보드 사용자가 삭제되면 캠페인, 세그먼트 및 캔버스와 같은 대시보드 내에서 생성한 자산에 대한 중요한 영향은 없습니다. 그러나 이러한 자산의 **작성자** 필드는 삭제된 사용자의 이메일 주소 대신 "null" 값을 표시합니다.

삭제된 사용자와 동일한 이메일 주소로 새 대시보드 사용자가 생성되면, Braze는 삭제된 사용자가 생성한 자산을 새 사용자와 다시 연결하지 않습니다. 새 대시보드 사용자는 깨끗한 상태에서 시작하며 대시보드 내의 기존 자산의 생성자로 인정받지 않습니다.
...
`testing+01@braze.com`을 사용할 때 받은편지함에서 메시지 활성화를 받지 못하면, 해당 이메일 주소에서 메시지를 수신할 수 있는지 IT 팀에 확인하십시오. 일부 관리자는 `+` 로 시작하는 이메일 주소로 보낸 메시지를 필터링합니다.
