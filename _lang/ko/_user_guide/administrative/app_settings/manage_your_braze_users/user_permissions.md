...
권한 세트를 사용하여 특정 주제 영역 또는 작업과 관련된 권한을 번들로 묶습니다. 여러 업무 공간에서 동일한 액세스 권한이 필요한 회사 사용자에게 적용할 수 있습니다. 권한 세트를 만들려면 **설정** > **권한 설정**으로 이동한 다음 **권한 세트 만들기**을 선택합니다. 각 권한에 대한 설명은 [권한 목록](#list-of-permissions)을 참조하십시오.
...
역할은 개별 커스텀 권한을 워크스페이스 액세스 제어와 함께 묶어 더 많은 구조를 허용합니다. 이는 하나의 대시보드에 여러 브랜드나 로컬 워크스페이스가 있는 경우 특히 유용합니다. 역할을 사용하면 회사 사용자를 올바른 워크스페이스에 추가하고 관련 권한을 직접 부여할 수 있습니다. 각 권한에 대한 설명은 [권한 목록](#list-of-permissions)을 참조하십시오.
...
| 회사 관리자 | 회사 관리자에게는 회사 사용자를 삭제할 수 있는 기능을 포함하여 더 광범위한 권한이 있습니다. 그러나 자신의 계정을 삭제할 수는 없으며 해당 작업은 다른 회사 관리자에게 문의해야 합니다. |
...
{% alert note %}
한 회사 사용자에서 다른 회사 사용자로 사용자 권한을 병합하거나 가져올 수 없습니다.
{% endalert %}
...
- 다른 [회사 사용자]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/) 추가, 편집, 삭제, 일시 중단 또는 일시 중단 해제하기
- 회사 사용자를 CSV로 내보내기
...
회사 사용자 및 해당 권한의 목록을 다운로드하려면 **설정** > **회사 사용자로** 이동한 다음 **사용자 내보내기를** 선택합니다. CSV 파일이 곧 이메일 주소로 전송될 예정입니다.
...
{% alert important %}
2024년 4월부터 회사 사용자가 프로모션 코드 목록을 만들거나 업데이트하려면 "캠페인, 캔버스, 카드, 세그먼트, 미디어 라이브러리 액세스" 권한이 필요합니다.
{% endalert %}
...
|워크스페이스|대시보드 사용자 관리| 비관리자가 **회사 사용자** 페이지를 보고, 편집하고, 관리할 수 있으며, 자신을 포함한 모든 사용자의 권한을 수정하여 워크스페이스에 있는 회사 사용자를 관리할 수 있습니다. Users with this permission can’t delete users (only administrators can delete users).<br><br>이는 레거시 권한 `MANAGE_DEVELOPERS_AND_PERMISSIONS` 에 해당합니다.|
...
|워크스페이스|랜딩 페이지 초안 편집|Allows users to create and save landing page drafts.|
|워크스페이스|랜딩 페이지 보기|Allows users to access the **Landing Pages** page.|
|워크스페이스|랜딩 페이지 템플릿 편집|Allows users to create and edit landing page templates.|
...
| 워크스페이스 | WhatsApp 메시지 템플릿 보기 | 사용자가 [WhatsApp 메시지 템플릿을]({{site.baseurl}}/user_guide/message_building_by_channel/whatsapp/whatsapp_campaign/create/?tab=template%20messages#step-2-compose-your-whatsapp-message) 볼 수 있도록 허용합니다. |
| 워크스페이스 | WhatsApp 메시지 템플릿 편집하기 | 사용자가 템플릿 빌더에서 WhatsApp 메시지 템플릿을 만들 수 있습니다. 이 기능은 현재 초기 액세스 중입니다. |
| 워크스페이스 | Whatsapp 메시지 템플릿 삭제 | 사용자가 템플릿 빌더에서 WhatsApp 메시지 템플릿을 삭제할 수 있도록 허용합니다. 이 기능은 현재 초기 액세스 중입니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }
