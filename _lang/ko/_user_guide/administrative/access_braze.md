---
nav_title: Braze 액세스
article_title: Braze 액세스
page_order: 0
layout: dev_guide
guide_top_header: "Braze 액세스"
guide_top_text: "Braze 계정이나 대시보드에 접근하고 탐색하는 방법을 배우세요. 이 섹션에서는 계정 접근, 싱글 사인온, 대시보드 탐색 및 지원 받기에 대해 다룹니다."

page_type: reference
description: "이 랜딩 페이지에는 Braze 계정 또는 대시보드에 액세스하는 방법에 대한 문서가 있습니다. 여기에서 SSO, 로그인, Braze 인스턴스, SDK 엔드포인트, 비밀번호 재설정 등에 대한 리소스를 찾을 수 있습니다."

guide_featured_title: "섹션 기사"
guide_featured_list:
- name: 계정에 접근하기
  link: /docs/user_guide/administrative/access_braze/accessing_your_account/
  image: /assets/img/braze_icons/user-circle.svg
- name: SAML 및 싱글 사인온
  link: /docs/user_guide/administrative/access_braze/single_sign_on/
  image: /assets/img/braze_icons/log-in-01.svg
- name: 언어 설정
  link: /docs/user_guide/administrative/access_braze/language/
  image: /assets/img/braze_icons/globe-04.svg
- name: API 및 SDK 엔드포인트
  link: /docs/user_guide/administrative/access_braze/sdk_endpoints/
  image: /assets/img/braze_icons/navigation-pointer-01.svg
- name: Braze 대시보드
  link: /docs/user_guide/administrative/access_braze/braze_dashboard/
  image: /assets/img/braze_icons/browser.svg
- name: 제품 포털
  link: /docs/user_guide/administrative/access_braze/portal/
  image: /assets/img/braze_icons/target-05.svg
- name: Braze 고객지원팀
  link: /docs/user_guide/administrative/access_braze/support/
  image: /assets/img/braze_icons/mail-01.svg
- name: 문제 해결
  link: /docs/user_guide/administrative/access_braze/troubleshooting/
  image: /assets/img/braze_icons/annotation-question.svg
---

<br>

## 관련 리소스

사용자, 작업 공간 및 팀 관리와 같은 관리 작업은 다음을 참조하세요:
- [Braze 사용자 관리]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/)
- [작업 공간 생성 및 관리]({{site.baseurl}}/user_guide/administrative/app_settings/workspaces/)
- [Teams]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/teams/)

<br>