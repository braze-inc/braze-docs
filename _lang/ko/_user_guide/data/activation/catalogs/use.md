...
카탈로그 항목에 사용자 프로필 필드가 포함되어 있는 경우(개인화 Liquid 태그 내), 이러한 값은 메시지에서 Liquid를 올바르게 렌더링하기 위해 템플릿화 전에 Liquid에서 미리 정의되어야 합니다. `:rerender` 플래그가 제공되지 않으면 원시 Liquid 콘텐츠가 렌더링됩니다.
