...
메시지에 하나의 항목으로 제한되지 않습니다. **개인화 추가** 모달을 사용하여 한 번에 최대 세 개의 카탈로그 항목을 추가합니다. 더 추가하려면 작성기에서 **개인화 추가**를 다시 선택하고 추가 카탈로그 항목 및 표시할 정보를 선택합니다.
...
카탈로그 항목에 사용자 프로필 필드가 포함되어 있는 경우(개인화 Liquid 태그 내), 이러한 값은 메시지에서 Liquid를 올바르게 렌더링하기 위해 템플릿화 이전에 Liquid에서 미리 정의되어야 합니다. `:rerender` 플래그가 제공되지 않으면 원시 Liquid 콘텐츠가 렌더링됩니다.
