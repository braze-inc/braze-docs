...
3. 글로벌 재입고 설정이 구성되지 않은 경우 재입고 알림을 트리거하는 데 사용할 사용자 지정 이벤트 및 속성을 설정하라는 메시지가 표시됩니다:
    ...
    - **구독을 위한 커스텀 이벤트는** 가입한 사용자에게 이월 상품 알림을 보내는 데 사용되는 Braze 커스텀 이벤트입니다. 이 이벤트가 발생하면 이벤트를 수행한 사용자가 구독됩니다.
...
5\. 알림 규칙을 설정하세요. 두 가지 옵션이 있습니다:
    ...
    - **알림 제한을 설정하면** 구성된 알림 기간에 따라 지정된 수의 고객에게 알림이 전송됩니다. Braze는 더 이상 알림을 받을 고객이 없거나 품목이 품절될 때까지 지정된 고객 수만큼 순차적으로 알림을 보냅니다. 알림 속도는 분당 10,000명의 사용자에게 알림을 보낼 수 없습니다.
...
카탈로그에서 이월 재고 기능을 설정한 후 다음 단계에 따라 캔버스에서 이 기능을 사용할 수 있습니다.
