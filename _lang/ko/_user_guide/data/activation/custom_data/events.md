...
Braze에서 표준 이벤트는 Braze가 플랫폼 전반에서 인식하는 미리 정의된 작업입니다. [커스텀 이벤트](#custom-events)와 달리, 표준 이벤트를 생성하거나 이름을 지정할 필요가 없습니다. 이들은 내장되어 있습니다. 그러나 모든 표준 이벤트가 동일한 방식으로 추적되는 것은 아닙니다.
...
- [구매 이벤트](#purchase-events): 귀하의 개발 팀은 SDK의 구매 방법을 사용하여 이를 기록합니다. 자세한 내용은 구매 이벤트 섹션을 참조하십시오.
- 이메일 참여 이벤트(예: 이메일 열기 및 링크 클릭): Braze 이메일을 구성하고 이메일 추적을 활성화할 때 Braze에 의해 추적됩니다.
- 푸시 참여 이벤트(예: 푸시 알림 열기 및 클릭): Braze에서 푸시를 구성하고 앱에서 Braze SDK와 푸시 처리를 통합한 후 추적됩니다.

마케터로서 표준 이벤트를 사용하여 사용자 행동 및 참여를 이해할 수 있습니다. 예를 들어, 세션 데이터는 사용자가 앱이나 사이트를 얼마나 자주 여는지를 보여주고, 구매 이벤트는 시간이 지남에 따라 매출을 추적하는 데 도움이 됩니다.

## 구매 이벤트

구매 이벤트는 사용자가 만든 구매를 기록하고 추적합니다. Braze SDK를 통합한 후, 귀하의 개발 팀은 SDK의 구매 방법을 사용하여 구매를 기록할 수 있습니다. 구매 이벤트를 사용하여 구매를 추적할 때, Braze에서 직접 시간 경과에 따른 매출과 다양한 매출 출처를 모니터링할 수 있습니다.
...
이 예에서 "product_name"은 구매된 제품의 이름이며, "USD"는 구매의 통화, "1.99"는 제품의 가격, "1"은 구매된 수량입니다.
