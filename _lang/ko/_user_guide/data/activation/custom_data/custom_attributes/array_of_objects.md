...
{% alert important %}
요청 내 중첩 고객 속성에 유효하지 않은 값(예: 잘못된 시간 형식 또는`null`값)이 포함된 경우, Braze는 해당 요청에서 중첩 고객 속성 업데이트를 모두 처리에서 제외합니다. 이는 해당 특정 속성 내의 모든 중첩된 구조에 적용됩니다. 중첩 고객 속성 내의 모든 값이 유효한지 확인한 후 전송하십시오. 자세한 내용은 [사용자 생성 및 업데이트를]({{site.baseurl}}/api/endpoints/user_data/post_user_track/#how-does-userstrack-handle-invalid-nested-custom-attributes) 참조하십시오.
{% endalert %}
...
동일한 배열 속성에 대해 단일`/users/track`요청이 ,`$remove` , 및`$update`  작업을`$add` 포함하는 경우, Braze는 다음 순서로 이를 처리합니다:

1. `$add`
2. `$remove`
3. `$update`

가  보다 먼저`$add` 실행되기`$remove` 때문에, 단일 요청 내에서  를`$remove`사용한 뒤`$add`  를 업셋 메커니즘으로 사용할 수 없습니다. 먼저  가`$add`처리되고, 그 다음  가 해당 항목을`$remove` 삭제합니다. 업서트하려면,  를  보다 먼저`$remove` 별도의 요청으로`$add` 전송하십시오.

### 타임스탬프

오브젝트 배열에 타임스탬프와 같은 필드를 포함할 때는 일반 문자열이나 Unix 에포크 정수 대신 형식을`$time` 사용하십시오.
...
{% alert tip %}
자세한 내용은 [중첩 고객 속성을]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/nested_custom_attribute_support) 참조하십시오.
{% endalert %}
...
![개체 배열을 기준으로 필터링합니다.]({% image_buster /assets/img_archive/array_of_objects_segmenting_1.gif %})
...
![반려동물 유형별 필터링은 뱀과 같습니다.]({% image_buster /assets/img_archive/array_of_objects_segmenting_3.png %})

또는 `dog` 의 `type`을 가진 반려동물을 필터링할 수도 있습니다. 여기서 사용자는 반려견을 한 마리 이상 키우고 있으므로 '반려견을 한 마리 이상 키우는 모든 사용자'라는 세그먼트에 해당합니다.

![반려동물 유형별로 필터링하면 개가 됩니다.]({% image_buster /assets/img_archive/array_of_objects_segmenting_2.png %})
...
속성을 생성, 업데이트 또는 제거하는지에 따라 데이터 포인트가 다르게 기록됩니다.

{% tabs local %}
{% tab Create %}

새로운 배열을 생성하면 객체의 각 속성에 대해 하나의 데이터 포인트가 기록됩니다. 이 예제에는 8개의 데이터 포인트가 필요하며, 각 애완동물 개체에는 4개의 속성이 있고 2개의 개체가 있습니다.
...
기존 배열을 업데이트할 때 추가된 각 속성마다 하나의 데이터 포인트가 기록됩니다. 이 예제에서는 두 개체에서 각각 하나의 속성만 업데이트하므로 두 개의 데이터 포인트가 필요합니다.
...
배열에서 객체를 제거할 때마다 전송한 제거 기준마다 하나의 데이터 포인트가 기록됩니다. 이 예제에서는 이 문으로 여러 개의 개를 제거하더라도 데이터 포인트가 3개 필요합니다.
