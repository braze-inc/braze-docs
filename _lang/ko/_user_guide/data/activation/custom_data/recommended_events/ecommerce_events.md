...
| `currency` | 예 | 문자열 | 제품 가격이 표시되는 통화(예: "USD" 또는 "EUR")이며, [ISO 4217 형식](https://www.iso.org/iso-4217-currency-codes.html)을 따릅니다. |
| `source` | 예 | 문자열 | 이벤트가 발생한 소스입니다. (Shopify의 경우 상점 프론트입니다.) |
| `metadata` | 아니요 | 오브젝트 | |
| `type` | 아니요 | 오브젝트 | [재고 복원 알림]({{site.baseurl}}/user_guide/data/activation/catalogs/catalog_triggers/back_in_stock_notifications) 및 [가격 인하 알림]({{site.baseurl}}/user_guide/data/activation/catalogs/catalog_triggers/price_drop_notifications)과 함께 작동합니다. |
...
**장바구니 업데이트 이벤트 수행** 트리거를 사용하여 제품이 장바구니에 추가, 제거 또는 업데이트될 때를 추적할 수 있습니다. 이 이벤트는 트리거되기 전에 다음 정보를 확인합니다:
...
`ecommerce.cart_updated` 이벤트에는 장바구니 매핑 오브젝트가 있습니다. 이 오브젝트는 쇼핑객의 장바구니에 있는 모든 제품을 포함하는 장바구니 매핑이 담긴 고객 프로필에 생성됩니다. Liquid 태그를 통해 장바구니에 있는 제품에 접근할 수 있습니다: 
...
장바구니가 업데이트되지 않고 10일 이내에 주문 완료 이벤트로 진행되지 않으면 장바구니와 관련 제품을 삭제합니다.
...
장바구니가 두 개 있는 경우, 두 개 모두 병합된 사용자에게 추가합니다. 가장 최근 장바구니 정보가 포함된 메시지를 보내기 위해 동일하거나 다른 장바구니인 경우 캔버스를 다시 대기줄에 추가합니다. `ecommerce.cart_updated` 이벤트에는 가장 최근 장바구니 ID와 장바구니의 가장 최근 제품이 포함됩니다.
...
| `price` | 예 | 플로트 | 조회 시점의 제품 배리언트 단가입니다. |
| `metadata` | 아니요 | 오브젝트 | 고객이 자신의 사용 사례를 위해 추가하려는 제품에 대한 추가 메타데이터 필드입니다. Shopify의 경우 SKU가 추가됩니다. <br> 일반 이벤트 등록정보 제한인 50KB에 따라 제한이 있습니다. |
| `sku` | 아니요 | 문자열 | (Shopify 전용) Shopify SKU입니다. 카탈로그 ID 필드로 구성할 수 있습니다. |
| `source` | 예 | 문자열 | 이벤트가 발생한 소스입니다. (Shopify의 경우 상점 프론트입니다.) |
| `metadata` | 아니요 | 오브젝트 | 고객이 자신의 사용 사례를 위해 추가하려는 제품에 대한 추가 메타데이터 필드입니다. Shopify의 경우 SKU가 추가됩니다. <br> 일반 이벤트 등록정보 제한인 50KB에 따라 제한이 있습니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation" }
...
체크아웃 프로세스를 시작했지만 주문을 완료하지 않은 고객을 리타겟하기 위해 체크아웃 시작 이벤트를 사용할 수 있습니다.

`ecommerce.cart_updated` 이벤트와 유사하게, 이 이벤트를 통해 쇼핑 카트 Liquid 태그를 활용하여 장바구니 내 모든 제품에 접근할 수 있으며, 포기된 체크아웃 메시지에 활용할 수 있습니다:
...
| `order_number` | 아니요 | 문자열 | (Shopify 전용) 주문의 고유 주문 번호입니다. |
| `tags` | 아니요 | 배열 | (Shopify 전용) 주문 태그입니다.
| `referring_site` | 아니요 | 문자열 | (Shopify 전용) 주문이 발생한 사이트(예: Meta)입니다. |
| `payment_gateway_names` | 아니요 | 배열 | (Shopify 전용) 결제 시스템 소스(예: POS 또는 모바일)입니다. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 .reset-td-br-4 role="presentation" }
...
| `discounts`           | 아니요       | 오브젝트 배열     | 주문에 적용된 할인의 상세 목록입니다. |
| `products`            | 예      | 오브젝트 배열     |  |
| `product_id`       | 예      | 문자열    | 조회된 제품의 고유 식별자입니다. 이 값은 제품 ID, SKU 또는 유사한 값이 될 수 있습니다. <br>부분 환불이 발행되고 환불에 할당된 `product_id`가 없는 경우(예: 주문 수준 환불), 일반화된 `product_id`를 제공하세요.             |
| `product_name`     | 예      | 문자열    | 조회된 제품의 이름입니다.                                                                      |
| `variant_id`       | 예      | 문자열    | 제품 배리언트의 고유 식별자(예: `shirt_medium_blue`)입니다.                                         |
| `image_url`        | 아니요       | 문자열    | 제품 이미지의 URL입니다.     |
| `product_url`      | 아니요       | 문자열    | 제품 페이지의 URL입니다.  |
| `quantity`         | 예      | 정수   | 장바구니에 있는 제품의 수량입니다.   |
| `price`            | 예      | 플로트     | 조회 시점의 제품 배리언트 단가입니다.  |
| `metadata`         | 아니요       | 오브젝트    | 고객이 자신의 사용 사례를 위해 추가하려는 제품에 대한 추가 메타데이터 필드입니다. Shopify의 경우 SKU가 추가됩니다. 일반 이벤트 등록정보 제한인 50KB에 따라 제한이 있습니다. |
| `sku`            | 아니요       | 문자열    | (Shopify 전용) Shopify SKU입니다. 카탈로그 ID 필드로 구성할 수 있습니다.  |
| `source`              | 예      | 문자열    | 이벤트가 발생한 소스입니다. (Shopify의 경우 상점 프론트입니다.)    |
| `metadata`            | 아니요       | 오브젝트    |                |
| `order_status_url`  | 아니요       | 문자열    | 주문 상태를 확인할 수 있는 URL입니다.     |
...
고객이 주문을 취소할 때 트리거되는 주문 취소 이벤트를 사용할 수 있습니다.
...
| `discounts`           | 아니요       | 오브젝트 배열     | 주문에 적용된 할인의 상세 목록입니다.             |
| `products`            | 예      | 오브젝트 배열     |         |
| `product_id`          | 예      | 문자열    | 조회된 제품의 고유 식별자입니다. 이 값은 제품 ID, SKU 또는 유사한 값이 될 수 있습니다.             |
| `product_name`        | 예      | 문자열    | 조회된 제품의 이름입니다.          |
| `variant_id`          | 예      | 문자열    | 제품 배리언트의 고유 식별자(예: `shirt_medium_blue`)입니다.        |
| `image_url`           | 아니요       | 문자열    | 제품 이미지의 URL입니다.           |
| `product_url`         | 아니요       | 문자열    | 제품 페이지의 URL입니다.                                                                     |
...
Braze는 체크아웃 프로세스를 시작했지만 주문을 완료하기 전에 이탈한 고객을 타겟팅하는 등 전자상거래 추천 이벤트를 기반으로 하는 사전 구축된 캔버스 템플릿을 제공합니다. 이러한 이벤트를 활용하여 메시징을 개인화하고 특정 오디언스를 타겟팅함으로써 사용자 여정을 향상시킬 수 있습니다.

캔버스 템플릿과 함께 이러한 이벤트를 활용하는 더 많은 방법은 전용 [전자상거래 활용 사례]({{site.baseurl}}/user_guide/engagement_tools/canvas/ideas_and_strategies/ecommerce_use_cases)를 확인하세요.
...
- **총 주문 수** = 고유한 주문 완료 이벤트 수 - 고유한 주문 취소 수
- **총 환불 금액** = 총 주문 환불 금액의 합계 

이 사용자 필드 계산은 고객 프로필의 **트랜잭션** 탭에도 포함되어 있습니다.

![사용자 계산 필드가 있는 "트랜잭션" 탭입니다.]({% image_buster /assets/img/Shopify/transactions_tab.png %}){: style="max-width:70%;"}

## 자주 묻는 질문

### 제품 수준의 구매 데이터는 어디에서 볼 수 있나요?

고객 프로필의 **트랜잭션** 탭에서는 총 매출 및 총 주문과 같은 상위 수준 계산 필드를 확인할 수 있습니다. 특정 사용자의 제품 수준 세부 정보를 보려면 [쿼리 빌더]({{site.baseurl}}/user_guide/analytics/query_builder/)를 사용하여 전자상거래 이벤트 데이터를 쿼리하거나 [커런츠]({{site.baseurl}}/user_guide/data/braze_currents/)를 통해 이벤트 데이터를 내보내세요.

레거시 구매 이벤트와 달리, 전자상거래 추천 이벤트는 `products` 배열 내에 중첩된 이벤트 속성정보로 제품 세부 정보를 저장합니다. 이 속성정보는 Liquid를 통한 메시징과 [세그먼트 확장]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/)을 통한 세분화에서 사용할 수 있습니다.

### 특정 제품으로 사용자를 세분화하려면 어떻게 해야 하나요?

세그먼터를 사용하면 사용자가 전자상거래 이벤트를 수행한 횟수로 필터링할 수 있습니다. 특정 제품 등록정보(예: `product_id` 또는 `product_name`)로 필터링하려면 중첩된 이벤트 속성정보 필터링을 지원하는 [세그먼트 확장]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/)을 사용하세요. 예를 들어, 지난 90일 동안 제품 "SKU-123"을 구매한 모든 사용자를 찾을 수 있습니다.

### 레거시 구매 이벤트와 전자상거래 추천 이벤트의 차이점은 무엇인가요?

레거시 구매 이벤트는 Braze [구매 오브젝트]({{site.baseurl}}/api/objects_filters/purchase_object/)를 사용하며 개별 제품 구매를 `product_id` 및 `price`로 기록합니다. 전자상거래 추천 이벤트(예: `ecommerce.order_placed`)는 커스텀 이벤트 등록정보를 사용하며 여러 제품, 할인 및 메타데이터를 포함한 전체 주문 컨텍스트를 단일 이벤트로 캡처합니다.

전자상거래 추천 이벤트가 출시됨에 따라 Braze는 향후 레거시 구매 이벤트를 단계적으로 폐지할 예정입니다. 현재 구매 이벤트를 사용하고 있다면 사전 통지를 받게 됩니다. 그동안 공식 사용 중단 날짜까지 구매 이벤트를 계속 사용할 수 있습니다. 자세한 내용은 [추천 이벤트 개요]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/)를 참조하세요.

### 전자상거래 추천 이벤트에 커스텀 등록정보를 추가할 수 있나요?

전자상거래 추천 이벤트는 필수 및 선택 사항 필드가 있는 정의된 스키마를 가지고 있습니다. 각 이벤트의 `metadata` 오브젝트 내에 추가 커스텀 데이터를 포함할 수 있습니다. 그러나 커스텀 주문 수준 태그나 독점 필드(예: 구매 채널 또는 소매점 정보)는 최상위 등록정보로 지원되지 않습니다. 세분화를 위해 이러한 필드가 필요하다면, 전자상거래 이벤트와 함께 별도의 커스텀 이벤트로 계속 전송하세요.

### 전자상거래 이벤트를 보낼 때 external_id를 포함해야 하나요?

이벤트를 보내는 방법에 따라 다릅니다:

- **SDK를 통해**: 아니요. Braze SDK를 사용하면 이벤트가 자동으로 SDK의 현재 사용자 컨텍스트(익명 또는 식별됨)와 연결됩니다. 각 이벤트 호출에 사용자 식별자를 전달할 필요 없이, `changeUser`와 같은 메서드를 사용하여 해당 컨텍스트의 사용자를 식별할 수 있습니다.
- **REST API를 통해** (`/users/track`): 예. 각 API 요청에는 `external_id`, `braze_id`, `user_alias`, `email` 또는 `phone`과 같은 사용자 식별자를 포함해야 합니다. API에는 "현재 사용자" 컨텍스트가 없기 때문입니다.

### 중첩된 제품 등록정보가 AI 추천 설정 드롭다운에 나타나지 않는 이유는 무엇인가요?

[AI 항목 추천]({{site.baseurl}}/user_guide/brazeai/recommendations/)을 구성할 때, **등록정보 이름** 드롭다운에는 최상위 이벤트 등록정보(예: `order_id`, `total_value`, `currency`)만 표시됩니다. `products` 배열 내의 중첩 등록정보(예: `products.product_id` 또는 `products.variant_id`)는 이 목록에 나타나지 않을 수 있지만, 필드에 점 표기법을 사용하여 수동으로 입력할 수 있습니다. 대부분의 전자상거래 구현에서 Braze는 `products.product_id`를 항목 식별자로 사용하고, 항목 ID가 `product_id` 또는 `variant_id` 값과 일치하는 [카탈로그]({{site.baseurl}}/user_guide/data/activation/catalogs/)와 함께 사용하는 것을 권장합니다.

### 전자상거래 이벤트 중 일부가 Braze에 나타나지 않는 이유는 무엇인가요?

이벤트가 고객 프로필이나 로그에 나타나지 않는 경우, 다음을 확인하세요:

- **SDK 데이터 플러시 타이밍**: Braze SDK는 데이터를 로컬에 캐시하고 주기적으로 업로드합니다(일반적으로 10~60초 이내). 즉시 업로드를 강제하려면 `logCustomEvent()` 후에 `requestImmediateDataFlush()`를 호출하세요.
- **필수 등록정보**: 전자상거래 이벤트에는 필수 등록정보가 있습니다. 필수 등록정보가 누락되거나 데이터 유형이 잘못된 경우 이벤트가 거부될 수 있습니다. 이벤트 페이로드가 [필수 스키마](#types-of-ecommerce-recommended-events)와 일치하는지 확인하세요.
- **이벤트 이름 정확성**: 전자상거래 이벤트 이름은 대소문자를 구분하며 정확히 일치해야 합니다(예: `ecommerce.checkout_started`이며, `ecommerce.checkoutStarted`가 아닙니다).