---
nav_title: 사용자 정의 데이터 관리
article_title: 사용자 정의 데이터 관리
page_order: 20
page_type: reference
...
{: start="2"}
2\. 사용자 지정 속성, 이벤트 또는 제품을 추가하려면 해당 페이지로 이동하여 **사용자 지정 속성 추가** 또는 **사용자 지정 이벤트 추가** 또는 **제품 추가를** 선택합니다.<br><br>For custom attributes, select a [data type]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/#custom-attribute-data-types) for this attribute (for instance, boolean or string). 속성의 데이터 유형은 해당 속성에 사용할 수 있는 세분화 필터를 결정합니다. <br><br>![새 속성 또는 이벤트 추가]({% image_buster /assets/img_archive/prepopulate_add.png %}){: style="max-width:80%;" }
3\. Select **Save**.
...
![커스텀 properties for a 커스텀 event.]({% image_buster /assets/img_archive/manageproperties1.png %}){: style="max-width:80%"}
...
가끔 사용자 정의 속성, 사용자 정의 이벤트 또는 구매 이벤트를 식별할 수 있습니다. 이들은 너무 많은 데이터 포인트를 기록하거나, 더 이상 마케팅 전략에 유용하지 않거나, 오류로 기록된 것입니다. 
...
사용자 정의 데이터를 차단하려면 작업 공간의 다음 드롭다운에서 [사용자 권한]({{site.baseurl}}/user_guide/administrative/app_settings/manage_your_braze_users/user_permissions/)이 필요합니다.
...
![사용자 정의 속성 페이지에서 차단된 여러 선택된 커스텀 속성입니다.]({% image_buster /assets/img_archive/blocklist_custom_attr.png %})
...
* 커스텀 데이터를 삭제해도 [데이터 포인트]({{site.baseurl}}/user_guide/data/data_points)이 기록되지 않으며, 새로운 데이터 포인트도 생성되지 않습니다.
...
![사용자 지정 속성 데이터 유형 드롭다운]({% image_buster /assets/img_archive/custom_events_view_data_type_dropdown.png %})
