...
대시보드의 **사용자 지정 이벤트 보고서** 페이지에서 각 사용자 지정 이벤트가 얼마나 자주 발생하는지 집계하여 볼 수 있습니다. 시간 시계열에 겹쳐진 회색 선은 마지막으로 캠페인이 전송된 시간을 나타내며, 이는 캠페인이 커스텀 이벤트 활동에 미친 영향을 보는 데 유용합니다.
...
![사용자 지정 이벤트 그래프 필터]({% image_buster /assets/img/custom_events_report_filters.png %}){: style="max-width:40%;"}
...
커스텀 이벤트 속성은 Braze 프로필에 저장되지 않으므로 데이터 포인트를 기록하지 않습니다(예외는 [데이터 포인트](#data-points) 참조).
...
예약된 키를 이벤트 속성 이름으로 사용할 수 없습니다. `properties` 객체에서 예약된 키를 사용하면 "유효하지 않은 '속성' 필드"라는 오류가 반환됩니다.

| Property | 예약 키 |
| --- | --- |
| 사용자 지정 이벤트 | `time` and `event_name` | 
...
![버려진 카드에 대한 사용자 지정 이벤트 속성 필터. 두 개의 필터가 AND 연산자로 결합되어 100달러에서 200달러 사이의 아이템 가격으로 장바구니를 포기한 사용자에게 이 캠페인을 전송합니다]({% image_buster /assets/img_archive/customEventProperties.png %} "customEventProperties.png")
...
![버려진 카드에 대한 사용자 지정 이벤트 속성 필터. 장바구니에 있는 품목의 가격이 100달러 이상인 경우 하나의 필터가 선택됩니다.]({% image_buster /assets/img_archive/customEventPropertiesNested.png %} "customEventPropertiesNested.png")
...
이벤트 속성의 최근성과 빈도를 기반으로 세그먼트를 생성하려면 "커스텀 이벤트 속성 세분화 편집" [사용자 권한]({{site.baseurl}}/user_guide/data/data_points/#viewing-data-point-usage)이 필요합니다.
...
![ '장바구니 포기'와 속성 '아이템 수'가 1회 이상 30일 이내에 2보다 큰 필터 그룹입니다.]({% image_buster /assets/img/nested_object3.png %})

데이터는 해당 이벤트 속성을 활성화한 후에만 기록되며, 이벤트 속성은 해당 날짜 이후에만 사용할 수 있습니다.
...
1. **30일 이내:** Braze 세그먼트 내에서 특정 이벤트 속성 값의 빈도와 최근성을 기반으로 이벤트 속성 세분화를 사용할 수 있습니다. 이 옵션은 데이터 사용에 영향을 미칩니다.<br><br>
2. **30일 이내 및 그 이후:** 단기 및 장기 이벤트 속정정보 세분화를 모두 포함하려면 [세그먼트 확장]({{site.baseurl}}/user_guide/engagement_tools/segments/segment_extension/)을 사용할 수 있습니다. 이 기능은 지난 2년 동안 추적된 사용자 지정 이벤트 및 이벤트 속성을 기반으로 사용자를 분류합니다. 이 옵션은 데이터 사용에 영향을 미치지 않습니다.
