...
데이터 포인트는 고객 프로필에 대해 기록된 정보를 기반으로 합니다. 이 정의에 대한 자세한 내용은 Braze 계약서에서 확인할 수 있습니다. 고객 성공팀은 고객의 요구에 맞는 데이터 모범 사례를 추천해 드릴 수 있습니다. 
...
데이터 포인트 사용량을 최소화하려면 동일하게 변경되지 않는 데이터를 보내지 않도록 프로그램을 설정하고 관련성 있는 새로운 데이터만 Braze에 전달하는 것이 좋습니다. Braze는 온보딩 과정에서 이러한 모범 사례를 확립하기 위해 여러분과 협력할 것입니다.
{% endalert %}
...
{% alert tip %}
단순 배열의 경우 전체 배열을 한 번에 설정하면 단일 데이터 포인트로 계산됩니다. 따라서 배열은 사용자 프로필을 관련 정보로 최신 상태로 유지하고 비용을 절감할 수 있는 훌륭한 도구입니다. <br><br> 오브젝트 배열은 업데이트되는 각 키에 대해 데이터 포인트를 소비합니다. Braze에 업데이트만 전달하여 불필요한 데이터 포인트 소비를 줄이세요.
{% endalert %}
...
| 가장 최근 위치 | 가장 최근의 모든 위치 | 지오펜스 데이터는 사용자 프로필에 대해 저장되지 않기 때문에 지오펜스를 입력하거나 종료해도 데이터 포인트가 기록되지 않습니다. 지오펜스는 Apple 및 Google 위치 서비스에 의해 모니터링되며, 사용자가 지오펜스를 트리거할 때만 Braze가 알림을 받습니다. |
| Twitter | 사용자 이름 | |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
