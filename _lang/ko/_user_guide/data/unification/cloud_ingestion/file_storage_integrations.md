...
{: start="2"}
2. Braze 대시보드에서 Braze AWS 계정 ID를 복사합니다. **클라우드 데이터 수집**으로 이동하여 **새 데이터 동기화 생성**을 선택하고 **S3 가져오기**를 선택합니다.
3. AWS에서 신뢰할 수 있는 엔터티 선택기 유형으로 **다른 AWS 계정**을 선택합니다. Braze 계정 ID를 입력합니다. **외부 ID 필요** 체크박스를 선택합니다.
4. Braze에서 **데이터 설정** > **클라우드 데이터 수집**으로 이동하여 **새 데이터 동기화 생성**을 선택하고 파일 소스 섹션에서 **S3 가져오기**를 선택합니다.
5. 자동으로 생성된 **Braze 계정 ID**를 복사합니다. 

![Braze 계정 ID 필드가 있는 자격 증명 섹션.]({% image_buster /assets/img/braze_account_id.png %})

{: start="6"}
6. AWS에서 계정 ID를 붙여넣고 **다음**을 선택합니다.
...
{: start="7"}
7. 5단계에서 만든 정책을 역할에 연결합니다. 검색창에서 정책을 검색하고 정책 옆의 체크 표시를 선택하여 연결합니다. 완료되면 **다음**을 선택합니다.
...
{: start="8"}
8. 생성한 역할의 ARN과 생성한 외부 ID를 기록해 두세요. 클라우드 데이터 수집 통합을 생성하는 데 필요합니다.

## Braze에서 클라우드 데이터 수집 설정하기

{% alert important %}
2026년 2월 이후에 온보딩하는 고객은 소스와 동기화를 별도로 구성하는 새로운 CDI UI에 조기 액세스할 수 있습니다. 이 새로운 UI에서는 먼저 자격 증명, 버킷 및 리전으로 S3 소스를 생성합니다. 그런 다음 SQS URL 및 폴더 경로(선택 사항)로 동기화를 만듭니다.
{% endalert %}
...
![새 가져오기 동기화를 만들기 위해 S3에 표시된 예제 보안 자격 증명.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_1.png %})

{: start="3"}
3. 통합의 이름을 지정하고 이 통합의 데이터 유형을 선택합니다. 

![데이터 유형으로 사용자 속성을 사용하여 "cdi-s3-as-source-integration"의 동기화 세부 정보를 설정합니다.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_2.png %})
...
Braze는 AWS에서 적용하는 것 외에 추가적인 파일 이름 요구 사항을 적용하지 않습니다. 파일 이름은 고유해야 합니다. 고유성을 위해 타임스탬프를 추가하는 것을 권장합니다.
...
| `EMAIL` | 사용자의 이메일 주소입니다. 동일한 이메일 주소를 가진 프로필이 여러 개 존재하는 경우, 가장 최근에 업데이트된 프로필이 우선적으로 업데이트됩니다. 이메일과 전화번호를 모두 포함하면 이메일이 기본 식별자로 사용됩니다. |
