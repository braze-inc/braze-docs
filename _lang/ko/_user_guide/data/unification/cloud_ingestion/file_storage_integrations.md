...
다음 단계에서 동일한 지역에 SQS 대기열을 생성할 것이므로 버킷을 생성한 지역을 기록해 두십시오.
...
![대기열에 접근할 수 있는 사람을 정의하기 위한 예제 JSON 객체와 함께 "고급"을 선택합니다.]({% image_buster /assets/img/cloud_ingestion/s3_ARN.png %})
...
{% alert note %}
If you upload your files to the root folder of an S3 bucket and then move some of the files to a specific folder in the bucket, you may encounter an unexpected error. 대신 접두사 안에 있는 파일에 대해서만 이벤트 알림을 보내도록 변경하거나, 해당 접두사 밖에 있는 S3 버킷에 파일을 넣지 않도록 하거나, 접두사 없이 통합을 업데이트하여 모든 파일을 수집할 수 있습니다.
{% endalert %}
...
{: start="2"}
2\. Braze 대시보드에서 Braze AWS 계정 ID를 복사합니다. Go to **Cloud Data Ingestion**, select **Create New Data Sync**, and select **S3 Import**.
3\. AWS에서 신뢰할 수 있는 엔터티 선택기 유형으로 **다른 AWS 계정**을 선택합니다. 귀하의 Braze 계정 ID를 제공하십시오. **외부 ID 필요** 체크박스를 선택하십시오.
4\. Braze에서 **데이터 설정** > **클라우드 데이터 수집**으로 이동하여 **새 데이터 동기화 생성**을 선택하고 파일 소스 섹션에서 **S3 가져오기**를 선택하십시오.
5\. 자동으로 생성된 **Braze 계정 ID**를 복사하십시오. 

![Braze 계정 ID 필드가 있는 자격 증명 섹션.]({% image_buster /assets/img/braze_account_id.png %})

{: start="6"}
6\. AWS에서 계정 ID를 붙여넣고 **다음**을 선택하십시오.
...
{: start="7"}
7\. 4단계에서 만든 정책을 역할에 첨부합니다. 검색창에서 정책을 검색하고 정책 옆의 확인 표시를 선택하여 첨부합니다. 완료되면 **다음**을 선택합니다.
...
{: start="8"}
8\. 생성한 역할의 ARN과 생성한 외부 ID를 기록해 두십시오. 클라우드 데이터 수집 통합을 생성하는 데 필요합니다.

## Braze에서 클라우드 데이터 수집 설정하기

{% alert important %}
2026년 2월 또는 그 이후에 온보딩하는 고객은 소스와 동기화가 별도로 구성되는 새로운 CDI UI에 조기 액세스할 수 있습니다. 이 새로운 UI에서 자격 증명, 버킷 및 지역으로 먼저 S3 소스를 생성하십시오. 그런 다음 SQS URL 및 폴더 경로(선택 사항)로 동기화를 만듭니다.
{% endalert %}
...
![새 가져오기 동기화를 만들기 위해 S3에 표시된 예제 보안 자격 증명입니다.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_1.png %})

{: start="3"}
3\. 통합의 이름을 지정하고 이 통합의 데이터 유형을 선택합니다. 

![데이터 유형으로 사용자 속성을 사용하여 "cdi-s3-as-source-integration"의 동기화 세부정보를 설정합니다.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_2.png %})

{: start="4"}
4\. 액세스 또는 권한 문제로 인해 동기화가 중단되는 경우 알림을 받을 연락처 이메일을 추가하세요. 선택 사항으로 사용자 수준 오류 및 동기화 성공에 대한 알림을 설정합니다. 

![동기화 오류 알림에 대한 알림 기본 설정을 설정합니다.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_3.png %})

{: start="5"}
5\. 마지막으로 **연결 테스트**를 선택하여 Braze가 귀하의 버킷에 접근하고 수집할 수 있는 파일 목록을 확인합니다(파일 내부의 데이터는 아님). 그런 다음 동기화를 저장합니다. 

![데이터 미리 보기로 연결을 테스트하는 옵션입니다.]({% image_buster /assets/img/cloud_ingestion/s3_ingestion_4.png %})
...
| `ALIAS_NAME` and `ALIAS_LABEL` | `alias_name` 은 고유 식별자이고 `alias_label` 은 별칭의 유형을 지정해야 하며, 이 두 열은 사용자 별칭 개체를 만듭니다. Users may have multiple aliases with different labels, but only one `alias_name` per `alias_label`. |
...
| `EMAIL` | 사용자의 이메일 주소입니다. 동일한 이메일 주소를 가진 여러 프로필이 존재하는 경우, 가장 최근에 업데이트된 프로필이 업데이트 우선순위가 됩니다. 이메일과 전화번호를 모두 포함하면 이메일을 기본 식별자로 사용하겠습니다. |
...
```
선택적 **삭제됨** 열을 포함합니다. `DELETED`가 `true`일 때 해당 카탈로그 항목은 Braze의 카탈로그에서 제거됩니다. [카탈로그 항목 삭제](#deleting-catalog-items)를 참조하십시오.
{% endtab %}
...
S3에 대한 클라우드 데이터 수집은 파일 업로드를 통해 사용자 및 카탈로그 항목 삭제를 지원합니다. 각각에 대해 별도의 동기화 및 파일 형식을 사용하십시오.

- **[사용자 삭제](#deleting-users)** – 데이터 유형 **사용자 삭제**로 동기화를 생성하고 사용자 식별자(페이로드 없음)만 포함된 파일을 업로드합니다.
- **[카탈로그 항목 삭제](#deleting-catalog-items)** – 기존 카탈로그 동기화를 사용하고 항목 제거를 표시하기 위해 `deleted`(또는 `DELETED`) 열을 추가합니다.

### 사용자 삭제

S3의 파일을 사용하여 Braze에서 사용자 프로필을 삭제하려면:
...
3. 사용자 식별자 열만 포함된 파일을 S3 버킷에 업로드합니다. `PAYLOAD` 열을 포함하지 마십시오. 페이로드가 존재하면 동기화가 실패하므로 우발적인 삭제를 피하십시오.

파일의 각 행은 다음 중 하나를 사용하여 정확히 하나의 사용자를 식별해야 합니다:

| 식별자 | 설명 |
| --- | --- |
| `EXTERNAL_ID` | Braze에서 사용되는 `external_id`와 일치합니다. |
| `ALIAS_NAME` and `ALIAS_LABEL` | 두 열이 함께 별칭으로 사용자를 식별합니다. |
| `BRAZE_ID` | Braze에서 생성된 사용자 ID(기존 사용자만 해당). |

{% alert important %}
사용자를 삭제하는 것은 영구적이며 되돌릴 수 없습니다. 제거할 의도가 있는 사용자만 포함하십시오. 자세한 내용은 [Cloud Data Ingestion으로 사용자 삭제]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/delete_users/)를 참조하십시오.
{% endalert %}
...
동기화가 실행되면 Braze는 버킷의 새 파일을 처리하고 해당 사용자 프로필을 삭제합니다.

### 카탈로그 항목 삭제

파일 저장소를 사용하여 카탈로그에서 항목을 제거하려면:

1. [카탈로그 데이터 동기화]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/sync_catalogs_data/)에 사용하는 동일한 S3 동기화를 사용하십시오(데이터 유형 **카탈로그**).
2. CSV 또는 JSON 파일에 선택적 **`deleted`** (또는 **`DELETED`**) 열을 추가하십시오.
3. Braze의 카탈로그에서 제거할 카탈로그 항목에 대해 `deleted`을 `true`로 설정하십시오.

각 행은 여전히 `ID` 및 `PAYLOAD`가 필요합니다. 삭제를 위해 표시된 행의 경우 페이로드는 최소화할 수 있습니다. Braze는 `ID`에 의해 항목을 제거합니다.
...
동기화가 실행될 때, `deleted: true`이 있는 행은 Braze에서 일치하는 카탈로그 항목이 삭제됩니다. 전체 카탈로그 동기화 및 삭제 동작에 대한 내용은 [동기화 및 삭제 카탈로그 데이터]({{site.baseurl}}/user_guide/data/unification/cloud_ingestion/sync_catalogs_data/)를 참조하십시오.
...
기존 파일을 사용하여 Braze가 귀하의 버킷에 접근하고 수집할 파일을 감지할 수 있는지 확인할 수 있지만, 이 파일들은 Braze에 동기화되지 않습니다. CDI가 이를 처리하려면, 동기화하려는 기존 파일을 S3에 다시 업로드해야 합니다. 
