...
동기화가 실행될 때마다 Braze는 `UPDATED_AT` 가 마지막으로 동기화된 타임스탬프와 같거나 그 이후에 있는 모든 행을 가져옵니다. 데이터 웨어하우스에서 카탈로그 데이터로 뷰를 만들어 동기화가 실행될 때마다 완전히 새로 고쳐지는 원본 테이블을 설정하는 것이 좋습니다. 조회를 사용하면 매번 쿼리를 다시 작성할 필요가 없습니다.
