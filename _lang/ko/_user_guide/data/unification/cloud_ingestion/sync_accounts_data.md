---
nav_title: 계정 데이터를 동기화하고 삭제합니다
article_title: CDI를 사용하여 계정 데이터를 동기화합니다
page_order: 4
page_type: reference
description: "CDI를 사용하여 Braze 계정 데이터를 동기화하는 방법을 배우십시오."

---

# CDI를 사용하여 계정 데이터를 동기화합니다

> CDI를 사용하여 Braze 계정 데이터를 동기화하는 방법을 배우십시오.

{% alert important %}
[계정 객체](https://braze.com/unlisted_docs/account_opportunity_object/)는 베타 버전이며 이 기능을 사용해야 합니다. Contact your Braze account manager if you’re interested in participating in the beta.
{% endalert %}

## 필수 조건

CDI를 사용하여 계정 데이터를 동기화하기 전에 [계정 스키마를 구성해야 합니다](https://braze.com/unlisted_docs/account_opportunity_object/).

{% alert note %}
동기화가 일시 중지되었거나 예정되어 있지 않을 때만 계정 스키마를 업데이트하여 데이터 웨어하우스 데이터와 Braze의 스키마 간의 충돌을 피하십시오.
{% endalert %}

## 동기화 작동 방식

- 각 동기화는 `UPDATED_AT`이 마지막 동기화된 타임스탬프보다 늦은 행을 가져옵니다.
- 통합에서 가져온 데이터는 제공된 `id`에 따라 계정을 생성하거나 업데이트합니다.
- `DELETED`이 `true`인 경우 계정이 삭제됩니다.
- 동기화는 데이터 포인트를 기록하지 않지만, 모든 동기화된 데이터는 총 저장 데이터로 측정되는 총 계정 사용량에 포함됩니다. 변경된 데이터만 제한할 필요는 없습니다.
- 계정 스키마에 없는 필드는 삭제됩니다. 새 필드를 동기화하기 전에 스키마를 업데이트하십시오.
- 동기화 이름 위에 마우스를 올리고 관련 작업을 선택하여 동기화를 새로 고치거나, 재개하거나, 일시 중지할 수 있습니다.
...
1. Snowflake에서 소스 테이블을 생성합니다. 예제의 이름을 사용하거나 자신의 데이터베이스, 스키마 및 테이블 이름을 선택하십시오. 테이블 대신 뷰 또는 물리적 뷰를 사용할 수도 있습니다.
  ```sql
...
데이터 웨어하우스에서 동기화 뷰를 생성하면 소스가 추가 쿼리를 다시 작성할 필요 없이 자동으로 새로 고침됩니다.

예를 들어, `account_details_1`이라는 계정 데이터 테이블이 있고 `account_id`, `account_name` 및 세 개의 추가 속성이 있는 경우 다음과 같은 동기화 뷰를 만들 수 있습니다:
