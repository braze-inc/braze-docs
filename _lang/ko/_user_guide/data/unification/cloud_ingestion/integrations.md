...
{% alert tip %}
Braze는 `UPDATED_AT` 에서 술어를 사용하여 자체 프로젝트에서 (미리 정의된 스키마를 사용하여) BigQuery 테이블을 쿼리합니다. 큰 테이블을 적절한 세부 수준(예: 일별 세부 수준)으로 `UPDATED_AT` 으로 분할하면 BigQuery에서 파티션을 잘라내어 관련 데이터만 스캔할 수 있습니다. 이는 성능/성과를 개선하고 비용을 절감하는 데 도움이 될 수 있습니다. 자세한 내용은 [BigQuery 파티셔닝 설명서를](https://docs.cloud.google.com/bigquery/docs/partitioned-tables) 참조하세요.
{% endalert %}
