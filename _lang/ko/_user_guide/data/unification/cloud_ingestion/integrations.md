...
- `UPDATED_AT` - 이 행이 테이블에 업데이트되거나 추가된 시간. 마지막 동기화 이후 추가되거나 업데이트된 행만 동기화됩니다.
- **사용자 식별자 열** \- 테이블에는 하나 이상의 사용자 식별자 열이 포함될 수 있습니다. 각 행에는 하나의 식별자( `external_id`, `alias_name` 과 `alias_label` 의 조합 , `braze_id`, `email` 또는 `phone`)만 포함해야 합니다. 소스 테이블에는 하나, 둘, 셋, 넷 또는 다섯 가지 식별자 유형 모두에 대한 열이 있을 수 있습니다.
    ...
    - `EMAIL` - 사용자의 이메일 주소. 동일한 이메일 주소를 가진 프로필이 여러 개 있는 경우 가장 최근에 업데이트된 프로필이 업데이트 우선순위를 갖습니다. 이메일과 전화번호를 모두 포함하는 경우 이메일이 기본 식별자로 사용됩니다.
...
CDI는 BigQuery용 파티션을 지원합니다. `UPDATED_AT` 함수로 파티션을 분할하는 경우(예: 데이터 집합의 크기에 따라 일, 주 또는 시간 단위로), BigQuery는 스캔해야 하는 데이터를 잘라낼 수 있습니다. 이렇게 하면 초대형 테이블의 성능/성과 효율성이 향상됩니다.

다른 필드로 파티션을 나누지 마세요. 다양한 구성을 테스트하여 특정 데이터에 가장 적합한 설정을 찾아보세요.

모든 CDI 쿼리는 `UPDATED_AT` 으로 필터링되지만 이 동작은 변경될 수 있습니다. 쿼리에 이 절이 _포함되지_ 않도록 테이블 스키마를 설계하세요.

자세한 내용은 [BigQuery 파티셔닝 설명서를](https://docs.cloud.google.com/bigquery/docs/partitioned-tables) 참조하세요.
{% endalert %}
...
Microsoft 패브릭에 대한 새 데이터 동기화를 만듭니다. Braze 대시보드에서 **데이터 설정** > **클라우드 데이터 수집으로** 이동하여 **새 데이터 동기화 만들기를** 선택한 다음 **Microsoft 패브릭 가져오기를** 선택합니다.
...
![Braze 대시보드의 Redshift에 대한 "새 가져오기 동기화 만들기" 페이지가 1단계로 설정되어 있습니다: "알림 환경설정 설정하기".]({% image_buster /assets/img/cloud_ingestion/fabric_setup_3.png %})
...
![Braze 대시보드의 Redshift에 대한 "새 가져오기 동기화 만들기" 페이지가 1단계로 설정되어 있습니다: "연결 테스트".]({% image_buster /assets/img/cloud_ingestion/ingestion_8.png %})
{% endsubtab %}
...
![Braze 대시보드의 BigQuery에 대한 "새 가져오기 동기화 만들기" 페이지가 3단계로 설정되어 있습니다: "연결 테스트".]({% image_buster /assets/img/cloud_ingestion/ingestion_13.png %})
...
![Braze 대시보드에서 데이터브릭에 대한 "새 가져오기 동기화 만들기" 페이지가 3단계로 설정됩니다: "연결 테스트".]({% image_buster /assets/img/cloud_ingestion/ingestion_13.png %})
...
![Braze 대시보드의 Redshift에 대한 "새 가져오기 동기화 만들기" 페이지가 1단계로 설정되어 있습니다: "연결 테스트".]({% image_buster /assets/img/cloud_ingestion/fabric_setup_4.png %})
...
![수직 타원 메뉴에서 '지금 동기화' 옵션이 표시된 Braze 대시보드의 데이터 브릭에 대한 '데이터 가져오기' 페이지.]({% image_buster /assets/img/cloud_ingestion/ingestion_18.png %})
