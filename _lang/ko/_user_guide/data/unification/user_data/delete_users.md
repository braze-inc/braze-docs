...
{% alert important %}
이 기능의 얼리 액세스는 일시적으로 종료되었습니다. 자세한 내용은 고객 성공 매니저에게 문의하세요.
{% endalert %}
...
![수직 타원 메뉴가 열려 있는 Braze의 사용자로, 사용자 삭제 옵션이 표시됩니다.]({% image_buster /assets/img/audience_management/deleting_users/delete_user.png %}){: style="max-width:85%;"}
...
![삭제를 위해 선택한 세그먼트가 표시된 팝업 창입니다.]({% image_buster /assets/img/audience_management/deleting_users/choose_segment_to_delete.png %}){: style="max-width:75%;"}

**삭제를** 입력하여 요청을 확인한 다음 **사용자 삭제를** 선택합니다.

![확인 상자에 '삭제'를 입력한 확인 페이지가 표시됩니다.]({% image_buster /assets/img/audience_management/deleting_users/confirm_segment_delete.png %}){: style="max-width:75%;"}
...
삭제 대기 중인 프로필의 개수가 포함된 확인 이메일을 Braze에서 보내드립니다.

삭제를 계속하려면 Braze에 로그인하여 삭제 요청을 확인합니다.
...
!['사용자 삭제' 탭의 '삭제 기록 상세정보' 창입니다.]({% image_buster /assets/img/audience_management/deleting_users/deletion_record_details.png %}){: style="max-width:55%;"}
...
### 1억 명의 사용자를 삭제할 수 없고 1천만 명의 사용자만 삭제할 수 있는 것 같습니다. 버그인가요?

아니요, 버그가 아닙니다. 특정 고객은 앞서 해보기(EA) 프로그램 기간 동안 삭제할 수 있는 사용자 수가 제한되어 있습니다.

EA 프로그램이 진행됨에 따라 이 용량은 모든 고객이 최대 1억 명의 사용자를 삭제할 수 있을 때까지 증가하도록 설계되었습니다.

이 용량을 늘리려면 Braze 계정 매니저에게 문의하세요. 요청은 제품 팀의 재량에 따라 승인됩니다.

### 사용자 병합 자동화가 사용자 삭제에 영향을 주나요?

예약된 병합에 삭제 대기 중인 고객 프로필이 포함된 경우 Braze는 해당 프로필을 건너뛰고 병합하지 않습니다. 이러한 프로필을 병합하려면 해당 프로필을 삭제에서 제거해야 합니다.
