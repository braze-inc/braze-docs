...
{% endtab %}
{% tab 구매 %}
 `time` 필드는 선택 사항이며 ISO 8601 문자열 또는 `yyyy-MM-dd'T'HH:mm:ss:SSSZ` 형식으로 형식화해야 합니다. `time` 필드가 없으면 `UPDATED_AT` 열 값이 이벤트 시간으로 사용됩니다. 기타 필드, `app_id`, `quantity` 및 `properties`를 포함하여 선택 사항입니다.
