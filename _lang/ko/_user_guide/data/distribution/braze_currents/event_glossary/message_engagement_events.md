...
![사용자 특정 속성, 캠페인 또는 캔버스 추적 속성, 이벤트 특정 속성으로 그룹화된 이메일 탈퇴 이벤트를 보여주는 메시지 참여 이벤트의 분석]({% image_buster /assets/img/message_engagement_event.png %})
...
- `abort_type`은 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시: `Frequency cap rule: 5 Banner messages every 1 week`
{% endapi %}
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송에 대한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. 같은 전송에 속하는 이벤트를 그룹화하려면 `dispatch_id`을 사용하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
- `abort_type`은 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시: `Frequency cap rule: 5 Content Card messages every 1 week`
{% endapi %}
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송을 위한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 email messages every 1 week`
{% endapi %}
...
이 이벤트는 인터넷 서비스 공급자가 하드바운스를 반환할 때 발생합니다. 하드 바운스는 영구적인 전달 가능성 실패를 의미합니다.
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송을 위한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
  - `dispatch_id`의 동작은 캔버스와 캠페인 간에 다르며, 이는 예약된 경우에도 Braze가 예약 가능한 진입 단계를 제외한 캔버스 단계를 트리거된 이벤트로 취급하기 때문입니다.
{% endapi %}
...
이 이벤트는 메시지가 우선 순위가 낮아지거나 빈도 제한이 설정될 때 발생하며, 구성된 재시도 창 내에서 나중에 재시도됩니다. 이것은 메시지 우선 순위 베타 고객에게만 제공됩니다.
...
이 이벤트는 이메일 전송 요청이 Braze와 SendGrid 간에 성공적으로 전달되었을 때 발생합니다. 그러나 이것은 이메일이 사용자의 받은편지함에 수신되었다는 것을 의미하지 않습니다. Braze는 이벤트가 이메일 이벤트와 관련된 이메일 및 사용자 ID 모두에 일치하지 않는 경우 사용자 프로필이나 Currents 대상(예: Snowflake)에 이벤트를 기록하지 않습니다.
...
{% alert important %}
`Unsubscribe` 이벤트는 사용자가 이메일의 탈퇴 링크(이메일 본문 또는 바닥글 내의 일반 탈퇴 링크 또는 [list-unsubscribe header<3> 사용)에서 클릭할 때 발생하는 특수 클릭 이벤트로 간주됩니다. 사용자가 탈퇴 상태로 변경될 때는 발생하지 않습니다. 구독 상태 변경이 API를 통해 전송되거나 커스텀(비-Braze) 탈퇴 링크로 전송되는 경우 Currents에서 이메일 탈퇴 이벤트를 트리거하지 않습니다.
{% endalert %}
...
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 in-app messages every 1 week`
{% endapi %}
...
이 이벤트는 예정된 LINE 메시지가 전달될 수 없을 때 발생하며, LINE으로 전송하기 전에 발생합니다.
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송을 위한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 LINE messages every 1 week`
{% endapi %}
...
이 이벤트는 사용자가 LINE 메시지에서 링크를 클릭할 때 발생하며, 링크의 도메인이 클릭 추적 도메인과 일치합니다.
...
{% api %}
## LINE 수신 이벤트 {#line-inbound-receive-events}

{% apitags %}
LINE, Inbound Received
{% endapitags %}

이 이벤트는 사용자가 LINE 메시지를 수신할 때 발생합니다.
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송을 위한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계 지을 수 있습니다(예: 전송, 반송 및 열기).
{% endapi %}
...
이 이벤트는 메시지가 우선 순위가 낮아지거나 빈도 제한이 걸릴 때 발생하며, 구성된 재시도 창 내에서 나중에 재시도됩니다. 이것은 메시지 우선 순위 베타 고객에게만 제공됩니다.
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송을 위한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계 지을 수 있습니다(예: 전송, 반송 및 열기).
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 push messages every 1 week`입니다.
{% endapi %}
...
{% alert note %}
드물게, 푸시 열기가 Currents 데이터에서 해당 푸시 전송 이벤트보다 먼저 나타날 수 있습니다. 그 이유는 다음과 같습니다:
- SDK의 시계가 잘못되었습니다.
- 높은 배치 쓰기 대기 시간. 기록된 전송 시간이 조기 전달에 비해 지연될 수 있으므로, 매우 빠른 열기가 배치의 최종 전송 타임스탬프가 기록되기 전에 기록될 수 있습니다. 대량 전송은 배치로 전송되고 기록됩니다.
{% endalert %}
...
이 이벤트는 Braze 내에서 감지된 오류로 인해 RCS 전송이 중단될 때 생성되며, 메시지가 삭제됩니다.
...
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 RCS messages every 1 week`입니다.
{% endapi %}
...
{% api %}
## RCS 수신 이벤트 {#rcs-inbound-received-events}
...
이 이벤트는 RCS 메시지가 Braze에서 우리의 최종 배송 파트너에게 전송될 때 생성됩니다.
...
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는: `Frequency cap rule: 5 SMS messages every 1 week`입니다.
{% endapi %}
...
{% api %}
## SMS 수신 이벤트 {#sms-inbound-received-events}
...
이 이벤트는 SMS 전송이 통신사에 의해 거부될 때 발생합니다. 이는 여러 가지 이유로 발생할 수 있습니다. 이 이벤트와 제공된 오류 코드를 사용하여 SMS 전송 관련 문제를 해결하세요.
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송에 대한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
{% endapi %}
...
- `dispatch_id`은 캠페인 전송과 같은 특정 메시지 전송에 대한 ID입니다. 같은 전송에서 발생하는 모든 푸시 이벤트는 동일한 `dispatch_id`를 포함합니다. `dispatch_id`을 사용하여 동일한 전송에 속하는 이벤트를 그룹화하여 해당 전송의 푸시 메시지 생애 주기를 그룹화하고 상관관계를 설정할 수 있습니다(예: 전송, 반송 및 열기).
- 메시지가 전역 빈도 제한 규칙으로 인해 중단된 경우 `abort_type`은 `frequency_capped`가 됩니다.
- `abort_log`은 중단을 유발한 특정 규칙에 대한 정보를 포함합니다. 예시는 다음과 같습니다: `Frequency cap rule: 5 webhook messages every 1 week`
{% endapi %}
...
이 이벤트는 WhatsApp 메시지가 사용자의 기기로 성공적으로 전송되었을 때 발생합니다.
...
이 이벤트는 WhatsApp 메시지가 사용자가 읽었을 때 발생합니다.
...
이 이벤트는 Braze와 WhatsApp 간에 전송 요청이 성공적으로 전달되었을 때 발생합니다. 그러나, 이것이 메시지가 사용자가 수신했음을 의미하지는 않습니다.
