...
> Braze는 엄선된 [파트너와]({{site.baseurl}}/user_guide/data/braze_currents/available_partners/) 함께 내부적으로 커런츠를 사용합니다.

이메일에서 데이터를 필터링하여 비즈니스 인사이트 도구인 Looker로 푸시 캠페인을 전송하지만, 그 경로가 약간 다릅니다. 우리는 추출, 변환, 로드(ETL) 방법론의 역방향 버전인 추출, 로드, 변환(ELT)으로 순서를 전환하여 사용합니다.
...
S3는 데이터 이동, 피벗, 분석을 위한 유연한 스토리지 및 라우팅 옵션을 제공합니다. 당사는 특정 구조를 유지하기 때문에 S3에서 데이터를 변환하지 않습니다.
...
Snowflake Reader Accounts는 사용자에게 Snowflake 계정이나 Snowflake와의 고객 관계를 요구하지 않고도 [Snowflake 데이터 공유]({{site.baseurl}}/partners/snowflake/)와 동일한 데이터 및 기능에 대한 액세스를 제공합니다. 리더 계정을 사용하면 Braze에서 데이터를 생성하여 계정으로 공유하고 로그인하여 데이터에 액세스할 수 있는 자격 증명을 제공합니다. This will result in all data sharing and usage billing being handled entirely by Braze. 
