...
메시지 보관은 추가 기능으로 제공됩니다. 메시지 아카이빙을 시작하려면 Braze 고객 성공 매니저에게 문의하세요.
...
{% alert note %}
메시지 보관을 위해 커런츠를 설정할 필요는 없으므로 파트너 설명서에서 해당 전제 조건을 건너뛸 수 있습니다.
{% endalert %}
...
다음은 메시징이 전송될 때마다 클라우드 스토리지 버킷에 전달되는 JSON 페이로드에 대한 참조입니다. 우리의 코드 예제 저장소에서 [메시지 아카이브 샘플 파일](https://github.com/braze-inc/braze-examples/tree/main/message-archiving)를 참조하십시오.
...
`extras` 필드에는 HTML 편집기에서 이메일을 작성할 때 **이메일 추가** 필드에 구성된 키-값 쌍이 포함됩니다. 이메일 추가 기능은 모든 이메일 서비스 제공업체(SendGrid 및 Sparkpost 포함)에서 작동하며, 어떤 제공업체를 사용하든 보관된 메시징에 포함됩니다. 이메일 추가 기능 구성에 대한 자세한 내용은 [이메일 캠페인 만들기를]({{site.baseurl}}/user_guide/message_building_by_channel/email/html_editor/creating_an_email_campaign/#adding-email-extras) 참조하세요. 커런츠로 데이터를 보내려면 [메시지 추가 정보]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/advanced_filters/message_extras/)를 참조하세요.
...
`external_id` 또는 `dispatch_id` 을 `user_id` 과 함께 사용하여 템플릿 메시지를 커런츠 데이터와 상호 참조하여 전달된 타임스탬프, 사용자가 메시지를 열었는지 또는 클릭했는지 여부 등 자세한 정보를 찾을 수 있습니다.
...
클라우드 저장소 자격 증명이 어느 시점에서든 유효하지 않은 경우, Braze는 클라우드 저장소 버킷에 메시지를 저장할 수 없으며 해당 메시지는 손실됩니다. 자격 증명 문제에 대한 알림을 받을 수 있도록 Amazon Web Services, Google Cloud Services 또는 Azure(Microsoft Cloud Services)에 대한 [알림 기본 설정을]({{site.baseurl}}/user_guide/administrative/app_settings/company_settings/notification_preferences/) 구성하는 것이 좋습니다.
...
### 메시지 보관 기능을 사용해 파일을 다른 작업 공간으로 그룹화할 수 있나요?

아니요. 메시지 보관에서는 작업 공간을 기준으로 파일 그룹화를 지원하지 않습니다. 대신 캠페인 또는 캔버스 단계 API ID가 속한 워크스페이스를 확인한 다음 해당 정보에 따라 그룹화할 수 있습니다.
