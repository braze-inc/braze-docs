...
사용자에게 최적의 전송 시간을 계산할 수 있는 관련 사용자 참여 데이터가 없는 경우, 대체 시간을 지정할 수 있습니다.
...
6. Specify a [fallback time](#campaign-fallback). 사용자 프로필에 최적의 시간을 계산할 수 있는 관련 이벤트가 없는 경우 메시징이 전송됩니다.

![대체 시간 및 방해금지 시간 설정이 포함된 Intelligent Timing이 표시된 캠페인 스케줄링 화면]({% image_buster /assets/img/intelligent_timing/campaign_scheduling.png %})
...
![밤새 메시지 전달을 차단하기 위해 시작 및 종료 시간을 설정하여 방해금지 시간 토글을 켭니다.]({% image_buster /assets/img/intelligent_timing/quiet_hours.png %})
...
![Android 푸시의 전달 미리보기 차트는 오후 12시에서 2시 사이에 참여가 가장 많은 시간대를 보여주며, 가장 인기 있는 앱 시간대는 오후 2시입니다.]({% image_buster /assets/img/intel-timing-preview.png %})
...
![캠페인이 1~4일 전에 앱을 처음 사용한 사용자를 타겟팅하는 타겟 오디언스 필터입니다.]({% image_buster /assets/img/intelligent_timing/first_used_app.png %})
...
![우승 기준, 전송 날짜 및 현지 전송 시간을 선택한 상태에서 우승 배리언트를 선택한 A/B 테스트를 보여주는 A/B 테스트 섹션]({% image_buster /assets/img/intelligent_timing/ab_testing_intelligent_timing.png %})
...
When specified, Braze only uses engagement data within that window to determine a user's optimal delivery time. 해당 기간 내에 관련 이벤트가 없는 경우 설정한 대체 시간에 메시징이 전송됩니다.
...
사용자 프로필에 관련 이벤트가 없는 경우 최적의 전달 시간을 계산하기 위해 사용할 대체 시간을 선택합니다.
...
![Android 푸시의 전달 시간 미리보기 예시입니다.]({% image_buster /assets/img/intel-timing-preview.png %})

Intelligent Timing 또는 캠페인 오디언스에 대한 설정을 변경할 때마다 데이터를 다시 새로고침하여 업데이트된 차트를 확인합니다.

차트에는 최적의 시간을 계산할 수 있는 관련 이벤트가 있는 사용자를 파란색으로, 대체 시간을 사용할 사용자를 빨간색으로 표시합니다. 계산 필터를 사용하여 미리 보기 보기를 조정하여 두 사용자 그룹을 더 세밀하게 살펴볼 수 있습니다.
{% endtab %}
...
Braze가 최적의 전송 시간을 계산할 수 있도록 관련 참여 데이터가 없는 오디언스의 사용자에게 보낼 메시징의 대체 시간을 선택합니다. {% multi_lang_include brazeai/intelligent_suite/fallback_time.md %}
...
![사용자의 최적 시간이 오후 2시인데 오후 2시 01분에 지연 단계를 입력하고 지연이 2일로 설정된 경우 일과 달력 날짜의 차이를 나타내는 그래픽입니다. Days는 사용자가 최적의 시간 이후에 단계를 입력했기 때문에 3일 후에 메시지를 전달하지만, 캘린더 날짜는 2일 후인 지연의 마지막 날에 메시지를 전달합니다.]({% image_buster /assets/img/intelligent_timing_daysvcalendardays.png %}){: style="border:none;"}
{% endtab %}
{% endtabs %}
...
사용자에 대한 관련 이벤트가 없는 경우(예: 참여도가 거의 또는 전혀 없는 신규 사용자), Braze는 구성된 대체 설정(커스텀 대체 시간 또는 모든 사용자 중 가장 많이 앱을 사용하는 시간)을 사용합니다.
...
#### 관련 이벤트가 없는 경우 대체 계산은 어떻게 되나요?

사용자와 관련된 이벤트가 없는 경우 Intelligent Timing은 메시지 설정에서 구성된 대체 설정(커스텀 대체 시간 또는 모든 사용자 중에서 앱을 가장 많이 사용하는 시간)을 사용합니다. 
...
사용자에 대한 관련 참여 이벤트가 없는 경우(예: 클릭 또는 실행 횟수가 거의 없거나 전혀 없는 신규 사용자) Intelligent Timing은 구성된 대체 설정(커스텀 대체 시간 또는 모든 사용자 중에서 앱을 가장 많이 사용하는 시간)을 사용합니다.
...
Rate limiting can be used on a campaign that uses Intelligent Timing. 그러나 속도 제한의 특성상 일부 사용자는 관련 이벤트가 없어 대체 시간에 속도 제한 규모에 비해 많은 수의 사용자가 예약되어 있는 경우 최적이 아닌 시간에 메시지를 수신할 수 있습니다. 
