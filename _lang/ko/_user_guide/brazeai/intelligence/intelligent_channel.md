...
> 지능형 또는 `Most Engaged` 채널 필터는 선택한 메시징 채널이 "최고의" 채널인 오디언스의 일부를 선택합니다. 

이 경우, 최선은 사용자의 기록을 고려할 때 인게이지먼트 가능성이 가장 높은 채널을 의미합니다. 이메일, SMS, 웹 푸시 또는 모바일 푸시(사용 가능한 모바일 OS 또는 기기 포함)를 채널로 선택할 수 있습니다.
...
인텔리전트 채널 필터를 활성화하려면 이메일, 웹 푸시 또는 모바일 푸시 캠페인을 생성할 때 **대상 사용자** 페이지에서 **인텔리전트 채널** 필터를 선택하세요.

{% alert important %}
SMS 채널의 참여율을 계산하려면 고급 추적 및 클릭 추적과 함께 [SMS 링크 단축]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/link_shortening/#overview/)을 켜세요.
{% endalert %}
...
예를 들어, 푸시 메시지를 선호하는 사용자가 푸시를 받도록 하고, 데이터가 충분하지 않은 사용자가 동일한 푸시 메시지를 받도록 하려는 경우를 가정해 보겠습니다. 그 경우, 인텔리전트 채널 필터를 **모바일**로 설정하고 **OR**을 사용하여 두 번째 인텔리전트 채널 필터를 **데이터 부족**으로 설정할 수 있습니다. 이메일을 선호하는 사용자를 대상으로 하는 별도의 캠페인은 인텔리전트 채널 필터를 이메일로 설정할 수 있습니다.
...
Braze가 사용자에게 가장 적합한 단일 채널을 선택하도록 하는 대신, 사용자가 선택한 특정 채널에서 메시지를 열 가능성이 있는지 여부에 따라 사용자를 필터링하고 싶을 수도 있습니다. 이를 위해 [세분화 필터]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters)에서 메시지 열람 가능성 필터를 사용할 수 있습니다.
...
[1]: {% image_buster /assets/img/intelligent_channel_filter.png %} "인텔리전트 채널 필터"
