...
# [![브레이즈 학습 과정]({% image_buster /assets/img/bl_icon3.png %})](https://learning.braze.com/most-engaged-channel){: style="float:right;width:120px;border:0;" class="noimgborder"}인텔리전트 채널 필터
...
인텔리전트 채널은 지난 6개월 동안의 활동에서 수신한 메시지 수에 대한 메시지 상호작용(열기 또는 클릭)의 비율을 계산하여 각 사용자의 각 사용 가능한 채널에 대한 참여율을 계산합니다. 사용 가능한 채널은 각각의 인게이지먼트 비율에 따라 순위가 매겨지며, 가장 높은 비율을 가진 채널이 해당 사용자의 "가장 많이 참여한 채널"입니다. 
...
Braze가 어떤 채널이 "최고"인지 결정하려면 충분한 데이터가 필요합니다. 이는 사용자가 세 개의 사용 가능한 채널 중 최소 두 개의 채널에서 채널당 세 개 이상의 메시지를 수신해야 함을 의미합니다. 메시지가 반드시 열려 있을 필요는 없습니다. 
...
![인텔리전트 채널은 모바일 푸시 필터 또는 데이터가 충분하지 않습니다.]({% image_buster /assets/img/intelligent_example.png %}){:style="border:none"}
