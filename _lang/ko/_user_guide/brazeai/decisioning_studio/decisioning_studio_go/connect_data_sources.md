...
- **오디언스 데이터는** CEP에 정의된 세그먼트 또는 목록(Braze, Salesforce 마케팅 클라우드 또는 클라비요)에서 직접 가져오며, 1P 데이터가 아닌 사전 정의된 특정 속성만 포함할 수 있습니다.
...
클라비요 통합을 위해서는 Decisioning Studio Go에 다음이 필요합니다:

1. **세그먼트 액세스**: 오디언스는 API 키가 액세스할 수 있는 클라비요 세그먼트로 정의되어야 합니다.
2. **프로필 데이터**: 고객 속성을 읽으려면 API 키에 프로필에 대한 전체 액세스 권한이 있어야 합니다.
3. **측정기준 액세스**: 참여 데이터를 캡처하려면 API 키에 측정기준 및 이벤트에 대한 전체 액세스 권한이 있어야 합니다.
