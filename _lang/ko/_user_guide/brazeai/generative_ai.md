---
nav_title: 생성형 AI
article_title: 생성형 AI
page_order: 5
layout: dev_guide
guide_top_header: "생성형 AI"
guide_top_text: "BrazeAI<sup>TM</sup>은 효율성과 창의성을 높이는 데 도움이 되는 다양한 자동화 생성 AI 기능을 제공합니다. 일상적인 카피라이팅 작업, 콘텐츠 개선, 이미지 제작에 소요되는 시간을 줄이면서 매력적인 크로스채널 고객 경험을 창출하세요."
description: "BrazeAI<sup>TM</sup>은 효율성과 창의성을 높이는 데 도움이 되는 다양한 자동화 생성 AI 기능을 제공합니다."

guide_featured_title: "기능"
guide_featured_list:
- name: AI 카피라이팅 어시스턴트
  link: /docs/user_guide/brazeai/generative_ai/copywriting/
  image: /assets/img/braze_icons/edit-05.svg
- name: AI 이미지 생성기
  link: /docs/user_guide/engagement_tools/templates_and_media/media_library/#generate-ai
  image: /assets/img/braze_icons/image-03.svg
- name: AI 콘텐츠 QA
  link: /docs/user_guide/brazeai/generative_ai/content_qa/
  image: /assets/img/braze_icons/clipboard-check.svg
- name: AI Liquid 어시스턴트
  link: /docs/user_guide/brazeai/generative_ai/liquid/
  image: /assets/img/braze_icons/beaker-02.svg
- name: SQL 쿼리 빌더
  link: /docs/user_guide/analytics/query_builder/
  image: /assets/img/braze_icons/tool-01.svg
- name: SQL Segment Extensions
  link: /docs/sql_segments/
  image: /assets/img/braze_icons/users-01.svg

---