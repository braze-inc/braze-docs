...
| 기간 | Definition |
| --- | --- |
| [모델]({{site.baseurl}}/user_guide/brazeai/agents/reference/#models) | 에이전트의 '두뇌', 이 경우에는 대규모 언어 모델(LLM)입니다. 입력을 해석하고, 응답을 생성하며, 추론을 수행합니다. 더 강력한 모델(더 많은 관련 데이터로 학습)은 에이전트의 능력과 활용도를 높여줍니다. |
| [지침]({{site.baseurl}}/user_guide/brazeai/agents/reference/#writing-instructions) | 상담원에게 제공하는 규칙 또는 가이드라인(시스템 프롬프트)입니다. 에이전트가 실행될 때마다 에이전트가 어떻게 작동해야 하는지 정의합니다. 명확한 지침은 상담원의 신뢰성과 예측 가능성을 높여줍니다. |
...
| [출력 형식]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents/#step-3-select-the-output) | 상담원 응답의 미리 정의된 데이터 구조입니다. |
| [온도]({{site.baseurl}}/user_guide/brazeai/agents/reference/#temperature) | 상담원 출력의 편차 수준입니다. 이는 상담원이 얼마나 정확하고 창의적으로 상담할 수 있는지를 정의합니다. |
