...
AI 품목 추천을 사용하여 가장 인기 있는 제품을 계산하거나 특정 [카탈로그에]({{site.baseurl}}/user_guide/data/activation/catalogs/) 대한 개인화된 AI 추천을 생성할 수 있습니다. 추천을 생성한 후 개인화를 사용하여 해당 제품을 메시지에 삽입할 수 있습니다.
...
!["추천 정의" 단계에서 "AI 개인화된"을 유형으로 선택하고 "사용자가 이전에 상호작용한 항목을 추천하지 않음" 옵션을 선택합니다.]({% image_buster /assets/img/item_recs_2-3.png %})
...
추천에 대한 더 많은 제어를 원하시면, [선택]({{site.baseurl}}/user_guide/data/activation/catalogs/selections/)를 선택하여 커스텀 필터를 적용하세요. 선택 항목은 카탈로그의 특정 열, 예를 들어 브랜드, 크기 또는 위치에 따라 추천을 필터링합니다. Liquid이 포함된 선택 항목은 추천에 사용할 수 없습니다.
...
{% alert note %}
커스텀 이벤트는 이벤트 목록에 나타나기 전에 충분한 데이터가 필요합니다. 커스텀 이벤트가 나타나지 않는 경우, Braze 백엔드에서 아직 처리되지 않았거나 모델 훈련을 위한 충분한 데이터가 부족할 수 있습니다. AI 추천은 통찰력을 생성하기 위해 역사적 데이터에 의존하므로, 새로 생성되거나 드물게 트리거된 이벤트는 더 많은 데이터가 수집될 때까지 사용할 수 없습니다.
{% endalert %}
...
![카탈로그의 항목 ID에 해당하는 선택된 속성 이름 "purchase_item".]({% image_buster /assets/img/item_recs_4.png %})
...
- **항목 추천을 훈련하기 위해 [eCommerce 이벤트]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events/)를 사용하는 경우:** 이벤트에서 제품 ID에 접근하려면 `products.product_id`을 추가하세요.
