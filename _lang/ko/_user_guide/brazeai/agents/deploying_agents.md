...
![캔버스에 대한 활성 에이전트 2개와 비활성 에이전트 1개를 표시하는 에이전트 사용량 섹션입니다.]( {% image_buster /assets/img/ai_agent/agent_usage.png %} )
...
| 제품 속성 강화하기 | 제품 이름과 세부 정보를 기반으로 색상군, 스타일, 계절 등의 누락된 값을 입력합니다. 예를 들어 제품 이름이 "라구나 편광 선글라스"인 경우 상담원은 스타일을 "스포츠"로 지정하고 색상군을 "파란색"으로 지정할 수 있습니다. |
| 파생 필드 계산 | 기존 필드를 사용하여 속성에 따른 '적합도 점수' 또는 판매 및 리뷰 수에 따른 '인기도 태그'와 같은 새로운 데이터를 생성할 수 있습니다. |
...
![카탈로그 필드에 'AI 에이전트 적용'을 선택하는 옵션입니다.]({% image_buster /assets/img/ai_agent/edit_agent_column.png %}){: style="max-width:80%;"}
...
상담원이 [필드를]({{site.baseurl}}/user_guide/brazeai/agents/creating_agents/#fields) 출력 형식으로 사용하는 경우에는 **응답 필드에** 대한 상담원에서 해당 필드를 선택하여 카탈로그 필드에 사용할 수 있습니다. 
