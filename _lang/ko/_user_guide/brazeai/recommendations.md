...
{% alert note %}
선택 항목을 사용하는 경우 정렬 필드 및 제한은 AI 항목 추천에 사용되지 않습니다. 즉, 특정 정렬 필드가 있는 선택 항목을 만들고 반환되는 항목 수를 제한하면 AI 항목 추천이 처리될 때 이러한 제약 조건이 사용되지 않습니다.
{% endalert %}
...
### 여러 날 동안 지속되는 모든 추천을 한꺼번에 만료하려면 어떻게 해야 하나요?

특정 날짜에 모든 다일 추천을 만료하여 모든 활성 추천이 한 번에 새 예측을 받도록 하려면 Braze 지원팀이나 고객 성공 매니저에게 문의하여 도움을 요청하세요. Braze AI 전문가가 수동으로 수행하여 모델 성능/성과를 극대화합니다.
