...
{% alert note %}
커스텀 이벤트는 이벤트 목록에 표시되기 전에 충분한 데이터가 있어야 합니다. 커스텀 이벤트가 표시되지 않는다면 Braze 백엔드에서 아직 이벤트를 처리하지 않았거나 모델 학습을 위한 고객 데이터가 부족하기 때문일 수 있습니다. AI 추천은 과거 데이터에 의존하여 인사이트를 생성하므로 새로 생성되거나 드물게 트리거되는 이벤트는 더 많은 데이터가 수집될 때까지 사용할 수 없습니다.
{% endalert %}
...
![카탈로그의 항목 ID에 해당하는 속성 이름 "purchase_item" 을 선택합니다.]({% image_buster /assets/img/item_recs_4.png %})
...
- **[이커머스 이벤트를]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events/) 사용하여 품목 추천을 훈련하는 경우:** `products.product_id` 을 추가하여 이벤트에서 제품 ID에 액세스합니다.
