---
nav\_title: 항목 추천 정보 article\_title: 항목 추천 설명 정보: "이 참조 문서에서는 Braze를 사용하여 고객에게 아이템을 추천하는 다양한 사용 사례에 대해 설명합니다." page\_order: 10
---

# 항목 추천에 대하여

이 기사에서는 고객이 관심을 가질 만한 항목을 제안하는 다양한 방법과 Braze를 사용하여 추천 엔진을 만드는 일반적인 사용 사례에 대한 영감을 얻는 방법을 배웁니다.

## 필수 조건

모든 추천 유형은 최소한 하나의 [카탈로그]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/catalog/)를 설정해야 합니다. 추천 항목이 거기에서 가져오기 때문입니다.

## 추천 유형

고객에게 추천할 항목을 설정할 때 사용할 수 있는 몇 가지 접근 방식이 있습니다.
...
[AI 항목 추천][1] 기능의 일부인 AI 개인화된 추천은 딥러닝을 활용하여 사용자가 과거에 관심을 보인 것을 바탕으로 다음에 관심을 가질 가능성이 높은 것을 예측합니다. 이 방법은 사용자 행동에 적응하는 동적 맞춤형 추천 시스템을 제공합니다.
...
AI 항목 추천을 사용하면 [선택]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/selections/) 가능한 항목을 추가로 필터링할 수도 있습니다. 추가로 필터링할 수도 있습니다. 그러나 Liquid을 사용한 선택은 AI 추천에 사용할 수 없으므로 카탈로그 선택을 구성할 때 이를 염두에 두세요.

예시는 이 기사에서 [사용 사례](#use-cases) 섹션을 참조하세요.

{% 알림 팁 %} AI 개인화 추천은 수백 또는 수천 개의 품목과 일반적으로 구매 또는 상호 작용 데이터가 있는 최소 30,000명의 사용자가 있을 때 가장 잘 작동합니다. 이는 대략적인 가이드이며 변동될 수 있습니다. 다른 권장 사항 유형은 더 적은 데이터로 작동할 수 있습니다. {% endalert %}

### 가장 인기 있는 아이템 추천 {#most-popular}

"AI 개인화된" 모델 외에도, [AI 항목 추천][1] 기능에는 사용자가 가장 많이 상호작용하는 항목을 특징으로 하는 "가장 인기 있는" 추천 모델도 포함되어 있습니다.

추적 중인 상호 작용 데이터를 기반으로 이 모델의 사용 사례에는 추천이 포함될 수 있습니다.

- [가장 인기 있는 항목](#most-popular-items)
- [가장 많이 좋아요를 받은 항목](#liked-items)
- [가장 많이 본 항목](#most-viewed-items)
- [사용자 장바구니의 인기 상품](#popular-items-in-users-carts)

### 가장 최근 항목 추천 {#최근}

"AI 개인화된" 모델 외에도, [AI 항목 추천][1] 기능에는 사용자가 가장 많이 상호작용하는 항목을 특징으로 하는 "가장 최근" 추천 모델도 포함되어 있습니다. 이 모델을 사용하여 고객 이탈을 줄이고, 휴면 사용자가 관련 콘텐츠에 다시 참여하도록 유도하세요.
...
- [장바구니에 최근 추가된 항목](#items-recently-added-to-cart)

### 인기 있는 항목 추천 {#트렌딩}

"AI 개인화된" 모델 외에도, [AI 항목 추천][1] 기능에는 "트렌딩" 추천 모델도 포함되어 있습니다. 이 모델은 최근 사용자 상호작용에서 가장 긍정적인 모멘텀을 보인 항목들을 특징으로 합니다. 

"가장 인기 있는" 모델과 달리, 지속적으로 높은 상호작용을 특징으로 하는 이 모델은 상호작용이 증가한 항목을 특징으로 합니다. 이를 사용하여 최근 주목받고 있는 인기 상품을 추천할 수 있습니다.

추적 중인 상호 작용 데이터를 기반으로 이 모델의 사용 사례에는 추천이 포함될 수 있습니다.

- [인기 구매 품목](#trending-purchased-items)
- [트렌딩 좋아요 항목](#trending-liked-items)
...
- **첫 구매자:** 첫 구매자에게 스타터 키트나 입문용 상품을 추천하여 두 번째 구매를 유도하세요. 로열티 프로그램: 고객의 현재 포인트 잔액을 기준으로 로열티 포인트 또는 보상을 최대화할 수 있는 제품을 강조 표시합니다.
- **교육 콘텐츠:** 이전에 소비하거나 구매한 자료의 주제를 기반으로 새로운 과정이나 콘텐츠를 제안합니다.

## 사용 사례

### 사용자가 다음에 구매할 가능성이 가장 높은 항목
...
1. [AI 항목 추천]({{site.baseurl}}/user_guide/brazeai/recommendations/ai_item_recommendations/)을 생성하세요.
