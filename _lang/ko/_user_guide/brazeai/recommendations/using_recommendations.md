---
nav\_title: 항목 추천 article\_title 사용: 메시징 설명에 항목 추천 사용하기: "이 문서에서는 메시지에서 항목 추천을 사용하는 방법에 대해 설명합니다." page\_order: 20
---

# 메시징에서 항목 추천 사용하기

> 추천이 학습된 후에는 Liquid를 사용하여 추천 항목을 가져와서 메시지에 표시할 수 있습니다. 여기서 핵심은 `제품_추천` 리퀴드 객체로 직접 작업하는 것입니다. 이 문서에서는 `제품_권장` 리퀴드 객체를 다루며, 해당 지식을 실제로 적용하는 데 도움이 되는 튜토리얼이 포함되어 있습니다.

{% 경고 팁 %} 이 문서에서는 Liquid 객체의 구문에 대해 자세히 설명합니다. 그러나 템플릿 텍스트 필드의 오른쪽 상단에 있는 **개인화 추가** 모달을 통해 기본값이 있는 [미리 서식이 지정된 변수를 삽입할]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/using_liquid/#inserting-pre-formatted-variables) 수 있습니다. {% 끝알람 %}
...
`제품_추천` 객체는 모델에서 추천하는 항목의 집합을 나타냅니다. 각 개체가 추천 항목을 나타내는 객체 배열로 구성된 관련 카탈로그에서 직접 데이터를 제공합니다.

- **구조:** 각 항목은 `items[index]`로 액세스되며, 인덱스는 0(첫 번째 항목의 경우)에서 시작하여 이후 항목에 대해 증가합니다.
...
`제품_추천` 개체에는 동적으로 생성된 제품 추천이 포함되어 있습니다. Liquid에서 이러한 데이터에 액세스하려면 메시지에서 데이터를 사용하기 전에 먼저 변수에 데이터를 할당해야 합니다.

### 추천 데이터 할당

항상 할당 태그로 시작하여 `제품_추천` 데이터를 가져와 변수에 저장합니다.
...
- `추천_이름`: 이 이름을 Braze에서 생성한 AI 추천의 이름으로 바꿉니다.
- `항목`: 추천 항목 배열을 저장하는 변수입니다.
...
```액체 {% 할당 품목 = {{product_recommendation.${RECOMMENDATION_NAME}}} %} {{ items[0].name }} for {{ items[0].price }} ```

{% endraw %}

여러 항목을 포함하려면 각 항목을 인덱스로 개별적으로 참조합니다. `.name` 및 `.price는` 카탈로그에서 해당 필드를 가져옵니다. 

{% 원시 %}

```액체 {% 할당 품목 = {{product_recommendation.${RECOMMENDATION_NAME}}} %} {{ items[0].name }} for {{ items[0].price }} {{ items[1].name }} for {{ items[1].price }} {{ items[2].name }} for {{ items[2].price }} ```

{% endraw %}

AI 추천은 여러 제품을 배열로 반환하며, 여기서 `items[0]` 은 첫 번째 항목, `items[1` ]은 두 번째 항목과 같은 방식으로 반환합니다. 권장 사항에서 하나의 항목만 반환하는 경우 `항목[1]` 을 참조하려고 하면 빈 필드가 생성됩니다.
...
{% 탭 끌어서 놓기%} 이미지 필드가 있는 작성기의 경우 작성기의 해당 필드에 다음 Liquid를 추가합니다:
...
\![드래그 앤 드롭 편집기의 이미지 속성 패널\]({% image\_buster /assets/img/image\_with\_liquid.png %}){: style="max-width:60%"}
...
HTML 이미지 참조의 경우 이미지 `src` 속성을 카탈로그의 이미지 URL 필드에 설정합니다. 제품 이름이나 설명과 같은 다른 필드를 대체 텍스트로 사용할 수 있습니다.
...
-  `내_추천_이름을` 추천의 이름으로 바꿉니다.
- `IMAGE_URL_FIELD를` 카탈로그에서 이미지 URL이 포함된 필드 이름으로 바꿉니다.


## 튜토리얼: 버려진 장바구니 이메일 생성
...
| 필드 | 데이터 유형 | 설명 | --- | --- | 아이디 | 문자열 | 카탈로그의 각 품목에 대한 고유 식별자 | | 이름 | 문자열 | 제품 이름(예: "스트라이프 니트 스웨터"). | 가격 | 숫자 | 제품 가격(예: "49.99"). | image\_url | 문자열 | 제품 이미지를 가리키는 URL입니다. HTTPS로 보안되어야 합니다. 이미지가 미디어 라이브러리에서 호스팅되는 경우 자산 위로 마우스를 가져가 URL을 복사합니다. | 카테고리 | 문자열 | "스웨터" 또는 "액세서리" 등의 제품 카테고리 | 색상 | 문자열 | "네이비/회색" 등의 제품 설명 색상 | {: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="프레젠테이션" }
...
```액체 {% 항목 할당 = {{product_recommendation.${abandoned_cart}}} %} {{ items[0].image_url }} ```
...
```액체 {% 품목 할당 = {{product_recommendation.${abandoned_cart}}} %} {{ items[0].name }} {{ items[0].category }} {{ items[0].color }} items[0].price }} ```

{% endraw %}

{: start="7"}

7. 두 스니펫 모두에 대해 Braze에서 `abandoned_cart를` 추천 이름으로 바꿉니다.
8. 항목 필드 이름`({{ items[0].field_name }}`)이 카탈로그의 열 이름과 일치하는지 다시 한 번 확인합니다.
9. 블록을 반복할 때마다 배열을 하나씩 증가시켜 카탈로그에서 다음 추천 항목을 가져옵니다. 예를 들어 배열이 `{{ items[0].name }}으로` 시작하므로 다음 항목은 `{{ items[1].name }}이` 됩니다.
