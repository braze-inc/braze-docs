...
[AI 아이템 추천]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/) 기능의 일환으로, AI 개인화된 추천은 딥 러닝을 활용하여 사용자가 과거에 관심을 보인 내용을 바탕으로 다음에 가장 관심을 가질 가능성이 높은 항목을 예측합니다. 이 방법은 사용자 행동에 적응하는 동적 맞춤형 추천 시스템을 제공합니다.

AI 개인화된 추천은 구매나 커스텀 이벤트와 같은 항목 상호작용 데이터의 지난 6개월을 사용하여 추천 모델을 구축합니다. 개인화된 목록에 대한 데이터가 충분하지 않은 사용자에게는 가장 인기 있는 항목이 대체로 제공되어 사용자가 여전히 관련된 제안을 받을 수 있습니다.
...
1. [AI 항목 추천]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/)을 생성하세요.
...
"트렌딩" 추천 모델은 최근 사용자 상호작용에서 가장 긍정적인 모멘텀을 보인 항목을 특징으로 합니다. 이는 약 10주간의 이벤트 기록에 대한 가중 분석을 사용하여 계산되며, 가장 최근의 약 2주에 가장 높은 가중치가 적용됩니다. 작은 변동이 추천 품질에 영향을 미치지 않도록 활동 임계값과 통계적 스무딩 기법을 적용합니다.
...
6. 추천을 훈련시킵니다.
7. [메시징에서 권장 사항을 사용합니다.]({{site.baseurl}}/user_guide/brazeai/item_recommendations/using_recommendations)
{% enddetails %}
{% endtab %}
...
{% alert note %}
선택을 사용하는 경우, 정렬 필드와 모든 제한은 AI 아이템 추천에 사용되지 않습니다. 이는 특정 정렬 필드로 선택을 생성하고 반환되는 항목 수를 제한하면, 이러한 제약 조건이 AI 아이템 추천이 처리될 때 사용되지 않음을 의미합니다.
{% endalert %}
...
규칙 기반 추천은 수동으로 설정해야 하는 고정된 논리에 기반합니다. 이는 추천이 사용자의 개별 구매 이력과 취향에 맞게 조정되지 않음을 의미하며, 로직을 업데이트하지 않는 한; 따라서 이 방법은 자주 업데이트가 필요하지 않은 추천에 가장 적합합니다.
...
추천은 독립적으로 작동하며 다른 모델이 추천하는 내용을 알지 못합니다. 이는 각 섹션이 동일한 이메일의 다른 AI 추천 섹션에 이미 표시된 중복 항목을 가질 수 있음을 의미합니다.
...
### 여러 날 지속되는 모든 추천을 한 번에 만료시키려면 어떻게 해야 하나요?

특정 날짜에 모든 다일 추천을 만료시키고 싶다면(모든 활성 추천이 한 번에 새로운 예측을 받도록), Braze 지원팀이나 고객 성공 매니저에게 도움을 요청하세요. Braze AI 전문가가 수동으로 이 작업을 수행하여 최대 모델 성능을 보장합니다.
