...
인텔리전트 채널은 지난 6개월 동안의 활동에서 수신한 메시지 수에 대한 메시지 상호작용(열기 또는 클릭)의 비율을 취하여 각 사용자의 각 사용 가능한 채널에 대한 참여율을 계산합니다. 사용 가능한 채널은 각각의 인게이지먼트 비율에 따라 순위가 매겨지며, 가장 높은 비율을 가진 채널이 해당 사용자의 "가장 많이 참여한 채널"입니다. 
...
Braze가 어떤 채널이 "최고"인지 결정하려면 충분한 데이터가 필요합니다. 이는 사용자가 세 개의 사용 가능한 채널 중 두 개 이상의 채널에서 채널당 최소 세 개 이상의 메시지를 수신해야 함을 의미합니다. 메시지가 반드시 열려 있을 필요는 없습니다. 
...
Braze가 사용자에게 가장 적합한 단일 채널을 선택하도록 하는 대신, 사용자가 선택한 특정 채널에서 메시지를 열 가능성이 있는지 여부에 따라 사용자를 필터링하기 위해 ["메시지 열기 가능성" 세분화 필터]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#message-open-likelihood)를 사용할 수 있습니다. 이 필터는 상호작용 비율을 채널당 마지막 100개의 메시지 수신 총계로 나눈 비율로 계산됩니다.
