...
Braze는 사용자의 앱 과거 상호작용 및 각 메시징 채널과의 상호작용에 대한 통계적 분석을 바탕으로 최적의 발송 시간을 계산합니다. The following interaction data is used: 
...
- SMS 클릭 수 ([링크 단축]({{site.baseurl}}/user_guide/message_building_by_channel/sms_mms_rcs/link_shortening/) 및 고급 추적 기능이 인에이블먼트된 경우에만)
...
6. Specify a [fallback time](#campaign-fallback). 고객 프로필에 최적 시간을 계산할 관련 이벤트가 없을 경우 메시지가 전송되는 시점입니다.

![캠페인 스케줄링 화면에 표시된 Intelligent Timing(대체 시간 및 방해금지 시간 설정 포함)]({% image_buster /assets/img/intelligent_timing/campaign_scheduling.png %})

#### Quiet Hours {#quiet-hours}

방해금지 시간을 설정하여 특정 시간대에 메시지가 전송되지 않도록 방지하세요. This is helpful when you want to avoid sending messages during early morning hours or overnight, while still allowing Intelligent Timing to determine the best delivery window.
...
![방해금지 시간 토글이 켜져 있으며 시작 및 종료 시간이 설정되어 밤새 메시지 전달을 차단합니다]({% image_buster /assets/img/intelligent_timing/quiet_hours.png %})
...
![Android 푸시 알림의 전달 미리보기 차트에 따르면, 최대 참여 시간대는 오후 12시부터 2시 사이이며, 가장 인기 있는 앱 사용 시간은 오후 2시입니다.]({% image_buster /assets/img/intel-timing-preview.png %})

### 2단계: Choose a send date

Next, select a send date for your campaign. Intelligent Timing으로 캠페인을 예약할 때 다음 사항을 유념하십시오:

#### Launch campaign 48 hours in advance

예정된 전송 날짜 최소 48시간 전에 캠페인을 시작하세요. 이는 시간대가 다르기 때문입니다. Braze는 세계에서 이른 표준 시간대 중 하나인 사모아 시간(UTC+13)의 자정을 기준으로 최적의 시간을 계산합니다. 전 세계적으로 하루는 약 48시간으로 구성됩니다. 즉, 이 48시간의 여유 시간 내에 캠페인을 시작하면 사용자의 시간대에서 최적의 시간이 이미 지나 메시지가 전송되지 않을 수 있습니다.
...
![캠페인이 1일에서 4일 전에 앱을 처음 사용한 사용자를 대상으로 하는 오디언스 필터]({% image_buster /assets/img/intelligent_timing/first_used_app.png %})
...
#### A/B 테스트 종료 2일 후 승리한 배리언트 일정 설정
...
![A/B 테스트 섹션: 승리한 배리언트 선택, 승리 기준, 발송 날짜 및 현지 발송 시간 선택 상태의 A/B 테스트 표시]({% image_buster /assets/img/intelligent_timing/ab_testing_intelligent_timing.png %})

### 3단계: Choose a delivery window (optional)
...
When specified, Braze only uses engagement data within that window to determine a user's optimal delivery time. 해당 시간대 내에 관련 이벤트가 없으면 메시지는 설정된 대체 시간에 발송됩니다.
...
### 4단계: Choose a fallback time {#campaign-fallback}

고객 프로필에 최적의 전달 시간을 계산할 관련 이벤트가 없는 경우 사용할 대체 시간을 선택하십시오.
...
![Android 푸시 알림 전달 시간 예시 미리보기]({% image_buster /assets/img/intel-timing-preview.png %})

Intelligent Timing 또는 캠페인 오디언스에 대한 설정을 변경할 때마다 데이터를 다시 새로고침하여 업데이트된 차트를 확인합니다.

차트는 최적 시간을 계산할 관련 이벤트가 있는 사용자를 파란색으로, 대체 시간을 사용할 사용자를 빨간색으로 표시합니다. 계산 필터를 사용하여 미리 보기 보기를 조정하여 두 사용자 그룹을 더 세밀하게 살펴볼 수 있습니다.
{% endtab %}
...
Braze가 최적의 발송 시간을 계산할 수 있는 관련 참여 데이터가 없는 오디언스 사용자들에게 메시지를 발송할 대체 시간을 선택하십시오. {% multi_lang_include brazeai/intelligent_suite/fallback_time.md %}

### 4단계: Add a Delay step
...
대신, 사용자가 캔버스에 진입한 시점과 Intelligent Timing 단계를 수신하는 시점 사이에 최소 2일 이상의 캘린더 일정을 [지연 단계로]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/delay_step/) 추가하십시오.
...
Intelligent Timing을 사용할 때는 지연 시간으로 24시간 단위 대신 캘린더 일자를 사용하는 것이 좋습니다. 이는 캘린더 일수를 기준으로 할 경우, 메시지가 지연 기간의 마지막 날에 최적의 시간에 발송되기 때문입니다. 하루가 24시간인 경우, 사용자의 최적 시간이 해당 단계 진입 전일 가능성이 있으며, 이는 지연 시간에 하루가 추가된다는 것을 의미합니다.
...
![사용자의 최적 시간이 오후 2시이지만, 지연 단계에 오후 2시 1분에 진입하고 지연이 2일로 설정된 경우, 날짜와 캘린더 일의 차이를 보여주는 그래픽. 사용자가 최적 시간 이후에 단계를 입력했기 때문에 '일' 단위는 3일 후에 메시지를 전달하는 반면, '캘린더일' 단위는 지연 기간의 마지막 날인 2일 후에 메시지를 전달합니다.]({% image_buster /assets/img/intelligent_timing_daysvcalendardays.png %}){: style="border:none;"}
{% endtab %}
{% endtabs %}

## Limitations

- 인앱 메시지와 웹훅은 즉시 전달되며 최적의 시간에 제공되지 않습니다.
...
사용자에게 관련 이벤트가 없는 경우(예: 참여도가 거의 없거나 전혀 없는 신규 사용자), Braze는 설정된 대체값을 사용합니다. 이는 커스텀 대체 시간 또는 전체 사용자 중 앱 사용 빈도가 가장 높은 시간 중 하나입니다.

### 시간대가 Intelligent Timing 전달에 미치는 영향

Intelligent Timing은 각 사용자의 지정된 현지 시간대를 기반으로 하므로, 예약된 전달 날짜와 시간은 사용자마다 다를 수 있습니다.

사용자가 예상대로 메시지를 받지 못하는 경우, 프로필의 시간대 필드가 올바르게 설정되었는지 확인하십시오. 시간대 필드가 비어 있는 경우, 사용자는 현지 시간이 아닌 회사의 시간대에 맞춰진 메시지를 수신할 수 있습니다.
...
#### 관련 이벤트가 없을 경우 대체 계산 방법은 무엇입니까?

사용자에게 관련 이벤트가 없는 경우, Intelligent Timing은 메시지 설정에서 구성된 대체 설정을 사용합니다. 이는 커스텀 대체 시간 또는 모든 사용자 중 앱 사용이 가장 활발한 시간 중 하나입니다. 
...
사용자에게 관련 참여 이벤트가 없는 경우(예: 클릭이나 열람이 거의 없거나 전혀 없는 신규 사용자), Intelligent Timing은 설정된 대체 시간(사용자가 지정한 대체 시간 또는 전체 사용자 중 앱 사용 빈도가 가장 높은 시간)을 적용합니다.
...
Be careful when filtering based on other campaign sends to avoid targeting ineligible segments. For example, if you were to send out two campaigns on the same day for different times, and add a filter that only allows users to receive the second campaign if they’ve received the first, users won’t receive the second campaign. 이는 캠페인이 처음 생성되고 세그먼트가 형성되었을 당시 자격을 갖춘 사람이 아무도 없었기 때문입니다.
...
Rate limiting can be used on a campaign that uses Intelligent Timing. 그러나 속도 제한의 특성상 일부 사용자는 최적의 시기가 아닌 시점에 메시지를 수신할 수 있습니다. 특히 속도 제한 규모에 비해 많은 수의 사용자가 관련 이벤트가 없어 대체 시간에 스케줄링될 경우 그러합니다. 
