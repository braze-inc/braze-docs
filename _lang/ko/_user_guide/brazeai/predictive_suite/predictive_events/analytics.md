...
단일 사용자의 가능성 점수를 보려면 대시보드에서 해당 사용자를 찾아 **참여** > **예측**으로 가서 점수를 확인하세요. 여러 사용자의 점수와 카테고리에 동시에 접근하려면 [세그먼트]({{site.baseurl}}/user_guide/engagement_tools/segments/creating_a_segment/)를 생성하고 [이벤트 가능성 점수]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/#event-likelihood-score) 또는 [이벤트 가능성 카테고리]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters/#event-likelihood-category) 필터를 사용한 다음 해당 세그먼트에서 사용자를 내보내세요. 내보낼 때 가능성 점수를 내보내는 데이터에 포함할 수 있습니다.

{% alert note %}
예측 이벤트와 [예측 이탈]({{site.baseurl}}/user_guide/brazeai/predictive_churn/) 모두 사용자에게 점수를 부여하지만 중요한 차이점이 있습니다:<br><br>

- **예측 이벤트** (구매 예측): 대상 이벤트를 이전에 수행했는지 여부에 관계없이 예측 오디언스의 모든 사용자를 고려합니다. 예를 들어, 구매 예측은 첫 구매를 할 가능성이 있는 사용자를 식별할 수 있습니다.
- **예측 이탈**: 이미 커스텀 이벤트를 수행한 사용자만 고려합니다. 이탈 예측은 이전에 무언가를 수행한 사용자 중에서 그것을 중단할 가능성이 있는 사용자를 식별합니다. 로그인한 적이 없는 사용자는 로그인하지 않더라도 "이탈"로 간주될 수 없습니다.

세그먼트에서 이탈 위험 점수를 내보낼 때, 이 점수는 구매 또는 다른 이벤트 예측 모델과 다른 이탈 예측 모델을 반영합니다.
{% endalert %}
...
![Braze 대시보드에 표시된 선택된 오디언스와 추정 정확도.]({% image_buster /assets/img/purchasePrediction/purchaseEstimatedResults.png %})
...
커스텀 이벤트에 대한 예측을 생성할 수 없는 경우, 이는 샘플 크기가 충분하지 않기 때문일 수 있습니다. Braze는 이벤트를 수행한 사용자 수를 추정하며, 충분한 사용자가 이벤트를 수행하지 않은 경우 샘플이 모델을 훈련하기에 충분한 데이터를 제공하지 않을 수 있습니다. 이 경우 시스템은 사용자가 없다고 추정하여 예측 생성을 방지할 수 있습니다.

성공적인 예측을 생성하려면 예측 오디언스의 충분한 수의 사용자가 목표 커스텀 이벤트를 수행했는지 확인하세요. 정확한 임계값은 다르지만, 사용자 기반에서 사용량이 매우 낮은 이벤트는 신뢰할 수 있는 모델 훈련을 위한 충분한 데이터를 제공하지 않을 수 있습니다.