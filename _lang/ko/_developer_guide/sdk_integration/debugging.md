...
{% alert tip %}
더 깊은 조사를 위해, 자세한 소프트웨어 개발 키트 출력을 캡처하기 위해 [상세 로깅을 인에이블먼트할]({{site.baseurl}}/developer_guide/sdk_integration/verbose_logging) 수도 있으며, 특정 채널에 대한 [상세 로그를 읽는 방법을 배울 수]({{site.baseurl}}/developer_guide/sdk_integration/reading_verbose_logs) 있습니다.
{% endalert %}

## 필수 조건

Braze 소프트웨어 개발 키트 디버거를 사용하려면 "PII 보기" 및 "PII 준수 고객 프로필 보기" 권한이 필요합니다. 디버깅 세션 로그를 다운로드하려면 "사용자 데이터 내보내기" 권한도 필요합니다. 또한 Braze SDK는 다음 최소 버전을 충족하거나 이를 가리켜야 합니다: 
...
!["소프트웨어 개발 키트 디버거"가 강조 표시된 "설정 및 테스트" 섹션.]({% image_buster /assets/img/sdk_debugger/select_sdk_debugger.png %})

**디버깅 세션 만들기를** 선택합니다.

!["소프트웨어 개발 키트 디버거" 페이지.]({% image_buster /assets/img/sdk_debugger/select_create_debugging_session.png %})
...
![세션 종료 후 표시되는 디버깅 페이지에 "로그 내보내기" 및 "세션 ID 복사" 기능이 포함됩니다.]({% image_buster /assets/img/sdk_debugger/copy_id_and_export_logs.png %})
