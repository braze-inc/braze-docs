---
nav_title: 피드
article_title: 콘텐츠 카드에 대한 피드를 사용자 정의하십시오
page_order: 3
...
{% alert important %}
Braze SDK는 성능 및 신뢰성을 위해 속도 제한을 적용합니다. 자동화된 테스트를 실행하거나 수동 QA를 수행할 때 이를 염두에 두십시오. 자세한 내용은 [Braze SDK 속도 제한]({{site.baseurl}}/developer_guide/sdk_integration/rate_limits/)를 참조하십시오.
{% endalert %}
...
{% endsubtab %}
{% subtab Jetpack Compose %}
Jetpack 작성으로 "빈 피드" 오류 메시지를 사용자 지정하려면 `emptyString` 을(를) [`ContentCardsList`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.jetpackcompose.contentcards/-content-cards-list.html). 으로 전달할 수도 있습니다. [`emptyTextStyle`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.jetpackcompose.contentcards.styling/-content-card-list-styling/index.html#1193499348%2FProperties%2F-1725759721) 을 `ContentCardListStyling` 으로 전달하여 이 메시지를 추가로 사용자 지정할 수도 있습니다.
...
대신 표시하고 싶은 컴포저블이 있는 경우 `emptyComposable` 으로 전달할 수 있습니다. [`ContentCardsList`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.jetpackcompose.contentcards/-content-cards-list.html). `emptyComposable` 을 지정하면 `emptyString` 이 사용되지 않습니다.
...
기본적으로 콘텐츠 카드 피드는 [`ContentCardsFragment`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards/-content-cards-fragment/index.html) 및 [`IContentCardsUpdateHandler`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards.handlers/-i-content-cards-update-handler/index.html) 를 수신한 후 표시할 카드 목록을 반환합니다. [`ContentCardsUpdatedEvent`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.events/-content-cards-updated-event/index.html) 를 받은 후 표시할 카드 목록을 반환합니다. 그러나 카드를 정렬할 뿐 필터링은 직접 처리하지 않습니다.

#### 2.1 단계: 사용자 지정 핸들러 만들기

설정한 키-값 쌍을 사용하여 사용자 정의( [`IContentCardsUpdateHandler`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards.handlers/-i-content-cards-update-handler/index.html) 로 설정한 키-값 쌍을 사용하여 대시보드의 [`Card.getExtras()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.models.cards/-card/extras.html) 로 설정한 키-값 쌍을 사용하여 사용자 지정한 다음 수정하여 목록에서 앞서 설정한 `feed_type` 값과 일치하지 않는 카드를 제거할 수 있습니다.
...
를 생성한 후 [`IContentCardsUpdateHandler`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards.handlers/-i-content-cards-update-handler/index.html)를 생성한 후 [`ContentCardsFragment`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.contentcards/-content-cards-fragment/index.html) 를 생성합니다. 이 커스ㅁ 피드는 다른 `ContentCardsFragment`와 마찬가지로 사용할 수 있습니다. 앱의 다른 부분에서 대시보드에 제공된 키에 따라 다른 콘텐츠 카드 피드를 표시합니다. 각 `ContentCardsFragment` 피드에는 각 조각의 커스텀 `IContentCardsUpdateHandler` 덕분에 고유한 카드 집합이 표시됩니다.
