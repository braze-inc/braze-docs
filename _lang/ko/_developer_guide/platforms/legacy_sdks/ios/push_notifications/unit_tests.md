...
이 선택적 가이드는 앱 대리자가 [푸시 통합 지침]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/push_notifications/integration/)에 설명된 단계를 올바르게 따르는지 확인하는 몇 가지 단위 테스트를 구현하는 방법을 설명합니다. 
...
처음에 [Braze SDK를 설치]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/initial_sdk_setup/overview/)하는 데 사용한 동일한 방법을 사용하여 동일한 SDK 설치가 단위 테스트 대상으로 사용 가능한지 확인합니다. 예를 들어, CocoaPods를 사용하는 경우:
