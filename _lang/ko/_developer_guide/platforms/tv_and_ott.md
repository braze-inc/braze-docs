...
![Android OTT 앱을 사용하여 만든 세그먼트입니다.]({% image_buster /assets/img/android_ott.png %})

## 헤드리스 UI {#custom-ui}

{% alert important %}
헤드리스 UI를 통해 인앱 메시지 또는 콘텐츠 카드를 지원하는 플랫폼에는 기본값 UI 또는 보기가 **포함되지 않습니다**. 인앱 메시지와 같은 커스텀 UI를 구축한 다음 소프트웨어 개발 키트에서 제공하는 데이터 모델을 사용하여 해당 UI를 채울 수 있습니다.
{% endalert %}
