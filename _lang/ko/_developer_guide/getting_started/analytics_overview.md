...
page_order: 4
description: "이 참고 문서에서는 자동으로 수집되는 데이터, 구매 이벤트 및 커스텀 이벤트, 데이터 수집 모범 사례 등 사용자 데이터 수집을 다룹니다."

---

# 애널리틱스 개요

> 이 가이드는 수집하는 데이터, Braze에서 커스텀 이벤트 및 커스텀 속성 간 차이점, 이러한 분석을 관리하기 위한 모범 사례를 이해하는 데 도움이 됩니다.

Braze 구현을 완료하기 전에 마케팅 팀과 개발 팀은 마케팅 목표에 대해 논의해야 합니다. 추적할 대상과 Braze에서 추적할 방법을 결정할 때는 이러한 목표를 고려하고 여기서부터 거꾸로 작업하는 것이 유용합니다. 

이 프로세스의 예는 이 가이드 끝에 있는 [택시/승차 공유 앱](#example-case) 사례를 참조하세요.
...
| ---------------------| --------------- | ------------- |
| 사용자 지정 이벤트가 **X회 이상** 발생했는지 확인합니다. | **초과** | **숫자** |
| 사용자 지정 이벤트가 **X 횟수 미만으로** 발생했는지 확인합니다. | **미만** | **숫자** |
| 커스텀 이벤트가 **정확히 X회** 발생했는지 확인합니다. | **정확히 일치** | **숫자** |
...
| 커스텀 이벤트가 **X회(최대 = 50) 미만**으로 발생했는지 확인합니다. | **미만** | 지난 **Y일 동안(Y = 1,3,7,14,21,30)** |
| 커스텀 이벤트가 **정확히 X회(최대 = 50)**만큼 발생했는지 확인합니다. | **정확히 일치** | 지난 **Y일 동안(Y = 1,3,7,14,21,30)** |
...
커스텀 이벤트 속성은 메시징을 개인화하거나 세분화된 실행 기반 전달 캠페인을 구축하는 데 도움이 되도록 설계되었습니다. 이벤트 속성 최근성 및 빈도를 기준으로 세그먼트를 만들려면 추가 데이터 비용이 발생할 수 있으므로 고객 성공 관리자 또는 지원팀에 문의하세요.
...
| 문자열 속성이 입력한 문자열과 **일치하지 않는지** 확인합니다.| **동등하지 않음** | **문자열** |
...
| 세분화 옵션 | 드롭다운 필터 | 입력 옵션 |
| ---------------------| --------------- | ------------- |
| 배열 속성에 입력한 값과 **정확히 일치하는 값이 포함**되었는지 확인합니다.| **값 포함** | **문자열** |
...
| 시간 속성이 **X일 미만**의 **향후 일수**인지 확인합니다. | **향후 미만** | **향후 일수**  |
...
> 이 방법으로 지출한 금액을 기록해서는 안 됩니다. 대신 [구매 방법]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview/#purchase-events--revenue-tracking)을 통해 기록해야 합니다.
...
| 숫자 속성이 **정확히** **숫자인지** 확인합니다.| **정확히 일치** | **숫자** |
| 숫자 속성이 **숫자**와 **같지 않은지** 확인합니다.| **동등하지 않음** | **숫자** |
...
| 세분화 옵션 | 드롭다운 필터 | 입력 옵션 |
| ---------------------| --------------- | ------------- |
| 총 지출 금액이 **숫자**보다 **큰지** 확인합니다.| **보다 큼** | **숫자** |
| 총 지출 금액이 **숫자**보다 **적은지** 확인합니다.| **미만** | **숫자** |
| 총 지출 **금액이 정확한** **수치인지** 확인합니다.| **정확히 일치** | **숫자** |
...
| 구매가 **정확히 X회(최대 = 50)**만큼 발생했는지 확인합니다. | **정확히 일치** | 지난 **Y일 동안(Y = 1,3,7,14,21,30)** |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

>  특정 구매가 발생한 횟수를 기준으로 세분화하려면 해당 구매를 [증분 사용자 지정 속성으로](#integers) 개별적으로 기록해야 합니다.
...
3. 택시를 호출해야 합니다.
4. 택시를 호출하려면 검색할 때 이용 가능한 택시가 있어야 합니다.
