...
[데이터 및 커스텀 데이터 자동 수집]({{site.baseurl}}/developer_guide/getting_started/analytics_overview/)에 대해 자세히 알아보세요.

### 사용자 지정 계획

원하는 사용자 지정에 대해 마케터와 상의합니다. 예를 들어 기본 Braze 콘텐츠 카드를 구현하고 싶으신가요? 브랜드 지침에 맞게 모양과 느낌을 약간 조정하고 싶으신가요? 컴포넌트에 대한 완전히 새로운 UI를 개발하고 Braze가 그 분석을 추적하도록 하고 싶으신가요? 사용자 지정 수준마다 다른 수준의 범위가 필요합니다. 자세한 내용은 [사용자 지정 개요]({{site.baseurl}}/developer_guide/getting_started/customization_overview)를 참조하세요.
...
* 계획된 [UI 컴포넌트 사용자 지정]({{site.baseurl}}/developer_guide/getting_started/customization_overview)(예: 커스텀 콘텐츠 카드)을 생성합니다. 완전한 커스텀 콘텐츠의 경우 SDK의 자동 데이터 수집이 새 구성요소를 인식하지 못하므로 분석을 기록해야 합니다. 기본 구성 요소에 이 구현을 패턴화할 수 있습니다.
...
Braze SDK는 '세션 열기' 및 '세션 닫기' 데이터 포인트를 생성합니다. Braze SDK는 정기적으로 데이터를 플러시하기도 합니다. 세션 추적 기본값은 이 링크를 참조하세요([Android]({{site.baseurl}}/developer_guide/platforms/android/analytics/tracking_sessions/), [iOS]({{site.baseurl}}/developer_guide/platforms/swift/analytics/tracking_sessions/), [웹]({{site.baseurl}}/developer_guide/platforms/web/analytics/tracking_sessions/)). 모두 사용자 지정할 수 있습니다.
