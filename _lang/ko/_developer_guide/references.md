...
5. 테스트 기기에 Droidboy APK를 자동으로 설치하려면 SDK 디렉토리 내에서 `./gradlew installDebug`를 실행합니다.
...
```
`ANDROID_HOME` 변수가 제대로 설정되어 있지 않거나 `local.properties` 폴더에 유효한 `sdk.dir` 폴더가 없는 경우, 이 플러그인이 기본 SDK도 설치해 줍니다. 자세한 내용은 [플러그인 리포지토리](https://github.com/JakeWharton/sdk-manager-plugin)를 참조하세요.
