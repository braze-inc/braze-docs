...
시작하려면 네 가지 고급 단계가 있습니다:

1. [서버 측 통합](#server-side-integration) \- 공개 및 비공개 키 페어를 생성하고, 비공개 키를 사용하여 현재 로그인한 사용자를 위한 JWT를 생성합니다.<br><br>
2. [SDK 통합](#sdk-integration) \- Braze SDK에서 이 기능을 활성화하고 서버에서 생성된 JWT를 요청합니다.<br><br>
3. [공개 키 추가](#key-management) \- Braze 대시보드의 **설정 관리** 페이지에 _공개 키_를 추가합니다.<br><br>
4. [Braze 대시보드에서 적용 토글](#braze-dashboard) \- 앱별로 Braze 대시보드에서 이 기능의 적용을 토글합니다.

## 서버-사이드 통합 {#server-side-integration}

### 공개/비공개 키 쌍 {#generate-keys} 생성
...
### 현재 사용자 {#create-jwt}을(를) 위한 JSON 웹 토큰을 생성하십시오
...
### Braze 소프트웨어 개발 키트에서 이 기능을 활성화하십시오.
...
### 잘못된 토큰 {#sdk-callback}에 대한 콜백 함수를 등록하십시오
...
## Braze 대시보드에서 활성화 {#braze-dashboard}

[서버 측 통합](#server-side-integration) 및 [SDK 통합](#sdk-integration)이 완료되면 특정 앱에 대해 이 기능을 활성화할 수 있습니다.
