---
nav_title: 분석
article_title: Braze SDK용 애널리틱스 정보
page_order: 2.6
description: "Braze SDK의 애널리틱스에 대해 알아보고 Braze가 수집하는 데이터, 사용자 지정 이벤트와 사용자 지정 속성의 차이점, 애널리틱스 관리를 위한 모범 사례를 더 잘 이해할 수 있습니다."
platform: 
  - Android
  - Swift
  - Web
...
> Braze SDK의 애널리틱스에 대해 알아보고 Braze가 수집하는 데이터, 사용자 지정 이벤트와 사용자 지정 속성의 차이점, 애널리틱스 관리를 위한 모범 사례를 더 잘 이해할 수 있습니다.

{% alert tip %}
Braze를 구현하는 동안 팀과 마케팅 목표에 대해 논의하여 추적할 데이터와 Braze로 추적할 방법을 가장 잘 결정할 수 있도록 하세요. 예시는 이 가이드 끝에 있는 [택시/승차 공유 앱](#example-case) 사례 연구를 참조하세요.
{% endalert %}

## 자동으로 수집된 데이터

특정 사용자 데이터(예: 처음 사용한 앱, 마지막으로 사용한 앱, 총 세션 수, 기기 OS 등)는 SDK에서 자동으로 수집됩니다. 통합 가이드에 따라 SDK를 구현하면 이 [기본 데이터 수집]({{site.baseurl}}/user_guide/data/user_data_collection/sdk_data_collection/) 기능을 활용할 수 있습니다. 이 목록을 확인하면 사용자에 대한 동일한 정보를 두 번 이상 저장하지 않도록 하는 데 도움이 됩니다. 세션 시작 및 종료 시점을 제외하고, 자동으로 추적되는 모든 기타 데이터는 데이터 포인트 사용량에 포함되지 않습니다.
...
예를 들어, 이커머스 애플리케이션에서 사용자가 장바구니를 이탈할 때 메시지를 보내려는 경우, 사용자 장바구니의 사용자 지정 이벤트 속성( `cart_value` )을 추가하여 타겟 고객을 추가로 개선하고 캠페인 개인화를 강화할 수 있습니다.

![장바구니를 포기하고 장바구니 값을 100 이상 200 미만으로 남겨둔 사용자에게 캠페인을 보내는 사용자 지정 이벤트 예제입니다.]({% image_buster /assets/img_archive/customEventProperties.png %} "customEventProperties.png")
...
커스텀 이벤트 속성은 메시징을 개인화하거나 세분화된 실행 기반 전달 캠페인을 구축하는 데 도움이 되도록 설계되었습니다. 이벤트 속성정보의 최근 발생 시점 및 빈도를 기준으로 세그먼트를 생성하려면 고객 성공 매니저 또는 지원팀에 문의하십시오.
...
문자열 속성은 즐겨 찾는 브랜드, 전화번호 또는 애플리케이션 내에서 마지막으로 검색한 문자열과 같은 사용자 입력을 저장하는 데 유용합니다. 문자열 속성은 사용자 정의 데이터에 대한 [길이 제약](#length-constraints) 조건(479바이트; 단일 바이트 문자 약 479자 또는 일본어와 같은 다중 바이트 스크립트의 경우 약 160자)을 따릅니다.
...
커스텀 속성 배열의 최대 요소 개수 기본값은 25개입니다. 개별 배열의 최대값은 Braze 대시보드의 **데이터 설정** > **커스텀 **속성에서 최대 500개까지 증가시킬 수 있습니다. 이 한도를 500 이상으로 늘리려면 Braze 고객 성공 매니저에게 문의하십시오. 최대 요소 개수를 초과하는 배열은 최대 요소 개수를 포함하도록 잘립니다.
...
| 세분화 옵션 | 드롭다운 필터 | 입력 옵션 |
| ---------------------| --------------- | ------------- |
| 숫자 속성이 **숫자** **이상인지** 확인합니다.| **초과** | **번호** |
| 숫자 속성이 **숫자보다** **작은** 지 확인합니다.| **미만** | **번호** |
| 숫자 속성이 **정확히** **숫자인지** 확인합니다.| **정확히** | **번호** |
| 숫자 속성이 **숫자**와 **같지 않은지** 확인합니다.| **동일하지 않음** | **번호** |
...
| 세분화 옵션 | 드롭다운 필터 | 입력 옵션 |
| ---------------------| --------------- | ------------- |
| 총 지출 금액이 **숫자**보다 **큰지** 확인합니다.| **다음보다 큼** | **번호** |
| 총 지출 금액이 **숫자**보다 **적은지** 확인합니다.| **미만** | **번호** |
...
마케터로서 Braze 도입 1~2년 후에 사용을 시작한다고 상상해 보세요. 추가적인 맥락 없이  와"usr_no_acct" 같은 이름들로 가득 찬 드롭다운 목록을 읽는 것은 부담스러울 수 있습니다. 이벤트와 속성에 식별 가능하고 읽기 쉬운 이름을 부여하면 플랫폼의 모든 사용자가 더 쉽게 작업할 수 있습니다. 다음 모범 사례를 고려하세요:
...
Braze는 커스텀 이벤트 이름, 커스텀 속성 이름(키), 커스텀 이벤트 문자열 값에 대해 바이트 단위의 길이 제한(479바이트)을 적용합니다. 이 한도를 초과하는 값은 잘립니다. 문자로 표현할 경우, 이는 약 479개의 단일 바이트 문자(예: ASCII)에 해당하거나, 일본어와 같은 다중 바이트 스크립트의 경우 약 160자(UTF-8에서 문자당 약 3바이트 가정)에 해당합니다. 가능하다면 앱의 네트워크 및 배터리 성능 향상을 위해 이름과 값을 최대한 짧게 유지하십시오. 가능하면 50자 이내로 제한하십시오.
