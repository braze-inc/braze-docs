...
platform: Swift
description: "이 문서에서는 Braze Swift SDK를 구성하는 추가 단계에 대해 설명합니다."
page_order: 3
...
사용 가능한 구성의 전체 목록은 [Braze.Configuration 클래스 설명서를][1] 참조하세요.

## 브레이즈 로그 레벨

다음 차트에서 Braze Swift SDK의 기본 로그 수준은 `.error` 입니다. 이 수준은 완전히 비활성화된 로깅보다 가장 최소한의 수준입니다.
...
| `.info`     | `BRZLoggerLevelInfo`     | 일반 SDK 정보(사용자 변경 사항 등) + `.error`. |
...
브레이즈 로거의 전체 사용법은 [로거 클래스 문서를][2] 참조하세요.
