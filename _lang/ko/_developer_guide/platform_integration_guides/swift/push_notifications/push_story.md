---
nav_title: 푸시 스토리
article_title: iOS용 푸시 스토리 통합
platform: Swift
page_order: 27
description: "이 문서에서는 Swift SDK용 iOS 푸시 스토리 설정 방법을 보여줍니다."
channel:
  - push
...
> 마케팅 담당자는 [푸시 스토리에서]({{site.baseurl}}/user_guide/message_building_by_channel/push/advanced_push_options/push_stories/) 사진 캐러셀 기능을 사용하여 푸시 알림 내에 페이지 시퀀스를 만들 수 있습니다. 이 페이지는 이미지, 클릭 동작, 제목 및 설명으로 구성됩니다. 

iOS 앱에 푸시 스토리를 설정하려면 표준 푸시 알림 통합 외에도 추가 단계가 필요하며, 이는 이 기사에 설명되어 있습니다.
...
[푸시 알림 통합 튜토리얼]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/push_notifications/integration/)을 따라 앱에서 푸시를 활성화했는지 확인합니다. 이 작업의 일환으로, 이 기능에 필요한 `UNNotification` 프레임워크를 구현해야 합니다.
...
[스위프트 패키지 매니저 통합 가이드]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/initial_sdk_setup/installation_methods/swift_package_manager/)를 수행한 후 `Notification Content Extension`에 `BrazePushStory`를 추가합니다.
