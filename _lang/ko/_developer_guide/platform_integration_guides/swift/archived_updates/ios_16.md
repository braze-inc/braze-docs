...
platform: 
  - iOS
description: "이 참조 문서에서는 iOS 16, 업그레이드 방법, SDK 업데이트 등에 대해 설명합니다."
hidden: true
noindex: true
---
...
> 이 가이드에서는 iOS 16(2022)에 도입된 관련 변경 사항과 Braze iOS SDK 통합에 미치는 영향에 대해 설명합니다. 전체 마이그레이션 가이드는 [iOS 16 릴리스 노트를][2] 참조하세요.
...
이전에는 Apple이 자체 Safari 푸시 API를 사용하여 macOS(데스크톱)에서 푸시 알림을 지원했습니다.

macOS Ventura(2022년 10월 24일 출시)부터 [Safari는](https://webkit.org/blog/12824/news-from-wwdc-webkit-features-in-safari-16-beta/#web-push-for-macos) Safari 푸시 외에 웹 푸시 API에 대한 [지원을 추가했습니다](https://webkit.org/blog/12824/news-from-wwdc-webkit-features-in-safari-16-beta/#web-push-for-macos). 이는 다른 인기 브라우저에서 사용되는 기존의 크로스 브라우저 API 표준입니다.
...
## iOS 16 준비하기 {#next-steps}

iOS 16을 위해 Braze iOS SDK를 업그레이드할 필요는 없지만, 두 가지 흥미로운 업데이트가 있습니다:

1. Braze에서 [새로운 Swift SDK를][3] 출시했습니다. 이를 통해 향상된 성능, 새로운 기능 및 많은 개선 사항이 제공됩니다.
2. 브라즈 스위프트 SDK는 새로운 ["노코드" 푸시 프라이머 기능을][7] 지원합니다!
