---
nav_title: 통합
article_title: 콘텐츠 카드 뷰 컨트롤러 통합 for iOS
platform: iOS
page_order: 1
description: "이 참조 문서에서는 iOS 애플리케이션에 사용할 수 있는 통합 단계, 데이터 모델 및 카드별 속성을 다룹니다."
channel:
  - content cards
search_rank: 3
noindex: true
...
콘텐츠 카드 데이터 모델은 iOS 소프트웨어 개발 키트에서 사용할 수 있습니다.

### 데이터 가져오기

콘텐츠 카드 데이터 모델에 액세스하려면 콘텐츠 카드 업데이트 이벤트에 가입하십시오:
...
카드 데이터를 Braze에서 보낸 후 변경하려면 카드 데이터의 딥 카피를 로컬에 저장하고 데이터를 업데이트한 후 직접 표시하는 것을 권장합니다. 카드는 [`ABKContentCardsController`](https://appboy.github.io/appboy-ios-sdk/docs/interface_a_b_k_content_cards_controller.html)를 통해 접근할 수 있습니다.

## 콘텐츠 카드 모델

Braze는 세 가지 콘텐츠 카드 유형을 제공합니다: 배너, 캡션 이미지 및 클래식. 각 유형은 기본 `ABKContentCard` 클래스에서 공통 속성을 상속받으며 다음과 같은 추가 속성을 가지고 있습니다.
...
| `viewed` | 이 속성은 사용자가 카드를 보았는지 여부를 반영합니다.|
...
| `pinned` | 이 속성은 카드가 대시보드에 "고정됨"으로 설정되었는지 여부를 반영합니다.|
| `dismissed` | 이 속성은 사용자가 카드를 닫았는지 여부를 반영합니다.|
| `url` | 카드를 클릭한 후 열리는 URL입니다. HTTP(s) URL 또는 프로토콜 URL일 수 있습니다.|
| `openURLInWebView` | 이 속성은 URL이 앱 내에서 열릴지 외부 웹 브라우저에서 열릴지를 결정합니다.|
| `extras`| 선택적 `NSDictionary` `NSString` 값.|
{: .reset-td-br-1 .reset-td-br-2}

### 배너 콘텐츠 카드 속성 - ABK배너콘텐츠카드

|등록정보|설명|
|---|---|
| `image` | 이 속성은 카드 이미지의 URL입니다.|
| `imageAspectRatio` | 이 속성은 카드 이미지의 종횡비이며 이미지 로딩이 완료되기 전에 힌트로 사용됩니다. 특정 상황에서는 속성이 제공되지 않을 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2}
...
| `domain` | 속성 URL의 링크 텍스트, 예를 들어 @"blog.braze.com". 카드의 UI에 표시되어 카드를 클릭하는 동작/방향을 나타낼 수 있습니다.|
{: .reset-td-br-1 .reset-td-br-2}
...
| `title` | 카드의 제목 텍스트. |
| `cardDescription` | 카드의 본문 텍스트. |
| `domain` | 속성 URL의 링크 텍스트, 예를 들어 @"blog.braze.com". 카드의 UI에 표시되어 카드 클릭의 동작과 방향을 나타낼 수 있습니다. |
{: .reset-td-br-1 .reset-td-br-2}
...
| `logContentCardClicked` | 특정 카드에 대해 Braze에 클릭을 수동으로 기록합니다. 소프트웨어 개발 키트는 카드에 유효한 값이 있는 `url` 속성이 있을 때만 카드 클릭을 기록합니다. |
| `logContentCardDismissed` | 특정 카드에 대해 Braze에 해제를 수동으로 기록합니다. 소프트웨어 개발 키트는 카드의 `dismissed` 속성이 이미 `true`로 설정되어 있지 않은 경우에만 카드 방출을 기록합니다. |
...
콘텐츠 카드는 두 가지 뷰 컨트롤러 컨텍스트인 내비게이션 또는 모달과 통합될 수 있습니다.
...
{% alert note %}
탐색 모음의 제목을 사용자 정의하려면 `ABKContentCardsTableViewController` 인스턴스의 `navigationItem`의 제목 속성을 설정하십시오.
{% endalert %}
...
뷰 컨트롤러 예제를 보려면 [콘텐츠 카드 샘플 앱](https://github.com/Appboy/appboy-ios-sdk/tree/master/Samples/ContentCards/BrazeContentCardsSampleApp)을 확인하세요.
