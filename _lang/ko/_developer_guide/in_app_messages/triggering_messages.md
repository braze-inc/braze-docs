---
nav_title: 트리거 메시지
article_title: Braze 소프트웨어 개발 키트를 통해 인앱 메시지를 트리거합니다
page_order: 0.2
...
사용자 세션이 시작되면 Braze는 모든 적격 인앱 메시지를 디바이스에 전달하는 동시에 에셋을 프리페칭하여 디스플레이 지연 시간을 최소화합니다. 트리거 이벤트에 적격 인앱 메시지가 두 개 이상 있는 경우 우선순위가 가장 높은 메시지만 전달됩니다. 자세한 내용은 [세션 수명 주기를]({{site.baseurl}}/developer_guide/analytics/tracking_sessions/#about-the-session-lifecycle) 참조하세요.
...
Braze에서 캠페인을 생성할 때 키-값 페어를 설정할 수 있으며`extras`, 인앱 메시징 객체가 이를 활용하여 데이터를 앱으로 전송할 수 있습니다.
...
메시지 타이밍에 대한 고급 제어(트리거된 메시지 지연 및 복원 포함)에 대해서는 다음 [튜토리얼을 참조하십시오: 트리거된 메시지의]({{site.baseurl}}/developer_guide/in_app_messages/tutorials/deferring_triggered_messages) 지연 및 복원
{% endtab %}

{% tab android %}
1. 커스텀 리스너를 설정하려면[`IInAppMessageManagerListener`](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?sdktab=android&tab=global%20listener#android_step-1-implement-the-custom-manager-listener)  를 구현하십시오.
2. 메서드를[`beforeInAppMessageDisplayed()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-displayed.html) 업데이트하여 반환하도록 [`InAppMessageOperation.DISCARD`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-operation/-d-i-s-c-a-r-d/index.html)하십시오.

메시지 타이밍에 대한 고급 제어(후속 표시 및 재대기열 추가 포함)에 대해서는 [메시지 커스텀](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?tab=global%20listener&subtab=kotlin#android_step-2-instruct-braze-to-use-the-custom-manager-listener) 페이지를 참조하십시오.
{% endtab %}

{% tab swift %}
1. 앱에서 `BrazeInAppMessageUIDelegate` 델리게이트를 구현합니다. 전체 안내를 보려면 [튜토리얼을 참조하십시오: 인앱 메시지 UI](https://braze-inc.github.io/braze-swift-sdk/tutorials/braze/c1-inappmessageui).
...
커스텀 푸시 콜백을 등록하여 특정 무음 푸시 알림을 수신 대기합니다. 자세한 내용은 [푸시 알림 설정을]({{site.baseurl}}/developer_guide/push_notifications#android_setting-up-push-notifications) 참조하십시오.
...
![두 개의 키-값 페어:IS_SERVER_EVENT하나는 "true"로 설정되고, 다른 하나는 "예시 캠페인CAMPAIGN_NAME 이름"으로 설정됩니다.]({% image_buster /assets/img_archive/kvpConfiguration.png %}){: style="max-width:70%;" }
...
!["IAM 캠페인 이름 예시"와 "campaign_name"일치할 때 인앱 메시지가 트리거되는 실행 기반 전달 캠페인입니다.]({% image_buster /assets/img_archive/iam_event_trigger.png %})
...
![두 개의 키-값 페어를 가진 실행 기반 전달 인앱 메시지 캠페인입니다. "인앱 메시지 "CAMPAIGN_NAME"이름 예시"로 설정하고, "true"IS_SERVER_EVENT""로 설정합니다.]({% image_buster /assets/img_archive/iOSServerPush.png %})
...
![사용자가 "인앱 메시지 트리거"라는 커스텀 이벤트를 수행할 때, 해당 이벤트의"campaign_name"조건이 "IAM 캠페인 이름 예시"와 일치하는 사용자에게 전달되는 실행 기반 앱 내 메시지 캠페인입니다.]({% image_buster /assets/img_archive/iosIAMeventTrigger.png %})
...
{% tabs %}
{% tab web %}
웹 소프트웨어 개발 키트의 경우,  를 `braze.showInAppMessage(inAppMessage)`사용하여 인앱 메시지를 표시하십시오. 자세한 내용과 예시는 [실시간 메시지 표시를](#displaying-a-message-in-real-time) 참조하십시오.
{% endtab %}
...
{% tab swift %}
메서드를 수동으로 호출합니다. [`present(message:)`](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/brazeinappmessagepresenter/present(message:))`inAppMessagePresenter` 메서드를 직접 호출하세요. 예를 들어, 다음과 같습니다.
