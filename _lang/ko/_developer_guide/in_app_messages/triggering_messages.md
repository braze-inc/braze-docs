---
nav_title: 트리거 메시지
article_title: Braze 소프트웨어 개발 키트를 통해 인앱 메시지를 트리거하세요.
page_order: 0.2
...
트리거된 메시지의 지연 및 복원을 포함하여 메시지 타이밍에 대한 보다 진행된 제어에 대해서는 [튜토리얼을 참조하세요: 트리거된 메시지 연기 및 복원하기]({{site.baseurl}}/developer_guide/in_app_messages/tutorials/deferring_triggered_messages).
{% endtab %}

{% tab android %}
1. 를 구현하여 [`IInAppMessageManagerListener`](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?sdktab=android&tab=global%20listener#android_step-1-implement-the-custom-manager-listener) 를 사용하여 커스텀 리스너를 설정합니다.
2. 메서드를 업데이트하여 [`beforeInAppMessageDisplayed()`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage.listeners/-i-in-app-message-manager-listener/before-in-app-message-displayed.html) 메서드를 업데이트하여 [`InAppMessageOperation.DISCARD`](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze.ui.inappmessage/-in-app-message-operation/-d-i-s-c-a-r-d/index.html).

나중에 표시하기, 다시 대기열에 추가하기 등 메시지 타이밍에 대한 더 자세한 진행 상황 제어는 [메시지 커스텀하기](https://www.braze.com/docs/developer_guide/in_app_messages/customization/?tab=global%20listener&subtab=kotlin#android_step-2-instruct-braze-to-use-the-custom-manager-listener) 페이지를 참조하세요.
{% endtab %}
...
커스텀 푸시 콜백을 등록하여 특정 무음 푸시 알림을 수신 대기합니다. 자세한 내용은 [푸시 알림 설정하기를]({{site.baseurl}}/developer_guide/push_notifications#android_setting-up-push-notifications) 참조하세요.
...
![고객 프로필에 커스텀 이벤트가 있는 사용자에게 전달되는 실행 기반 전달 인앱 메시지 캠페인 "server_event".]({% image_buster /assets/img_archive/iosServerSentPush.png %})입니다.

푸시 캠페인에는 이 푸시 캠페인이 SDK 커스텀 이벤트를 기록하기 위해 전송되었음을 나타내는 키-값 페어 추가 항목이 포함되어야 합니다. 이 이벤트는 인앱 메시지를 트리거하는 데 사용됩니다.

!["CAMPAIGN_NAME" 을 "인앱 메시지 이름 예시"로 설정하고 "IS_SERVER_EVENT" 을 "true"로 설정한 실행 기반 전달 인앱 메시지 캠페인입니다.]({% image_buster /assets/img_archive/iOSServerPush.png %})
...
!["campaign_name" 이 "IAM 캠페인 이름 예시"와 같은 커스텀 이벤트 "인앱 메시지 트리거"를 수행하는 사용자에게 전달되는 실행 기반 전달 인앱 메시지 캠페인입니다.]({% image_buster /assets/img_archive/iosIAMeventTrigger.png %})
