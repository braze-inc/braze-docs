...
[Braze 데이터 트랜스포메이션]({{site.baseurl}}/user_guide/data_and_analytics/data_transformation/overview/)을 사용하면 웹훅 통합을 구축 및 관리하여 외부 플랫폼에서 Braze 고객 프로필로의 데이터 흐름을 자동화할 수 있습니다. 이렇게 새롭게 통합된 사용자 데이터는 더욱 정교한 마케팅 사용 사례를 뒷받침할 수 있습니다.
...
[전달 가능성 센터]({{site.baseurl}}/user_guide/data_and_analytics/analytics/deliverability_center)는 Gmail 포스트마스터 도구를 사용하여 전송된 이메일의 데이터를 추적하고 전송 도메인에 대한 데이터를 수집할 수 있도록 지원하여 이메일 성능에 대한 더 많은 인사이트를 제공합니다. 
...
{% alert important %}
커스텀 템플릿 저장 기능은 현재 얼리 액세스 중입니다. 얼리 액세스에 참여하려면 Braze 계정 매니저에게 문의하세요.
{% endalert %}
...
[메시지 아카이빙]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/message_archiving/)을 사용하면 보관 또는 규정 준수 목적으로 사용자에게 보낸 메시지의 사본을 S3 버킷에 저장할 수 있습니다. 메시지가 전송될 때마다 S3 버킷에 전달되는 JSON 페이로드에 다음 필드가 추가되었습니다:
