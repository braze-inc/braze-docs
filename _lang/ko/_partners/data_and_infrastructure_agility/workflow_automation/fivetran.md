...
[Fivetran 대시보드](https://fivetran.com/dashboard)에서 **\+ 커넥터**를 클릭하고 **Braze** 커넥터를 선택하여 설정 양식을 시작합니다. 다음으로 **Amazon S3를** 선택합니다. 여기에 제공된 외부 ID는 Fivetran이 S3 버킷에 액세스할 수 있도록 허용하는 데 필요합니다. 
...
그런 다음 **JSON** 탭을 클릭하고 다음 정책을 붙여넣습니다. `{your-bucket-name}`을 S3 버킷의 이름으로 바꿔야 합니다.
...
마지막으로, **정책 검토**를 클릭하고 정책의 고유한 이름과 설명을 제공합니다. **정책 생성**을 클릭하여 정책을 빌드합니다. 
...
다음으로, **다음: 권한**을 클릭하여 방금 생성한 정책을 선택합니다.

![]({% image_buster /assets/img/fivetran_as3_select_policy.png %})

**다음: 검토**를 클릭하고 새 역할의 이름(예: Fivetran)을 지정한 다음, **역할 생성**을 클릭합니다. 역할이 생성된 후 해당 역할을 클릭하고 표시되는 역할 ARN을 기록합니다.
...
Fivetran에서 **\+ 커넥터**를 클릭하고 **Braze** 커넥터를 선택하여 설정 양식을 시작합니다. 양식 내에서 주어진 필드를 적절한 값으로 채웁니다:
...
- `Bucket`: **통합 > 커런츠 > [현재 이름] > 버킷 이름**으로 이동하여 Braze 계정에서 찾을 수 있습니다.
...
마지막으로 **저장 및 테스트**를 클릭하면 Braze 계정의 데이터와 동기화하여 Fivetran에서 나머지 작업을 수행합니다!

### Google 클라우드 스토리지용 Braze Currents 설정하기

#### 1단계: Google Cloud Storage에서 Fivetran 이메일 검색 {#step-one2}

[Fivetran 대시보드](https://fivetran.com/dashboard)에서 **\+ 커넥터**를 클릭하고 **Braze** 커넥터를 선택하여 설정 양식을 시작합니다. 다음으로 **Google 클라우드 스토리지를** 선택합니다. 표시되는 이메일 주소를 메모해 두세요.
...
[Google 스토리지 콘솔](https://console.cloud.google.com/storage/browser)로 이동하여 Braze 커런츠를 구성한 버킷을 선택한 다음, **버킷 권한 편집**을 클릭합니다.

![Google 스토리지 콘솔에서 사용 가능한 버킷입니다. 버킷을 찾아 세로 점 3개 기호를 클릭하면 버킷 권한을 편집할 수 있는 드롭다운이 열립니다.]({% image_buster /assets/img/fivetran_edit_bucket_permissions_gcs.png %})
...
- `API URL`: Braze REST API 엔드포인트.
- `API Key`: Braze REST API 키입니다. 
- `Bucket Name`: **통합 > 커런츠 > [현재 이름] > 버킷 이름**으로 이동하여 Braze 계정에서 찾을 수 있습니다.
- `Folder`: **통합 > 커런츠 > [현재 이름] > 접두사**로 이동하여 Braze 계정에서 찾을 수 있습니다.
