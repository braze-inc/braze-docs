...
| 브레이즈 계정   | An active Braze account with the [Braze Web SDK]({{site.baseurl}}/developer_guide/sdk_integration/?sdktab=web) integrated on your webpage. You'll also need event property segmentation enabled. To request it, see [Considerations](#request-event-property-segmentation). |
{: .reset-td-br-1 .reset-td-br-2 role="presentation"}

## Integrating VWO with Braze

### 1단계: Enable the Braze integration in VWO
...
### 2단계: Create a segment in Braze with VWO event properties
...
8. If needed, configure the number of visits and time frame. 완료했으면 **저장을** 선택합니다.
