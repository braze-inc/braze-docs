...
Snowflake의 데이터 공유 작동 방식에 대해 자세히 알아보려면 [보안 데이터 공유 소개](https://docs.snowflake.net/manuals/user-guide/data-sharing-intro.html#how-does-secure-data-sharing-work)를 참조하세요.

## 전제 조건

이 통합에 관심이 있으면 Braze 계정 또는 고객 성공 매니저에게 연락하여 Snowflake와의 보안 데이터 공유에 대해 Braze 데이터 전략 서비스에 문의하세요. Braze 내부에서 요청을 처리하여 바로 원하는 보기를 지원해 드립니다!

| 요구 사항 | 설명 |
| ----------- | ----------- |
| Snowflake 계정 | 이 파트너십을 활용하려면 관리자 수준의 권한이 있는 Snowflake 계정이 필요합니다. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

## 통합

Braze 계정과 보안 데이터 공유를 설정하려면 다음 단계를 따르세요.

1. Braze 대시보드에서 **파트너 통합** > **데이터 공유**로 이동합니다.
2. Snowflake 계정 세부 정보를 입력하세요. 대상 계정에서 `SELECT CURRENT_ACCOUNT()`을(를) 실행하여 Snowflake 계정 ID를 찾을 수 있습니다.
3. CRR 공유를 사용하는 경우 클라우드 제공자와 지역을 지정하십시오.
4. **Create Datashare**을 선택하십시오.

얼마 후에 데이터 공유가 Snowflake 인스턴스에 나타납니다. 공유에서 데이터베이스를 생성하여 테이블을 보고 쿼리할 수 있습니다. 데이터 공유를 보려면 계정 관리자여야 합니다.

![인바운드 데이터 공유]({% image_buster /assets/img/inbound-data-share.png %})

데이터 공유 맥락에서, Braze는 [데이터 제공자](https://docs.snowflake.net/manuals/user-guide/data-sharing-intro.html#providers)이며, Snowflake 계정을 통해 공유를 생성하고 다른 Snowflake 계정이 소비할 수 있도록 지원합니다. 당신은 [데이터 소비자](https://docs.snowflake.net/manuals/user-guide/data-sharing-intro.html#consumers)입니다. 데이터 제공자가 제공한 공유로부터 데이터베이스를 생성하기로 선택한 모든 계정입니다.

## 사용 및 시각화

데이터 공유가 프로비저닝되면, 수신 데이터 공유에서 데이터베이스를 생성해야 합니다. 그래야 공유된 모든 테이블이 Snowflake 인스턴스에 나타나고 인스턴스에 저장된 다른 데이터처럼 쿼리할 수 있습니다. 그러나 공유된 데이터는 읽기 전용이며 쿼리만 가능하고 수정하거나 삭제할 수 없습니다.
...
[Braze가 빌드한 데이터 툴](https://www.braze.com/perspectives/benchmarks)인 벤치마크를 통해 Braze 잠재 고객과 고객이 자신의 측정기준을 Braze의 업계 벤치마크와 비교하여 업계 최고 기업과 어떻게 비교되는지 확인할 수 있습니다.
