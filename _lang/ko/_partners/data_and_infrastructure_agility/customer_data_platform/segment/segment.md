...
- [두 플랫폼에서 데이터를 가져옵니다](#integration-options). Android, iOS, 웹 애플리케이션을 위한 나란히 배치된 SDK 통합과 데이터를 Braze의 REST API에 동기화하기 위한 서버 간 통합을 제공합니다.
...
Segment에서 **대상 > Braze > Braze 구성 > 소스 선택 > 설정**으로 이동합니다.
...
| 통합 | 세부 정보 |
| ----------- | ------- |
| [병렬<br>(기기 모드)](#side-by-side-sdk-integration) |Segment의 SDK를 사용하여 이벤트를 Braze의 기본 호출로 변환함으로써 서버 간 통합보다 더 심층적인 기능에 액세스하고 더 포괄적으로 Braze를 사용할 수 있습니다.<br><br>Segment는 일부 Braze 메서드(예: 콘텐츠 카드)를 지원하지 않습니다. 해당 매핑을 통해 매핑되지 않은 Braze 메서드를 사용하려면 코드베이스에 네이티브 Braze 코드를 추가하여 메서드를 호출해야 합니다. |
...
Braze를 Android 소스의 기기 모드 대상으로 설정하려면 대상 프레임워크로 **클래식**을 선택하고 **저장**을 클릭합니다. 
...
Braze를 iOS 소스의 기기 모드 대상으로 설정하려면 대상 프레임워크로 **클래식**을 선택하고 **저장**을 클릭합니다. 
...
Braze를 웹 소스의 기기 모드 대상으로 설정하려면 Segment의 새로운 Braze 웹 모드(작업) 프레임워크가 권장됩니다. 

설정 UI에서 대상 프레임워크로 **작업**을 선택하고 연결 모드로 **기기 모드**를 선택합니다.
...
Braze를 각 소스의 기기 모드 대상으로 설정하려면 대상 프레임워크로 **클래식**을 선택하고 **저장**을 클릭합니다.
...
클라우드 모드라고도 하는 이 통합은 Segment의 데이터를 Braze의 REST API로 전달합니다. 세그먼트의 새로운 [Braze 클라우드 모드(작업)](https://segment.com/docs/connections/destinations/catalog/braze-cloud-mode-actions/) 프레임워크를 사용하여 모든 소스에 대한 클라우드 모드 대상을 설정하세요. 

병렬 통합과 달리 서버 간 통합은 인앱 메시징, 콘텐츠 카드 또는 자동 푸시 토큰 등록과 같은 Braze의 UI 기능을 지원하지 않습니다. 클라우드 모드에서는 사용할 수 없는 [자동 캡처]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/#user-data-collection)된 데이터(예: 익명 사용자 및 기기 수준 필드)도 존재합니다.
...
| 인앱 메시지 z 인덱스 | 이 옵션의 값을 제공하여 Braze의 기본 z-인덱스를 재정의합니다. | 
...
| 외부에서 서비스 종사자 관리 | 사용자가 등록하고 생애주기를 제어하는 자체 서비스 종사자가 있는 경우 이 옵션을 `TRUE`로 설정하면 Braze SDK는 서비스 종사자를 등록하거나 등록 취소하지 않습니다. 이 옵션을 `TRUE`로 설정한 경우 푸시가 올바르게 작동하려면 `requestPushPermission`으로 호출하기 전에 서비스 종사자를 직접 등록하고 `self.importScripts('https://js.appboycdn.com/web-sdk-develop/4.1/service-worker.js');` 또는 해당 파일의 콘텐츠를 직접 포함하여 Braze의 서비스 종사자 코드가 포함되어 있는지 확인해야 합니다. 이 옵션이 `TRUE`인 경우 `serviceWorkerLocation` 옵션은 관련이 없으며 무시됩니다. |
...
다른 모든 특성은 [사용자 지정 속성으로]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/) 기록됩니다.
...
| [제품으로 추적](https://segment.com/docs/spec/track/) | [구매 이벤트로]({{site.baseurl}}/developer_guide/platform_integration_guides/web/analytics/logging_purchases/) 기록됩니다. | 세그먼트: `analytics.track("Order Completed", {products: [product_id: "ab12", price: 19]});` <br>Braze: `Braze.logPurchase("ab12", 19);` |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

##### 주문 완료 {#order-completed}

Segment의 [이커머스 API](https://segment.com/docs/spec/ecommerce/v2/)에 설명된 형식을 사용하여 `Order Completed`라는 이름의 이벤트를 추적하면, 등록한 제품을 [구매]({{site.baseurl}}/user_guide/data_and_analytics/export_braze_data/exporting_revenue_data/#revenue-data)로 기록합니다.
