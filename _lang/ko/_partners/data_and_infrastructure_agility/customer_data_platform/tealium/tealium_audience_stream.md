...
{% alert important %}
Tealium AudienceStreams 및 EventStreams는 배치 및 비배치 커넥터 작업을 모두 제공합니다. 비배치 커넥터는 실시간 요청이 사용 사례에 중요하고 Braze의 API 사용량 제한 사양에 도달할 염려가 없는 경우에 사용해야 합니다. 궁금한 점이 있으면 Braze [지원팀]({{site.baseurl}}/braze_support/) 또는 고객 성공 관리자에게 문의하세요.
{% endalert %}
...
[6]: {{site.baseurl}}/api/basics?redirected=true#엔드포인트
