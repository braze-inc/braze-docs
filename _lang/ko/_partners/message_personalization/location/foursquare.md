...
| Braze REST API 키 | A Braze REST API key with `users.track` permissions. <br><br> 이것은 **설정** > **API 키**에서 Braze 대시보드에서 생성할 수 있습니다. |
...
### 2단계: Pilgrim 콘솔 구성
![그룹 ID, Android 앱 ID, iOS 앱 ID를 묻는 Pilgrim 콘솔의 이미지.]({% image_buster /assets/img_archive/pilgrim-dev-console.png %}){: style="float:right;max-width:40%;margin-left:15px;"}
...
!["커스텀 이벤트 수행" 옵션으로 "도착"을 선택한 전달 단계의 실행 기반 캠페인으로, "위치 유형"이 "집"과 같습니다.]({% image_buster /assets/img_archive/action-based-campaign.png %})
...
![타겟 사용자 단계에서 "last_location" 을 표시하는 액션 기반 캠페인은 '피자 플레이스'와 동일합니다.]({% image_buster /assets/img_archive/last-location-segment.png %})
