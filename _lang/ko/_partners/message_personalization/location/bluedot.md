...
![전달 단계의 액션 기반 캠페인입니다. 여기에는 사용자가 커스텀 `bluedot_entry` 또는 `bluedot_exit` event.]({%image_buster /assets/img_archive/Campaign-Delivery-BD.png %})을 수행하면 캠페인을 전송하는 두 가지 예약 옵션이 있습니다.{: style="max-width:80%"}

#### 사용자 타겟팅

워크스페이스에 대해 **모든 사용자**를 타겟팅해야 합니다.
![타겟팅 사용자 단계가 있는 액션 기반 캠페인으로 "모든 사용자"를 원하는 segment.]({%image_buster /assets/img_archive/Campaign-Target_users-BD.png %}로 선택하도록 권장합니다.){: style="max-width:80%"}
