...
> Crowdin은 클라우드 기반 현지화 관리 플랫폼입니다. Crowdin을 사용하면 Android 및 iOS 앱, 웹사이트, 스토어 스크린샷 및 기타 콘텐츠를 번역할 수 있습니다. 번역은 사내 팀, 번역 에이전시 또는 기계 번역 엔진을 통해 수행할 수 있습니다.
...
![몇 가지 기본 번역이 추가된 Crowdin 편집기 이메일 작성기의 이미지.]({% image_buster /assets/img/crowdin/crowdin_editor_email_preview.png %})

### 3단계: Braze에 번역 추가하기

번역이 완료되면 Crowdin에서 Braze 앱을 열고 왼쪽 패널에서 번역된 파일(각 파일에 대해 모든 대상 언어 또는 특정 언어만 선택할 수 있음)을 선택한 후 **Braze에 동기화를** 클릭합니다.

![사용자가 번역 파일을 선택하고 Braze에 동기화하는 이미지입니다.]({% image_buster /assets/img/crowdin/sync_translations.png %})
