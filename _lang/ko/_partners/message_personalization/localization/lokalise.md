...
Braze와 Lokalise의 통합은 연결된 콘텐츠를 사용하여 사용자 언어 설정을 기반으로 번역된 콘텐츠를 Braze 캠페인에 쉽게 삽입할 수 있도록 합니다.
...
### `en` 로캘을 가지고 있지만 Lokalise에서 `en-US` 로 재정의하면 Braze가 `en-US` 로 읽나요?

아니요. Braze와 Lokalise에서 로캘 ISO 코드가 일치해야 합니다.

### Lokalise 콘텐츠를 연결할 때 `:rerender` 플래그를 사용할 수 있나요?

네, 물론이죠. 이 플래그를 추가하는 방법은 Braze 문서를 참조하세요.

### Lokalise에서 번역 파일을 새로고침한 후 Braze에서 번역된 콘텐츠의 변경 사항을 볼 수 없는 이유는 무엇인가요?
