...
> [클라우디너리는](https://www.cloudinary.com?utm_source=braze_partner_page) 채널과 고객 여정에 걸쳐 모든 캠페인에 이미지와 비디오를 대규모로 관리, 편집, 최적화 및 전달하는 데 사용되는 이미지 및 비디오 플랫폼입니다. 통합 및 인에이블된 Cloudinary의 미디어 매니저는 Braze 캠페인과 캔버스를 위한 동적, 상황별, 개인화된 자산 전달을 지원합니다. 
...
`f_auto:animated` [Cloudinary 변환을](https://cloudinary.com/documentation/image_transformations/) 사용하여 동영상 자산을 GIF로 자동 변환할 수 있습니다. GIF는 이메일 페이로드를 줄이기 위해 최적화되어 있으며, 너무 높으면 전달 가능성 문제를 일으킬 수 있으므로 Braze 이메일 채널을 사용하는 경우 특히 유용합니다. 
