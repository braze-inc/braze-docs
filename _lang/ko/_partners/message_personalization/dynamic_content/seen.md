...
nav_title: 본
article_title: 본
description: "SEEN의 개인화된 동영상은 기업이 고객 여정 전반에서 타의 추종을 불허하는 관심과 참여를 이끌어내는 데 도움을 줍니다."
alias: /partners/seen/
page_type: partner
search_tag: Partner
---
...
> [SEEN의](https://seen.io/) 개인화된 동영상은 기업이 고객 여정 전반에서 타의 추종을 불허하는 관심과 참여를 이끌어내는 데 도움을 줍니다. 간단한 3단계로 SEEN으로 동영상을 맞춤 설정하세요:<br>1\. 데이터를 중심으로 동영상을 디자인하세요.<br>2\. 클라우드에서 대규모로 개인화하세요.<br>3\. 가장 적합한 곳에 배포하세요.

## 사용 사례

SEEN은 전체 고객 여정에서 자동화된 비디오 개인화 기능을 제공합니다. 일반적으로 온보딩, 로열티, 가입/전환, 윈백/탈락 방지 등의 용도로 사용됩니다.
...
SEEN API는 현재 시간당 1,000건의 호출을 수락합니다.

## SEEN과 Braze 통합

다음 예시에서는 동영상 생성을 위해 사용자 데이터를 SEEN으로 전송하고, 배포를 위해 고유한 랜딩 페이지 링크와 개인화된 고유 썸네일을 Braze로 다시 받게 됩니다. 이 예에서는 POST 웹훅을 사용하여 데이터를 SEEN으로 전송하고 데이터 변환을 통해 데이터를 Braze로 다시 수신합니다. SEEN을 사용하는 동영상 캠페인이 여러 개 있는 경우, 이 과정을 반복하여 모든 동영상 캠페인에 Braze를 연결합니다.

{% alert tip %}
문제가 발생하면 SEEN의 고객 성공 관리자에게 문의하여 도움을 받으세요.
{% endalert %}

### 1단계: 웹훅 캠페인 생성

브레이즈에서 새 [웹훅 캠페인을](https://www.braze.com/docs/user_guide/message_building_by_channel/webhooks) 생성합니다. 캠페인에 이름을 지정한 다음 다음 표를 참조하여 웹훅을 작성하세요:
...
모든 것이 의도한 대로 작동하면 Braze로 이동하여 캠페인 전송 속도를 **분당** 10개의 **메시지로** 설정합니다. 이렇게 하면 SEEN의 시간당 통화 수 제한인 1,000건을 초과하지 않습니다.

### 2단계: 데이터 변환 생성

1. `landing_page_url` 및 `email_thumbnail_url` 에 대한 새 [사용자 지정 속성](https://www.braze.com/docs/user_guide/data_and_analytics/custom_data/custom_attributes/#managing-custom-attributes) 필드를 만듭니다. 이 예제에서 사용할 두 가지 속성은 다음과 같습니다.
2. **데이터 설정에서** [데이터 변환](https://www.braze.com/docs/user_guide/data_and_analytics/data_transformation/creating_a_transformation/#prerequisites) 도구를 열고 **변환 만들기를** 선택합니다.
...
{: start="6"}
6\. 제공된 엔드포인트로 테스트 페이로드를 전송합니다. [SEEN 문서에](https://docs.seen.io/api-documentation/ntRoJJ3rXoHzFXhA94JiHB/callbacks/k9DEbcgkq3Vr2pxbHyPQbp) 정의된 콜백 페이로드를 사용하려면 [Postman](https://www.postman.com/) 또는 다른 유사한 서비스를 통해 직접 보낼 수 있습니다:
...
8\. 모든 것이 의도한 대로 작동하면 **저장** 및 **활성화를** 선택합니다.
