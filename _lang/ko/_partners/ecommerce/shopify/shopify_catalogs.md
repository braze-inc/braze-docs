...
!['Shopify 배리언트 ID'가 '카탈로그 제품 식별자'인 설정 프로세스의 3단계.]({% image_buster /assets/img/Shopify/sync_products_step1.png %}){: style="max-width:70%;"}

Braze 카탈로그에 동기화된 제품은 [카탈로그 한도에]({{site.baseurl}}/user_guide/data/activation/catalogs/create/#tiers) 영향을 미칩니다.
...
Note that if your initial sync exceeds your [catalog limit]({{site.baseurl}}/user_guide/data/activation/catalogs/create/#tiers), Braze will stop syncing any more products. If you exceed the limit after the sync is successful due to new products being added over time, the sync will no longer be active. In both these cases, product updates from Shopify will no longer be reflected in Braze. 티어 업그레이드를 고려하려면 계정 매니저에게 문의하세요. 
