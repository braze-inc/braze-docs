...
![오디언스 이름, 오디언스 유형, 오디언스 크기, 오디언스 유지 일수를 포함하는 지정된 Snapchat 오디언스의 오디언스 세부 정보.][9]
...
{% details 잘못된 토큰 오류가 발생하면 다음에 무엇을 해야 하나요? %}
Snapchat 파트너 페이지에서 Snapchat 계정 연결을 끊었다가 다시 연결할 수 있습니다. 동기화하려는 광고 계정에 대한 적절한 권한이 있는지 Snapchat 비즈니스 매니저에게 확인합니다.
{% enddetails %}

{% details 왜 내 캔버스가 시작되지 않나요? %}
스냅챗 파트너 페이지에서 스냅챗 광고 계정이 Braze에 성공적으로 연결되었는지 확인합니다. 선택한 광고 계정이 있고, 새 오디언스의 이름을 입력했으며, 일치시킬 필드를 선택했는지 확인하십시오.
{% enddetails %}

{% details Snapchat에 사용자를 전달한 후 사용자가 일치하는지 어떻게 알 수 있나요? %}
Snapchat은 데이터 프라이버시 정책과 관련해 이 정보를 제공하지 않습니다.
{% enddetails %}

{% details Snapchat이 지원할 수 있는 오디언스는 몇 명인가요? %}
현재 Snapchat 계정에서는 1,000명의 오디언스만 보유할 수 있습니다.
이 제한을 위반하는 경우, Braze는 새로운 오디언스를 생성할 수 없음을 알립니다.
Snapchat 광고 계정으로 이동하여 더 이상 사용하지 않는 오디언스를 제거해야 합니다.
{% enddetails %}
