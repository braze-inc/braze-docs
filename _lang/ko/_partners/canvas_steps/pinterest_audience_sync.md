...
확인을 선택하면 다시 Braze로 리디렉션되어 동기화할 Pinterest 광고 계정을 선택할 수 있습니다. 
...
Pinterest 파트너 페이지에서 Pinterest 계정을 간단히 연결 해제하고 다시 연결할 수 있습니다. Pinterest 비즈니스 허브 관리자에게 동기화하려는 광고 계정에 대한 적절한 권한이 있는지 확인하십시오.
{% enddetails %}

{% details 왜 내 캔버스가 시작되지 않나요? %}
Pinterest 계정이 Pinterest 파트너 페이지에서 Braze에 성공적으로 연결되도록 하십시오.
선택한 광고 계정이 있고, 새 오디언스의 이름을 입력했으며, 일치시킬 필드를 선택했는지 확인하십시오.
{% enddetails %}

{% details Pinterest에 사용자를 전달한 후 사용자가 일치하는지 어떻게 알 수 있나요? %}
Pinterest는 자체 데이터 프라이버시 정책에 대해 이 정보를 제공하지 않습니다.
{% enddetails %}

{% details Pinterest에 오디언스를 채우는 데 얼마나 걸리나요? %}
오디언스 크기는 Pinterest의 광고 매니저의 오디언스 페이지에서 24~48시간 이내에 업데이트됩니다.
{% enddetails %}
