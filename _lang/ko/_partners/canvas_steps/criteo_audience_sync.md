...
브레이즈 오디언스 동기화를 사용하여 Criteo에 브랜드는 자신의 브레이즈 통합에서 사용자 데이터를 Criteo 고객 목록에 추가하여 행동 트리거, 세분화 등을 기반으로 광고를 제공할 수 있습니다. 브레이즈 캔버스에서 사용자 데이터를 기반으로 메시지(푸시, 이메일, SMS, 웹훅 등)를 트리거하는 데 일반적으로 사용하는 모든 기준을 이제 Criteo 고객 목록에서 해당 사용자에게 광고를 트리거하는 데 사용할 수 있습니다.
...
{% tab 기존 오디언스와 동기화 %}
**기존 오디언스와 동기화**<br>
브레이즈는 또한 기존 Criteo 오디언스에 사용자를 추가할 수 있는 기능을 제공하여 이러한 오디언스가 최신 상태인지 확인합니다. 기존 오디언스와 동기화하려면 드롭다운에 기존 오디언스 이름을 입력하고 **오디언스에 추가합니다**. 그러면 Braze는 오디언스 동기화 구성요소에 사용자가 들어오는 대로 거의 실시간으로 사용자를 추가합니다.
...
오디언스 동기화를 Criteo에 구성한 후에는 캔버스를 실행하기만 하면 됩니다! 새 오디언스가 생성되며, 오디언스 동기화 단계를 거친 사용자는 Criteo의 이 오디언스로 전달됩니다. 캔버스에 후속 구성요소가 포함되어 있으면 사용자는 사용자 여정의 다음 단계로 진행하게 됩니다.
...
사용자가 오디언스 동기화 단계에 도달하면, Braze는 Criteo의 API 사용량 제한을 준수하면서 거의 실시간으로 이러한 사용자를 동기화합니다. 이것이 실제로 의미하는 바는 브레이즈가 Criteo에 이러한 사용자를 보내기 전에 매 5초마다 가능한 한 많은 사용자를 배치하고 처리하려고 시도한다는 것입니다.

Criteo의 API 속도 제한은 분당 250개 요청을 초과할 수 없습니다. Braze 고객이 이 사용량 제한에 도달하면 Braze 캔버스는 최대 13시간 동안 동기화를 다시 시도합니다. 동기화가 불가능한 경우 이러한 사용자는 사용자 오류 측정기준 아래에 나열됩니다. 
...
Criteo 파트너 페이지에서 귀하의 Criteo 광고 계정이 브레이즈에 성공적으로 연결되었는지 확인하십시오. 다음으로 광고 계정을 선택하고 새 오디언스의 이름을 입력하고 일치할 필드를 선택했는지 확인하십시오.

### 사용자를 Criteo로 전달한 후 사용자가 일치했는지 어떻게 알 수 있습니까?

Criteo는 데이터 프라이버시 정책과 관련해 이 정보를 제공하지 않습니다.

### Criteo는 몇 개의 오디언스를 지원할 수 있습니까?

현재 Criteo 계정에서는 1,000명의 오디언스만 보유할 수 있습니다. 이 한도를 초과하면 브레이즈가 새 오디언스를 생성할 수 없음을 알려줍니다. Criteo 광고 계정에서 더 이상 사용하지 않는 오디언스를 제거해야 합니다.
