...
Branch와 Braze 통합은 강력한 기여도 분석과 [딥링킹]({{site.baseurl}}/partners/channel_extensions/deep_linking/branch_for_deeplinking/)을 통해 사용자의 여정을 개인화하는 방법은 물론, 사용자 유치 시기와 위치를 정확히 이해할 수 있도록 지원합니다.
...
1. Branch의 **내보내기** 섹션에서 **데이터 피드**를 클릭합니다.
2. **데이터 피드 관리자** 페이지에서 페이지 상단의 **데이터 통합** 탭을 클릭합니다. 
3. 사용 가능한 데이터 파트너 목록에서 Braze를 선택합니다. 
4. Braze 내보내기 페이지에서 Braze 대시보드에서 찾은 데이터 가져오기 키와 REST 엔드포인트를 제공하고 **활성화**를 클릭합니다.
