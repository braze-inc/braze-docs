...
> [Yotpo](https://www.yotpo.com/), the leading eCommerce marketing platform, helps thousands of forward-thinking brands accelerate direct-to-consumer growth. Yotpo의 단일 플랫폼 접근 방식은 리뷰, 로열티, SMS 마케팅 등을 위한 데이터 기반 솔루션을 통합하여 브랜드가 더 스마트하고 전환율이 높은 고객 경험을 만들 수 있도록 지원합니다.
...
`<YOTPO-API-KEY>`를 Yotpo 리뷰 API 키로 바꿉니다. `product_id`는 Braze에서 동적으로 가져옵니다. For the integration to work, the `product_id` in Braze must match the product ID in Yotpo (typically the eCommerce parent product ID).
