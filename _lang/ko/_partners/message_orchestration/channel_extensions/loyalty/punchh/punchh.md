...
Punchh `user_id`는 통합에 사용되기 위해 Braze 고객 프로필에 커스텀 속성 'punchh_user_id'로 추가되어야 합니다. 마찬가지로 Braze에서 사용되는 `external_id`는 Punchh 고객 프로필의 `external_source_id` 필드에 포함되어야 합니다. 

### 1단계: 외부 ID 수집 엔드포인트 설정
