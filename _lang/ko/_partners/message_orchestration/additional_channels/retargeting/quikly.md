...
2. 사용자를 기록하거나 업데이트하십시오.
  ...
  - 사용자가 존재하지 않는 경우:
    - Quikly는 Braze의 [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)를 통해 별칭 전용 프로필을 생성하며, 사용자의 이메일을 사용자 별칭으로 설정하여 향후 해당 사용자를 참조합니다 (사용자는 외부 ID를 가지지 않기 때문입니다).
...
2. Quikly는 [식별자 엔드포인트로 고객 프로필 내보내기]({{site.baseurl}}/api/endpoints/export/user_data/post_users_identifier/)를 사용하여 주어진 `email_address`에 고객 프로필이 존재하는지 확인합니다. 사용자가 존재하지 않는 경우, Braze의 [`/users/track` 엔드포인트]({{site.baseurl}}/api/endpoints/user_data/post_user_track/)를 통해 별칭 전용 프로필을 생성하고, 사용자의 이메일을 사용자 별칭으로 설정하여 향후 해당 사용자를 참조합니다 (사용자가 외부 ID를 가지지 않기 때문입니다).
...
기존의 이중 옵트인 SMS 가입 워크플로를 지원하기 위해, Quikly는 위의 워크플로 대신 Braze에 커스텀 이벤트를 보낼 수 있습니다. 이 경우, 가입 상태를 직접 업데이트하는 대신, [커스텀 이벤트가 이중 옵트인 프로세스를 트리거]({{site.baseurl}}/user_guide/message_building_by_channel/sms/non_native/double_opt_in/)하고 가입 상태를 주기적으로 모니터링하여 Quikly 활성화에서 크레딧을 부여하기 전에 사용자가 완전히 옵트인했는지 확인합니다.
...
{% alert note %}
[이전 탐색을]({{site.baseurl}}/navigation) 사용하는 경우 **참여** > **템플릿 및 미디어** > **웹훅 템플릿으로** 이동합니다.
{% endalert %}
