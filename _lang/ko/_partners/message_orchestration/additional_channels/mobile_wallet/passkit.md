...
고객의 모바일 지갑 경험을 더욱 풍부하게 하려면 PassKit 대시보드 내에서 Braze의 [`/users/track` 엔드포인트][7]를 통해 데이터를 Braze로 전달하도록 선택할 수 있습니다. 
...
{% alert note %}
[이전 탐색을]({{site.baseurl}}/navigation) 사용하는 경우 **참여** > **템플릿 및 미디어** > **콘텐츠 블록 라이브러리로** 이동합니다.
{% endalert %}
...
패스를 생성하고 업데이트하는 것 외에도 Braze의 [연결된 콘텐츠][15]를 통해 사용자의 패스 메타데이터를 검색하여 메시징 캠페인에 개인화된 패스 세부 정보를 통합할 수도 있습니다.
