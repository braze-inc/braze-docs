...
대상 데이터 형식은 HTTPS를 통한 JSON입니다. 이벤트는 크기를 구성할 수 있는 이벤트 배치로 그룹화되어 모든 이벤트가 포함된 JSON 배열로 엔드포인트로 전송됩니다. 배치가 전송되는 형식은 다음과 같습니다:
...
필요한 경우 [RFC 6750](https://tools.ietf.org/html/rfc6750#section-2.1)에 명시된 대로 `Bearer` 권한 부여 스키마를 통해 HTTP `Authorization` 헤더에 토큰을 전달함으로써 인증을 수행합니다. 또한 `Authorization` 헤더를 사용하면 향후 원하는 경우 [RFC 7235](https://tools.ietf.org/html/rfc7235)를 준수하는 커스텀(Braze 고유) 키-값 페어 권한 부여 스키마로 전환할 수 있으므로(예: AWS의 커스텀 인증 스키마 작동 방식) 향후 구현할 수 있는 모든 커스텀 인증 스키마와 자동으로 정방향 호환이 가능합니다.

RFC 6750에 따라 토큰은 최소 하나의 문자(Base64 인코딩된 값)입니다. (물론 파트너와 고객이 매우 취약한 토큰을 선택할 가능성이 적다는 점을 확인할 수 있도록 파트너와 고객을 심사해야 합니다.) RFC 6750에서 주목할 사항은 토큰에 일반 Base64 문자 외에 '-', '.', '_', '~'의 문자를 포함할 수 있다는 점입니다. 토큰의 정확한 내용은 당사 시스템에 전혀 영향을 미치지 않으므로 파트너가 토큰에 이러한 문자를 포함시키기로 결정하든 그렇지 않든 상관하지 않습니다.
...
개별 이벤트는 기존 Avro 스키마와 동일한 진화 규칙을 따릅니다. 즉, 모든 이벤트의 필드는 다음 규칙을 포함한 역호환성에 대한 Avro의 정의에 따라 이전 버전의 이벤트 페이로드와 역호환이 보장됩니다.
...
- 필수 입력란은 절대 삭제되지 않습니다.
  - '필수'로 간주되는 항목은 설명서를 통해 지정됩니다. 이 설명서는 Avro 스키마에서 중앙 소스로 자동 생성할 가능성이 큽니다. 이를 위해서는 Avro 스키마 필드에 일부 메타데이터를 주석으로 추가해야 하며, 해당 메타데이터를 읽을 수 있는 특수 스크립트를 통해 설명서를 생성해야 합니다.
...
- **401** (승인되지 않음), **403** (금지됨), **404**
  - 커넥터가 잘못된 자격 증명으로 구성되었습니다. 커넥터 작업이 전송을 중지하고 "실패"로 표시됩니다. 이벤트 데이터는 2~5분 정도 지연 후에 다시 전송됩니다(연결 작업 다시 시작기에서 처리됨). 고객이 48시간 이내에 이 문제를 해결하지 않으면 이벤트 데이터가 삭제됩니다.<br><br>
- **413** \- 페이로드가 너무 큼
