...
| `401` | 잘못된 파트너 API 키 |
|  | 잘못된 클라이언트 비밀 |
|  | 클라이언트 비밀이 있는 클라이언트에 대해 파트너가 인에이블먼트되지 않았습니다: **<client secret>** |
|  | 무단 액세스 |
| `423` | 리소스 잠김 |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

추가적인 문제 해결 방법은 [오류 & 응답을]({{site.baseurl}}/api/errors/) 참조하세요. 여기에는 Braze API를 사용하는 동안 발생할 수 있는 다양한 오류와 서버 응답에 대한 설명이 나와 있습니다.
