...
{% alert note %}
Braze verarbeitet die über die API übergebenen Daten zum Nennwert. Kunden:in sollten nur Deltas (sich ändernde Daten) übergeben, um die unnötige Protokollierung von Datenpunkten zu minimieren. Um mehr zu erfahren, referenzieren Sie auf [Datenpunkte]({{site.baseurl}}/user_guide/data/data_points/).
{% endalert %}
...
### Rate-Limits-Kopfzeilen für monatlich aktive Nutzer:innen CY 24-25

Alle Antworten ohne Rate-Limits (z.B. nicht`429`) enthalten die folgenden HTTP-Antwort-Header, die dem Client den Status des stündlichen Rate-Limits-Fensters anzeigen. Wir empfehlen Ihnen, diese Anfrage-Header zu verwenden, um Ihre Anfrage-Rate zu verwalten:
