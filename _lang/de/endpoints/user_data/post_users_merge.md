...
Wenn ein `email` oder `phone` als Bezeichner angegeben wird, ist ein zusätzlicher `prioritization` Wert im Bezeichner erforderlich. Bei `prioritization` sollte es sich um ein geordnetes Array handeln, das angibt, welcher Nutzer:innen zusammengeführt werden soll, wenn mehrere Nutzer:innen gefunden werden. Das heißt, wenn mehr als ein Nutzer:innen aus einer Priorisierung übereinstimmt, wird die Zusammenführung nicht stattfinden.
...
Die Verwendung von `most_recently_updated` filtert die Abfragen auf nur einen Nutzer:innen (einen nicht identifizierten Nutzer für `identifier_to_merge` und einen identifizierten Nutzer für `identifier_to_keep`).
