...
page_order: 3
description: "Auf dieser Seite erfahren Sie, wie Sie vollständige Ereignisdaten aufbewahren, wenn die Braze Aufbewahrungsrichtlinie angewendet wird."
page_type: partner
search_tag: Partner
---

# Snowflake Datenspeicherung

> Braze anonymisiert - entfernt persönlich identifizierbare Informationen (PII) - aus allen Ereignisdaten, die älter als zwei Jahre sind. Wenn Sie die Snowflake-Datenfreigabe verwenden, können Sie die vollständigen Ereignisdaten in Ihrer Umgebung aufbewahren, indem Sie eine Kopie in Ihrem Snowflake-Konto speichern, bevor die Aufbewahrungsrichtlinie angewendet wird.
...
{% alert warning %}
Braze anonymisiert automatisch die Ereignisdaten von Benutzern, die von Braze gelöscht werden, wie in der [Technischen Hilfe zum Datenschutz]({{site.baseurl}}/dp-technical-assistance/) beschrieben. Alle Daten, die außerhalb der gemeinsamen Datenbank kopiert werden, werden bei diesem Vorgang nicht berücksichtigt, da Braze sie nicht mehr verwaltet.
{% endalert %}

## Kopieren Sie alle Daten in eine andere Snowflake-Datenbank

Sie können nicht anonymisierte Daten beibehalten, indem Sie Ihre Daten aus dem gemeinsamen Schema `BRAZE_RAW_EVENTS` in eine andere Datenbank und ein anderes Schema in Snowflake kopieren. Gehen Sie dazu folgendermaßen vor:
...
{% alert note %}
Durch die wiederholte Ausführung der Prozedur werden keine doppelten Datensätze erstellt, da diese Prozedur die aktuellste `SF_CREATED_AT` prüft und nur Daten sichert, die neuer sind.
{% endalert %}

## Daten auf die Bühne entladen
...
1. Erstellen Sie die Prozedur `UNLOAD_BRAZE_SHARE`, mit der Sie alle von Braze freigegebenen Daten in die angegebene Stufe kopieren.
