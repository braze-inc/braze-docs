...
page_order: 1
description: "Auf dieser Partnerseite finden Sie einige Beispielabfragen für mögliche Anwendungsfälle, auf die Sie sich bei der Einrichtung Ihrer Snowflake-Abfragen beziehen können."
page_type: partner
search_tag: Partner
...
> Auf dieser Partnerseite finden Sie einige Beispielabfragen für mögliche Anwendungsfälle, auf die Sie sich bei der Einrichtung Ihrer Abfragen beziehen können.
...
Sie können sie nach dem Zeitpunkt des Auftretens filtern. Die Ereignistabellen sind nach `time` geclustert, so dass die Filterung nach `time` optimal ist:
```sql
...
LIMIT 10;
```
Sie können Ereignisse auch nach dem Zeitpunkt filtern, zu dem sie im Snowflake Data Warehouse persistiert wurden, indem Sie `sf_created_at` verwenden. `sf_created_at` und `time` sind nicht identisch, liegen aber in der Regel nahe beieinander, so dass diese Abfrage ähnliche Leistungsmerkmale haben sollte:
```sql
...
```
{% alert note %}
Der Wert von `sf_created_at` ist nur für Ereignisse verlässlich, die nach `Nov 15th, 2019 9:31 pm UTC` aufrechterhalten wurden.
{% endalert %}
{% endtab %}

{% tab Changelogs abfragen%}
  
Die Namen der Kampagnen und Canvas sind nicht in den Ereignissen selbst enthalten. Stattdessen werden sie in einer Änderungsprotokolltabelle veröffentlicht. 

Sie können die Namen von Kampagnen für Ereignisse, die sich auf eine Kampagne beziehen, sehen, indem Sie eine Abfrage wie diese mit der Kampagnen-Changelog-Tabelle verbinden:
...
- Die linke Verknüpfung stellt sicher, dass auch Ereignisse, die nicht mit einer Kampagne in Verbindung stehen, berücksichtigt werden.
- Wenn Sie Ereignisse mit `campaign_id`s sehen, aber keine Kampagnennamen, dann besteht die Möglichkeit, dass die Kampagne mit einem Namen erstellt wurde, bevor es Data Sharing als Produkt gab.
- Sie können die Namen von Leinwänden mit einer ähnlichen Abfrage anzeigen, wobei Sie stattdessen die Tabelle `CHANGELOGS_CANVAS_SHARED` verwenden.

Wenn Sie sowohl die Namen von Kampagnen als auch die Namen von Leinwänden sehen möchten, müssen Sie möglicherweise die folgende Unterabfrage verwenden:
```sql
...
```
{% endtab %}
{% tab Trichter schieben %}

Sie können diese Push-Trichter-Abfrage verwenden, um Push-Sende-Rohdaten zu aggregieren, bis hin zu Liefer-Rohdaten und Öffnungs-Rohdaten. Diese Abfrage zeigt, wie alle Tabellen miteinander verbunden werden sollten, da jedes Raw Event in der Regel eine eigene Tabelle hat:
...
{% endtab %}
{% tab E-Mail Kadenz %}
Sie können diese Abfrage für die tägliche E-Mail-Nachrichtenkadenz verwenden, um die Zeit zwischen den E-Mails zu analysieren, die ein Benutzer erhält.

Wenn ein Benutzer zum Beispiel zwei E-Mails an einem Tag erhalten hat, fallen diese unter `0 "days since last received"`. Wenn sie eine E-Mail am Montag und eine am Dienstag erhalten haben, würden sie zur Kohorte `1 "days since last received"` gehören.
...
```
{% endtab %}
{% tab Einzigartige E-Mail-Klicks %}
...
Sie können diese Abfrage für eindeutige E-Mail-Klicks verwenden, um die eindeutigen E-Mail-Klicks in einem bestimmten Zeitfenster zu analysieren. Der Algorithmus zur Berechnung lautet wie folgt:
  ...
  2. Ordnen Sie die Ereignisse in jeder Partition nach Zeit, wobei das erste Ereignis immer ein einmaliges Ereignis ist.
  3. Denn jedes nachfolgende Ereignis, das mehr als sieben Tage nach seinem Vorgänger eintritt, wird als einmaliges Ereignis betrachtet.
  
Dazu können wir die [Windowing-Funktionen](https://docs.snowflake.com/en/sql-reference/functions-analytic.html) von Snowflake verwenden. Die folgende Abfrage liefert uns alle E-Mail-Klicks der letzten 365 Tage und zeigt in der Spalte `is_unique` an, welche Ereignisse einzigartig sind:
...
```
Um die Anzahl einzigartiger Ereignisse gruppiert nach E-Mail-Adressen zu sehen:
```sql
