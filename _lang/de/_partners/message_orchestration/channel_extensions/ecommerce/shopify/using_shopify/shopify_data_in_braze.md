...
description: "Dieser Referenzartikel beschreibt, wie Sie Shopify-Daten in Braze für die Personalisierung und Segmentierung verwenden können."
page_type: partner
search_tag: Partner
alias: "/shopify_data/"
page_order: 1
...
> Mit der Unterstützung von verschachtelten Objekten für benutzerdefinierte Ereignisse können Braze Shopify-Kunden Liquid-Vorlagenvariablen der verschachtelten Ereigniseigenschaften verwenden.

Nachdem die Installation der App abgeschlossen ist, erstellt Braze automatisch Ihren Webhook und die ScriptTag-Integration mit Shopify. In der folgenden Tabelle finden Sie weitere Einzelheiten dazu, wie die unterstützten Shopify-Ereignisse den benutzerdefinierten Ereignissen und Attributen von Braze zugeordnet werden.
...
**Typ**: [Angepasstes Event]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/)


{% raw %}
| Variabel | Flüssigschablone |
| --- | --- |
| Artikel-ID | `{{event_properties.${id}}}` |
| Artikeltitel | `{{event_properties.${title}}}` |
| Artikelpreis | `{{event_properties.${price}}}` |
| Artikelverkäufer | `{{event_properties.${vendor}}}` |
| Artikelbilder | `{{event_properties.${images}}}` |
...
| Rabatt-Codes | `{{event_properties.${discount_codes}}}` |
| Artikel-ID | `{{event_properties.${line_items}[0].product_id}}` |
| Artikelmenge | `{{event_properties.${line_items}[0].quantity}}` |
| Artikel SKU | `{{event_properties.${line_items}[0].sku}}` |
| Artikeltitel | `{{event_properties.${line_items}[0].title}}` |
...
| Lieferadresse Stadt | `{{event_properties.${shipping_address[0].city}}}` |
| Lieferadresse Land | `{{event_properties.${shipping_address[0].country}}}` |
| Lieferadresse Vorname | `{{event_properties.${shipping_address[0].first_name}}}` |
...
| Rechnungsadresse Stadt | `{{event_properties.${billing_address[0].city}}}` |
...
**Typ**: [Braze Purchase Event]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/purchase_events/)
...
| --- | --- |
| Bestell-ID | `{{event_properties.${order_id}}}` |
| Bestellschein | `{event_properties.${note}}}` |
| Artikel-ID | `{{event_properties.${line_items}[0].product_id}}` |
| Artikelmenge | `{{event_properties.${line_items}[0].quantity}}` |
| Artikel SKU | `{{event_properties.${line_items}[0].sku}}` |
| Artikeltitel | `{{event_properties.${line_items}[0].title}}` |
...
## Unterstützte benutzerdefinierte Shopify-Attribute
{% tabs local %}
{% tab Shopify Benutzerdefinierte Attribute %}
| Attribut Name | Beschreibung |
| --- | --- |
| `shopify_tags` | Tags, die der Shop-Betreiber dem Kunden zugewiesen hat, formatiert als Zeichenfolge von durch Komma getrennten Werten. Ein Kunde kann bis zu 250 Schilder haben. Jedes Tag kann bis zu 255 Zeichen enthalten. |
| `shopify_total_spent` | Der Gesamtbetrag, den der Kunde in seiner Bestellhistorie ausgegeben hat. |
| `shopify_order_count` | Die Anzahl der Bestellungen, die mit diesem Kunden verbunden sind. Test- und archivierte Bestellungen werden nicht gezählt. |
| `shopify_last_order_id` | Die ID der letzten Bestellung des Kunden. |
| `shopify_last_order_name` | Der Name der letzten Bestellung des Kunden. Dies steht in direktem Zusammenhang mit dem Feld `name` in der Auftragsressource. |
| `shopify_zipcode` | Die Postleitzahl des Kunden aus seiner Standardadresse. |
| `shopify_province` | Die Provinz des Kunden aus seiner Standardadresse. |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

### Flüssige Personalisierung

Um eine Liquid-Personalisierung für Ihre benutzerdefinierten Shopify-Attribute hinzuzufügen, wählen Sie **\+ Personalisierung**. Wählen Sie dann **Benutzerdefinierte Attribute** als Ihren Personalisierungstyp.

![Der Bereich "Personalisierung hinzufügen" mit dem erweiterten Dropdown-Menü "Attribut".]({% image_buster /assets/img/Shopify/add_personalization_2.png %}){: style="max-width:40%;"}

Nachdem Sie Ihr benutzerdefiniertes Attribut ausgewählt haben, geben Sie einen Standardwert ein und kopieren das Liquid-Snippet in Ihre Nachricht.
...
{% alert note %}
Braze aktualisiert die unterstützten benutzerdefinierten Shopify-Attribute und die Braze-Standardattribute nur dann, wenn es einen Unterschied in den Daten des bestehenden Benutzerprofils gibt. Wenn die eingehenden Shopify-Daten beispielsweise den Vornamen Bob enthalten und Bob bereits als Vorname im Braze-Benutzerprofil vorhanden ist, löst Braze keine Aktualisierung aus, und Ihnen wird kein Datenpunkt berechnet.
{% endalert %}

## Segmentierung

Sie können die Shopify-Ereignisse mit allen [vorhandenen benutzerdefinierten Filtern]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/) in der Segmentierung filtern. 

![Segmentdetailseite für ein Shopify_Test-Segment mit dem Filter für das benutzerdefinierte Ereignis "shopify_checkouts_abandon" hervorgehoben.][12]{: style="max-width:80%;"}

Darüber hinaus können Sie auch den Braze-Filter für die Breite der Käufe verwenden, um Nutzersegmente auf der Grundlage von Kriterien zu erstellen:
- Erster/letzter Kauf
...
![Segmentierungsfilter für Nutzer, die nach dem 17\. Oktober 2020 zum ersten Mal einen Kauf getätigt haben.][13]

![Suchen Sie nach einer bestimmten Produkt-ID als Segmentierungsfilter.][14]

{% alert note %}
Wenn Sie nach benutzerdefinierten Ereigniseigenschaften segmentieren möchten, stellen Sie sicher, dass Sie mit Ihrem Customer Success Manager oder dem [Braze-Support]({{site.baseurl}}/braze_support/) zusammenarbeiten, um die Filterung für alle relevanten Ereigniseigenschaften zu aktivieren, die Sie für die Segmentierung und Liquid verwenden möchten.
{% endalert %} 

## Auslösen von Kampagnen und Canvas 

Mit den benutzerdefinierten Shopify-Ereignissen in Braze können Sie Canvases oder Kampagnen auslösen, wie Sie es mit jedem anderen [benutzerdefinierten Ereignis]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/#custom-event-storage) tun würden. Sie können zum Beispiel ein aktionsbasiertes Canvas erstellen, das durch das Shopify-Ereignis `shopify_checkouts_abandon` innerhalb der Canvas-Eingabekriterien ausgelöst wird. 

![Aktionsbasiertes Canvas, das Benutzer erfasst, die das benutzerdefinierte Ereignis "shopify_checkouts_abandon" ausführen.][5]

Mit der Unterstützung von verschachtelten Objekten für benutzerdefinierte Ereigniseigenschaften können Kunden jetzt Kampagnen und Canvases mithilfe einer verschachtelten Ereigniseigenschaft auslösen. Im Folgenden finden Sie ein Beispiel für die Auslösung einer Kampagne mit einem bestimmten Produkt aus dem benutzerdefinierten Ereignis `shopify_created_order`. Stellen Sie sicher, dass Sie `list_items[].product_id` verwenden, um Ihre Artikelliste zu indizieren und auf die Produkt-ID zuzugreifen.

![Aktionsbasierte Kampagne, die an Benutzer gesendet wird, die das benutzerdefinierte Ereignis "shopify_created_order" ausführen, bei dem die verschachtelte Eigenschaft "product_id" gleich einer bestimmten Zahl ist.][26]
