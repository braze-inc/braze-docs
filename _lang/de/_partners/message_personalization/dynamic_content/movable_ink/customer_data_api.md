...
![Tab des Webhook-Composers in Braze mit der URL des Movable Ink-Endpunkts und dem auf JSON Schlüssel-Wert-Paare eingestellten Request Body.][img1]{: style="max-width:75%" }
...
![Tab Einstellungen des Webhook-Composers in Braze mit Schlüssel-Wert-Paaren für Content-Type und Authorization.][img2]{: style="max-width:75%" }
...
![Tab des Webhook-Composers in Braze mit JSON Schlüssel-Wert-Paaren für ID, Zeitstempel, Nutzer:in und Ereignistyp.][img3]{: style="max-width:75%" }
...
![Webhook-Antwortnachricht in Braze mit einer 200 OK-Antwort.][img4]{: style="max-width:75%" }

### Schritt 2: Schließen Sie die Einrichtung Ihrer Kampagne ab

#### Schritt 2a: Zeitplan für Ihre Kampagne

Wenn Sie den Webhook fertiggestellt und getestet haben, [planen Sie Ihre Kampagne][2]. 

Braze unterstützt geplante, aktionsbasierte und API-ausgelöste Zustellungen. Die [aktionsbasierte Zustellung][3] ist in der Regel die beste Lösung für die meisten verhaltensorientierten Ereignisanwendungen. Wenn Sie Fragen dazu haben, was für Ihren Anwendungsfall sinnvoll ist, wenden Sie sich an Ihre Customer-Success-Manager von Braze und Movable Ink.
...
![A/B-Tests Panel in einer Braze-Kampagne mit 100%iger Verteilung der Variante 1 und ohne Kontrollgruppe.][img5]
...
Wenn Sie mehr über angepasste Event-Eigenschaften und das erwartete Format der in den Eigenschaften enthaltenen Daten erfahren möchten, referenzieren Sie auf [Angepasste Event-Eigenschaften][5].
