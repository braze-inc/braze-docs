...
## Canvas Flow
Die neueste Version des Canvas-Produkts, [Canvas Flow]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/create_a_canvas/#editing-a-step), wurde veröffentlicht. Mit Canvas Flow erhalten Sie Zugriff auf leichtgewichtige [Canvas-Komponenten]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components), [persistente Eingabeeigenschaften]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_persistent_entry_properties/) und die [Bearbeitung nach dem Start]({{site.baseurl}}/post-launch_edits).

![]({% image_buster /assets/img/canvas_flow.png %})

## Array von Objekten
Verwenden Sie ein [Array von Objekten]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/array_of_objects#array-of-objects), um verwandte Attribute zu gruppieren. Sie können z.B. eine Gruppe von Haustierobjekten, Songobjekten und Kontoobjekten haben, die alle zu einem Benutzer gehören. Diese Arrays von Objekten können verwendet werden, um Ihre Nachrichten mit Liquid zu personalisieren oder Zielgruppensegmente zu erstellen, wenn ein Element innerhalb eines Objekts den Kriterien entspricht.

## Aktualisierte Partner-Integrationen
...
- Die unter [Voraussetzungen]({{site.baseurl}}/partners/message_orchestration/channel_extensions/ecommerce/shopify/shopify/#prerequisites) aufgeführten erforderlichen Benutzerberechtigungen wurden aktualisiert.
...
## Vorhersage-Updates
Das maximale Zeitfenster für Abwanderungsprognosen und Kaufprognosen wurde von 14 Tagen auf 60 Tage erhöht.
...
- [React Native SDK 1.37.0](https://github.com/braze-inc/braze-react-native-sdk/blob/master/CHANGELOG.md#1370)
  - Das Braze React Native SDK exportiert jetzt sein Standardobjekt als ES-Modul. Wenn Sie das SDK derzeit über `require()` importieren, müssen Sie es jetzt als Standard-ES-Modul importieren (z.B. importieren Sie Braze von "`@braze/react-native-sdk`").
- [Android SDK 22.0.0](https://github.com/braze-inc/braze-android-sdk/blob/master/CHANGELOG.md#2200)
  - `Appboy.java` ist jetzt `Braze.kt`. Kotlin-Clients müssen ihren Code aktualisieren, um die Verwendung von Kotlin-Eigenschaften für das Braze-Singleton zu unterstützen, wo dies erforderlich ist.
