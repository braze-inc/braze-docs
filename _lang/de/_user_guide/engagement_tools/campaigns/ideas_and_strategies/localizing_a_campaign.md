...
page_order: 3
page_type: reference
description: "Der Artikel beschreibt die Vorteile der verschiedenen Orchestrierungsansätze für Kampagnen und Canvases und listet verschiedene Möglichkeiten auf, wie Benutzer die Personalisierung ihrer Nachrichten handhaben können."
tool:
  - Campaigns
  - Canvas
...
> Braze sammelt nach der Integration des SDK automatisch Gebietsschema-Informationen von Nutzergeräten. Das Gebietsschema enthält die Sprache und einen Bezeichner für die Region. Diese Informationen sind im Braze Segmentierungstool unter **Land** und **Sprache** verfügbar.

In den folgenden [iOS-](https://developer.apple.com/library/ios/documentation/MacOSX/Conceptual/BPInternational/LanguageandLocaleIDs/LanguageandLocaleIDs.html) und [Android/FireOS-Ressourcen](http://developer.android.com/reference/java/util/Locale.html) finden Sie technische Details dazu, wie das Gebietsschema auf Ihrer Plattform empfangen wird.

Für Unternehmen mit Kunden in vielen Ländern kann die frühzeitige Lokalisierung in Ihrer Braze-Journey Ihrem Unternehmen Zeit und Ressourcen sparen. Der folgende Artikel listet die Vorteile der verschiedenen Orchestrierungsansätze für Kampagnen und Canvases auf und nennt außerdem verschiedene Möglichkeiten, wie Benutzer die Personalisierung ihrer Nachrichten handhaben können.

- **Optionen für die Orchestrierung**
  - [Kampagne](#campaign) (ein Template für alle im Vergleich zu einem Template pro Land)
  - [Canvas](#canvas) (eine Reise für alle versus eine Reise pro Land)<br><br>
- **Optionen zur Personalisierung**
  - [Manueller Entry](#option-1-manual-entry)
  - [Content-Blöcke](#option-2-content-blocks)
  - [Kataloge](#option-3-catalogs)
  - [Partner für die Lokalisierung](#option-4-localization-partners)
  - [Übersetzungen in einem öffentlichen Google Sheet](#option-5-translations-in-a-public-google-sheet)
  - [Google-Tabellenkalkulation in eine JSON-API über SheetDB](#option-6-google-spreadsheet-into-a-json-api-via-sheetdb)
...
{% tabs local %}
{% tab Eine Vorlage für alle %}

Beim Ansatz „ein Template für alle“ wird die Lokalisierung auf ein einzelnes Template in Braze mithilfe von [Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid) angewendet. Nach dem Versand bietet das Dashboard aggregierte Kampagnen-Analysen. Das Engagement auf Benutzerebene kann mit benutzerdefinierten Segmenttrichtern gemessen werden, z. B. durch die Kombination von Filtern für **Land** und **empfangene Kampagne**.
...
{% tabs local %}
{% tab Eine Journey für alle %}

Bei dem Ansatz „eine Journey für alle“ wird die Lokalisierung innerhalb von [Canvas-Journeys]({{site.baseurl}}/user_guide/engagement_tools/canvas/get_started/the_basics/#building-the-customer-journey) und Liquid durchgeführt, um Nachrichten für jede:n Nutzer:in zu definieren. 
...
### Option 1: Manueller Entry

Bei der manuellen Eingabe müssen Sie Ihren Inhalt manuell in den Textkörper Ihrer Nachricht einfügen und [Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/) verwenden, um dem Empfänger [bedingt]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid/conditional_logic/#conditional-logic) die richtige Sprache anzuzeigen.
...
Dies kann über das oben genannte Format oder über das Braze-Dashboard erfolgen: 
1. Wählen Sie beim Verfassen Ihrer Nachricht die Schaltfläche **Sprache**, um für jede von Ihnen gewählte Sprache eine flüssige bedingte Logik zu erzeugen.
2. Nachdem Sie Ihren Template-Text in Ihre Nachricht eingefügt haben, geben Sie für jede Sprache unterschiedliche Varianten ein. Für jedes Feld mit Templates sollten Sie die Variationen nach dem in Klammern gesetzten Segment des Templates eingeben. Die Variation sollte dem Sprachcode entsprechen, auf den in den Klammern davor verwiesen wird.
...
### Option 4: Gebietsschema hinzufügen

Fügen Sie [in Ihren Nachrichten Gebietsschemata]({{site.baseurl}}/user_guide/message_building_by_channel/email/using_locales) hinzu und verwenden Sie diese, um Nutzer in verschiedenen Sprachen innerhalb einer einzigen E-Mail-Kampagne oder eines Canvas anzusprechen. 
...
### Option 5: Partner für die Lokalisierung
...
### Option 6: Übersetzungen in einem öffentlichen Google Sheet 

Eine weitere Übersetzungsoption ist die Unterbringung von Übersetzungen in Google Sheets; oft wird dies in Zusammenarbeit mit einer Übersetzungsagentur durchgeführt. Die hier untergebrachten Übersetzungen können mit Connected Content abgefragt werden. Die für eine:n Nutzer:in relevante Übersetzung wird dann zum Zeitpunkt des Versands in die Kampagne eingefügt. 
...
### Option 7: Google Tabellenkalkulation in eine JSON-API über SheetDB  
