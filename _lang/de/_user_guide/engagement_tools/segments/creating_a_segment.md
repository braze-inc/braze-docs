...
]({% image_buster /assets/img_archive/Segment2.png %})Segment Details Panel mit der ausgewählten Option "Nutzer:innen aus allen Apps" im Abschnitt Verwendete Apps.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img_archive/edit_filter_group_name.png %})Filtergruppe mit einem Bearbeitungssymbol neben ihrem Namen.()
...
Wenn `not included`, `is not` oder `does not equal` zusammen mit dem Operator `does not match regex` zwei- oder mehrmals in einer Anweisung verwendet werden, werden Nutzer:innen mit allen Werten für das betreffende Attribut gezielt angesprochen.
...
]({% image_buster /assets/img_archive/segmenter_filter_groups.png %})Segmente filtern Gruppen mit dem Operator AND.()
...
]({% image_buster /assets/img_archive/segmenter_exclusion_groups.png %})Eine Ausschlussgruppe mit zwei Filtern.()
...
]({% image_buster /assets/img_archive/user_lookup.png %})Abschnitt Nutzer:innen mit einem Suchfeld.{: style="max-width:70%;"}()
...
]({% image_buster /assets/img_archive/user_lookup_match.png %})Die Suche nach dem Nutzer:in "testuser" triggert einen Alarm, der besagt, dass "testuser mit allen Segmenten, Filtern und Apps übereinstimmt.()
...
]({% image_buster /assets/img_archive/user_lookup_nomatch.png %})Eine Nutzer:innen-Suche mit der Meldung "test1 stimmt nicht mit den folgenden Targeting-Kriterien überein:" und zeigt fehlende Kriterien an.()
...
]({% image_buster /assets/img_archive/send_to_last_device.png %})"Zusätzliche Einstellungen" mit einem Kontrollkästchen, um nur an das zuletzt verwendete Gerät des Nutzers zu senden.{: style="max-width:60%;"}()
