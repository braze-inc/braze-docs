...
> Wenn ein Canvas durch ein benutzerdefiniertes Ereignis, einen Kauf oder einen API-Aufruf ausgelöst wird, können Sie Metadaten aus dem API-Aufruf, dem benutzerdefinierten Ereignis oder dem Kaufereignis zur Personalisierung in jedem Schritt Ihres Canvas-Workflows verwenden. Sie können diese Eigenschaften nutzen, um besser kuratierte Nachrichten zu versenden.

{% alert important %}
Persistente Eingangs-Eigenschaften sind ein Überbleibsel des ursprünglichen Canvas-Editors, daher gibt es veraltete Verweise auf Begriffe wie Canvas-Eingangs-Eigenschaften, die aus historischen Gründen beibehalten wurden. Informationen zum aktuellen aktualisierten Canvas-Editor werden in [den Kontext- und Event-Eigenschaften]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties) referenziert.
{% endalert %}
...
In dieser Anfrage ist der globale Wert für "Lebensmittelallergien" "keine". Customer_123,Der Wert lautet „Milchprodukte“. Nachrichten in diesem Canvas, die das Liquid-Snippet enthalten, werden mit „dairy“ fürCustomer_123und {%raw%}`{{canvas_entry_properties.${food_allergies}}}`{%endraw%}„none“ für alle anderen als Template verwendet. 
