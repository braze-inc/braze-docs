...
{% tabs local %}
  ...
  {% endtab %}
  {% tab Entry-Zeitplan %}
    Hier entscheiden Sie, wie Ihre Benutzer Ihr Canvas betreten werden:
  ...
  {% endtab %}
  {% tab Zusammenfassung %}
    Hier finden Sie eine Übersicht über Ihre Canvas-Details. Wenn Sie den [Canvas-Genehmigungsworkflow]({{site.baseurl}}/canvas_approval/) aktiviert haben, können Sie die aufgeführten Canvas-Details vor dem Start genehmigen.

  {% endtab %}
{% endtabs %}

### Schritt 2a: Beginnen Sie mit Ihren Canvas-Grundlagen
...
Wählen Sie den Typ Ihres Konversions-Events und wählen Sie dann die Konversionen aus, die Sie aufzeichnen möchten. Diese [Konvertierungsereignisse]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/conversion_events/) messen die Effizienz Ihres Canvas. 
...
### Schritt 2b: Bestimmen Sie Ihren Canvas-Entry-Zeitplan
...
{% details Deduplizierungsverhalten für Canvase unter Verwendung des Original-Editors %}
...
### Schritt 2c: Entry-Zielgruppe festlegen

Sie können die Zielgruppe für Ihr Canvas im Schritt **Zielgruppe** festlegen. Nur die Nutzer:innen, die den von Ihnen festgelegten Kriterien entsprechen, können an der Journey teilnehmen. Das bedeutet, dass Braze die Zielgruppe zunächst auf ihre Qualifikation hin überprüft, bevor die Nutzer:innen an der Canvas-Journey teilnehmen. Wenn Sie zum Beispiel neue Nutzer ansprechen möchten, können Sie ein Segment von Nutzern auswählen, die Ihre App vor weniger als einer Woche zum ersten Mal verwendet haben.

Unter **Entry-Kontrollen** können Sie die Anzahl der Nutzer:innen bei jedem Zeitplan für den Canvas begrenzen. Bei API trigger- und aktionsbasierten Canvase tritt dieses Limit zu jeder UTC-Stunde ein. 

{% alert warning %}
Vermeiden Sie es, eine aktionsbasierte Kampagne oder ein Canvas mit demselben Auslöser wie den Zielgruppenfilter zu konfigurieren (z. B. ein geändertes Attribut oder die Durchführung eines benutzerdefinierten Ereignisses). Es kann eine Race-Condition auftreten, bei der der oder die Nutzer:in zu dem Zeitpunkt, an dem er oder sie das Trigger-Event ausführt, nicht in der Zielgruppe ist, was bedeutet, dass er oder sie die Kampagne nicht erhält oder den Canvas nicht aufruft.  
{% endalert %}
...
![Aufschlüsselung der Zielpopulation mit Option zur Berechnung der genauen Statistik.][2]

### Schritt 2d: Wählen Sie Ihre Sendeeinstellungen aus

Wählen Sie **Sendeeinstellungen**, um Ihre Abonnementeinstellungen zu bearbeiten, die Tarifbegrenzung zu aktivieren und die Ruhezeiten einzuschalten. Durch die Aktivierung von [rate limiting][6b] oder [frequency capping][6c] können Sie den Marketingdruck auf Ihre Nutzer verringern und sicherstellen, dass Sie sie nicht übermäßig ansprechen.
...
- **Nutzer:in** hat ein Opt-in für E-Mail und ist für Push aktiviert. Dieser Benutzer erhält sowohl die E-Mail als auch die Push-Mitteilung.
...
![Die Seite „Ruhezeiten“ zeigt ein Kontrollkästchen zur Aktivierung der Ruhezeiten an. Wenn diese Option aktiviert ist, können Sie die Startzeit, die Endzeit und das Fallback-Verhalten festlegen.][50]

## Schritt 3: Canvas erstellen
...
![Zwei Beispiel-Varianten in einem Braze-Canvas.][12]
...
5. Passen Sie die Zweige Ihres Canvas so an, dass sie nicht mehr durch zufällig gewählte Varianten, sondern nach den Regeln der Zielgruppe unterteilt sind. Legen Sie in den Zielgruppen-Regeln der einzelnen Branches den Zielgruppen-Filter entsprechend Ihres angepassten Attributs fest. <br><br>Zum Beispiel kann ein Branch „Lotterienummer ist kleiner als 3“ als Zielgruppenfilter haben, während ein anderer Branch „Lotterienummer ist größer als 3 und kleiner als 6“ als Zielgruppenfilter haben kann.
...
![Der Schritt „Nachrichten einrichten“ mit ausgewählten „Nachrichtenkanälen“, der die Liste der verfügbaren Nachrichtenkanäle anzeigt, wie z. B. Android Push, Content-Cards, E-Mail und mehr.]({% image_buster /assets/img_archive/message_setup_settings_flow.png %})
...
## Schritt 4: Verwenden Sie multivariate Tests mit Canvas
...
## Schritt 5: Speichern und starten Sie Ihr Canvas
