...
]({% image_buster /assets/img/canvas_details.png %})Die Canvas-Detailseite mit Feldern für den Canvas-Namen, die Beschreibung, den Standort und die Tags.{: style="max-width:70%;"}()
...
]({% image_buster /assets/img/add_canvas_conversions.png %})Primäres Konversions-Event A mit dem Konversions-Event-Typ Macht Kauf, um Konversationen für Nutzer:innen aufzuzeichnen, die innerhalb einer dreitägigen Konversionsfrist einen Kauf tätigen.()
...
]({% image_buster /assets/img_archive/user_lookup.png %})Das Feld "Nutzer:innen", das Ihnen die Suche nach der externen ID oder der Braze ID ermöglicht.{: style="max-width:100%;"}({: style="max-width:80%;"})
...
]({% image_buster /assets/img_archive/canvas_exact_stats.png %})Aufschlüsselung der Zielpopulation mit der Option zur Berechnung exakter Statistiken.()
...
]({% image_buster /assets/img_archive/canvas_add_variant.gif %})Der Button "Variante hinzufügen" ausgewählt, um ein Kontextmenü mit der Option "Variante hinzufügen" anzuzeigen.{: style="float:right;max-width:40%;margin-left:15px;"}()
...
]({% image_buster /assets/img_archive/Canvas_Multiple_Variants.png %})Zwei Beispielvarianten eines Braze-Canvas.()
...
]({% image_buster /assets/img_archive/add_components_flow.png %})Das Fenster für die Komponentensuche fügt dem Braze-Canvas einen Verzögerungsschritt hinzu.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img_archive/edit_delay_flow.png %})Ein Beispiel für einen „Delay“-Schritt, bei dem der Delay auf „Bis zu einem bestimmten Tag“ eingestellt ist.()
...
]({% image_buster /assets/img_archive/action_paths_flow.png %})Der zweite Schritt im Canvas „Aktionseinstellungen“ mit einem auf 1 Tag eingestellten Bewertungsfenster.()
...
]({% image_buster /assets/img_archive/message_setup_settings_flow.png %})Der Schritt „Nachrichten einrichten“ mit ausgewählten „Nachrichtenkanälen“, der die Liste der verfügbaren Nachrichtenkanäle anzeigt, wie z. B. Android Push, Content-Cards, E-Mail und mehr.()
...
]({% image_buster /assets/img_archive/Canvas_Analytics.png %})Ein Beispiel für ein Canvas in Braze.()
