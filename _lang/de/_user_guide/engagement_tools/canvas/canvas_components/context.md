...
]({% image_buster /assets/img/context_step3.png %})Ein Context-Schritt als erster Schritt eines Canvas.{: style="float:right;max-width:40%;margin-left:15px;"}()
...
- Sie können bis zu 50 KB pro Kontextvariable speichern. Wenn Sie beispielsweise ein HTML- und Liquid-Snippet in einer Kontextvariablen speichern, die diesen Wert überschreitet, wird die Kontextvariable für den Nutzer:innen nicht ausgewertet oder gespeichert.
...
|---|---|---|
|Boolesch| loyalty_program |WAHR| 
|Zahl| credit_score |740\.|
|String| product_name |{% raw %}<code>{% raw %} |
|Array| favorite_products|{% raw %}<code>{% raw %}|
|Uhrzeit (in UTC) | last_purchase_date|{% raw %}<code>2025-12-25T08:15:30:250-0800</code>{% endraw %}|
|Objekt (abgeflacht) | user_profile|{% raw %}<code>{% raw %}<br> "first_name": "{{user.first_name}}",<br> "last_name": "{{user.last_name}}",<br> E-Mail<br> "loyalty_points":{{user.loyalty_points}} "loyalty_points":<br> "preferred_categories": {{user.preferred_categories}}<br></code>{% endraw %}<//code> |
...
]({% image_buster /assets/img/context_example4.png %})Kontextvariable, die eingerichtet wurde, um zu verfolgen, ob sich ein Passagier für den Zugang zur VIP-Lounge qualifiziert.{: style="max-width:90%"}()
...
]({% image_buster /assets/img/context_example3.png %})Ein Nachrichtenschritt mit verschiedenen Nachrichten, die je nach Art des gekauften Flugtickets gesendet werden.{: style="max-width:90%"}()
