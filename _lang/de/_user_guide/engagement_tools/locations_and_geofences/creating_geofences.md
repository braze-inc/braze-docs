...
]({% image_buster /assets/img/geofence_example.png %})A geofence set with two geofences "EastCoastGreaterNY" and "WesternRegion" with two circles on the map.()

## Bulk upload geofences {#creating-geofence-sets-via-bulk-upload}

Geofences können in großen Mengen als GeoJSON-Objekt vom Typ `FeatureCollection` hochgeladen werden. Each geofence is a `Point` geometry type in the feature collection. The properties for each feature require a  key, and an optional  key for each geofence. 
...
]({% image_buster /assets/img_archive/action_based_geofence_trigger.png %})An action-based campaign with a geofence that will trigger when a user enters German airports.()
...
Vermeiden Sie es, GeoFences einzurichten, die sich gegenseitig überschneiden, da dies zu Problemen beim Triggern von Benachrichtigungen führen kann.
