...
Je nach Nachrichten-Editor gibt es zwei gängige Möglichkeiten für den Zugriff auf die Medienbibliothek. Eine zeigt den E-Mail-Drag-and-Drop-Editor mit dem Titel "Bilder und GIFs" und einem Button "Aus der Medien-Bibliothek hinzufügen". The other shows the standard editors, such as push and in-app messages, with the title "Media" and a button to "Add Image".]({% image_buster /assets/img_archive/media_library_composers.png %})({: style="border:none"})

{% alert tip %} Weitere Hilfe zur Mediathek finden Sie in unseren [FAQ zu Vorlagen und Medien]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/faqs). &
