...
> Verwenden Sie die globale Kontrollgruppe, um einen Prozentsatz aller Nutzer:innen festzulegen, die keine Kampagnen oder Canvase erhalten sollen. So können Sie die Gesamtwirkung Ihrer Messaging-Bemühungen im Laufe der Zeit analysieren. 

Wenn Sie das Verhalten von Nutzern, die Messaging erhalten, mit dem von Nutzern vergleichen, die keine Messaging-Nachrichten erhalten, können Sie besser verstehen, wie Ihre Marketing-Kampagnen und Canvase zu einem Anstieg der Sitzungen und angepassten Events beitragen.

## Wie die globale Kontrollgruppe funktioniert

Mit der Globalen Kontrollgruppe können Sie einen Prozentsatz aller Benutzer als Kontrollgruppe festlegen. Nach dem Speichern erhalten die Nutzer:innen der Gruppe keine Kampagnen oder Canvase. 

{% alert important %}
Ihre globale Kontrollgruppe gilt für alle Kanäle, Kampagnen und Canvase, außer für [API-Kampagnen]({{site.baseurl}}/api/api_campaigns). Das bedeutet, dass Nutzer:innen in Ihrer Kontrollgruppe weiterhin API Kampagnen erhalten. Diese Ausnahme gilt jedoch nicht für Content-Cards. Wenn Sie eine API-getriggerte Content-Card-Kampagne verwenden, erhalten die Nutzer:innen in Ihrer Kontrollgruppe diese nicht.
{% endalert %}
...
Wenn Ihre globale Kontrollgruppe erstellt wird, sind alle Nutzer:innen mit zufälligen Bucket-Nummern Teil der Gruppe. Außerdem werden neue Nutzer:innen mit diesen zufälligen Bucket-Nummern der globalen Kontrollgruppe hinzugefügt, wenn sie nach diesem Zeitpunkt beitreten (d.h. nachdem die globale Kontrollgruppe erstellt wurde). Wenn viele Nutzer:innen gelöscht werden, können Sie davon ausgehen, dass die Größe Ihrer globalen Kontrollgruppe schrumpft, da ein bestimmter Prozentsatz dieser gelöschten Nutzer:innen in diese Gruppe gefallen ist. Dadurch bleibt die Größe Ihrer Gruppe als konstanter Prozentsatz im Verhältnis zu Ihrer gesamten Nutzer:innen-Basis erhalten.

### Weisen Sie Nutzer:innen nach dem Zufallsprinzip der Behandlungsgruppe für die Berichterstattung zu
...
Geben Sie einen Prozentsatz für Ihre Kontrollgruppe ein und wählen Sie **Speichern**. Nach der Eingabe zeigt Ihnen Braze eine Schätzung an, wie viele Nutzer:innen in Ihre Globale Kontroll-, Behandlungs- und Behandlungsstichprobe fallen. Beachten Sie, dass diese Schätzung umso genauer ist, je mehr Nutzer:innen Sie in Ihrem Workspace haben. 

Die Anzahl der Nutzer:innen in Ihrer globalen Kontrollgruppe wird nach der Ersteinrichtung automatisch aktualisiert, damit sie proportional zu diesem Prozentsatz bleibt, wenn weitere Nutzer:innen zu Ihrem Workspace hinzugefügt werden. Außerdem werden Nutzer:innen, die nach der Einrichtung der globalen Kontrollgruppe beitreten und zufällige Bucket-Nummern haben, ebenfalls der globalen Kontrollgruppe hinzugefügt. Wenn viele Nutzer:innen hinzukommen, wächst die Größe Ihrer globalen Kontrollgruppe, um einen konstanten Prozentsatz im Verhältnis zu Ihrer gesamten Nutzerbasis zu erhalten. Wenn die Größe Ihrer globalen Kontrollgruppe wächst, bleiben die Nutzer:innen, die zuvor in der Gruppe waren, in der Gruppe (es sei denn, Sie nehmen Änderungen an Ihrer Gruppe vor, indem Sie sie deaktivieren und eine neue Gruppe erstellen).

Richtlinien für den Prozentsatz finden Sie unter [Bewährte Testverfahren](#percentage-guidelines).

![Die globalen Kontrollgruppeneinstellungen mit den Zielgruppeneinstellungen auf "Fünf Prozent der globalen Kontrollgruppenpopulation zuweisen".]({% image_buster /assets/img/control_group/control_group4.png %})

### Schritt 3: Ausschlusseinstellungen zuweisen

Mit Tags können Sie Ausschlusskriterien zu der globalen Kontrollgruppe hinzufügen. Alle Kampagnen oder Canvases, die die in den Ausschlusseinstellungen enthaltenen Tags verwenden, nutzen Ihre Globale Kontrollgruppe nicht. Diese Kampagnen und Canvase werden weiterhin an alle Nutzer:innen der Zielgruppe gesendet, einschließlich derer in Ihrer globalen Kontrollgruppe.
...
Sie können Ihre globale Kontrollgruppe jederzeit auf dem Tab **Globale Kontrollgruppen-Einstellungen** deaktivieren. Beachten Sie jedoch, dass dies dazu führt, dass Nutzer:innen in dieser Gruppe sofort für Kampagnen und Canvase zugelassen werden.
...
Wenn Sie die Kontrollgruppe deaktiviert haben, können Sie eine neue Gruppe speichern. Wenn Sie einen Prozentsatz eingeben und speichern, erzeugt Braze eine neue, zufällig ausgewählte Gruppe von Benutzern. Wenn Sie den gleichen Prozentsatz wie zuvor eingeben, erstellt Braze eine neue Gruppe von Nutzer:innen für Ihre Kontroll- und Behandlungsgruppen.

![Ein Dialogfenster mit dem Titel "Sie nehmen Änderungen an den globalen Messaging-Einstellungen vor" mit einer Warnung, dass Ihre globale Kontrollgruppe, sobald sie deaktiviert ist, nicht mehr von neuen oder aktiven Kampagnen oder Canvase ausgeschlossen ist.]({% image_buster /assets/img/control_group/control_group2.png %}){: style="max-width:60%" }
...
![Ein Abschnitt "Verschiedenes" meldet, dass der Nutzer:in eine zufällige Bucket-Nummer von 6356 hat und nicht in der globalen Kontrollgruppe ist.]({% image_buster /assets/img/control_group/control_group1.png %}){: style="max-width:50%;"}
...
Benutzer in Ihrer globalen Kontrollgruppe erhalten keine Nachrichten außer denen mit Tag-Ausnahmen. Wenn Sie einer Kampagne oder einem Canvas ein Steuerelement hinzufügen, hält Braze einen Teil Ihrer globalen Behandlungsgruppe vom Empfang dieser bestimmten Kampagne oder dieses Canvas zurück. Das heißt, wenn ein Mitglied der globalen Kontrollgruppe nicht für eine bestimmte Kampagne oder ein bestimmtes Canvas berechtigt ist, ist es nicht in der Kontrollgruppe für diese bestimmte Kampagne oder dieses Canvas vertreten.
...
- Der ideale Prozentsatz Ihrer gesamten Zielgruppe hängt davon ab, wie groß Ihre Zielgruppe insgesamt ist. Je größer sie ist, desto kleiner kann der Prozentsatz sein. Wenn Sie jedoch eine kleine Zielgruppe haben, benötigen Sie einen größeren Prozentsatz für Ihre Kontrollgruppe.
...
{% alert tip %}
Je länger Sie dieselbe Kontrollgruppe heraushalten, desto mehr weicht sie von der Behandlungsgruppe ab, was zu Verzerrungen führen kann. Wenn Sie die Globale Kontrollgruppe zurücksetzen, wird die Population wieder ausgeglichen.
{% endalert %}
