---
nav_title: Connected-Content
article_title: Connected-Content
page_order: 1
layout: dev_guide
search_rank: 3
guide_top_header: "Connected-Content"
guide_top_text: "Braze Connected Content erweitert die Personalisierung des Marketings, um die Kundenbindung und die Konversionsrate zu erhöhen. Mit dieser Funktion können Sie alle über die API zugänglichen Informationen direkt in die Nachrichten einfügen, die Sie an Benutzer senden. Mit Connected-Content können Sie Inhalte entweder direkt von Ihrem Webserver oder von öffentlich zugänglichen APIs beziehen. Sie können unsere Connected Content-Funktionen auch mit unseren Technologiepartnern nutzen, z. B. <b><a href='/docs/partners/data_augmentation/contextual_location/accuweather/'>AccuWeather</a></b> und <b><a href='/docs/partners/channel_extensions/localization/transifex/'>Transifex</a></b>.<br><br>Zusätzlich zu den nachfolgend aufgeführten Artikeln empfehlen wir Ihnen einen Blick auf unsere <b><a href='https://learning.braze.com/connected-content'>Vernetzter Inhalt</a></b> Braze Learning Kurs."
description: "Auf dieser Landing Page finden Sie alles rund um Connected Content. Hier finden Sie Artikel über API-Aufrufe, lokale Connected-Content-Variablen, den Abbruch von Inhalten und mehr."

guide_featured_title: "Abschnittsartikel"
guide_featured_list:
- name: Einen API-Aufruf tätigen
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/making_an_api_call/
  image: /assets/img/braze_icons/navigation-pointer-01.svg
- name: Lokal verknüpfte Inhaltsvariablen
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/local_connected_content_variables/
  image: /assets/img/braze_icons/server-01.svg
- name: Abbrechen von Connected-Content
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/aborting_connected_content/
  image: /assets/img/braze_icons/stop-circle.svg
- name: Wiederholungen verbundener Inhalte
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/connected_content_retries/
  image: /assets/img/braze_icons/refresh-ccw-01.svg
- name: Öffentliche APIs
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/public_apis/
  image: /assets/img/braze_icons/target-05.svg
- name: Abrufen von Benutzerprofildaten
  link: /docs/user_guide/personalization_and_dynamic_content/connected_content/user_profile_fields_connected_content/
  image: /assets/img/braze_icons/users-01.svg
- name: Fehlerbehebung bei Anfragen für verbundene Inhalte 
  link: /docs/help/help_articles/api/webhook_connected_content_errors/
  image: /assets/img/braze_icons/check-square-broken.svg
---

<br><br>

[16]: [success@braze.com](mailto:success@braze.com)
