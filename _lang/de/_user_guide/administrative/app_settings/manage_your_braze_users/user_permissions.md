...
|Workspace|Kampagnen genehmigen und ablehnen|Ermöglicht es Benutzern, Kampagnen zu genehmigen oder abzulehnen. Der [Genehmigungsworkflow für Kampagnen]({{site.baseurl}}/user_guide/engagement_tools/campaigns/managing_campaigns/campaign_approval) muss aktiviert sein, damit diese Berechtigung gilt. Diese Einstellung befindet sich derzeit in der Early Access-Phase. Wenden Sie sich an Ihren Account Manager, wenn Sie Interesse an einer Teilnahme haben.|
...
|Workspace|Nutzerdaten exportieren|Ermöglicht es Benutzern, ihre Benutzerdaten aus Segmenten, Kampagnen und Canvases zu exportieren.|
...
|Workspace|Dashboard-Nutzer:innen verwalten|Ermöglicht Benutzern das Anzeigen, Bearbeiten und Verwalten der Seite **Unternehmensbenutzer**. Benutzer mit dieser Berechtigung können die Berechtigungen eines jeden Benutzers, einschließlich ihrer eigenen, ändern. Daher sollte diese Berechtigung als eine administrative Zugriffsebene betrachtet werden. Diese Berechtigung gilt nicht für die Löschung von Nutzerkonten, da diese Aktion Administratoren vorbehalten ist.|
...
|Workspace|Als PII gekennzeichnete angepasste Attribute anzeigen|Ermöglicht es diesem Benutzer, benutzerdefinierte Attribute, die als PII markiert sind, einzusehen, ohne Administrator zu sein.|
|Workspace|PII anzeigen|Ermöglicht es Benutzern, die von Ihrem Unternehmen definierten Felder für persönlich identifizierbare Informationen (PII) innerhalb des Dashboards anzuzeigen. Benutzer können PII-Felder auch auf der Registerkarte **Vorschau als Benutzer** der Nachrichtenvorschau sehen. |
|Workspace|Nutzerprofile PII-konform anzeigen|Berechtigt zur Anzeige von Nutzerprofilen, wobei jedoch alle Felder unkenntlich gemacht werden, die laut Unternehmensdefinition personenbezogene Daten enthalten.|
|Workspace|Transformationen anzeigen|Ermöglicht es Benutzern, [Braze Data Transformations]({{site.baseurl}}/user_guide/data_and_analytics/data_transformation/overview/) anzuzeigen.|
