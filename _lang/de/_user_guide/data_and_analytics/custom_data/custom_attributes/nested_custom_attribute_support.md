...
# Verschachtelte benutzerdefinierte Attribute

Verschachtelte benutzerdefinierte Attribute ermöglichen es Ihnen, eine Reihe von Attributen als Eigenschaft eines anderen Attributs zu definieren. Mit anderen Worten: Wenn Sie ein benutzerdefiniertes Attributobjekt definieren, können Sie eine Reihe von zusätzlichen Attributen für dieses Objekt festlegen.

Nehmen wir zum Beispiel an, Sie möchten ein angepasstes Attribut im Nutzerprofil mit der Bezeichnung `favorite_book` definieren. Dieses angepasste Attribut kann als ein Objekt definiert werden, das die verschachtelten Attribute `title`, `author` und `publishing_date` enthält, etwa so:
...
Mit diesen verschachtelten Daten können Sie Segmente anhand von Informationen aus einem angepassten Attribut-Objekt erstellen und Ihre Nachrichten mit einem angepassten Attribut-Objekt und Liquid personalisieren.

Angepasste Attribut-Objekte können [Datentypen][1] enthalten, wie z.B.:
...
- Arrays
- Uhrzeit
  - Wenn Sie ein verschachteltes angepasstes Attribut für die Zeit filtern, können Sie wählen, ob Sie nach "Tag des Jahres" oder "Zeit" filtern möchten. Bei "Tag des Jahres" werden nur der Monat und der Tag zum Vergleich herangezogen. "Zeit" vergleicht den vollständigen Zeitstempel, einschließlich des Jahres.
- Andere Objekte
- [Arrays von Objekten]({{site.baseurl}}/array_of_objects/)
...
- Objekte haben eine maximale Größe von 50 KB.
- Schlüsselnamen und String-Werte dürfen maximal 255 Zeichen lang sein.
- Schlüsselnamen dürfen keine Leerzeichen enthalten.
- Nicht alle Braze-Partner unterstützen verschachtelte benutzerdefinierte Attribute. Sehen Sie in der [Dokumentation des Partners]({{site.baseurl}}/partners/home) nach, ob die Integration diese Funktion unterstützt.
...
Nachdem diese Anfrage eingegangen ist, sieht das Angepasstes-Attribut-Objekt wie folgt aus:
...
{% alert note %}
Wenn Sie anfangs keine Daten als Objekteigenschaften erfasst haben, empfehlen wir, diese Daten unter Verwendung des Schlüssels `$time` für alle Nutzer:innen erneut zu senden. Andernfalls kann dies bei Verwendung des Attributs `$time` zu unvollständigen Segmenten führen.
{% endalert %}
...
Beachten Sie, dass Braze bei verschachtelten angepassten Attributen, wenn das Jahr kleiner als 0 oder größer als 3000 ist, diese Werte nicht für die:den Nutzer:in speichert.

## Liquid-Templates

Das folgende Liquid-Template-Beispiel zeigt, wie Sie die Angepasstes-Attribut-Objekteigenschaften referenzieren, die aus der vorangegangenen API-Anfrage gespeichert wurden, und sie in Ihren Nachrichten verwenden.
...
![Verwendung von Liquid als Template für den Namen eines Titels und die Anzahl der Abspielungen dieses Titels in einer Nachricht][5]
...
Nachdem Sie einen Pfad zu Ihrer Eigenschaft hinzugefügt haben, klicken Sie auf **Validieren**, um zu überprüfen, ob der Wert im Feld Pfad gültig ist.

![Filterung auf der Grundlage eines benutzerdefinierten Attributs für den meistgespielten Titel, wenn ein Hörer einen Titel mehr als eine bestimmte Anzahl von Malen gespielt hat][6]
...
Bei der Segmentierung mit verschachtelten angepassten Attributen haben Sie Zugriff auf einen neuen Vergleicher, der nach Datentyp gruppiert ist. Da `play_analytics.count` beispielsweise eine Zahl ist, können Sie einen Komparator unter der Kategorie **Zahl** auswählen.

![Ein:e Nutzer:in wählt einen Operator auf der Grundlage des Datentyps für das verschachtelte angepasste Attribut][7]
...
Sie können auch das Feature **Liquid für Segmente kopieren** verwenden, um einen Liquid Code für dieses Segment zu generieren und diesen in einer Nachricht zu verwenden. Nehmen wir zum Beispiel an, Sie haben ein Array von Kontoobjekten und ein Segment, das auf Kund:innen mit aktiven steuerpflichtigen Konten abzielt. Wenn Sie Kund:innen dazu bringen möchten, einen Beitrag zu einem ihrer aktiven und steuerpflichtigen Konten zu leisten, müssen Sie eine Nachricht erstellen, um sie anzustacheln. 

![Ein Beispielsegment mit ausgewähltem Kontrollkästchen für die multikriterielle Segmentierung.][14]
...
Von hier aus können Sie `segmented_nested_objects` verwenden und Ihre Nachricht personalisieren. In unserem Anwendungsfall wollen wir ein Ziel aus dem ersten aktiven steuerpflichtigen Konto nehmen und es personalisieren:
...
Sie können ein Schema für Ihre Objekte generieren, um Segmente zu filtern, ohne sich verschachtelte Objektpfade merken zu müssen. Führen Sie dazu die folgenden Schritte aus.
...
Navigieren Sie im Braze Dashboard zu **Dateneinstellungen** > **Benutzerdefinierte Attribute**.
...
Suchen Sie nach Ihrem Objekt oder Objekt-Array. Klicken Sie in der Spalte **Attributname** auf **Schema generieren**.

![][8]

{% alert tip %}
Je nachdem, wie viele Daten Sie uns geschickt haben, kann es ein paar Minuten dauern, bis Ihr Schema generiert ist.
{% endalert %}
...
Klicken Sie auf die Schaltfläche <i class="fas fa-plus"></i> plus im Pfadfeld. Daraufhin wird eine Darstellung Ihres Objekts oder Objektarrays angezeigt. Sie können jedes der aufgelisteten Elemente auswählen und Braze fügt sie für Sie in das Pfadfeld ein. Für unseren Anwendungsfall müssen wir den Saldo finden. Wählen Sie den Saldo aus und der Pfad (in diesem Fall `[].balance`) wird automatisch in das Pfadfeld eingefügt.

![][12]{: style="max-width:70%" }

Sie können auf **Validieren** klicken, um zu überprüfen, ob der Inhalt des Pfadfelds gültig ist, und dann den Rest des Filters nach Bedarf erstellen. Hier haben wir festgelegt, dass der Saldo weniger als 100 betragen soll.
...
Sie können einen Trigger auslösen, wenn sich ein verschachteltes angepasstes Attributobjekt ändert. Diese Option ist für Änderungen an Objektarrays nicht verfügbar. Wenn Sie keine Option zum Anzeigen des Pfad-Explorers sehen, überprüfen Sie, ob Sie ein Schema generiert haben. 
...
Nachdem ein Schema generiert wurde, kann es einmal alle 24 Stunden neu generiert werden. Dieser Abschnitt beschreibt, wie Sie Ihr Schema neu generieren können. Ausführlichere Informationen zu Schemas finden Sie im Abschnitt über die [Erstellung eines Schemas](#generate-schema) in diesem Artikel.
...
2. Suchen Sie nach Ihrem verschachtelten benutzerdefinierten Attribut.
3. Klicken Sie in der Spalte **Attributname** für Ihr Attribut auf <i class="fas fa-plus"></i>, um das Schema zu verwalten.
4. Es wird ein Modal angezeigt. Klicken Sie auf **Schema neu generieren**.
...
{% alert important %}
Wenn Sie das Schema für ein Objekt-Array mit einem bestehenden Objekt zurücksetzen möchten, müssen Sie ein neues angepasstes Attribut erstellen. Bei der Schema-Neugenerierung werden vorhandene Objekte nicht gelöscht.
{% endalert %}

Wenn die Daten nach der Neugenerierung des Schemas nicht wie erwartet erscheinen, wird das Attribut möglicherweise nicht oft genug aufgenommen. Nutzerdaten werden anhand früherer Daten, die für das angegebene verschachtelte Attribut an Braze gesendet wurden, abgefragt. Wenn das Attribut nicht ausreichend erfasst wird, wird es nicht in das Schema aufgenommen.
