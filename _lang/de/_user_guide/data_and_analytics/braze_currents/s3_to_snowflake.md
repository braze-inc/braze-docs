...
{% alert note %}
Wenn Sie spezifischere Anwendungsfälle haben und möchten, dass Braze Ihre Currents-Instanz wartet, wenden Sie sich an Ihren Braze-Kundenbetreuer und fragen Sie ihn nach Braze Data Professional Services.
{% endalert %}
...
### Schritt 1: Externe S3-Stufe konfigurieren
...
2. Erstellen Sie die folgenden Dateien in der angegebenen Reihenfolge:
  ...
  2.3. Erstellen Sie eine Snowflake S3-Stufe (genannt `braze_data`):
...
{: start="4"}
4\. Verwenden Sie schließlich den Befehl `show pipes;`, um Ihre SQS-Informationen anzuzeigen. Der Name der SQS-Warteschlange wird in einer neuen Spalte mit der Bezeichnung `NOTIFICATION_CHANNEL` angezeigt, da diese Pipe als Auto-Ingest-Pipe erstellt wurde.

### Schritt 2: Bucket-Events erstellen
...
![Tab „AWS-Eigenschaften“][1]{: height="50%" width="50%"}

{: start="2"}
2\. Erstellen Sie je nach Bedarf neue Ereignisse für jeden Satz von Currents-Daten[(Messaging]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/message_engagement_events/), [Nutzer:innen]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/customer_behavior_events/)) oder für beide.

![Erstellen eines neuen Events in AWS][2]{: height="50%" width="50%"}
...
## Konfigurieren der Auto-Ingest-Snowpipes {#auto-ingest-snowpipes}

Damit die AWS SQS-Konfiguration die richtigen Tabellen erzeugt, müssen Sie die Struktur der eingehenden Daten richtig definieren, indem Sie die folgenden Beispiele und Schemata verwenden, die in unserer Currents-Dokumentation für [Message Engagement oder Messaging Events]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/message_engagement_events/), [User oder Customer Behavior Events]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/customer_behavior_events/) oder beides festgelegt sind.
...
{% alert note %}
  Je nach Currents-Integration gibt es verschiedene Events, die Sie einrichten müssen (z.B. [Nachrichten-Engagement oder Messaging-Events]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/message_engagement_events/) und [Nutzer:innen- oder Kundenverhalten-Events]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/customer_behavior_events/)). Sie können auch ein Skript für einige oder alle dieser Vorgänge schreiben.
{% endalert %}

{% tabs %}
  {% tab Nutzerverhalten-Events %}

1. Erstellen Sie eine `INTO`-Tabelle, die wir kontinuierlich mit der folgenden Struktur aus dem Currents-Schema laden werden:
...
{: start="2"}
2\. Erstellen Sie die Pipe `auto_ingest` und geben Sie Folgendes an:
