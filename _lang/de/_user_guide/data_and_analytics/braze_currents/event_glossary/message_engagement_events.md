...
Speicherschemata gelten für die Flat File-Ereignisdaten, die wir an Data Warehouse Storage Partner (Google Cloud Storage, Amazon S3 und Microsoft Azure Blob Storage) senden. Schemata, die für andere Partner gelten, finden Sie in unserer Liste der [verfügbaren Partner]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/available_partners/) und auf den entsprechenden Seiten.

Wenden Sie sich an Ihre:n Account Manager:in oder öffnen Sie ein [Support-Ticket]({{site.baseurl}}/braze_support/), wenn Sie Zugang zu zusätzlichen Event-Berechtigungen benötigen. Wenn Sie in diesem Artikel nicht finden, was Sie brauchen, sehen Sie sich unsere [Bibliothek für Kundenverhalten-Events]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/customer_behavior_events/) oder unsere [Currents-Beispiele für Daten](https://github.com/Appboy/currents-examples/tree/master/sample-data) an.

{% details Erläuterung der Event-Struktur für das Engagement von Nachrichten und der Plattformwerte %}

### Event-Struktur

Diese Aufschlüsselung der Events zeigt, welche Art von Informationen in der Regel in einem Nachrichten-Event enthalten sind. Mit einem soliden Verständnis seiner Komponenten können Ihre Entwickler:in und Ihr Business-Intelligence Strategie Team die eingehenden Currents Ereignisdaten nutzen, um datengestützte Berichte und Charts zu erstellen und andere wertvolle Metriken zu nutzen.

![Aufschlüsselung eines Nachrichten-Engagement-Events, das ein E-Mail-Abmelde-Event mit den aufgeführten Eigenschaften zeigt, gruppiert nach nutzerspezifischen Eigenschaften, Kampagnen- oder Canvas-Tracking-Eigenschaften und Event-spezifischen Eigenschaften]({% image_buster /assets/img/message_engagement_event.png %})

Nachrichten-Engagement-Events setzen sich aus **nutzerspezifischen Eigenschaften**, **Kampagnen-/Werbeaktionen-Tracking**-Eigenschaften und **Event-spezifischen** Eigenschaften zusammen.
...
## WhatsApp-Lese-Events

{% apitags %}
WhatsApp, Lesen
{% endapitags %}

Dieses Event tritt ein, wenn eine WhatsApp-Nachricht von einem oder einer Endnutzer:in gelesen wird.
...
## WhatsApp-Sende-Events

{% apitags %}
WhatsApp, Sendet
{% endapitags %}

Dieses Event tritt ein, wenn eine Sendeanfrage erfolgreich zwischen Braze und WhatsApp übermittelt wurde. Dies bedeutet jedoch nicht, dass die Nachricht bei den Endnutzer:innen angekommen ist.
...
## WhatsApp-Abbruchnachrichten-Events
...
## Content-Card-Abbruchnachrichten-Events

{% apitags %}
Abbruch, Content-Cards
{% endapitags %}

Dieses Events tritt ein, wenn eine Content-Card Nachricht aufgrund von Liquid-Abbrüchen, Ruhezeiten usw. abgebrochen wurde.
...
## E-Mail-Abbruchnachrichten-Events
...
## Abbruch von Events durch Push-Benachrichtigung
...
## Webhook-Abbruchnachrichten-Events
...
## Durchgeführter Canvas-Exit-Event-Events

{% apitags %}
Exit, Canvas
{% endapitags %}

Dieses Event tritt ein, wenn ein:e Nutzer:in ein Canvas durch die Ausführung eines Events verlassen hat.
...
## Canvas-Exit stimmt mit Zielgruppe überein-Events
...
Dieses Event tritt ein, wenn ein:e Nutzer:in einen Canvas-Experiment-Schritt-Pfad betritt.
...
Dieses Event tritt ein, wenn ein:e Nutzer:in für einen Canvas-Experiment-Schritt konvertiert.
...
- Für `ad_id`, `ad_id_type` und `ad_tracking_enabled` müssen Sie die iOS IDFA und Android Google Ad-ID explizit über die nativen SDKs erfassen. Erfahren Sie mehr über diese Einrichtung für [iOS]({{site.baseurl}}/developer_guide/platform_integration_guides/swift/analytics/swift_idfv/) und [Android]({{site.baseurl}}/developer_guide/platform_integration_guides/android/initial_sdk_setup/optional_gaid_collection/#optional-google-advertising-id).
- Wenn Sie Kafka zur Aufnahme von [Currents]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/)-Daten verwenden, wenden Sie sich an Ihren Customer-Success-Manager, um das Senden von `ad_id` zu aktivieren.
- `message_extras` ermöglichen es Ihnen, Ihre Sende-Events mit dynamischen Daten aus Connected-Content, angepassten Attributen (z. B. Sprache, Land) und Canvas-Entry-Eigenschaften zu versehen. Weitere Informationen finden Sie unter [Extras für Nachrichten]({{site.baseurl}}/message_extras_tag/).
  {% endapi %}
  {% api %}
...
Dieses Ereignis tritt ein, wenn ein Nutzer:innen direkt auf die Push-Benachrichtigung klickt, um die Anwendung zu öffnen. Derzeit referenzieren Push-Öffnung-Events auf „Direkte Öffnungen“ und nicht auf „Gesamte Öffnungen“. Dazu gehören nicht die auf Kampagnenebene angezeigten Statistiken über "beeinflusste Öffnungen", da diese nicht auf der Ebene der Nutzer:innen attributiert werden.
...
Dieses Event wird von unserem [Swift SDK](https://github.com/braze-inc/braze-swift-sdk) nicht unterstützt und ist jetzt bei Verwendung unseres [Obj-C SDK](https://github.com/Appboy/appboy-ios-sdk) veraltet.
...
## Push-Benachrichtigungen sind nicht zustellbar

{% apitags %}
Push, Sendungen, Bounce
{% endapitags %}

Dieses Event tritt ein, wenn ein Fehler entweder vom Apple Push Notification Service oder von Fire Cloud Messaging empfangen wird. Dies bedeutet, dass die Push Nachricht abgelehnt wurde und daher dem Gerät des Nutzers:innen nicht zugestellt werden konnte.
...
- Wenn Sie Kafka zur Aufnahme von [Currents]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/) Daten verwenden, wenden Sie sich an Ihren Customer-Success-Manager oder Account Manager:in, um das Feature Flipper für das Senden von `ad_id` zu aktivieren.
  {% endapi %}
  {% api %}

## E-Mail-Sendung-Events

{% apitags %}
E-Mail, Sendet
{% endapitags %}

Dieses Event tritt ein, wenn eine Anfrage zum Senden von E-Mails erfolgreich zwischen Braze und SendGrid übermittelt wurde. Dies bedeutet jedoch nicht, dass die E-Mail im Posteingang des Endnutzers:in eingegangen ist.
...
- Das Verhalten für `dispatch_id` unterscheidet sich zwischen Canvas und Kampagnen, da Braze Canvas-Schritte (mit Ausnahme von Entry-Schritten, die geplant werden können) als getriggerte Events behandelt, auch wenn sie geplant sind. Erfahren Sie mehr über das [Versand-ID-Verhalten]({{site.baseurl}}/help/help_articles/data/dispatch_id/).
- `message_extras` ermöglichen es Ihnen, Ihre Sende-Events mit dynamischen Daten aus Connected-Content, angepassten Attributen (wie Sprache oder Land) und Canvas-Entry-Eigenschaften zu versehen. Weitere Informationen finden Sie unter [Extras für Nachrichten]({{site.baseurl}}/message_extras_tag/).
  {% endapi %}


{% api %}

## E-Mail-Zustellung-Events
...
Dieses Event tritt ein, wenn ein Internetanbieter einen Hard Bounce zurückgibt. Ein Hard Bounce bedeutet einen dauerhaften Zustellbarkeitsfehler.
...
## E-Mail Soft Bounce-Event

{% apitags %}
E-Mail, Bounce
{% endapitags %}

Dieses Event tritt ein, wenn ein Internetanbieter einen Soft Bounce zurückgibt. Ein Soft Bounce bedeutet, dass eine E-Mail aufgrund einer vorübergehenden Störung der Zustellbarkeit nicht zugestellt werden konnte.
...
Dieses Event tritt ein, wenn der oder die Endnutzer:in auf den Button „Spam“ in der E-Mail drückt. Beachten Sie, dass dies nicht bedeutet, dass die E-Mail im Spam-Ordner gelandet ist, da Braze dies nicht trackt.
...
## In-App-Nachricht-Impression-Event

{% apitags %}
In-App-Nachrichten, Impressionen
{% endapitags %}

Dieses Event tritt ein, wenn ein:e Nutzer:in eine In-App-Nachricht sieht.
...
## In-App-Nachricht-Klick-Events

{% apitags %}
In-App-Nachrichten, Klicks
{% endapitags %}

Dieses Event tritt ein, wenn ein:e Nutzer:in auf eine In-App-Nachricht klickt.
...
{% api %}
## Content-Card-Impressionen-Events
...
{% api %}
## Content-Card-Klick-Events

{% apitags %}
Content-Cards, Klicks
{% endapitags %}

Dieses Event tritt ein, wenn ein:e Nutzer:in auf eine Content-Card klickt.
...
{% api %}
## Content-Card-Ausblendung-Events

{% apitags %}
Content-Cards, Ausblendung
{% endapitags %}
...
## SMS-Sendungen an Träger-Events
...
Dieses Event tritt ein, wenn eine SMS an den Netzbetreiber gesendet wird.
...
## SMS-Ablehnung-Events

{% apitags %}
SMS, Ablehnung
{% endapitags %}

Dieses Ereignis tritt ein, wenn eine SMS vom Netzbetreiber abgelehnt wird. Dies kann aus verschiedenen Gründen geschehen. Verwenden Sie dieses Event und die bereitgestellten Fehlercodes, um Probleme mit der SMS-Zustellung zu beheben.
...
## SMS-Zustellungsfehler-Events

{% apitags %}
SMS, Zustellung
{% endapitags %}

Dieses Ereignis tritt ein, wenn eine SMS nicht zugestellt werden kann. Verwenden Sie dieses Event und die bereitgestellten Fehlercodes, um Probleme mit der SMS-Zustellung zu beheben.
...
## Eingehende SMS-Nachrichten empfange-Events
...
## Feature-Flags-Impressions-Events
...
Dieses Event tritt ein, wenn ein:e Nutzer: den Canvas aufruft. Dieses Event teilt Ihnen mit, welche Variante der oder die Nutzer:in aufgerufen hat.
...
Dieses Ereignis tritt ein, wenn ein Nutzer:innen einen Schritt in einem Canvas mit einem bestimmten Ergebnis durchläuft. Derzeit erzeugen nur Split-Schritte – Zielgruppenpfade, Decision-Split, Aktionspfade, Experiment – und voranbringende Ergebnisse Schritt-Progression-Events.
{% tabs %}
{% tab Mixpanel %}
```json
...
Dieses Ereignis tritt ein, wenn sich ein Nutzer:innen in eine Kontrollvariante einer multivariaten Kampagne einträgt. Dieses Event wird erzeugt, da es für diese:n Nutzer:in kein Kanal-Sende-Event geben wird.
...
| Abo Seite | Wenn sich ein:e Nutzer:in über einen E-Mail-Link abmeldet, der nicht das Präferenzzentrum ist |
...
{% alert important %}
Dieses Event wird nicht getriggert, wenn der oder die Nutzer:in die App tatsächlich deinstalliert, da es unmöglich ist, das genau zu verfolgen. Braze sendet täglich einen stillen Push, um festzustellen, ob die App noch auf dem Gerät Ihres Nutzers:innen vorhanden ist. Wenn wir bei diesem stillen Push einen Fehler erhalten, wird davon ausgegangen, dass die App deinstalliert wurde.
{% endalert %}
