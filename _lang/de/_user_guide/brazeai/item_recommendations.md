...
Als Teil der [KI]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/)-Artikelempfehlungsfunktion nutzen personalisierte KI-Empfehlungen Deep Learning, um auf der Grundlage der bisherigen Interessen Ihrer Nutzer:innen vorherzusagen, woran diese als Nächstes am ehesten interessiert sein könnten. Diese Methode bietet ein dynamisches und maßgeschneidertes Empfehlungssystem, das sich dem Nutzerverhalten anpasst.
...
- Ein Verfahren zum Tracking von Käufen, entweder eines Kauf-Objekts oder eines angepassten Events.
{% enddetails %}
...
7. [Verwenden Sie die Empfehlung in der Nachrichtenübermittlung]({{site.baseurl}}/user_guide/brazeai/item_recommendations/using_recommendations/).
{% enddetails %}
{% endtab %}
{% endtabs %}
...
Regelbasierte Empfehlungen basieren auf einer festen Logik, die Sie manuell einstellen müssen. Dies bedeutet, dass sich Ihre Empfehlungen nicht an den individuellen Kaufverlauf und die Vorlieben einer Nutzer:in anpassen, es sei denn, Sie aktualisieren die Logik. Daher eignet sich diese Methode am besten für Empfehlungen, die keine häufigen Updates erfordern.
...
Bitte beachten Sie, dass Empfehlungen unabhängig voneinander funktionieren und keine Kenntnis darüber haben, was die anderen Modelle empfehlen. Dies bedeutet, dass jeder Abschnitt doppelte Artikel enthalten kann, die bereits in anderen KI-Empfehlungsabschnitten derselben E-Mail angezeigt werden.
