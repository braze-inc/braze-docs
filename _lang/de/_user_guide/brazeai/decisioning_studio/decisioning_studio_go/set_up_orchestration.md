...
| **Klaviyo** | Flows mit metrischen Triggern | Template-basierte, triggernde Splits |
...
Um Decisioning Studio Go in Klaviyo zu integrieren, richten Sie einen API-Schlüssel ein, erstellen einen Template-Flow als Platzhalter und bauen einen Flow, der ausgelöste Sendungen verarbeitet.
...
### Teil 2: Erstellen Sie ein Template für einen Platzhalter in Klaviyo
...
{% alert note %}
Der Zweck dieses Platzhalters ist es, die gewünschten Inhalte in BrazeAI Decisioning Studio™ Go zu importieren. Sie müssen in einem späteren Schritt einen separaten Ablauf erstellen, den BrazeAI Decisioning Studio™ Go verwendet, um Aktivierungen zu triggern, sobald Ihr Experimentator live ist.
{% endalert %}
...
### Teil 3: Erstellen Sie einen Fluss in Klaviyo
...
5\. Stimmen Sie den Namen der Metrik mit dem Namen des triggernden Ereignisses aus Ihrem BrazeAI Decisioning Studio™ Go Portal ab.
...
{% alert note %}
Wenn Ihr Experimentator eine Basis-Template hat, fahren Sie mit Schritt 2 fort. Wenn Ihr Experimentator über zwei oder mehr Basis-Templates verfügt, fahren Sie mit [Schritt 3 fort: Fügen Sie einen Trigger-Split zu Ihrem Fluss hinzu](#step-3-add-a-trigger-split-to-your-flow).
{% endalert %}

#### Schritt 2: Fügen Sie eine E-Mail zu Ihrem Fluss hinzu (einzelnes Template)
...
#### Schritt 3: Fügen Sie einen Trigger-Split zu Ihrem Fluss hinzu (mehrere Templates)
...
**Fügen Sie Ihr E-Mail Template hinzu:**
...
![Ein Klaviyo-Flow mit einem Trigger-Split-Knoten, der einen Ja-Zweig hat, der zu einem E-Mail-Knoten führt, und einen Nein-Zweig, der mit einem anderen Trigger-Split verbunden ist.]({% image_buster /assets/img/decisioning_studio_go/flow8.png %})
...
**Fügen Sie einen neuen Trigger-Split für jede zusätzliche Vorlage hinzu:**

1. Ziehen Sie per Drag-and-Drop einen anderen **geteilten** Knoten **des Triggers** in den Zweig **Nein** des vorherigen **geteilten** Knotens **des Triggers**.
...
5\. Wiederholen Sie die oben beschriebenen Schritte zur Einrichtung der E-Mail-Vorlage, um die entsprechende Vorlage auszuwählen.
6\. Setzen Sie die **Betreffzeile** auf {% raw %}`{{event.SubjectLine}}`{% endraw %}, und deaktivieren Sie das Kontrollkästchen **Zuletzt gemailte Profile überspringen**.
7\. Wiederholen Sie diesen Vorgang, bis Sie einen **Trigger-Split-Knoten** und einen **E-Mail-Knoten** für jede Basis-Vorlage haben, die Ihr Experimentator verwendet. Ihr letzter Trigger-Split sollte nichts im Branch "Nein" enthalten.
...
{: start="8"}
8\. Aktualisieren Sie in jedem Ihrer **E-Mail-Knoten** den Modus von **Entwurf** auf **Live**.
