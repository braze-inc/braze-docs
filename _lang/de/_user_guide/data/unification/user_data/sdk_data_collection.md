...
| Land | Durch Geolocation der IP-Adresse identifiziertes Land. Wenn die Geolocation der IP-Adresse nicht verfügbar ist, wird diese durch die [Lokalisierung des Geräts](#optional-data-collected-by-default) identifiziert. Der Wert könnte alternativ auch derjenige sein, den die SDKs direkt mit festlegen. `setCountry`Beachten Sie jedoch, dass die Übergabe eines Attributwerts über SDK oder API Datenpunkte protokolliert.| Dieses Attribut wird verwendet, um Nachrichten auf der Grundlage des Standorts zu versenden. |
...
| Sprache                | Android, iOS, Internet | Geräte- oder Browsersprache, entnommen aus dem Gebietsschema des Geräts                                                            | Dieses Attribut wird verwendet, um Nachrichten in die bevorzugte Sprache eines Nutzers:in zu übersetzen. Sie basiert auf dem Gebietsschema des Geräts. <br><br>Durch die Einstellung`language`in einem Nutzerprofil über die [REST API]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) wird verhindert, dass Braze diese Informationen künftig automatisch über das SDK erfasst.                                                                                            |
