...
Nehmen wir an, Sie möchten ein angepasstes Attribut für das Nutzerprofil mit der Bezeichnung `favorite_book` definieren. Dieses angepasste Attribut kann als ein Objekt definiert werden, das die verschachtelten Attribute `title`, `author` und `publishing_date` enthält, etwa so:
...
![Filtern auf der Grundlage eines angepassten Attributs für den meistgespielten Titel, wenn ein Hörer einen Titel mehr als eine bestimmte Anzahl von Malen gespielt hat][6]
...
![Ein Nutzer:innen wählt einen Operator auf der Grundlage des Datentyps für das verschachtelte angepasste Attribut][7]
