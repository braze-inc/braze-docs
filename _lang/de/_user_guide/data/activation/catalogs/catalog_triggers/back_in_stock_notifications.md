...
article_title: Benachrichtigungen bei Wiederverfügbarkeit einrichten
page_order: 2
...
3. Wenn die globalen „Wieder verfügbar“-Einstellungen nicht konfiguriert wurden, werden Sie aufgefordert, die angepassten Events und Eigenschaften einzurichten, die zum Auslösen von „Wieder verfügbar“-Benachrichtigungen verwendet werden sollen:
    ...
    - **Das angepasste Event für Abonnements** ist das angepasste Braze-Event, das verwendet wird, um einen Nutzer:in für Benachrichtigungen über wieder verfügbare Artikel zu registrieren. Wenn das Ereignis eintritt, erhält dieser ein Abonnement.
...
Nachdem Sie das Feature „Wieder auf Lager“ in einem Katalog eingerichtet haben, befolgen Sie bitte die folgenden Schritte, um es mit Canvas zu verwenden.
...
Um ein Template mit Details zu dem Artikel zu erstellen, der wieder verfügbar ist, können Sie den `context`-Liquid-Tag verwenden, um auf die `item_id` zuzugreifen. 

Die Verwendung von {%raw%}``{{context.${catalog_update}.item_id}}``{%endraw%} gibt die ID des Artikels zurück, der wieder auf Lager ist. {%raw%}``{{context.${catalog_update}.previous_value}}``{%endraw%} gibt den Bestandswert des Artikels vor der Aktualisierung zurück, und {%raw%}``{{context.${catalog_update}.new_value}}``{%endraw%} gibt den neuen Bestandswert nach der Aktualisierung zurück.
...
- Braze unterstützt bis zu 50.000 aktualisierte Artikel pro Tag, die triggern lassen können, um Benachrichtigungen über die Wiederverfügbarkeit zu erhalten. Es können bis zu 100 Millionen aktive Abonnements gleichzeitig bestehen, wobei jedes Abonnement ein Nutzerprofil darstellt, das für die Anzeige eines Katalogartikels abonniert ist.
