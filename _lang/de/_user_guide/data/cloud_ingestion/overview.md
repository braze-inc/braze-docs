...
Bei jeder Synchronisierung sucht Braze nach Zeilen, die noch nicht synchronisiert wurden. Wir überprüfen dies anhand der Spalte `UPDATED_AT` in Ihrer Tabelle oder Ansicht. Alle Zeilen, bei denen `UPDATED_AT` später als die letzte synchronisierte Zeile ist, werden ausgewählt und in Braze gezogen.
...
Bei jeder Synchronisierung sucht Braze nach Zeilen, die noch nicht synchronisiert wurden. Wir überprüfen dies anhand der Spalte `UPDATED_AT` in Ihrer Tabelle oder Ansicht. Alle Zeilen, bei denen `UPDATED_AT` nach der letzten synchronisierten Zeile liegt, werden ausgewählt und in Braze eingefügt, unabhängig davon, ob sie mit den aktuellen Daten im Nutzerprofil übereinstimmen. Wir empfehlen daher, nur Attribute zu synchronisieren, die Sie hinzufügen oder aktualisieren möchten.
...
Ihre CDI-Synchronisierung enthält möglicherweise doppelte Daten, wenn eines der Felder von `UPDATED_AT` zum exakt gleichen Zeitpunkt wie Ihre vorherige Synchronisierung stattfindet. Der Grund dafür ist, dass das CDI eine "einschließende Grenze" wählt, wenn es eine Zeile entdeckt, die mit der vorherigen Synchronisierung übereinstimmt, und die Zeilen für die Synchronisierung geeignet macht. Das CDI testet diese Zeilen erneut und erstellt doppelte Daten. 
