...
- **SMS-Fallback:** Braze empfiehlt dringend, dass jede Abo-Gruppe, die einen RCS-Sender enthält, auch mindestens einen SMS-Code als Fallback enthält. Dies ist wichtig für die Zustellbarkeit, falls RCS-Nachrichten nicht zugestellt werden können. Gründe dafür können unter anderem die Inkompatibilität der Nutzer:innen-Geräte und eine unvollständige Netzabdeckung in einem bestimmten Land oder einer bestimmten Region sein. Durch die Aktivierung von SMS-Fallback kann Ihre RCS-Nachricht weiterhin per SMS zugestellt werden, wenn RCS nicht möglich ist – so verpassen Sie keine Opportunity, mit Ihren Nutzer:innen in Kontakt zu treten.
