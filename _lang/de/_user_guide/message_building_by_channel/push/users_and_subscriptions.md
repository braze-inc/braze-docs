...
]({% image_buster /assets/img/push_example.png %})Nutzerprofil für John Doe, dessen Push-Abonnement auf Abo gesetzt ist.{: style="float:right;max-width:35%;margin-left:15px;"}()
...
|--|--|--|
|iOS| ]({% image_buster /assets/img/push_implementation_guide/ios-push-prompt.png %})Eine iOS-eigene Push-Aufforderung mit der Frage "Meine App möchte Ihnen Benachrichtigungen senden" mit zwei Buttons "Nicht zulassen" und "Zulassen" am unteren Rand der Nachricht.{: style="max-width:410px;"}() | Dies gilt nicht für die Beantragung einer [vorläufigen Push-Erlaubnis](#provisional-push).|
|Android| ]({% image_buster /assets/img/push_implementation_guide/android-push-prompt.png %})Eine Android Push-Nachricht mit der Frage "Erlauben Sie Kitchenerie, Ihnen Benachrichtigungen zu senden?" mit den beiden Buttons "Zulassen" und "Nicht zulassen" am unteren Rand der Nachricht.{: style="max-width:410px;"}() | Diese Push-Erlaubnis wurde in Android 13 eingeführt. Vor Android 13 war eine Genehmigung zum Senden von Push-Nachrichten nicht erforderlich.|
|Internet| Braze.comDie systemeigene Push-Eingabeaufforderung eines Webbrowsers mit der Frage "]({% image_buster /assets/img/push_implementation_guide/web-push-prompt.png %}) möchte eine Benachrichtigung anzeigen" mit zwei Buttons "Blockieren" und "Zulassen" am unteren Rand der Nachricht.{: style="max-width:410px;"}() | |
...
]({% image_buster /assets/img/push_implementation_guide/ios-provisional-push.png %})Eine Benachrichtigung im Benachrichtigungszentrum des Systems mit einer Nachricht am unteren Rand mit der Frage: "Erhalten Sie weiterhin Benachrichtigungen von der Yachtr App?" und zwei Buttons darunter zum "Behalten" oder "Ausschalten"{: style="float:right;max-width:430px;width:40%;margin-left:15px;border:0"}()
...
]({% image_buster /assets/img/push_token_changelog.png %})Änderungsprotokoll für Push-Token auf dem Tab \*\*Engagement** des Profils eines Nutzers, das auflistet, wann der Push-Token auf einen anderen Nutzer:in verschoben wurde und um welchen Token es sich handelt.()
...
]({% image_buster /assets/img/push_enablement.png %})Ein Screenshot des Dashboards, der zeigt, dass ein Nutzer:innen "Push Registered for Marketing (iOS)" ist{: style="float:right;max-width:50%;margin-left:15px;"}()
