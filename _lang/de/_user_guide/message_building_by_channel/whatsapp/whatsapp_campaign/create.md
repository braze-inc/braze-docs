...
]({% image_buster /assets/img/whatsapp/whatsapp_templates.png %})Liste der Templates mit einer Vorschau auf die Nachrichten, die zugewiesenen Sprachen und den Genehmigungsstatus.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp_response_messages.png %})Der Nachrichten-Editor für eine Antwortnachricht, die neue Nutzer:innen mit einem Rabattcode begrüßt.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp8.png %})Eine Vorschau der Nachricht für einen angepassten Nutzer:in namens Max.{: style="max-width:80%;"}()
...
| Bilder | Bilder können in den Text eingebettet werden. Sie müssen 8-Bit sein und entweder ein RGB- oder RGBA-Farbmodell verwenden. | < 5 MB | ``.jpg``.png` |
...
| Videos | Videos können in den Text eingebettet werden. Die Dateien müssen über eine URL oder in der [Bibliothek von Braze]({{site.baseurl}}/user_guide/engagement_tools/templates_and_media/media_library) gehostet werden. | < 16 MB | ``.mp4``.3gp` |
