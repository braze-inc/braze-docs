...
Sie können die Produkte entweder manuell mit IDs auswählen oder, wenn Sie bei der [Einrichtung](#setting-up-product-messages) die Katalogberechtigungen aktiviert haben, den Dropdown SELEKTOR verwenden.
...
]({% image_buster /assets/img/whatsapp/share_meta_catalog.png %})Fenster zur Freigabe eines Katalogs für einen Partner, das Felder zur Eingabe einer Partner Business ID und zur Zuweisung der Berechtigung "Katalog verwalten" enthält.{: style="max-width:70%;"}()
...
\![WhatsApp Manager:in Einstellungsseite für den "Catalog_products" Katalog.]({% image_buster /assets/img/whatsapp/meta_catalog_settings.png %}){: style="max-width:90%;"}
...
]({% image_buster /assets/img/whatsapp/multi_product_template.png %})Artikelliste mit Feldern zur Eingabe der Titel Ihrer Abschnitte und der ID des Inhalts.{: style="max-width:60%;"}()
...
`ecommerce.cart_updated`Canvas-Schritt für einen `whats_app`.]({% angepassten Event-Trigger mit der grundlegenden Eigenschaft "Quelle" gleich image_buster./assets/img/whatsapp/product_message_canvas_step.png()
...
Das Diagramm zeigt den Checkout-Workflow für einen Connected-Content-Aufruf: Meta sendet eine Nachricht über den Eingang einer Bestellung an Braze, das mit einer E-Commerce-Plattform hin- und hertelefoniert, und sendet dann eine WhatsApp-Nachricht.]({% image_buster /assets/img/whatsapp/connected_content_checkout.png %})()
