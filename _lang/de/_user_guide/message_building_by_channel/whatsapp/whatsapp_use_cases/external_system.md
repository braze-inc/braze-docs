...
article_title: Integrieren Sie Braze und WhatsApp mit einem externen System
page_order: 2
...
# Integrieren Sie Braze und WhatsApp mit einem externen KI- oder Kommunikationssystem
...
4. Wählen Sie **Rohtext** für den Textkörper der Anfrage aus und geben Sie eine Nutzlast mit Personalisierung ein, die die `external_id` und die Telefonnummer des Nutzers:innen, den Inhalt der Nachricht und andere relevante Informationen enthält, z.B:
...
{: start="3"}
3\. Fügen Sie dem Nachrichtentext die Eigenschaft API-Trigger hinzu, z. B. {% raw %}```{{api_trigger_properties.${external_system_msg+body}}}```{% endraw %}. Dies erlaubt es Ihrem KI-System, die zu versendende Nachricht zu füllen.
