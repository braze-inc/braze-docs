...
]({% image_buster /assets/img_archive/iam_compose.png %})Ein Beispiel für eine In-App-Nachricht einer Marke, um neue Kund:innen zu begrüßen und sie aufzufordern, ein Nutzerprofil anzulegen.{: style="max-width:85%" }()
...
]({% image_buster /assets/img/ai_copywriter/ai_copywriter_iam.png %})Gehen Sie auf die Schaltfläche "KI Copywriter" im Nachrichtenfeld des In-App-Editors.{: style="max-width:60%"}()
...
]({% image_buster /assets/img/primary-secondary-buttons.png %})Primäre und sekundäre Buttons in einer In-App-Nachricht()
...
]({% image_buster /assets/img_archive/in_app_schedule.png %})Aktionsbasierte Kampagne mit der Triggeraktion "Sitzung starten".{: style="max-width:80%"}()
...
]({% image_buster /assets/img_archive/bucket_prioritization.png %})Beispiel für die Priorisierung von In-App-Nachrichtenkampagne und Canvas.{: style="max-width:70%"}()
...
]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %})Kontrollkästchen für "Kampagnenfähigkeit vor Anzeige neu bewerten" ausgewählt.{:style="max-width:60%"}()
