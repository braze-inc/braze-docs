...
1. Gehen Sie in LINE auf die Registerkarte **Messaging API** und bearbeiten Sie Ihre **Webhook-Einstellungen**:
   ...
   - Aktivieren Sie **Webhook verwenden** und **Webhook-Zustellung**. <br><br> ]({% image_buster /assets/img/line/webhook_settings.png %})Webhook-Einstellungsseite, um die Webhook-URL zu überprüfen oder zu bearbeiten, "Webhook verwenden", "Webhook erneut zustellen" und "Fehlerstatistiken aggregieren" ein- oder auszuschalten.{: style="max-width:70%;"}()
...
]({% image_buster /assets/img/line/response_settings.png %})Antwort-Einstellungsseite mit Umschaltmöglichkeiten, wie Ihr Konto Chats behandelt.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/line/integration.png %})LINE Messaging Integration Seite mit LINE Integration Abschnitt.{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/line/line_subscription_groups.png %})Abschnitt LINE Abo-Gruppen, der eine Abo-Gruppe für den Kanal "LINE" anzeigt.{: style="max-width:80%;"}()
...
1. Erstellen Sie ein aktionsbasiertes Canvas, das auf einer Änderung des Abo-Status basiert und ausgelöst wird, wenn ein:e Nutzer:in Ihren LINE-Kanal abonniert.<br>]({% image_buster /assets/img/line/account_link_1.png %})Canvas, das triggert, wenn ein Nutzer:in den Kanal LINE abonniert wird.()
...
4\. (Optional) Erstellen Sie eine aktionsbasierte Kampagne oder ein Canvas, das ausgelöst wird, wenn der LINE-Benutzer identifiziert wird, um dem Benutzer seinen Gutscheincode zu senden. <br>]({% image_buster /assets/img/line/account_link_2.png %})Aktionsbasierte Kampagne, die ausgelöst wird, wenn der LINE-Nutzer:in identifiziert wird.()
...
1. Richten Sie ein Canvas ein, das die Braze ID eines Nutzers:innen bei einem bestimmten triggernden Wort zurückgibt. <br><br>Beispiel Auslöser <br><br>]({% image_buster /assets/img/line/trigger.png %})Triggern Sie die Kampagne an Nutzer:in, die eine eingehende LINE an eine bestimmte Abo-Gruppe gesendet haben.{: style="max-width:80%;"}()<br><br>Beispielnachricht<br><br>]({% image_buster /assets/img/line/message.png %})LINE Nachricht mit der ID des Braze-Benutzers.{: style="max-width:40%;"}()<br><br>
