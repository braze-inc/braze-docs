...
Die folgenden Filter für die Segmentierung gelten für Klick-Events, die zum Zeitpunkt der Verarbeitung des Events getrackt werden.  Weitere Einzelheiten finden Sie unter [Segmentierungsfilter]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters).
...
## Link-Aliasing für von Liquid generierte URLs
 ...
 Dies erlaubt Braze das Anhängen von Abfrageparametern (`lid = somevalue`), so dass Link-Aliasing richtig funktionieren kann. Wenn Sie nicht angeben, wo die Abfrageparameter angehängt werden sollen, erkennt Link-Aliasing diese URLs nicht und die Link-Templates können nicht angewendet werden.
