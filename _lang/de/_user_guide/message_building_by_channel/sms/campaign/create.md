...
{% alert important %}
Bevor Sie fortfahren, lesen Sie unsere Richtlinien für [SMS-Nachrichtensegmente und Kopier-Limits]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/segments/). SMS-Nachrichtensegmente sind die Zeichenpakete, die die Telefongesellschaften zur Messung von Textnachrichten verwenden. Nachrichten werden pro Nachrichtensegment abgerechnet. Es ist also eine gute Idee, die Feinheiten der Aufteilung von Nachrichten zu verstehen.
{% endalert %}
...
Wie Nachrichten von rechts nach links letztendlich aussehen, hängt weitgehend davon ab, wie die Diensteanbieter sie darstellen. 
...
![Vorschau der SMS-Kopie auf dem Tab „Testen“ des Editors. Im Profilbereich ist das Feld „Vorname“ auf „James“ eingestellt. Im Vorschaubereich lautet die SMS nun: „Hallo James, vielen Dank für Ihre Unterstützung!“]({% image_buster /assets/img/sms_campaign_test.png %})
...
Nun bauen Sie die weitere Kampagne auf. In den folgenden Abschnitten erfahren Sie mehr darüber, wie Sie unsere Tools zur Erstellung von SMS-Nachrichten am besten einsetzen.
...
Als Nächstes müssen Sie mithilfe von Segmenten oder Filtern eine [Zielgruppe erstellen]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/targeting_users/). Sie sollten bereits die Abonnementgruppe ausgewählt haben, die die Nutzer nach der Ebene oder Kategorie der Kommunikation mit Ihnen eingrenzt. In diesem Schritt wählen Sie die größere Zielgruppe aus Ihren Segmenten aus und grenzen dieses Segment mit unseren Filtern weiter ein, wenn Sie dies wünschen. Sie erhalten automatisch einen Überblick über die ungefähre Zusammensetzung dieses Segments. Denken Sie daran, dass die genaue Segmentzugehörigkeit immer erst kurz vor dem Versand der Nachricht berechnet wird.

{% alert tip %}
Interessieren Sie sich für SMS-Retargeting? Besuchen Sie unseren [Artikel über SMS-Retargeting]({{site.baseurl}}/user_guide/message_building_by_channel/sms/campaign/retargeting/), um mehr zu erfahren.
{% endalert %}
...
Sehen Sie sich als nächstes die [SMS-Berichterstattung]({{site.baseurl}}/user_guide/message_building_by_channel/sms/sms_campaign_analytics/) an, um zu erfahren, wie Sie auf die Ergebnisse Ihrer SMS-Kampagnen zugreifen können.
