...
page_type: update
description: "Dieser archivierte Artikel befasst sich mit der Partnerschaft zwischen Apptimize und Braze. Braze hat die Unterstützung für die Partnerschaft mit Apptimize zum September 2019 eingestellt."
---

# Apptimize

{% alert update %}
Braze hat die Unterstützung für die Partnerschaft mit Apptimize zum September 2019 eingestellt.

<br>

Sollten Sie derzeit Apptimize mit Braze verwenden, werden Sie keine Unterbrechung des Dienstes feststellen. Sie können die angepassten Attribute von Apptimize immer noch an Nutzerprofile von Braze anpassen. Es wird jedoch keine formale Eskalationsunterstützung mit dem Partner angeboten.

<br>

Bitte wenden Sie sich an die Braze- oder Apptimize-Vertretung, falls Sie weitere Fragen haben.
{% endalert %}


[Apptimize](https://apptimize.com/) ist eine Plattform für das Testen und die Weiterentwicklung mobiler Anwendungen.
die es Kund:innen ermöglicht, während des gesamten App-Entwicklungsprozesses schnell zu iterieren.

Apptimize kann in Verbindung mit Braze eingesetzt werden, um Ihr Wachstum zu ergänzen.
Marketing-/CRM-Strategien mit Produkt-UI-Tests durch die Synchronisierung von Experimenten und
Daten auf beiden Plattformen.

## Anwendungsfälle

Mit Braze und Apptimize können Sie beide Plattformen nutzen, um leistungsstarke End-to-End-Erlebnisse zu schaffen:

* Synchronisieren Sie das In-App- und CRM-Marketing für eine angepasste Aktion.
* Testen Sie eine neue Onboarding-Erfahrung in Apptimize und nutzen Sie Braze, um die Nutzer:innen während des gesamten neuen Ablaufs zu begleiten.
* Bitte testen Sie gleichzeitig die Produkt-Features und die dazugehörigen Nutzer-Nachrichten.
* Passen Sie die App-Erlebnisse und die entsprechenden Nachrichten für verschiedene Nutzersegmente an.

## Funktionsweise

Braze und Apptimize können miteinander integriert werden, um Daten von SDK zu SDK zu übertragen.
Sie können aktive Apptimize-A/B-Testgruppen mit Braze synchronisieren, wodurch es zulässig ist,
Die Nutzer:innen eines bestimmten Apptimize-Tests innerhalb von Braze über Push-Benachrichtigungen, E-Mails,
oder In-App-Nachrichten.

Wir verfügen über einen Beispiel-Integrationscode, der veranschaulicht, wie Braze und Apptimize
SDKs können Daten an Braze übermitteln, um angepasstes Targeting und Segmentierung zu ermöglichen, basierend auf
Apptimize-Experimentdaten.

Diese Beispielintegration legt benutzerdefinierte Attribute für die Braze-Nutzer:innen Ihrer Nutzer fest.
Profil für die folgenden Apptimize-Daten:

* Die vollständige Liste der aktiven Experimente, an denen die Nutzer:innen derzeit teilnehmen.
* Die vollständige Liste aller Experimente, an denen die Nutzer:innen jemals teilgenommen haben, einschließlich abgeschlossener Experimente.
* Die Variante(n), die die Nutzer:innen im Rahmen einer Experimentteilnahme gesehen haben.

> Feature-Flags werden als Experimente betrachtet, bei denen die einzige Variante darin besteht, ob das Feature-Flag aktiviert ist. Wenn das Feature-Flag deaktiviert ist, werden keine Daten gemeldet.

Darüber hinaus protokolliert diese Integration ein angepasstes Braze-Event für das erste
Teilnahme an einem Experiment. Es gibt zwei Möglichkeiten, dies zu tun:

* Es wird ein angepasstes Event mit Daten zu den Eigenschaften des Experiments, der Experiment-ID, dem Namen der Variante und der Varianten-ID generiert. Anschließend können Sie Nutzer:innen über Realtime-Trigger mithilfe der aktionsbasierten Zustellung und Canvases von Braze erneut Retargeten. Verwenden Sie diese Eigenschaften, um den genauen Bezeichner des Apptimize-Experiments zu identifizieren, das Sie triggern möchten.
* Es wird ein Attributarray mit Einträgen für jede erfolgte Teilnahme generiert. Jede Teilnahme wird wie folgt formatiert: `experiment_id_EXPERIMENT_ID:variant_id_VARIANT_ID:experiment_name_EXPERIMENT_NAME:variant_name_VARIANT_NAME`

Anschließend können Sie die aktionsbasierten Kampagnen für die Zustellung oder Canvases von Braze nutzen, um
Folgenachrichten an Nutzer:innen in Realtime, wenn diese Ereignisse ausgelöst werden.

## Integration

### iOS
Um die Integration in Ihre App durchzuführen, importieren Sie bitte Folgendes`Appboy-Apptimize.m`:
`Apptimize-Appboy.h` Dateien in Ihr Xcode-Projekt importieren, importieren Sie die `Appboy-Apptimize.h`
Fügen Sie den Header in Ihre AppDelegate-Implementierung ein und fügen Sie Folgendes hinzu:
`didFinishLaunchingWithOptions` Nach der Initialisierung von Appboy und Apptimize:
...
Importieren Sie die`apptimizeappboy.java`Klasse in Ihre App und in Ihre Hauptfunktion. `activity`
...
Anschließend in Ihrer onCreate-Methode, nach der Initialisierung von Braze und Apptimize:
