...
> Mit Feature-Flags können Sie Funktionen für eine ausgewählte Nutzergruppe remote aktivieren oder deaktivieren. Erstellen Sie ein neues Feature-Flag im des Braze-Dashboard. Geben Sie einen Namen und eine `ID`, eine Zielgruppe und einen Prozentsatz der Nutzer an, für die dieses Feature aktiviert werden soll. Dann können Sie mit der gleichen `ID` im Code Ihrer Anwendung oder Website bestimmte Teile Ihrer Geschäftslogik bedingt ausführen. Mehr über Feature-Flags und wie Sie sie in Braze verwenden können, erfahren Sie unter [Über Feature-Flags]({{site.baseurl}}/developer_guide/platform_wide/feature_flags/about/).
...
{% alert tip %}
Sie können beliebig viele Eigenschaften hinzufügen. Die Gesamtgröße der Eigenschaften eines Feature-Flags ist jedoch auf 10 KB begrenzt. Sowohl die Eigenschaftswerte als auch die Schlüssel sind auf eine Länge von 255 Zeichen begrenzt.
{% endalert %}
{% endtab %}
{% endtabs %}
...
Wenn ein Feature-Flag nicht aktiviert ist oder eine Eigenschaft, die Sie referenzieren, nicht existiert, geben diese Methoden `null` zurück.
...
Hier können Sie überprüfen, wann eine Änderung vorgenommen wurde, wer sie vorgenommen hat, zu welcher Kategorie sie gehört und vieles mehr.
