---
nav_title: Übersicht über Analytics
article_title: Übersicht über Analytics
page_order: 4
description: "Dieser Artikel referenziert die Datenerfassung von Nutzer:innen, einschließlich der automatisch erfassten Daten, Kauf-Events und angepassten Events, sowie bewährte Methoden der Datenerfassung."

---

# Übersicht über Analytics

> In diesem Leitfaden erfahren Sie, welche Daten Braze sammelt, was der Unterschied zwischen einem angepassten Event und einem angepassten Attribut in Braze ist und wie Sie diese Analytics am besten verwalten.

Bevor Sie Ihre Braze-Implementierung abschließen, sollten Ihr Marketing- und Entwickler-Team Ihre Marketingziele besprechen. Wenn Sie entscheiden, was Sie mit Braze tracken wollen und wie Sie es tun wollen, ist es sinnvoll, diese Ziele zu berücksichtigen und rückwärts zu arbeiten. 

Referenzieren Sie unseren Fall einer [Taxi/Ride-Sharing-App](#example-case) am Ende dieses Leitfadens, um ein Beispiel für diesen Prozess zu erhalten.

## Automatisch erfasste Daten

Bestimmte Nutzerdaten werden von unserem SDK automatisch erfasst, z. B. die zuerst verwendete App, die zuletzt verwendete App, die Gesamtzahl der Sitzungen, das Betriebssystem des Geräts usw. Wenn Sie unseren Integrationsleitfäden folgen, um unsere SDKs zu implementieren, können Sie die Vorteile dieser [Standard Datenerfassung]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/sdk_data_collection/) nutzen. Wenn Sie diese Liste überprüfen, können Sie vermeiden, die gleichen Informationen über Nutzer mehrfach zu speichern. Mit Ausnahme von Sitzungsbeginn und -ende werden alle anderen automatisch getrackten Daten nicht auf Ihr Datenpunkt-Kontingent angerechnet.

In unserem Artikel über [SDK Primer]({{site.baseurl}}/developer_guide/platform_integration_guides/sdk_primer/) können Sie Prozesse auflisten, die die standardmäßige Datenerfassung bestimmter Artikel blockieren.
...
| ---------------------| --------------- | ------------- |
| Prüfen Sie, ob das angepasste Event **mehr als X-mal** aufgetreten ist | **MEHR ALS** | **ZAHL** |
| Prüfen Sie, ob das angepasste Event **weniger als X-mal** aufgetreten ist | **WENIGER ALS** | **ZAHL** |
| Prüfen Sie, ob das angepasste Event **exakt X-mal** aufgetreten ist | **EXAKT** | **ZAHL** |
| Prüfen Sie, ob das angepasste Event zuletzt **nach dem Datum X** aufgetreten ist | **NACH** | **TIME** |
| Prüfen Sie, ob das angepasste Event zuletzt **vor dem Datum X** aufgetreten ist | **BEVOR** | **TIME** |
| Prüfen Sie, ob das angepasste Event zuletzt **vor mehr als X Tagen** stattgefunden hat. | **MEHR ALS** | **ANZAHL DER TAGE VORHER** (positive) Zahl) |
...
| Prüfen Sie, ob das angepasste Event **weniger als X (Max = 50) Mal aufgetreten ist ** | **WENIGER ALS** | in den letzten **Y Tagen (Y = 1,3,7,14,21,30)** |
| Prüfen Sie, ob das angepasste Event **exakt X (Max = 50) Mal aufgetreten ist** | **EXAKT** | in den letzten **Y Tagen (Y = 1,3,7,14,21,30)** |
...
Angepasste Event-Eigenschaften helfen Ihnen dabei, Ihr Messaging anzupassen oder granulare aktionsbasierte Zustellungskampagnen zu erstellen. Wenn Sie Segmente auf der Grundlage der Häufigkeit von Event-Eigenschaften erstellen möchten, wenden Sie sich an Ihren Customer-Success-Manager oder an unser Support Team, da dies zusätzliche Datenkosten verursachen kann.
...
| Prüfen Sie, ob das Zeit-Attribut **mehr als** **X Tage in der Zukunft liegt**. | **MEHR ALS** | **ANZAHL DER TAGE IN DER ZUKUNFT** |
...
> Das ausgegebene Geld sollte nicht auf diese Weise erfasst werden. Vielmehr sollte sie über unsere [Kaufmethoden]({{site.baseurl}}/developer_guide/platform_wide/analytics_overview/#purchase-events--revenue-tracking) erfasst werden.
