...
# [![Braze Learning-Kurs]({% image_buster /assets/img/bl_icon3.png %})](https://learning.braze.com/path/developer){: style="float:right;width:120px;border:0;" class="noimgborder"} Erste Schritte: Übersicht über die Plattform
...
Braze ist eine Plattform zur Kundenbindung. Es erfasst Nutzerdaten, zeigt Benutzeraktionen und -verhalten an und ermöglicht es Ihnen, darauf zu reagieren. Die Plattform umfasst drei Hauptkomponenten: das SDK, das Dashboard und die REST API.
...
![Braze hat verschiedene Schichten. Insgesamt besteht es aus dem SDK, der API, dem Dashboard und den Partnerintegrationen. Diese tragen jeweils zu einem Datenaufnahme-Layer, einem Klassifizierungs-Layer, einem Orchestrierungs-Layer, einem Personalisierungs-Layer und einem Aktions-Layer bei. Die Aktionsschicht verfügt über verschiedene Kanäle, darunter Push-Benachrichtigungen, In-App-Nachrichten, Connected Catalog, Webhook, SMS und E-Mail.]({% image_buster /assets/img/getting-started/getting-started-vertically-integrated-stack.png %}){: style="max-width:55%;float:right;margin-left:15px;"}
...
![Ein Segment im Braze-Dashboard mit dem Titel „Kürzlich gekaufte Artikel” wird neben einem Telefonbildschirm angezeigt, auf dem eine E-Mail mit dem Titel „Top-Empfehlungen für Linda” zu sehen ist.]({% image_buster /assets/img/getting-started/getting-started-segment.png %}){: style="max-width:80%"}

### App-Analysen

Das Braze-Dashboard zeigt Grafiken an, die in Realtime auf der Grundlage von Analytics-Metriken und angepassten Events, die Sie instrumentieren, aktualisiert werden. Konsistente Messungen und Optimierungen mithilfe von A/B-Tests, benutzerdefinierten Berichten, Analytics und Automatisierung unterstützen Ihr Customer-Engagement und Ihre Differenzierung.
...
Braze ist für eine schnelle Integration konzipiert. Die durchschnittliche Amortisationszeit beträgt in unserem Kundenstamm sechs Wochen. Weitere Informationen zur Integration finden Sie unter [Erste Schritte: Übersicht über die Integration]({{site.baseurl}}/developer_guide/getting_started/integration_overview/).
