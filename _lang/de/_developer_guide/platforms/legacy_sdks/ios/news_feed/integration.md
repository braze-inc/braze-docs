...
{% alert note %}
Der News Feed, der durch das Standardverhalten eines Klicks auf eine In-App-Nachricht aufgerufen wird, beachtet keine Delegierten, die Sie für den News Feed festgelegt haben. Hierzu müssen Sie [den Delegaten auf `ABKInAppMessageUIController` festlegen]({{site.baseurl}}/developer_guide/platforms/legacy_sdks/ios/in-app_messaging/customization/setting_delegates/) und die Delegate-Methode [`onInAppMessageClicked:`]({{site.baseurl}}/developer_guide/platform_integration_guides/ios/in-app_messaging/customization/behavior_on_click/#customizing-in-app-message-body-clicks) für `ABKInAppMessageUIDelegate` implementieren.
{% endalert %}
