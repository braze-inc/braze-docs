Durch die Installation des Braze SDK erhalten Sie grundlegende Analysefunktionen{% if include.platform == 'iOS' %} sowie eine funktionierende In-App-Nachricht, mit der Sie Ihre Benutzer einbinden können{% endif %}.
...
Jetzt sollten Sie den neuen, von CocoaPods erstellten Xcode-Projektarbeitsbereich öffnen können. Stellen Sie sicher, dass Sie diesen Xcode-Arbeitsbereich anstelle Ihres Xcode-Projekts verwenden. 
...
Fügen Sie in Ihrer Datei `AppDelegate.m` das folgende Snippet in Ihre Methode `application:didFinishLaunchingWithOptions` ein:
...
{% alert warning %}
Stellen Sie sicher, dass Sie Braze im Hauptthread Ihrer Anwendung initialisieren. Eine asynchrone Initialisierung kann zu fehlerhaften Funktionen führen.
{% endalert %}
...
Braze sollte nun Daten von Ihrer Anwendung sammeln und Ihre grundlegende Integration sollte abgeschlossen sein. {% if include.platform == 'iOS' %}Bitte lesen Sie die folgenden Abschnitte, um die benutzerdefinierte Ereignisverfolgung, Push-Nachrichten, den News-Feed und die gesamte Palette der Braze-Funktionen zu aktivieren.{% else %}Bitte beachten Sie, dass Bitcode beim Kompilieren Ihrer tvOS-App und aller anderen Bibliotheken von Drittanbietern aktiviert sein muss.{% endif %}
...
Fügen Sie in `AppDelegate.swift` innerhalb Ihrer Methode `application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool` die folgende Lötmethode hinzu:
...
- `launchOptions` - Die Optionen `NSDictionary`, die Sie von `application:didFinishLaunchingWithOptions:`
- `appboyOptions` - Eine optionale `NSDictionary` mit Startkonfigurationswerten für Braze

Siehe [Appboy.h][apple_initial_setup_5] für eine Liste der Braze-Starttasten.

## Appboy.sharedInstance() und Swift Nullbarkeit
Abweichend von der üblichen Praxis ist das Singleton `Appboy.sharedInstance()` optional. Das liegt daran, dass `sharedInstance` `nil` ist, bevor `startWithApiKey:` aufgerufen wird, und es gibt einige nicht standardisierte, aber nicht ungültige Implementierungen, in denen eine verzögerte Initialisierung verwendet werden kann.

Wenn Sie `startWithApiKey:` in Ihrem `didFinishLaunchingWithOptions:` Delegaten aufrufen, bevor Sie auf `sharedInstance` (die Standardimplementierung) von Appboy zugreifen, können Sie eine optionale Verkettung wie `Appboy.sharedInstance()?.changeUser("testUser")` verwenden, um lästige Überprüfungen zu vermeiden. Dies ist vergleichbar mit einer Objective-C-Implementierung, die von einem Nicht-Null-Wert `sharedInstance` ausgeht.
