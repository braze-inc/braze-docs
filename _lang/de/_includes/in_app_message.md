...
[`Slideup`]{% if include.platform == "iOS" %}[in_app_message_1]{% elsif include.platform == "Android" %}[in_app_message_2]{% endif %} in-app messages werden so genannt, weil sie vom oberen oder unteren Bildschirmrand "nach oben" oder "nach unten" rutschen.  Sie bedecken nur einen kleinen Teil des Bildschirms und bieten eine effektive und unaufdringliche Möglichkeit zur Nachrichtenübermittlung.

![Slideup Beispiel][in_app_message_9]

### Modale In-App-Nachrichten

[`Modal`]{% if include.platform == "iOS" %}[in_app_message_3]{% elsif include.platform == "Android" %}[in_app_message_4]{% endif %} in-app Nachrichten erscheinen in der Mitte des Bildschirms und werden von einem durchsichtigen Feld eingerahmt. Sie können mit bis zu zwei Schaltflächen mit Klickfunktion und Analysefunktion ausgestattet werden und sind für kritische Nachrichten nützlich.

![Modales Beispiel][in_app_message_10]

### Vollständige In-App-Nachrichten

[`Full`]{% if include.platform == "iOS" %}[in_app_message_5]{% elsif include.platform == "Android" %}[in_app_message_6]{% endif %} in-app Nachrichten sind nützlich, um den Inhalt und die Wirkung Ihrer Benutzerkommunikation zu maximieren.  Die obere Hälfte einer `full` In-App-Nachricht enthält ein Bild und die untere Hälfte zeigt Text sowie bis zu zwei Schaltflächen für Klick-Aktionen und Analysen an.

![Vollständiges Beispiel][in_app_message_11]
...
![HTML5 Beispiel][in_app_message_23]

 {% elsif include.platform == "Android" %}Das folgende Beispiel zeigt eine von SoundCloud erstellte Umfrage HTML Full in-app message.

![HTML5 Beispiel][in_app_message_12]
{% endif %}

Der gesamte Inhalt der In-App-Nachrichten wird in einer WKWebView angezeigt und kann optional andere Rich Content-Inhalte wie Bilder und Schriftarten enthalten. So haben Sie die volle Kontrolle über das Aussehen und die Funktionalität der Nachrichten. **Bitte beachten Sie, dass wir derzeit die Anzeige von benutzerdefinierten HTML-In-App-Nachrichten in einem iFrame auf den Plattformen iOS und Android nicht unterstützen.**

## Zustellung von In-App-Nachrichten

### In-App-Nachrichten (ausgelöst)

Die folgende Dokumentation bezieht sich auf das Produkt `In-App Messaging` von Braze, auch bekannt als "ausgelöste In-App-Nachrichten", die wie unten im Dropdown-Menü "Kampagne erstellen" hervorgehoben sind:

![In-App Messaging Composer][in_app_message_13]

Sie können auch die Dokumentation für unser veraltetes [`Original In-App Messaging`][in_app_message_14] Produkt einsehen.

#### Auslöser-Typen

Mit unserem Produkt für In-App-Nachrichten können Sie die Anzeige von In-App-Nachrichten als Folge verschiedener Ereignistypen auslösen: `Any Purchase`, `Specific Purchase`, `Session Start`, `Custom Event`, `Push Click`.  Außerdem können `Specific Purchase` und `Custom Event` Auslöser robuste Eigenschaftsfilter enthalten.

{% alert note %}
Ausgelöste In-App-Nachrichten funktionieren nur bei benutzerdefinierten Ereignissen, die über das Braze SDK protokolliert werden. In-App-Nachrichten können nicht über die API oder durch API-Ereignisse (wie Kaufereignisse) ausgelöst werden. Wenn Sie mit Android arbeiten, sehen Sie sich an, wie man [benutzerdefinierte Ereignisse unter Android protokolliert][in_app_message_24]. Wenn Sie mit iOS arbeiten, sehen Sie sich an, wie man [benutzerdefinierte Ereignisse unter iOS protokolliert][in_app_message_25].
{% endalert %}

#### Semantik der Lieferung

Alle In-App-Nachrichten, für die ein Nutzer berechtigt ist, werden zu Beginn der Sitzung an das Gerät des Nutzers gesendet. Weitere Informationen über die Semantik des Sitzungsstarts im SDK finden Sie in unserer [session lifecycle documentation]{% if include.platform == "iOS" %}[in_app_message_15a]{% elsif include.platform == "Android" %}[in_app_message_15b]{% endif %}. Bei der Auslieferung holt das SDK die Assets vorab, so dass sie zum Zeitpunkt des Auslösens sofort verfügbar sind und die Anzeige-Latenzzeit minimiert wird.

Wenn ein auslösendes Ereignis mit mehr als einer in Frage kommenden In-App-Nachricht verbunden ist, wird nur die In-App-Nachricht mit der höchsten Priorität zugestellt.

Bei In-App-Nachrichten, die sofort nach der Auslieferung angezeigt werden (z. B. Sitzungsstart, Push-Klick), kann es zu einer gewissen Latenz kommen, da die Assets nicht vorab geholt werden.
