...
layout: api_page
page_type: reference
description: "Dieser Artikel beschreibt Details zum Braze-Endpunkt Kampagnenliste exportieren."

---
{% api %}
# Kampagnenliste exportieren
{% apimethod get %}
/campaigns/list
{% endapimethod %}

> Verwenden Sie diesen Endpunkt, um eine Liste von Kampagnen zu exportieren, die jeweils den Namen, die API-Kennung der Kampagne, die Angabe, ob es sich um eine API-Kampagne handelt, und die mit der Kampagne verbundenen Tags enthält.
...
| --------- | -------- | --------- | ----------- |
| `page` | Optional | Integer | Die Seite der zurückzugebenden Kampagnen, Standardwert ist 0 (gibt den ersten Satz von bis zu 100 zurück). |
| `include_archived` | Optional | Boolesche | Ob archivierte Kampagnen einbezogen werden sollen oder nicht, Standardwert ist false. |
...
| `last_edit.time[gt]` | Optional | Zeit | Filtert die Ergebnisse und gibt nur Kampagnen zurück, die länger als die angegebene Zeit bearbeitet wurden. Das Format ist `yyyy-MM-DDTHH:mm:ss`. |
