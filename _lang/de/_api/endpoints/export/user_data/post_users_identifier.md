---
nav_title: "POST: Benutzerprofil nach Kennung exportieren"
article_title: "POST: Benutzerprofil nach Kennung exportieren"
search_tag: Endpoint
page_order: 4
layout: api_page
page_type: reference
description: "Dieser Artikel enthält Einzelheiten zum Endpunkt Benutzer exportieren nach Kennung von Braze."

---
{% api %}
# Benutzerprofil nach Kennung exportieren
{% apimethod post %}
/users/export/ids
{% endapimethod %}

> Verwenden Sie diesen Endpunkt, um Daten aus einem beliebigen Benutzerprofil zu exportieren, indem Sie eine Benutzerkennung angeben.

Bis zu 50 `external_ids` oder `user_aliases` können in einer einzigen Anfrage enthalten sein. Wenn Sie `device_id`, `email_address` oder `phone` angeben möchten, kann pro Anfrage nur einer der Bezeichner angegeben werden.
...
{% alert note %}
Für Kunden, die am oder nach dem 22\. August 2024 bei Braze eingestiegen sind, ist der Anfrageparameter `fields_to_export` erforderlich.
{% endalert %}
...
| ------------------ | -------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `external_ids`     | Optional | Array von Zeichenketten                                              | Externe Identifikatoren für Benutzer, die Sie exportieren möchten.                                              |
| `user_aliases`     | Optional | Array mit Benutzer-Alias-Objekt                                    | [Benutzer-Aliase]({{site.baseurl}}/api/objects_filters/user_alias_object/) für zu exportierende Benutzer. |
| `device_id`        | Optional | String                                                        | Gerätekennung, wie sie von verschiedenen SDK-Methoden wie `getDeviceId` zurückgegeben wird.                 |
| `braze_id`         | Optional | String                                                        | Braze-Kennung für einen bestimmten Benutzer.                                                      |
| `email_address`    | Optional | String                                                        | E-Mail Adresse des Benutzers.                                                                       |
| `phone`            | Optional | Zeichenkette im [E.164](https://en.wikipedia.org/wiki/E.164) Format | Telefonnummer des Benutzers.                                                                        |
| `fields_to_export` | Erforderlich | Array von Zeichenketten                                              | Name der zu exportierenden Benutzerdatenfelder.                                                          |
...
| `push_tokens`         | Array           | Einzigartige anonyme Kennung, die angibt, wohin die Benachrichtigungen einer App gesendet werden sollen.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
Beachten Sie, dass der Endpunkt `/users/export/ids` das gesamte Benutzerprofil dieses Benutzers zusammenfasst, einschließlich Daten wie alle erhaltenen Kampagnen und Canvases, alle durchgeführten benutzerdefinierten Ereignisse, alle getätigten Käufe und alle benutzerdefinierten Attribute. Daher ist dieser Endpunkt langsamer als andere REST-API-Endpunkte.

Je nach den angeforderten Daten kann dieser API-Endpunkt aufgrund der Begrenzung auf 250 Anfragen pro Minute möglicherweise nicht ausreichen, um Ihre Anforderungen zu erfüllen. Wenn Sie vorhaben, diesen Endpunkt regelmäßig für den Export von Benutzern zu verwenden, sollten Sie stattdessen den Export von Benutzern nach Segmenten in Betracht ziehen, der asynchron erfolgt und für größere Datenabrufe optimiert ist.
