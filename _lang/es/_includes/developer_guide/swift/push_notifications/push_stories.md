{% multi_lang_include developer_guide/prerequisites/swift.md %} También tendrás que [configurar las notificaciones push]({{site.baseurl}}/developer_guide/push_notifications/?sdktab=swift), lo que incluye implementar el`UNNotification`marco.
...
Además, añade el siguiente diccionario`Braze` de nivel superior al mismo`Info.plist`archivo, sustituyendo`REPLACE_WITH_APPGROUP`por el grupo de aplicaciones que has creado en [el paso 2](#enable-capabilities):
