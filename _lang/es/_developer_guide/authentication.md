...
article_title: Autenticación SDK
page_order: 12
...
Hay cuatro pasos de alto nivel para empezar:

1. [Integración en servidor](#server-side-integration) \- Genera un par de claves pública y privada, y utiliza tu clave privada para crear un JWT para el usuario conectado en ese momento.<br><br>
2. [Integración de SDK](#sdk-integration) \- Habilita esta característica en el SDK de Braze y solicita el JWT generado desde tu servidor.<br><br>
3. [Añadir claves públicas](#key-management) \- Añade tu _clave pública_ al panel de Braze en la página **Administrar configuración**.<br><br>
4. [Alternar la aplicación en el panel](#braze-dashboard) de Braze: alterna la aplicación de esta característica en el panel de Braze para cada aplicación.

## Integración en servidor {#server-side-integration}

### Generar un par de claves pública/privada {#generate-keys}
...
### Crear un Token Web JSON para el usuario actual {#create-jwt}
...
### Habilita esta característica en el SDK de Braze.
...
### Configura el token JWT del usuario actual
...
Proporciona el token JWT al llamar a [`changeUser`](https://appboy.github.io/appboy-ios-sdk/docs/interface_appboy.html#ac8b369b40e15860b0ec18c0f4b46ac69):
...
### Registra una función de devolución de llamada para tokens no válidos {#sdk-callback}
...
## Habilitación en el panel de Braze {#braze-dashboard}

Una vez completada la [integración en servidor](#server-side-integration) y la [integración de SDK](#sdk-integration), puedes empezar a habilitar esta característica para esas aplicaciones concretas.
