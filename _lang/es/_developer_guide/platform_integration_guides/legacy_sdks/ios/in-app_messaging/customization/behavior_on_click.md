...
| -------------------------- | -------- |
| `ABKInAppMessageDisplayNewsFeed` | Cuando se haga clic en el mensaje, se mostrará la fuente de noticias y se descartará el mensaje. Ten en cuenta que el parámetro `uri` se ignorará, y que la propiedad `uri` de `ABKInAppMessage` se establecerá en cero. |
