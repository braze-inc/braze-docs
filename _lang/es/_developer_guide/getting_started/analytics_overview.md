---
nav_title: Resumen de análisis
article_title: Resumen de análisis
page_order: 4
description: "Este artículo de referencia trata de la recopilación de datos de usuario, incluidos los datos que se recopilan automáticamente, los eventos de compra y los eventos personalizados, así como las mejores prácticas de recopilación de datos."

---

# Resumen de análisis

> Esta guía te ayudará a comprender qué datos recopila Braze, la diferencia entre un evento personalizado y un atributo personalizado en Braze, y las mejores prácticas para gestionar estos análisis.

Antes de completar tu implementación de Braze, tu equipo de marketing y tu equipo de desarrolladores deben discutir tus objetivos de marketing. A la hora de decidir a qué quieres hacerle seguimiento y cómo quieres hacerlo con Braze, es útil tener en cuenta estos objetivos y trabajar hacia atrás a partir de ahí. 

Consulta nuestro caso de una [aplicación de taxi/viaje](#example-case) compartido al final de esta guía para ver un ejemplo de este proceso.

## Datos recogidos automáticamente

Nuestro SDK recopila automáticamente determinados datos de usuario, por ejemplo, la primera aplicación utilizada, la última aplicación utilizada, el recuento total de sesiones, el sistema operativo del dispositivo, etc. Si sigues nuestras guías de integración para implementar nuestros SDK, podrás aprovechar esta [recopilación de datos predeterminada]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/sdk_data_collection/). Comprobar esta lista puede ayudarte a evitar almacenar la misma información sobre los usuarios más de una vez. A excepción del inicio y fin de sesión, el resto de datos de seguimiento automático no cuentan para tu asignación de puntos de datos.

Consulta nuestro artículo [de introducción al SDK]({{site.baseurl}}/developer_guide/platform_integration_guides/sdk_primer/) para permitir procesos de lista que bloqueen la recogida predeterminada de determinados elementos de datos.
...
| Comprueba si el evento personalizado ocurrió por última vez **antes de X fecha** | **ANTES** | **TIME** |
| Comprueba si el evento personalizado ocurrió por última vez **hace más de X días** | **MÁS DE** | **NÚMERO DE DÍAS ATRÁS** (Positivo) Número) |
...
Las propiedades del evento personalizadas están diseñadas para ayudarte a personalizar tus mensajes o a crear campañas de entrega basadas en acciones granulares. Si quieres crear segmentos basados en la frecuencia y la recurrencia de las propiedades del evento, ponte en contacto con tu administrador del éxito del cliente o con nuestro equipo de soporte, ya que esto puede conllevar costes de datos adicionales.
...
- [Depurador y comprobador de regex](https://regex101.com/)
...
>  Si quieres segmentar en función del número de veces que se ha producido una compra concreta, también debes registrar esa compra individualmente como un [atributo personalizado incremental](#integers).
...
4. Para llamar un taxi, deben estar disponibles cuando lo busquen.
