...
article_title: Archivo de usuarios
...
{% alert important %}
Los usuarios archivados serán eliminados permanentemente. <br><br>Puedes [personalizar tu política de archivado de usuarios](#customizing-your-user-archival-policy) utilizando Canvas. Los clientes tienen pleno control sobre si un usuario está inactivo o no. Canvas ofrece la posibilidad de hacerlo automáticamente, lo que te permite desactivar esta función para algunos o todos tus usuarios inactivos.
{% endalert %}
...
Si estableces ID de usuario para identificar a los usuarios cuando un nuevo usuario se conecte, se contará como un usuario activo independiente. Los usuarios que se actualicen a través de la API también se contabilizarán como usuarios activos en el periodo de tiempo en que se actualicen.
...
1. Dirígete a usuarios que cumplan los criterios de archivo de tu marca y que te gustaría conservar. Por ejemplo, podrías retener a los usuarios que:
    - Recibió un mensaje por última vez hace más de 23 semanas o nunca ha recibido un mensaje<br>Y<br>
