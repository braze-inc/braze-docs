...
La facturación del punto de datos para la ingesta de datos en la nube es equivalente a la facturación de las actualizaciones a través del [punto final `/users/track`]({{site.baseurl}}/api/endpoints/user_data/post_user_track#user-track). Consulte [Puntos de datos]({{site.baseurl}}/user_guide/data/data_points/) para obtener más información. 
...
{% endtab %}
{% tab Comprar %}
 El campo `time`, que es opcional, debe formatearse como una cadena ISO 8601 o en formato `yyyy-MM-dd'T'HH:mm:ss:SSSZ`. Si el campo `time` no está presente, se utiliza el valor de la columna `UPDATED_AT` como hora del evento. Otros campos, como `app_id`, `quantity` y `properties` son opcionales.
