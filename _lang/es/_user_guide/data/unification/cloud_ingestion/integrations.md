...
Tras crear la cuenta de servicio y conceder los permisos, genera una clave JSON. Consulta más información sobre cómo hacerlo [aquí](https://cloud.google.com/iam/docs/keys-create-delete). La cargarás en el dashboard de Braze más adelante. 
...
3. Introduce un nombre y, a continuación, selecciona `Accounts in this organizational directory only` como tipo de cuenta admitido. A continuación, selecciona **Registrar**. 
...
* En la **configuración del desarrollador**, habilita **Los principales de servicio pueden utilizar API de Fabric** para que Braze pueda conectarse utilizando Microsoft Entra ID.
...
Cualquier recurso de Fabric que quieras conectar a Braze debe colocarse en un espacio de trabajo compartido. Si solo has estado usando el espacio de trabajo predeterminado **Mi espacio de trabajo**, crea un nuevo espacio de trabajo compartido:

1. En el menú de navegación, selecciona **Espacios de trabajo** y luego selecciona **+ Nuevo espacio de trabajo**.
2. Introduce un **Nombre** para el espacio de trabajo y selecciona **Aplicar**.

Una vez que tengas un espacio de trabajo compartido, concede acceso al principal de servicio:

1. Selecciona el espacio de trabajo y luego selecciona **Administrar acceso**.
2. Selecciona **+ Añadir personas o grupos**.
3. Busca y selecciona el nombre del principal de servicio que creaste en el paso 1.1. Si no aparece, confirma que has habilitado la configuración **Los principales de servicio pueden utilizar API de Fabric** en el paso 1.2.
4. En el desplegable de rol, selecciona **Colaborador**.
...
Braze admite tanto tablas como vistas en Fabric Warehouses. Si necesitas crear un nuevo almacén, créalo dentro del espacio de trabajo compartido del paso 1.3. Ve a **Crear > Almacén de datos > Almacén** en la consola de Fabric.
...
Necesitarás el punto de conexión SQL de tu almacén para que Braze pueda conectarse. Para recuperarlo, ve al **espacio de trabajo** en Fabric, y en la lista de elementos, pasa el ratón por encima del nombre del almacén y selecciona **Copiar cadena de conexión SQL**.
...
En el dashboard de Braze, ve a **Configuración de datos** > **Ingesta de datos en la nube**, selecciona **Crear nueva sincronización de datos** y, a continuación, selecciona **Importación de Snowflake**.
...
{% alert note %}
En el campo **Localizador de cuenta de Snowflake**, introduce tu [identificador de cuenta](https://docs.snowflake.com/en/user-guide/admin-account-identifier) de Snowflake, que suele seguir un formato como `xy12345.us-east-1.aws`. No es lo mismo que el nombre de una base de datos o el nombre de un almacén.
{% endalert %}
...
En el dashboard de Braze, ve a **Configuración de datos** > **Ingesta de datos en la nube**, selecciona **Crear nueva sincronización de datos** y, a continuación, selecciona **Importación de Amazon Redshift**.
...
{% alert note %}
En el dashboard de Braze, el campo **Nombre de la base de datos** solo acepta letras (A–Z, a–z), números (0–9) y guiones bajos (_), aunque Amazon Redshift admite caracteres adicionales en los identificadores de bases de datos.
{% endalert %}
...
En el dashboard de Braze, ve a **Configuración de datos** > **Ingesta de datos en la nube**, selecciona **Crear nueva sincronización de datos** y, a continuación, selecciona **Importación de Google BigQuery**.
...
En el dashboard de Braze, ve a **Configuración de datos** > **Ingesta de datos en la nube**, selecciona **Crear nueva sincronización de datos** y, a continuación, selecciona **Importación de Databricks**.
...
Crearás una nueva sincronización de datos para Microsoft Fabric. En el dashboard de Braze, ve a **Configuración de datos** > **Ingesta de datos en la nube**, selecciona **Crear nueva sincronización de datos** y, a continuación, selecciona **Importación de Microsoft Fabric**.
...
- El nombre de credenciales es una etiqueta para estas credenciales en Braze; puedes establecer un valor descriptivo aquí
...
Vuelve al dashboard de Braze y selecciona **Probar conexión**. Si tiene éxito, verás una vista previa de los datos. Si por alguna razón no podemos conectar, mostraremos un mensaje de error para ayudarte a solucionar el problema.
{% endtab %}
...
{% tabs %}
{% tab Snowflake %}
Una vez activada, la sincronización se ejecutará según la planificación configurada durante la instalación. Si deseas ejecutar la sincronización fuera de la planificación normal de pruebas o recuperar los datos más recientes, selecciona **Sincronizar ahora**. Esta ejecución no afectará a las futuras sincronizaciones programadas regularmente.
