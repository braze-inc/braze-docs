...
article_title: "Cómo empezar: Usuarios anónimos"
page_order: 0
page_type: reference
...
Los usuarios anónimos no tienen `external_ids`, pero puedes asignar perfiles de usuario anónimos con un identificador alternativo: alias de usuario. Esto te permite realizar las mismas acciones sobre un perfil de usuario anónimo que si estuviera identificado por `external_ids`. Por ejemplo, puedes utilizar la API de Braze para registrar eventos y atributos asociados a usuarios anónimos, y dirigirte a esos usuarios en tu mensajería con el filtro de segmentación [ID de usuario externo está en blanco]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters#external-user-id).
...
Supongamos que asignas el atributo personalizado "is_lead_profile" a cada perfil de usuario anónimo. Podrías dirigirte a estos perfiles con uno de estos filtros o con ambos:
