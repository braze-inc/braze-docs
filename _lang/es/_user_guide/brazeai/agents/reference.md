...
> A medida que crees agentes personalizados, consulta este artículo para obtener más información sobre configuraciones clave, como instrucciones y esquemas de salida. Para obtener una introducción, consulta [Agentes de Braze]({{site.baseurl}}/user_guide/brazeai/agents/).

## Modelos

Al configurar un agente, puedes elegir el modelo que utilizará para generar respuestas. Tienes dos opciones: utilizar un modelo basado en Braze o traer tu propia clave de API.

{% alert important %}
El modelo **Auto** con tecnología Braze está optimizado para modelos cuyas capacidades de razonamiento son suficientes para realizar tareas como la búsqueda en catálogos y la pertenencia a segmentos. Si utilizas otros modelos, te recomendamos que realices pruebas para confirmar que tu modelo funciona bien para tu caso de uso. Es posible que tengas que ajustar tus [instrucciones](#writing-instructions) para proporcionar diferentes niveles de detalle o explicaciones paso a paso a modelos con diferentes velocidades y capacidades.
{% endalert %}

### Opción 1: Utiliza un modelo con tecnología Braze.
...
{% alert important %}
Recomendamos probar periódicamente los modelos más recientes, ya que los modelos antiguos pueden dejar de fabricarse o quedar obsoletos al cabo de unos meses.
{% endalert %}

Para configurarlo:

1. Ve a **la integración del socio** > **Socios tecnológicos** y busca tu proveedor.
...
Las instrucciones son las reglas o directrices que tú le das al agente (solicitud del sistema). Definen cómo debe comportarse el agente cada vez que se ejecuta. Las instrucciones del sistema pueden tener un tamaño máximo de 25 KB.

A continuación, se incluyen algunas prácticas recomendadas generales para empezar a utilizar las indicaciones:
...
4. Solicita la estructura («Devuelve JSON/lista con viñetas/tabla...»).
...
8. Prueba, inspecciona y Iterate. Pequeños ajustes pueden suponer grandes mejoras en la calidad.
9. Maneja los casos extremos, añade barreras de protección e instrucciones de rechazo.
10. Mide y realiza la documentación de lo que funciona internamente para reutilizarlo y ampliarlo.

### Utilizar Liquid

Incluir [Liquid]({{site.baseurl}}/user_guide/personalization_and_dynamic_content/liquid) en las instrucciones de tu agente puede añadir un nivel adicional de personalización a su respuesta. Puedes especificar la variable Liquid exacta que obtiene el agente e incluirla en el contexto de tu mensaje. Por ejemplo, en lugar de escribir explícitamente «nombre», puedes utilizar el fragmento de {% raw %}`{{${first_name}}}`{% endraw %}código Liquid:
...
Para obtener más información sobre las prácticas recomendadas para las indicaciones, consulta las guías de los siguientes proveedores de modelos:
...
Elige catálogos específicos para que un agente los consulte y proporciónale el contexto necesario para que comprenda tus productos y otros datos de usuario cuando sea pertinente. Los agentes utilizan herramientas para encontrar solo los elementos relevantes y los envían al LLM para minimizar el uso de tokens.

![El catálogo «restaurantes» y"Loyalty_Program"la columna seleccionada para que el agente realice la búsqueda.]({% image_buster /assets/img/ai_agent/search_catalog.png %}){: style="max-width:75%;"}

## Contexto de pertenencia al segmento de segmentación

Puedes seleccionar hasta cinco segmentos para que el agente compare la pertenencia a segmentos de cada usuario cuando se utiliza el agente en un Canvas. Supongamos que tu agente tiene seleccionada la pertenencia al segmento «Usuarios de fidelización» y que el agente se utiliza en un Canvas. Cuando los usuarios entran en un paso de agente, este puede cruzar referencias para comprobar si cada usuario es miembro de cada segmento que hayas especificado en la consola del agente y utilizar la pertenencia (o no pertenencia) de cada usuario como contexto para el LLM.

![El segmento «Usuarios fieles» seleccionado para acceder a la membresía de agente.]({% image_buster /assets/img/ai_agent/segment_membership_context.png %}){: style="max-width:75%;"}
...
![Página del administrador de agentes con agentes archivados.]({% image_buster /assets/img/ai_agent/archived_agents.png %})
