...
page_order: 10
search_rank: 1
description: "Descubre todo lo que necesitas saber sobre las herramientas de recomendaciones de artículos en Braze."
---

# Recomendaciones

> Mejora tus recomendaciones con Braze creando una herramienta de recomendaciones que pueda sugerir a tus usuarios los artículos y contenidos que realmente desean. Desde personalizar experiencias con IA hasta construir tus propios motores con Liquid o Contenido conectado, encontrarás todo lo que necesitas para que cada recomendación cuente.

## Requisitos previos

Antes de poder crear o utilizar recomendaciones de artículos en Braze, deberás [crear al menos un catálogo]({{site.baseurl}}/user_guide/data/activation/catalogs/create/); solo se recomendarán a los usuarios los artículos de ese catálogo.

## Tipos y casos de uso

### Personalización de IA {#ai}

Como parte de la característica [de recomendaciones de artículos mediante IA]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/), las recomendaciones personalizadas mediante IA aprovechan el aprendizaje profundo para predecir lo que más te puede interesar en función de lo que te ha interesado en el pasado. Este método proporciona un sistema de recomendación dinámico y a medida que se adapta al comportamiento del usuario.

Las recomendaciones personalizadas de IA utilizan los datos de los últimos 6 meses de interacción con los artículos, como compras o eventos personalizados, para construir el modelo de recomendación. Para los usuarios que no disponen de datos suficientes para crear una lista personalizada, los artículos más populares sirven como alternativa, de modo que los usuarios siguen recibiendo sugerencias relevantes.
...
Según los datos de interacción que se realizan, los casos de uso de este modelo podrían incluir:
...
- Un método para realizar el seguimiento de las compras, ya sea un objeto de compra o un evento personalizado.
{% enddetails %}

{% details Setting it up %}
1. Crea una [recomendación de artículo de IA]({{site.baseurl}}/user_guide/brazeai/item_recommendations/creating_recommendations/ai/).
...
La característica del modelo de recomendación «Más populares» es que muestra los artículos con los que los usuarios tienen más interacciones.
...
La característica del modelo de recomendación «Más reciente» es que muestra los artículos con los que los usuarios tienen más interacción. Utiliza este modelo para reducir el abandono animando a los usuarios que abandonan a volver a interactuar con contenidos relevantes.
...
### Artículo de tendencia {#trending}

La característica del modelo de recomendaciones «Tendencias» es que presenta los artículos que han mostrado un impulso más positivo en las interacciones recientes de los usuarios. Calculamos esto utilizando un análisis ponderado de aproximadamente 10 semanas de historial de eventos, aplicando la mayor ponderación a las últimas 2 semanas aproximadas. Para evitar que las pequeñas fluctuaciones afecten a la calidad de las recomendaciones, aplicamos un umbral de actividad y técnicas de suavizado estadístico.
...
{% alert note %}
Si utilizas selecciones, el campo de clasificación y cualquier límite no se utilizarán con las recomendaciones de artículos de IA. Esto significa que si creas una selección con un campo de clasificación específico y limitas el número de elementos devueltos, esas restricciones no se utilizarán cuando se procesen las recomendaciones de elementos de IA.
{% endalert %}
