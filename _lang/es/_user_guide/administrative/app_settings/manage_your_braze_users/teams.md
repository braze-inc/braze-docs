...
page_type: reference
alias: /teams/
description: "Este artículo de referencia explica cómo utilizar Braze Teams en el dashboard. Aquí puedes aprender a crear equipos, asignar roles y asignar etiquetas y filtros."
...
## ¿En qué se diferencian los equipos de los conjuntos de permisos y los roles?
...
Introduce el **nombre del equipo**. Si lo deseas, utiliza el campo **Definir equipo (opcional)** para seleccionar un atributo personalizado, una ubicación o un idioma para definir con mayor precisión a qué datos de usuario tiene acceso el equipo. Por ejemplo, un posible caso de uso es realizar [pruebas con equipos](#test-with-teams) creando un equipo de desarrollo que solo tenga acceso a usuarios de prueba, identificados por un atributo personalizado. Otro caso de uso es restringir la comunicación con los usuarios en función del producto.

Si un equipo se define por un atributo personalizado, idioma o país, puedes utilizar el equipo para filtrar a los usuarios finales para características como campañas, Canvas, tarjetas de contenido, segmentos y mucho más. Para obtener más información, consulta [Asignar etiquetas de equipo](#tags-and-filters).
...
![Permisos de plantillas de páginas de inicio a nivel de equipo.]({% image_buster /assets/img/teams.png %})

### Permisos disponibles a nivel de equipo

A continuación se enumeran todos los permisos disponibles que puedes asignar a nivel de equipo. Cualquier permiso que no aparezca en esta lista solo se concede a nivel del espacio de trabajo, y estos permisos aparecerán como "--" en la columna de permisos de **Equipos**.
...
- Ver informes del dashboard
- Editar informes del dashboard
- Eliminar informes del dashboard
- Ver PII

{% endtab %}
{% tab Legacy permissions %}

- Acceder a campañas, Canvas, tarjetas, bloques de contenido, conmutadores de características, segmentos, biblioteca de medios y centros de preferencias
...
- Ver plantillas de Canvas
- Archivar plantillas de Canvas
- Editar plantillas de páginas de inicio
- Ver plantillas de páginas de inicio
- Archivar plantillas de páginas de inicio
...
Puedes asignar un equipo a Canvas, campañas, tarjetas de contenido, segmentos, plantillas de correo electrónico, plantillas de webhook, bloques de contenido y activos de la biblioteca de medios con el filtro **Añadir equipo**.
...
- En función de los *permisos* asignados, los miembros del equipo solo podrán acceder a las herramientas de interacción del dashboard que tengan configurado el filtro de su equipo. Si tienen permisos limitados o nulos en el espacio de trabajo, deben añadir un filtro de equipo a determinados objetos antes de poder guardarlos o lanzarlos. Los miembros del equipo también pueden filtrar Canvas, campañas, tarjetas de contenido y segmentos por equipo para identificar el contenido relevante para ellos.
...
En este escenario, Michelle es una usuaria con permisos limitados que no tiene permisos a nivel del espacio de trabajo. Sus permisos son más o menos así:
...
Según los permisos asignados a Michelle, cada vez que crea una campaña, solo puede asignar el equipo «Desarrollo» a esa campaña. No puede lanzar la campaña a menos que se asigne el equipo, y no puede ver ni acceder a ninguna otra etiqueta de equipo.
...
En este escenario, Michelle sigue siendo miembro del equipo de desarrollo, pero también tiene un permiso adicional a nivel del espacio de trabajo.

![Permisos personalizados con un permiso a nivel del espacio de trabajo y 15 permisos basados en equipos.]({% image_buster /assets/img_archive/scenario2.png %})

Dado que Michelle tiene el permiso a nivel del espacio de trabajo de «Acceder a campañas, Canvas, tarjetas, bloques de contenido, conmutadores de características, segmentos, biblioteca de medios y centros de preferencias», puede ver y asignar otros filtros de equipo a la campaña que crea.
...
Al igual que en el primer escenario, Michelle debe añadir la etiqueta del equipo «Desarrollo» a la campaña antes de poder lanzarla.
...
Un posible caso de uso de los equipos es crear un sistema de aprobación basado en equipos para probar y lanzar contenido en un entorno de producción.

Para ello, crea un equipo de «Desarrollo» que solo tenga acceso a los usuarios de prueba. Puedes limitar un equipo para que solo acceda a usuarios de prueba si estos son identificables mediante un atributo personalizado. A continuación, añade el atributo personalizado como definición al crear o editar el equipo (consulta la sección anterior [Crear equipos](#creating-Teams)). Tus aprobadores deben tener acceso a todos los usuarios.

El proceso general sería el siguiente:

1. El equipo de desarrollo crea una campaña y añade la etiqueta del equipo «Desarrollo».
...
1. El equipo de desarrollo clona la campaña en curso, añade la etiqueta del equipo «Desarrollo» y guarda los cambios.
2. El equipo de desarrollo realiza modificaciones y las comparte con el equipo de aprobación.
3. El equipo de aprobación elimina la etiqueta del equipo «Desarrollo», pausa la campaña anterior y lanza la nueva campaña.
