...
![Modal para seleccionar el registro de SMS o WhatsApp como plantilla al crear un mensaje dentro de la aplicación.]({% image_buster /assets/img_archive/dnd_iam_phone_capture_template.png %}){: style="max-width:80%"}
...
Si sus usuarios introducen un número de teléfono que incluye caracteres especiales no aceptados, verán un indicador de error genérico no personalizable y no podrán enviar el formulario. Puedes ver el comportamiento del error en la pestaña de **prueba de la vista previa & ** y en tu dispositivo de prueba. Consulte este artículo para saber [cómo formatea Braze los números de teléfono]({{site.baseurl}}/user_guide/message_building_by_channel/sms/phone_numbers/user_phone_numbers/#importing-phone-numbers).
