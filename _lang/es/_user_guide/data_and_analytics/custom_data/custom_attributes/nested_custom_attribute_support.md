...
Los atributos personalizados anidados permiten definir un conjunto de atributos como propiedad de otro atributo. En otras palabras, cuando se define un objeto de atributo personalizado, se puede definir un conjunto de atributos adicionales para ese objeto.

Por ejemplo, supongamos que desea definir un atributo personalizado en el perfil de usuario denominado `favorite_book`. Este atributo personalizado puede definirse como un objeto que tiene los atributos anidados `title`, `author`, y `publishing_date`, de la siguiente manera:
...
Los objetos de atributo personalizados pueden contener [tipos de datos][1], como:
...
- Los objetos tienen un tamaño máximo de 50 KB.
...
- No todos los socios Braze admiten atributos personalizados anidados. Consulta la [documentación del socio]({{site.baseurl}}/partners/home) para confirmar si la integración admite esta característica.
...
{% alert note %}
Si inicialmente no ha capturado las fechas como propiedades del objeto, le recomendamos que reenvíe estos datos utilizando la clave `$time` para todos los usuarios. De lo contrario, podrían aparecer segmentos incompletos al utilizar el atributo `$time`.
{% endalert %}
...
Tenga en cuenta que para los atributos personalizados anidados, si el año es menor que 0 o mayor que 3000, Braze no almacena estos valores en el usuario.
...
![Uso de Liquid para incluir en un mensaje el nombre de una canción y el número de veces que un oyente la ha reproducido][5]
...
Después de añadir una ruta a su propiedad, haga clic en **Validar** para comprobar que el valor del campo de ruta es válido.

![Filtrado basado en un atributo personalizado de canción más reproducida cuando un oyente ha reproducido una canción un número determinado de veces][6]
...
Cuando trabaje con la segmentación de atributos personalizados anidados, tendrá acceso a un nuevo comparador agrupado por tipo de datos. Por ejemplo, como `play_analytics.count` es un número, puede seleccionar un comparador en la categoría **Número**.

![Un usuario que elige un operador basado en el tipo de datos del atributo personalizado anidado][7]
...
![Un segmento de ejemplo con la casilla seleccionada para la Segmentación Multicriterio.][14]
...
Desde aquí, puedes utilizar `segmented_nested_objects` y personalizar tu mensaje. En nuestro caso de uso, queremos tomar un objetivo de la primera cuenta imponible activa y personalizarlo:
...
En el panel Braze, vaya a **Configuración de datos** > **Atributos personalizados**.

{% alert note %}
Si utilizas la [navegación antigua]({{site.baseurl}}/navigation), puedes encontrar **Atributos personalizados** en **Gestionar configuración**.
{% endalert %}

Busca tu objeto o matriz de objetos. En la columna **Nombre de atributo**, haga clic en **Generar esquema**.
...
Haz clic en el botón más de <i class="fas fa-plus"></i> en el campo de la ruta. Esto mostrará una representación de su objeto o matriz de objetos. Puedes seleccionar cualquiera de los elementos de la lista y Braze los insertará en el campo de ruta por ti. Para nuestro caso de uso, necesitamos conseguir el equilibrio. Selecciona la balanza y la ruta (en este caso, `[].balance`) se rellenará automáticamente en el campo ruta.

![][12]{: style="max-width:70%" }

Puede hacer clic en **Validar** para comprobar que el contenido del campo de ruta es válido y, a continuación, crear el resto del filtro según sea necesario. Aquí hemos especificado que el saldo debe ser inferior a 100.
...
Puede disparar cuando cambia un objeto de atributo personalizado anidado. Esta opción no está disponible para los cambios en las matrices de objetos. Si no ve una opción para ver el explorador de rutas, compruebe que ha generado un esquema. 
...
3. En la columna **Nombre** del atributo, haga clic en <i class="fas fa-plus"></i> para gestionar el esquema.
4. Aparecerá un modal. Haga clic en **Regenerar esquema**.
...
{% alert important %}
Si desea restablecer el esquema de una matriz de objetos con un objeto existente, deberá crear un nuevo atributo personalizado. La regeneración del esquema no elimina los objetos existentes.
{% endalert %}

Si los datos no aparecen como se esperaba tras regenerar el esquema, es posible que el atributo no se ingiera con la frecuencia suficiente. Los datos del usuario se muestrean en los datos anteriores enviados a Braze para el atributo anidado dado. Si el atributo no se ingiere lo suficiente, no se recogerá para el esquema.
