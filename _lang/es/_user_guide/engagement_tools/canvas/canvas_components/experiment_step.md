...
## Creación de una ruta de experimentos
...
{% alert important %}
Para evitar la contaminación del experimento, si tu Canvas tiene un experimento de ruta ganadora o ruta personalizada activo o en curso y actualizas el Canvas activo, independientemente de si actualizas el paso de la ruta de experimentos en sí, el experimento en curso finalizará y el paso del experimento no determinará una ruta ganadora ni rutas personalizadas. Para reiniciar el experimento, puedes desconectar la ruta de experimentos existente e iniciar una nueva, o duplicar el Canvas y crear uno nuevo. De lo contrario, los usuarios seguirán la ruta de experimentos como si no se hubiera seleccionado ningún método de optimización. Tampoco puede activar Rutas Personalizadas o Rutas Ganadoras para un Lienzo ya activo con un paso de Ruta de Experimento.<br><br>Para más información, consulta [Editar Canvas después del lanzamiento]({{site.baseurl}}/post-launch_edits/).
{% endalert %}

## Seguimiento del rendimiento

En la página **Canvas Analytics**, selecciona la ruta de experimentos para abrir una [tabla detallada]({{site.baseurl}}/user_guide/engagement_tools/canvas/get_started/measuring_and_testing_with_canvas_analytics/#performance-breakdown-by-variant) idéntica a la pestaña **Analizar variantes** y comparar estadísticas detalladas de rendimiento y conversión entre las distintas rutas. También puede exportar la tabla mediante CSV y comparar los cambios porcentuales de las métricas de interés en relación con la ruta o el control que seleccione.

Cada paso de cada ruta muestra estadísticas en la vista [de análisis de Canvas]({{site.baseurl}}/user_guide/engagement_tools/canvas/testing_canvases/measuring_and_testing_with_canvas_analytics/), al igual que cualquier paso en Canvas. Sin embargo, ten en cuenta que el análisis de pasos individuales y el análisis de rutas de experimentos miden las conversiones de forma diferente:

- **El análisis de la ruta de experimentos** realiza el seguimiento de las conversiones desde el momento en que el usuario entra en el paso de la ruta de experimentos. Esta es la vista recomendada para comparar el rendimiento entre rutas, ya que todas las rutas comparten el mismo punto de partida.
- **Los análisis de pasos individuales** (como los análisis de pasos de mensajes) realizan el seguimiento de las conversiones desde el momento en que el usuario recibe ese paso específico (por ejemplo, cuando se envía el mensaje).

Dado que estas ventanas de conversión tienen diferentes puntos de partida, pueden mostrar diferentes tasas de conversión para la misma ruta, especialmente cuando hay retrasos entre el paso del experimento y un mensaje posterior. Para obtener la comparación más fiable entre rutas, utiliza el análisis de rutas de experimentos.
...
Las rutas de experimentos registran a los usuarios que entran en cada paso y se convierten mientras están en la ruta asignada. Esto realiza el seguimiento de todos los eventos de conversión especificados en la configuración de Canvas. En la pestaña **de configuración adicional**, introduce el número de días (entre 1 y 30) durante los que deseas que este experimento realice el seguimiento de las conversiones. El intervalo de tiempo que especifiques aquí determina durante cuánto tiempo se realizarán el seguimiento de los eventos de conversión (seleccionados en la configuración de Canvas) para el experimento. Las ventanas de conversión por evento especificadas en la configuración de Canvas no se aplican al seguimiento de este paso y se sustituyen por esta ventana de conversión.

La ventana de conversión se abre cuando el usuario entra en el paso «Ruta de experimentos», no cuando se envía un mensaje posterior. Si una ruta incluye retrasos, como un paso de retraso o [Intelligent Timing]({{site.baseurl}}/user_guide/brazeai/intelligence/intelligent_timing/), esos retrasos consumen parte de la ventana de conversión.

{% alert important %}
Si utilizas Intelligent Timing en un paso de mensaje dentro de una ruta de experimentos, el tiempo transcurrido entre la entrada en el experimento y el envío real del mensaje reduce la ventana de conversión efectiva para esa ruta de experimentos. Por ejemplo, si tu experimento tiene un periodo de conversión de 5 días y Intelligent Timing retrasa el mensaje 2 días, los usuarios de esa ruta solo tendrán 3 días después de recibir el mensaje para realizar la conversión dentro del periodo del experimento, aunque los propios análisis del paso Mensaje realicen un seguimiento de las conversiones desde el momento en que se envía el mensaje.<br><br>Para obtener análisis de experimentos más claros, coloca cualquier retraso (como los pasos de retraso) **antes** del paso de ruta de experimentos, en lugar de dentro de una ruta de experimentos. De esta manera, todas las rutas comienzan desde el mismo punto y los retrasos no consumen nada de la ventana de conversión.
{% endalert %}