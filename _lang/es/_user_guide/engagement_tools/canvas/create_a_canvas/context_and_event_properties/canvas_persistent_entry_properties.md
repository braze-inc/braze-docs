...
{% alert important %}
Las propiedades de entrada persistentes son un artefacto del editor Canvas original, por lo que hay referencias obsoletas a términos como «propiedades de entrada de Canvas» que se mantienen como referencia histórica. Para obtener información sobre la versión actualizada del editor Canvas, consulta [Propiedades]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties) [de contexto y propiedades del evento]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/context_and_event_properties).
{% endalert %}
...
En esta solicitud, el valor global de "alergias alimentarias" es "ninguna". PorqueCustomer_123,el valor es «lácteos». Los mensajes de este Canvas que contengan el fragmento de {%raw%}`{{canvas_entry_properties.${food_allergies}}}`{%endraw%}código Liquid utilizarán la plantilla «dairy» paraCustomer_123  y «none» para todos los demás. 
