...
- **Identificador de usuario:** Cada perfil de usuario se identifica unívocamente mediante un identificador de usuario, denominado `external_id`. Este identificador permite a Braze rastrear y asociar los datos de los usuarios a través de diferentes canales y dispositivos, proporcionando una visión unificada de las interacciones de cada usuario con su marca. [Los perfiles de usuario anónimos]({{site.baseurl}}/user_guide/getting_started/users_segments/anonymous_users/) (usuarios que visitan tu sitio web o aplicación sin iniciar sesión) no tienen un `external_id`, pero se les pueden asignar [alias de usuario]({{site.baseurl}}/user_guide/getting_started/users_segments/anonymous_users/#assigning-user-aliases) como identificador alternativo.
