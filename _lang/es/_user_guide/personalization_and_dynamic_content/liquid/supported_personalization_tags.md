...
{% raw %}
- `dispatch_id` El comportamiento difiere porque Braze trata los pasos en Canvas como eventos desencadenados, incluso cuando están "programados" (excepto los pasos de entrada, que pueden programarse). 
...
| `{{most_recently_used_device.${idfa}}}` |  Para dispositivos que no sean iOS, este valor será nulo. |
...
| `{{targeted_device.${idfa}}}` |  Para dispositivos que no sean iOS, este valor será nulo. |
...
### Utilizar la lógica condicional en lugar de un valor predeterminado
 ...
 La lógica condicional permite enviar mensajes que difieren en función del valor de un atributo personalizado.  
...
En este caso de uso, un usuario con un nombre en blanco o nulo recibirá el mensaje "Gracias por descargar". 
...
Después de crear una variable, puede hacer referencia a esa variable en su lógica de mensajería o mensaje.  
