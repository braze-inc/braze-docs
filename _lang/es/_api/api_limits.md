...
Las siguientes solicitudes tienen un límite de velocidad de 250 000 solicitudes por hora, compartidas entre ellas.
...
- [`/campaigns/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns/) (solo para llamadas no difundidas, aquellas que especifican`external_user_ids`  o `aliases`)
...
- [`/canvas/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/) (solo para llamadas no transmitidas)
...
- [`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/) (solo para llamadas no transmitidas)
...
Esto se aplica a los tres puntos finales siguientes: [`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/), y[`/campaigns/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_campaigns/) [`/canvas/trigger/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_triggered_canvases/).
...
- La campaña o el Canvas que se desencadena (el`campaign_id`  o`canvas_id`  en tu solicitud de API, si se especifica).
- La audiencia a la que te diriges (los segmentos o filtros, o para las campañas API, el`segment_id`  en tu solicitud API).
- Los filtros de audiencia conectados (el`audience`objeto en tu solicitud de API, si se especifica)
...
### Solicitudes por lotes para el punto final de usuarios de Track {#batch-user-track}

Cada solicitud `/users/track` puede contener hasta 75 objetos de evento, 75 objetos de atributo y 75 objetos de compra. Cada objeto (evento, atributo y matrices de compra) puede actualizar un usuario cada uno. En total, esto significa que se puede actualizar hasta 225 usuarios en una sola llamada. Además, un mismo perfil de usuario puede ser actualizado por varios objetos.
...
{% alert tip %}
Puedes utilizar el [panel de control de uso de la API]({{site.baseurl}}/user_guide/analytics/dashboard/api_usage_dashboard/) para ver y comparar el tráfico entrante con tus límites de velocidad.
{% endalert %}
...
En condiciones normales, el tiempo que tarda en producirse la consistencia eventual de nuestros datos es de 10 a 100 ms (1/10 de segundo). Sin embargo, puede haber casos en los que esa coherencia tarde más tiempo en producirse, por lo que te recomendamos que dejes pasar 5 minutos entre una llamada y otra para minimizar la probabilidad de error.
