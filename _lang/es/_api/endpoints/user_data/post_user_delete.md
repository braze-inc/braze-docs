...
Se pueden incluir hasta 50 `external_ids`, `user_aliases`, o `braze_ids` en una sola solicitud. En una misma solicitud solo puede incluirse una de las siguientes opciones: `external_ids`, `user_aliases`, o `braze_ids`.

{% alert warning %}
La eliminación de perfiles de usuario no se puede deshacer. Eliminará permanentemente a los usuarios que puedan causar discrepancias en tus datos. Obtén más información sobre lo que ocurre cuando [eliminas un perfil de usuario a través de la API]({{site.baseurl}}/help/help_articles/api/delete_user/) en nuestra documentación de Ayuda.
{% endalert %}
