---
nav_title: "GET: Consulta de números de teléfono no válidos"
...
| `phone_numbers` | Opcional <br>(ver nota) | Matriz de cadenas en formato e.164  | Si se facilita, devolveremos el número de teléfono si se ha comprobado que no es válido. |
...
{% alert note %}
Debe proporcionar un `start_date` y un `end_date` O `phone_numbers`. Si proporciona los tres, `start_date`, `end_date`, y `phone_numbers`, damos prioridad a los números de teléfono indicados y no tenemos en cuenta el intervalo de fechas.
{% endalert %}
