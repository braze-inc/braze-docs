---
nav_title: "GET: Exportar análisis de envío"
article_title: "GET: Exportar análisis de envío"
search_tag: Endpoint
page_order: 4
layout: api_page
page_type: reference
description: "En este artículo se describen los detalles del punto final Exportar análisis de envíos de Braze."
...
> Utiliza este punto final para recuperar una serie diaria de diversas estadísticas de una campaña de seguimiento de la API `send_id`.

Braze almacena los análisis de los envíos durante 14 días después del envío. Las conversiones de la campaña se atribuirán al `send_id` más reciente que un usuario determinado haya recibido de la campaña.
...
Este punto final es solo para campañas de API. Para utilizar este punto final, necesitarás una [clave de API]({{site.baseurl}}/api/basics#rest-api-key/) con el permiso `sends.data_series`.
...
| --------- | -------- | --------- |------------ |
| `campaign_id` | Obligatoria | Cadena | Ver [identificador API de campaña]({{site.baseurl}}/api/identifier_types/). |
| `send_id` | Obligatoria | Cadena | Ver [Enviar identificador API]({{site.baseurl}}/api/identifier_types/). |
