...
La respuesta te proporciona un `schedule_id` que debes guardar por si más adelante necesitas cancelar o actualizar el mensaje que programaste:
...
Recibes un `schedule_id` para el mensaje programado que has creado.
...
Las respuestas del punto final de programación de mensajes incluyen la dirección `dispatch_id` del mensaje para que sirva de referencia al envío del mensaje. El `dispatch_id` es el ID del envío del mensaje (ID único para cada "transmisión" enviada desde Braze).