...
![El sitio de la documentación de Braze muestra la interfaz japonesa]({% image_buster /assets/img/braze-docs-japan.png %}){: style="max-width:70%;"}
...
Hemos añadido compatibilidad con [las propiedades WhatsApp y SMS Liquid en el flujo Canvas]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/canvas_entry_properties_event_properties). Ahora, cuando un paso en Canvas contiene un desencadenante "Enviado un mensaje SMS entrante" o "Enviado un mensaje WhatsApp entrante", los pasos en Canvas posteriores pueden incluir una propiedad SMS o WhatsApp Liquid. Esto refleja cómo funcionan las propiedades del evento en Canvas. De este modo, puede aprovechar sus mensajes para guardar y consultar datos de origen sobre perfiles de usuario y mensajería conversacional.
