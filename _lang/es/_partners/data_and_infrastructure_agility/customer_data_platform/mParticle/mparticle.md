...
- [Envía la interacción del usuario Braze a mParticle a través de Currents]({{site.baseurl}}/partners/data_and_infrastructure_agility/customer_data_platform/mParticle/mparticle_for_currents/), haciéndola procesable en todo el stack de crecimiento. 
...
| Instancia de soldadura | Puede encontrar su instancia Braze en la [página de descripción general de la API]({{site.baseurl}}/api/basics/#endpoints). (Por ejemplo: US-01, US-02, etc.) |
...
![mMembresía del segmento de partículas][6]
...
![Atributo personalizado mParticle][7]
...
- **Atributo de matriz única:** Seleccione `SegmentMembershipArray` como filtro. A continuación, utiliza la opción "incluye valor" e introduce el ID de audiencia que desees. ![Filtro de segmento mParticle "SegmentMembershipArray" configurado como "incluye valor" e ID de audiencia.][11]<br><br>
- **Un atributo por segmento:** Seleccione su atributo personalizado como filtro. A continuación, utilice la opción "equals" y elija la lógica adecuada. ![mParticle segment filter "in possible parisians" set as "equals" and "true".][8]
...
| Activar el reenvío de flujos de eventos | (Servidor a servidor) Si se activa, todos los eventos se reenviarán en tiempo real. Si no, todos los eventos se reenviarán en bloque. Cuando decida activar el reenvío de flujos de eventos, asegúrese de que los datos que pasa a Braze respetan [los límites de velocidad]({{site.baseurl}}/api/basics/#api-limits). |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }
...
- [Las propiedades de eventos personalizados]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_events/) admiten objetos de cadena, numéricos, booleanos o de fecha. No admite matrices ni objetos anidados.
- [Los atributos personalizados]({{site.baseurl}}/user_guide/data_and_analytics/custom_data/custom_attributes/) admiten objetos de cadena, numéricos, booleanos, de fecha y matrices, pero no admiten objetos ni objetos anidados. 
