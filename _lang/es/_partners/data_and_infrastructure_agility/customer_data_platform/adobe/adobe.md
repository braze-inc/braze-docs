...
1. En el paso de mapeado, selecciona **Añadir nuevo mapeado**.<br>![][5]{: style="max-width:50%;"}<br><br>
2. En la sección de campo vacío, selecciona el botón de flecha situado junto al campo vacío para abrir la ventana de selección de campo vacío.<br>![][6]<br><br>
3. En la ventana, selecciona los atributos de Adobe para mapearlos con tus atributos Braze. <br>![][7]{: style="max-width:70%;"}<br><br>A continuación, selecciona el espacio de nombres de identidad. Esta opción se utiliza para asignar un espacio de nombres de identidad de plataforma a un espacio de nombres Braze.<br>![][8]{: style="max-width:80%;"}<br> Elige tus campos fuente y selecciona **Seleccionar**.<br><br>
4. En la sección del campo de destino, selecciona el icono de mapeado situado junto al campo.<br>![][9]{: style="max-width:90%;"} <br><br>
5. En la ventana de selección de campos de destino, puede elegir entre tres categorías de campos de destino:<br><br>- **Selecciona el espacio de nombres de identidad**: Utilice esta opción para asignar espacios de nombres de identidad de Platform a espacios de nombres de identidad de Braze.<br>- **Selecciona atributos personalizados**: Utilice esta opción para asignar atributos Adobe XDM a atributos Braze personalizados que haya definido en su cuenta Braze. <br><br>![][10]{: style="max-width:60%;"}<br><br>**También puede utilizar esta opción para renombrar atributos XDM existentes en Braze.** Por ejemplo, al mapear un atributo `lastname` XDM a un atributo personalizado `Last_Name` en Braze, se creará el atributo `Last_Name` en Braze si aún no existe, y se mapeará el atributo `lastname` XDM a él. <br><br> Elige los campos de destino y selecciona **Seleccionar**.<br><br>
6. Tu campo mapeado debería aparecer en la lista.<br>![][11]<br><br>
...
![Asignaciones de destino: IdentityMap:IDFA asignado a IdentityMap:external_id, IdentityMap:GAID asignado a IdentityMap:external_id, IdentityMap:Email asignado a IdentityMap:external_id, xdm:mobilePhone.number asignado a CustomAttribute:PhoneNumber, xdm:person.name.lastName asignado a CustomAtrribute:LastName, xdm:person.name.firstName asignado a CustomAttribute:FirstName][12]
