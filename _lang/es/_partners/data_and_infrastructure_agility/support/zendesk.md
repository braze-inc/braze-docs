...
| Token de la API de Zendesk | Se necesita un [token de la API de][2] Zendesk para enviar solicitudes desde Braze al punto final de tickets de Zendesk. |
...
Otros casos de uso pueden gestionarse a través de [API de soporte de Zendesk][4], que cambiaría en consecuencia el punto final `/api/v2/` al final de la URL del webhook.
...
Defina los detalles del ticket como tipo, asunto y estado en la carga útil de su webhook. Los detalles de los tickets se pueden ampliar y personalizar en función de la [API de tickets de Zendesk][6]. Utilice el siguiente ejemplo como ayuda para estructurar su carga útil e introduzca los campos que desee.
...
3. Seleccione **Activación** o **Automatización** y haga clic en **Siguiente**.<br>![][9]{: style="max-width:70%;"}<br><br>
...
- Seleccione el método de autenticación de token de portador para el webhook y proporcione su [clave de API Braze](https://www.braze.com/docs/api/basics/#creating-and-managing-rest-api-keys).
  - Asegúrate de que la clave de API que utilizas tiene los [permisos correctos](https://www.braze.com/docs/api/basics/#rest-api-key-permissions) para el punto final Braze que utiliza tu webhook.<br><br>
...
4. Selecciona **Añadir condición** para configurar qué condiciones deben desencadenar el webhook. Por ejemplo, "Categoría de estado cambiada a cerrado" o "Categoría de estado cambiada a resuelto".![][8]{: style="max-width:70%;"}<br><br>
...
6. Defina el cuerpo JSON para que se ajuste a su punto final Braze, usando marcadores de posición variables de Zendesk para rellenar dinámicamente los campos relevantes.<br>![][10]{: style="max-width:70%;"}<br><br>
