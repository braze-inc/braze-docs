...
|---|---|
| Cuenta y aplicación RevenueCat | Se necesita una cuenta [RevenueCat][9] para beneficiarse de esta asociación. También debe tener una aplicación RevenueCat configurada. |
| SDK RevenueCat | Además del SDK de Braze necesario, recomendamos instalar el [SDK de RevenueCat][8] para proporcionar alias de usuario a RevenueCat. |
| Instancia de soldadura | Puedes obtener tu instancia de Braze a través de tu administrador de incorporación a Braze o en la [página de resumen de la API]({{site.baseurl}}/api/basics/#endpoints).<br><br>RevenueCat requiere que la instancia de Braze envíe desde el servidor al punto final REST Braze correcto. |
...
| Clave | Descripción |
|---|---|
| `$brazeAliasName` | El Braze `alias_name` en el [objeto alias de usuario][2] |
| `$brazeAliasLabel` | El Braze `alias_label` en el [objeto alias de usuario][2] |
{: .reset-td-br-1 .reset-td-br-2 role="presentation" }

Ambos atributos son necesarios para que el [objeto alias de usuario][2] se envíe junto con los datos del evento. Estas propiedades pueden establecerse manualmente, como cualquier otro atributo de suscriptor [RevenueCat][4]. En el primer paso se muestran fragmentos de código de ejemplo.
...
![Configuración de Braze en RevenueCat con campos para la instancia de Braze, el identificador de la clave de API y el identificador del sandbox.][3]
...
Si sólo proporcionas una clave de API REST de Braze a RevenueCat, sólo se enviarán los eventos de producción. Si también quieres enviar eventos de prueba de sandbox, [crea otra clave de API REST de Braze][11] y añádela a tu configuración de Braze en RevenueCat.
