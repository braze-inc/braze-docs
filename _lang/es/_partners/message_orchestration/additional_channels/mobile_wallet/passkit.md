...
| Punto final REST Braze  | La URL de su punto final REST. Tu punto final dependerá de la [URL de Braze para tu instancia][6]. |
{: .reset-td-br-1 .reset-td-br-2 .reset-td-br-3 role="presentation" }

## Integración

Para enriquecer aún más la experiencia de tus clientes con el monedero móvil, desde tu panel PassKit puedes optar por pasar los datos a Braze a través del punto final [`/users/track` de Braze][7]. 
...
1. Dentro de **Ajustes**, en **Integraciones** en su proyecto o programa PassKit pass haga clic en **Conectar** en la pestaña **Braze**.<br>![El azulejo de integración Braze en la plataforma PassKit.][5]{: style="max-width:80%"}<br><br>
...
Para más información sobre la creación de enlaces SmartPass encriptados, consulta este [artículo de PassKit][8].
...
Para obtener una lista completa de los campos disponibles, sus tipos y descripciones útiles, echa un vistazo a la [Documentación de PassKit en GitHub][10].
...
{% alert note %}
Si utiliza la [navegación anterior]({{site.baseurl}}/navigation), vaya a **Compromiso** > **Plantillas y medios** > **Biblioteca de bloques de contenido**.
{% endalert %}
...
A continuación, creará su firma de cifrado utilizando un hash [SHA1 HMAC][16] de la URL del proyecto y la carga útil. 
...
![Se muestra una imagen del creador de mensajes del Bloque de Contenido con el JSON capturado y la referencia del Bloque de Contenido.][1]{: style="max-width:70%"}

La URL de salida de la muestra es:
![La URL de salida que incluye una larga cadena de letras y números generada aleatoriamente.][2]{: style="max-width:70%"}

La URL de salida será larga. La razón es que contiene todos los datos de paso e incorpora la mejor seguridad de su clase para garantizar la integridad de los datos y que no se alteren mediante la modificación de la URL. Si utilizas SMS para distribuir esta URL, quizá quieras pasarla por un proceso de acortamiento de enlaces como [bit.ly][3]. Esto puede hacerse mediante una llamada de Contenido conectado a un punto final de bit.ly.
...
- [Proyectos de miembros][12]
- [Proyectos de cupones][13]
- [Proyectos de vuelos][14]
...
Además de crear y actualizar pases, también puedes recuperar los metadatos de los pases de tus usuarios a través del [Contenido conectado][15] de Braze para incorporar detalles personalizados de los pases en tus campañas de mensajería.
