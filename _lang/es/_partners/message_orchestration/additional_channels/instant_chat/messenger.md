...
Cuando esté seguro de que recibe PSID, coordínelo y compártalo con sus desarrolladores para enviar los PSID a Braze como [atributo personalizado]({{site.baseurl}}/user_guide/Data_and_Analytics/Custom_Data/Custom_Attributes/#custom-attributes). Los PSID son cadenas a las que se puede acceder mediante una [llamada a la API](https://developers.facebook.com/docs/messenger-platform/reference/send-api).
...
![Pestaña de prueba en la plantilla del webhook de Facebook Messenger que muestra que puedes obtener una vista previa del mensaje enviándolo a un usuario existente.][60]

Si recibe el mensaje correctamente, puede configurar sus opciones de entrega.

## Mediante esta integración

Una vez configurada, utiliza esta integración para dirigirte a los usuarios de Facebook Messenger. Si no envía mensajes utilizando los números de teléfono de los usuarios y planea enviar mensajes de Messenger repetidamente, debe [crear un segmento][62] para todos los usuarios para los que exista el ID de Messenger como atributo personalizado y activar [seguimiento de análisis][61] para realizar un seguimiento de sus tasas de suscripción a Messenger a lo largo del tiempo. 

![Filtro de segmento "messenger_id" establecido en "no está en blanco".][63]
