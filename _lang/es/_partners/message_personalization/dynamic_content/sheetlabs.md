...
> [Sheetlabs][1] es una plataforma que permite convertir hojas de cálculo en API potentes y bien documentadas. Puedes importar datos de Google Sheets o Excel, convertirlos en una API y luego utilizar esa API en otras aplicaciones, como Braze.

La integración de Sheetlabs y Braze le permite aprovechar [Connected Content][2] para incluir las API de Sheetlabs en sus campañas de marketing Braze. Se suele utilizar para establecer un puente entre una hoja de cálculo de Google (que el equipo de marketing actualiza directamente) y las plantillas de Braze. Esto le permite conseguir más con las plantillas Braze, como traducciones o conjuntos más amplios de atributos personalizados.

## Requisitos previos

| Requisito | Descripción |
| ----------- | ----------- |
| Cuenta Sheetlabs | Para beneficiarse de esta colaboración es necesario disponer de una [cuenta en Sheetlabs][1]. |
...
Consulte [Sheetlabs][3] para obtener más información sobre estos casos de uso.
...
En este punto, deberías poder acceder a tu API con un enlace como:<br> [`https://sheetlabs.com/ACME/email1_translations?country=en`][4].
