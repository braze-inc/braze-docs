...
4. Select the interaction site, the location where the agent is deployed.
5. Choose the [model](#models) your agent will use.
...
You can allow your agent to reference a catalog or segment by selecting **Add context**. This allows your agent to 
...
[Test the agent](#testing-your-agent) output and adjust the instructions as needed. When you’re ready, select **Create Agent** to activate the agent. 
