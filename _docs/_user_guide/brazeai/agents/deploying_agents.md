...
You can refresh and edit the fields in your catalog that use agents. To remove an agent from a column, unselect **Apply AI agent**. This reverts the column and fields to the original values before the agent had applied changes in each cell the last time it ran on the catalog.
