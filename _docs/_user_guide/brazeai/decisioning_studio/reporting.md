...
The performance report shows high-level agent metrics comparing treatment groups (from Braze) to one or more control groups (such as revenue). It supports two views: **Trending** and **Driver Tree**. By default, the report uses the **Trending** view to show how BrazeAI™ performs over time relative to your control groups and to track uplift over time. Note that the metrics are calculated on a daily basis, and KPIs are calculated when you open the performance report in Braze.
