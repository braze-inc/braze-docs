...
{% alert important %}
Do not filter your prediction audience by whether users have already performed the event you're predicting. The prediction audience defines who you want the model to learn from, not the event outcome itself. The model needs to observe both users who have completed the event and users who have not in order to accurately learn and predict the likelihood of future event completion. Focus your filters on user characteristics relevant to your use case, such as active users, new users, high-value users, or users in a specific country.
{% endalert %}
