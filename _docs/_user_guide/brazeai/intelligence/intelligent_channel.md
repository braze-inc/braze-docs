...
Note that a user has a maximum threshold of three messages received for a specific channel before they can have a likelihood score for that channel. Users without sufficient data to measure a likelihood for a channel can be selected using "is blank."
