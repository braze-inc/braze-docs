...
The "Trending" recommendation model features items that had the most positive momentum when it comes to recent user interactions. We calculate this using an exponentially weighted analysis of event history, with a look back of ~10 weeks and the heaviest weighting applied to the most recent weeks (the last ~2 weeks in particular). To prevent small or noisy fluctuations from affecting the recommendation quality, a minimum activity threshold and statistical smoothing are applied.
