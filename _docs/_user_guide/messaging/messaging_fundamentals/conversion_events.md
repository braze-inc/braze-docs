...
| **Places Order**        | A user is counted as having converted when they trigger the [Order Placed eCommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events?tab=ecommerce.order_placed#ecommerce-recommended-events). This tracks any order by default, or you can filter by a specific product.<br><br>The "Places Order" event is currently in early access. Contact your Braze account manager if you’re interested in participating in this early access. |
