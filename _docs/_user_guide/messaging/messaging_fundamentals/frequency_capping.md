...
- The following messages will not be throttled by or counted towards the rate limit:
    - Test sends
    - Seed groups
    - Content Cards configured to create "at first impression" (This will be controlled by the rate of app impressions. Refer to [Card creation]({{site.baseurl}}/user_guide/channels/content_cards/create_a_content_card/#card-creation#differences) for more information on the differences between Card Creation options.)
