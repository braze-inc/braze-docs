...
The Braze web SDK is initialized immediately when a user navigates to the landing page through a link generated by a {% raw %}`{% landing_page_url %}`{% endraw %} Liquid tag in a Braze message.
...
- Schedule [bulk merging]({{site.baseurl}}/user_guide/audience/manage_audience/merge_duplicate_users/#bulk-merging) to periodically merge duplicate profiles based on matching identifiers.
