...
{% alert tip %}
To track link engagement metrics, make sure your link precedes with either HTTP or HTTPS. To turn off click tracking for specific links, refer to [Universal links and App Links]({{site.baseurl}}/user_guide/channels/email/drag_and_drop/universal_links/#turning-off-click-tracking-on-a-link-to-link-basis).
{% endalert %}
