...
]({% image_buster /assets/img/add_new_user_2.png %})ユーザーの詳細のフィールド。{: style="max-width:60%;"}()
...
]({% image_buster /assets/img/add_new_user_3.png %})カスタム権限のフィールドのセクションがある [ワークスペースレベルの権限]。()
...
]({% image_buster /assets/img_archive/suspend_user.png %})ユーザーをサスペンドするオプション。()

管理者は、リストからユーザー名を選択し、フッターの [**ユーザーを一時停止**] をクリックしてユーザーを一時停止することもできます。

]({% image_buster /assets/img_archive/suspend_user2.png %})ユーザー詳細の編集時にユーザーを一時停止する。{: style="max-width:70%;"}()
...
]({% image_buster /assets/img_archive/delete_user_new.png %})ユーザーを削除する()
