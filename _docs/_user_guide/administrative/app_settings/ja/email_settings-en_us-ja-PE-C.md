...
]({% image_buster /assets/img/email_settings/bcc_address.png %})電子メール設定」タブの「BCCアドレス」セクション{: style="max-width:75%;" }()
...
[]({% image_buster /assets/img/bl_icon3.png %})ブレイズ・ラーニング・コース{: style="float:right;width:120px;border:0;" class="noimgborder"}()](https://learning.braze.com/email-open-tracking-pixel/)
...
]({% image_buster /assets/img/open_pixel.png %})SendGrid、SparkPost、または Amazon SES の移動オプションが表示されている [開封追跡ピクセル] セクション。{: style="max-width:80%;" }()
...
]({% image_buster /assets/img_archive/list_unsub_img1.png %})
...
]({% image_buster /assets/img/email_settings/email_unsub_header_workspaces.png %})送信先のユーザーとして [登録済みまたはオプトイン済みのユーザー] を選択する。{: style="float:right;max-width:50%;margin-left:15px;"}()
...
]({% image_buster /assets/img/email_settings/email_unsubscribe_header.png %})配信登録またはオプトインしているユーザーに送信されるメールに、list-unsubscribe ヘッダーを自動的に含めるオプション。()
...
]({% image_buster /assets/img/email_settings/email_unsubscribe_header_custom.png %})

## メールの件名行の追加

テストメールとシードメールの件名行に「[TEST]」と「[SEED]」を含めるには、次のトグルを使用します。これは、テストとして送信されたメールキャンペーンの識別に役立ちます。

]({% image_buster /assets/img/email_settings/test_and_seed_email_subject_line.png %})
...
]({% image_buster /assets/img/email_settings/resubscribe_users.png %})
