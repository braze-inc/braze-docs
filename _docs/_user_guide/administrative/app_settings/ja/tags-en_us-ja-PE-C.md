...
]({% image_buster /assets/img_archive/tags_add_tag.png %})キャンペーン作成時にタグを追加する{: style="max-width:60%;" }()
...
]({% image_buster /assets/img_archive/tags_apply_multiple.gif %})複数のキャンペーンに同時にタグを追加する()
...
]({% image_buster /assets/img_archive/tag_details_page.png %})キャンペーン分析ページに表示されるタグ{: style="max-width:60%;" }()

### タグでフィルターをかける

また、キャンペーン、キャンバス、またはセグメントのリストにも、[**アーカイブ済み**] や [**下書き**] などの追加のステータスラベルのタグとともにタグが表示されます。タグでフィルターをかけるには、タグのリストからタグ名を選択する。

]({% image_buster /assets/img_archive/tags_grid.png %})キャンペーン一覧のタグ。()
...
]({% image_buster /assets/img_archive/tags_view.png %})[設定の管理] ページの [タグ] タブ。()
...
]({% image_buster /assets/img_archive/tag_nested.png %})ネストされたタグを作成する{: style="max-width:70%;" }()
