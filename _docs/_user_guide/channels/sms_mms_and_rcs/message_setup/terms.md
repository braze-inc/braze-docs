...
glossary_top_text: "Check out the following terms to learn more about SMS, MMS, and RCS ecosystems, technologies, and processes."
page_type: glossary
...
glossaries:
  ...
  - name: MMS (Multimedia Message Service)
    description: MMS is used to send messages containing multimedia assets (JPEG, GIF, PNG) to mobile phones. Like SMS, MMS is a high urgency messaging channel that allows you to communicate with customers immediately. MMS extends the capabilities of SMS by giving you the ability to add media to otherwise text-only SMS.
  - name: RCS (Rich Communication Services)
    description: Rich Communication Services (RCS) enhances traditional SMS by enabling brands to deliver messages that are not only informative but also far more engaging. RCS brings features like high-quality media, interactive buttons, and branded sender profiles directly into users' pre-installed messaging apps.
  - name: RCS-Verified Sender
    description: The sending entity of an RCS message, or what the recipient sees on their device to identify where the message is coming from. RCS-verified senders contain a company name, caption, visual branding, and a verified badge. After you provide the necessary RCS sender registration information to Braze, Braze takes care of registration and subscription group setup.
  - name: SMS Fallback
    description: If an RCS message is unable to be delivered (for example, lack of carrier support in the region), Braze will still attempt to deliver the message through SMS when an SMS code exists within the subscription group.
  - name: Basic RCS
    description: Text-only RCS messages up to 160 characters. Billed as a single message. This category is only used in the global model.
  - name: Single RCS
    description: Text-only RCS messages that are over 160 characters or include any rich elements, like buttons or media. This category is only used in the global model.
  - name: Rich RCS
    description: Text-only RCS messages, with or without limited suggestions or buttons. Billed per segment (160 UTF-8 bytes). This category is only used in the United States model.
  - name: Rich Media RCS
    description: RCS messages that include a media file (image, video) or a Rich Card. Billed as a single message, regardless of message length. This category is only used in the United States model.
---
