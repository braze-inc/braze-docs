...
2. キャンバスに「リードスコアリングキャンバス」などの関連する名前を付け、探しやすくするために、「リードの管理」などのタグを付けます。<br><br>]({% image_buster /assets/img/b2b/step_1_simple.png %})キャンバスを作成するステップ 1。「Creating a Lead-Scoring Workflow」という名前と「Lead Management」タグがある。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/b2b/step_2_simple.png %})キャンバスを作成するステップ 2。「アクションベース」のエントリスケジュールと、カスタム属性「lead score」の変更とメールアドレスの追加のアクションベースオプションがある。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/b2b/step_3_simple.png %})キャンバスを作成するステップ 3。セグメントとフィルターを選択して、エントリオーディエンスを絞り込むオプションがある。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/b2b/entry_controls_simple.png %})[ユーザーがこのキャンバスに再エントリできるようにする] が選択され、[指定された期間] が 0 秒の [エントリコントロール] セクション。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/b2b/step_4_simple.png %})キャンバスを作成するステップ 4。メッセージ送信オプションを設定する。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/b2b/action_paths_simple.png %})プラスアイコンで開いたメニューに [アクションパス] が表示されているキャンバス。{: style="max-width:60%;"}()
...
]({% image_buster /assets/img/b2b/action_paths_selected_simple.png %})1 ポイント、5 ポイント、および 10 ポイントを加算するアクショングループ、1 ポイントと 10 ポイントを減算するアクショングループ、および「その他のユーザー」のアクショングループを含むアクションパス。{: style="max-width:20%;"}()
...
]({% image_buster /assets/img/b2b/action_groups_simple.png %})[任意のアプリでセッションを開始] と [カスタムイベントの実行] のトリガーで 1 ポイント加算するアクショングループ。{: style="max-width:80%;"}()

#### ステップ 4d: ユーザアップデートステップの追加

アクションパスの下に作成した各キャンバスパスに、ユーザー更新ステップを追加します。 

]({% image_buster /assets/img/b2b/user_update_paths_simple.png %})アクションパスと、アクショングループごとに分岐したユーザー更新パスが表示されているキャンバス。{: style="max-width:80%;"}()
...
| ヘッダー | コンテンツ |
| --- | --- |
| 許可 | {% raw %}`Bearer {{result.access_token}}`{% endraw %}<br><br>トークンを取得するには、OAuth 2.0 クライアント認証情報フローの[接続アプリ](https://help.salesforce.com/s/articleView?id=sf.connected_app_client_credentials_setup.htm&type=5)を設定してから、コネクテッドコンテンツを使用して営業チームからベアラーを取得します。<br><br>{% raw %}<code>{% connected_content https://[instance].my.salesforce.com/services/oauth2/トークン <br>: メソッドポスト <br> client_id=[client_id]&client_secret=[client_secret]&grant_type=client_credentials本文client_id=[client_id]&client_secret=[client_secret]&grant_type=client_credentials<br>:保存結果%}{% endraw %}<br> ベアラー {% raw %} |
| Content_Type | application/json |
{: .reset-td-br-1 reset-td-br-2}

]({% image_buster /assets/img/b2b/webhook.png %})Salesforce Webhook URL、PATCH HTTP メソッド、生のテキストリクエスト本文、およびリクエストヘッダーを使用して作成されている Webhook。{: style="max-width:80%;"}()
