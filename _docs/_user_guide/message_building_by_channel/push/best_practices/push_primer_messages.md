...
{% alert important %} Using a user-level filter like `Push Subscription Status is not Opted In` will exclude users who are already opted-in on another device, preventing them from receiving the prompt on their new device. {% endalert %}
