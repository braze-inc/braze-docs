...
A "Push Subscription State" in Braze identifies a **user's** global preference regarding push notifications. Because the subscription state is user-based, it is not specific to any individual app or device.
...
|---|---|
|`Subscribed`| Default push subscription state when a user profile is created in Braze. Users have not explicitly opted in but haven't unsubscribed either.|
|`Opted-In`| State when a user explicitly opts in to receive push notifications. Braze will automatically move a user's subscription state to `Opted-In` if a user accepts an OS-level push prompt.<br><br>This does not apply to Android 12 or below users.|
|`Unsubscribed`| State when a user explicitly unsubscribed from push through your application or other methods your brand provides. By default, Braze push campaigns won't target these users.|
...
Before iOS 12, all users must explicitly opt-in to receive push notifications.
