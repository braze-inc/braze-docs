...
**Background push** are available regardless of whether a particular device has opted-in to receive push notifications from that brand. Background push allow brands to send silent push notifications - notifications that intentionally aren't displayed - to devices in order to support key functionalities like uninstall tracking.
