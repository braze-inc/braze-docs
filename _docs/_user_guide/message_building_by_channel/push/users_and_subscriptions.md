...
Braze recommends providing settings within your app to make it simple for users to determine their push notification status. This helps prevent users from going into their device settings and disabling push at the app level, which causes the removal of the foreground push token in Braze completely.
...
### Checking a user's push subscription state
