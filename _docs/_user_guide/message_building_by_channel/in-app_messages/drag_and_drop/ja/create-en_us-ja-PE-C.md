...
]({% image_buster /assets/img_archive/dnd_iam_select_template.png %})「基本」、「背景画像」、「電話番号キャプチャ」、「空白」の各テンプレートを選択できる「Braze テンプレート」セクション。()
...
]({% image_buster/assets/img_archive/dnd_iam_multipage.gif %})ユーザーがアプリ内メッセージの2ページ目に移動するための1次的なアクションボタンを編集している。()
...
]({% image_buster /assets/img/identifier_for_reporting.png %})値が「0」の [レポート用の識別子] フィールド。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img_archive/dnd_iam_save_as_template.png %})製品ツアーのアプリ内メッセージのプレビュー。()
