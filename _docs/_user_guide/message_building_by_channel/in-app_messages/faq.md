...
You can customize this interval through the SDK. Braze does not recommend setting it lower than 10 seconds in production apps to avoid overwhelming users. In test environments, you can temporarily set it to a lower value (such as five seconds) to verify that your messages are triggering as expected.
