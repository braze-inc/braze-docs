...
]({% image_buster /assets/img_archive/iam_compose.png %})新しい顧客を歓迎し、ユーザープロファイルを設定するように促す、サンプルブランドのアプリ内メッセージ。{: style="max-width:85%" }()
...
]({% image_buster /assets/img/ai_copywriter/ai_copywriter_iam.png %})アプリ内メッセージ作成画面のメッセージフィールドにある [AI コピーライター] ボタンをクリックします。{: style="max-width:60%"}()
...
]({% image_buster /assets/img/primary-secondary-buttons.png %})アプリ内メッセージのプライマリボタンとセカンダリボタン()
...
]({% image_buster /assets/img_archive/in_app_schedule.png %})トリガーアクションが「セッション開始」に設定されたアクションベースのキャンペーン。{: style="max-width:80%"}()
...
]({% image_buster /assets/img_archive/bucket_prioritization.png %})アプリ内メッセージキャンペーンとキャンバスの優先順位の設定例。{: style="max-width:70%"}()
...
]({% image_buster /assets/img_archive/re-evaluate-iam-membership.png %})[表示する前にキャンペーンの適格性を再評価する] チェックボックスが選択されている。{:style="max-width:60%"}()
