...
{% alert tip %}
Interested in expanding your opt-out processing? Try [fuzzy opt-out]({{site.baseurl}}/user_guide/message_building_by_channel/sms/keywords/fuzzy_opt_out/), a feature that attempts to recognise when a inbound message does not match a standard opt-out keyword, but indicates opt-out intent.
{% endalert %}
