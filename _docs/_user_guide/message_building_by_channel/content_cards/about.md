...
This section provides an overview of the three most common ways to place Content Cards within your app or website:

- [Message inbox](#message-inbox)
- [Carousel](#carousel)
- [Banner](#banner)

The logic and implementation of these placements are not a default in Braze, so your engineering team must supply and support the work for achieving these use cases. For an overview on how to implement these placements, refer to the [creating custom Content Card article]({{site.baseurl}}/developer_guide/customization_guides/content_cards/creating_custom_content_cards).
...
When a user is eligible for a card, it will automatically appear in their inbox. Content Cards are inherently built to be viewed in bulk, so users can view all cards that they’re eligible for at once.
