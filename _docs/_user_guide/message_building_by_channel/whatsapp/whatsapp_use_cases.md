...
![A Facebook ad from Calorie Rocket that advertises free delivery, and the respective WhatsApp conversation that occurs when a user selects the ad's button.]()
...
![Ads Manager with a composer to create an engagment ad.]()
...
![Ads Manager template composer with a pre-filled message of "I want free delivery".]()
...
![Entry schedule for an action-based Braze Canvas, with the trigger event of "Send a WhatsApp inbound message" and a message body that matches regex of "free delivery".]()
...
![WhatsApp message composer with button replies of "Yes" and "No Thanks".]()

![Canvas step with an "Opting in" group with a trigger event of "Sent inbound WhatsApp to subscription group" and a trigger word of "YES".]()
...
![User Update Canvas step that uses the advanced JSON editor to update the user profile.]()

![Canvas showing the workflow for sending ads that click to WhatsApp, including three action paths: Opting in, Opting Out, and Everyone Else.]()
