...
3. **Edit template**<br>
Next, create your message template. <br><br>You can provide a text or media header, the text body, a message footer, and buttons. Note that video and document headers are not currently available, and headers must be of either text or image type. Any media you add serves as an example for the review process and **are not** included in the template message. Media needs to be added in Braze. A preview of your message will display in a panel. <br><br>While Meta does not support Liquid, you can template in variables that can be later replaced in Braze for Liquid variables. Select the **+ Add variable** button to do so.<br><br>![Template composer.]({% image_buster /assets/img/whatsapp/whatsapp4.png %}){: style="max-width:100%;"}
