...
]({% image_buster /assets/img/whatsapp/whatsapp27.png %})アクションベースのキャンペーンスケジューリングオプション。()

{% endtab %}
{% tab Canvas %}

]({% image_buster /assets/img/whatsapp/whatsapp25.png %})アクションベースのキャンバススケジューリングオプション。()
...
]({% image_buster /assets/img/whatsapp/whatsapp11.png %})アクションボタンの呼び出しを示す電話画面は、クリックされたボタンのテキストに応答します。{: style="float:right;max-width:25%;margin-left:15px;border: 0;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp13.png %})テキストと3 つのアクションボタンの呼び出しを示すWhatsApp メッセージ。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp12.png %})WhatsApp メッセージテンプレートマネージャUI では、CTA ボタンの作成方法を示し、ボタンタイプ(カスタム) とボタンテキストを指定します。{: style="max-width:80%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp14.png %})入力されたクイック返信テンプレートが表示されている WhatsApp ステップメッセージ作成画面。()

メッセージステップに続くアクションステップを作成する。このアクションステップでは、クイック返信オプションごとに1つのグループを作成する。

]({% image_buster /assets/img/whatsapp/whatsapp15.png %})評価アクションが「WhatsApp インバウンドメッセージを送信」であるキャンバス。()

各クイック返信オプション・グループに対して、マッチさせるボタンとして正確なテキストを指定する。キーワードは大文字でなければなりません。 

]({% image_buster /assets/img/whatsapp/whatsapp16.png %})アクション"send a whatsapp inbound message"が、特定のメッセージ本文が受信されたときに送信するように設定されているキャンバスステップ。()
...
]({% image_buster /assets/img/whatsapp/whatsapp18.png %})それぞれのコールトゥアクションボタンに対するレスポンスを示すキャンバス。()
...
]({% image_buster /assets/img/whatsapp/list_message_example.png %})異なるファッションスタイルの行を持つWhatsApp リストメッセージの例。{: style="max-width:40%;"}()
...
]({% image_buster /assets/img/whatsapp/list_message_option.png %})[リストメッセージ] をはじめ、作成できる WhatsApp 応答メッセージのタイプを示す選択可能なcollection。{: style="max-width:70%;"}()
...
]({% image_buster /assets/img/whatsapp/list_content.png %})2つのセクション、複数の行と行の説明が入力されている [リストのコンテンツ] セクション。{: style="max-width:60%;"}()

セクションと行の順序を変更するには、名前の横にあるアイコンを選択してドラッグします。

]({% image_buster /assets/img/whatsapp/drag_list_order.png %})リストセクションを新しい場所にドラッグします。{: style="max-width:60%;"}()
...
]({% image_buster /assets/img/whatsapp/action_path_list_message.png %})異なる衣服スタイルのグループを持つアクションパスのコンポーザー。()
...
]({% image_buster /assets/img/whatsapp/regex_list_message.png %})[次に部分一致（正規表現も使用可)] フィルターを使用して「ankle boots」を含む応答メッセージをキャプチャする WhatsApp トリガー。()
...
]({% image_buster /assets/img/whatsapp/whatsapp_inbound_message_trigger.png %})[WhatsApp インバウンドメッセージを送信] トリガーが設定されたアクションベースのエントリステップ。()
...
`ecommerce.cart_updated`.]({%[カスタムイベントを実行] トリガーが設定されたアクションパスimage_buster。/assets/img/whatsapp/ecommerce_cart_updated.png()
