...
{% alert tip %}
WhatsAppチャンネルに参加するためのURLやQRコードは、[WhatsAppマネージャー](https://business.facebook.com/wa/manage/phone-numbers/)内の「**電話番号**」>「**メッセージリンク**」から作成できる。<br>]({% image_buster /assets/img/whatsapp/whatsapp115.png %})WhatsApp QR コード作成画面。{: style="max-width:55%;"}()
{% endalert %}
...
]({% image_buster /assets/img/whatsapp/whatsapp116.png %})WhatsApp インバウンドメッセージを送信したユーザーをエントリーするアクションベースのキャンバスエントリーステップ。{: style="max-width:85%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp117.png %})メッセージ本文が「STOP」または「NO MESSAGE」である WhatsApp 受信メッセージを送信するメッセージステップ{: style="max-width:85%;"}()
...
1. マーケティングオプトアウト」クイック返信でメッセージテンプレートを作成する。<br>]({% image_buster /assets/img/whatsapp/whatsapp121.png %})"Marketing opt-out"のフッターオプションを持つメッセージテンプレート。()<br><br>]({% image_buster /assets/img/whatsapp/whatsapp122.png %})マーケティングオプトアウトボタンを設定するセクション。()<br><br>
...
]({% image_buster /assets/img/whatsapp/whatsapp_inbound_message.png %})WhatsApp メッセージステップ。メッセージ本文は「START」である。{: style="max-width:80%;"}()

{: start="2"}
2\.キャンバスで、[**ユーザーの更新を設定**] ステップを作成し、[**アクション**] で [**高度な JSON エディター**] を選択します。<br><br>]({% image_buster /assets/img/whatsapp/user_update.png %})アクションが「高度なJSONエディター」であるユーザー更新ステップ。()<br><br>
...
{: start="4"}
4\.後続のWhatsAppメッセージステップを追加する。<br><br>]({% image_buster /assets/img/whatsapp/message_step.png %})キャンバスの「ユーザの更新」ステップ{: style="max-width:25%;"}()
...
]({% image_buster /assets/img/whatsapp/whatsapp_inbound_message.png %})メッセージ本文が「START」であるWhatsAppメッセージステップ{: style="max-width:85%;"}()

{: start="2"}
2\.キャンペーンまたはキャンバスで、Webhook メッセージステップを作成し、[**リクエスト本文**] を [**生のテキスト**] に変更します。

]({% image_buster /assets/img/whatsapp/webhook_step.png %})Webhook のメッセージステップ。{: style="max-width:85%;"}()
...
]({% image_buster /assets/img/whatsapp/campaigns_webhook_url.png %})[Webhook を作成] セクションの下の[Webhook URL] フィールド。{: style="max-width:70%;"}()
