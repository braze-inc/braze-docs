...
| **Creation** | Standard editor with customization via key-value pairs | Drag-and-drop editor for easy content building |
| **Persistence** | Up to 30 days before expiring | Unlimited (campaign window-based) |
| **Customization** | Highly flexible with custom integrations using key-value pairs | Custom HTML/CSS/JavaScript available in editor |
| **Placements** | Custom placement logic via key-value pairs | Native placement support with prioritization |
