...
### Why aren't my Liquid-generated URLs being tracked by link aliasing?

If you're using Liquid to generate or assign URLs (such as from a catalog, Connected Content, or `assign` statement), link aliasing requires a question mark (`?`) appended to the Liquid tag in the `href` attribute. Without it, Braze can't append the `lid` (link identifier) parameter, so the URL won't be recognized for link aliasing, link templates, or click tracking.

For example, if your Content Block or catalog sets a URL like {% raw %}`{{items[0].cta_url}}`{% endraw %}, use the following pattern in your anchor tag:

{% raw %}
```html
<a href="{{items[0].cta_url}}?">Shop Now</a>
```
{% endraw %}

If the Liquid URL already contains query parameters (a `?`), use `&` instead:
...
For more details, refer to [Link aliasing for URLs generated by Liquid]({{site.baseurl}}/user_guide/message_building_by_channel/email/templates/link_aliasing/#link-aliasing-for-urls-generated-by-liquid).
