...
]({% image_buster /assets/img/dnd_compose_error.png %})サンプルメールのエラーと警告の一覧。{: style="float:right;max-width:40%;margin-left:15px;"}()
...
]({% image_buster /assets/img_archive/newEmailTest.png %})テスト用に送信されるサンプルメール プレビュー。()
...
]({% image_buster /assets/img/email_templates/template5.png %})メールテンプレートの下部にあるAPI識別子。()
...
]({% image_buster /assets/img/email_templates/template_comment.png %})「Looks good to me」と書かれたメールテンプレートコメント。()
