...
For URLs that are generated by Liquid, such as `assign` statements in the HTML or from a Content Block, you must add a question mark (`?`) to the the liquid tag. This allows Braze to append query parameters (`lid = somevalue`) so that link aliasing can work properly. Without identifying where to append query parameters, link aliasing will not recognize these URLs and link templates won't apply.
