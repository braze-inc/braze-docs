...
You can create action-based messages targeting any link (tracked or not tracked) or retarget users based on whether they clicked an alias across any email campaign or Canvas component. Note that this is only possible when link alias tracking is turned on.
...
For links that are included in a Content Block, you must append a `?` or `&` to the link in the Content Block, then save it.
...
### Conditional Liquid statement

If you're using a conditional Liquid statement to display one of multiple links, we recommend assigning the URL in an HTML block, then templating that value instead. For example:

{% raw %}
```liquid
{% if {{custom_attribute.${last_pro_app_active}}} != blank or {{custom_attribute.${signup_source_product}}} contains 'Pro' or {{custom_attribute.${last_spot_trade_transaction_ts}}} != blank %}
{% assign url = 'https://test.com/abc/verify' %}
{% else %}
{% assign url = 'https://test.com/abc/account' %}
{% endif %}
```
{% endraw %}
...
- You must manually remove the link aliases from any existing messages that were created.
- If you have link templates, you'll need to re-add these if you update an existing message or campaign.
- Click reporting may look inaccurate for messages that were launched while the feature was on.
- Existing segments that reference link aliases may break.
- All new messages and Content Blocks that are created won't have the `lid` query parameter.
