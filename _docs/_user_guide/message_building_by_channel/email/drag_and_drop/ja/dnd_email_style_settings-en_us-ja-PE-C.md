...
]({% image_buster /assets/img_archive/dnd_global_style_settings.png %})[ドラッグ＆ドロップメールエディターの設定] タブの [メールグローバルスタイルの設定] セクション()
...
]({% image_buster /assets/img_archive/dnd_basic_styling.png %})メールとコンテンツのバックグラウンドカラー、デフォルトのフォント名、デフォルトのリンクカラーを編集するオプションを含む、基本的なスタイリングオプション。() 
...
]({% image_buster /assets/img_archive/dnd_fallbacks.png %})フォールバックフォントとして「Arial」を使用し、フォントファミリとして「Sans-serif」を使用する例。()
...
]({% image_buster /assets/img_archive/dnd_title_styling.png %})中央揃えのメインヘッダーとセカンダリーヘッダーのタイトルスタイル設定()
...
]({% image_buster /assets/img_archive/dnd_paragraph_styling.png %})14ptフォントのテキストの段落スタイル設定。()
...
]({% image_buster /assets/img_archive/dnd_list_styling.png %})箇条書きリストのリストのスタイル設定()
...
]({% image_buster /assets/img_archive/dnd_button_styling.png %})青色の背景の長方形ボタンのスタイル設定。()
...
]({% image_buster /assets/img_archive/dnd_email_template_width.png %})メールテンプレートを幅が600pxに設定。()

## コンテンツブロック幅

メールドラッグアンドドロップエディタでコンテンツブロック幅を設定することもできます。コンテンツブロックの幅をメール テンプレートの幅に合わせることをお勧めします。

]({% image_buster /assets/img_archive/dnd_content_block_width.png %})コンテンツブロックの幅を600pxに設定。()
