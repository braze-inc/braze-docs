...
After building your Gmail Promotion card, select **Generate HTML code**. Copy and paste the script into the `<head>` element of your email's HTML. The code generated from the HTML Editor can also be copied and used in the Drag-and-Drop Editor by pasting the code into the HTML block.
