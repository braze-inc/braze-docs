...
]({% image_buster /assets/img_archive/custom_footer.png %})カスタムフッターの例。()
...
]({% image_buster /assets/img_archive/email_unsub_protocol.png %})カスタムフッターに必要なプロトコルとURLの値。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img_archive/no_unsub_link_warning.png %})フッターなしで作成された電子メールの例。()

キャンペーンコンポーザーの警告は次のとおりです。

]({% image_buster /assets/img_archive/no_footer_test.png %})フッターなしで作成されたキャンペーン。()
...
]({% image_buster /assets/img_archive/custom_footer_save_changes.png %})[カスタムプレーンテキストフッターを設定] オプションを選択したメール。{: style="max-width:70%" }()
