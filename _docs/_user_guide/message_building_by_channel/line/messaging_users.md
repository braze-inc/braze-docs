...
You may receive the `request_timeout` error if it takes more than two seconds for LINE to receive a response from Canvas. However, the request itself may have reached Braze without any problems.