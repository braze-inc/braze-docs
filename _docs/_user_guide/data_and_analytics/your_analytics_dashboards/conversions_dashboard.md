...
4. Decide if you want your report to include untracked events (note that this feature is currently ONLY available for conversions that are custom events; it is not available for "started session" or "made any purchase"). Untracked events are events that were not originally assigned as conversion events for the campaigns and Canvases in your report. If unselected, then your report will only include conversion metrics for events that were originally included as conversion events. For example: let's say you are running your report on a particular campaign A (see the next section, on Filtering your report, for information on how to filter reports to a specific campaign). You're interested in seeing the conversion rate for users who received campaign A and performed custom event X, but custom event X was not one of the four conversion events tracked for this campaign. If you do NOT opt to include untracked events, then your report will show 0 conversions. However, if you do include untracked events, then Braze will generate a report of users who performed custom event X after receiving/opening/clicking campaign A.
...
6. Customize your report with filters - learn more about filters in the next section of this article. 
7. Click **Refresh**.
