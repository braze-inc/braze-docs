...
- **`assign`** is for simple variables that store a single value, such as a boolean, number, or simple string. Use `assign` for static values that aren't expected to change. You can also apply a single filter in the same line.
