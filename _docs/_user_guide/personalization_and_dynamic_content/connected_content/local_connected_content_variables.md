...
By default, Connected Content will set a Content-Type and Accept header on a GET HTTP request that it makes to "application/json". If you require another content type, specify it explicitly by adding `:content_type your/content-type` to the tag. Braze will then set both the Content-Type and Accept header to the type you specify.
