...
{% alert important %}
プロモーションコードは、キャンバスのアプリ内メッセージでは送信できません。[早期アクセスに](#promotion-codes-iam-campaigns)参加している場合は、アプリ内メッセージキャンペーンでプロモーションコードを送ることができる。
{% endalert %}
...
]({% image_buster /assets/img/promocodes/promocode1.png %})プロモーションコードを作成するボタン。{: style="float:right;max-width:30%;margin-left:15px;"}()
...
]({% image_buster /assets/img/promocodes/promocode3.png %})コードスニペット「spring25」が示されている「SpringSale2025」という名前のプロモーションコードリスト。{: style="max-width:80%"}()
...
]({% image_buster /assets/img/promocodes/promocode4.png %})残りのすべてのコードが2025年4 月30 日の午前12 時に期限切れになる期限設定をリストします。{: style="max-width:80%"}()
...
abc.com"marketing@]({% image_buster /assets/img/promocodes/promocode5.png %})" を通知するしきい値アラートの例。プロモーションコードリストの有効期限が5 日以内の場合。{: style="max-width:80%"}()
...
]({% image_buster /assets/img/promocodes/promocode7.png %})"springsale" という名前のCSV ファイル。正常にアップロードされました。()
...
]({% image_buster /assets/img/promocodes/promocode8.png %})アップロード中のプロモーションコード。()
...
]({% image_buster /assets/img/promocodes/promocode9.png %})メッセージに貼り付けるスニペットをコピーするオプション。{: style="max-width:50%"}()
...
]({% image_buster /assets/img/promocodes/promocode10.png %})「Treat yourself to something nice this spring with our exclusive offer」とその後にコードスニペットが続くメッセージの例。{: style="max-width:50%"}()
...
後続のメッセージで同じプロモーションコードを参照するには、コードをカスタム属性としてユーザープロファイルに保存する必要があります。これは、メッセージステップの前に直接、割引コードを "Promo Code "のようなカスタム属性に割り当てる[ユーザー更新ステップによって]({{site.baseurl}}/user_guide/engagement_tools/canvas/canvas_components/user_update/)行うことができる。
...
]({% image_buster /assets/img/promocodes/promocode11.png %})未使用コードがあるプロモーションコードの例。()

このコード数は、既存のプロモーションコード一覧ページを再訪した際にも確認できる。未使用のコードをCSV ファイルとしてエクスポートすることもできます。 

]({% image_buster /assets/img/promocodes/promocode12.png %})"Black Friday Sale"という名前のプロモーションコード。残りのコードが992 個あります。{: style="max-width:50%"}()
