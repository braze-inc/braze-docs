...
Note that message archiving captures the message payload itself, but does not include delivery metadata such as device tokens, priority settings, time-to-live (TTL), collapse IDs, APNs headers, expiration timestamps, and other delivery configuration fields that are sent to FCM or APNs. These fields act as delivery instructions to the push provider and are typically not considered part of the message content.
