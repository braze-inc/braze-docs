...
{% alert note %}
This event does not include a `user_id` field and is considered an **anonymous user event**. To receive this event, be sure to select **Send Anonymous Users** when [configuring your Currents integration]({{site.baseurl}}/user_guide/data/distribution/braze_currents/setting_up_currents/#requirements).
{% endalert %}
