...
]({% image_buster /assets/img/message_engagement_event.png %})メールの購読解除イベントを示すメッセージエンゲージメントの内訳。ユーザー固有のプロパティ、キャンペーンまたはキャンバスの追跡プロパティ、およびイベント固有のプロパティで、プロパティをグループ化してリストしています()
...
このイベントは、Braze バックエンドがライブアクティビティに関するリクエストをプロバイダーに対して行ったときに発生します。
