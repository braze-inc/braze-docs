...
- Whenever a push token enters Braze, we'll record its lifecycle events. There are three types of token change events ("add", "update", and "remove") recorded in the `push_token_state_change_type` field. Note the following details:
  - If we're inserting a new token that never existed before, this ingests one "add" event.
  - If we're updating the token with the same token string on the same user (gateway or `foreground_push_disabled` or other "secondary" fields changed), this will ingest one "update" event on the same token.
