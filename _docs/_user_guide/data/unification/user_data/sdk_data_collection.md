...
| Language                | Android, iOS, Web | Device or browser language, taken from the device locale. **Once the language has been set manually (via SDK method, REST API, or CSV upload), the SDK will no longer automatically update this value.** To resume automatic updates, set the value to `null`                                                            | This attribute is used to translate messages to a user’s preferred language. It's based on the device locale.                                                                                            |
