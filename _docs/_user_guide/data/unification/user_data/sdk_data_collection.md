...
| Country | Country identified by IP address geolocation. If the IP address geolocation isn't available, this is identified by the [device locale](#optional-data-collected-by-default). The value could alternatively be whatever the SDKs set directly with `setCountry`, but note that passing an attribute value through by SDK or API will log data points. **Once the country has been set manually (via SDK method, REST API, or CSV upload), the SDK will no longer automatically update this value.**| This attribute is used to target messages based on location. |
...
| Language                | Android, iOS, Web | Device or browser language, taken from the device locale                                                            | This attribute is used to translate messages to a user’s preferred language. It's based on the device locale. <br><br>Setting `language` on a user profile through the [REST API]({{site.baseurl}}/api/endpoints/user_data/post_user_track/) prevents Braze from automatically capturing this information through the SDK going forward.                                                                                            |
