...
{% alert important %}
Not all data is merged from the anonymous profile. Push tokens and messaging history are carried over, but **custom attributes, custom events, and purchase history from the anonymous profile are not merged** into the identified user. See [merge behavior]({{site.baseurl}}/api/endpoints/user_data/post_users_merge/#merge_updates-behavior) for the full list of fields that are and aren't transferred.
{% endalert %}
