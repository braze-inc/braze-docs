...
If you used [file validation](#file-validation), start with the error report as it includes the specific issue for each flagged row and a description of how to fix it. For rows that failed during import rather than validation, download the error report on the **Import Users** page.
