...
Add an `UPDATED_AT` timestamp to your source table. This timestamp indicates the time that this row was updated or added to the table. Braze syncs any rows where `UPDATED_AT` is equal to or later than the last synced `UPDATED_AT` value. This comparison uses an inclusive boundary.
