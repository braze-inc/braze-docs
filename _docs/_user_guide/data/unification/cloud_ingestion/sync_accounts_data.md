...
- Each sync imports rows where `UPDATED_AT` is later than the last synced timestamp. Rows at the exact boundary timestamp may be re-synced if new rows share that same timestamp. For more information, refer to [Avoid resyncing rows with duplicate timestamps]({{site.baseurl}}/user_guide/data/cloud_ingestion/best_practices/#avoid-resyncing-rows-with-duplicate-timestamps).
