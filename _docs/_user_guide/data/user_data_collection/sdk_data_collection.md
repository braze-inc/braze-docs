...
| Country | Country identified by IP address geolocation. If the IP address geolocation isn't available, this is identified by the [device locale](#optional-data-collected-by-default). | This attribute is used to target messages based on location. |
...
| Language                | Android, iOS, Web | Device or browser language, taken from the device locale                                                            | This attribute is used to translate messages to a user's preferred language.                                                                                            |
