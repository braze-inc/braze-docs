...
Any user without a designated `external_id` is called an [anonymous user]({{site.baseurl}}/user_guide/data/user_data_collection/anonymous_users/). For example, these could be users who visited your website but didn't sign up, or users who downloaded your mobile app but didn't create a profile.
