...
Any changes to the catalog schema (for example, adding new fields or changing field type) must be made through the catalog dashboard or via the Catalog Fields API before ingesting data through CDI. The schema will not be autogenerated from your first sync—if a field isn't defined in the schema, your sync will fail with a missing-field error. We recommend making these updates when the sync is paused or not scheduled to run to avoid conflicts between your data warehouse data and the schema in Braze.
...
- Any fields in your payload that aren't part of the catalog schema will be dropped, and if you omit a field during an update sync, that field's value will be set to null in the catalog.
- The sync leverages the [Asynchronous Catalog Items API]({{site.baseurl}}/api/endpoints/catalogs/catalog_items/asynchronous/put_update_catalog_items/) under the hood to ingest catalog data.
...
File Storage (S3) integrations also support catalog syncs. The setup is identical to data warehouse integrations—choose **File Storage** as your source when creating a new CDI sync, then supply your S3 bucket, SQS URL, and credentials. Your payload file must include a column named `ITEMS` (or `items`) containing a JSON array of catalog-item objects. For example:

```csv
ITEMS
"[{\"id\":\"123\",\"price\":9.99,\"deleted\":false,\"updated_at\":\"2025-05-01T00:00:00Z\"}]"
```

The file-storage integration will create or update items in the target catalog based on the `id` value in each object.
