...
- Arrays of objects have no limit on the number of items but do have a maximum size of 100&nbsp;KB. If an update (such as `$add` or `$update`) would cause the array to exceed this limit, Braze drops the update and the attribute is unchanged. The API request still returns a success response. To bring the array back under the limit, use `$remove` to delete items from the array.
