...
No. Forcing a data type change only affects **new data** coming into Braze. Any data ingested before the type change will continue to be stored as the old type and may not be segmentable with the new type's filters. A warning will appear on the affected users' profiles.
...
- **Timestamps stored as strings**: Make sure your date columns use a timestamp or datetime type in your source database, not a varchar/string.
