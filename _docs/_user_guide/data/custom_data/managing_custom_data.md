...
Deleting doesn't prevent additional recording of the custom data objects on user profiles, so make sure the custom data is no longer being recorded before deleting the event or attribute. Deleting doesn't consume any [data points]({{site.baseurl}}/user_guide/data/data_points) and also doesn't generate new data points to use.
