...
]({% image_buster /assets/img/query_builder_example.png %})例を示すクエリビルダーの [変数] タブ。()
...
]({% image_buster /assets/img_archive/sql_variables_campaigns.png %})Brazeでレンダリングしたラジオボタンの例。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img_archive/sql_variables_productname.png %})Brazeでレンダリングされた複数選択リストの例。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img_archive/query_builder_time_range.png %})Brazeでレンダリングしたカレンダーの例。{: style="max-width:50%;"}()
