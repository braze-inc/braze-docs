...
| Exception or exit event | The user was previously eligible to receive the message, but either {::nomarkdown}<ul><li> performed an [exception event]({{site.baseurl}}/user_guide/engagement_tools/campaigns/building_campaigns/delivery_types/triggered_delivery/#step-3-select-exception-events) for an action-based campaign so the message was aborted, or </li><li> met the Canvas [exit criteria]({{site.baseurl}}/user_guide/engagement_tools/canvas/create_a_canvas/exit_criteria/#setting-up-exit-criteria) so they were dropped mid-journey.</li></ul>{:/} |
