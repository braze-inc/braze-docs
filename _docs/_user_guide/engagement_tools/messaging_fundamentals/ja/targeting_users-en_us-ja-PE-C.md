...
]({% image_buster /assets/img_archive/additional_filters.png %})1 日以内にアプリを最後に開いたユーザ、キャンペーンまたはキャンバスステップを受信したことがなく、30 日未満前に購入したユーザをターゲットとするメッセージの追加フィルタ。{: style="max-width:90%;"}()
{% endtab %}
...
]({% image_buster /assets/img_archive/target_segmenter.png %})2 つのセグメントが含まれ、最後の購入の追加フィルタが30 日以内に作成されたメッセージのターゲットオプション。{: style="max-width:90%;"}()
{% endtab %}
...
]({% image_buster /assets/img_archive/has_app_hello_world.png %})「Hello, World (Android)」アプリを所有しているユーザーのフィルター。{: style="max-width:60%;"}()
...
]({% image_buster /assets/img_archive/target_users_by_segment_example.png %})サンプルセグメントが選択されている「ターゲットオーディエンス」ステップ()
...
]({% image_buster /assets/img_archive/user_lookup.png %})[ユーザーをルックアップ検索] ボタンがある [ユーザールックアップ検索] セクション。{: style="max-width:70%"}()
...
]({% image_buster /assets/img_archive/audience_summary.png %})、最大ユーザー上限または配信速度のレート制限を設定するオプションがある [オーディエンスの概要] セクション。()
...
]({% image_buster /assets/img_archive/multi_channel_footer.png %})各ターゲットチャネルの到達可能なユーザーの推定数を示す [合計人数] セクション。()
...
- **ターゲットオーディエンス**過去7日間に10ドル以上を費やしたユーザー
- **エントリイベント:**任意の購入
