...
| Braze カタログ | 製品ブロック設定で使用される以下のフィールドを含むBraze カタログを作成する必要があります。{::nomarkdown}<ul><li>product_title</li><li>product_url</li><li>variant_image_url</li></ul></code>{:/} |
...
]({% image_buster /assets/img/product_blocks/dynamic_fields.png %})カタログデータとイベントデータ{: style="max-width:50%;"}()に分割された動的製品ブロックの製品フィールド
...
]({% image_buster /assets/img/product_blocks/static_fields.png %})静的製品ブロックの製品項目。これらはすべてカタログデータとして分類されます。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img/product_blocks/layout_options.png %})製品の向き、配置、行あたりの最大製品数、製品間隔のレイアウトオプション。{: style="max-width:50%;"}()
...
]({% image_buster /assets/img/ecommerce/sample_product_catalog.png %})他に加えて必要なフィールドを持つサンプルCSV ファイル。()
...
]({% image_buster /assets/img/product_blocks/product_block.png %})製品ブロックなどのエディターブロックがある [コンテンツ] タブ。{: style="max-width:40%;"}()
...
]({% image_buster /assets/img/product_blocks/liquid.png %})商品価格の前にドル側が付加された商品ブロック。{: style="max-width:45%;"}()
...
]({% image_buster /assets/img/product_blocks/preview_as_a_user.png %})4つの項目を表示することを指定する [ダイナミック製品ブロック] ドロップダウンセクションがある [ユーザーとしてプレビュー] タブ。{: style="max-width:40%;"}()
...
]({% image_buster /assets/img/product_blocks/static_block_preview.png %})複数の項目タイルを含む製品生成ブロックを示すメールドラッグ＆ドロップ作成画面。()
