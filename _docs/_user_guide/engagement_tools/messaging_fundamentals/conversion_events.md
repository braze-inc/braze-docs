...
The primary conversion event count is the number of users who performed the primary conversion action. Braze calculates the primary conversion event rate by dividing this count by the number of unique recipients. Braze considers a user a recipient once the message is sent or shown, depending on the channel. For example, in push or email, a user becomes a recipient after Braze sends the message. For in-app messages or Content Cards, the user must view the message to be considered a recipient.
