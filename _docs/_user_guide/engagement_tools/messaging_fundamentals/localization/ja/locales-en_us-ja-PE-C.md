...
]({% image_buster /assets/img/multi-language_support/html_iam_editor_translation_tags.png %})翻訳 ID が表示されている従来のエディター。{: style="max-width:60%;"}()

{: start="3"}
3\.タグを追加したら、メッセージを下書きとして保存する。
4\.**言語の管理**を選択し、ドロップダウンを使用してメッセージのロケールを追加します。

]({% image_buster /assets/img/multi-language_support/manage_languages_modal.png %})1つのロケールが選択されている [言語を管理] モーダル。()

{: start="5"}
5\.[**テンプレートをダウンロード**] を選択して、翻訳テンプレートを CSV ファイルとしてダウンロードします。次に、CSVファイルに翻訳を記入する。

]({% image_buster /assets/img/multi-language_support/translation_csv_example.png %})翻訳 CSV ファイルの例。()
...
]({% image_buster /assets/img/multi-language_support/dnd_iam_editor_translation_tags.png %})2つの翻訳 ID が表示されているドラッグ＆ドロップエディター。{: style="max-width:70%;"}()
...
]({% image_buster /assets/img/multi-language_support/dnd_iam_download_template.png %})テンプレートをダウンロードするボタンがある [多言語] パネル。{: style="max-width:40%;"}()

{: start="6"}
6. CSVファイルに翻訳を記入する。ステップ1から翻訳タグを直接コピー＆ペーストした場合は、CSVファイルの**翻訳タグ**欄から`<code>` 。
7. **Upload translationsを**選択し、翻訳が完了したCSVファイルをアップロードする。

]({% image_buster /assets/img/multi-language_support/dnd_iam_upload_translations.png %})テンプレートをダウンロードするボタンと翻訳をアップロードするボタンがある [多言語] パネル。{: style="max-width:40%;"}()
...
4. [**テンプレートをダウンロード**] を選択して、翻訳テンプレートを CSV ファイルとしてダウンロードします。次に、CSVファイルに翻訳を記入する。<br>]({% image_buster /assets/img/multi-language_support/translation_csv_example.png %})翻訳 CSV ファイルの例。()
...
1. 翻訳タグ{% raw %}`{% translation %}` と`{% endtranslation %}`{% endraw %} を追加して、翻訳するすべてのテキストと画像、リンクURLをラップする。 
2. 各翻訳タグに ID タグを追加します。例を挙げよう： {% raw %}`{% translation id_1 %}`{% endraw %}<br>]({% image_buster /assets/img/multi-language_support/dnd_editor_translation_example.png %})2つの翻訳 ID が表示されているドラッグ＆ドロップエディター。()
3. タグを追加したら、メッセージを下書きとして保存する。
...
]({% image_buster /assets/img/multi-language_support/push_translation_tags.png %})プッシュ通知作成画面のタイトルフィールドとメッセージフィールドに翻訳タグが追加されている。()
...
]({% image_buster /assets/img/multi-language_support/upload_translation.png %})[多言語メッセージ] ウィンドウで2つのロケールが選択され、テンプレートをダウンロードするボタンと、翻訳をアップロードするボタンが表示されている。()
