...
You can also [create a webhook]({{site.baseurl}}/user_guide/message_building_by_channel/webhooks/braze_to_braze_webhooks/#setting-up-your-braze-to-braze-webhook) on the user receiving the step to log this as a custom attribute, which can be used to filter out users who have received the step during their Canvas journey.
