...
]({% image_buster /assets/img/canvas_template_example.png %})「Use for annual spring promotion」という説明が付いた「Annual sale Canvas template」という名前のキャンバステンプレートの例。()
...
]({% image_buster /assets/img/braze_canvas_templates.png %})「キャンバステンプレート」セクションの「Braze テンプレート」に、6 種類の使用可能なテンプレートが表示されている。()
