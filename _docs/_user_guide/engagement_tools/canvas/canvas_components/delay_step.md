...
{% tabs %}
  ...
  The **After a duration** option allows you to delay users for a set number of seconds, minutes, hours, days or weeks, and at a specific time. For example, you can delay users for four hours or for one day. Note the difference between how "days" and "calendar days" are calculated. A "day" is 24 hours and calculated from the time the user enters the Delay step. A "calendar day" means that a day is the period from when the user eenters the Delay step to midnight in their time zone.
