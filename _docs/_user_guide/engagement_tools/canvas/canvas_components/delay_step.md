...
{% tabs %}
  ...
  Note the difference between how "days" and "calendar days" are calculated. 
    ...
    - A "calendar day" means that a day is the period from when a time is specified, you can choose to delay at company time or at a user's local time. If a time is not specified, the user will be delayed until midnight the next day in company time.
