...
]({% image_buster /assets/img/experiment_step/experiment_winning_settings.png %})実験期間が 12 時間でコンバージョンイベント [クリック数] が選択されている勝者パス設定。()
...
]({% image_buster /assets/img/experiment_step/experiment_winning_statistical.png %})テスト結果が統計的に有意ではない場合にユーザーに対する動作として [すべての将来のユーザーにパスの組み合わせの送信を続行] が選択されている。()
...
]({% image_buster /assets/img/experiment_step/experiment_one_time.png %})勝者パスの遅延グループを使用した実験ステップ{: style="max-width:75%"}()

テストが完了して勝者パスが決まると、遅延グループに割り当てられたユーザーは選択されたパスに誘導され、キャンバスを進みます。

]({% image_buster /assets/img/experiment_step/experiment_one_time_results.png %})勝者パスに送られる遅延グループのある実験ステップ{: style="max-width:75%"}()
