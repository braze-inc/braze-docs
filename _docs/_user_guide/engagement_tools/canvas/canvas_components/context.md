...
In this Context step, we'll use binary logic to set {% raw %}`{{custom_attribute.${Purchased Flight}}}`{% endraw %} to `true` to determine if the type of flight they've purchased is first class.
