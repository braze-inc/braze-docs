...
Segment your users with eCommerce filters by going to the **Ecommerce** section within the segmenter. The following filters are currently in BETA:

- **Last order placed (last 730 days):** Date of the user's last placed order, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day, and the maximum lookback window is the past 2 years.
- **Total orders count (last 730 days):** Net total count of a user's orders within the last 2 years, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). This count excludes any canceled orders, which must be tracked using the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order canceled. Users are evaluated for this filter once per day.
- **Total orders count:** Net total count of a user's orders across their lifetime, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). This count excludes any canceled orders, which must be tracked using the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order canceled. Users are evaluated for this filter in real time.
- **Total cancelled orders count (last 730 days):** Total count of orders a user cancelled within the last 2 years, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.
- **Customer lifetime value (last 730 days):** CLTV is calculated by the total revenue a customer is expected to generate over their purchasing history with your brand. It is calculated by looking at the past 730 days and taking the Average Order Value (AOV), multiplying it by the total number of orders placed, and factoring in the customer's active purchasing duration (the time span between their very first and their most recent order). This filter uses data tracked in [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events)s (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.
- **Total refund value (last 730 days):** Total value of refunds granted to user over the last 2 years, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order refunded (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.
- **Total refund value:** Total value of refunds granted to user across their lifetime, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order refunded (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter in realtime.
- **Total revenue (last 730 days):** Total revenue generated from a user's orders over the last 2 years, calculated based on subtracting the revenue associated with the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order refunded from the revenue associated with the eCommerce event for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.
- **Total revenue:** Total revenue generated from a user's orders across the user's lifetime, calculated based on subtracting the revenue associated with the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order refunded from the revenue associated with the eCommerce event for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter in realtime.
- **Average order value (last 730 days):** The average (mean) value of a user's orders over the last 2 years, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.

For more information on all available segment filters, refer to [Segmentation filters]({{site.baseurl}}/user_guide/engagement_tools/segments/segmentation_filters).
