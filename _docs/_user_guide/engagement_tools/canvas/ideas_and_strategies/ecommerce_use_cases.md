...
- **Total canceled orders count (last 730 days):** Total count of orders a user canceled within the last 2 years, based on the [ecommerce recommended event]({{site.baseurl}}/user_guide/data/activation/custom_data/recommended_events/ecommerce_events) for order placed (workspaces not tracking eCommerce events will not have data for this filter). Users are evaluated for this filter once per day.
