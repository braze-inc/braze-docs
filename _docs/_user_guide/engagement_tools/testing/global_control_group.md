...
If you'd like to see which users are in your Global Ccontrol Ggroup, you can export your Group's members via CSV or API. To run a CSV export, navigate to the global control group set up page under Global Message Settings. To export via API, use the [global control group API endpoint]({{site.baseurl}}/api/endpoints/export/user_data/post_users_global_control_group/).
