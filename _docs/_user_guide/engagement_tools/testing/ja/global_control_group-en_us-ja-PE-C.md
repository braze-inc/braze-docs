...
]({% image_buster /assets/img/control_group/control_group4.png %})オーディエンス設定を「全ユーザーの5パーセントをグローバルコントロールグループに割り当てる」に設定したグローバルコントロールグループ設定()
...
]({% image_buster /assets/img/control_group/control_group2.png %})グローバルメッセージング設定を変更しています」というタイトルのダイアログボックスが表示され、グローバルコントロールグループが無効になると、新規またはアクティブなキャンペーンやキャンバスから除外されなくなるという警告文が表示される{: style="max-width:60%" }()
...
]({% image_buster /assets/img/control_group/control_group1.png %})ユーザーのランダムバケット番号が6356であり、グローバルコントロールグループに所属していることを報告する [その他] セクション{: style="max-width:50%;"}()
