...
### グローバルコントロールグループの使用状況

ランダムバケット番号は、グローバルコントロールグループ（キャンペーンやキャンバスを受け取らないユーザーグループ）で使用される。Braze はランダムなバケット番号の複数の範囲をランダムに選択し、選択されたバケットのユーザーを含めます。バケット番号は、重み付けや最近割り当てられた番号を考慮することなく、ランダムに割り当てられる。 

{% alert note %}
ユーザーが削除され、再度作成された場合、そのユーザーは新しいユーザーとみなされるため、異なるランダムなバケット番号が割り当てられる。
{% endalert %}
...
]({% image_buster /assets/img_archive/random_buckets_filterexample.png %})「3000」以下のランダム バケット番号を絞り込むセグメントンフィルター。()
