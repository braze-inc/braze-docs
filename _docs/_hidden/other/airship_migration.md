...
It is required to [migrate tokens via API]({{site.baseurl}}/help/help_articles/push/push_token_migration/#migration-via-api). The documentation linked contains specific steps, as well as an example payload, but the overall process is as follows:
...
Given the scenario where user profiles and push tokens are stored in separate locations, we recommend importing push tokens anonymously and then a subsequent migration of your existing user profiles. It is not necessary to map them together since the Braze iOS SDK will handle the token resolution upon a successful integration. 

- We recommend to migrate users via API but if there is a need to import a static list of users, it can be done via CSV, however, push tokens cannot because there is no way to specify the "push_token" object in the CSV. To read more about CSV imports, check out our documentation [here]({{site.baseurl}}/user_guide/data_and_analytics/user_data_collection/user_import/#csv). 
