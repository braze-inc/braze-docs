...
- **Basic tracking** enables campaign-level click tracking. Static URLs will have a length of 20–21 characters, and personalized URLs will have a length of 25 characters.
...
Link shortening is also enabled for API-only messages through the [`/messages/send`]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/) endpoint. To also enable basic or advanced tracking, use the `link_shortening_enabled` or `user_click_tracking_enabled` [request parameters]({{site.baseurl}}/api/endpoints/messaging/send_messages/post_send_messages/#request-parameters).
