...
REST API | Users won’t be enrolled when their subscription status is set by `/users/track` or the `/v2/subscription/status/set` endpoints. Users can enrolled when subscription status is set via /subscription/status/set when the optional parameter `use_double_opt_in_logic` is passed as `true` (for example, [{"subscription_group_id" : "subscription_group_identifier", "subscription_state" : "subscribed", "use_double_opt_in_logic": true}]). 
