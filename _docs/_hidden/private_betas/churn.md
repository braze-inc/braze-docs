...
Once the Prediction model is built and done training, users that currently fit the definition of the Prediction Audience will be assigned a Churn Risk Score between 0 and 100 denoting how likely they are to Churn according to your definition. Updating the risk scores of the Prediction Audience can be done with a [frequency you choose](#step-4-choose-the-update-frequency-for-churn-predictions). This way, you can reach out to users who are at risk of Churning before they actually do and prevent it from happening in the first place.
...
{% alert note %}
There is a limit of 3 concurrently active Predictions.
{% endalert %}
...
{% alert note %}
During the beta of Predictive Churn, the Prediction Audience cannot exceed 5 million users.
{% endalert %}
...
Our measure of Production Quality is Lift Quality, which is the average percentage lift the Prediction provides across all possible messaging thresholds as shown by historical data. This approach measures how much better than random guessing the model is on the test set. With this measure, 0% means the model is no better than randomly guessing about who will churn, and 100% indicates perfect knowledge of future knowledge of who will churn.

{% alert note %}
Lift is a description of how much better a model identifies the desired users over random guessing. For more detail, see [here](https://towardsdatascience.com/the-lift-curve-unveiled-998851147871). If 20% of your users usually churn on average, and you pick a random subset of 20% of your users and label them as churned (whether they truly are or not), you’d expect to correctly identify only 20% of the actual churners. That's random guessing. If the model were to only do that well, the lift would be 1 for this case.

If the model, on the other hand, allowed you to identify 20% of the users as churned and correctly identify all the “true” churners, the lift would be 100% / 20% = 5. In reality, models are usually somewhere in between. If you chart this ratio for every proportion of the likeliest churners you could message, you get the Lift Curve. Lift Quality (and also Prediciton Quality) is a measure, as a percentage, of how far along the the way between random guessing and perfection the Prediction is at identifying churners (on historical data). For the original paper on Lift Quality see [here](https://dl.acm.org/doi/10.1145/380995.381018).
{% endalert %}
