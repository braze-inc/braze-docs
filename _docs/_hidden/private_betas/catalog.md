...
First, create your CSV file. The first column of the CSV file must contain a header of `id`, and each item's `id` must be unique. All other column names must be unique. Additionally, the following limitations apply to catalog CSV files:
