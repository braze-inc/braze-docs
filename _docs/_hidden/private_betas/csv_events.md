...
# Importing user data and CSV events
...
{% alert important %}
User import also supports recording and updating user custom events. Like with user attributes, you can import with an `external_id`, `braze_id` or with `user_alias_name` with `user_alias_label`. See [Importing custom events] for more details.
{% endalert %}
...
**Download:** [CSV Import Template][import_template]
...
**Download:** [CSV Alias Events Import Template]
...
{% alert important %}
Arrays and push tokens are not supported in user import. Especially for arrays, commas in your CSV file will be interpreted as a column separator, so any commas in values will cause errors parsing the file. <br>For uploading these kinds of values, use the `/users/track` endpoint or Cloud Data Ingestion.
{% endalert %}
...
## Importing custom events (early access)
...
To import your CSV file, go to the **User Import** page under the **Audiences** section. The upper box will contain importing directions and a selection button to choose an attributes or events upload. Next, select **Browse Files** and select your file of interest, then select **Start Upload**. Braze will upload your file and check the column headers as well as the data types of each column.

upload image here

In the lower text box, Recent Imports, there will be a table that lists up to twenty of your most recent imports, their file names, CSV type, number of lines in the file, number of lines successfully imported, total lines in each file, and the status of each import.
...
Once the upload is complete, you will see a modal with a preview of the contents of your file. All the information in this table is based on the values in the top few rows of your CSV file.
