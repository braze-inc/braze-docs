...
The Braze Segment Swift Plugin v2.1.0 adds support for nested custom attributes. This release also adds the key `subscription_group_state` for setting the subscribed or unsubscribed status when using Braze subscription groups in an Identify call.
