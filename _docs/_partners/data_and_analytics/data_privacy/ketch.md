...
2. Give your Braze connection an identifiable name, which is used to refer to this connection in API-based processes. Note that a code will also be created for that connection. This code should be unique across all connections.
...
1. To configure the marketing preference identity, go to the **Settings** page in Ketch, and click  **Identity space**. You must either create a new identity space or edit an existing identity space to assign that identity space as the marketing preference identity. Check that the Ketch tag deployed on the property properly captures that identity space.
