...
A full list of the events and event properties that can be exported from Braze to Amplitude can be found in the following sections. All events sent to Amplitude will include the user's `external_user_id` as the Amplitude user ID. Braze-specific event properties is sent under the `event_properties` key in the data sent to Amplitude.

{% alert important %}
To use this feature, your Amplitude user ID must match the Braze external ID.
{% endalert %}

Braze will only send event data for users who have their `external_user_id` set or anonymous users who have their `device_id` set. For the anonymous users, you must sync your Amplitude device ID with the Braze device ID in the SDK. For example:
...
All cohorts named and imported into Braze is prefixed with `[Amplitude]` and suffixed with their `cohort_id`. This means that a cohort named "TEST_COHORT" with the `cohort_id` "abcd1234" is titled `[Amplitude] TEST_COHORT: abcd1234` in Braze filters.
...
{% alert warning %}
Keep your Amplitude API Key up to date. If your connector's credentials expire, the connector will stop sending events. If this persists for more than **48 hours**, the connector's events are dropped, and data is permanently lost.
{% endalert %}

### Step 2: Create Braze Current

In Braze, navigate to **Currents > + Create Current > Create Amplitude Export**. Provide an integration name, contact email, Amplitude export API key, and Amplitude region in the listed fields. Next, select the events you want to track; a list of available events are provided. Lastly, click **Launch Current**
