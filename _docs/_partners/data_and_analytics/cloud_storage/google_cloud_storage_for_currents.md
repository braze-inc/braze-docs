...
| Currents | In order to export data back into Google Cloud Storage, you must have [Braze Currents]({{site.baseurl}}/user_guide/data_and_analytics/braze_currents/#access-currents) set up for your account. |
...
{% alert important %}
It's important to keep your credentials file up to date; if your connector's credentials expire, the connector will stop sending events. If this persists for more than **5 days**, the connector's events are dropped, and data is permanently lost.
{% endalert %}
...
Keep in mind that the organization and contents of any exported files are identical across AWS S3, Microsoft Azure, and Google Cloud Storage integrations.
...
- All dashboard reports and CSV reports is sent to the users email for download (no storage permissions required) and backed up on Data Storage.
