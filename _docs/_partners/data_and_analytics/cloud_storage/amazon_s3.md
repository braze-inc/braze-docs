...
4. You're prompted to create your bucket and select a region. The region you choose here is the region Braze uses—Braze does not have a region setting. The integration sends data to the bucket name you provide; AWS routes requests to the bucket's region automatically.

Braze does not let you choose or configure region in the dashboard. The region is fixed by where you create the bucket in the AWS console. If your connector tries to connect to a different region than you want (for example, `eu-west-1` instead of `eu-central-1`), create or use an S3 bucket in your desired region in AWS. There is nothing to change on the Braze side.
