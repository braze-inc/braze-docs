...
>
> The Braze and Seen integration allows you to send user data from Braze to Seen, dynamically generate personalized videos, and return video assets—such as a unique player URL and thumbnail—back into Braze for use in campaigns and Canvases.
...
- **Trigger node**: Defines how and when a Journey starts (for Braze integrations, this should use an `On Create` trigger)
...
Because Journey responses are configurable, it’s important that the output fields returned by Seen match the attributes expected by your Braze Data Transformation.
...
- **Webhook URL**:  
  ...
  You can find your Workspace ID in the Seen Platform settings.
...
- If everything works as intended, proceed to finish the webhook setup.
...
Each Journey is a configurable workflow composed of nodes, allowing you to control both video generation logic and the response payload.
...
If your use case requires additional attributes, you may include them in the response and map them in Braze.
...
6. Send a test payload to the provided endpoint. You can send this by sending data to Seen Platform to run your Journey, or send this yourself to Braze directly with [Postman](https://www.postman.com/) or another similar service.
