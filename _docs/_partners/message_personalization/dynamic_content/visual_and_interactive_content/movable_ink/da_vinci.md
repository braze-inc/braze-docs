...
| Braze REST API key | A Braze REST API key with `messages.send`, `sends.id.create`, and `campaigns.details` permissions is required. This can be created in the Braze dashboard from **Settings*** > **API Keys**. <br><br>Your Movable Ink account team provides more setup instructions directly. Refer to the [Integration](#integration) section.|
...
To get started with the integration, contact your Movable Ink account team for assistance. Movable Ink provides access and setup instructions accordingly. You must provide Movable Ink with a set of Braze API credentials to enable Da Vinci to send email deployments through Braze’s Messaging API.
