...
Set up an action-based campaign within Braze that sends messages based on a set location. For your trigger, use a custom event of `bluedot_entry` or `bluedot_exit` as shown in the following screenshot:

![An action-based campaign in the delivery step. Here, you have two scheduling options that sends the campaign if a user performs a custom `bluedot_entry` or `bluedot_exit` event.]({%image_buster /assets/img_archive/Campaign-Delivery-BD.png %}){: style="max-width:80%"}
