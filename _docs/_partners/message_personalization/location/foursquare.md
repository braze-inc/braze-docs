...
To integrate the two platforms, you must integrate the two SDKs and map the matching user fields. After integrating the Pilgrim SDK, you receive location events on the device or to a webhook. 
...
To send a campaign that sends messages based on a set location:
