...
{% alert important %}
Lob's Data Transformation template sends events using your [`/users/track` endpoint]({{site.baseurl}}/api/endpoints/user_data/post_user_track), which logs data points if your Braze pricing includes data points. We recommend setting a rate limit in your Lob webhook settings, so you can avoid over-logging data.
{% endalert %}
