...
To support back-in-stock notifications, follow the instructions [here](({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/back_in_stock_notifications#back-in-stock-notifications).

To support price drop notifications, follow the instructions [here](({{site.baseurl}}/user_guide/personalization_and_dynamic_content/catalogs/price_drop_notifications).

Note that with the Shopify integration, you will need to create a custom event that captures a customer’s subscription status to your Catalog for each use case. The custom event will require an event property that will map to either the [SKU or Shopify variant ID](({{site.baseurl}}/partners/message_orchestration/channel_extensions/ecommerce/shopify/shopify_features/shopify_catalogs#step-2-select-your-product-identifier)) that you have selected as part of your Shopify product sync. 
