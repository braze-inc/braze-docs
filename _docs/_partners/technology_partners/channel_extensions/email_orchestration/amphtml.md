...
Google Access Tokens | Access tokens authenticate the user and are supplied and checked by the email sender. These tokens must be cryptographically secure, as well as time- and scope-limited. They are included within the URL or the request with `amp-list`. <br> Learn more and see an example [here](https://developers.google.com/gmail/ampemail/authenticating-requests#access_tokens).
Google Proxy Assertion Tokens | Proxy Assertion Tokens are an extra level of security achieved with JWT-based tokens that ensure the request came from the Gmail proxy server, that the Gmail user received a valip AMP email, and that a user is requesting a URL within that valid AMP email. <br> Learn more about these tokens, how to set them up, and how to verify them [here](https://developers.google.com/gmail/ampemail/authenticating-requests#proxy_assertion_tokens).
