...
| Checkout token user alias | An alias that Braze creates when the user starts the checkout process. This token is created by using the Shopify checkout token. If Shop Pay is used as an accelerated checkout option, the `ecommerce.checkout_started` event is bypassed, preventing Braze from creating the Checkout token user alias. |
