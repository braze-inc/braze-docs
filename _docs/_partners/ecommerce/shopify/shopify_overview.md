...
| Checkout token user alias | An alias that Braze creates when the user starts the checkout process. This token is created by using the Shopify checkout token. If a user uses Shop Pay as an accelerated checkout option, the checkout started event is bypassed. This prevents Braze from adding the checkout token alias and reconciling the user's profile through the integration. |
