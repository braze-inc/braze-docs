...
Feature flags are refreshed at session start and when changing active users. Feature flags can also be manually refreshed using the SDK's [refresh method][refreshing]. Feature flag refreshes are rate limited to once every 5 minutes (subject to change).
