...
> FCM has a limit of 4 Notification IDs per app. If you use more than 4 Notification IDs, FCM does not make any guarantees as to which ones will be respected. Braze uses 2 of these by default for campaigns and feedback responses, so make sure to specify only up to 2 additional Notification IDs for Android messages.
