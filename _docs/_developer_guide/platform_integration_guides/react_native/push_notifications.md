...
Follow [the iOS integration instructions][2].

## Test Displaying Push Notifications
...
{% alert important %}
You cannot test push notification related app behavior on an iOS simulator because simulators do not support device tokens that are required to be able to send and receive a push notification.
{% endalert %}
...
3. Compose your test notification and head over to the **Test** tab. Add the same *user-id* as the test user and `Send Test`. You should receive the notification on your device shortly.
