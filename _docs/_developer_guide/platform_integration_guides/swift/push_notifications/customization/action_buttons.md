...
{% alert note %}
Defining custom notification categories on the Braze dashboard is only required for action buttons that have special actions, like deep linking into the application or opening a web URL. You do not need to define click behavior for action buttons that simply dismiss a notification.
{% endalert %}
