...
{% alert note %} The Braze SDK does not collect IDFA automatically. Apps may optionally pass IDFA to Braze by implementing Braze's `ABKIDFADelegate` protocol. Apps must obtain explicit end user opt-in to tracking through the App Tracking Transparency framework before passing IDFA to Braze. {% endalert %}
