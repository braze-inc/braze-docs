...
{% alert note %}
The Braze SDK does not collect IDFA automatically. Apps may optionally pass IDFA to Braze by implementing Braze's `ABKIDFADelegate` protocol. Apps must obtain explicit opt-in to tracking by the end user through the app tracking transparency framework before passing IDFA to Braze.
{% endalert %}
