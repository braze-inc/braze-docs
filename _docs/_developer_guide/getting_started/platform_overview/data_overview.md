...
Data in braze should always have a purpose.

At Braze, data means action. That’s why sending the right data&mdash;not all of the data&mdash;will set your brand up for success. Having only the data you need allows you to more easily identify and target the right people with the right messages. It’s hard enough to know what dozens of custom events, attributes, triggers, and what properties are, let alone hundreds or even thousands! Do yourself and your marketers a favor by simplifying and streamlining your data pipelines to only the required values.
...
<br>
![A speedometer going as fast as it can before hitting the red][2]
{% endtab %}
{% endtabs %}
...
These identifiers include `braze_id` (randomly generated by Braze), `external_id` (which you define), and any number of user aliases (alternate identifiers) that you choose to set.
...
[1]: {% image_buster /assets/img/getting_started/integration-engine-1.jpg %}
[2]: {% image_buster /assets/img/getting_started/integration-engine-2.jpg %}
[3]: {% image_buster /assets/img/getting_started/integration-engine-3.jpg %}
