...
> Learn about Identifier for Vendor (IDFV) collecting for the Swift Braze SDK. 

## Background

In previous versions of the Braze iOS SDK, the Identifier for Vendor (IDFV) field was automatically collected as the user's device ID. Beginning in Swift SDK v5.7.0, the IDFV field was optionally disabled, and instead, Braze would set a random UUID as the device ID. Starting in Swift SDK v7.0.0, the IDFV field will not be collected by default, and a UUID will be set as the device ID instead.
