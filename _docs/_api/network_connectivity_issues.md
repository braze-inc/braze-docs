...
To allowlist Braze API endpoints through your firewall, our CDN provides access to the list of assigned IP ranges via a JSON dump. For a list of Braze API IP ranges, refer to both [Fastly's public IP list][3] and [Cloudflare's public IP list][6].  Note that these IPs may change at any time.
