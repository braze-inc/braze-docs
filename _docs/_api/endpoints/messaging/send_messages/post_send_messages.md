...
|`campaign_id`| Optional* | String | See [campaign identifier]({{site.baseurl}}/api/identifier_types/#campaign-identifier/) for more information. <br><br>*Required if you wish to track campaign metrics (such as _Sends_, _Clicks_, or _Bounces_) on the Braze dashboard, or if you want to see events associated with this message in the user profile [message history tab]({{site.baseurl}}/user_guide/engagement_tools/segments/user_profiles/#engagement-tab). |
