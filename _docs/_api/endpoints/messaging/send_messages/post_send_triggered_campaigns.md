...
Additionally, a user's subscription sroup status can be updated via the inclusion of a `subscription_groups` parameter within the `attributes` object. Further details can be found in the [User Attributes Object specification]({{site.baseurl}}/api/objects_filters/user_attributes_object).
