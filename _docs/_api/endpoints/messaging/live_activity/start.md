...
| `dismissal_date` | Optional | Datetime <br>([ISO-8601](https://en.wikipedia.org/wiki/ISO_8601) string) | This parameter defines the time to remove the Live Activity from the user's UI.<br><br>This dismissal date will be respected after receiving a `messages/live_activity/update` request with `end_activity`. |
