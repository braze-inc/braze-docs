...
```json
{
  "canvas_id": (required, string) the campaign identifier,
...
| --------- | ---------| --------- | ----------- |
| `canvas_id`| Required | String | See [campaign identifier]({{site.baseurl}}/api/identifier_types/). |
| `schedule_id` | Required | String | The `schedule_id` to delete (obtained from the response to create schedule). |
