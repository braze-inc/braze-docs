...
{% alert important %}
**Export output format**: When an export is successful, the HTTP response contains a URL to download a compressed archive (ZIP or GZIP file), regardless of the amount of user data exported. If the export fails, you receive an email notification instead. Setting up cloud storage credentials (S3, Azure, or Google Cloud Storage) minimizes the chance of failures because data size is not an issue when using cloud storage.
{% endalert %}
