...
If you do not have S3 credentials provided, the response to the request provides an obfuscated but publicly available URL where a zip file containing all the user files can be downloaded. The URL will only become a valid location once the export is ready. We strongly suggest that customers who use this endpoint set up their own S3 credentials so that customers can better protect the security of their customer data by enforcing their own S3 bucket policies.
