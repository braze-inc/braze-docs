...
{% alert warning %}
Because API campaigns are typically transactional, all users are eligible for API campaigns, even those in your Global Control Group. The [one-click list-unsubscribe] header ({{site.baseurl}}/user_guide/administrative/app_settings/email_settings/#list-unsubscribe) is not added to these sends. If you'd like to add a one-click list-unsubscribe header to all API campaigns, contact your customer success manager.
{% endalert %}
