...
{% alert note %}
**Header type requirement**: You can use `header_variables` only with templates that have TEXT-type headers. For IMAGE headers, use `header_image_uri` instead. DOCUMENT, VIDEO, and other media header types are not supported via the messages/send API.
{% endalert %}
