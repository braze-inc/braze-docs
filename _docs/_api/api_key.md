...
Prior to April 2020, API keys would be included as a part of the API request body or within the request url as a parameter. Braze now has updated the way in which we read API keys. API keys are now set with the HTTP Authorization header, making your API keys more secure.

While the old way of passing API keys continues to work, after a period of time this will be permenatly removed so we urge users to create new more secure integrations using the header. 
