...
The [Sisu Data]({{site.baseurl}}/partners/data_and_analytics/business_intelligence/sisu_data/) and Braze integration allows you to understand across all campaigns or at a campaign level why metrics (for example, open rate, click-through rate, conversion rate, etc.) are changing and what drives the most optimal outcomes. Once these segments are identified, company users can materialize the outputs in their data warehouse or send them directly from Sisu to Braze to retarget and re-engage users.
