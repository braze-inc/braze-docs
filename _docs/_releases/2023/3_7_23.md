...
For more information, refer to [card creation]({{site.baseurl}}/user_guide/channels/content_cards/create_a_content_card/card_creation/).
...
This new article describes how to use [user lookup]({{site.baseurl}}/user_guide/audience/segments/creating_a_segment/) to search for a specific user directly from the composer to test if your filters and segments are set up correctly. This can also be helpful when troubleshooting a campaign or Canvas that isn’t sending as expected—for example, if users aren’t receiving a message when they should be.
...
This new article describes how to retire a custom data object by [blocklisting or deleting custom data]({{site.baseurl}}/user_guide/data/activation/custom_data/blocklist_custom_data/).
...
The [Sisu Data]({{site.baseurl}}/partners/) and Braze integration allows you to understand across all campaigns or at a campaign level why metrics (for example, open rate, click-through rate, conversion rate, etc.) are changing and what drives the most optimal outcomes. Once these segments are identified, company users can materialize the outputs in their data warehouse or send them directly from Sisu to Braze to retarget and re-engage users.
